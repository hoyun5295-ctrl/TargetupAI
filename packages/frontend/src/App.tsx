...
function App() {
  ...
  useEffect(() => {
    ...
    // ★ 2026-07-18 (Codex 2R): 인증 복원(loadFromStorage) 완료 전엔 판단 금지 — 복원 전 기본값(비로그인)을
    //   보고 슈퍼관리자·에이전트 저장 세션까지 Dashboard를 이중 선로딩하던 결함 차단.
    if (isLoading) return;
    const path = window.location.pathname;
    if (path !== '/' && path !== '/login') return;
    // ★ 2026-07-18 (Codex 2R): '/?mall_id=…'(카페24 공개 진입) 등 쿼리 동반 착지는 별도 동선 —
    //   실제 착지 자원과 경합하지 않도록 선로딩 제외.
    if (window.location.search) return;
    if (!isAuthenticated) {
      // ★ 2026-07-18 (Codex M1 재시도 라운드): 로그인 직후엔 navigate가 먼저라 pathname 가드에 걸려
      //   선로딩이 영영 안 돌던 결함 정정 — 폼 입력 중(대역폭 유휴)에 최다 착지(Dashboard)를 선로딩.
      //   super_admin·에이전트는 소수라 착지 청크는 기존처럼 라우트 렌더 시 로드(과다 선로딩 회피).
      prefetchPage(() => import('./pages/Dashboard'));
      return;
    }
    // 저장된 세션으로 '/'·'/login' 착지(새로고침·재방문) — 사용자 유형별 실제 착지 청크 선로딩.
