/**
 * ★ 전단AI 페이지 — 전단지 생성/관리/발행
 *
 * 라우트: /flyer
 * 권한: company_admin + company_user
 */
...
export default function FlyerPage() {
  ...
  const loadFlyers = useCallback(async () => {
    try {
      const res = await fetch('/api/flyer/flyers', {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editingFlyer ? `/api/flyer/flyers/${editingFlyer.id}` : '/api/flyer/flyers';
      ...
      if (res.ok) {
        setAlertModal({
          show: true,
          title: editingFlyer ? '수정 완료' : '생성 완료',
          message: editingFlyer ? '전단지가 수정되었습니다.' : '전단지가 생성되었습니다.',
          type: 'success'
        });
        setShowForm(false);
        resetForm();
  ...
  const handlePublish = async (flyerId: string) => {
    try {
      const res = await fetch(`/api/flyer/flyers/${flyerId}/publish`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (res.ok) {
  ...
  const handleDelete = async (flyerId: string) => {
    try {
      const res = await fetch(`/api/flyer/flyers/${flyerId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (res.ok) {
  ...
  const handleEdit = (flyer: Flyer) => {
    ...
    setStoreName(flyer.store_name || '');
    setPeriodStart(flyer.period_start || '');
    setPeriodEnd(flyer.period_end || '');
  ...
  const addCategory = (name?: string) => {
    setCategories([...categories, { name: name || '새 카테고리', items: [{ name: '', originalPrice: 0, salePrice: 0 }] }]);
  };
  ...
  const addItem = (catIdx: number) => {
    const updated = [...categories];
    updated[catIdx].items.push({ name: '', originalPrice: 0, salePrice: 0 });
    setCategories(updated);
  };
  ...
  const updateItem = (catIdx: number, itemIdx: number, field: keyof FlyerItem, value: string | number) => {
    const updated = [...categories];
    ...
    <div className="min-h-screen bg-gray-50">
                  ...
                  <div key={flyer.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className={`px-4 py-3 ${flyer.status === 'published' ? 'bg-green-50 border-b border-green-100' : 'bg-gray-50 border-b border-gray-100'}`}>
                      <div className="flex justify-between items-start">
                        ...
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${
                          flyer.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'
                        }`}>
                    ...
                    <div className="px-4 py-3">
                      {(flyer.period_start || flyer.period_end) && (
                        <p className="text-xs text-gray-500 mb-2">
                          📅 {formatDate(flyer.period_start)} ~ {formatDate(flyer.period_end)}
                        </p>
                      )}

                      <div className="flex items-center gap-4 text-sm">
                        <span className="text-gray-500">
                          템플릿: <span className="font-medium text-gray-700">{TEMPLATES.find(t => t.value === flyer.template)?.label || flyer.template}</span>
                        </span>
                        {flyer.status === 'published' && (
                          <span className="text-blue-600 font-medium">
                            👆 {flyer.click_count || 0}클릭
                          </span>
                        )}
                      </div>
                      ...
                      {flyer.short_code && (
                        <div className="mt-2 flex items-center gap-2">
                          <code className="text-xs bg-gray-100 px-2 py-1 rounded flex-1 truncate">
                            hanjul-flyer.kr/{flyer.short_code}
                          </code>
                          <button
                            onClick={() => handleCopyUrl(flyer.short_code!)}
                    ...
                    <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        ...
                        {flyer.status !== 'published' && (
                          <button onClick={() => handlePublish(flyer.id)} className="text-xs text-green-600 hover:text-green-800 font-medium">발행</button>
                        )}
                        {flyer.short_code && (
                          <a
                            href={`/api/flyer/p/${flyer.short_code}`}
                            target="_blank"
                        ...
                        <button
                          onClick={() => setDeleteModal({ show: true, flyerId: flyer.id, flyerTitle: flyer.title })}
        ...
        {showForm && (
          <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
                ...
                <div key={catIdx} className="mb-4 border border-gray-100 rounded-lg p-4">
                  <div className="flex justify-between items-center mb-3">
                    <input
                      ...
                      className="font-bold text-sm text-gray-800 border-b border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none pb-1"
                    />
                    <button
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-12 gap-2 text-xs text-gray-500 px-1">
                      ...
                      <div className="col-span-2">원가</div>
                      <div className="col-span-2">할인가</div>
                      <div className="col-span-3">뱃지</div>
                    ...
                    {cat.items.map((item, itemIdx) => (
                      <div key={itemIdx} className="grid grid-cols-12 gap-2 items-center">
                        <input
                          ...
                          placeholder="상품명"
                          className="col-span-4 px-2 py-1.5 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <input
                          type="number"
                          value={item.originalPrice || ''}
                          onChange={e => updateItem(catIdx, itemIdx, 'originalPrice', Number(e.target.value))}
                          placeholder="원가"
                          className="col-span-2 px-2 py-1.5 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <input
                          type="number"
                          value={item.salePrice || ''}
                          onChange={e => updateItem(catIdx, itemIdx, 'salePrice', Number(e.target.value))}
                          placeholder="할인가"
                          className="col-span-2 px-2 py-1.5 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <input
                          type="text"
                          value={item.badge || ''}
                          onChange={e => updateItem(catIdx, itemIdx, 'badge', e.target.value)}
                          placeholder="뱃지"
                          className="col-span-3 px-2 py-1.5 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <button
                    ...
                    <button
                      ...
                      className="w-full py-1.5 text-xs text-blue-600 hover:bg-blue-50 rounded border border-dashed border-blue-300 transition-colors"
                    >
                      + 상품 추가
                    </button>
                  </div>
              ...
              <button
                onClick={() => addCategory()}
                className="w-full py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg border border-dashed border-gray-300 transition-colors"
              >
                + 카테고리 추가
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={handleSave}
                className="px-6 py-2.5 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors font-medium"
              >
                {editingFlyer ? '수정 저장' : '전단지 저장'}
              </button>
            </div>
          </div>
        )}
      ...
      {alertModal.show && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 animate-in zoom-in-95">
      ...
      {/* ── 복사 완료 토스트 ── */}
      {copyModal && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-lg text-sm shadow-lg z-50 animate-in slide-in-from-bottom">
          URL이 복사되었습니다
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 animate-in zoom-in-95">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-gray-500 mt-1">
                "<span className="font-medium">{deleteModal.flyerTitle}</span>"을(를) 삭제하시겠습니까?<br/>
                발행된 단축URL도 함께 삭제됩니다.
              </p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteModal({ show: false, flyerId: '', flyerTitle: '' })}
                className="flex-1 py-2.5 border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors text-sm"
              >
                취소
              </button>
              <button
                onClick={() => handleDelete(deleteModal.flyerId)}
