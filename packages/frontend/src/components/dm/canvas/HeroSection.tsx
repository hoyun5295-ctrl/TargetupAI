...
export default function HeroSection({ props }: { props: HeroProps }) {
  ...
  return (
    <div className="dm-section dm-hero" style={{ position: 'relative', minHeight: height, overflow: 'hidden', background: 'var(--dm-neutral-900)' }}>
      ...
      >
        {props.headline && <div className="dm-text-hero" style={{ fontWeight: 800 }}>{props.headline}</div>}
        {props.sub_copy && <div className="dm-text-body" style={{ marginTop: 'var(--dm-sp-3)', opacity: 0.9 }}>{props.sub_copy}</div>}
      </div>
    </div>
  );
}
