  ...
  if (t === 'typographic') {
    return (
      <div className="dm-section dm-hero" style={{ background: 'var(--dm-bg)', padding: 'calc(var(--dm-sp-12) * var(--dm-section-pad-scale)) var(--dm-sp-5)', display: 'flex', flexDirection: 'column', gap: 'var(--dm-sp-4)' }}>
        {headNode({ fontSize: 'var(--dm-fs-hero)', fontWeight: FW_HERO, letterSpacing: 'var(--dm-ls-hero)', lineHeight: 1.12, fontFamily: 'var(--dm-font-display)', color: 'var(--dm-neutral-900)' })}
  ...
  if (t === 'full_bleed') {
    ...
    return (
      <div className="dm-section dm-hero" style={{ position: 'relative', minHeight: h, overflow: 'hidden', background: baseBg }}>
        {imgSrc && <img src={imgSrc} alt={props.headline || ''} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />}
        {imgSrc && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.32)' }} />}
        <div style={{ position: 'relative', minHeight: h, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: 'var(--dm-sp-6) var(--dm-sp-5)', color: '#fff' }}>
          {headNode({ fontSize: 'var(--dm-fs-hero)', fontWeight: FW_HERO, letterSpacing: 'var(--dm-ls-hero)', lineHeight: 1.1, fontFamily: 'var(--dm-font-display)' })}
    ...
    return (
      <div className="dm-section dm-hero" style={{ background: 'var(--dm-bg)' }}>
        <div style={{ background: 'var(--dm-primary)', color: '#fff', padding: 'calc(var(--dm-sp-8) * var(--dm-section-pad-scale)) var(--dm-sp-5)' }}>
          {headNode({ fontSize: 'var(--dm-fs-hero)', fontWeight: FW_HERO, letterSpacing: 'var(--dm-ls-hero)', lineHeight: 1.12, fontFamily: 'var(--dm-font-display)' })}
