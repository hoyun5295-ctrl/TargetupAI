/**
 * DiagnosisReportView — 진단 리포트 (2026-08-16 신설 · 설계서 §5-4·§5-5)
 *
 * "리포트가 본체, 설문은 문진"(Harold 원칙) — 전 블록이 답변·실적에서 파생된 서버 스냅샷(result)만 그린다.
 * 프론트는 요금제를 해석하지 않는다(§2-1) — outcome·추천·수치 전부 서버 값 표시 전용.
 *
 * 구성 = 1 히어로(outcome별) + 근거 1 효과(Source caption 의무) + 근거 2 추천 요금제 + 업종 예시 목업.
 * 목업 = /diagnosis-examples/{industry}-{dm|email|inapp}.html — sandbox iframe(JS 0)·클릭 확대만·캡션 의무.
 */
