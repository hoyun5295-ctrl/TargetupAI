...
export default function DirectTargetFilterModal({ show, onClose, onExtracted }: DirectTargetFilterModalProps) {
  ...
  // 동적 필터 → API 포맷
  // ★ 2026-09-12 규칙 본체를 프론트 CT(`utils/customerFilterBuild`)로 옮겼다 — 모바일DM 타겟 추출의
  //   직접 선택 탭이 같은 규칙으로 조건을 만들어야 두 화면의 인원수가 갈라지지 않는다.
  //   (본체는 옮기기 전과 글자 단위로 같다. 규칙을 고칠 일이 생기면 CT만 고친다.)
