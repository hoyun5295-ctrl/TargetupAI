...
async function main() {
  ...
  // 27행: 발송 ID (★2026-09-05 §21-2 · Harold "양식에 발송ID 적는 란만 만들어주면 되는 거 아냐")
  //   ⛔ 1~26행이 업체 실물 복제 구간이고 여기부터가 우리 칸이다. 위로 끼워 넣지 마라(모양이 바뀐다).
  //   ⛔ 값 칸은 **비운다**. 여기에 "예: 금강제화" 같은 안내문을 넣으면 파서가 그 문자열을 값으로 읽어
  //      billing_target_not_found 반려가 난다(PLACEHOLDER_VALUES 목록이 늘어난다 · 위 규약).
  //   라벨 둘째 줄 괄호는 파서 labelKey가 떼므로 대조 키는 '발송ID' 하나다(6행 제목 칸과 같은 방식).
  ws.mergeCells('B27:B27');
  label('B27', '발송 ID\n(안내받은 계정 이름)');
  ...
  const guide = [
    ...
    ['발신번호', '두 가지 중 하나로 적습니다.\n1) 번호 직접: 등록된 발신번호를 적습니다. 예: 0507-0000-0000\n2) 고객리스트의 열 이름: 열 이름을 그대로 적으면 고객마다 그 매장 번호로 나갑니다. 번호가 여러 종류여도 접수는 하나이고, 문안 검사와 담당자 문자, 승인도 한 번입니다. 모든 번호는 발신번호로 미리 등록돼 있어야 합니다'],
    // ★2026-09-10 남지현 접수 원문 그대로(줄만 칸 폭에 맞게 나눔). 메일 접수는 확인 화면이 없어 AI 매칭을 안 한다(확정 경로 AI 0)
    ['문안 항목', '%열이름%을 메시지 내용에 넣으면 고객마다 고객리스트의 그 열 값으로 바뀝니다. 예: %고객명%님 → 김하나님\n항목은 4개까지, 메시지 내용에만 넣을 수 있습니다(제목에는 넣지 않습니다)\n화면 접수: 열 이름과 달라도 됩니다. 접수 확인 화면에서 AI가 맞는 열을 골라 두고, 직접 바꿀 수도 있습니다\n메일 접수: 메시지 내용과 열이름이 동일해야 접수 및 매핑이 가능합니다.\n예: 메시지 내용 - %이름%, %매장명% / 열이름 - 이름, 매장명 동일하게 기재.'],
    ['광고 표시', '대행발송 문자는 기본으로 광고로 처리되어 (광고) 표시와 무료 수신거부 번호가 자동으로 붙습니다. 광고가 아닌 안내 문자면 내용 시트 아무 빈 줄의 B칸에 "광고 여부", C칸에 "아니오"를 적어 주세요'],
    // ★2026-09-10 임은지 접수: 화면·메일 둘 다 서버가 규격에 맞춘다(utils/mms-image-fit.ts). 못 읽는 파일만 반려
    ['이미지 문자', '이미지는 이 파일에 넣지 않습니다.\n화면 접수: 확인 단계에서 "이미지 넣기"로 첨부합니다. 최대 3장까지 받습니다\n메일 접수: 요청서와 함께 이미지 파일을 별도로 첨부합니다. 최대 3장까지 받습니다\n큰 사진이나 JPG가 아닌 사진도 규격(JPG, 한 장 300KB 이하)에 맞게 자동으로 줄여 접수합니다\n읽을 수 없는 파일(아이폰 고효율 사진 HEIC 등)은 접수되지 않으니 JPG로 저장해 보내 주세요'],
    ['발송 ID', '한 이메일 주소로 여러 계정의 발송을 요청하는 경우에만 적습니다. 내용 시트 맨 아래 "발송 ID" 칸에 안내받은 계정 이름을 적으면 그 계정 명의로 접수되고 청구됩니다. 계정이 하나면 비워 두시면 그 계정으로 나갑니다\n메일 한 통에 요청서를 여러 개 보내실 때는 요청서마다 적어 주세요. 한 요청서는 한 계정으로만 나갑니다'],
    ['요청서 여러 개', '메일 한 통에 요청서 파일을 최대 5개까지 첨부할 수 있습니다. 파일마다 "내용" 시트와 "고객리스트" 시트가 모두 들어 있어야 하고(이 양식 그대로), 접수는 파일 수만큼 나뉘어 건마다 검사와 승인이 따로 진행됩니다\n이미지가 들어가는 요청서는 메일 한 통에 하나씩 보내 주세요. 요청서가 여러 개면 어느 요청서의 이미지인지 알 수 없어 접수되지 않습니다'],
