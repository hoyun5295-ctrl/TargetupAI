...
async function main() {
  ...
  st.value = '아래 파란 칸을 채워 주세요(수신자 열 이름은 선택입니다). 고객 명단은 별도 파일로 함께 올립니다.';
  ...
  const fields = [
    ...
    // ★2026-08-26 §18-4 신설(선택) — 파서 FIELD_ALIASES '수신자 열 이름'과 계약
    {
      ...
      help: '명단에서 받는 분 휴대폰 번호가 든 열의 이름(선택). 예: 휴대폰번호\n비워 두시면 번호 모양을 보고 자동으로 찾습니다.',
    },
  ];
  ...
  const guide = [
    ...
    ['고객 명단', '세 번째 시트(고객 명단 예시)의 모양처럼 첫 줄이 열 이름인 엑셀이나 CSV를 별도 파일로 올립니다. 휴대폰 번호 열은 자동으로 찾아 드리고, 확인 화면에서 바꿀 수도 있습니다.\n첫 줄부터 바로 데이터가 시작되는 파일(열 이름 없음)도 받습니다. 이때 열은 "열1, 열2…"로 불립니다'],
    ['수신자 열 이름', '명단에서 받는 분 휴대폰 번호가 든 열의 이름을 적으면 그 열로 확정됩니다(선택). 비워 두시면 번호 모양을 보고 자동으로 찾습니다.\n적은 이름이 명단에 없으면 접수되지 않으니 명단의 열 이름과 똑같이 적어 주세요'],
  ];
  let gr = 3;
