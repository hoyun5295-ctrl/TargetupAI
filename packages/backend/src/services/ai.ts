...
//   문안생성(BRAND_SYSTEM_PROMPT 290줄 카피라이팅 가이드) 격차 해소.
//   각 톤 = 실제 마케팅 시나리오 기반 + 시그니처 표현 + 특수문자 + in-context 예시 1개.
export type RefineTone =
  | 'friendly'  // 친근       — 카톡 친구톤
  | 'formal'    // 공식       — 격조 있는 안내
  | 'sale'      // 세일강타   — 할인/특가 폭발
  | 'urgent'    // 긴급마감   — 시급성/손실회피
  | 'vip'       // VIP케어    — 단골/회원 우대
  | 'warm'      // 감성따뜻   — 진심 어린 편지감
  | 'launch'    // 신상런칭   — 새로움/기대감
  | 'winback';  // 재방문유도 — 휴면 깨우기
...
interface ToneSpec {
  ...
  signature: string[];  // 톤별 시그니처 표현 3~4개
  chars: string;        // SMS 호환 특수문자 가이드
  scenario: string;     // 적합 사용 시나리오
  example: {            // in-context 1개 (선택 톤만 동적 박음 — input 토큰 절약)
    original: string;
    refined: string;
  };
}
...
const TONE_GUIDE: Record<RefineTone, ToneSpec> = {
  friendly: {
    label: '친근',
    concept: '카톡 친구가 살짝 추천해주는 느낌. 일상 단골에게 가볍게 알려주는 1:1 대화 톤. 부담 없는 친밀감.',
    signature: ['혹시 아세요?', '~좋아하실 것 같아', '먼저 알려드려요', '~챙겨드리려고요'],
    chars: '♥ / ★ 1~2개 자연스럽게',
    scenario: '일상 안내 / 신상 소식 / 가벼운 추천',
    example: {
      original: '내일 신상품 입고됩니다',
      refined: '내일! 기다리시던 신상이 입고되는데, 가장 먼저 알려드려요 ★',
    },
  },
  formal: {
    label: '공식',
    concept: '브랜드가 정중하게 안내하는 격조 있는 톤. 절제된 우아함과 신뢰감. 비즈니스/프리미엄 브랜드 톤.',
    signature: ['선보입니다', '안내 드립니다', '정성스럽게 준비했습니다', '함께해주셔서 감사합니다'],
    chars: '▶ / ※ 미니멈',
    scenario: '브랜드 공지 / 정중한 안내 / 격조 있는 마케팅',
    example: {
      original: '5월 이벤트 안내드립니다',
      refined: '5월 감사의 마음을 담은 이벤트를 정성스럽게 준비하여 안내 드립니다 ※',
    },
  },
  sale: {
    label: '세일강타',
    concept: '할인/특가가 핵심인 폭발 톤. 숫자 임팩트가 시선을 끄는 강한 도입부 + 손실회피 자극.',
    signature: ['★ 30% OFF', '단 X일 한정', '놓치면 아까운', '역대 최대 혜택'],
    chars: '★ / ▶ / ■ 강조',
    scenario: '할인/세일 이벤트 / 프로모션 강조 / 특가전',
    example: {
      original: '이번 주말 30% 할인 진행',
      refined: '★ 이번 주말, 놓칠 수 없는 30% 할인이 시작됩니다 ▶ 지금 바로 만나보세요',
    },
  },
  urgent: {
    label: '긴급마감',
    concept: '시급성과 손실회피를 자극하는 행동 유도 톤. "지금 안 하면" 느낌 + 구체적 CTA.',
    signature: ['지금 바로', '마감 D-X', '잔여 ~개', '오늘까지'],
    chars: '▶ / ●',
    scenario: '마감 임박 / 한정 수량 / 시급한 안내',
    example: {
      original: '쿠폰 사용 마감 임박',
      refined: '▶ 잊지 마세요, 쿠폰 사용이 곧 마감됩니다 — 지금 바로 챙기세요',
    },
  },
  vip: {
    label: 'VIP케어',
    concept: '단골/회원 우대 톤. 소속감과 독점감을 자극하는 1:1 케어 느낌. "당신만의 혜택" 강조.',
    signature: ['%이름%님만의', '%등급% 고객님이시니까', '먼저 알려드려요', '단골 고객님께'],
    chars: '◆ / ◇',
    scenario: '회원 우대 / 단골 마케팅 / VIP 이벤트',
    example: {
      original: '%이름%님 신상 할인 안내',
      refined: '◆ %이름%님, 단골 고객님께 먼저 알려드리는 신상 할인 — 오직 %이름%님만의 혜택입니다',
    },
  },
  warm: {
    label: '감성따뜻',
    concept: '진심이 느껴지는 편지 같은 감성 톤. 단골에게 마음 전하는 느낌. 차분하고 정성스럽게.',
    signature: ['소중한 분께', '마음을 담아', '기다리시던 분들께', '진심으로'],
    chars: '♥ / ♡ 1개',
    scenario: '감사 인사 / 시즌 인사 / 감성 마케팅',
    example: {
      original: '신상 입고 안내',
      refined: '기다리시던 분들께, 마음을 담아 새 시즌 신상을 소개해드립니다 ♥',
    },
  },
  launch: {
    label: '신상런칭',
    concept: '새로움과 기대감의 트렌드 톤. "드디어 출시" 첫 공개 느낌 + 한정성.',
    signature: ['드디어 출시', '올해 첫', '기다리시던 신상', '최초 공개'],
    chars: '★ / ◎',
    scenario: '신상품 출시 / 시즌 런칭 / 신메뉴',
    example: {
      original: '봄 신상 입고됩니다',
      refined: '★ 드디어, 올해 첫 봄 신상이 도착했습니다 — 기다리신 분들께 가장 먼저 ◎',
    },
  },
  winback: {
    label: '재방문유도',
    concept: '오랜만에 안부 전하는 잔잔한 톤. 추억 환기 + 따뜻한 부담 없는 초대. 휴면 고객 깨우기.',
    signature: ['오랜만이에요', '그동안 잘 지내셨어요?', '잊지 않고', '다시 만나뵙고 싶어요'],
    chars: '♡ (거의 없음)',
    scenario: '휴면 고객 깨우기 / 재구매 유도 / 시즌 안부',
    example: {
      original: '%이름%님 오랜만의 할인 안내',
      refined: '%이름%님, 오랜만이에요. 잊지 않고 인사드리며, 다시 만나뵙고 싶어 작은 혜택을 준비했어요 ♡',
    },
  },
};
...
### ${spec.label} 톤 다듬기 예시
원본: "${spec.example.original}"
다듬: "${spec.example.refined}"
(✓ 원본 사실 100% 보존 + ✓ ${spec.label} 톤 시그니처 자연 반영 + ✓ 시즌감/감각 표현 풍성)
