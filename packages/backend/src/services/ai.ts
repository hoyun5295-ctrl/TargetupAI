...
const TONE_GUIDE: Record<RefineTone, string> = {
  friendly: '편안하고 가까운 말투. 친구에게 알려주는 느낌. 수식어와 감성 표현 적극 활용해 단순 정리가 아니라 더 매력적인 카피로. SMS 호환 특수문자(♥/★/▶) 1~2개.',
  formal:   '정중하고 신뢰감 있는 말투. 비즈니스 톤. 정확한 정보 전달. 격조 있는 수식어("정성스럽게"/"선보입니다") 활용. SMS 호환 특수문자 없거나 ▶/※/☞ 정도.',
  urgent:   '주목 끄는 긴박한 말투. "지금"/"바로"/"이번 기회" 시급성 단어 + 강조 표현. SMS 호환 특수문자(▶/■/●) 활용. 단순 정리가 아니라 행동 유도.',
  warm:     '진심 어린 감성 말투. 따뜻한 표현. 고객을 배려하는 느낌. 감성 단어("소중한"/"특별한"/"마음을 담아") + 시즌 묘사 활용. SMS 호환 특수문자(♥/♡) 1개 정도.',
};
...
2. **🚨 절대 지어내기 금지 (사고 차단)**:
   ...
   - 예시: 원본 "내일 신상품 입고" → "매장에서 만나뵐게요" 추가 X (매장 약속 X).
   - 예시: 원본 "30% 할인" → "단 3일 한정" 추가 X (기간 정보 X).
   - 예시: 원본에 없는 "최저가/100% 보장/이벤트 매출 30% 증가" 같은 통계/사실 지어내기 X.
   - 풍성하게 ≠ 사실 창작. 표현/수식어/계절감만 자유, 사실은 보존.

3. **🚨 유니코드 이모지(😊⚡✨🤝🔥💚 픽토그램) 절대 사용 금지** — SMS/LMS EUC-KR 인코딩에서 깨지거나 물음표로 변환됨 = 발송 사고.
   - **SMS 호환 특수문자만 사용 가능**: ★ ☆ ♥ ♡ ◆ ◇ ■ □ ▲ △ ▶ ◀ ● ○ ◎ ♨ ※ ☞ ☎ ① ② ③ ↑ ↓ ← → ㈜ ㎝ ㎏ ㎡ — 이것들만 (Dashboard 특수문자 모달 리스트, EUC-KR 호환 검증됨).

4. **🌸 현재 시즌 컨텍스트 (자동 반영 권장)**:
   - 현재 월: ${monthLabel}
   - 시즌 키워드: ${seasonHint}
   - 원본에 시즌 정보 없어도 자연스러운 시즌 묘사 1~2개 자유롭게 추가 가능 (예: "감사의 달", "선선해진 가을", "한 해 마무리").
   - ⚠️ 시즌 묘사는 일반적/계절적 사실만 (5월=감사의 달은 OK). 구체적 사실/매출/통계 지어내기 X.
5. 변수 자리(%이름%, %등급%, %포인트%, %기타1%, %기타2%, %기타3%, %회신번호%, %매장명% 등):
   ...
   - 원본에 변수 없으면 결과에도 변수 추가하지 말고, 변수 없이 자연스럽게 처리.
6. (광고) 표기: 원본에 박혀있으면 결과 첫머리에 (광고) 그대로 유지. 원본에 없으면 박지 않음.
7. **무료수신거부 / 무료거부 / 수신거부 080-XXXX-XXXX 같은 표기는 절대 박지 마세요** — 한줄로 시스템이 자동 부착합니다. AI가 박으면 중복 발송 사고.
8. 톤 가이드: ${toneGuide}
9. KISA 스팸 차단 키워드(무료체험, 무이자, 100% 보장, 대출, 도박 등) 회피.
10. 사람이 직접 작성한 듯한 자연스러운 한국어. AI 티 나는 표현(예: "안녕하세요 고객님" 남발) 자제.
11. 가장 자연스럽고 효과적인 **단 하나의 안**만 제시. 여러 후보 나열 금지.

${brandLine}

## 모범 예시 (풍성한 다듬기 — 사실 보존 + 표현 풍성 + 시즌감 + SMS 호환만)

### 케이스 A — 변수 없는 원본 + 친근 톤 (계절감 자연 반영)
원본: "내일 신상품 입고됩니다!"
다듬: "내일! 따스해진 ${monthLabel}, 기다리시던 신상품을 만나보실 수 있습니다 ★"
(✓ 보존: "내일", "신상품 입고" 핵심 사실 + ✓ 풍성: "따스해진 5월", "기다리시던" 자연스러운 시즌/감성 표현 + 매력적 카피)
(✗ 절대 추가 X: 매장명/할인율/시간/장소 같은 원본에 없는 사실)

### 케이스 B — 변수 없는 원본 + 긴급 톤 (강조 + 행동 유도)
원본: "이번 주말 30% 할인 이벤트입니다"
다듬: "▶ 이번 주말, 놓칠 수 없는 30% 할인 이벤트가 진행됩니다"
(✓ 보존: "이번 주말", "30% 할인 이벤트" 핵심 사실 + ✓ 풍성: "놓칠 수 없는" 강조 표현으로 행동 유도)
(✗ 절대 추가 X: "단 3일 한정"/"전 매장"처럼 원본에 없는 기간/장소)

### 케이스 C — 변수 있는 원본 + 따뜻함 톤 (변수 보존 + 감성 표현)
원본: "%이름%님 안녕하세요. 내일 신상품 입고됩니다."
다듬: "%이름%님, 안녕하세요. 소중한 분께 먼저 알려드립니다 — 내일 신상품이 새롭게 입고됩니다 ♥"
(✓ 보존: %이름% + "내일 신상품 입고" + ✓ 풍성: "소중한 분께 먼저 알려드립니다" 진심 어린 감성 + "새롭게" 매력 표현)
(✗ 절대 추가 X: 새 변수/매장 약속/혜택)

### 케이스 D — 변수 + 광고 표기 + 공식 톤 (격조 있는 표현)
원본: "(광고) %이름%님 %매장명% 5월 이벤트 안내"
다듬: "(광고) %이름%님, %매장명%에서 ${monthLabel} 감사의 마음을 담은 이벤트를 정성스럽게 준비했습니다 ※"
(✓ 보존: (광고) + 모든 변수 + "5월 이벤트" + ✓ 풍성: "감사의 마음을 담은", "정성스럽게 준비했습니다" 격조 + 시즌감(감사의 달))
(✗ 절대 추가 X: 구체적 혜택/할인율/일시 — 원본에 없는 정보)

### 잘못된 예시 (절대 따라하지 마세요)
원본: "내일 30% 할인 이벤트"
잘못된 다듬 1: "내일 단 하루! 전 매장에서 30% 할인 이벤트가 시작됩니다 😊 매장에서 만나뵐게요"
(X "단 하루", "전 매장", "매장에서 만나뵐게요" — 모두 원본에 없는 사실/약속 창작)
(X 😊 이모지 — SMS EUC-KR 발송 시 깨짐)

잘못된 다듬 2: "내일 할인 진행"
(X 너무 짧음 — 원본 길이 50% 미만, 정보 손실. "30%" 핵심 숫자 누락)

올바른 다듬: "내일! 기다리시던 30% 할인 이벤트가 진행됩니다 ★"
(✓ 핵심 사실 100% 보존 + ✓ "기다리시던" 풍성한 표현 + ✓ SMS 호환 특수문자만)
