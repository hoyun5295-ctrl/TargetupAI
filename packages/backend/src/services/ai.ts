...
// ============================================================
/**
 * ★ 2026-08-21 사용자 노출 문안 공통 문장 부호 규칙 (Harold: 글자 중간의 줄표가 "AI 티" 1순위).
 *   AI 호출 단일 관문(callAIWithFallback)에서 모든 system 프롬프트 끝에 붙인다. 경로별로 따로 넣지 않는다
 *   (게이트는 효과가 만들어지는 함수 안에). 관문을 거치지 않는 직접 호출(refineDirectMessage·routes/analysis)은 withCopyRules로 명시 부착.
 *   규칙 본문에 줄표 문자를 쓰지 않는다(프롬프트 파일 줄표 0 계약 테스트 대상).
 */
export const COPY_PUNCTUATION_RULE = `

## 문장 부호 규칙 (모든 출력 공통)
- 문안·제목·설명·사유 어디에도 줄표(em dash, U+2014)를 쓰지 않는다.
- 나열은 "·", 부연은 ":" 또는 괄호, 문장이 둘이면 마침표로 나눈다.`;

/** 시스템 프롬프트에 공통 규칙을 붙인다(이미 붙어 있으면 중복 부착하지 않는다). */
export function withCopyRules(system: string): string {
  return system.includes(COPY_PUNCTUATION_RULE) ? system : system + COPY_PUNCTUATION_RULE;
}
...
### 계절감/시기 반영 (내용 풍성화 핵심)
- 오늘 날짜 정보가 주어집니다. 해당 월/계절 감성을 도입부·공감 문장에 적극 녹여 내용을 풍성하게 채우세요
- 봄(3~5월): 꽃, 새로운 시작, 따뜻함, 환절기 케어, 봄나들이
- 여름(6~8월): 시원함, 자외선, 바캉스, 가벼움, 수분
- 가을(9~11월): 풍성함, 선선함, 환절기, 추석, 단풍
- 겨울(12~2월): 따뜻함, 보습, 연말, 새해, 크리스마스, 설날
- 계절감은 제품/행사와 자연스럽게 연결하세요(맥락 없이 단어만 나열 X). 단순 안내로 끝내지 말고 계절·시의성으로 한 겹 더 풍성하게. 단, 없는 혜택·날짜는 지어내지 말 것
...
${smsByteInstruction}
${kakaoInstruction}
${channel === 'LMS' || channel === 'MMS' ? '- LMS/MMS는 단순 안내문이 아니라 도입부 공감 → 본문 전개 → 마무리/CTA까지 충분한 길이로 풍성하게 작성하세요(너무 짧고 가볍지 않게). 줄바꿈·문단으로 가독성 좋게, 이모지 금지!\n- ⚠️ subject(제목)에는 %변수% 절대 사용 금지! 고정 텍스트만!' : ''}
...
export async function refineDirectMessage(
  ...
  const systemPrompt = withCopyRules(await buildSystemPromptWithBrandVoice(companyId, baseSystemPrompt));  // ★ 2026-08-21 직접 호출 경로라 관문을 안 거친다: 명시 부착
