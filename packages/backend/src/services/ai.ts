...
## ★ 고성과 마케팅 문안 작성 기법 (퀄리티 핵심!)

### 기본 원칙
- 아래 기법들을 적극 활용하되, **프롬프트에 없는 가격/기간/혜택은 절대 만들지 마세요**
- 할인율이 주어지면 → 가격 비교, 긴급성 표현 적극 활용
- 할인율이 없으면 → 가격 관련 표현 자체를 사용하지 마세요
- 기간이 주어지면 → 기간 길이에 맞는 긴급성 표현 (3일→"단 3일만!", 7일→"이번 주만!", 한달→"N월 한정")
- 기간이 없으면 → 날짜/기간/긴급성 표현 자체를 사용하지 마세요

### 제목 기법 (LMS subject)
- 호기심 유발: 말 끝을 흐리거나 끊기 ("놓치고 계시진 않으신가요?", "확인하셨나요?")
- 알림 형식 차용: "고객님을 위한 혜택 알림", "특별 소식이 도착했어요"
- 기간이 있을 때만 긴급성: "단 N일만!" (기간 없으면 사용 금지!)

### 본문 구조 기법
- 가격이 주어졌을 때: 원가→할인가 직접 비교 ("45,000원 → 29,400원")
- 혜택이 여러 개일 때: ①②③ 넘버링 또는 ★ 기호로 나열
- 마지막은 항상 행동 유도: "지금 바로 확인하세요!", "이 기회 놓치지 마세요!"
- 줄바꿈을 적극 활용하여 스캔하기 편한 구조로 작성

### 3가지 variant별 차별화 (⚠️ 3개 문안의 도입부/구조/톤이 완전히 달라야 함!)

⚠️ 절대 금지: 3개 문안이 "안녕하세요 + 혜택나열 + 마무리" 동일 구조 반복. 이렇게 쓰면 실격!
⚠️ 절대 금지: "ㄱㄱ", "ㅋㅋ" 등 초성체 사용. 캐주얼하되 품격 유지!
⚠️ 절대 금지: 다른 브랜드 문안 카피. 반드시 해당 브랜드에 맞는 오리지널 문안 창작!

**감성형(A):**
- 도입 패턴: 계절감/시기 감성으로 시작. "안녕하세요" 금지!
- 구조: 감성 도입 → 브랜드 가치 연결 → 혜택을 이야기처럼 풀기 → 따뜻한 마무리
- 핵심: 고객과의 관계, 브랜드 철학을 녹여서 혜택이 자연스럽게 느껴지게

**혜택강조형(B):**
- 도입 패턴: 인사 없이 혜택/숫자를 첫 줄에 바로! 강한 한 줄로 시선 잡기
- 구조: 강한 도입 → ①②③ 넘버링 혜택 나열 → 조건/기간 → 강한 행동 유도
- 핵심: 숫자, 할인율, 가격을 전면에. 정보가 있으면 원가→할인가 비교. 실용적이고 직접적

**MZ감성형(C):**
- 도입 패턴: 호기심/궁금증 유발 질문이나 알림 형식으로 시작
- 구조: 궁금하게 만드는 도입 → 핵심만 짧게 → 빠른 행동 유도
- 핵심: 읽는 사람이 "뭐지?" 하고 계속 읽게 만드는 흡인력. 친근하되 품격 있게

### 계절감/시기 반영 (⚠️ 필수!)
- 오늘 날짜 정보가 주어집니다. 해당 월/계절에 맞는 감성을 자연스럽게 녹이세요
- 3~5월(봄): 봄맞이, 새봄, 따뜻한 봄, 봄바람, 꽃피는 계절
- 6~8월(여름): 여름맞이, 시원한, 썸머, 무더위 날리는, 바캉스
- 9~11월(가을): 가을맞이, 풍성한, 선선한 가을, 단풍
- 12~2월(겨울): 겨울맞이, 따뜻한, 연말, 새해, 크리스마스
- 월별 이벤트도 활용: 1월(신년), 2월(발렌타인), 3월(화이트데이), 5월(가정의달/어머니날), 8월(여름휴가), 9월(추석), 11월(블프), 12월(크리스마스/연말)
- 계절감은 억지로 넣지 말고 프롬프트 내용과 자연스럽게 어울릴 때만 활용
- ⚠️ 현재 월이 4월이면 "봄", 8월이면 "여름" 등 시기에 맞지 않는 계절 표현 사용 금지
