...
export const OUTREACH_DM_SECTION_CONTRACT = [
  ...
  `${SECTION_CONTRACT_COMMON.textCard}   (DM에서는 각 gallery 바로 앞에 1개씩 두고 headline·body 는 [배너 문구]와 홈페이지 본문에 있는 말만 · 그 밖의 자리에는 0~1개)`,
...
export const OUTREACH_EMAIL_SECTION_CONTRACT = [
  ...
  SECTION_CONTRACT_COMMON.header,
  SECTION_CONTRACT_COMMON.hero,
  `${SECTION_CONTRACT_COMMON.textCard}   (상품 묶음 앞에는 반드시 text_card 1개를 두고 tag·headline·body 세 칸을 모두 채워라 · 빈 칸 금지 · 각 gallery 바로 앞에도 text_card 1개 · 그 headline·body 는 [배너 문구]와 홈페이지 본문에 있는 말만)`,
  SECTION_CONTRACT_COMMON.carousel,
  SECTION_CONTRACT_COMMON.gallery,
  SECTION_CONTRACT_COMMON.coupon,
  SECTION_CONTRACT_COMMON.cta,
