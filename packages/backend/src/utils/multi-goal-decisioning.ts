...
export async function analyzeGoalConflicts(input: {
  ...
  const userMessage = `다음 ${goals.length}개 목표의 충돌을 분석하고 박을 순서를 박아주세요:

${goals.map((g, i) => `${i + 1}. "${g.name}" (가중치 ${g.weight}${g.description ? ` — ${g.description}` : ''})`).join('\n')}

각 목표에 대해 sub_plan을 박고, 다른 목표와의 충돌 영역(동일 고객 동시 발송 / 메시지 중복 / 시점 겹침 등)을 박아주세요.
응답은 반드시 valid JSON으로만 박아주세요 (다른 텍스트 X).`;
...
회사 정보:
- 회사: ${companyInfo.company_name || '미박힘'}
- 브랜드: ${companyInfo.brand_name || '미박힘'}
...
분석 원칙 (영구):
1. 각 목표에 대해 target_criteria + channel_recommended + timing_recommended + priority 박음
2. 다른 목표와의 충돌(동일 고객 동시 발송 / 메시지 중복 / 시점 겹침)을 conflicts 배열에 박음
3. 우선순위 + 가중치 기반으로 recommended_order 박음 (목표 이름 배열)
...
7. AI 단독 실행 X — 본 분석은 추천만 박음, 발송은 사용자 승인 후
  ...
  "sub_plans": [
    {
      ...
      "conflicts": ["휴면 회수 목표와 동일 고객 겹칠 수 있음 — 등급 우선 박음"],
      "priority": 1,
      "should_execute": true,
      "reasoning": "VIP 가중치 0.5로 최우선 박음"
    }
  ],
  "overall_strategy": "...",
  "conflict_matrix": "| 목표 A | 목표 B | 충돌 영역 |\\n|---|---|---|\\n| VIP 재구매 | 휴면 회수 | 등급 우선 박음 |",
...
function fallbackAnalysis(goals: OperatorGoal[]): MultiGoalAnalysis {
  ...
  return {
    ...
    subPlans: sorted.map((g, idx) => ({
      ...
      channelRecommended: 'SMS',
      timingRecommended: '오후 2~4시 (KST)',
      conflicts: idx > 0 ? [`${sorted[0].name}과 동일 고객 겹칠 가능성 — 우선순위 박음`] : [],
      priority: idx + 1,
      shouldExecute: true,
      reasoning: `가중치 ${g.weight} 기반 fallback 박음 (AI 분석 실패)`,
    })),
    overallStrategy: `${goals.length}개 목표를 가중치 기반으로 박음 (AI 충돌 분석 실패 — 표준 흐름 fallback).`,
    conflictMatrix: 'AI 분석 실패 — 충돌 매트릭스 박지 X. 사용자 직접 검토 박아주세요.',
