...
/**
 * 계정명을 붙이는 채널 — 요금제·추가 항목은 이 칸에 계정명을 쓰지 않는다.
 *
 * ⚠ **`send-usage-aggregation.ts`의 `USER_SHEET_CHANNELS`와 뜻이 다른 집합이다.** 저기는
 *   "어느 장에 실리나", 여기는 "구분 칸에 계정명을 쓰나"다. 추가 항목(`extra`)이 저기에만 있는
 *   이유가 그것이다 — 계정 앞으로 지정한 부가서비스도 구분 칸은 '추가 항목'이라야 읽힌다.
 *   ★ 2026-09-16 그전에는 이 둘이 **엇갈려 있었다** — 여기는 테스트·스팸에 계정명을 찍는데
 *   장 분할은 그 둘을 공통 장으로 보내, 계정을 아는 행이 공통 장에 앉아 있었다(서수란 접수).
 *   두 집합을 고칠 때는 언제나 나머지 하나를 함께 본다.
 */
