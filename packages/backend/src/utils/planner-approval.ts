/**
 * planner-approval.ts — 마케팅 플래너 월간 브리핑·결재 CT (★ 2026-08-13 Phase 2)
 *
 ...
 *   차감 멱등키 · 결재 링크 토큰 · 결재 통지 문구 · 제출/승인 전이.
...
import { countOperatorAudienceFor } from './operator-audience';
...
/** 차감 멱등키 — 회사·월 고정(설계서 §3-4). */
export function buildApprovalIdempotencyKey(companyId: string, planMonth: string): string {
  return `planner:${companyId}:${planMonth}`;
}
...
/**
 * 채널 축 대상 수 실쿼리 — 두 축만 셀 수 있다.
 *  - 문자·DM = 자동마케팅 대상 단일 문(countOperatorAudienceFor). 실행 엔진이 그것이라
 *    같은 게이트로 세야 "브리핑에 보여준 수 = 실제로 나가는 수"가 된다.
 *  - 이메일 = 이메일 채널 CT의 수신 가능 고객 수(같은 안전 필터).
 *  실패는 error 상태로 내린다 — 0으로 내리면 "대상 없음"으로 읽히고, 그냥 null이면 승인이 열린다.
 */
async function loadAudienceByChannel(companyId: string): Promise<Record<PlannerChannel, ChannelAudience>> {
  const [messaging, email] = await Promise.all([
    countOperatorAudienceFor({ companyId })
      .then((r) => r.count)
      .catch((e: any) => {
  ...
  const countable = (count: number | null, channel: PlannerChannel): ChannelAudience =>
    count == null
      ? { count: null, state: 'error', note: AUDIENCE_ERROR_NOTE }
      : { count, state: 'known', note: channelAudienceNote(channel) };
  return {
    sms: countable(messaging, 'sms'),
    dm: countable(messaging, 'dm'),
    email: countable(email, 'email'),
...
export async function loadMonthlyBrief(companyId: string, planMonth: string): Promise<MonthlyBrief> {
  const [evRes, audience, creditState, approvalRow, agencyPaid] = await Promise.all([
    query(
      `SELECT id, title, starts_on, ends_on, benefit_text, products, status
         FROM planner_events
        WHERE company_id = $1::uuid AND plan_month = $2 AND status <> 'cancelled'
        ORDER BY starts_on ASC, created_at ASC`,
      [companyId, planMonth],
    ),
    loadAudienceByChannel(companyId),
    getCreditState(companyId),
    loadApprovalRow(companyId, planMonth),
    hasAgencyDeduction(companyId, buildApprovalIdempotencyKey(companyId, planMonth)),
  ]);
  ...
  if (eventIds.length > 0) {
    ...
    const tp = await query(
      `SELECT id, event_id, channel, timing_rule, format, est_credits
  ...
  const events: BriefEvent[] = evRes.rows.map((r: any) => {
    const startsOn = String(r.starts_on).slice(0, 10);
    const endsOn = String(r.ends_on).slice(0, 10);
    const touchpoints: BriefTouchpoint[] = (tpByEvent.get(String(r.id)) || []).map((t: any) => {
...
// ── 통지 ─────────────────────────────────────────────────────────────
/** 플래너 통지 머리말 — 자동마케팅 문자와 섞이지 않게 축을 밝힌다. */
...
async function claimApproval(companyId: string, planMonth: string, attempt: string): Promise<{
  eventIds: string[]; agencyCredits: number; planHash: string;
} | null> {
  const r = await query(
    `UPDATE planner_monthly_approvals
      ...
      WHERE company_id = $1::uuid AND plan_month = $2
        AND (status = 'pending'
             OR (status = 'approving' AND updated_at < NOW() - INTERVAL '${APPROVE_LEASE_MINUTES} minutes'))
      RETURNING event_ids, agency_credits, plan_hash`,
...
export async function approveMonthlyBrief(
  ...
  const idempotencyKey = buildApprovalIdempotencyKey(companyId, planMonth);
