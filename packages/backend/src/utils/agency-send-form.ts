...
export function parseAgencyRequestForm(buffer: Buffer): ParsedAgencyForm {
  ...
  try {
    ...
    return {
      ...
      isAd: true, managerPhones: [], phoneColumnName: '',
  ...
  return { subject, content, requestedAtText, requestedAt, callbackRaw, isAd, managerPhones, phoneColumnName, errors };
}
...
export function looksLikeRequestForm(buffer: Buffer): boolean {
  try {
    ...
    return (wb.SheetNames || []).some((n) => normLabel(n) === '요청서');
  } catch {
    return false;
  }
}
