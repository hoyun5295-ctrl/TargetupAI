/**
 * 실존 최상위 도메인(TLD) 원장 — 출처 = IANA `tlds-alpha-by-domain.txt` (Version 2026092200).
 *
 * 왜 있나 — 링크 한 글자 오타(`.com` → `.cpm`)가 발송을 통째로 죽인다. 카카오는 규격 밖 주소를
 * 받지 않아 큐에는 들어가고 발송만 실패하며(0922 실측 = `status_code 9999` "기타 오류"),
 * 화면에는 사유가 안 남고 **차감은 이미 끝난 뒤**다. 존재하지 않는 TLD는 100% 오타이므로
 * 네트워크 없이 그 자리에서 판정한다(DNS·HTTP 확인은 하지 않는다 — 느리고 오탐이 난다).
 *
 * ⛔ 손으로 고치지 마라. 갱신은 IANA 파일을 다시 받아 같은 형식으로 덮어쓴다.
 *    새 TLD는 드물게 늘어날 뿐이라 갱신이 늦어도 **정상 주소를 막는 방향으로만** 틀린다.
 *
 * 프론트 사본(`packages/frontend/src/constants/tld-list.ts`)과 같아야 한다 — `tld-parity.test.ts`가 고정한다.
 */
export const TLD_LIST_VERSION = '2026092200';

/** 공백으로 이어 붙여 보관한다 — 한 줄 한 항목으로 두면 파일이 1,400줄이 된다 */
const RAW = `
...
export const KNOWN_TLDS: ReadonlySet<string> = new Set(RAW.trim().split(/\s+/));
