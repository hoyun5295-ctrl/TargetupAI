 ...
 *   - 1순위: 자사몰이 보낸 "전송 1건 고유값" (event_id / delivery_id / webhook_id)
 *     ★ 2026-09-25 한줄로 전수점검 C-12: event_no 제외 — 카페24 event_no는 전송 고유값이 아니라 **이벤트 종류 번호**다
 *     (90023=주문 접수 · cafe24-client.ts CAFE24_EVENT_NO_MAP). 키가 `order.created:evt:90023`로 늘 같아져
 *     회사마다 종류별 첫 1건 뒤의 주문·입금·취소·환불·장바구니 웹훅이 전부 duplicate로 버려졌다.
...
const DELIVERY_ID_FIELDS = ['event_id', 'delivery_id', 'webhook_id'] as const;
