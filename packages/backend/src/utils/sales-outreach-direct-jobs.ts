...
/** 사람이 브라우저에서 저장한 기획전 페이지 → 행사 텍스트(붙여넣기와 같은 성격 · DB 쓰기 0) */
export async function extractOutreachStorePageText(jobId: string, html: string, operatorSuperAdminId: string | null | undefined): Promise<{ text: string; chars: number }> {
  await assertOperator(operatorSuperAdminId);
