...
export const JOB_GROUPS: { key: string; label: string; jobs: string[] }[] = [
  { key: 'start', label: '시작 준비', jobs: ['sender-register', 'credits-and-plan', 'manage-accounts', 'ai-usage', 'ai-batches', 'sns-connect'] },
...
const READY: FeatureJob[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      '왼쪽에 내용을 씁니다. 이름처럼 사람마다 다른 값은 내용 칸 아래 "변수"로 넣고, 광고면 아래 발송 바의 "광고 표기"를 켭니다',
      '내용 아래 "스팸 검사"와 "맞춤법 검사"로 한 번 걸러 보고, "미리보기"로 받는 사람 화면을 확인합니다',
      '아래 발송 바에서 "발신번호"를 고릅니다. "수신거부제거"와 "중복제거"는 기본으로 켜져 있습니다. 전송 버튼을 누르면 확인 창이 뜨고, 확인하면 나갑니다(걸러내지 않으려면 체크를 푸세요)',
      '스팸 검사를 하지 않고 보내면 "발송 전에 스팸 검사를 하셨나요?" 창이 한 번 더 묻습니다. "스팸 검사하고 보내기"나 "그냥 보내기"를 고르고, "24시간 다시 보지 않기"를 켜면 하루 동안 뜨지 않습니다',
    ],
    blockers: [
      { symptom: '발송 버튼이 눌리지 않습니다', fix: '발신번호가 선택됐는지, 수신번호가 한 건이라도 있는지, 내용이 비어 있지 않은지 확인하세요' },
      { symptom: '광고 문자인데 맨 앞에 (광고)가 안 붙습니다', fix: '아래 발송 바의 "광고 표기"를 켜면 (광고)와 무료수신거부 문구가 자동으로 붙습니다' },
  ...
  },
  {
    ...
    steps: [
      ...
      '직접발송 화면에서 이미 쓴 내용은 "맞춤법 검사" 결과 창의 "문장까지 AI로 다듬기"로 자연스럽게 고칠 수 있습니다',
    ...
    status: 'ready', stubUntil: null,
    sourceFile: 'frontend/src/pages/Dashboard.tsx(AI 문구 생성) · frontend/src/components/direct-send/DirectSpellModal.tsx(AI 다듬기)',
  },
  {
    id: 'view-customer',
    title: '고객 한 명 이력 보기',
  ...
  },
  {
    ...
    steps: [
      '"직접발송"은 아래 발송 바의 "예약"을, "직접 타겟 발송"은 "예약전송"을 누릅니다',
    ...
    entry: { path: '/dashboard', via: '"직접발송" 아래 발송 바의 "예약"', open: 'open=direct-send' },
    planKey: 'basic_send',
    creditSource: null,
    related: ['send-direct', 'send-target', 'check-results'],
    status: 'ready', stubUntil: null,
    sourceFile: 'frontend/src/components/DirectSendPanel.tsx(발송 바 예약) · frontend/src/pages/Settings.tsx(발송 정책)',
  },
  {
    id: 'send-alimtalk',
    title: '알림톡으로 보내기',
  ...
  },
  {
    id: 'check-spam',
    title: '보내기 전 스팸 확인하기',
    goal: '보내기 전에 통신사 3사에서 스팸으로 막히는지 확인한다',
    keywords: ['스팸', '스팸필터', '스팸 테스트', '차단 확인', '스팸함', '필터 테스트', '스팸필터테스트', '통신사 차단'],
    steps: [
      '상단 메뉴 "직접발송"에서 수신번호를 먼저 올리고 발신번호와 내용을 채웁니다',
      '내용 아래 "스팸 검사"를 누르면 "스팸 검사" 창이 열립니다',
      '"검사 시작"을 누르면 통신사 3사에서 같은 내용으로 스팸 검사를 하고, 통신사마다 통과했는지 막혔는지 표시됩니다',
      '기다리기 싫으면 "창 닫고 계속 쓰기"로 닫아 두면 내용 아래 칸이 결과를 따라가고, 지난 결과는 "지난 검사"에서 다시 봅니다',
      '막힌 곳이 나오면 문구를 고쳐 "다시 검사"합니다. 결과는 참고용이라 실제 도착을 보장하지는 않습니다',
      '요금제에 가입하지 않은 회사는 무료로 세 번 써 볼 수 있습니다(차감 없음). 요금제 회사의 테스트 문자는 발송 요금으로 청구됩니다',
    ],
    blockers: [
      ...
      { symptom: '"무료 스팸 검사 3번을 모두 썼어요" 창이 뜹니다', fix: '무료 체험 세 번을 다 쓴 경우입니다. 요금제에 가입하면 보낼 때마다 검사할 수 있습니다' },
    ],
    entry: { path: '/dashboard', via: '"직접발송" 내용 아래 "스팸 검사"', open: 'open=direct-send' },
    planKey: 'spam_filter',
    creditSource: null,
  ...
  },
  {
    ...
    steps: [
      ...
      '"빠른 시작"의 시나리오 카드를 누르면 그 주제로 바로 만들어집니다. "직접 제작"은 빈 캔버스, "완성 슬라이드"는 완성 이미지를 슬라이드 DM으로, "카탈로그 DM"은 쪽 이미지를 올려 휴대폰에서는 슬라이드로 넘기고 PC에서는 책처럼 펼쳐 보이는 DM으로, "라이브러리 불러오기"는 저장 소재로 만듭니다. 편집 화면 위쪽 토글(스크롤·슬라이드·카탈로그)로 나중에 바꿀 수도 있습니다',
  ...
  {
    // ★ 2026-09-15 AI 자동제작 기준으로 다시 씀(0914 원클릭 캠페인 승격 · 옛 문안이 옛 화면 사용법을 답하고 있었다)
    id: 'quick-campaign',
    title: 'AI 자동제작',
    goal: '행사 내용과 사진만 넣으면 DM·이메일 완성본이 나온다',
    keywords: ['AI 자동제작', '자동제작', '원클릭', '원클릭 캠페인', '한 번에', '행사 캠페인', '완성본', '재료만 넣으면', '빠른 캠페인'],
    steps: [
      '대시보드 "AI Operator"로 들어가 "AI 자동제작" 카드를 누릅니다. 모바일 DM이나 Email 캠페인 화면의 자동제작 안내에서도 들어갈 수 있습니다',
      '위쪽에서 "모바일 DM" · "이메일" · "카탈로그 DM" 중 만들 채널을 고릅니다. 이메일이면 "광고 메일로 보내기"를 켤지 함께 정합니다. 카탈로그 DM은 쪽 이미지를 장수 제한 없이 올리면 그대로 카탈로그 DM이 됩니다(글자 읽기·문구 생성 없음)',
      '"행사 재료"에 행사 제목과 내용을 붙여 넣고 "사진 추가"나 "라이브러리"로 사진을 넣습니다. 내용을 비워 두면 올린 사진에서 글자를 읽어 채웁니다. 행사는 "행사 추가"로 늘립니다',
      '"상품"에서 "연동몰에서 불러오기"로 몰 상품을 담거나 "직접 추가"로 상품명과 가격을 붙여 넣습니다. 몰 상품 가격은 만들 때 몰에서 다시 확인합니다',
      '아래쪽 "AI 자동제작"을 누르고 확인 창에서 드는 크레딧을 본 뒤 진행하면 완성본이 편집기에 바로 열립니다',
      '편집기 위쪽 "다시 만들기"로 같은 재료로 새로 만들 수 있습니다. 다시 만들 때마다 생성 크레딧이 듭니다',
    ],
    blockers: [
      { symptom: '"행사 내용 40자 이상 또는 첫 화면이 될 사진 1장이 필요해요"라고 뜨고 버튼이 눌리지 않습니다', fix: '행사 내용을 더 적거나 가로로 긴 배너·상품 사진을 한 장 올리세요. 로고나 세로로 긴 사진은 첫 화면이 되지 않습니다' },
      { symptom: '"이메일 발신 설정이 먼저 필요해요"라고 뜹니다', fix: '"발신 설정으로"를 눌러 Email 캠페인 화면에서 발신 메일을 먼저 등록하면 이메일로 만들 수 있습니다' },
      { symptom: '"지금 만드는 중이에요"라고 뜹니다', fix: '같은 회사에서 이미 만들고 있는 완성본이 있습니다. 끝난 뒤 다시 누르세요' },
    ],
    entry: { path: '/quick-campaign', via: '대시보드 "AI Operator" → "AI 자동제작"' },
    planKey: 'mobile_dm',
    creditSource: 'dm-ai-generate',
    related: ['mobile-dm', 'email-campaign', 'image-studio'],
    status: 'ready', stubUntil: null,
    sourceFile: 'frontend/src/pages/QuickCampaignPage.tsx · frontend/src/components/ai-build/BuildCardsInput.tsx · frontend/src/components/ai-build/ProductPickList.tsx',
  },
  {
    id: 'journeys',
    title: '사건 자동화 만들기',
  ...
  //   steps는 실제 화면 순서, blockers는 코드가 실제로 내는 차단 문구에서 뽑았다.
  {
    ...
    steps: [
      '상단 메뉴 "대행발송"을 누릅니다. 요청서 양식을 받아 채워 두셨다면 "요청서로 접수"에 그 파일 하나만 올리면 나머지는 자동으로 채워집니다(첫 시트에 내용, 고객리스트 시트에 명단)',
      '직접 넣으실 때는 "새 접수"를 누르고 명단을 올리거나 번호를 붙여 넣습니다. 전화번호 열은 자동으로 골라 두니 다르면 바꿉니다',
      '문안과 제목을 쓰고, 필요하면 이미지를 넣습니다. 이름처럼 사람마다 다른 값은 퍼센트 기호로 감싸면 명단의 같은 이름 열에 자동으로 맞춰집니다. 열 이름이 달라도 접수 확인 화면에서 AI가 골라 둔 열을 확인해 맞출 수 있습니다',
      '보내는 번호와 보낼 시각, 테스트 문자를 받을 담당자 번호를 넣고 접수합니다',
      '검사를 마치면 담당자 번호로 문안이 그대로 옵니다. 함께 온 주소를 열거나 목록에서 승인하면 그 시각에 예약됩니다',
      '발송 2시간 전에 한 번 더 검사합니다. 그때 걸리면 예약을 취소하고 다듬은 문안으로 다시 승인을 받습니다',
    ],
    blockers: [
      { symptom: '보낼 시각이 요청한 것보다 뒤로 잡혔습니다', fix: '문안 검사와 담당자 승인에 시간이 필요해, 40분보다 촉박하게 넣으신 건은 30분 뒤로 옮겨 접수합니다. 담당자 문자에 옮긴 시각이 적혀 있고, 화면에서 시각을 다시 바꿀 수 있습니다' },
  ...
  },
  {
    ...
    blockers: [
      { symptom: '연결했는데 "계정 확인 필요"로 표시됩니다', fix: '인스타그램은 프로페셔널(비즈니스·크리에이터) 계정만 연결됩니다. 인스타그램 앱에서 계정 종류를 바꾼 뒤 다시 연결해 주세요' },
    ...
    status: 'ready', stubUntil: null,
    sourceFile: 'frontend/src/pages/SnsPage.tsx · backend/src/routes/sns.ts · backend/src/utils/sns-accounts.ts',
  },
];
...
export const NOT_DOCUMENTED_ROUTES: Record<string, string> = {
  ...
  '/charge-approve': '충전 승인 담당자가 문자 속 주소로 여는 승인 화면(로그인 없음 · 내부 운영용)', // ★2026-08-28(3) 충전 링크 승인
