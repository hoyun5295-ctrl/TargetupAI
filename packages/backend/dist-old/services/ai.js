...
exports.validatePersonalizationVars = validatePersonalizationVars;
exports.extractVarCatalog = extractVarCatalog;
exports.filterVarCatalogByData = filterVarCatalogByData;
exports.generateMessages = generateMessages;
exports.detectActiveFields = detectActiveFields;
exports.buildFilterFieldsPrompt = buildFilterFieldsPrompt;
exports.recommendTarget = recommendTarget;
exports.parseBriefing = parseBriefing;
exports.generateCustomMessages = generateCustomMessages;
exports.countFilteredCustomers = countFilteredCustomers;
exports.relaxFilters = relaxFilters;
exports.recommendNextCampaign = recommendNextCampaign;
exports.checkAPIStatus = checkAPIStatus;
...
if (!process.env.ANTHROPIC_API_KEY)
    console.warn('[AI] ANTHROPIC_API_KEY not configured — Claude AI 기능 비활성 상태');
if (!process.env.OPENAI_API_KEY)
    console.warn('[AI] OPENAI_API_KEY not configured — OpenAI 기능 비활성 상태');
...
async function callAIWithFallback(params) {
    ...
    try {
        const response = await anthropic.messages.create({
            model: defaults_1.AI_MODELS.claude,
    ...
    try {
        const gptResponse = await openai.chat.completions.create({
            model: defaults_1.AI_MODELS.gpt,
...
function buildVarCatalogFromFieldMap() {
    ...
    for (const f of standard_field_map_1.FIELD_MAP) {
        if (f.storageType === 'custom_fields')
            continue;
        if (f.fieldKey === 'phone' || f.fieldKey === 'sms_opt_in')
            continue;
        const varName = f.displayName;
        fieldMappings[varName] = {
            column: f.columnName,
            type: f.dataType === 'boolean' ? 'string' : f.dataType,
 ...
 */
function parsePersonalizationDirective(input, availableVars) {
    ...
    for (const name of fieldNames) {
        ...
        if (!found)
            found = availableVars.find(v => v.includes(name) || name.includes(v));
...
function formatRejectNumber(num) {
    if (!num)
        return '';
 ...
 */
function getAvailableSmsBytes(isAd, rejectNumber) {
    if (!isAd)
        return 88; // 비광고: 90 - 여유 2
    if (!rejectNumber) {
 ...
 */
function validatePersonalizationVars(message, availableVars) {
 ...
 */
function buildVarCatalogPrompt(catalog, availableVars) {
 ...
 */
function extractVarCatalog(customerSchema) {
    ...
    (0, standard_field_map_1.applyFieldAliases)(result.fieldMappings, result.availableVars, (col) => Object.values(result.fieldMappings).find((e) => e.column === col));
    return result;
}
/**
 ...
 */
async function filterVarCatalogByData(varCatalog, availableVars, companyId) {
    // 1. 직접 컬럼 필드 — COUNT FILTER로 데이터 유무 확인
    const detectableFields = (0, standard_field_map_1.getColumnFields)().filter(f => f.fieldKey !== 'name' && f.fieldKey !== 'phone');
    ...
    if (countFilters.length > 0) {
        const dataCheck = await (0, database_1.query)(`SELECT ${countFilters.join(', ')} FROM customers WHERE company_id = $1 AND is_active = true`, [companyId]);
    ...
    if (customEntries.length > 0) {
        ...
        if (safeEntries.length > 0) {
            const customCountFilters = safeEntries.map(([_, v]) => `COUNT(*) FILTER (WHERE custom_fields->>'${v.column}' IS NOT NULL AND custom_fields->>'${v.column}' != '') as cnt_${v.column}`);
            const customCheck = await (0, database_1.query)(`SELECT ${customCountFilters.join(', ')} FROM customers WHERE company_id = $1 AND is_active = true`, [companyId]);
 ...
 */
function detectPersonalizationVars(objective, fieldMappings, availableVars) {
    const detected = [];
    ...
    if (gradeMapping && gradeMapping.sample_values) {
        const gradeValues = gradeMapping.sample_values;
        if (Array.isArray(gradeValues)) {
            keywordMap['등급'] = keywordMap['등급'].concat(gradeValues.map((v) => String(v)));
        }
    }
    for (const varName of availableVars) {
        if (!fieldMappings[varName])
            continue;
...
// ============================================================
function replaceBrandPlaceholder(text, brandName) {
    if (!text || !brandName || brandName === '브랜드')
        return text;
    return text
...
function deepReplaceBrand(obj, brandName) {
    if (!brandName || brandName === '브랜드')
        return obj;
    if (typeof obj === 'string')
        return replaceBrandPlaceholder(obj, brandName);
    if (Array.isArray(obj))
        return obj.map(item => deepReplaceBrand(item, brandName));
    if (obj && typeof obj === 'object') {
        const result = {};
        for (const key of Object.keys(obj)) {
            result[key] = deepReplaceBrand(obj[key], brandName);
        }
        return result;
    }
    return obj;
...
// ============================================================
async function generateMessages(prompt, targetInfo, extraContext) {
    if (!process.env.ANTHROPIC_API_KEY) {
...
${extraContext.recentMessages.map((m, i) => `${i + 1}. ${m.replace(/\(광고\)/g, '').replace(/무료거부\d+/g, '').replace(/무료수신거부\s?\d{3}-?\d{3,4}-?\d{4}/g, '').trim().slice(0, 300)}`).join('\n')}
    ...
    try {
        ...
        if (result.variants) {
            for (const variant of result.variants) {
                let msgField = variant.message_text || '';
                ...
                variant.message_text = msgField;
                // ★ D28: 제목에서 %변수% 강제 제거 (AI가 프롬프트 무시 시 안전장치)
                if (variant.subject) {
                    variant.subject = (0, messageUtils_1.cleanLeftoverVars)(variant.subject).replace(/  +/g, ' ').trim();
                }
 ...
 */
async function detectActiveFields(companyId) {
    // 1) 직접 컬럼 필드 — 데이터 존재 여부 한 번에 체크
    const detectableFields = (0, standard_field_map_1.getColumnFields)().filter(f => f.fieldKey !== 'name' && f.fieldKey !== 'phone' && f.fieldKey !== 'sms_opt_in');
    const countFilters = detectableFields.map(f => {
        const c = f.columnName;
        if (f.fieldKey === 'age')
            return `COUNT(*) FILTER (WHERE ${c} IS NOT NULL AND ${c} > 0 OR birth_date IS NOT NULL) as cnt_${f.fieldKey}`;
        if (f.dataType === 'number')
            return `COUNT(*) FILTER (WHERE ${c} IS NOT NULL AND ${c} > 0) as cnt_${f.fieldKey}`;
        if (f.dataType === 'date')
            return `COUNT(*) FILTER (WHERE ${c} IS NOT NULL) as cnt_${f.fieldKey}`;
        return `COUNT(*) FILTER (WHERE ${c} IS NOT NULL AND ${c} != '') as cnt_${f.fieldKey}`;
    });
    // 2) 커스텀 필드 라벨 조회
    const [fieldCountRes, fieldDefsRes] = await Promise.all([
        (0, database_1.query)(`SELECT ${countFilters.join(', ')} FROM customers WHERE company_id = $1 AND is_active = true`, [companyId]),
        (0, database_1.query)(`SELECT field_key, field_label FROM customer_field_definitions WHERE company_id = $1 AND (is_hidden = false OR is_hidden IS NULL) ORDER BY display_order`, [companyId]),
    ]);
    ...
    try {
        ...
        if (stringFields.length > 0) {
            const distinctQueries = stringFields.map(f => (0, database_1.query)(`SELECT DISTINCT ${f.columnName} FROM customers WHERE company_id = $1 AND ${f.columnName} IS NOT NULL AND ${f.columnName} != '' ORDER BY ${f.columnName} LIMIT 200`, [companyId]));
            const results = await Promise.all(distinctQueries);
            stringFields.forEach((f, i) => {
                const vals = results[i].rows.map((r) => r[f.columnName]).filter(Boolean);
                if (vals.length > 0)
                    distinctValues[f.fieldKey] = vals;
            });
        }
        // 커스텀 필드도 DISTINCT 조회 (라벨 있는 필드만)
        for (const [key, label] of Object.entries(customFieldLabels)) {
            try {
                const cfRes = await (0, database_1.query)(`SELECT DISTINCT custom_fields->>'${key}' as val FROM customers WHERE company_id = $1 AND custom_fields->>'${key}' IS NOT NULL AND custom_fields->>'${key}' != '' ORDER BY val LIMIT 200`, [companyId]);
                const vals = cfRes.rows.map((r) => r.val).filter(Boolean);
                if (vals.length > 0)
                    distinctValues[`custom_fields.${key}`] = vals;
            }
            catch (cfErr) {
 ...
 */
function buildFilterFieldsPrompt(fields) {
    ...
    const columnLines = activeColumnFields.map(f => {
        ...
        if (key === 'age')
            return `- age: ${label} (between 연산자로 범위 지정, 예: [20, 29])`;
        if (key === 'birth_date')
            return `- birth_date: ${label} (birth_month 연산자로 월 필터, 예: {"operator": "birth_month", "value": 3} → 3월 생일)`;
        if (key === 'store_code')
            return `- store_code: ${label} (eq/in 연산자)${vals ? ' → 반드시 다음 값 중 하나만 정확히 사용: ' + vals.join(', ') : ''}`;
        if (f.dataType === 'number')
            return `- ${key}: ${label} (gte, lte, between 연산자)`;
        if (f.dataType === 'date')
            return `- ${key}: ${label} (days_within, gte, lte 연산자)`;
...
// ============================================================
async function recommendTarget(companyId, objective, customerStats, companyInfo) {
    if (!process.env.ANTHROPIC_API_KEY) {
        return {
            filters: {},
...
⚠️ 나이 기반 필터링 시: birth_date 직접 컬럼의 birth_month 연산자 또는 age 직접 컬럼을 사용하세요. 커스텀 필드의 생년월일로 나이를 계산하지 마세요.`;
    try {
        ...
        aiPersonalizationVars = aiPersonalizationVars.filter((v) => availableVars.includes(v));
...
// 폴백 메시지
// ============================================================
function getFallbackVariants(extraContext) {
...
function calculateKoreanBytes(text) {
    let bytes = 0;
 ...
 */
function buildParseBriefingSystem(filterFieldsPrompt) {
...
async function parseBriefing(briefing, companyId) {
    if (!process.env.ANTHROPIC_API_KEY) {
        return {
            promotionCard: {
                name: '프로모션',
 ...
 */
function fieldKeyToVarName(fieldKey) {
    const mapped = (0, standard_field_map_1.getFieldByKey)(fieldKey);
...
async function generateCustomMessages(options) {
    const { briefing, promotionCard, personalFields, fieldLabels, url, tone, brandName, brandTone, channel, isAd, rejectNumber, } = options;
 ...
 */
async function countFilteredCustomers(companyId, filters, userId, storeFilter = '', baseParams = []) {
    try {
        const effectiveBaseParams = baseParams.length > 0 ? baseParams : [companyId];
        const { sql: filterWhere, params: filterParams } = (0, customer_filter_1.buildFilterWhereClauseCompat)(filters, effectiveBaseParams.length + 1);
       ...
       AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdx} AND u.phone = c.phone)`, [...effectiveBaseParams, ...filterParams, userId]);
        const count = parseInt(countResult.rows[0].count);
        const unsubResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers c
       WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true${storeFilter} ${filterWhere}
       AND EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdx} AND u.phone = c.phone)`, [...effectiveBaseParams, ...filterParams, userId]);
 ...
 */
async function relaxFilters(originalFilters, originalReasoning, customerStats, activeFieldsPrompt, companyInfo) {
 ...
 */
async function recommendNextCampaign(companyId, performanceData, customerStats, companyInfo) {
    const brandName = companyInfo?.brand_name || companyInfo?.company_name || '브랜드';
    const businessType = companyInfo?.business_type || '기타';
    const userMessage = `## 회사 정보
- 업종: ${businessType}
