...
const training_logger_1 = require("../utils/training-logger");
...
const mms_image_util_1 = require("../utils/mms-image-util");
const mms_validator_1 = require("../utils/mms-validator");
const stats_aggregation_1 = require("../utils/stats-aggregation");
const campaign_lifecycle_1 = require("../utils/campaign-lifecycle");
const customer_filter_1 = require("../utils/customer-filter");
const callback_filter_1 = require("../utils/callback-filter");
const deduplicate_1 = require("../utils/deduplicate");
const test_contact_helper_1 = require("../utils/test-contact-helper");
const campaign_validation_1 = require("../utils/campaign-validation");
 ...
 * @returns 조정된 발송 시간
 */
function calcSplitSendTime(baseTime, batchIndex, sendStartHour = defaults_1.SEND_HOURS.start, sendEndHour = defaults_1.SEND_HOURS.end) {
    const result = new Date(baseTime.getTime());
    result.setMinutes(result.getMinutes() + batchIndex);
    // 한국시간 기준으로 시각 확인 (KST = UTC+9)
    const kstHour = parseInt(result.toLocaleString('en-US', { timeZone: 'Asia/Seoul', hour: '2-digit', hour12: false }));
    if (kstHour >= sendEndHour) {
        // 종료 시각 초과 → 다음날 시작 시각으로 이월
        // 초과한 분수 계산
        const kstMinutes = parseInt(result.toLocaleString('en-US', { timeZone: 'Asia/Seoul', minute: '2-digit' }));
...
router.get('/', async (req, res) => {
    try {
        ...
        const companyTables = await (0, sms_queue_1.getCompanySmsTables)(companyId, userId);
        ...
        if (status) {
            ...
            if (status === 'scheduled') {
                ...
                const scheduleParams = [companyId];
                if (userType === 'company_user' && userId) {
                ...
                const scheduledCampaigns = await (0, database_1.query)(scheduleQuery, scheduleParams);
                for (const camp of scheduledCampaigns.rows) {
                    // 예약 시간이 아직 안 됐으면 스킵 (MySQL에 데이터 없는게 정상)
                    const campDetail = await (0, database_1.query)(`SELECT scheduled_at FROM campaigns WHERE id = $1`, [camp.id]);
                    ...
                    const tablesWithLogs = await (0, sms_queue_1.getCompanySmsTablesWithLogs)(companyId);
                    const sentCount = await (0, sms_queue_1.smsCountAll)(tablesWithLogs, 'app_etc1 = ?', [camp.id]);
                    const successCount = await (0, sms_queue_1.smsCountAll)(tablesWithLogs, `app_etc1 = ? AND status_code IN (${sms_result_map_1.SUCCESS_CODES.join(',')})`, [camp.id]);
                    const failCount = await (0, sms_queue_1.smsCountAll)(tablesWithLogs, `app_etc1 = ? AND status_code NOT IN (${[...sms_result_map_1.SUCCESS_CODES, ...sms_result_map_1.PENDING_CODES].join(',')})`, [camp.id]);
                    const newStatus = sentCount === 0 ? 'failed' : 'completed';
                    await (0, database_1.query)(`UPDATE campaigns SET status = $1, sent_count = $2, success_count = $3, fail_count = $4, sent_at = COALESCE(sent_at, scheduled_at, NOW()), updated_at = NOW() WHERE id = $5`, [newStatus, sentCount, successCount, failCount, camp.id]);
                    // ★ 선불 실패건 환불
                    if (failCount > 0) {
                        const campInfo = await (0, database_1.query)('SELECT company_id, message_type FROM campaigns WHERE id = $1', [camp.id]);
                        if (campInfo.rows.length > 0) {
                            await (0, prepaid_1.prepaidRefund)(campInfo.rows[0].company_id, failCount, campInfo.rows[0].message_type, camp.id, '발송 실패 환불');
                        }
                    }
                }
            }
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM campaigns ${whereClause}`, countParams);
       ...
       LIMIT $${paramIndex++} OFFSET $${paramIndex}`, params);
        const listSmsMap = await (0, stats_aggregation_1.aggregateSmsCountsByCampaign)(result.rows);
        const listKakaoMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(result.rows.map((c) => c.id));
        const listSentTimeMap = await (0, stats_aggregation_1.aggregateSmsSendTimesByCampaign)(result.rows);
...
router.post('/test-send', async (req, res) => {
    try {
        ...
        const testMmsCheck = (0, mms_validator_1.validateMmsPayload)(messageType, req.body.mmsImagePaths);
        if (!testMmsCheck.ok) {
        ...
        if (testChannel === 'kakao' || testChannel === 'both') {
            const kakaoCheck = await (0, database_1.query)('SELECT kakao_enabled FROM companies WHERE id = $1', [companyId]);
            if (!kakaoCheck.rows[0]?.kakao_enabled) {
                return res.status(403).json({ error: '카카오 브랜드메시지가 활성화되지 않은 고객사입니다.', code: 'KAKAO_NOT_ENABLED' });
            }
        }
        // 회사 설정에서 스키마 가져오기
        const companyResult = await (0, database_1.query)('SELECT customer_schema FROM companies WHERE id = $1', [companyId]);
        ...
        const testFieldMappings = await (0, messageUtils_1.prepareFieldMappings)(companyId);
        let testFirstCustomer;
        if (req.body.sampleCustomer && typeof req.body.sampleCustomer === 'object' && Object.keys(req.body.sampleCustomer).length > 0) {
            ...
            const testMappingCols = Object.values(testFieldMappings).filter((m) => m.storageType !== 'custom_fields').map((m) => m.column);
            const testSelectCols = [...new Set(['phone', 'custom_fields', ...testMappingCols])].join(', ');
            const testFirstCustomerResult = await (0, database_1.query)(`SELECT ${testSelectCols} FROM customers WHERE company_id = $1 AND is_active = true AND sms_opt_in = true ORDER BY name ASC NULLS LAST LIMIT 1`, [companyId]);
            testFirstCustomer = testFirstCustomerResult.rows[0] || {};
        }
        // 회신번호 가져오기 (callback_numbers 테이블에서)
        const callbackResult = await (0, database_1.query)('SELECT phone FROM callback_numbers WHERE company_id = $1 AND is_default = true LIMIT 1', [companyId]);
        const callbackNumber = (0, normalize_phone_1.normalizePhone)(callbackResult.rows[0]?.phone || '');
        if (!callbackNumber) {
        ...
        const managerContacts = await (0, test_contact_helper_1.getUserTestContacts)(companyId, userId);
        if (managerContacts.length === 0) {
            return res.status(400).json({ error: '등록된 담당자 번호가 없습니다. 설정에서 번호를 추가해주세요.' });
        }
        // ★ 선불 잔액 체크
        const testMsgType = (messageType || 'SMS');
        const testDeduct = await (0, prepaid_1.prepaidDeduct)(companyId, managerContacts.length, testMsgType, '00000000-0000-0000-0000-000000000000', userId);
        if (!testDeduct.ok) {
        ...
        const msgType = (0, sms_queue_1.toQtmsgType)(messageType || 'SMS');
        ...
        const mmsImagePathsRaw = req.body.mmsImagePaths || [];
        const mmsImagePaths = (0, mms_image_util_1.normalizeMmsImagePaths)(mmsImagePathsRaw);
        ...
        const testOpt080 = isAd ? await (0, messageUtils_1.getOpt080Number)(userId || null, companyId) : '';
        for (const contact of managerContacts) {
            try {
                const cleanPhone = (0, normalize_phone_1.normalizePhone)(contact.phone);
                // ★ D103: prepareSendMessage 컨트롤타워 — 변수 치환 + (광고)+080 + ★ KISA 2026-05 제목(광고) 통합
                const { message: testMsg, subject: testSubject } = (0, messageUtils_1.prepareSendMessage)(messageContent, testFirstCustomer, testFieldMappings, {
                ...
                if (testChannel === 'sms' || testChannel === 'both') {
                    ...
                    await (0, sms_queue_1.insertTestSmsQueue)(cleanPhone, callbackNumber, testMsg, messageType || 'SMS', 'test', testSubject, {
        ...
        if (testFailCount > 0) {
            await (0, prepaid_1.prepaidRefund)(companyId, testFailCount, testMsgType, '00000000-0000-0000-0000-000000000000', '테스트 발송 실패 자동 환불');
        }
        // ★ C5: 실패 건 DB 기록 (비동기, 발송 응답에 영향 없음)
        if (failedContacts.length > 0) {
            try {
                await (0, database_1.query)(`INSERT INTO campaign_runs (campaign_id, run_number, target_count, sent_count, status, created_at)
           VALUES ($1, 0, $2, $3, 'failed', NOW())`, [testBillId, managerContacts.length, sentCount]);
            }
            catch (logErr) {
        ...
        return res.json({
            ...
            contacts: managerContacts.map(c => ({
                name: c.name || '이름없음',
                phone: `${(0, normalize_phone_1.normalizePhone)(c.phone).slice(0, 3)}-****-${(0, normalize_phone_1.normalizePhone)(c.phone).slice(-4)}`
            })),
        });
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const { campaignName, messageType, targetFilter, messageContent, subject, scheduledAt, isAd, eventStartDate, eventEndDate, mmsImagePaths, 
        // 카카오 브랜드메시지 필드
        ...
        const aiMmsCheck = (0, mms_validator_1.validateMmsPayload)(messageType, mmsImagePaths);
        if (!aiMmsCheck.ok) {
        ...
        const schedCheck = (0, campaign_validation_1.validateScheduledAt)(scheduledAt, { allowNull: true });
        if (!schedCheck.valid) {
        ...
        let targetCount = 0;
        if (targetFilter) {
            const filterQuery = (0, customer_filter_1.buildFilterQueryCompat)(targetFilter, companyId);
            const unsubIdx = 1 + filterQuery.params.length + 1;
            const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers c WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true ${filterQuery.where}
         AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdx} AND u.phone = c.phone)`, [companyId, ...filterQuery.params, userId]);
            targetCount = parseInt(countResult.rows[0].count);
        }
        const result = await (0, database_1.query)(`INSERT INTO campaigns (
...
router.post('/:id/send', async (req, res) => {
    try {
        ...
        if (!(await (0, sms_queue_1.hasCompanyLineGroup)(companyId))) {
        ...
        // 캠페인 조회
        const campaignResult = await (0, database_1.query)('SELECT * FROM campaigns WHERE id = $1 AND company_id = $2', [id, companyId]);
        ...
        if (!useIndividualCallback) {
            const senderCallback = (0, normalize_phone_1.normalizePhone)(campaign.callback_number || defaultCallback);
            ...
            const senderCheck = await (0, database_1.query)(`SELECT phone FROM (
        ...
        ) t WHERE phone = $2 LIMIT 1`, [companyId, senderCallback]);
        ...
        const companySchemaResult = await (0, database_1.query)('SELECT customer_schema FROM companies WHERE id = $1', [companyId]);
        const { availableVars } = (0, ai_1.extractVarCatalog)(companySchemaResult.rows[0]?.customer_schema);
        ...
        const mappingColumns = Object.values(fieldMappings).filter((m) => m.storageType !== 'custom_fields').map((m) => m.column);
        ...
        const customersResult = await (0, database_1.query)(`SELECT ${selectColumns} FROM customers c
       WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true ${filterQuery.where}${storeFilterFinal}
       AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubParamIdx} AND u.phone = c.phone)`, [companyId, ...filterQuery.params, ...storeParams, userId]);
        const customers = customersResult.rows;
        if (customers.length === 0) {
            return res.status(400).json({ error: '발송 대상이 없습니다.' });
        }
        // ★ 발송 전 메시지 변수 검증 (잘못된 변수가 고객에게 노출되는 것을 방지)
        const messageValidation = (0, ai_1.validatePersonalizationVars)(campaign.message_content || '', availableVars);
        ...
        // 제외 대상 필터링
        let filteredCustomers = customers.filter((c) => !excludedPhones.includes((0, normalize_phone_1.normalizePhone)(c.phone)));
        ...
        if (useIndividualCallback) {
            ...
            const cbResult = await (0, callback_filter_1.filterByIndividualCallback)(filteredCustomers, companyId, cbUserId, individualCallbackColumn);
            ...
            if (cbResult.callbackSkippedCount > 0 && !confirmCallbackExclusion) {
                const confirmBody = (0, callback_filter_1.buildCallbackConfirmResponse)(cbResult, filteredCustomers.length);
                return res.status(200).json(confirmBody);
            }
        }
        if (filteredCustomers.length === 0) {
            const errBody = (0, callback_filter_1.buildCallbackErrorResponse)(callbackMissingCount, callbackUnregisteredCount);
            return res.status(400).json(errBody);
        }
        // ★ D100: 동일 캠페인 중복 발송 방지 — 이미 sending/scheduled run이 있으면 차단
        const existingRun = await (0, database_1.query)(`SELECT id FROM campaign_runs WHERE campaign_id = $1 AND status IN ('sending', 'scheduled') LIMIT 1`, [id]);
        if (existingRun.rows.length > 0) {
            return res.status(400).json({ error: '이미 발송이 진행 중이거나 예약되어 있습니다.' });
        }
        // campaign_runs에 발송 이력 생성 (CT-08 확인 모달 통과 후에만 INSERT)
        const runNumberResult = await (0, database_1.query)(`SELECT COALESCE(MAX(run_number), 0) + 1 as next_run
       FROM campaign_runs WHERE campaign_id = $1`, [id]);
        ...
        const runResult = await (0, database_1.query)(`INSERT INTO campaign_runs (
      ...
      RETURNING *`, [
        ...
        const sendDeduct = await (0, prepaid_1.prepaidDeduct)(companyId, filteredCustomers.length, deductType, id, userId);
        if (!sendDeduct.ok) {
        ...
        try {
            ...
            const sendTime = isScheduled
                ? (0, sms_queue_1.toKoreaTimeStr)(new Date(campaign.scheduled_at))
                : (0, sms_queue_1.toKoreaTimeStr)(new Date()); // 즉시발송도 JS 타임스탬프를 파라미터로 전달
            // MMS 이미지 경로 (campaigns 테이블에서 가져옴)
            // ★ D124 N4: mms_image_paths가 객체 배열({path, originalName}) 또는 문자열 배열 혼재 가능 → 정규화
            const campaignMmsImages = (0, mms_image_util_1.normalizeMmsImagePaths)(campaign.mms_image_paths);
            const aiMsgTypeCode = (0, sms_queue_1.toQtmsgType)(campaign.message_type);
            ...
            const opt080Number = campaign.is_ad ? await (0, messageUtils_1.getOpt080Number)(userId || null, companyId) : '';
            let opt080Auth = '';
            ...
            for (const customer of filteredCustomers) {
                // ★ D103: prepareSendMessage 컨트롤타워 — 변수 치환 + (광고)+080 + ★ KISA 2026-05 제목(광고) 통합
                const { message: personalizedMessage, subject: personalizedSubject } = (0, messageUtils_1.prepareSendMessage)(campaign.message_content || '', customer, fieldMappings, {
                ...
                const customerCallback = (0, callback_filter_1.resolveCustomerCallback)(customer, useIndividualCallback, campaign.callback_number || defaultCallback);
                const cleanPhone = (0, normalize_phone_1.normalizePhone)(customer.phone);
            ...
            if (sendChannel === 'sms' || sendChannel === 'both') {
                aiSentCount += await (0, sms_queue_1.bulkInsertSmsQueue)(companyTables, aiSmsRows, !isScheduled);
            }
            // 3단계: 카카오 발송 (개별 호출 — 카카오 API 특성상 bulk 미지원)
            for (const kakaoItem of aiKakaoQueue) {
                try {
                    await (0, sms_queue_1.insertKakaoQueue)(kakaoItem);
            ...
            if (aiFailCount > 0) {
                ...
                try {
                    await (0, prepaid_1.prepaidRefund)(companyId, aiFailCount, deductType, id, `AI발송 부분실패 ${aiFailCount}건 환불`);
                }
            ...
            await (0, database_1.query)(`UPDATE campaign_runs SET
    sent_count = $1,
    status = $2
    ${isScheduled ? '' : ', sent_at = CURRENT_TIMESTAMP'}
   WHERE id = $3`, [aiSentCount, aiSentCount === 0 ? 'failed' : (isScheduled ? 'scheduled' : 'completed'), campaignRun.id]);
            // 캠페인 상태 업데이트
            // ★ #6: 예약 캠페인은 sent_at 설정하지 않음 (예약 시점 발송 시 set)
            // ★ C1: aiSentCount 기반
            // ★ D144 후속: 동일 정책 — 즉시발송은 'completed', 예약은 'scheduled' 유지, 0건이면 'failed'
            await (0, database_1.query)(`UPDATE campaigns SET
    status = $1,
    sent_count = COALESCE(sent_count, 0) + $2,
    target_count = $3
    ${isScheduled ? '' : ', sent_at = CURRENT_TIMESTAMP'}
   WHERE id = $4`, [aiSentCount === 0 ? 'failed' : (isScheduled ? 'scheduled' : 'completed'), aiSentCount, filteredCustomers.length, id]);
            // ★ AI 학습 데이터 적재 (비동기, 실패해도 발송에 영향 없음)
            const trainingCompanyInfo = await (0, database_1.query)('SELECT name, brand_tone FROM companies WHERE id = $1', [companyId]);
            (0, training_logger_1.logTrainingData)({
            ...
            try {
                await (0, prepaid_1.prepaidRefund)(companyId, filteredCustomers.length, deductType, id, '발송 전체 실패 자동 환불');
                await (0, database_1.query)(`UPDATE campaigns SET status = 'failed', updated_at = NOW() WHERE id = $1`, [id]);
            }
            catch (refundErr) {
...
router.get('/test-stats', async (req, res) => {
    try {
        ...
        for (const lt of logTables) {
            try {
                await (0, database_1.mysqlQuery)(`SELECT 1 FROM ${lt} LIMIT 0`);
                allTables.push(lt);
            }
            catch { /* 테이블 없으면 스킵 */ }
        }
        const allResults = await (0, sms_queue_1.smsSelectAll)(allTables, 'seqno, dest_no, msg_contents, msg_type, sendreq_time, status_code, mobsend_time, bill_id', `app_etc1 = 'test' AND app_etc2 = ?${userFilter}${dateFilter}`, queryParams, 'ORDER BY sendreq_time DESC');
        // 시간순 정렬 (여러 테이블 합산이므로 재정렬)
        allResults.sort((a, b) => new Date(b.sendreq_time).getTime() - new Date(a.sendreq_time).getTime());
        // 발송자 정보 조회 (관리자용)
        const senderIds = [...new Set(allResults.map((r) => r.bill_id).filter(Boolean))];
        let senderMap = {};
        if (senderIds.length > 0) {
            const senderResult = await (0, database_1.query)(`SELECT id, name FROM users WHERE id = ANY($1::uuid[])`, [senderIds]);
            senderResult.rows.forEach((u) => {
        ...
        const stats = {
            ...
            success: allResults.filter((r) => (0, sms_result_map_1.isSuccess)(r.status_code)).length,
            fail: allResults.filter((r) => (0, sms_result_map_1.isFail)(r.status_code)).length,
            pending: allResults.filter((r) => sms_result_map_1.PENDING_CODES.includes(r.status_code)).length,
            cost: 0,
        };
        // 비용 계산 (회사 실제 단가 기준)
        const costResult = await (0, database_1.query)('SELECT cost_per_sms, cost_per_lms, cost_per_mms FROM companies WHERE id = $1', [companyId]);
        const costSms = Number(costResult.rows[0]?.cost_per_sms) || defaults_1.DEFAULT_COSTS.sms;
        const costLms = Number(costResult.rows[0]?.cost_per_lms) || defaults_1.DEFAULT_COSTS.lms;
        const costMms = Number(costResult.rows[0]?.cost_per_mms) || defaults_1.DEFAULT_COSTS.mms;
        allResults.forEach((r) => {
            if ((0, sms_result_map_1.isSuccess)(r.status_code)) {
        ...
        const list = allResults.map((r) => ({
            ...
            status: (0, sms_result_map_1.isSuccess)(r.status_code) ? 'success' : sms_result_map_1.PENDING_CODES.includes(r.status_code) ? 'pending' : 'fail',
            testType: 'manager',
            senderName: senderMap[r.bill_id] || '-',
        }));
        // ========== 스팸필터 테스트 통합 ==========
        // ★ D104: 날짜 필터 컨트롤타워 사용
        const spamDr = (0, stats_aggregation_1.buildDateRangeFilter)('t.created_at', fromDate, toDate, 2);
        const spamDateWhere = spamDr.sql;
        const spamParams = [companyId, ...spamDr.params];
        ...
        const spamAgg = await (0, database_1.query)(`
      SELECT
        COUNT(*) as total,
    ...
    `, spamParams);
        ...
        let sfCostCalc = 0;
        // 스팸필터 리스트 (최근 100건)
        const spamListResult = await (0, database_1.query)(`
      SELECT
      ...
      LIMIT 100
    `, spamParams);
        const spamFilterList = spamListResult.rows.map((r) => {
            ...
            return {
                ...
                result: r.result || sms_result_map_1.SPAM_RESULT.PASS,
                carrier: r.carrier,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT c.*, ${unsubscribe_helper_1.CAMPAIGN_OPT080_SELECT_EXPR}
       FROM campaigns c
       ${unsubscribe_helper_1.CAMPAIGN_OPT080_LEFT_JOIN}
       WHERE c.id = $1 AND c.company_id = $2`, [id, companyId]);
        ...
        // 발송 이력도 함께 조회
        const runs = await (0, database_1.query)(`SELECT * FROM campaign_runs WHERE campaign_id = $1 ORDER BY created_at DESC`, [id]);
        return res.json({
            ...result.rows[0],
            runs: runs.rows
        });
...
router.post('/sync-results', async (req, res) => {
    try {
        ...
        const result = await (0, campaign_lifecycle_1.syncCampaignResults)(companyId);
...
router.post('/direct-send', async (req, res) => {
    try {
        ...
        dedupEnabled = true, unsubFilterEnabled = true, } = req.body;
        // ★ D143 (2026-05-04, 정식 오픈 D-Day 1일 전) — D142+ 자동 승격 정책 폐지
        //   정책 변경 사유 (Harold님 명시): 사용자가 광고체크 OFF + 본문에 (광고)/무료거부 복붙한
        //   케이스에서 D142+가 본문 깎고 is_ad 강제 승격 → 사용자 의도 무시 → 정합성 위반
        //   새 정책: 사용자 입력 본문 그대로 저장 + 광고체크 의도 그대로 저장
        ...
        // ★ D131: MMS 이미지 첨부 필수 가드 — mms-validator 컨트롤타워
        const directMmsCheck = (0, mms_validator_1.validateMmsPayload)(msgType, mmsImagePaths);
        ...
        if (scheduled) {
            const dsCheck = (0, campaign_validation_1.validateScheduledAt)(scheduledAt, { allowNull: false });
            if (!dsCheck.valid) {
        ...
        if (dedupEnabled !== false) {
            const dedupResult = (0, deduplicate_1.deduplicateByPhone)(recipients);
        ...
        if (!useIndividualCallback && callback) {
            const normalizedCallback = (0, normalize_phone_1.normalizePhone)(callback);
            // 회신번호 최소 길이 검증 (한국 전화번호 최소 8자리)
          ...
          SELECT REPLACE(phone_number, '-', '') as phone FROM sender_numbers WHERE company_id = $1 AND is_active = true
          UNION SELECT REPLACE(phone, '-', '') as phone FROM callback_numbers WHERE company_id = $1
        ) t WHERE phone = $2 LIMIT 1`, [companyId, normalizedCallback]);
            if (senderCheck.rows.length === 0) {
        ...
        let validRecipients = [...finalRecipients];
        let callbackSkippedCount = 0;
        let callbackMissingCount = 0;
        let callbackUnregisteredCount = 0;
        if (useIndividualCallback) {
            ...
            const cbResult = await (0, callback_filter_1.filterByIndividualCallback)(validRecipients, companyId, cbUserId);
            ...
            // ★ 미등록 회신번호 확인 모달 — 제외 건이 있고 confirmCallbackExclusion 없으면 항상 확인 모달 반환
            if (cbResult.callbackSkippedCount > 0 && !confirmCallbackExclusion) {
                const confirmBody = (0, callback_filter_1.buildCallbackConfirmResponse)(cbResult, validRecipients.length);
        ...
        if (targetFilter && Object.keys(targetFilter).length > 0) {
            const amountFields = Object.keys(targetFilter).filter(k => k.includes('amount') || k.includes('purchase') || k.includes('금액'));
            if (amountFields.length > 0) {
                const recipientPhones = finalRecipients.map((r) => (0, normalize_phone_1.normalizePhone)(r.phone));
                let filterWhere = 'c.company_id = $1 AND c.phone = ANY($2::text[]) AND c.is_active = true';
                const filterParams = [companyId, recipientPhones];
                let pIdx = 3;
                ...
                const validResult = await (0, database_1.query)(`SELECT c.phone FROM customers c WHERE ${filterWhere}`, filterParams);
                const validPhones = new Set(validResult.rows.map((r) => (0, normalize_phone_1.normalizePhone)(r.phone)));
                const beforeCount = targetFilteredRecipients.length;
                targetFilteredRecipients = targetFilteredRecipients.filter((r) => validPhones.has((0, normalize_phone_1.normalizePhone)(r.phone)));
        ...
        if (unsubFilterEnabled !== false) {
            const phones = targetFilteredRecipients.map((r) => (0, normalize_phone_1.normalizePhone)(r.phone));
            const unsubResult = await (0, database_1.query)(`SELECT DISTINCT phone FROM unsubscribes WHERE user_id = $1 AND phone = ANY($2)`, [userId, phones]);
            const unsubPhones = new Set(unsubResult.rows.map((r) => r.phone));
            filteredRecipients = targetFilteredRecipients.filter((r) => !unsubPhones.has((0, normalize_phone_1.normalizePhone)(r.phone)));
        ...
        const campaignResult = await (0, database_1.query)(`INSERT INTO campaigns (company_id, campaign_name, message_type, message_content, subject, callback_number, target_count, send_type, status, scheduled_at, message_template, message_subject, created_by, mms_image_paths,
       ...
       RETURNING id`, [
        ...
        const directDeduct = await (0, prepaid_1.prepaidDeduct)(companyId, filteredRecipients.length, directDeductType, campaignId, userId);
        if (!directDeduct.ok) {
            // 캠페인 레코드 롤백
            await (0, database_1.query)('DELETE FROM campaigns WHERE id = $1', [campaignId]);
            return res.status(402).json({
                success: false,
        ...
        try {
            ...
            const directOpt080 = finalIsAd ? await (0, messageUtils_1.getOpt080Number)(userId, companyId) : '';
            // ★ D102: prepareFieldMappings 컨트롤타워로 통합 (customer_schema 조회 + extractVarCatalog + enrichWithCustomFields)
            const directFieldMappings = await (0, messageUtils_1.prepareFieldMappings)(companyId);
            // ★ storageType 기반 동적 필터 — 직접 컬럼만 SELECT, JSONB 내부 키는 custom_fields 컬럼에서 접근 (D72)
            const directMappingCols = Object.values(directFieldMappings).filter((m) => m.storageType !== 'custom_fields').map((m) => m.column);
            const directSelectCols = [...new Set(['phone', 'custom_fields', ...directMappingCols])].join(', ');
            const directPhoneList = filteredRecipients.map((r) => (0, normalize_phone_1.normalizePhone)(r.phone));
            const directCustomersResult = await (0, database_1.query)(`SELECT ${directSelectCols} FROM customers WHERE company_id = $1 AND phone = ANY($2)`, [companyId, directPhoneList]);
            const directCustomerMap = new Map();
            directCustomersResult.rows.forEach((c) => {
                directCustomerMap.set((0, normalize_phone_1.normalizePhone)(c.phone), c);
            });
            ...
            if (directChannel === 'sms' || directChannel === 'both') {
                ...
                for (let i = 0; i < filteredRecipients.length; i++) {
                    ...
                    const cleanPhone = (0, normalize_phone_1.normalizePhone)(recipient.phone);
                    ...
                    const { message: finalMessage, subject: finalSubject } = (0, messageUtils_1.prepareSendMessage)(sanitizedMessage, dbCustomer, directFieldMappings, {
                    ...
                    if (isScheduledSend) {
                        if (splitEnabled && splitCount > 0) {
                            ...
                            sendTime = (0, sms_queue_1.toKoreaTimeStr)(calcSplitSendTime(new Date(scheduledAt), batchIndex));
                        }
                        else {
                            sendTime = (0, sms_queue_1.toKoreaTimeStr)(new Date(scheduledAt));
                        }
                    }
                    else if (splitEnabled && splitCount > 0) {
                        const batchIndex = Math.floor(i / splitCount);
                        sendTime = (0, sms_queue_1.toKoreaTimeStr)(calcSplitSendTime(new Date(), batchIndex));
                    }
                    else {
                        sendTime = ''; // useNow=true이면 bulkInsertSmsQueue에서 NOW() 사용
                    }
                    // ★ D103: resolveCustomerCallback 컨트롤타워
                    const recipientCallback = (0, callback_filter_1.resolveCustomerCallback)(recipient, useIndividualCallback, callback);
                    // ★ D124 N4: mmsImagePaths가 객체 배열({path, originalName}) 또는 문자열 배열 혼재 가능 → 정규화
                    const directMmsPaths = (0, mms_image_util_1.normalizeMmsImagePaths)(mmsImagePaths);
                    directSmsRows.push([
                        cleanPhone, recipientCallback, finalMessage,
                        (0, sms_queue_1.toQtmsgType)(msgType),
                        finalSubject, sendTime, campaignId, companyId,
                        directMmsPaths[0] || '', directMmsPaths[1] || '', directMmsPaths[2] || ''
                    ]);
                }
                directSmsSentCount = await (0, sms_queue_1.bulkInsertSmsQueue)(companyTables, directSmsRows, useNow);
            }
            ...
            if (directChannel === 'kakao' || directChannel === 'both') {
                for (let i = 0; i < filteredRecipients.length; i++) {
                    try {
                        ...
                        const cleanKakaoPhone = (0, normalize_phone_1.normalizePhone)(recipient.phone);
                        ...
                        const finalMessage = (0, messageUtils_1.replaceVariables)(sanitizedMessage, dbKakaoCustomer, directFieldMappings, {
                            name: recipient.name,
                            extra1: recipient.extra1,
                            extra2: recipient.extra2,
                            extra3: recipient.extra3,
                        ...
                        if (isScheduledSend) {
                            if (splitEnabled && splitCount > 0) {
                                const batchIndex = Math.floor(i / splitCount);
                                kakaoSendTime = (0, sms_queue_1.toKoreaTimeStr)(calcSplitSendTime(new Date(scheduledAt), batchIndex));
                            }
                            else {
                                kakaoSendTime = (0, sms_queue_1.toKoreaTimeStr)(new Date(scheduledAt));
                            }
                        }
                        // ★ D103: resolveCustomerCallback 컨트롤타워
                        const recipientCallback = (0, callback_filter_1.resolveCustomerCallback)(recipient, useIndividualCallback, callback);
                        await (0, sms_queue_1.insertKakaoQueue)({
                            ...
                            senderKey: kakaoSenderKey || '',
                            phone: (0, normalize_phone_1.normalizePhone)(recipient.phone),
            ...
            if (directChannel === 'sms' || directChannel === 'both') {
                ...
                if (smsFailCount > 0) {
                    ...
                    try {
                        ...
                        await (0, prepaid_1.prepaidRefund)(companyId, smsFailCount, smsDeductType, campaignId, `직접발송 SMS 부분실패 ${smsFailCount}건 환불`);
                    }
                    catch (refundErr) {
            ...
            if (directChannel === 'kakao' || directChannel === 'both') {
                ...
                if (kakaoFailCount > 0) {
                    ...
                    try {
                        await (0, prepaid_1.prepaidRefund)(companyId, kakaoFailCount, 'KAKAO', campaignId, `직접발송 카카오 부분실패 ${kakaoFailCount}건 환불`);
                    }
                    catch (refundErr) {
            ...
            if (directChannel === 'alimtalk') {
                ...
                const gateCheck = await (0, database_1.query)(`SELECT t.id AS tid,
                t.status AS tstatus,
                p.id AS pid,
                p.approval_status,
                p.profile_key
          ...
          LIMIT 1`, [companyId, alimtalkTemplateCode]);
                ...
                const etcObj = {};
                if (gate.profile_key)
                    etcObj.senderkey = gate.profile_key;
                ...
                const toExtraFromVarMap = (recipient, dbCustomer) => {
                    const out = {};
                    if (!alimtalkVariableMap || typeof alimtalkVariableMap !== 'object')
                        return out;
                ...
                const alimtalkRows = filteredRecipients.map((recipient) => {
                    const dbAlimCustomer = directCustomerMap.get((0, normalize_phone_1.normalizePhone)(recipient.phone)) || null;
                    const extraVars = toExtraFromVarMap(recipient, dbAlimCustomer);
                    // 템플릿 content를 #{변수} 치환 — QTmsg 알림톡은 본문이 필수
                    let finalMessage = (0, messageUtils_1.replaceVariables)(message, dbAlimCustomer, directFieldMappings, {
                    ...
                    return {
                        phone: (0, normalize_phone_1.normalizePhone)(recipient.phone),
                        callback: (0, normalize_phone_1.normalizePhone)(callback),
                        message: finalMessage,
                ...
                try {
                    directAlimtalkSentCount = await (0, sms_queue_1.insertAlimtalkQueue)(companyTables, alimtalkRows);
            ...
            if (!scheduled) {
                ...
                await (0, database_1.query)(`UPDATE campaigns SET status = $1, sent_count = $2, fail_count = $3, sent_at = NOW(), updated_at = NOW() WHERE id = $4`, [immediateStatus, directTotalSent, directFailTotal, campaignId]);
                // ★ D101: 직접발송도 campaign_runs INSERT (슈퍼관리자 캠페인 상세조회에서 데이터 필요)
                try {
                    await (0, database_1.query)(`INSERT INTO campaign_runs (campaign_id, run_number, target_count, sent_count, status, sent_at, created_at)
           VALUES ($1, 1, $2, $3, $4, NOW(), NOW())`, [campaignId, filteredRecipients.length, directTotalSent, immediateStatus]);
                }
                catch (runErr) {
            ...
            const directCompanyInfo = await (0, database_1.query)('SELECT name, brand_tone FROM companies WHERE id = $1', [companyId]);
            (0, training_logger_1.logTrainingData)({
            ...
            try {
                await (0, prepaid_1.prepaidRefund)(companyId, filteredRecipients.length, directDeductType, campaignId, '발송 전체 실패 자동 환불');
                await (0, database_1.query)(`UPDATE campaigns SET status = 'failed', updated_at = NOW() WHERE id = $1`, [campaignId]);
            }
            catch (refundErr) {
...
router.post('/:id/cancel', async (req, res) => {
    try {
        ...
        const result = await (0, campaign_lifecycle_1.cancelCampaign)(campaignId, companyId, {
            cancelledBy: userId,
...
router.get('/:id/recipients', async (req, res) => {
    try {
        ...
        // 캠페인 확인
        const campaign = await (0, database_1.query)(`SELECT * FROM campaigns WHERE id = $1 AND company_id = $2`, [campaignId, companyId]);
        ...
        const recipientTables = await (0, sms_queue_1.getCompanySmsTables)(companyId);
        if (camp.status === 'scheduled') {
            // 검색 조건
            const searchCondition = search ? ` AND dest_no LIKE ?` : '';
            const searchParams = search ? [campaignId, `%${(0, normalize_phone_1.normalizePhone)(String(search))}%`] : [campaignId];
            const mysqlRecipients = await (0, sms_queue_1.smsSelectAll)(recipientTables, 'seqno as idx, dest_no as phone, call_back as callback, msg_contents as message', `app_etc1 = ? AND status_code = 100${searchCondition}`, searchParams, `ORDER BY seqno LIMIT ${limit} OFFSET ${offset}`);
            // MySQL에 데이터 있으면 그걸 반환
            if (mysqlRecipients && (mysqlRecipients.length > 0 || offset > 0 || search)) {
                const totalCount = await (0, sms_queue_1.smsCountAll)(recipientTables, `app_etc1 = ? AND status_code = 100${searchCondition}`, searchParams);
                return res.json({
                    success: true,
                    campaign: camp,
        ...
        if (camp.status === 'scheduled' || camp.status === 'draft') {
            ...
            const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers c
         WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true
         ${filterQuery.where}${storeFilter}${searchFilter}${excludeFilter}
         AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdx} AND u.phone = c.phone)`, [companyId, ...filterQuery.params, ...storeParams, ...searchParams, ...excludeParams, userId]);
            const total = parseInt(countResult.rows[0].count);
            // 수신자 목록 (상위 10개)
            const limitIdx = unsubIdx + 1;
            const recipients = await (0, database_1.query)(`SELECT phone, name, phone as idx
         FROM customers c
         WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true
         ${filterQuery.where}${storeFilter}${searchFilter}${excludeFilter}
         AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdx} AND u.phone = c.phone)
         ORDER BY name, phone
         LIMIT $${limitIdx}`, [companyId, ...filterQuery.params, ...storeParams, ...searchParams, ...excludeParams, userId, 10]);
            return res.json({
                success: true,
                campaign: camp,
        ...
        const searchParams2 = search ? [campaignId, `%${(0, normalize_phone_1.normalizePhone)(String(search))}%`] : [campaignId];
        const recipients = await (0, sms_queue_1.smsSelectAll)(recipientTables, 'seqno as idx, dest_no as phone, call_back as callback, msg_contents as message, sendreq_time, status_code', `app_etc1 = ? AND status_code = 100${searchCondition2}`, searchParams2, `ORDER BY seqno LIMIT ${limit} OFFSET ${offset}`);
        const totalCount = await (0, sms_queue_1.smsCountAll)(recipientTables, `app_etc1 = ? AND status_code = 100${searchCondition2}`, searchParams2);
        res.json({
            success: true,
            campaign: camp,
...
// 예약 캠페인 개별 수신자 삭제
router.delete('/:id/recipients/:idx', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const campaignId = req.params.id;
        const phone = req.params.idx; // idx가 아니라 phone으로 사용
        // 캠페인 확인
        const campaign = await (0, database_1.query)(`SELECT * FROM campaigns WHERE id = $1 AND company_id = $2 AND status = 'scheduled'`, [campaignId, companyId]);
        if (campaign.rows.length === 0) {
        ...
        const delTables = await (0, sms_queue_1.getCompanySmsTables)(companyId);
        const mysqlCount = await (0, sms_queue_1.smsCountAll)(delTables, 'app_etc1 = ? AND status_code = 100', [campaignId]);
        if (mysqlCount > 0) {
            // 회사 테이블에서 삭제
            await (0, sms_queue_1.smsExecAll)(delTables, `DELETE FROM SMSQ_SEND WHERE app_etc1 = ? AND dest_no = ? AND status_code = 100`, [campaignId, phone]);
            const remainingCount = await (0, sms_queue_1.smsCountAll)(delTables, 'app_etc1 = ? AND status_code = 100', [campaignId]);
            await (0, database_1.query)(`UPDATE campaigns SET target_count = $1, updated_at = NOW() WHERE id = $2`, [remainingCount, campaignId]);
            return res.json({ success: true, message: '삭제되었습니다', remainingCount });
        }
        // MySQL에 없으면 excluded_phones에 추가
        await (0, database_1.query)(`UPDATE campaigns SET excluded_phones = array_append(excluded_phones, $1), target_count = target_count - 1, updated_at = NOW() WHERE id = $2`, [phone, campaignId]);
        const updated = await (0, database_1.query)(`SELECT target_count FROM campaigns WHERE id = $1`, [campaignId]);
...
router.put('/:id/reschedule', async (req, res) => {
    try {
        ...
        const rsCheck = (0, campaign_validation_1.validateScheduledAt)(scheduledAt, { allowNull: false, minMinutesFromNow: 15 });
        if (!rsCheck.valid) {
        ...
        const reschTables = await (0, sms_queue_1.getCompanySmsTables)(companyId);
        const currentMinTime = await (0, sms_queue_1.smsMinAll)(reschTables, 'sendreq_time', 'app_etc1 = ? AND status_code = 100', [campaignId]);
        ...
        if (currentMinTime) {
            ...
            await (0, sms_queue_1.smsExecAll)(reschTables, `UPDATE SMSQ_SEND SET sendreq_time = DATE_ADD(sendreq_time, INTERVAL ? SECOND) WHERE app_etc1 = ? AND status_code = 100`, [diffSeconds, campaignId]);
        }
        // PostgreSQL 캠페인 업데이트 (항상 실행)
        await (0, database_1.query)(`UPDATE campaigns SET scheduled_at = $1, updated_at = NOW() WHERE id = $2`, [new Date(scheduledAt), campaignId]);
...
router.put('/:id/message', async (req, res) => {
    try {
        ...
        const msgTables = await (0, sms_queue_1.getCompanySmsTables)(companyId);
        const recipients = await (0, sms_queue_1.smsSelectAll)(msgTables, 'seqno, dest_no', 'app_etc1 = ? AND status_code = 100', [campaignId]);
        // MySQL에 데이터 없으면 PostgreSQL만 업데이트 (예약 상태)
        if (recipients.length === 0) {
            await (0, database_1.query)(`UPDATE campaigns SET message_template = $1, message_subject = $2, message_content = $3, updated_at = NOW() WHERE id = $4`, 
        ...
        // ★ D102: prepareFieldMappings 컨트롤타워로 통합 (customer_schema 조회 + extractVarCatalog + enrichWithCustomFields)
        const editFieldMappings = await (0, messageUtils_1.prepareFieldMappings)(companyId);
        // ★ D32: 동적 컬럼 SELECT — fieldMappings에서 필요한 컬럼 자동 추출
        const editBaseColumns = ['phone', 'custom_fields'];
        // ★ storageType 기반 동적 필터 — 직접 컬럼만 SELECT, JSONB 내부 키는 custom_fields 컬럼에서 접근 (D72)
        const editMappingColumns = Object.values(editFieldMappings).filter((m) => m.storageType !== 'custom_fields').map((m) => m.column);
        const editSelectColumns = [...new Set([...editBaseColumns, ...editMappingColumns])].join(', ');
        const phones = recipients.map((r) => r.dest_no);
        const customersResult = await (0, database_1.query)(`SELECT ${editSelectColumns} FROM customers WHERE company_id = $1 AND phone = ANY($2)`, [companyId, phones]);
        // 전화번호 → 고객정보 맵
        ...
        const optOut080 = adEnabled ? await (0, messageUtils_1.getOpt080Number)(campUserId, companyId) : '';
        ...
        const batchSize = defaults_1.BATCH_SIZES.messageUpdate;
        let processedCount = 0;
        // Redis에 진행률 저장 (공유 인스턴스 사용)
        await defaults_1.redis.set(`message_edit:${campaignId}:progress`, JSON.stringify({
            total: recipients.length,
            processed: 0,
            percent: 0
        }), 'EX', defaults_1.CACHE_TTL.messageEditProgress);
        for (const [table, tableRecipients] of Object.entries(tableGroups)) {
            for (let i = 0; i < tableRecipients.length; i += batchSize) {
                ...
                for (const recipient of batch) {
                    ...
                    const { message: finalMessage, subject: finalSubject } = (0, messageUtils_1.prepareSendMessage)(sanitizedEditMessage, customer, editFieldMappings, {
        ...
        `;
                ...
                await (0, database_1.mysqlQuery)(updateQuery, []);
...
// 문안 수정 진행률 조회
router.get('/:id/message/progress', async (req, res) => {
    try {
        const campaignId = req.params.id;
        const data = await defaults_1.redis.get(`message_edit:${campaignId}:progress`);
        if (data) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // 캠페인 조회 — 소유권 확인
        const campResult = await (0, database_1.query)(`SELECT id, status, campaign_name, created_by, scheduled_at FROM campaigns WHERE id = $1 AND company_id = $2`, [campaignId, companyId]);
        ...
        await (0, database_1.query)(`DELETE FROM campaign_runs WHERE campaign_id = $1`, [campaignId]);
        await (0, database_1.query)(`DELETE FROM campaigns WHERE id = $1 AND company_id = $2`, [campaignId, companyId]);
...
// ============================================================
router.post('/brand-send', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        if (!companyId || !userId) {
        ...
        const { mode, // 'free' (자유형) | 'template' (기본형)
        bubbleType, // TEXT, IMAGE, WIDE, ...
        senderKey, targeting, isAd, phones, // string[]
        message, header, additionalContent, buttons, image, coupon, commerce, video, itemList, carouselHead, carouselItems, carouselTail, resendType, resendFrom, resendMessage, resendTitle, unsubscribePhone, unsubscribeAuth, reservedDate, 
        // 기본형(템플릿) 전용
        templateCode, messageVariableJson, buttonVariableJson, couponVariableJson, imageVariableJson, videoVariableJson, commerceVariableJson, carouselVariableJson, } = req.body;
        if (!phones || !Array.isArray(phones) || phones.length === 0) {
            return res.status(400).json({ error: '수신자 목록이 필요합니다' });
        }
        // 캠페인 레코드 생성
        const campaignResult = await (0, database_1.query)(`INSERT INTO campaigns (company_id, user_id, name, message_content, message_type, send_channel, status, created_at)
       VALUES ($1, $2, $3, $4, 'LMS', 'kakao_brand', 'sending', NOW())
       RETURNING id`, [companyId, userId, `브랜드메시지 ${bubbleType || 'TEXT'}`, message || `[${bubbleType}] 브랜드메시지`]);
        const campaignId = campaignResult.rows[0].id;
        const baseParams = {
        ...
        let result;
        if (mode === 'template') {
            const { sendBrandMessageTemplate } = await Promise.resolve().then(() => __importStar(require('../utils/brand-message')));
            ...
            const { sendBrandMessage } = await Promise.resolve().then(() => __importStar(require('../utils/brand-message')));
            result = await sendBrandMessage(baseParams);
        }
        if (!result.success) {
            // 실패 시 캠페인 상태 변경
            await (0, database_1.query)(`UPDATE campaigns SET status = 'failed' WHERE id = $1`, [campaignId]);
            return res.status(400).json({ error: result.error });
        }
        // 성공 시 캠페인 업데이트
        await (0, database_1.query)(`UPDATE campaigns SET status = 'completed', target_count = $1 WHERE id = $2`, [result.sentCount, campaignId]);
        // campaign_runs INSERT
        await (0, database_1.query)(`INSERT INTO campaign_runs (campaign_id, company_id, total_sent, total_success, sent_at)
       VALUES ($1, $2, $3, $3, NOW())`, [campaignId, companyId, result.sentCount]);
        return res.json({
            success: true,
            campaignId,
