...
router.get('/status', async (req, res) => {
    const status = (0, ai_1.checkAPIStatus)();
...
router.post('/generate-message', async (req, res) => {
    try {
        ...
        // 회사 정보 조회 (브랜드 정보 포함)
        const companyResult = await (0, database_1.query)('SELECT COALESCE(reject_number, opt_out_080_number) as reject_number, brand_name, brand_slogan, brand_description, brand_tone, customer_schema FROM companies WHERE id = $1', [companyId]);
        ...
        const fieldDefsForVars = await (0, database_1.query)(`SELECT field_key, field_label FROM customer_field_definitions WHERE company_id = $1 AND (is_hidden = false OR is_hidden IS NULL)`, [companyId]);
        ...
        await (0, ai_1.filterVarCatalogByData)(varCatalog, availableVars, companyId);
        // 타겟 정보 조회
        let targetQuery = 'SELECT COUNT(*) as total FROM customers WHERE company_id = $1 AND is_active = true AND sms_opt_in = true';
        const targetResult = await (0, database_1.query)(targetQuery, [companyId]);
       ...
       FROM customers WHERE company_id = $1 AND is_active = true`, [companyId]);
        const targetInfo = {
        ...
        let kakaoSenderKey;
        if (channel === '카카오') {
            const kakaoResult = await (0, database_1.query)('SELECT profile_key FROM kakao_sender_profiles WHERE company_id = $1 AND is_active = true LIMIT 1', [companyId]);
        ...
        const recentMsgResult = await (0, database_1.query)(`
      ...
      LIMIT 10
    `, [companyId]);
        const recentMessages = recentMsgResult.rows.map((r) => r.content);
        const extraContext = {
            productName,
            discountRate,
            eventName,
        ...
        const result = await (0, ai_1.generateMessages)(prompt, targetInfo, extraContext);
...
router.post('/recommend-target', async (req, res) => {
    try {
        ...
        {
            const ctx = await (0, plan_guard_1.loadPlanContext)(companyId);
            if (!ctx)
                return res.status(404).json({ error: '회사 정보를 찾을 수 없습니다.' });
            const check = (0, plan_guard_1.canUseFeature)(ctx, 'ai_messaging');
            if (!check.allowed) {
        ...
        // 회사 정보 조회 (스키마 포함)
        const companyResult = await (0, database_1.query)(`SELECT company_name, business_type, COALESCE(reject_number, opt_out_080_number) as reject_number, brand_name, customer_schema FROM companies WHERE id = $1::uuid`, [companyId]);
        const companyInfo = companyResult.rows[0] || {};
        ...
        if (userOpt080)
            companyInfo.reject_number = userOpt080;
        // 카카오 프로필 존재 여부 확인
        const kakaoProfileResult = await (0, database_1.query)('SELECT COUNT(*) FROM kakao_sender_profiles WHERE company_id = $1 AND is_active = true', [companyId]);
        const hasKakaoProfile = parseInt(kakaoProfileResult.rows[0].count) > 0;
        companyInfo.has_kakao_profile = hasKakaoProfile;
        ...
        AVG((custom_fields->>'total_spent')::numeric) as avg_total_spent
       FROM customers
       WHERE company_id = $1 AND is_active = true${storeFilter}`, [...baseParams, (0, normalize_1.getGenderVariants)('M'), (0, normalize_1.getGenderVariants)('F')]);
        const result = await (0, ai_1.recommendTarget)(companyId, objective, statsResult.rows[0], companyInfo);
        ...
        const filterResult = await (0, ai_1.countFilteredCustomers)(companyId, result.filters, userId, storeFilter, baseParams);
        ...
        if (actualCount === 0 && Object.keys(result.filters).length > 0 && autoRelaxRequested) {
            ...
            const premiumCtx = await (0, plan_guard_1.loadPlanContext)(companyId);
            const isPremium = premiumCtx ? (0, plan_guard_1.canUseFeature)(premiumCtx, 'ai_premium').allowed : false;
            if (isPremium) {
                console.log('[AI] 0명 매칭 → 자동 조건완화 시도 (프로 이상)');
                originalFilters = { ...result.filters };
                // 최대 2회 시도
                for (let attempt = 1; attempt <= 2; attempt++) {
                    const relaxResult = await (0, ai_1.relaxFilters)(result.filters, result.reasoning, statsResult.rows[0], '' // activeFieldsPrompt는 recommendTarget 내부에서 이미 사용됨
                    );
                    ...
                    let relaxCount;
                    try {
                        relaxCount = await (0, ai_1.countFilteredCustomers)(companyId, relaxResult.filters, userId, storeFilter, baseParams);
                    }
                    catch (relaxErr) {
        ...
        result.unsubscribe_count = unsubscribeCount;
        result.has_kakao_profile = hasKakaoProfile;
        result.auto_relaxed = autoRelaxed;
        if (autoRelaxed && originalFilters) {
            result.original_filters = originalFilters;
            result.relaxed_fields = relaxedFields;
        }
        // ★ D85: 샘플 고객 1명 조회
        ...
        const { sql: sampleFilterWhere, params: sampleFilterParams } = (0, customer_filter_1.buildFilterWhereClauseCompat)(result.filters, baseParams.length + 1);
        const sampleUnsubIdx = baseParams.length + sampleFilterParams.length + 1;
        let sampleCustomer = {};
        let sampleCustomerRaw = {};
        ...
        let sampleCustomersRaw = [];
        try {
            const sampleResult = await (0, database_1.query)(`SELECT name, gender, age, grade, points, email, address,
         ...
         ORDER BY c.updated_at DESC NULLS LAST LIMIT 100`, [...baseParams, ...sampleFilterParams, userId]);
            ...
            for (const row of sampleResult.rows) {
                ...
                for (const fk of Object.keys(standard_field_map_1.FIELD_DISPLAY_MAP)) {
                    if (rawRow[fk] != null)
                        rawRow[fk] = (0, standard_field_map_1.reverseDisplayValue)(fk, rawRow[fk]);
                }
            ...
            if (sampleResult.rows[0]) {
                ...
                for (const fk of Object.keys(standard_field_map_1.FIELD_DISPLAY_MAP)) {
                    if (sampleCustomerRaw[fk] != null) {
                        sampleCustomerRaw[fk] = (0, standard_field_map_1.reverseDisplayValue)(fk, sampleCustomerRaw[fk]);
                    }
                }
                // 표준 필드 → displayName 매핑 (프론트 미리보기용)
                for (const f of standard_field_map_1.FIELD_MAP) {
                    if (f.storageType === 'custom_fields' || f.fieldKey === 'phone' || f.fieldKey === 'sms_opt_in')
                        continue;
                    ...
                    if (val !== null && val !== undefined && val !== '') {
                        ...
                        if (standard_field_map_1.FIELD_DISPLAY_MAP[f.fieldKey]) {
                            sampleCustomer[f.displayName] = (0, standard_field_map_1.reverseDisplayValue)(f.fieldKey, val);
                        }
                            ...
                            sampleCustomer[f.displayName] = (0, messageUtils_1.formatDateValue)(val);
                        }
                        else {
                ...
                if (row.custom_fields && typeof row.custom_fields === 'object') {
                    const defResult = await (0, database_1.query)('SELECT field_key, field_label, field_type FROM customer_field_definitions WHERE company_id = $1', [companyId]);
        ...
        result.sample_customer = sampleCustomer;
        result.sample_customer_raw = sampleCustomerRaw;
        // ★ D142+ B4: 머지값 max byte 계산용 N명 sample (자동발송 미리보기 byte 정확도 보장)
        result.sample_customers_raw = sampleCustomersRaw;
        return res.json(result);
    }
    catch (error) {
...
// ============================================================
router.post('/recommend-next-campaign', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        if (!companyId) {
        ...
        // ★ CT-17: AI 프리미엄 게이팅 (PRO+)
        {
            ...
            if (!ctx)
                return res.status(404).json({ error: '회사 정보를 찾을 수 없습니다.' });
            const check = (0, plan_guard_1.canUseFeature)(ctx, 'ai_premium');
            if (!check.allowed) {
                return res.status(403).json({ error: check.errorMsg, code: check.errorCode });
            }
        }
        ...
        const performanceData = await (0, stats_aggregation_1.aggregateCampaignPerformance)(companyId, analysisMonths);
       ...
       FROM customers WHERE company_id = $1 AND is_active = true`, [companyId]);
        // 3) 회사 정보
        const companyResult = await (0, database_1.query)('SELECT company_name, business_type, brand_name FROM companies WHERE id = $1', [companyId]);
        // 4) AI 추천 — services/ai.ts 컨트롤타워
        const recommendation = await (0, ai_1.recommendNextCampaign)(companyId, performanceData, statsResult.rows[0], companyResult.rows[0]);
        return res.json({
            ...recommendation,
            performance_data: performanceData,
        });
...
// ============================================================
router.post('/recount-target', auth_1.authenticate, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        if (targetCondition.gender) {
            ...
            if (gKey)
                targetFilters.gender = gKey;
        }
        if (targetCondition.grade) {
            const grades = targetCondition.grade.split(/[,\/\s]+/).map((g) => g.trim().toUpperCase()).filter(Boolean);
            if (grades.length > 0)
                targetFilters.grade = { value: grades, operator: 'in' };
        }
        ...
        const { sql: filterSql, params: filterParams } = (0, customer_filter_1.buildFilterWhereClauseCompat)(targetFilters, baseParams.length + 1);
       ...
       AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdxB} AND u.phone = c.phone)`, [...baseParams, ...filterParams, userId]);
        const estimatedCount = parseInt(countResult.rows[0].count);
        const unsubResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers c
       WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true${storeFilter} ${filterSql}
       AND EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdxB} AND u.phone = c.phone)`, [...baseParams, ...filterParams, userId]);
...
router.post('/parse-briefing', auth_1.authenticate, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        const result = await (0, ai_1.parseBriefing)(briefing.trim(), companyId);
        // 사용자 매장 필터 (일반 사용자는 본인 store_codes만)
        ...
        const { sql: filterWhere, params: filterParams } = (0, customer_filter_1.buildFilterWhereClauseCompat)(targetFilters, baseParams.length + 1);
        // ★ B17-01: 수신거부 user_id 기준 통일
       ...
       AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdxC} AND u.phone = c.phone)`, [...baseParams, ...filterParams, userId]);
        const estimatedCount = parseInt(countResult.rows[0].count);
        const unsubResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers c
       WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true${storeFilter} ${filterWhere}
       AND EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdxC} AND u.phone = c.phone)`, [...baseParams, ...filterParams, userId]);
        const unsubscribeCount = parseInt(unsubResult.rows[0].count);
        // ★ D88: 타겟 필터에 맞는 샘플 고객 1명 반환 — 미리보기용
        // enabled-fields의 sample(타겟 무관)과 달리, 실제 타겟 고객에서 샘플링
        let sampleCustomer = {};
        try {
            const sampleResult = await (0, database_1.query)(`SELECT * FROM customers c
         WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true${storeFilter} ${filterWhere}
         AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubIdxC} AND u.phone = c.phone)
         ORDER BY c.updated_at DESC NULLS LAST LIMIT 1`, [...baseParams, ...filterParams, userId]);
            if (sampleResult.rows.length > 0) {
                ...
                if (row.custom_fields && typeof row.custom_fields === 'object') {
                    for (const [k, v] of Object.entries(row.custom_fields)) {
                        if (sampleCustomer[k] === undefined)
                            sampleCustomer[k] = v == null ? '' : String(v);
                    }
                }
            }
        }
        catch (e) {
            console.warn('[parse-briefing] 샘플 고객 조회 실패:', e);
        }
        return res.json({
            ...result,
            estimatedCount,
...
router.post('/generate-custom', auth_1.authenticate, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        if (!companyId) {
        ...
        // 회사 정보 조회
        const companyResult = await (0, database_1.query)('SELECT COALESCE(reject_number, opt_out_080_number) as reject_number, brand_name, brand_slogan, brand_description, brand_tone FROM companies WHERE id = $1', [companyId]);
        const companyInfo = companyResult.rows[0] || {};
        // ★ D102: getOpt080Number 컨트롤타워 사용 (인라인 조회 제거)
        const userOpt080 = await (0, messageUtils_1.getOpt080Number)(userId || null, companyId);
        if (userOpt080)
            companyInfo.reject_number = userOpt080;
        const result = await (0, ai_1.generateCustomMessages)({
            briefing,
            promotionCard,
            personalFields,
            fieldLabels,
