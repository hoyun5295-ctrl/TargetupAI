...
function getOnlineStatus(lastHeartbeatAt) {
    if (!lastHeartbeatAt)
        return 'offline';
    const diffMinutes = (Date.now() - new Date(lastHeartbeatAt).getTime()) / (1000 * 60);
    if (diffMinutes <= 70)
        return 'online'; // 1주기(60분) + 10분 여유
    if (diffMinutes <= 130)
        return 'delayed'; // 2주기(120분) + 10분 여유 — 한 번 놓침 허용
    return 'offline'; // 2주기 초과 = 확실한 이상
}
// ----------------------------------------------------------------------------
// GET /api/admin/sync/agents — Agent 목록
// ----------------------------------------------------------------------------
router.get('/agents', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const { rows: agents } = await (0, database_1.query)(`
      SELECT
        sa.id,
        sa.company_id,
        c.company_name,
    ...
    `);
        ...
        let errorCountMap = {};
        // 오늘 동기화 건수
        const { rows: todayRows } = await (0, database_1.query)(`
      ...
      GROUP BY agent_id
    `, [agentIds]);
        todayRows.forEach((r) => { todaySyncMap[r.agent_id] = r.cnt; });
        // 최근 24시간 에러 건수
        const { rows: errorRows } = await (0, database_1.query)(`
      SELECT agent_id, COUNT(*)::int as cnt
      FROM sync_logs
      ...
      GROUP BY agent_id
    `, [agentIds]);
        errorRows.forEach((r) => { errorCountMap[r.agent_id] = r.cnt; });
...
// ----------------------------------------------------------------------------
router.get('/agents/:agentId', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
    ...
    `, [agentId]);
        ...
        // 최근 동기화 로그 20건
        const { rows: recentLogs } = await (0, database_1.query)(`
      SELECT
    ...
    `, [agentId]);
        // 오늘 통계
        const { rows: statsRows } = await (0, database_1.query)(`
      SELECT
...
// ----------------------------------------------------------------------------
router.put('/agents/:agentId/config', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const { agentId } = req.params;
        ...
        // Agent 존재 확인
        const { rows } = await (0, database_1.query)('SELECT id, config FROM sync_agents WHERE id = $1', [agentId]);
        if (rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Agent를 찾을 수 없습니다.' });
        }
...
// ----------------------------------------------------------------------------
router.post('/agents/:agentId/command', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const { agentId } = req.params;
        const { type } = req.body;
        // ★ D131 후속(2026-04-21): pause/resume 추가 — 원격 동기화 중단/재개 지원
        //   pause: Agent 스케줄러 stop (heartbeat 유지), resume: 재개
        const ALLOWED_COMMAND_TYPES = ['full_sync', 'restart', 'pause', 'resume'];
        ...
        if (nextStatus) {
         ...
         SET config = jsonb_set(COALESCE(config, '{}'), '{commands}', $1::jsonb),
             status = $2,
             updated_at = NOW()
         WHERE id = $3`, [JSON.stringify(commands), nextStatus, agentId]);
        }
        else {
            await (0, database_1.query)(`UPDATE sync_agents SET config = jsonb_set(COALESCE(config, '{}'), '{commands}', $1::jsonb), updated_at = NOW() WHERE id = $2`, [JSON.stringify(commands), agentId]);
        }
        res.json({
            success: true,
...
// ----------------------------------------------------------------------------
router.delete('/agents/:agentId', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const { agentId } = req.params;
        const force = req.query.force === 'true';
        // Agent 존재 + heartbeat 최신성 확인
        const agentResult = await (0, database_1.query)(`SELECT id, agent_name, company_id, status, last_heartbeat_at,
              EXTRACT(EPOCH FROM (NOW() - last_heartbeat_at)) / 60 AS minutes_since_heartbeat
       FROM sync_agents WHERE id = $1`, [agentId]);
        ...
        const deleteResult = await (0, database_1.query)(`DELETE FROM sync_agents WHERE id = $1 RETURNING id, agent_name`, [agentId]);
...
// ----------------------------------------------------------------------------
router.get('/agents/:agentId/logs', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const { agentId } = req.params;
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const limit = Math.min(100, Math.max(1, parseInt(req.query.limit) || 20));
        const syncType = req.query.sync_type;
        const offset = (page - 1) * limit;
        // Agent 존재 확인
        const agentCheck = await (0, database_1.query)('SELECT id FROM sync_agents WHERE id = $1', [agentId]);
        ...
        // 총 건수
        const { rows: countRows } = await (0, database_1.query)(`SELECT COUNT(*)::int as total FROM sync_logs ${whereClause}`, params);
        // 로그 조회
        const { rows: logs } = await (0, database_1.query)(`SELECT
        id, sync_type, mode, batch_index, total_batches,
        total_count, success_count, fail_count,
        duration_ms, error_message,
...
//# sourceMappingURL=admin-sync.js.map