...
// ============================================================
async function getAnalysisLevel(companyId) {
...
async function getCompanyName(companyId) {
    const result = await (0, database_1.query)(`SELECT company_name FROM companies WHERE id = $1`, [companyId]);
...
// 프롬프트: 프로 분석 (1회 호출)
// ============================================================
function buildProPrompt(collectedData) {
...
// 프롬프트: 비즈니스 분석 (3회 호출)
// ============================================================
function buildBusinessPrompt1(collectedData) {
...
function buildBusinessPrompt2(collectedData) {
...
function buildBusinessPrompt3(turn1Insights, turn2Insights, collectedData) {
...
// ============================================================
async function callClaude(userMessage, maxRetries = 2) {
    // 1차: Claude 재시도
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            const response = await anthropic.messages.create({
                model: defaults_1.AI_MODELS.claude,
                max_tokens: defaults_1.AI_MAX_TOKENS.analysis,
                temperature: 0.3,
            ...
            if (attempt < maxRetries) {
                await new Promise(resolve => setTimeout(resolve, defaults_1.TIMEOUTS.aiRetryDelay));
            }
        }
    }
    // 2차: gpt-5.1 fallback
    ...
    try {
        const gptResponse = await openai.chat.completions.create({
            ...
            max_completion_tokens: defaults_1.AI_MAX_TOKENS.analysis,
            temperature: 0.3,
            messages: [
...
router.get('/preview', async (req, res) => {
    try {
        ...
        // ── 기본 4개 필드 (모든 요금제) ──
        // 1) 최근 30일 캠페인 수
        const campaignCountResult = await (0, database_1.query)(`
    ...
    `, [companyId]);
        ...
        const sentCountResult = await (0, database_1.query)(`
      SELECT
    ...
    `, [companyId]);
        // 3) 평균 성공률
        const successRateResult = await (0, database_1.query)(`
      SELECT 
        CASE 
    ...
    `, [companyId]);
        // 4) 전체 활성 고객 수
        const customerCountResult = await (0, database_1.query)(`
    ...
    `, [companyId]);
        ...
        if (analysisLevel !== 'none') {
            // 5) 최적 발송 시간대 (성공률 기준)
            const bestTimeResult = await (0, database_1.query)(`
        SELECT 
      ...
      `, [companyId]);
            ...
            // 6) 최고 성과 요일
            const bestDayResult = await (0, database_1.query)(`
        SELECT 
          EXTRACT(DOW FROM cr.sent_at) as dow,
          ROUND(SUM(cr.success_count)::numeric / NULLIF(SUM(cr.sent_count), 0) * 100, 1) as success_rate
        FROM campaign_runs cr
      ...
      `, [companyId]);
            // 7) TOP 캠페인 (성공률 기준)
            const topCampaignResult = await (0, database_1.query)(`
        SELECT 
          c.campaign_name,
          ROUND(SUM(cr.success_count)::numeric / NULLIF(SUM(cr.sent_count), 0) * 100, 1) as success_rate
        FROM campaign_runs cr
      ...
      `, [companyId]);
            ...
            const unsubCountResult = await (0, database_1.query)(`
      ...
      `, [userId]);
            ...
            const churnResult = await (0, database_1.query)(`
      ...
      `, [companyId]);
            ...
            const companyForCost = await (0, database_1.query)('SELECT cost_per_sms, cost_per_lms, cost_per_mms, cost_per_kakao FROM companies WHERE id = $1', [companyId]);
            const costs = (0, defaults_1.getCompanyCosts)(companyForCost.rows[0] || {});
            const sentRow = sentCountResult.rows[0];
            const estimatedCost = parseInt(sentRow.sms_sent) * costs.sms +
...
router.post('/run', async (req, res) => {
    try {
        ...
        if (!forceRefresh) {
            const cached = await (0, database_1.query)(`
      ...
      `, [companyId, analysisLevel, dateFrom, dateTo]);
        ...
        // 1) 캠페인 성과 요약
        const campaignSummary = await (0, database_1.query)(`
      SELECT 
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        // 2) 채널별 성과
        const channelStats = await (0, database_1.query)(`
      SELECT 
        cr.message_type,
        COUNT(*) as run_count,
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        // 3) 요일별 성과
        const dayOfWeekStats = await (0, database_1.query)(`
      SELECT 
        EXTRACT(DOW FROM cr.sent_at) as dow,
        SUM(cr.sent_count) as sent,
        SUM(cr.success_count) as success,
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        // 4) 시간대별 성과
        const hourStats = await (0, database_1.query)(`
      SELECT 
        EXTRACT(HOUR FROM cr.sent_at) as hour,
        SUM(cr.sent_count) as sent,
        SUM(cr.success_count) as success,
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        const heatmapData = await (0, database_1.query)(`
      SELECT
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        // 5) 고객 분포 (성별/등급)
        const genderDist = await (0, database_1.query)(`
      ...
      ORDER BY count DESC
    `, [companyId]);
        const gradeDist = await (0, database_1.query)(`
    ...
    `, [companyId]);
        ...
        const unsubTrend = await (0, database_1.query)(`
      SELECT
    ...
    `, [userId, dateFrom, dateTo]);
        ...
        // 7) TOP 5 캠페인
        const topCampaigns = await (0, database_1.query)(`
      SELECT 
        c.campaign_name,
        c.message_type,
        SUM(cr.sent_count) as sent,
    ...
    `, [companyId, dateFrom, dateTo]);
        ...
        if (analysisLevel === 'advanced') {
            // 8) 개별 캠페인 상세 (최근 20건)
            const campaignDetails = await (0, database_1.query)(`
        SELECT 
          c.campaign_name,
          c.message_type,
          c.send_type,
      ...
      `, [companyId, dateFrom, dateTo]);
            ...
            // 9) 이탈 위험 고객 TOP 20
            const churnRisk = await (0, database_1.query)(`
        SELECT 
          cu.name,
          cu.phone,
          cu.grade,
      ...
      `, [companyId]);
            ...
            // 10) RFM 세그먼트 분석용 데이터
            const rfmData = await (0, database_1.query)(`
        SELECT 
          cu.grade,
      ...
      `, [companyId]);
            ...
            // 11) 구매 전환 분석
            const conversionData = await (0, database_1.query)(`
        SELECT 
          c.campaign_name,
          cr.sent_at,
          cr.sent_count,
      ...
      `, [companyId, dateFrom, dateTo]);
        ...
        if (analysisLevel === 'basic') {
            ...
            // level 강제 설정
            allInsights = allInsights.map(i => ({ ...i, level: 'basic' }));
        }
            ...
            allInsights = [
                ...turn1Insights.map(i => ({ ...i, level: 'advanced' })),
                ...turn2Insights.map(i => ({ ...i, level: 'advanced' })),
                ...turn3Insights.map(i => ({ ...i, level: 'advanced' })),
            ];
        }
        // ── 캐시 저장 (UPSERT) ──
        const saveResult = await (0, database_1.query)(`
...
router.get('/pdf', async (req, res) => {
    ...
    `, [analysisId, companyId]);
        ...
        // 인사이트 페이지들
        // ============================
        const renderInsight = (insight, idx) => {
            ...
            if (insight.keyMetrics && insight.keyMetrics.length > 0) {
                ...
                insight.keyMetrics.forEach((m, mi) => {
                    if (mi % 2 === 0)
                        doc.rect(50, y, pageWidth, 18).fill('#fafafa');
                    setFont(false);
