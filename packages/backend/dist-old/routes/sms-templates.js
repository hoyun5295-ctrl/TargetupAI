...
function filterExistingImagePaths(rawPaths) {
    if (!rawPaths)
        return [];
    let paths = rawPaths;
       ...
       LIMIT 50`, [companyId, userId]);
        ...
        const templates = await Promise.all(result.rows.map(async (t) => {
            ...
            if (validPaths.length !== originalPaths.length) {
                try {
                    await (0, database_1.query)(`UPDATE sms_templates SET mms_image_paths = $1, updated_at = NOW() WHERE id = $2`, [validPaths.length > 0 ? JSON.stringify(validPaths) : null, t.id]);
                }
                catch (e) {
...
router.post('/', async (req, res) => {
    try {
        ...
        // 동일 이름 중복 체크
        const existing = await (0, database_1.query)(`SELECT id FROM sms_templates WHERE company_id = $1 AND user_id = $2 AND template_name = $3`, [companyId, userId, templateName]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO sms_templates (id, company_id, user_id, template_name, message_type, subject, content, mms_image_paths, is_ad, created_at, updated_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())
       RETURNING id, template_name, message_type, subject, content, mms_image_paths, is_ad, created_at`, [companyId, userId, templateName, messageType || 'SMS', subject || null, content, mmsImagePaths ? JSON.stringify(mmsImagePaths) : null, isAdValue]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(403).json({ error: '권한이 없습니다.' });
        const { id } = req.params;
        const result = await (0, database_1.query)(`DELETE FROM sms_templates WHERE id = $1 AND company_id = $2 AND user_id = $3 RETURNING id`, [id, companyId, userId]);
