...
router.get('/dashboard', async (req, res) => {
    try {
        const [companies, users, campaigns, customers] = await Promise.all([
            (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_companies WHERE deleted_at IS NULL`),
            (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_users WHERE deleted_at IS NULL`),
            ...
            (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_customers WHERE deleted_at IS NULL`),
        ]);
        return res.json({
...
router.get('/companies', async (req, res) => {
    try {
        ...
        const countRes = await (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_companies WHERE ${where}`, params);
        params.push(limit, offset);
        const listRes = await (0, database_1.query)(`SELECT id, company_name, business_type, owner_name, owner_phone,
...
router.post('/companies', async (req, res) => {
    try {
        const { company_name, business_type, business_number, owner_name, owner_phone, address, store_hours, pos_type, 
        // ★ D114: 사업자등록증 + 세금계산서 전체 필드
        business_reg_name, business_reg_owner, business_category, business_item, business_address, tax_email, tax_manager_name, tax_manager_phone, admin_login_id, admin_password, admin_name, admin_email, } = req.body;
        if (!company_name || !admin_login_id || !admin_password) {
            return res.status(400).json({ error: '회사명, 관리자 아이디, 비밀번호는 필수입니다' });
        }
        // ★ D114: 총판명 중복 방지
        const dupCheck = await (0, database_1.query)('SELECT id FROM flyer_companies WHERE company_name = $1 AND deleted_at IS NULL', [company_name]);
        if (dupCheck.rows.length > 0) {
            return res.status(400).json({ error: `"${company_name}" 총판이 이미 존재합니다.` });
        }
        // 1. 회사 생성
        const companyRes = await (0, database_1.query)(`INSERT INTO flyer_companies
       ...
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8,
               ...
               NOW())
       RETURNING id`, [company_name, business_type || 'mart', business_number || null,
        ...
        await (0, database_1.query)(`INSERT INTO flyer_users (id, company_id, login_id, email, password_hash, name, role, created_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, 'flyer_admin', NOW())`, [companyId, admin_login_id, admin_email || '', hash, admin_name || owner_name || '관리자']);
...
router.get('/companies/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)(`SELECT * FROM flyer_companies WHERE id = $1 AND deleted_at IS NULL`, [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
        const users = await (0, database_1.query)(`SELECT id, email, name, role, last_login_at, created_at
       FROM flyer_users WHERE company_id = $1 AND deleted_at IS NULL ORDER BY created_at`, [id]);
        const customers = await (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_customers WHERE company_id = $1 AND deleted_at IS NULL`, [id]);
        return res.json({
            ...result.rows[0],
            users: users.rows,
...
router.put('/companies/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE flyer_companies SET ${sets.join(', ')} WHERE id = $1`, params);
...
router.delete('/companies/:id', async (req, res) => {
    try {
        await (0, database_1.query)(`UPDATE flyer_companies SET deleted_at = NOW() WHERE id = $1`, [req.params.id]);
...
router.get('/users', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT u.id, u.email, u.name, u.role, u.last_login_at, u.created_at,
              c.company_name
       FROM flyer_users u
...
router.post('/users', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`INSERT INTO flyer_users (id, company_id, login_id, email, password_hash, name, role, created_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, NOW())
       RETURNING id, login_id, name, role`, [company_id, login_id, email || '', hash, name || null, role || 'flyer_staff']);
...
router.post('/users/:id/reset-password', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE flyer_users SET password_hash = $1, updated_at = NOW() WHERE id = $2`, [hash, req.params.id]);
...
router.get('/stores', async (req, res) => {
    try {
        ...
        if (businessType) {
            ...
            where += ` AND u.business_type = $${params.length}`;
        }
        if (paymentStatus) {
            params.push(paymentStatus);
            where += ` AND u.payment_status = $${params.length}`;
        }
        if (search) {
            params.push(`%${search}%`);
            where += ` AND (u.store_name ILIKE $${params.length} OR u.name ILIKE $${params.length} OR u.login_id ILIKE $${params.length})`;
        }
        const countRes = await (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_users u WHERE ${where}`, params);
        params.push(limit, offset);
        const listRes = await (0, database_1.query)(`SELECT u.id, u.login_id, u.name, u.store_name, u.business_type,
...
router.post('/stores', async (req, res) => {
    try {
        const { company_id, login_id, password, name, phone, role, business_type, store_name, 
        // 사업자등록증
        business_number, business_reg_name, business_reg_owner, business_category, business_item, business_address, 
        // 세금계산서
        tax_email, tax_manager_name, tax_manager_phone, 
        // 담당자
        ...
        // 총판 존재 확인
        const companyCheck = await (0, database_1.query)(`SELECT id FROM flyer_companies WHERE id = $1 AND deleted_at IS NULL`, [company_id]);
        if (companyCheck.rows.length === 0) {
            return res.status(400).json({ error: '존재하지 않는 총판입니다' });
        }
        const hash = await bcryptjs_1.default.hash(password, 10);
        const result = await (0, database_1.query)(`INSERT INTO flyer_users (
...
router.get('/stores/:id', async (req, res) => {
    try {
        const result = await (0, database_1.query)(`SELECT u.*, c.company_name
       FROM flyer_users u
       JOIN flyer_companies c ON c.id = u.company_id
...
router.put('/stores/:id', async (req, res) => {
    try {
        ...
        if (sets.length === 0)
            return res.status(400).json({ error: 'Nothing to update' });
        sets.push(`updated_at = NOW()`);
        await (0, database_1.query)(`UPDATE flyer_users SET ${sets.join(', ')} WHERE id = $1 AND deleted_at IS NULL`, params);
        return res.json({ message: '수정되었습니다' });
    }
    catch (error) {
        return res.status(500).json({ error: 'Server error' });
...
router.post('/stores/:id/activate', async (req, res) => {
    try {
        ...
        if (chargeAmount <= 0)
            return res.status(400).json({ error: '입금 금액을 입력해주세요' });
        const result = await (0, database_1.query)(`UPDATE flyer_users
       SET prepaid_balance = prepaid_balance + $1, updated_at = NOW()
       WHERE id = $2 AND deleted_at IS NULL
       RETURNING id, store_name, prepaid_balance, payment_status`, [chargeAmount, req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
 ...
 */
router.post('/stores/:id/charge', async (req, res) => {
    try {
        const { amount } = req.body;
        const chargeAmount = parseInt(String(amount || '0'), 10);
        if (chargeAmount <= 0)
            return res.status(400).json({ error: '충전 금액은 1원 이상' });
        const result = await (0, database_1.query)(`UPDATE flyer_users
       SET prepaid_balance = prepaid_balance + $1, updated_at = NOW()
       WHERE id = $2 AND deleted_at IS NULL
       RETURNING id, store_name, prepaid_balance`, [chargeAmount, req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
...
router.get('/business-types', async (_req, res) => {
    try {
        const { getAllBusinessTypes } = await Promise.resolve().then(() => __importStar(require('../../utils/flyer/config/flyer-business-types')));
...
// ══════════════════════════════════════════
router.get('/pos-agents', async (req, res) => {
    try {
        const result = await (0, database_1.query)(`SELECT pa.*, c.company_name
...
router.post('/pos-agents/generate-key', async (req, res) => {
    try {
        const { company_id, pos_type } = req.body;
        if (!company_id)
            return res.status(400).json({ error: 'company_id 필수' });
        ...
        const result = await (0, database_1.query)(`INSERT INTO flyer_pos_agents (id, company_id, agent_key, pos_type, sync_status, created_at)
       VALUES (gen_random_uuid(), $1, $2, $3, 'disconnected', NOW())
       RETURNING id, agent_key`, [company_id, agentKey, pos_type || null]);
        // flyer_companies에도 agent_key/pos_type 업데이트
        await (0, database_1.query)(`UPDATE flyer_companies SET pos_agent_key = $1, pos_type = COALESCE($2, pos_type) WHERE id = $3`, [agentKey, pos_type, company_id]);
        return res.status(201).json(result.rows[0]);
    }
    catch (error) {
        return res.status(500).json({ error: 'Server error' });
...
router.get('/billing', async (req, res) => {
    try {
        ...
        const params = companyId ? [companyId] : [];
        const result = await (0, database_1.query)(`SELECT bh.*, c.company_name
...
router.get('/audit-logs', async (req, res) => {
    try {
        ...
        const result = await (0, flyer_audit_log_1.queryFlyerAuditLogs)({
        ...
        return res.json({ ...result, actionLabels: flyer_audit_log_1.AUDIT_ACTION_LABELS });
    }
    catch (error) {
...
//# sourceMappingURL=flyer-admin.js.map