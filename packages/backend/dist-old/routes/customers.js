...
const standard_field_map_1 = require("../utils/standard-field-map");
...
const sync_active_check_1 = require("../middlewares/sync-active-check");
const customer_upsert_1 = require("../utils/customer-upsert");
...
router.get('/', async (req, res) => {
    try {
        ...
        if (filterUserId && (userType === 'company_admin' || userType === 'super_admin')) {
            const fuResult = await (0, database_1.query)('SELECT store_codes FROM users WHERE id = $1 AND company_id = $2', [filterUserId, companyId]);
        ...
        const filterStoreCode = req.query.filterStoreCode;
        ...
        if (filters) {
            ...
            const filterResult = (0, customer_filter_1.buildDynamicFilterCompat)(parsedFilters, paramIndex);
        ...
        // 개별 파라미터 필터 (기존 방식 호환)
        if (gender) {
            const gf = (0, normalize_1.buildGenderFilter)(String(gender), paramIndex);
            whereClause += gf.sql;
        ...
        // 총 개수
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers_unified ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].count);
        // 목록 조회 — ★ B17-01: 수신거부 user_id 기준 통일
        const unsubCaseIdx = paramIndex++;
        params.push(userId);
        params.push(Number(limit), offset);
        const result = await (0, database_1.query)(`SELECT id, name, phone, gender, TO_CHAR(birth_date, 'YYYY-MM-DD') as birth_date, age, email, address, grade, region, points,
...
router.get('/download', async (req, res) => {
    try {
        ...
        listParams.push(userId);
        const { selectExpr } = (0, enabled_fields_1.buildDynamicSelectExpr)(fields, {
            unsubParamIndex,
        ...
        const result = await (0, database_1.query)(`SELECT ${selectExpr}
         FROM customers_unified
        WHERE ${listWhere}
        ORDER BY created_at DESC`, listParams);
        ...
        const rows = result.rows.map((c) => {
            ...
            for (const f of fields) {
                ...
                row[label] = (0, standard_field_map_1.renderFieldValue)(val, f.field_key);
            }
            return row;
        });
        // ─── 6. XLSX 생성 ───
...
router.post('/filter', async (req, res) => {
    try {
        ...
        if (filters) {
            const filterResult = (0, customer_filter_1.buildDynamicFilterCompat)(filters, paramIndex);
            whereClause += filterResult.where;
            params.push(...filterResult.params);
        }
        // ★ B17-01: 수신거부 user_id 기준 통일
        params.push(userId);
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM customers_unified c ${whereClause}
       AND NOT EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${params.length} AND u.phone = c.phone)`, params);
        return res.json({
...
router.get('/fields', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT DISTINCT jsonb_object_keys(custom_fields) as field_name
       FROM customers
...
router.post('/', sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        if (!companyId) {
            return res.status(403).json({ error: '회사 권한이 필요합니다' });
        }
        const { phone, name, gender, birthDate, email, address, grade, points, storeCode, storeName, smsOptIn, customFields } = req.body;
        if (!phone) {
            return res.status(400).json({ error: '전화번호는 필수입니다' });
        }
        // 요금제 제한 체크
        const limitCheck = await (0, database_1.query)(`
      SELECT 
        c.id,
        p.max_customers,
        p.plan_name,
    ...
    `, [companyId]);
        ...
        const manualUpsertBuilder = (0, customer_upsert_1.createCustomerUpsertBuilder)({
            source: 'manual',
        ...
        const result = await (0, database_1.query)(manualSql, manualValues);
        // customer_stores N:N 매핑 (store_code가 있을 때)
        const customerId = result.rows[0]?.id;
        if (customerId && storeCode) {
            await (0, database_1.query)(`INSERT INTO customer_stores (company_id, customer_id, store_code)
         VALUES ($1, $2, $3)
         ON CONFLICT (customer_id, store_code) DO NOTHING`, [companyId, customerId, storeCode]);
        }
...
router.post('/bulk', sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        if (!companyId) {
            return res.status(403).json({ error: '회사 권한이 필요합니다' });
    ...
    `, [companyId]);
        ...
        const bulkUpsertBuilder = (0, customer_upsert_1.createCustomerUpsertBuilder)({
            source: 'manual',
            includeUploadedBy: false,
        });
        const bulkRows = [];
        ...
        if (bulkRows.length > 0) {
            try {
                ...
                await (0, database_1.query)(bulkSql, bulkValues);
...
router.get('/stats', async (req, res) => {
    try {
        ...
        try {
            const cached = await defaults_1.redis.get(cacheKey);
            if (cached)
       ...
       WHERE c.company_id = $1 AND c.is_active = true${storeFilter}`, [...params, (0, normalize_1.getGenderVariants)('M'), (0, normalize_1.getGenderVariants)('F')]);
        // 회사 요금 정보 조회
        const companyResult = await (0, database_1.query)(`SELECT monthly_budget, cost_per_sms, cost_per_lms, cost_per_mms, cost_per_kakao, use_db_sync, use_file_upload
       FROM companies WHERE id = $1`, [companyId]);
        ...
        const monthlySendFilterParams = userType === 'company_user' && userId ? [companyId, userId] : [companyId];
        const monthlyMetaResult = await (0, database_1.query)(`SELECT c.id, c.company_id, c.created_by, c.message_type
       FROM campaigns c
       WHERE c.company_id = $1
         AND c.status NOT IN ('cancelled', 'draft', 'scheduled')
         AND c.created_at >= date_trunc('month', (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Seoul'))::date::timestamp AT TIME ZONE 'Asia/Seoul'${monthlySendFilterClause}`, monthlySendFilterParams);
        const monthlySmsMap = await (0, stats_aggregation_1.aggregateSmsCountsByCampaign)(monthlyMetaResult.rows);
        const monthlyKakaoMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(monthlyMetaResult.rows.map((c) => c.id));
        ...
        const costSms = parseFloat(company.cost_per_sms) || defaults_1.DEFAULT_COSTS.sms;
        const costLms = parseFloat(company.cost_per_lms) || defaults_1.DEFAULT_COSTS.lms;
        const costMms = parseFloat(company.cost_per_mms) || defaults_1.DEFAULT_COSTS.mms;
        const costKakao = parseFloat(company.cost_per_kakao) || defaults_1.DEFAULT_COSTS.kakao;
        let monthlyCost = smsSent * costSms +
            lmsSent * costLms +
            mmsSent * costMms +
            kakaoSent * costKakao;
        ...
        try {
            ...
            const planResult = await (0, database_1.query)(`SELECT p.plan_code FROM companies c JOIN plans p ON c.plan_id = p.id WHERE c.id = $1`, [companyId]);
            ...
            if (!isProOrAbove) {
                ...
                const testCostParams = userType === 'company_user' && userId ? [companyId, userId] : [companyId];
                const testCostResult = await (0, database_1.query)(`SELECT COALESCE(SUM(amount), 0)::numeric as total
           FROM balance_transactions
           WHERE company_id = $1
             AND type = 'deduct'
             AND reference_id = '00000000-0000-0000-0000-000000000000'
             AND created_at >= date_trunc('month', (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Seoul'))::date::timestamp AT TIME ZONE 'Asia/Seoul'${testCostFilter}`, testCostParams);
                ...
                const sfCostParams = userType === 'company_user' && userId ? [companyId, userId] : [companyId];
                const sfCostResult = await (0, database_1.query)(`SELECT COALESCE(SUM(bt.amount), 0)::numeric as total
           FROM balance_transactions bt
           WHERE bt.company_id = $1
             AND bt.type = 'deduct'
             AND bt.reference_id IN (SELECT id FROM spam_filter_tests WHERE company_id = $1)
             AND bt.created_at >= date_trunc('month', (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Seoul'))::date::timestamp AT TIME ZONE 'Asia/Seoul'${sfCostFilter}`, sfCostParams);
        ...
        const responseData = {
            stats: {
                ...
                cost_per_sms: parseFloat(company.cost_per_sms) || defaults_1.DEFAULT_COSTS.sms,
                cost_per_lms: parseFloat(company.cost_per_lms) || defaults_1.DEFAULT_COSTS.lms,
                cost_per_mms: parseFloat(company.cost_per_mms) || defaults_1.DEFAULT_COSTS.mms,
                cost_per_kakao: parseFloat(company.cost_per_kakao) || defaults_1.DEFAULT_COSTS.kakao,
        ...
        // ★ Redis 캐시 저장 (60초)
        try {
            await defaults_1.redis.setex(cacheKey, defaults_1.CACHE_TTL.customerStats, JSON.stringify(responseData));
        }
        catch (e) { /* 캐시 실패 무시 */ }
...
router.get('/schema', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)('SELECT customer_schema FROM companies WHERE id = $1', [companyId]);
        ...
        res.json({
            ...
            phoneFields: ['phone', 'mobile', 'phone_number', 'tel', 'cell_phone'].filter(f => defaultFields.some(df => df.name === f) || fields.some(cf => cf.name === f))
        });
    }
    catch (error) {
...
router.post('/filter-count', async (req, res) => {
    try {
        ...
        if (dynamicFilters && typeof dynamicFilters === 'object' && Object.keys(dynamicFilters).length > 0) {
            ...
            const df = (0, customer_filter_1.buildDynamicFilterCompat)(dynamicFilters, paramIndex);
            ...
            if (region) {
                const regionResult = (0, normalize_1.buildRegionFilter)(String(region), paramIndex);
        ...
        const result = await (0, database_1.query)(`SELECT COUNT(*) FROM customers ${whereClause}`, params);
...
router.post('/extract', async (req, res) => {
    try {
        ...
        const planCheck = await (0, database_1.query)(`SELECT p.customer_db_enabled FROM companies c
       LEFT JOIN plans p ON c.plan_id = p.id
       WHERE c.id = $1`, [companyId]);
        ...
        const columnFields = (0, standard_field_map_1.getColumnFields)();
        const selectParts = columnFields.map(f => {
            // phone은 phoneField 파라미터에 따라 별칭 처리
            if (f.columnName === 'phone')
                return `${phoneColumn} as phone`;
            return f.columnName;
        });
        ...
        const result = await (0, database_1.query)(`SELECT ${selectClause}
      FROM customers
      ${whereClause}
      ORDER BY created_at DESC`, params);
        ...
        const flatRecipients = result.rows.map((r) => {
            let flat;
            if (r.custom_fields && typeof r.custom_fields === 'object') {
                const { custom_fields, ...rest } = r;
                const customFlat = Object.fromEntries(Object.entries(custom_fields).map(([k, v]) => [k, v == null ? '' : String(v)]));
                flat = { ...rest, ...customFlat };
            }
            else {
                flat = { ...r };
            }
            for (const fk of Object.keys(standard_field_map_1.FIELD_DISPLAY_MAP)) {
                if (flat[fk] != null) {
                    flat[fk] = (0, standard_field_map_1.reverseDisplayValue)(fk, flat[fk]);
                }
            }
            return flat;
        });
...
router.get('/filter-options', async (req, res) => {
    try {
        ...
        const gendersResult = await (0, database_1.query)(`SELECT DISTINCT gender FROM customers WHERE ${scopeWhere} AND gender IS NOT NULL AND gender != '' ORDER BY gender`, scopeParams);
        const gradesResult = await (0, database_1.query)(`SELECT DISTINCT grade FROM customers WHERE ${scopeWhere} AND grade IS NOT NULL AND grade != '' ORDER BY grade`, scopeParams);
        const regionsResult = await (0, database_1.query)(`SELECT DISTINCT region FROM customers WHERE ${scopeWhere} AND region IS NOT NULL AND region != '' ORDER BY region`, scopeParams);
        // ★ 브랜드(store_code) 목록 — 고객사관리자/슈퍼관리자가 브랜드 필터에 사용
        let storeCodes = [];
        if (userType === 'company_admin' || userType === 'super_admin') {
            const storeResult = await (0, database_1.query)(`SELECT DISTINCT store_code FROM customer_stores WHERE company_id = $1 AND store_code IS NOT NULL AND store_code != '' ORDER BY store_code`, [companyId]);
            storeCodes = storeResult.rows.map((r) => r.store_code);
        }
        res.json({
            genders: gendersResult.rows.map((r) => r.gender),
            grades: gradesResult.rows.map((r) => r.grade),
...
router.get('/enabled-fields', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        const { fields } = await (0, enabled_fields_1.detectEnabledFields)({ companyId, scopeWhere, scopeParams });
        ...
        for (const f of fields) {
            if (f.is_custom || f.data_type !== 'string' || HIGH_CARDINALITY.includes(f.field_key))
                continue;
            const mapped = (0, standard_field_map_1.getFieldByKey)(f.field_key);
            const col = mapped?.columnName || f.field_key;
            try {
                const optResult = await (0, database_1.query)(`SELECT DISTINCT ${col} FROM customers WHERE ${scopeWhere} AND ${col} IS NOT NULL AND ${col} != '' ORDER BY ${col} LIMIT 100`, scopeParams);
                if (optResult.rows.length > 0 && optResult.rows.length <= 100) {
                    options[f.field_key] = optResult.rows.map((r) => r[col]);
                }
            }
            catch (e) { /* 컬럼 없으면 무시 */ }
        }
        ...
        for (const f of fields) {
            if (!f.is_custom || f.data_type !== 'string')
                continue;
            try {
                const optResult = await (0, database_1.query)(`SELECT DISTINCT custom_fields->>'${f.field_key}' as val FROM customers WHERE ${scopeWhere} AND custom_fields->>'${f.field_key}' IS NOT NULL AND custom_fields->>'${f.field_key}' != '' ORDER BY val LIMIT 100`, scopeParams);
                if (optResult.rows.length > 0 && optResult.rows.length <= 100) {
                    options[f.field_key] = optResult.rows.map((r) => r.val);
                }
            }
        ...
        let sample = {};
        try {
            const sampleResult = await (0, database_1.query)(`SELECT * FROM customers
         WHERE ${scopeWhere} AND name IS NOT NULL AND name != ''
         ORDER BY updated_at DESC LIMIT 1`, scopeParams);
            if (sampleResult.rows.length > 0) {
                const row = sampleResult.rows[0];
                for (const f of fields) {
                    const key = f.field_key;
                    const mapped = (0, standard_field_map_1.getFieldByKey)(key);
                    let val;
                    ...
                    if (standard_field_map_1.FIELD_DISPLAY_MAP[key]) {
                        sample[key] = (0, standard_field_map_1.reverseDisplayValue)(key, val);
                    }
                    else {
                        sample[key] = val;
                    }
        ...
        try {
            ...
            const knownPhoneKeys = standard_field_map_1.FIELD_MAP.filter(f => f.normalizeFunction === 'normalizePhone' || f.normalizeFunction === 'normalizeStorePhone').map(f => f.fieldKey).filter(k => k !== 'phone'); // phone(수신자번호) 제외
            ...
            const phoneSampleResult = await (0, database_1.query)(`SELECT custom_fields, store_phone FROM customers WHERE ${scopeWhere} AND custom_fields IS NOT NULL AND custom_fields != '{}' LIMIT 10`, scopeParams);
            const customPhoneFields = (0, callback_filter_1.detectPhoneFields)(phoneSampleResult.rows, fields.filter((f) => f.is_custom));
            phoneFields = [...new Set([...enabledKnown, ...customPhoneFields.map(f => f.field_key)])];
        }
        catch (e) { /* phone_fields 감지 실패 시 빈 배열 */ }
        res.json({ fields, options, sample, categories: standard_field_map_1.CATEGORY_LABELS, phoneFields });
    }
    catch (error) {
...
router.delete('/:id', sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        let companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        // 삭제 대상 확인
        const target = await (0, database_1.query)('SELECT id, name, phone FROM customers WHERE id = $1 AND company_id = $2 AND is_active = true', [id, companyId]);
        if (target.rows.length === 0) {
            return res.status(404).json({ error: '고객을 찾을 수 없습니다' });
        }
        const customer = target.rows[0];
        // 연관 데이터 삭제 (purchases, consents)
        await (0, database_1.query)('DELETE FROM purchases WHERE customer_id = $1 AND company_id = $2', [id, companyId]);
        await (0, database_1.query)('DELETE FROM consents WHERE customer_id = $1', [id]);
        // 고객 삭제 (하드 삭제)
        await (0, database_1.query)('DELETE FROM customers WHERE id = $1 AND company_id = $2', [id, companyId]);
        // 감사 로그
...
router.post('/bulk-delete', sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        let companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        // 실제 존재하는 고객만 필터
        const existing = await (0, database_1.query)('SELECT id, name, phone FROM customers WHERE id = ANY($1) AND company_id = $2 AND is_active = true', [ids, companyId]);
        const validIds = existing.rows.map((r) => r.id);
        if (validIds.length === 0) {
            return res.status(404).json({ error: '삭제할 고객이 없습니다' });
        }
        // 연관 데이터 삭제
        await (0, database_1.query)('DELETE FROM purchases WHERE customer_id = ANY($1) AND company_id = $2', [validIds, companyId]);
        await (0, database_1.query)('DELETE FROM consents WHERE customer_id = ANY($1)', [validIds]);
        // 고객 삭제
        const deleteResult = await (0, database_1.query)('DELETE FROM customers WHERE id = ANY($1) AND company_id = $2', [validIds, companyId]);
        // 감사 로그
        await (0, database_1.query)(`INSERT INTO audit_logs (user_id, action, target_type, target_id, details, ip_address, user_agent)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
            userId,
            ...
            JSON.stringify({
                ...
                sample_phones: existing.rows.slice(0, 5).map((r) => r.phone)
            }),
            req.ip,
            req.headers['user-agent'] || ''
        ]);
...
router.post('/delete-all', sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        if (!confirmCompanyName)
            return res.status(400).json({ error: '회사명 확인이 필요합니다' });
        // 회사명 확인
        const companyResult = await (0, database_1.query)('SELECT company_name FROM companies WHERE id = $1', [deleteCompanyId]);
        ...
        // 삭제 전 건수 확인
        const countResult = await (0, database_1.query)('SELECT COUNT(*) FROM customers WHERE company_id = $1 AND is_active = true', [deleteCompanyId]);
        const totalCount = parseInt(countResult.rows[0].count);
        if (totalCount === 0) {
            return res.status(400).json({ error: '삭제할 고객 데이터가 없습니다' });
        }
        // 연관 데이터 삭제 (해당 회사 전체)
        const purchaseResult = await (0, database_1.query)('DELETE FROM purchases WHERE company_id = $1', [deleteCompanyId]);
        await (0, database_1.query)('DELETE FROM consents WHERE customer_id IN (SELECT id FROM customers WHERE company_id = $1)', [deleteCompanyId]);
        // ★ D114 P1: 수신거부 + 필드 정의 + customer_schema + customer_stores 정리
        // 고객 전체삭제 시 customer_field_definitions가 잔존하면 다음 업로드에서 매핑 충돌 오감지
        await (0, database_1.query)('DELETE FROM unsubscribes WHERE company_id = $1', [deleteCompanyId]);
        await (0, database_1.query)('DELETE FROM customer_field_definitions WHERE company_id = $1', [deleteCompanyId]);
        await (0, database_1.query)('DELETE FROM customer_stores WHERE company_id = $1', [deleteCompanyId]);
        await (0, database_1.query)(`UPDATE companies SET customer_schema = '{}'::jsonb WHERE id = $1`, [deleteCompanyId]);
        // 고객 전체 삭제
        const deleteResult = await (0, database_1.query)('DELETE FROM customers WHERE company_id = $1', [deleteCompanyId]);
        // 감사 로그
        await (0, database_1.query)(`INSERT INTO audit_logs (user_id, action, target_type, target_id, details, ip_address, user_agent)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
            userId,
...
router.get('/:id', async (req, res) => {
    try {
       ...
       FROM customers_unified c WHERE c.id = $1 AND c.company_id = $2 AND c.is_active = true`, [id, companyId, userId]);
