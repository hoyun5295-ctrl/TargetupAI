...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        // 사용자 필터 (고객사관리자용)
        const filterUserId = req.query.filter_user_id;
        if (filterUserId) {
...
router.post('/:id/cancel', async (req, res) => {
    ...
    try {
        // 캠페인 조회 (소유권 확인용)
        const check = await (0, database_1.query)('SELECT status, scheduled_at, company_id FROM campaigns WHERE id = $1', [id]);
        ...
        const result = await (0, campaign_lifecycle_1.cancelCampaign)(id, companyId, {
            reason: reason.trim(),
            cancelledBy: userId,
            cancelledByType,
...
//# sourceMappingURL=manage-scheduled.js.map