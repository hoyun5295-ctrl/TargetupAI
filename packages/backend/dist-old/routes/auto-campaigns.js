...
const target_sample_1 = require("../utils/target-sample");
 ...
 */
async function checkPlanGating(companyId) {
    ...
    const check = (0, plan_guard_1.canUseFeature)(ctx, 'auto_campaign');
    if (!check.allowed) {
        return { allowed: false, maxAutoCampaigns: null, errorMsg: check.errorMsg, ctx };
    }
    return { allowed: true, maxAutoCampaigns: (0, plan_guard_1.resolveMaxAutoCampaigns)(ctx), ctx };
}
/** AI 프리미엄 게이팅 (CT-17 래퍼) */
async function checkAiPremiumGating(companyId) {
    const ctx = await (0, plan_guard_1.loadPlanContext)(companyId);
    if (!ctx)
        return false;
    return (0, plan_guard_1.canUseFeature)(ctx, 'ai_premium').allowed;
}
/** 활성 자동캠페인 수 제한 체크 */
async function checkActiveLimit(companyId, maxAutoCampaigns) {
    // NULL = 무제한 (ENTERPRISE)
    if (maxAutoCampaigns === null || maxAutoCampaigns === undefined) {
        return { ok: true, current: 0 };
    }
    const countResult = await (0, database_1.query)(`SELECT COUNT(*)::int as cnt FROM auto_campaigns WHERE company_id = $1 AND status = 'active'`, [companyId]);
...
async function checkOwnership(autoCampaignId, companyId, userType, userId) {
    // ★ B2: opt_out_080_number 포함을 위해 LEFT JOIN (자동발송은 ac.user_id 기반)
    const result = await (0, database_1.query)(`SELECT ac.*, ${unsubscribe_helper_1.CAMPAIGN_OPT080_SELECT_EXPR}
     FROM auto_campaigns ac
     ${(0, unsubscribe_helper_1.buildCampaignOpt080LeftJoin)('ac', 'user_id')}
     WHERE ac.id = $1 AND ac.company_id = $2 AND ac.status != 'deleted'`, [autoCampaignId, companyId]);
...
// ★ D111 E2: calcNextRunAt을 utils/auto-campaign-worker.ts로 통합 — 컨트롤타워 원칙.
//   이전: 이 파일과 worker 양쪽에 동일 로직 중복 → 한쪽 수정 시 발송 시각 오차 재발.
//   이후: worker에서 export, routes에서 import.
...
router.post('/preview-sample', async (req, res) => {
    try {
        ...
        const result = await (0, target_sample_1.fetchTargetSampleCustomer)({
            companyId,
...
router.get('/', async (req, res) => {
    try {
        ...
        const planCtx = await (0, plan_guard_1.loadPlanContext)(companyId);
        // ★ B2: opt_out_080_number 포함을 위해 LEFT JOIN (자동발송은 ac.user_id 기반)
        const result = await (0, database_1.query)(`SELECT ac.*,
       ...
       ORDER BY ac.created_at DESC`, params);
        ...
        let planForFront = { auto_campaign_enabled: false, max_auto_campaigns: null, ai_premium_enabled: false };
        if (planCtx) {
            const autoOk = (0, plan_guard_1.canUseFeature)(planCtx, 'auto_campaign').allowed;
            const premiumOk = (0, plan_guard_1.canUseFeature)(planCtx, 'ai_premium').allowed;
            planForFront = {
                auto_campaign_enabled: autoOk,
                max_auto_campaigns: autoOk ? (0, plan_guard_1.resolveMaxAutoCampaigns)(planCtx) : null,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        // 실행 이력 조회 (최근 20건)
        const runsResult = await (0, database_1.query)(`SELECT acr.*,
...
router.post('/', async (req, res) => {
    try {
        ...
        const { campaign_name, description, schedule_type, schedule_day, schedule_time, target_filter, store_code, message_type, message_content, message_subject, callback_number, sender_number_id, is_ad, pre_notify, notify_phones, use_individual_callback, // ★ D93: 수신자별 회신번호 칼럼 사용
        // ★ AI 프리미엄 필드 (기능 3)
        ai_generate_enabled, ai_prompt, ai_tone, fallback_message_content, 
        // ★ D130: 알림톡 필드 (설계서 §6-3-D)
        channel, alimtalk_profile_id, alimtalk_template_id, alimtalk_template_code, alimtalk_variable_map, alimtalk_next_type, alimtalk_next_contents, } = req.body;
        // 필수값 검증
        ...
        if (!use_individual_callback && callback_number) {
            const cbCheck = await (0, database_1.query)(`SELECT id FROM callback_numbers WHERE company_id = $1 AND regexp_replace(phone, '\\D', '', 'g') = $2`, [companyId, (0, normalize_phone_1.normalizePhone)(callback_number)]);
        ...
        if (use_individual_callback && !forceCreate) {
            try {
                const filterResult = (0, customer_filter_1.buildFilterQueryCompat)(target_filter || {}, companyId);
                ...
                const customersResult = await (0, database_1.query)(`SELECT c.id, c.phone, c.callback, c.store_phone, c.custom_fields
           FROM customers c
           WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true
           ${filterResult.where}${storeFilter}${unsubFilter}`, [companyId, ...filterResult.params, ...storeParams, userId]);
                if (customersResult.rows.length > 0) {
                    const cbResult = await (0, callback_filter_1.filterByIndividualCallback)(customersResult.rows, companyId);
                    // 미등록 회신번호가 있으면 모달용 데이터 반환 (에러 아님)
                    if (cbResult.callbackUnregisteredCount > 0 || cbResult.callbackMissingCount > 0) {
                        // 미등록 회신번호별 제외 건수 집계
                        const unregisteredDetailsMap = new Map();
                        for (const c of customersResult.rows) {
                            ...
                            const isFiltered = cbResult.filtered.find((f) => f.id === c.id);
                            if (!isFiltered) {
        ...
        // next_run_at 계산
        const nextRunAt = (0, auto_campaign_worker_1.calcNextRunAt)(schedule_type, schedule_day, schedule_time);
        const finalStoreCode = store_code || userStoreCode || null;
        ...
        const insertResult = await (0, database_1.query)(`INSERT INTO auto_campaigns (
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { campaign_name, description, schedule_type, schedule_day, schedule_time, target_filter, message_type, message_content, message_subject, callback_number, sender_number_id, is_ad, pre_notify, notify_phones, use_individual_callback, // ★ D93: 수신자별 회신번호 칼럼 사용
        // ★ AI 프리미엄 필드 (기능 3)
        ai_generate_enabled, ai_prompt, ai_tone, fallback_message_content, 
        ...
        if ((schedule_type || schedule_day !== undefined || schedule_time) && ownership.campaign.status === 'active') {
            nextRunAt = (0, auto_campaign_worker_1.calcNextRunAt)(finalScheduleType, finalScheduleDay, finalScheduleTime);
        }
        // ★ AI 모드 OFF 전환 시 generated 필드 초기화
        const clearGenerated = ai_generate_enabled === false;
        const updateResult = await (0, database_1.query)(`UPDATE auto_campaigns SET
...
// ============================================================
router.put('/:id/generated-message', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const userId = req.user?.userId;
        const userType = req.user?.userType;
        ...
        updated_at = NOW()
      WHERE id = $1 AND company_id = $4`, [req.params.id, generated_message_content.trim(), generated_message_subject?.trim() || null, companyId]);
...
router.post('/:id/pause', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE auto_campaigns SET status = 'paused', next_run_at = NULL, updated_at = NOW() WHERE id = $1`, [req.params.id]);
...
router.post('/:id/resume', async (req, res) => {
    try {
        ...
        // next_run_at 재계산
        const nextRunAt = (0, auto_campaign_worker_1.calcNextRunAt)(ownership.campaign.schedule_type, ownership.campaign.schedule_day, ownership.campaign.schedule_time);
        await (0, database_1.query)(`UPDATE auto_campaigns SET status = 'active', next_run_at = $2, updated_at = NOW() WHERE id = $1`, [req.params.id, nextRunAt]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE auto_campaigns SET status = 'deleted', next_run_at = NULL, updated_at = NOW() WHERE id = $1`, [req.params.id]);
...
router.post('/:id/preview', async (req, res) => {
    try {
        ...
        const filterResult = (0, customer_filter_1.buildFilterQueryCompat)(campaign.target_filter, companyId);
        // store_code 필터
        let storeFilter = '';
        const storeParams = [];
        ...
        // 수신거부 필터 (user_id 기준)
        const unsubFilter = (0, unsubscribe_helper_1.buildUnsubscribeFilter)(`'${userId}'`, 'c.phone');
        const countResult = await (0, database_1.query)(`SELECT COUNT(*)::int as cnt FROM customers c
       WHERE c.company_id = $1${filterResult.where}${storeFilter}${unsubFilter}
       AND c.sms_opt_in = true`, [companyId, ...filterResult.params, ...storeParams]);
        // 샘플 고객 5명
        const sampleResult = await (0, database_1.query)(`SELECT c.name, c.phone, c.gender, c.grade FROM customers c
       WHERE c.company_id = $1${filterResult.where}${storeFilter}${unsubFilter}
       AND c.sms_opt_in = true
       ORDER BY RANDOM() LIMIT 5`, [companyId, ...filterResult.params, ...storeParams]);
        res.json({
...
router.post('/:id/cancel-next', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`UPDATE auto_campaign_runs
       SET status = 'cancelled', cancelled_at = NOW(), cancel_reason = $2
       WHERE auto_campaign_id = $1 AND status IN ('pending', 'notified')`, [req.params.id, req.body.reason || '사용자 취소']);
        // next_run_at을 그 다음 스케줄로 갱신
        const newNextRunAt = (0, auto_campaign_worker_1.calcNextRunAt)(campaign.schedule_type, campaign.schedule_day, campaign.schedule_time);
        await (0, database_1.query)(`UPDATE auto_campaigns SET next_run_at = $2, updated_at = NOW() WHERE id = $1`, [req.params.id, newNextRunAt]);
...
//# sourceMappingURL=auto-campaigns.js.map