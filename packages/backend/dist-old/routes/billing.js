...
const getBillingCompanyTables = (companyId) => (0, sms_queue_1.getCompanySmsTables)(companyId);
const getBillingTestTables = () => (0, sms_queue_1.getTestSmsTables)();
async function getBillingLogTables() {
    const rows = await (0, database_1.mysqlQuery)(`SELECT TABLE_NAME FROM information_schema.TABLES
...
async function getTablesForBillingPeriod(baseTables, startDate, endDate) {
...
async function smsAggByDateType(tables, whereClause, params) {
    const allRows = [];
    for (const t of tables) {
        const rows = await (0, database_1.mysqlQuery)(`SELECT msg_type, DATE(sendreq_time) as send_date,
              COUNT(*) as total_count,
              SUM(CASE WHEN status_code IN (${sms_result_map_1.SUCCESS_CODES_SQL}) THEN 1 ELSE 0 END) as success_count,
              SUM(CASE WHEN status_code NOT IN (${sms_result_map_1.SUCCESS_CODES_SQL},${sms_result_map_1.PENDING_CODES_SQL}) THEN 1 ELSE 0 END) as fail_count,
              SUM(CASE WHEN status_code IN (${sms_result_map_1.PENDING_CODES_SQL}) THEN 1 ELSE 0 END) as pending_count
       FROM ${t} WHERE ${whereClause}
       GROUP BY msg_type, DATE(sendreq_time)`, params);
        allRows.push(...rows);
    }
    return allRows;
}
async function smsAggByRunAndType(tables, whereClause, params) {
    const allRows = [];
    for (const t of tables) {
        const rows = await (0, database_1.mysqlQuery)(`SELECT app_etc1 as run_id, msg_type,
              COUNT(*) as total_count,
              SUM(CASE WHEN status_code IN (${sms_result_map_1.SUCCESS_CODES_SQL}) THEN 1 ELSE 0 END) as success_count
       FROM ${t} WHERE ${whereClause}
       GROUP BY app_etc1, msg_type`, params);
        allRows.push(...rows);
    }
    return allRows;
}
async function smsAggTestByType(tables, whereClause, params) {
    const allRows = [];
    for (const t of tables) {
        const rows = await (0, database_1.mysqlQuery)(`SELECT msg_type,
              COUNT(*) as total_count,
              SUM(CASE WHEN status_code IN (${sms_result_map_1.SUCCESS_CODES_SQL}) THEN 1 ELSE 0 END) as success_count
       FROM ${t} WHERE ${whereClause}
       GROUP BY msg_type`, params);
        allRows.push(...rows);
    }
    return allRows;
}
...
router.post('/generate', async (req, res) => {
    try {
        ...
        // 1) 중복 체크 (기간 겹침)
        const existCheck = await database_1.default.query(`SELECT id, status, billing_start, billing_end FROM billings
       WHERE company_id = $1
         AND COALESCE(user_id, '00000000-0000-0000-0000-000000000000') = COALESCE($2::uuid, '00000000-0000-0000-0000-000000000000')
         AND billing_start <= $4::date AND billing_end >= $3::date`, [company_id, user_id || null, billing_start, billing_end]);
        ...
        // 2) 고객사 단가 조회 (스냅샷)
        const companyResult = await database_1.default.query(`SELECT cost_per_sms, cost_per_lms, cost_per_mms, cost_per_kakao,
      ...
      WHERE c.company_id = $1
        ...
        const runsParams = [company_id, billing_start, billing_end];
        if (user_id) {
            runsParams.push(user_id);
            runsSql += ` AND c.user_id = $${runsParams.length}`;
        }
        const runsResult = await database_1.default.query(runsSql, runsParams);
        ...
        if (runIds.length > 0) {
            ...
            rows.forEach((row) => {
                ...
                if (!dayData[d])
                    dayData[d] = {};
                if (!dayData[d][t])
                    dayData[d][t] = { total: 0, success: 0, fail: 0, pending: 0 };
        ...
        if (!user_id) {
            ...
            const testRows = await smsAggByDateType(testTables, `app_etc1 = 'test' AND app_etc2 = ? AND sendreq_time >= ? AND sendreq_time < DATE_ADD(?, INTERVAL 1 DAY)`, [company_id, billing_start, billing_end]);
        ...
        if (!user_id) {
            const spamResult = await database_1.default.query(`
        SELECT
          r.message_type,
      ...
      `, [company_id, billing_start, billing_end]);
        ...
        if (runIds.length > 0) {
            ...
            const campaignIdsResult = await database_1.default.query(`SELECT DISTINCT c.id as campaign_id
         FROM campaign_runs cr
         JOIN campaigns c ON c.id = cr.campaign_id
         WHERE cr.id = ANY($1::uuid[])
           AND (c.send_channel = 'kakao' OR c.send_channel = 'both')`, [runIds]);
            const kakaoCampaignIds = campaignIdsResult.rows.map((r) => r.campaign_id);
            if (kakaoCampaignIds.length > 0) {
                const kph = kakaoCampaignIds.map(() => '?').join(',');
                const kakaoRows = await (0, database_1.mysqlQuery)(`SELECT DATE(REQUEST_DATE) as send_date,
                  COUNT(*) as total_count,
           ...
           GROUP BY DATE(REQUEST_DATE)`, [...kakaoCampaignIds, billing_start, billing_end]);
                kakaoRows.forEach((row) => {
                    const d = row.send_date instanceof Date
                        ? row.send_date.toISOString().slice(0, 10)
                        : String(row.send_date).slice(0, 10);
                    if (!dayData[d])
                        dayData[d] = {};
                    if (!dayData[d]['KAKAO'])
                        dayData[d]['KAKAO'] = { total: 0, success: 0, fail: 0, pending: 0 };
        ...
        {
            const directKakaoResult = await database_1.default.query(`SELECT id FROM campaigns
         WHERE company_id = $1
           AND send_type = 'manual'
           AND (send_channel = 'kakao' OR send_channel = 'both')
           AND sent_at >= $2::date
           AND sent_at < ($3::date + interval '1 day')`, [company_id, billing_start, billing_end]);
            const directKakaoIds = directKakaoResult.rows.map((r) => r.id);
            if (directKakaoIds.length > 0) {
                const dkph = directKakaoIds.map(() => '?').join(',');
                const dkRows = await (0, database_1.mysqlQuery)(`SELECT DATE(REQUEST_DATE) as send_date,
                  COUNT(*) as total_count,
                  SUM(CASE WHEN REPORT_CODE = '0000' THEN 1 ELSE 0 END) as success_count,
                  SUM(CASE WHEN REPORT_CODE != '0000' AND STATUS IN ('3','4') THEN 1 ELSE 0 END) as fail_count,
                  SUM(CASE WHEN STATUS IN ('1','2') THEN 1 ELSE 0 END) as pending_count
           ...
           GROUP BY DATE(REQUEST_DATE)`, directKakaoIds);
                dkRows.forEach((row) => {
                    const d = row.send_date instanceof Date
                        ? row.send_date.toISOString().slice(0, 10)
                        : String(row.send_date).slice(0, 10);
        ...
        Object.values(dayData).forEach(types => {
            if (types.SMS)
                totalSms += types.SMS.success;
            if (types.LMS)
                totalLms += types.LMS.success;
            if (types.MMS)
                totalMms += types.MMS.success;
            if (types.KAKAO)
                totalKakao += types.KAKAO.success;
            if (types.TEST_SMS)
                totalTestSms += types.TEST_SMS.success;
            if (types.TEST_LMS)
                totalTestLms += types.TEST_LMS.success;
            if (types.SPAM_SMS)
                totalSpamSms += types.SPAM_SMS.success;
            if (types.SPAM_LMS)
                totalSpamLms += types.SPAM_LMS.success;
        });
        // 스팸필터 단가 = 일반 단가와 동일 (D16 결정)
        const spamSmsCost = prices.SMS;
        const spamLmsCost = prices.LMS;
        const subtotal = (totalSms * prices.SMS) + (totalLms * prices.LMS) +
        ...
        // 8) billings INSERT
        const billingResult = await database_1.default.query(`INSERT INTO billings (
      ...
      RETURNING *`, [
        ...
        const itemValues = [];
        const allPrices = { ...prices, SPAM_SMS: spamSmsCost, SPAM_LMS: spamLmsCost };
...
router.get('/list', async (req, res) => {
    try {
        ...
        if (year) {
            params.push(year);
            sql += ` AND b.billing_year = $${params.length}`;
        }
        if (status) {
            params.push(status);
...
router.get('/company-users/:companyId', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT id, name, login_id, department, role
       FROM users WHERE company_id = $1 AND is_active = true AND COALESCE(is_system, false) = false
       ORDER BY name`, [req.params.companyId]);
        return res.json(result.rows);
    }
    catch (error) {
...
router.get('/:id/items', async (req, res) => {
    try {
        const billing = await database_1.default.query(`SELECT b.*, c.company_name, u.name as user_name
       FROM billings b
       JOIN companies c ON c.id = b.company_id
       LEFT JOIN users u ON u.id = b.user_id
       WHERE b.id = $1`, [req.params.id]);
        ...
        const items = await database_1.default.query(`SELECT * FROM billing_items WHERE billing_id = $1 ORDER BY item_date ASC, message_type ASC`, [req.params.id]);
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`UPDATE billings SET status = $1, updated_at = now() WHERE id = $2 RETURNING *`, [status, req.params.id]);
...
router.delete('/:id', async (req, res) => {
    try {
        const check = await database_1.default.query('SELECT status FROM billings WHERE id = $1', [req.params.id]);
        if (check.rows.length === 0)
            return res.status(404).json({ error: '정산을 찾을 수 없습니다' });
        // billing_items는 ON DELETE CASCADE로 자동 삭제
        await database_1.default.query('DELETE FROM billings WHERE id = $1', [req.params.id]);
...
router.get('/:id/pdf', async (req, res) => {
    try {
        // 1) 정산 + 회사 정보
        const result = await database_1.default.query(`SELECT b.*, c.company_name, c.business_number, c.ceo_name, c.address,
       ...
       WHERE b.id = $1`, [req.params.id]);
        ...
        // 2) 일자별 상세
        const itemsResult = await database_1.default.query(`SELECT * FROM billing_items WHERE billing_id = $1 ORDER BY item_date ASC, message_type ASC`, [req.params.id]);
        ...
        doc.pipe(stream);
        const setFont = (bold = false) => { if (hasFont)
            doc.font(bold ? fontBoldPath : fontPath); };
        ...
        y += 18;
        doc.text(`상호: ${defaults_1.INVITO_INFO.companyName}`, 50, y);
        y += 14;
        doc.text(`대표: ${defaults_1.INVITO_INFO.ceoName}`, 50, y);
        y += 14;
        doc.text(`사업자번호: ${defaults_1.INVITO_INFO.bizNumber}`, 50, y);
        y += 14;
        doc.text(`업태/종목: ${defaults_1.INVITO_INFO.bizType}`, 50, y);
        y += 14;
        doc.text(`주소: ${defaults_1.INVITO_INFO.address}`, 50, y);
        y += 14;
        doc.text(`연락처: ${defaults_1.INVITO_INFO.phone} / ${defaults_1.INVITO_INFO.email}`, 50, y);
        y = 130;
        setFont(true);
        ...
        doc.fontSize(9).fillColor(dark);
        y += 18;
        doc.text(`상호: ${bil.company_name || '-'}`, rightX, y);
        y += 14;
        doc.text(`대표: ${bil.ceo_name || '-'}`, rightX, y);
        y += 14;
        doc.text(`사업자번호: ${bil.business_number || '-'}`, rightX, y);
        y += 14;
        doc.text(`업태/종목: ${bil.business_type || '-'} / ${bil.business_category || '-'}`, rightX, y);
        y += 14;
        doc.text(`주소: ${bil.address || '-'}`, rightX, y);
        y += 14;
        ...
        y += 25;
        const drawRow = (label, count, price, amount, bg = 'white') => {
            if (count <= 0)
                return;
            if (bg !== 'white')
        ...
        if (items.length > 0) {
            ...
            setFont(false);
            doc.fontSize(9).fillColor(gray).text(`${bil.company_name} | ${bil.billing_year}년 ${bil.billing_month}월${bil.user_name ? ' | ' + bil.user_name : ''}`, 50, 72);
            doc.moveTo(50, 90).lineTo(545, 90).strokeColor('#e5e7eb').stroke();
            // 테이블 헤더
            const cols = [
                { label: '일자', x: 50, w: 75, align: 'left' },
                { label: '유형', x: 125, w: 60, align: 'left' },
                { label: '전송', x: 185, w: 55, align: 'right' },
                { label: '성공', x: 240, w: 55, align: 'right' },
                { label: '실패', x: 295, w: 55, align: 'right' },
                { label: '대기', x: 350, w: 55, align: 'right' },
                { label: '단가', x: 405, w: 65, align: 'right' },
                { label: '금액', x: 470, w: 75, align: 'right' },
            ];
            let iy = 95;
            const rowH = 18;
            ...
            items.forEach((item, idx) => {
                ...
                if (isSpam)
                    doc.rect(50, iy, 495, rowH).fill('#fef3c7');
                else if (isTest)
                    doc.rect(50, iy, 495, rowH).fill('#fefce8');
                else if (idx % 2 === 0)
                    doc.rect(50, iy, 495, rowH).fill('#fafafa');
                setFont(false);
...
router.get('/preview', async (req, res) => {
    try {
       ...
       FROM companies WHERE id = $1`, [company_id]);
        ...
        const runsResult = await database_1.default.query(`SELECT cr.id as run_id, c.callback_number, cb.store_code, cb.store_name
       FROM campaign_runs cr
       JOIN campaigns c ON c.id = cr.campaign_id
       LEFT JOIN callback_numbers cb ON cb.phone = c.callback_number AND cb.company_id = c.company_id
       WHERE c.company_id = $1
         AND cr.sent_at >= $2::date
         AND cr.sent_at < ($3::date + interval '1 day')`, [company_id, start, end]);
        const runIds = runsResult.rows.map((r) => r.run_id);
        const runStoreMap = {};
        runsResult.rows.forEach((r) => {
        ...
        if (runIds.length > 0) {
            const companyTables = await getBillingCompanyTables(company_id);
            const billingTables = await getTablesForBillingPeriod(companyTables, start, end);
            const placeholders = runIds.map(() => '?').join(',');
            normalCounts = await smsAggByRunAndType(billingTables, `app_etc1 IN (${placeholders}) AND sendreq_time >= ? AND sendreq_time < DATE_ADD(?, INTERVAL 1 DAY)`, [...runIds, start, end]);
        }
        // 4) MySQL에서 테스트발송 집계 — 멀티테이블
        const testBaseTables = await getBillingTestTables();
        const testTables = await getTablesForBillingPeriod(testBaseTables, start, end);
        const testRows = await smsAggTestByType(testTables, `app_etc1 = 'test' AND app_etc2 = ? AND sendreq_time >= ? AND sendreq_time < DATE_ADD(?, INTERVAL 1 DAY)`, [company_id, start, end]);
    ...
    `, [company_id, start, end]);
        ...
        if (runIds.length > 0) {
            ...
            const kakaoCampResult = await database_1.default.query(`SELECT DISTINCT c.id
         FROM campaign_runs cr JOIN campaigns c ON c.id = cr.campaign_id
         WHERE cr.id = ANY($1::uuid[]) AND (c.send_channel = 'kakao' OR c.send_channel = 'both')`, [runIds]);
            const kakaoCampIds = kakaoCampResult.rows.map((r) => r.id);
            if (kakaoCampIds.length > 0) {
                const kph = kakaoCampIds.map(() => '?').join(',');
                const kakaoCountRows = await (0, database_1.mysqlQuery)(`SELECT COUNT(*) as success_count
           FROM IMC_BM_FREE_BIZ_MSG
           WHERE REQUEST_UID IN (${kph}) AND REPORT_CODE = '0000'`, kakaoCampIds);
                kakaoSuccessTotal = Number(kakaoCountRows[0]?.success_count || 0);
            }
        }
        // 직접발송 카카오도 합산
        {
         ...
         WHERE company_id = $1 AND send_type = 'manual'
           AND (send_channel = 'kakao' OR send_channel = 'both')
           AND sent_at >= $2::date AND sent_at < ($3::date + interval '1 day')`, [company_id, start, end]);
            const dkIds = directKakaoResult.rows.map((r) => r.id);
            if (dkIds.length > 0) {
                const dkph = dkIds.map(() => '?').join(',');
                const dkCountRows = await (0, database_1.mysqlQuery)(`SELECT COUNT(*) as success_count FROM IMC_BM_FREE_BIZ_MSG WHERE REQUEST_UID IN (${dkph}) AND REPORT_CODE = '0000'`, dkIds);
                kakaoSuccessTotal += Number(dkCountRows[0]?.success_count || 0);
            }
        }
        // 5) 집계 계산
        ...
        if (type === 'brand') {
            ...
            normalCounts.forEach((row) => {
                ...
                if (row.msg_type === 'S')
                    brandMap[key].sms_success += Number(row.success_count);
                if (row.msg_type === 'L')
                    brandMap[key].lms_success += Number(row.success_count);
            });
            // 카카오는 브랜드 구분 없이 전체 합산 (default에 넣기)
            if (kakaoSuccessTotal > 0) {
                if (!brandMap['default'])
                    brandMap['default'] = { store_code: 'default', store_name: '본사', sms_success: 0, lms_success: 0, mms_success: 0, kakao_success: 0 };
            ...
            normalCounts.forEach((row) => {
                if (row.msg_type === 'S')
                    sms_success += Number(row.success_count);
                if (row.msg_type === 'L')
                    lms_success += Number(row.success_count);
            });
            const summary = {
                sms_success, lms_success, mms_success, kakao_success,
...
function buildTestSummary(testRows, company) {
    let test_sms = 0, test_lms = 0;
    testRows.forEach((row) => {
        if (row.msg_type === 'S')
            test_sms += Number(row.success_count);
        if (row.msg_type === 'L')
            test_lms += Number(row.success_count);
    });
    return {
        test_sms, test_lms,
...
function buildSpamSummary(spam_sms, spam_lms, company) {
...
router.post('/invoices', async (req, res) => {
    try {
        const { company_id, store_code, store_name, billing_start, billing_end, invoice_type = 'combined', billing_id, sms_success_count = 0, sms_unit_price = 0, lms_success_count = 0, lms_unit_price = 0, mms_success_count = 0, mms_unit_price = 0, kakao_success_count = 0, kakao_unit_price = 0, test_sms_count = 0, test_sms_unit_price = 0, test_lms_count = 0, test_lms_unit_price = 0, spam_filter_count = 0, spam_filter_unit_price = 0, notes, created_by } = req.body;
        if (!company_id || !billing_start || !billing_end) {
            return res.status(400).json({ error: '필수: company_id, billing_start, billing_end' });
        }
        const subtotal = (sms_success_count * sms_unit_price) +
        ...
        const result = await database_1.default.query(`INSERT INTO billing_invoices (
...
router.get('/invoices', async (req, res) => {
    try {
        ...
        if (company_id) {
            params.push(company_id);
            sql += ` AND bi.company_id = $${params.length}`;
        }
        if (status) {
            params.push(status);
            sql += ` AND bi.status = $${params.length}`;
        }
...
router.get('/invoices/:id', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT bi.*, c.company_name, c.business_number, c.ceo_name, c.address
       FROM billing_invoices bi
       JOIN companies c ON c.id = bi.company_id
       WHERE bi.id = $1`, [req.params.id]);
        if (result.rows.length === 0) {
...
router.put('/invoices/:id/status', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`UPDATE billing_invoices SET status = $1, updated_at = now() WHERE id = $2 RETURNING *`, [status, req.params.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: '거래내역서를 찾을 수 없습니다' });
        }
        return res.json(result.rows[0]);
...
router.get('/invoices/:id/pdf', async (req, res) => {
    try {
       ...
       WHERE bi.id = $1`, [req.params.id]);
        ...
        doc.fontSize(9).fillColor(dark);
        iy += 18;
        doc.text(`상호: ${defaults_1.INVITO_INFO.companyName}`, 50, iy);
        iy += 14;
        doc.text(`대표: ${defaults_1.INVITO_INFO.ceoName}`, 50, iy);
        iy += 14;
        doc.text(`사업자번호: ${defaults_1.INVITO_INFO.bizNumber}`, 50, iy);
        iy += 14;
        doc.text(`업태/종목: ${defaults_1.INVITO_INFO.bizType}`, 50, iy);
        iy += 14;
        doc.text(`주소: ${defaults_1.INVITO_INFO.address}`, 50, iy);
        iy += 14;
        doc.text(`연락처: ${defaults_1.INVITO_INFO.phone} / ${defaults_1.INVITO_INFO.email}`, 50, iy);
        iy = 120;
        setFont(true);
        ...
        doc.fontSize(9).fillColor(dark);
        iy += 18;
        doc.text(`상호: ${inv.company_name || '-'}`, rightX, iy);
        iy += 14;
        doc.text(`대표: ${inv.ceo_name || '-'}`, rightX, iy);
        iy += 14;
        doc.text(`사업자번호: ${inv.business_number || '-'}`, rightX, iy);
        iy += 14;
        doc.text(`업태/종목: ${inv.business_type || '-'} / ${inv.business_category || '-'}`, rightX, iy);
        iy += 14;
        doc.text(`주소: ${inv.address || '-'}`, rightX, iy);
        iy += 14;
        ...
        iy += 25;
        const drawInvRow = (label, count, price, amount, bg = 'white') => {
            if (count <= 0)
                return;
            if (bg !== 'white')
                doc.rect(50, iy, 495, 22).fill(bg);
            setFont(false);
            doc.fontSize(9).fillColor(dark);
        ...
        doc.end();
        await new Promise((resolve) => stream.on('finish', resolve));
        await database_1.default.query('UPDATE billing_invoices SET pdf_path = $1, updated_at = now() WHERE id = $2', [pdfPath, req.params.id]);
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(pdfFilename)}"`);
        const fileStream = fs.createReadStream(pdfPath);
        fileStream.pipe(res);
...
router.post('/:id/send-email', async (req, res) => {
    try {
        ...
        const path = require('path');
        // 1) 정산 + 회사 정보 조회
        const result = await database_1.default.query(`SELECT b.*, c.company_name, c.contact_email, c.contact_name
       FROM billings b
       JOIN companies c ON c.id = b.company_id
       WHERE b.id = $1`, [req.params.id]);
        if (result.rows.length === 0) {
      ...
      <div style="font-family: 'Apple SD Gothic Neo', sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #ffffff; padding: 24px; border: 1px solid #E5E7EB; border-top: none;">
          ...
          <p style="font-size: 13px; color: #9CA3AF; margin-top: 16px;">
            ...
            문의사항이 있으시면 ${defaults_1.INVITO_INFO.phone}로 연락 부탁드립니다.
          </p>
        </div>
    ...
    `;
        ...
        // 4) 발송 기록
        await database_1.default.query('UPDATE billings SET email_sent_at = now(), updated_at = now() WHERE id = $1', [req.params.id]);
...
router.post('/invoices/:id/send-email', async (req, res) => {
    try {
        ...
        const path = require('path');
        // 1) 거래내역서 + 회사 정보 조회
        const result = await database_1.default.query(`SELECT bi.*, c.company_name, c.contact_email, c.contact_name
       FROM billing_invoices bi
       JOIN companies c ON c.id = bi.company_id
       WHERE bi.id = $1`, [req.params.id]);
        if (result.rows.length === 0) {
    ...
    `;
        ...
        // 4) 발송 기록
        await database_1.default.query('UPDATE billing_invoices SET email_sent_at = now(), updated_at = now() WHERE id = $1', [req.params.id]);
