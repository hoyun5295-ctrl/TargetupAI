...
const ai_mapping_1 = require("../utils/ai-mapping");
...
function recordIpFailure(ip) {
    ...
    if (entry && now < entry.resetAt) {
        ...
        ipFailures.set(ip, { count: 1, resetAt: now + defaults_1.RATE_LIMITS.windowMs });
    }
}
...
function ipRateLimit(req, res, next) {
    ...
    const entry = ipFailures.get(ip);
    if (entry && Date.now() < entry.resetAt && entry.count >= defaults_1.RATE_LIMITS.ipFailThreshold) {
...
function companyRateLimit(req, res, next) {
    const companyId = req.companyId;
    if (!companyId)
        return next(); // syncAuth 통과 못했으면 skip
    const now = Date.now();
    const entry = companyRequests.get(companyId);
    if (entry && now < entry.resetAt) {
        if (entry.count >= defaults_1.RATE_LIMITS.companyMaxPerMinute) {
...
async function syncAuth(req, res, next) {
    try {
        const apiKey = req.headers['x-sync-apikey'];
        const apiSecret = req.headers['x-sync-secret'];
        if (!apiKey || !apiSecret) {
            const ip = req.ip || req.headers['x-forwarded-for'] || 'unknown';
            recordIpFailure(ip);
        ...
        const result = await (0, database_1.query)(`SELECT id, name, company_name, status, use_db_sync 
       FROM companies 
       WHERE api_key = $1 AND api_secret = $2`, [apiKey, apiSecret]);
 ...
 */
async function getSyncConfigForAgent(companyId) {
    const res = await (0, database_1.query)(`SELECT id, config, sync_interval_customers, sync_interval_purchases, updated_at
     FROM sync_agents
 ...
 */
async function registerSyncUnsubscribes(companyId, companyName) {
    try {
        // 1. 시스템 user 조회 (없으면 즉시 생성 — 마이그레이션 누락 방어)
        let sysUserRes = await (0, database_1.query)(`SELECT id FROM users WHERE company_id = $1 AND is_system = true LIMIT 1`, [companyId]);
        let systemUserId = sysUserRes.rows[0]?.id || null;
        if (!systemUserId) {
            try {
                const created = await (0, database_1.query)(`INSERT INTO users (id, company_id, login_id, user_type, name, is_active, is_system, password_hash, status)
           ...
           RETURNING id`, [companyId]);
                ...
                if (!systemUserId) {
                    // 경합으로 이미 생성된 경우 재조회
                    const re = await (0, database_1.query)(`SELECT id FROM users WHERE company_id = $1 AND is_system = true LIMIT 1`, [companyId]);
        ...
        if (systemUserId) {
            const r1 = await (0, database_1.query)(`INSERT INTO unsubscribes (company_id, user_id, phone, source)
         ...
         ON CONFLICT (user_id, phone) DO NOTHING`, [companyId, systemUserId]);
        ...
        const r2 = await (0, database_1.query)(`INSERT INTO unsubscribes (company_id, user_id, phone, source)
       ...
       ON CONFLICT (user_id, phone) DO NOTHING`, [companyId]);
        ...
        const r3Count = await (0, unsubscribe_helper_1.registerBulkCompanyUserUnsubscribes)(companyId, 'sync');
        if (r3Count > 0) {
...
router.post('/register', async (req, res) => {
    try {
        ...
        const existing = await (0, database_1.query)(`SELECT id FROM sync_agents WHERE company_id = $1 AND agent_name = $2`, [companyId, agentName]);
        let agentId;
        if (existing.rows.length > 0) {
            // 기존 Agent 업데이트
            agentId = existing.rows[0].id;
            await (0, database_1.query)(`UPDATE sync_agents 
         SET agent_version = $1, os_info = $2, db_type = $3, 
             status = 'active', ip_address = $4, updated_at = NOW()
         WHERE id = $5`, [agentVersion, osInfo, dbType, ipAddress, agentId]);
        }
        else {
            // 새 Agent 등록
            const insertResult = await (0, database_1.query)(`INSERT INTO sync_agents (company_id, agent_name, agent_version, os_info, db_type, ip_address)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`, [companyId, agentName, agentVersion, osInfo, dbType, ipAddress]);
            agentId = insertResult.rows[0].id;
        }
        // 회사 설정 조회 (Agent에 전달할 config)
        const configResult = await (0, database_1.query)(`SELECT setting_key, setting_value 
       FROM company_settings 
       WHERE company_id = $1 AND setting_key LIKE 'sync_%'`, [companyId]);
        const config = {};
...
router.post('/heartbeat', async (req, res) => {
    try {
        const { agentId, agentVersion, status, osInfo, dbType, lastSyncAt, totalCustomersSynced, queuedItems, uptime } = req.body;
        if (!agentId) {
            return res.status(400).json({
                success: false,
        ...
        // Agent 존재 & 소유권 확인
        const agent = await (0, database_1.query)(`SELECT id FROM sync_agents WHERE id = $1 AND company_id = $2`, [agentId, req.companyId]);
        ...
        const { rows: configRows } = await (0, database_1.query)(`UPDATE sync_agents
       SET agent_version = COALESCE($1, agent_version),
           ...
           updated_at = NOW()
       WHERE id = $8
       RETURNING config`, [agentVersion, status, osInfo, dbType, lastSyncAt,
            queuedItems, uptime, agentId]);
        const currentConfig = configRows[0]?.config || {};
        const pendingCommands = Array.isArray(currentConfig.commands) ? currentConfig.commands : [];
        // commands 전달할 게 있으면 큐 비워서 중복 실행 방지 (At-Most-Once 의도)
        if (pendingCommands.length > 0) {
            const newConfig = { ...currentConfig, commands: [] };
            await (0, database_1.query)(`UPDATE sync_agents SET config = $1::jsonb WHERE id = $2`, [JSON.stringify(newConfig), agentId]);
        }
        return res.json({
            success: true,
...
router.post('/customers', async (req, res) => {
    ...
    try {
        ...
        if (customers.length > defaults_1.BATCH_SIZES.syncCustomer) {
            return res.status(400).json({
                success: false,
    ...
    `, [companyId]);
        ...
        const agentResult = await (0, database_1.query)(`SELECT id FROM sync_agents WHERE company_id = $1 AND status = 'active' ORDER BY updated_at DESC LIMIT 1`, [companyId]);
        const agentId = agentResult.rows[0]?.id;
        let upsertedCount = 0;
        let failedCount = 0;
        const failures = [];
        // ── FIELD_MAP 기반 동적 컬럼 목록 (하드코딩 금지) ──
        const columnFieldDefs = (0, standard_field_map_1.getColumnFields)(); // 고정 21개 (storageType='column')
        const customFieldDefs = (0, standard_field_map_1.getCustomFields)(); // 커스텀 15개 (storageType='custom_fields' — custom_1~15)
        ...
        for (const c of customers) {
            ...
            const phone = (0, normalize_1.normalizePhone)(c.phone);
            if (!phone) {
                failedCount++;
            ...
            let derivedBirthMonthDay = null;
            let derivedAge = null;
            let derivedRegion = null;
            let birthDateValue = c.birth_date || null;
            ...
            if (birthDateValue && birthDateValue instanceof Date) {
                birthDateValue = (0, normalize_1.normalizeDate)(birthDateValue) || null;
            }
            if (birthDateValue) {
                ...
                if (/^\d{4}$/.test(bd) && parseInt(bd) >= 1900 && parseInt(bd) <= 2099) {
                    ...
                    const normalized = (0, normalize_1.normalizeDate)(bd);
                    if (normalized) {
            ...
            if (c.region) {
                derivedRegion = (0, normalize_1.normalizeRegion)(c.region);
            }
            else if (c.address && typeof c.address === 'string') {
                const firstToken = c.address.split(/[\s,]/)[0];
                if (firstToken)
                    derivedRegion = (0, normalize_1.normalizeRegion)(firstToken);
            }
            ...
            for (const cf of customFieldDefs) {
                if (c[cf.fieldKey] != null && c[cf.fieldKey] !== '') {
                    customObj[cf.columnName] = (0, normalize_1.normalizeCustomFieldValue)(c[cf.fieldKey]);
                }
            }
        ...
        const upsertBuilder = (0, customer_upsert_1.createCustomerUpsertBuilder)({
            source: 'sync',
        ...
        for (let i = 0; i < validRows.length; i += CHUNK_SIZE) {
            ...
            try {
             ...
             ON CONFLICT (customer_id, store_code) DO NOTHING`, storeValues);
                }
            }
            catch (chunkError) {
                ...
                for (const row of chunk) {
                    try {
                        ...
                        const rowResult = await (0, database_1.query)(rowSql, rowValues);
                 ...
                 ON CONFLICT (customer_id, store_code) DO NOTHING`, [companyId, row.phone, row.store_code]);
                        }
                    }
                    catch (rowError) {
                        failedCount++;
        ...
        // sync_logs 기록
        if (agentId) {
            await (0, database_1.query)(`INSERT INTO sync_logs (agent_id, company_id, sync_type, mode, batch_index, total_batches, total_count, success_count, fail_count, failures, completed_at)
         VALUES ($1, $2, 'customers', $3, $4, $5, $6, $7, $8, $9, NOW())`, [agentId, companyId, mode || 'full', batchIndex || 1, totalBatches || 1,
        ...
        try {
            ...
            if (customFieldKeys.size > 0) {
                ...
                const existing = await (0, database_1.query)(`SELECT field_key FROM customer_field_definitions
            WHERE company_id = $1 AND field_key = ANY($2::text[])`, [companyId, keyArr]);
                const existingSet = new Set(existing.rows.map((r) => r.field_key));
                ...
                if (missing.length > 0) {
                    ...
                    const upsertedDefs = await (0, standard_field_map_1.upsertCustomFieldDefinitions)(companyId, defs);
                    console.log(`[Sync customers] customer_field_definitions 안전망 자동 UPSERT: ${upsertedDefs}건 `
            ...
            console.warn('[Sync customers] customer_field_definitions 안전망 실패 (sync 정상 완료):', autoErr?.message);
        }
...
router.post('/purchases', async (req, res) => {
    ...
    try {
        ...
        if (purchases.length > defaults_1.BATCH_SIZES.syncPurchase) {
            return res.status(400).json({
                success: false,
        ...
        for (const p of purchases) {
            ...
            const phone = (0, normalize_1.normalizePhone)(p.customer_phone);
            if (!phone) {
                failedCount++;
        ...
        if (allPhones.length > 0) {
            const cidResult = await (0, database_1.query)(`SELECT id, phone FROM customers WHERE company_id = $1 AND phone = ANY($2)`, [companyId, allPhones]);
        ...
        for (let i = 0; i < validPurchases.length; i += P_CHUNK) {
            ...
            try {
                ...
                for (let j = 0; j < chunk.length; j++) {
                    ...
                    const r = chunk[j];
                    values.push(companyId, phoneToCustomerId[r.phone] || null, r.phone, r.purchase_date, r.store_code, r.store_name, r.product_code, r.product_name, r.quantity, r.unit_price, r.total_amount);
                }
                const result = await (0, database_1.query)(`INSERT INTO purchases (
            ...
            quantity, unit_price, total_amount, created_at
                ...
                for (const r of chunk) {
                    try {
                        await (0, database_1.query)(`INSERT INTO purchases (
                company_id, customer_id, customer_phone, purchase_date,
                store_code, store_name, product_code, product_name,
                quantity, unit_price, total_amount, created_at
        ...
        if (agentId) {
            await (0, database_1.query)(`INSERT INTO sync_logs (agent_id, company_id, sync_type, mode, batch_index, total_batches, total_count, success_count, fail_count, failures, completed_at)
         VALUES ($1, $2, 'purchases', $3, $4, $5, $6, $7, $8, $9, NOW())`, [agentId, companyId, mode || 'full', batchIndex || 1, totalBatches || 1,
                purchases.length, insertedCount, failedCount, JSON.stringify(failures)]);
            // Agent 통계 업데이트
            await (0, database_1.query)(`UPDATE sync_agents 
         SET total_purchases_synced = total_purchases_synced + $1, 
             last_sync_at = NOW(), updated_at = NOW()
         WHERE id = $2`, [insertedCount, agentId]);
        }
        // ===== Agent 설정 응답 (설정 폴링 제거 대체) =====
        const agentConfig = await getSyncConfigForAgent(companyId);
...
router.post('/log', async (req, res) => {
    try {
        const companyId = req.companyId;
        const { agent_id, sync_type, sync_mode, total_count, success_count, fail_count, duration_ms, error_message, started_at, completed_at } = req.body;
        // 필수 필드 검증
        ...
        // agent_id 소유권 확인
        const agentCheck = await (0, database_1.query)('SELECT id FROM sync_agents WHERE id = $1 AND company_id = $2', [agent_id, companyId]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO sync_logs (
      ...
      RETURNING id`, [
        ...
        await (0, database_1.query)('UPDATE sync_agents SET last_sync_at = NOW(), updated_at = NOW() WHERE id = $1', [agent_id]);
...
router.get('/config', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT id, config, sync_interval_customers, sync_interval_purchases
       FROM sync_agents
       WHERE id = $1 AND company_id = $2`, [agentId, companyId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Agent not found'
        ...
        return res.json({
            ...
            config: {
                ...
                batch_size: config.batch_size ?? defaults_1.BATCH_SIZES.syncCustomer,
...
router.get('/version', async (req, res) => {
    try {
        ...
        if (agentId) {
            await (0, database_1.query)('UPDATE sync_agents SET agent_version = $1, updated_at = NOW() WHERE id = $2 AND company_id = $3', [currentVersion, agentId, companyId]);
        }
        // 최신 활성 릴리스 조회
        const result = await (0, database_1.query)(`SELECT version, download_url, checksum, release_notes, force_update, released_at
       FROM sync_releases
...
router.post('/field-definitions', async (req, res) => {
    try {
        ...
        const validKeys = new Set((0, standard_field_map_1.getCustomFields)().map(f => f.fieldKey));
        ...
        const upsertedCount = await (0, standard_field_map_1.upsertCustomFieldDefinitions)(companyId, mapped);
...
router.post('/ai-mapping', async (req, res) => {
    try {
        const companyId = req.companyId;
        const { target, tableName, dbType, columns } = req.body;
        ...
        const result = await (0, ai_mapping_1.callAiMapping)(companyId, {
            target,
            tableName: tableName || '',
            dbType,
            columns,
        ...
        if (error instanceof ai_mapping_1.AiMappingQuotaExceeded) {
            return res.status(429).json({
                success: false,
                error: error.message,
                code: error.code,
        ...
        if (error instanceof ai_mapping_1.AiMappingUnavailable) {
...
// ============================================
router.get('/field-map', async (_req, res) => {
    try {
        return res.json({
            success: true,
            data: {
                fieldMap: standard_field_map_1.FIELD_MAP.map(f => ({
                    fieldKey: f.fieldKey,
                    category: f.category,
                ...
                categoryLabels: standard_field_map_1.CATEGORY_LABELS,
