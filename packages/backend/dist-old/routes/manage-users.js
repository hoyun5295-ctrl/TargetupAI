...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        let storeCodeList = [];
        if (companyScope) {
            const companyResult = await database_1.default.query('SELECT store_code_list FROM companies WHERE id = $1', [companyScope]);
...
router.post('/', async (req, res) => {
    ...
    try {
        const existing = await database_1.default.query('SELECT id FROM users WHERE login_id = $1', [loginId]);
        if (existing.rows.length > 0) {
            return res.status(400).json({ error: '이미 사용중인 로그인 ID입니다.' });
        }
        // max_users 상한 체크
        const companyCheck = await database_1.default.query('SELECT max_users FROM companies WHERE id = $1', [targetCompanyId]);
        if (companyCheck.rows.length > 0 && companyCheck.rows[0].max_users) {
            // ★ D131 후속: max_users 상한 체크에서 system 가상 계정 제외
            const userCount = await database_1.default.query('SELECT COUNT(*) FROM users WHERE company_id = $1 AND is_active = true AND COALESCE(is_system, false) = false', [targetCompanyId]);
        ...
        if (storeCodes) {
            if (Array.isArray(storeCodes)) {
                ...
                storeCodesArr = storeCodes.split(',').map((s) => s.trim()).filter(Boolean);
            }
        }
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (callerType === 'company_admin') {
            const check = await database_1.default.query('SELECT company_id FROM users WHERE id = $1', [id]);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        if (callerType === 'company_admin') {
            const check = await database_1.default.query('SELECT company_id, user_type FROM users WHERE id = $1', [id]);
            if (check.rows.length === 0)
                return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
            if (check.rows[0].company_id !== callerCompanyId) {
        ...
        const result = await database_1.default.query('DELETE FROM users WHERE id = $1 RETURNING id, login_id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
        }
...
router.post('/:id/reset-password', async (req, res) => {
    ...
    const { userType: callerType, companyId: callerCompanyId } = req.user;
    try {
        const userResult = await database_1.default.query('SELECT id, login_id, name, phone, company_id FROM users WHERE id = $1', [id]);
    ...
    `, [passwordHash, id]);
        ...
        if (user.phone) {
            try {
                ...
                if (!callback)
                    throw new Error('SYSTEM_SMS_CALLBACK 환경변수가 설정되지 않았습니다');
                await (0, database_1.mysqlQuery)(`INSERT INTO ${authTable} (dest_no, call_back, msg_contents, msg_type, sendreq_time, status_code, rsv1) VALUES (?, ?, ?, 'S', NOW(), 100, '1')`, [phone, callback, message]);
                smsSent = true;
            }
            catch (smsError) {
