...
const upload_mapping_validator_1 = require("../utils/upload-mapping-validator");
...
// ================================================================
router.post('/parse', auth_1.authenticate, upload.single('file'), async (req, res) => {
    try {
        ...
        if (companyIdForGating && !isDirectSendParse) {
         ...
         WHERE c.id = $1`, [companyIdForGating]);
        ...
        if (isFirstRowHeader) {
            ...
            headers = firstRow.map((_, idx) => `컬럼${idx + 1}`);
            dataRows = data;
        }
        // 미리보기 (최대 5행)
        ...
        const preview = dataRows.slice(0, 5).map(row => {
            ...
            headers.forEach((h, idx) => {
                ...
                if (val instanceof Date) {
                    val = (0, normalize_1.normalizeDate)(val) || val;
                }
                obj[h] = val;
            });
            return obj;
        ...
        const fileId = path_1.default.basename(filePath);
        // Redis에 메타 저장 (/save에서 활용 — 파일 재파싱 없이 totalRows 확인)
        await defaults_1.redis.set(`upload:${fileId}:meta`, JSON.stringify({
            totalRows,
            headers
        }), 'EX', defaults_1.CACHE_TTL.uploadMeta); // 10분
        const response = {
            success: true,
            fileId,
            headers,
...
// ================================================================
router.post('/mapping', auth_1.authenticate, async (req, res) => {
    try {
        const { headers } = req.body;
        // FIELD_MAP 기반 동적 매핑 대상 생성
        const mappingTargets = {};
        for (const field of standard_field_map_1.FIELD_MAP) {
...
⚠️ 반드시 DB컬럼의 영문 key(phone, name, gender 등)를 값으로 넣어야 합니다. 한글 설명을 넣지 마세요!`;
        ...
        try {
            const response = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                body: JSON.stringify({
                    ...
                    max_tokens: defaults_1.AI_MAX_TOKENS.fieldMapping,
                    messages: [{ role: 'user', content: mappingPrompt }]
                })
            });
            const aiResult = await response.json();
            if (aiResult.error)
                throw new Error(aiResult.error.message || 'Claude API error');
            ...
            const gptResponse = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    max_completion_tokens: defaults_1.AI_MAX_TOKENS.fieldMapping,
                    messages: [{ role: 'user', content: mappingPrompt }]
                })
            });
        ...
        const standardFields = standard_field_map_1.FIELD_MAP.filter(f => f.storageType === 'column').map(f => ({
            fieldKey: f.fieldKey,
        ...
        standardFields.push({ fieldKey: 'birth_year', displayName: '출생연도', category: 'basic', dataType: 'number', sortOrder: 5.1 }, { fieldKey: 'birth_month_day', displayName: '생일(월-일)', category: 'basic', dataType: 'string', sortOrder: 5.2 });
        standardFields.sort((a, b) => a.sortOrder - b.sortOrder);
        return res.json({
            success: true,
            mapping,
            unmapped,
...
// ================================================================
router.post('/validate-mapping', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const filePath = path_1.default.join(uploadDir, safeFileId);
        ...
        const result = await (0, upload_mapping_validator_1.validateUploadMapping)(companyId, mapping, customLabels || {}, sampleData);
...
// ================================================================
router.post('/save', auth_1.authenticate, sync_active_check_1.blockIfSyncActive, async (req, res) => {
    try {
        const { fileId, mapping, customLabels } = req.body;
        const companyId = req.user?.companyId;
        ...
        let totalRows = 0;
        const metaStr = await defaults_1.redis.get(`upload:${fileId}:meta`);
        if (metaStr) {
    ...
    `, [companyId]);
        ...
        // Redis 초기 상태
        await defaults_1.redis.set(`upload:${fileId}:progress`, JSON.stringify({
            status: 'processing',
            total: totalRows,
            processed: 0,
            percent: 0,
            ...
            startedAt,
            message: '처리 시작...'
        }), 'EX', defaults_1.CACHE_TTL.uploadProgress);
        // 즉시 응답 (1초 이내)
...
// 백그라운드 업로드 처리 함수
// ================================================================
async function processUploadInBackground(fileId, filePath, mapping, companyId, userId, startedAt, customLabels) {
    let insertCount = 0;
    ...
    try {
        ...
        const BATCH_SIZE = defaults_1.BATCH_SIZES.customerUpload;
        ...
        const uploadUpsertBuilder = (0, customer_upsert_1.createCustomerUpsertBuilder)({
            source: 'upload',
            includeUploadedBy: true,
        });
        for (let i = 0; i < rows.length; i += BATCH_SIZE) {
            const batch = rows.slice(i, i + BATCH_SIZE);
            const batchRows = []; // 컨트롤타워 buildBatch 입력
            const batchPhones = [];
            ...
            for (const row of batch) {
                ...
                for (const key of Object.keys(record)) {
                    if (key === 'birth_date')
                        continue;
                    if ((0, standard_field_map_1.getFieldByKey)(key)) {
                        record[key] = (0, normalize_1.normalizeByFieldKey)(key, record[key]);
                    }
                }
                ...
                if (record.birth_date) {
                    ...
                    if (record.birth_date instanceof Date) {
                        const normalized = (0, normalize_1.normalizeDate)(record.birth_date);
                ...
                if (record.region) {
                    derivedRegion = (0, normalize_1.normalizeRegion)(record.region);
                }
                ...
                const customObj = {};
                for (const cf of (0, standard_field_map_1.getCustomFields)()) {
                    if (record[cf.fieldKey] != null && record[cf.fieldKey] !== '') {
                        customObj[cf.columnName] = (0, normalize_1.normalizeCustomFieldValue)(record[cf.fieldKey]);
                    }
                }
            ...
            if (batchRows.length > 0) {
                try {
                    ...
                    const { sql, values: queryValues } = uploadUpsertBuilder.buildBatch(companyId, batchRows, userId || null);
                    const result = await (0, database_1.query)(sql, queryValues);
        ...
        if (companyId) {
            try {
                ...
                if (customMappings.length > 0) {
                    await (0, standard_field_map_1.upsertCustomFieldDefinitions)(companyId, customMappings);
                }
            }
            catch (defErr) {
        ...
        if (companyId && userId) {
            try {
                const userTypeResult = await (0, database_1.query)('SELECT user_type FROM users WHERE id = $1', [userId]);
                ...
                if (uploaderType === 'company_admin') {
                    ...
                    const adminUnsubResult = await (0, database_1.query)(`
          ...
          `, [companyId, userId]);
                    ...
                    const unsubCount = await (0, unsubscribe_helper_1.registerBulkCompanyUserUnsubscribes)(companyId, 'db_upload');
                    if (unsubCount > 0) {
                    ...
                    const unsubResult = await (0, database_1.query)(`
            INSERT INTO unsubscribes (company_id, user_id, phone, source)
            SELECT $1, $2, phone, 'db_upload'
            FROM customers
            WHERE company_id = $1 AND sms_opt_in = false AND is_active = true
...
// ================================================================
router.get('/progress/:fileId', auth_1.authenticate, async (req, res) => {
    try {
        const { fileId } = req.params;
        const data = await defaults_1.redis.get(`upload:${fileId}:progress`);
        if (data) {
...
cleanupStaleUploads();
setInterval(cleanupStaleUploads, defaults_1.TIMEOUTS.uploadCleanup);
