...
router.get('/groups', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT group_name, COUNT(*) as count, MAX(created_at) as created_at
       FROM address_books
       WHERE company_id = $1${userFilter}
       GROUP BY group_name
       ORDER BY MAX(created_at) DESC`, params);
...
router.get('/:groupName', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT id, phone, name, extra1, extra2, extra3
       FROM address_books
...
router.post('/', async (req, res) => {
    try {
        ...
        const existCheck = await (0, database_1.query)(`SELECT COUNT(*) FROM address_books WHERE company_id = $1 AND group_name = $2 AND user_id = $3`, [companyId, groupName, userId]);
        ...
        for (const contact of contacts) {
            ...
            if (phone.length >= 10) {
                await (0, database_1.query)(`INSERT INTO address_books (company_id, user_id, group_name, phone, name, extra1, extra2, extra3)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [companyId, userId, groupName, phone, contact.name || '', contact.extra1 || '', contact.extra2 || '', contact.extra3 || '']);
                insertCount++;
            }
        }
...
router.delete('/:groupName', async (req, res) => {
    try {
        ...
        const userFilter = userType === 'company_user' && userId ? ' AND user_id = $3' : '';
        const params = userType === 'company_user' && userId ? [companyId, groupName, userId] : [companyId, groupName];
        await (0, database_1.query)(`DELETE FROM address_books WHERE company_id = $1 AND group_name = $2${userFilter}`, params);
...
//# sourceMappingURL=address-books.js.map