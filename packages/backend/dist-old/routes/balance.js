...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT billing_type, balance, cost_per_sms, cost_per_lms, cost_per_mms, cost_per_kakao
...
router.get('/transactions', async (req, res) => {
    try {
        ...
        const type = req.query.type; // charge, deduct, refund 등 필터
        ...
        // 총 건수
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM balance_transactions ${where}`, params);
        const total = parseInt(countResult.rows[0].count);
        // 이력 조회
        const result = await (0, database_1.query)(`SELECT id, type, amount, balance_after, description, reference_type, reference_id, created_at
...
router.get('/summary', async (req, res) => {
    try {
       ...
       GROUP BY month
       ORDER BY month DESC`, [companyId, months]);
...
router.post('/deposit-request', async (req, res) => {
    try {
        ...
        // 선불 고객사인지 확인
        const companyResult = await (0, database_1.query)('SELECT billing_type, company_name FROM companies WHERE id = $1', [companyId]);
        ...
        const duplicateCheck = await (0, database_1.query)(`SELECT id FROM deposit_requests 
       WHERE company_id = $1 AND amount = $2 AND status = 'pending' 
         AND created_at > NOW() - INTERVAL '10 minutes'`, [companyId, amount]);
        if (duplicateCheck.rows.length > 0) {
            return res.status(400).json({ error: '동일 금액의 입금 요청이 이미 접수되어 있습니다. 잠시 후 다시 시도해주세요.' });
        }
        // deposit_requests에 저장
        const result = await (0, database_1.query)(`INSERT INTO deposit_requests (company_id, amount, depositor_name, status, created_at)
       VALUES ($1, $2, $3, 'pending', NOW())
       RETURNING id, amount, depositor_name, status, created_at`, [companyId, amount, depositorName.trim()]);
