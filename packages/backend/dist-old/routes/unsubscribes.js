...
// ================================================================
async function syncCustomerOptIn(companyId, phone, optIn) {
    const planResult = await (0, database_1.query)(`SELECT plan_id FROM companies WHERE id = $1`, [companyId]);
    if (!planResult.rows[0]?.plan_id)
        return; // 플랜 없으면 스킵
    await (0, database_1.query)(`UPDATE customers SET sms_opt_in = $1, updated_at = NOW()
     WHERE company_id = $2 AND phone = $3 AND sms_opt_in = $4`, [optIn, companyId, phone, !optIn]);
}
// 벌크 버전 (업로드용)
async function syncCustomerOptInBulk(companyId, phones, optIn) {
    const planResult = await (0, database_1.query)(`SELECT plan_id FROM companies WHERE id = $1`, [companyId]);
    if (!planResult.rows[0]?.plan_id)
        return; // 플랜 없으면 스킵
    if (phones.length === 0)
        return;
    await (0, database_1.query)(`UPDATE customers SET sms_opt_in = $1, updated_at = NOW()
     WHERE company_id = $2 AND phone = ANY($3) AND sms_opt_in = $4`, [optIn, companyId, phones, !optIn]);
}
// ================================================================
...
// ================================================================
router.get('/080callback', async (req, res) => {
    try {
        ...
        const result = await (0, unsubscribe_helper_1.process080Callback)(phone, opt080Number);
        if (!result.success) {
...
router.get('/', async (req, res) => {
    try {
        ...
        const { data: paged, total } = await (0, unsubscribe_helper_1.getUserUnsubscribes)(userId, {
            page: Number(page),
            limit: Number(limit),
            search: search,
            companyId,
        ...
        const userInfo = await (0, database_1.query)(`SELECT opt_out_080_number, opt_out_auto_sync FROM users WHERE id = $1`, [userId]);
        ...
        if (!opt080Number) {
            const companyInfo = await (0, database_1.query)(`SELECT opt_out_080_number, opt_out_auto_sync FROM companies WHERE id = $1`, [companyId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        await (0, unsubscribe_helper_1.registerUnsubscribe)(companyId, userId, userType || 'company_user', cleanPhone, 'manual');
...
router.post('/upload', async (req, res) => {
    try {
        ...
        for (const phone of phones) {
            ...
            if (cleanPhone.length >= 10) {
                // CT-03: admin이면 고객 store_code 기준 브랜드 사용자에게 자동 배정
                const cnt = await (0, unsubscribe_helper_1.registerUnsubscribe)(companyId, userId, userType || 'company_user', cleanPhone, 'upload');
                if (cnt > 0) {
                    insertCount += cnt;
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const target = await (0, database_1.query)(`SELECT phone FROM unsubscribes WHERE id = $1 AND user_id = $2`, [id, userId]);
        if (target.rows.length > 0) {
            const targetPhone = target.rows[0].phone;
            // unsubscribes에서 해당 사용자의 해당 번호 삭제
            await (0, database_1.query)(`DELETE FROM unsubscribes WHERE user_id = $1 AND phone = $2`, [userId, targetPhone]);
...
router.post('/check', async (req, res) => {
    try {
        ...
        const dedupResult = (0, deduplicate_1.deduplicateByPhone)(phones.map((p) => ({ phone: String(p || '') })));
        const duplicateCount = dedupResult.duplicateCount;
        const cleanPhones = dedupResult.unique
            .map((r) => String(r.phone || '').replace(/\D/g, ''))
            .filter(Boolean);
        const result = await (0, database_1.query)(`SELECT DISTINCT phone FROM unsubscribes WHERE user_id = $1 AND phone = ANY($2)`, [userId, cleanPhones]);
        return res.json({
            unsubscribeCount: result.rows.length,
            unsubscribePhones: result.rows.map((r) => r.phone),
            duplicateCount,
        });
    }
    catch (error) {
