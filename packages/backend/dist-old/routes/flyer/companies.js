...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT id, company_name, business_type, business_number, owner_name, owner_phone,
       ...
       FROM flyer_companies WHERE id = $1 AND deleted_at IS NULL`, [companyId]);
 ...
 */
router.put('/', flyer_auth_1.requireFlyerAdmin, async (req, res) => {
    try {
        const companyId = req.flyerUser.companyId;
        const { company_name, business_type, business_number, owner_name, owner_phone, address, store_hours, opt_out_080_number, opt_out_080_auto_sync, } = req.body;
        await (0, database_1.query)(`UPDATE flyer_companies SET
...
router.get('/callback-numbers', async (req, res) => {
    try {
        const companyId = req.flyerUser.companyId;
        const result = await (0, database_1.query)(`SELECT id, phone, label, is_default, created_at
...
router.post('/callback-numbers', flyer_auth_1.requireFlyerAdmin, async (req, res) => {
    try {
        const companyId = req.flyerUser.companyId;
        const { phone, label, is_default } = req.body;
        if (!phone)
            return res.status(400).json({ error: '번호가 필요합니다' });
        if (is_default) {
            await (0, database_1.query)(`UPDATE flyer_callback_numbers SET is_default = false WHERE company_id = $1`, [companyId]);
        }
        const result = await (0, database_1.query)(`INSERT INTO flyer_callback_numbers (id, company_id, phone, label, is_default, created_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW())
       RETURNING id, phone, label, is_default`, [companyId, phone, label || null, !!is_default]);
...
router.delete('/callback-numbers/:id', flyer_auth_1.requireFlyerAdmin, async (req, res) => {
    try {
        const companyId = req.flyerUser.companyId;
        const { id } = req.params;
        await (0, database_1.query)(`UPDATE flyer_callback_numbers SET deleted_at = NOW() WHERE id = $1 AND company_id = $2`, [id, companyId]);
