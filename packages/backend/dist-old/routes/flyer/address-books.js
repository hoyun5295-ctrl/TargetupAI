...
router.get('/groups', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT source AS group_name, COUNT(*)::int AS count
...
router.get('/:groupName', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT id, name, phone, gender, email
       FROM flyer_customers
       WHERE company_id = $1 AND source = $2 AND deleted_at IS NULL
       ORDER BY name ASC
       LIMIT 500`, [companyId, groupName]);
        return res.json(result.rows);
    }
    catch (error) {
        return res.status(500).json({ error: 'Server error' });
...
router.post('/', async (req, res) => {
    try {
        ...
        for (const r of recipients) {
            if (!r.phone)
                continue;
            await (0, database_1.query)(`INSERT INTO flyer_customers (id, company_id, name, phone, source, created_at)
         VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW())
         ON CONFLICT (company_id, phone) DO UPDATE SET source = $4, updated_at = NOW()`, [companyId, r.name || null, r.phone, groupName]);
            saved++;
        }
...
router.delete('/:groupName', async (req, res) => {
    try {
        const { companyId } = req.flyerUser;
        const { groupName } = req.params;
        await (0, database_1.query)(`UPDATE flyer_customers SET source = 'manual' WHERE company_id = $1 AND source = $2`, [companyId, groupName]);
