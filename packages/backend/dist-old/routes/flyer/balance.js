...
router.get('/', async (req, res) => {
    try {
        ...
        const userRes = await (0, database_1.query)(`SELECT u.monthly_fee, u.prepaid_balance, u.payment_status, u.plan_started_at, u.plan_expires_at,
       ...
       WHERE u.id = $1`, [userId]);
        ...
        const usage = await (0, flyer_1.aggregateFlyerMonthlyUsage)(companyId, yearMonth);
        return res.json({
...
router.get('/history', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT billing_month, monthly_fee, sms_overage, total_amount, payment_status, paid_at
...
router.get('/transactions', async (req, res) => {
    try {
        const { companyId } = req.flyerUser;
        const result = await (0, database_1.query)(`SELECT id, billing_month, monthly_fee, sms_overage, total_amount, payment_status, paid_at, created_at
       FROM flyer_billing_history
       WHERE company_id = $1
       ORDER BY billing_month DESC
       LIMIT 50`, [companyId]);
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT TO_CHAR(billing_month, 'YYYY-MM') AS month, total_amount, sms_overage, payment_status
       FROM flyer_billing_history
       WHERE company_id = $1
         AND billing_month >= (CURRENT_DATE - ($2 || ' months')::interval)
       ORDER BY billing_month DESC`, [companyId, months]);
...
router.post('/deposit-request', async (req, res) => {
    try {
        ...
        await (0, database_1.query)(`INSERT INTO flyer_billing_history (id, company_id, billing_month, monthly_fee, total_amount, payment_status, created_at)
       VALUES (gen_random_uuid(), $1, DATE_TRUNC('month', CURRENT_DATE), $2, $2, 'pending', NOW())
       ON CONFLICT (company_id, billing_month) DO UPDATE SET
         total_amount = flyer_billing_history.total_amount + EXCLUDED.total_amount`, [companyId, amount]);
...
router.post('/subscribe', async (req, res) => {
    try {
        ...
        // 매장 정보 조회
        const userRes = await (0, database_1.query)(`SELECT id, store_name, monthly_fee, prepaid_balance, payment_status, plan_expires_at
       ...
       RETURNING id, store_name, prepaid_balance, payment_status, plan_started_at, plan_expires_at`, [fee, userId]);
