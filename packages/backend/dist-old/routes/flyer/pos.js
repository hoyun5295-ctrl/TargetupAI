...
async function agentAuth(req, res, next) {
    ...
    if (!agentKey)
        return res.status(401).json({ error: 'agent_key required' });
    const agent = await (0, flyer_1.verifyPosAgent)(agentKey);
    if (!agent)
        return res.status(401).json({ error: 'Invalid agent_key' });
    req.agent = agent;
    next();
}
// ============================================================
...
router.post('/register', agentAuth, async (req, res) => {
    try {
        const { companyId, agentId } = req.agent;
        ...
        await (0, database_1.query)(`UPDATE flyer_pos_agents SET
         ...
         last_heartbeat = NOW(),
         updated_at = NOW()
       WHERE id = $1`, [agentId, hostname, ip_address, pos_type, pos_version]);
        // 기존 스키마 매핑이 있으면 반환
        const existing = await (0, flyer_1.getSchemaMapping)(agentId);
        return res.json({
            ok: true,
            agentId,
            companyId,
...
// ============================================================
router.post('/analyze-schema', agentAuth, async (req, res) => {
    try {
        ...
        // Claude API로 스키마 분석
        const mapping = await (0, flyer_1.analyzeSchema)(rawSchema);
        // 결과 저장
        await (0, flyer_1.saveSchemaMapping)(agentId, mapping);
...
router.get('/config', agentAuth, async (req, res) => {
    try {
        ...
        const mapping = await (0, flyer_1.getSchemaMapping)(agentId);
        // 회사 설정 조회 (싱크 주기 등)
        const companyResult = await (0, database_1.query)(`SELECT company_name, business_type FROM flyer_companies WHERE id = $1`, [companyId]);
...
// ============================================================
router.post('/push', agentAuth, async (req, res) => {
    try {
        const { companyId, agentId } = req.agent;
        ...
        switch (type) {
            case 'sales':
                result = await (0, flyer_1.ingestSales)(companyId, agentId, items);
                break;
            case 'inventory':
                result = await (0, flyer_1.ingestInventory)(companyId, agentId, items);
                break;
            case 'members':
                result = await (0, flyer_1.ingestMembers)(companyId, agentId, items);
                break;
            case 'promotions':
                result = await (0, flyer_1.ingestPromotions)(companyId, agentId, items);
                break;
...
router.post('/heartbeat', agentAuth, async (req, res) => {
    try {
        ...
        await (0, flyer_1.updateAgentHeartbeat)(agentId, last_sync_at || new Date().toISOString(), pending_count, error_count_24h);
...
router.get('/top-selling', flyer_auth_1.flyerAuthenticate, async (req, res) => {
    try {
        const companyId = req.flyerUser?.companyId;
        if (!companyId)
            return res.status(403).json({ error: '회사 정보 없음' });
        const limit = Math.min(Number(req.query.limit) || 20, 50);
        const period = Math.min(Number(req.query.period) || 30, 90);
        const products = await (0, flyer_1.getTopSellingProducts)(companyId, limit, period);
...
router.get('/agents', async (req, res) => {
    try {
        ...
        const agents = await (0, flyer_1.getPosAgentStatusList)();
