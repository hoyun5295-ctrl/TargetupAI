...
const flyer_naver_search_1 = require("../../utils/flyer/product/flyer-naver-search");
const flyer_ai_copy_1 = require("../../utils/flyer/product/flyer-ai-copy");
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await (0, flyer_1.getCatalogItems)(companyId, { category, search });
...
router.post('/', async (req, res) => {
    try {
        const { companyId } = req.flyerUser;
        const id = await (0, flyer_1.upsertCatalogItem)(companyId, req.body);
...
router.post('/:id/touch', async (req, res) => {
    try {
        await (0, flyer_1.touchCatalogUsage)(req.params.id);
...
router.delete('/:id', async (req, res) => {
    try {
        const { companyId } = req.flyerUser;
        await (0, database_1.query)(`DELETE FROM flyer_catalog WHERE id = $1 AND company_id = $2`, [req.params.id, companyId]);
        return res.json({ ok: true });
    }
    catch (error) {
        return res.status(500).json({ error: 'Server error' });
...
router.get('/find-image', async (req, res) => {
    try {
        const name = req.query.name;
        if (!name)
            return res.json({ image_url: null });
        const result = await (0, database_1.query)(`SELECT image_url FROM flyer_catalog
...
router.post('/search-image', async (req, res) => {
    try {
        ...
        if (!product_name)
            return res.status(400).json({ error: 'product_name 필수' });
        const result = await (0, flyer_naver_search_1.searchNaverShopping)(product_name, 5);
        return res.json({
            query: result.query,
            total: result.total,
...
router.post('/select-image', async (req, res) => {
    try {
        ...
        if (!image_url)
            return res.status(400).json({ error: 'image_url 필수' });
        const savedUrl = await (0, flyer_naver_search_1.downloadAndSaveImage)(image_url, companyId);
        if (!savedUrl)
            return res.status(500).json({ error: '이미지 다운로드 실패' });
        // 카탈로그 아이템에 이미지 업데이트
        if (catalog_id) {
            await (0, database_1.query)(`UPDATE flyer_catalog SET image_url = $1, updated_at = NOW() WHERE id = $2 AND company_id = $3`, [savedUrl, catalog_id, companyId]);
        }
...
router.post('/auto-match', async (req, res) => {
    try {
        ...
        if (!product_name)
            return res.status(400).json({ error: 'product_name 필수' });
        const result = await (0, flyer_naver_search_1.autoMatchImage)(product_name, companyId);
        return res.json(result);
    }
    catch (error) {
...
router.post('/batch-match', async (req, res) => {
    try {
        ...
        const results = await (0, flyer_naver_search_1.batchAutoMatchImages)(limited, companyId);
...
router.post('/generate-copy', async (req, res) => {
    try {
        ...
        const validTypes = ['recipe', 'benefit', 'storage', 'selling_point'];
        if (!copy_type || !validTypes.includes(copy_type)) {
            return res.status(400).json({ error: `유효한 문구 유형: ${validTypes.join(', ')}` });
        }
        const copy = await (0, flyer_ai_copy_1.generateProductCopy)(product_name, category || null, copy_type);
        return res.json({
            copy,
            copy_type,
            copy_type_label: flyer_ai_copy_1.COPY_TYPE_LABELS[copy_type],
            product_name,
        });
    }
    catch (error) {
...
// ============================================================
router.post('/generate-batch-copy', async (req, res) => {
    try {
        const { items, copy_type } = req.body;
        if (!items || !Array.isArray(items) || items.length === 0) {
            return res.status(400).json({ error: '상품 목록이 필요합니다.' });
        }
        const validType = ['recipe', 'benefit', 'storage', 'selling_point'].includes(copy_type) ? copy_type : 'selling_point';
        const result = await (0, flyer_ai_copy_1.generateBatchProductCopy)(items.map((it) => ({ name: String(it.name || ''), category: it.category || '' })), validType);
        return res.json({
            copies: result,
            copy_type: validType,
            copy_type_label: flyer_ai_copy_1.COPY_TYPE_LABELS[validType],
