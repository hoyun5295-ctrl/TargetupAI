 ...
 */
router.post('/', flyer_auth_1.requireFlyerAdmin, async (req, res) => {
    try {
        const { companyId } = req.flyerUser;
        const { phone, label } = req.body;
        if (!phone)
            return res.status(400).json({ error: '발신번호가 필요합니다' });
       ...
       RETURNING id, phone, label`, [companyId, phone, label || null]);
