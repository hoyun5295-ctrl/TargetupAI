...
async function smsUnionSelect(tables, fields, whereClause, params, orderBy, limit, offset) {
    ...
    if (limit !== undefined)
        suffix += `LIMIT ${Number(limit)} `;
    if (offset !== undefined)
        suffix += `OFFSET ${Number(offset)}`;
    return await (0, sms_queue_1.smsSelectAll)(tables, fields, where, params, suffix.trim() || undefined);
}
...
router.get('/summary', async (req, res) => {
    try {
       ...
       WHERE c.company_id = $1`;
        ...
        const summaryDr = (0, stats_aggregation_1.buildPeriodFilter)('COALESCE(c.sent_at, c.scheduled_at, c.created_at)', {
        ...
        const summaryMeta = await (0, database_1.query)(summaryQuery, summaryParams);
        const summarySmsMap = await (0, stats_aggregation_1.aggregateSmsCountsByCampaign)(summaryMeta.rows);
        const summaryKakaoMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(summaryMeta.rows.map((c) => c.id));
        ...
        const costResult = await (0, database_1.query)(`SELECT cost_per_sms, cost_per_lms, cost_per_mms, cost_per_kakao FROM companies WHERE id = $1`, [companyId]);
        ...
        return res.json({
            ...
            costs: {
                perSms: parseFloat(costs.cost_per_sms) || defaults_1.DEFAULT_COSTS.sms,
                perLms: parseFloat(costs.cost_per_lms) || defaults_1.DEFAULT_COSTS.lms,
                perMms: parseFloat(costs.cost_per_mms) || defaults_1.DEFAULT_COSTS.mms,
                perKakao: parseFloat(costs.cost_per_kakao) || defaults_1.DEFAULT_COSTS.kakao,
            },
        });
    }
    catch (error) {
...
router.get('/campaigns', async (req, res) => {
    try {
        ...
        //   발송 완료(sent_at) 우선 → 예약 대기(scheduled_at) → 미발송(created_at) 폴백
        //   정산이 발송일 기준이므로 4/30 등록 + 5/7 예약 캠페인은 5월 결과에 표시되어야 함
        const campDr = (0, stats_aggregation_1.buildPeriodFilter)('COALESCE(sent_at, scheduled_at, created_at)', {
            fromDate: fromDate ? String(fromDate) : undefined,
            toDate: toDate ? String(toDate) : undefined,
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM campaigns ${whereClause}`, params);
       ...
       LIMIT $${paramIndex++} OFFSET $${paramIndex}`, params);
        const campListSmsMap = await (0, stats_aggregation_1.aggregateSmsCountsByCampaign)(result.rows);
        const campListKakaoMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(result.rows.map((c) => c.id));
        const campListSentTimeMap = await (0, stats_aggregation_1.aggregateSmsSendTimesByCampaign)(result.rows);
...
router.get('/campaigns/:id', async (req, res) => {
    try {
        ...
        const companyTables = await (0, sms_queue_1.getCompanySmsTablesWithLogs)(companyId, userId);
        const userType = req.user?.userType;
        // ★ B2: opt_out_080_number 포함을 위해 LEFT JOIN
        let detailQuery = `SELECT c.*, ${unsubscribe_helper_1.CAMPAIGN_OPT080_SELECT_EXPR}
                       FROM campaigns c
                       ${unsubscribe_helper_1.CAMPAIGN_OPT080_LEFT_JOIN}
                       WHERE c.id = $1 AND c.company_id = $2`;
        const detailParams = [id, companyId];
        // ★ 사용자는 본인 캠페인만 조회 가능
        if (userType === 'company_user' && userId) {
            detailQuery += ` AND c.created_by = $3`;
            detailParams.push(userId);
        }
        const campaignResult = await (0, database_1.query)(detailQuery, detailParams);
        if (campaignResult.rows.length === 0) {
            return res.status(404).json({ error: '캠페인을 찾을 수 없습니다.' });
        }
        const campaign = campaignResult.rows[0];
        const runsResult = await (0, database_1.query)(`SELECT * FROM campaign_runs WHERE campaign_id = $1 ORDER BY created_at DESC`, [id]);
        ...
        let cacheHit = false;
        try {
            const cached = await defaults_1.redis.get(chartCacheKey);
            if (cached) {
        ...
        if (!cacheHit) {
            ...
            if (sendChannel === 'sms' || sendChannel === 'both') {
                // 실패사유별 집계
                const statusAgg = await (0, sms_queue_1.smsGroupByAll)(companyTables, 'status_code', 'WHERE app_etc1 = ?', [id]);
                for (const [codeStr, cnt] of Object.entries(statusAgg)) {
                    const code = parseInt(codeStr);
                    if (![...sms_result_map_1.SUCCESS_CODES, ...sms_result_map_1.PENDING_CODES].includes(code)) {
                        const label = (0, sms_result_map_1.getStatusLabel)(code);
                        errorStats[label] = (errorStats[label] || 0) + cnt;
                    }
                }
                // 통신사별 집계 (성공 건만) — sms-result-map.ts 상수 사용
                const carrierAgg = await (0, sms_queue_1.smsGroupByAll)(companyTables, 'mob_company', `WHERE app_etc1 = ? AND status_code IN (${sms_result_map_1.SUCCESS_CODES.join(',')})`, [id]);
                for (const [carrier, cnt] of Object.entries(carrierAgg)) {
                    const label = (0, sms_result_map_1.getCarrierLabel)(carrier);
                    carrierStats[label] = (carrierStats[label] || 0) + cnt;
                }
            }
            // ===== 카카오 결과 집계 =====
            if (sendChannel === 'kakao' || sendChannel === 'both') {
                const kakaoErrorResult = await (0, sms_queue_1.kakaoSelectWhere)('REPORT_CODE, COUNT(*) as cnt', `WHERE REQUEST_UID = ?`, [id], 'GROUP BY REPORT_CODE');
                kakaoErrorResult.forEach((row) => {
                ...
                const kakaoResendResult = await (0, sms_queue_1.kakaoSelectWhere)('RESEND_REPORT_CODE, COUNT(*) as cnt', `WHERE REQUEST_UID = ? AND RESEND_MT_TYPE != 'NO' AND RESEND_REPORT_CODE IS NOT NULL AND RESEND_REPORT_CODE != ''`, [id], 'GROUP BY RESEND_REPORT_CODE');
                kakaoResendResult.forEach((row) => {
                    const cnt = parseInt(row.cnt);
            ...
            try {
                const ttl = isCompleted ? defaults_1.CACHE_TTL.resultChartCompleted : defaults_1.CACHE_TTL.resultChartActive;
                await defaults_1.redis.setex(chartCacheKey, ttl, JSON.stringify({ errorStats, carrierStats }));
            }
            catch (e) { /* Redis 실패 시 무시 — 다음 요청에서 재조회 */ }
        }
        // ★ D144: campaign.success_count/fail_count/sent_at 캐시 의존 제거 → MySQL 직접
        const chartSmsMap = await (0, stats_aggregation_1.aggregateSmsCountsByCampaign)([campaign]);
        const chartKakaoMap = await (0, sms_queue_1.kakaoBatchAggByGroup)([campaign.id]);
        const chartSentTimeMap = await (0, stats_aggregation_1.aggregateSmsSendTimesByCampaign)([campaign]);
...
router.get('/campaigns/:id/messages', async (req, res) => {
    try {
        ...
        // ★ userId 전달: 사용자별 라인그룹 테이블 포함 조회
        const msgTables = await (0, sms_queue_1.getCompanySmsTablesWithLogs)(companyId, userId);
        // 캠페인 채널+상태 확인
        const campResult = await (0, database_1.query)('SELECT send_channel, status FROM campaigns WHERE id = $1 AND company_id = $2', [id, companyId]);
        const sendChannel = campResult.rows[0]?.send_channel || 'sms';
        const campStatus = campResult.rows[0]?.status || '';
        // ===== UNION ALL 서브쿼리 빌드 =====
        const dataSubqueries = [];
        const countSubqueries = [];
        const dataParams = [];
        const countParams = [];
        // ----- SMS 서브쿼리 (테이블 수만큼 UNION ALL) -----
        if (sendChannel === 'sms' || sendChannel === 'both') {
            let smsWhere = 'WHERE app_etc1 = ?';
            const smsBaseParams = [id];
            if (searchType && searchValue) {
                ...
                const sv = `%${cleanValue}%`;
                if (searchType === 'phone') {
                    smsWhere += ' AND REPLACE(dest_no, \'-\', \'\') LIKE ?';
                    smsBaseParams.push(sv);
                }
                else if (searchType === 'callback') {
                    smsWhere += ' AND REPLACE(call_back, \'-\', \'\') LIKE ?';
                    smsBaseParams.push(sv);
                }
                else if (searchType === 'content') {
                    smsWhere += ' AND msg_contents LIKE ?';
                    smsBaseParams.push(sv);
                }
            }
            if (status === 'success')
                smsWhere += ` AND status_code IN (${sms_result_map_1.SUCCESS_CODES.join(',')})`;
            else if (status === 'fail')
                smsWhere += ` AND status_code NOT IN (${[...sms_result_map_1.SUCCESS_CODES, ...sms_result_map_1.PENDING_CODES].join(',')})`;
        ...
        if (sendChannel === 'kakao' || sendChannel === 'both') {
            ...
            if (searchType && searchValue) {
                ...
                const sv = `%${cleanValue}%`;
                if (searchType === 'phone') {
                    kakaoWhere += ' AND REPLACE(PHONE_NUMBER, \'-\', \'\') LIKE ?';
                    kakaoBaseParams.push(sv);
                }
                else if (searchType === 'content') {
        ...
        if (countCacheKey) {
            try {
                const cachedCount = await defaults_1.redis.get(countCacheKey);
                if (cachedCount) {
        ...
        if (!countCacheHit) {
            ...
            const countRows = await (0, database_1.mysqlQuery)(countSql, countParams);
            total = parseInt(countRows[0]?.total || '0');
            // 필터 없는 전체 카운트 캐시 (완료 캠페인: 24h / 진행중: 5min)
            if (countCacheKey) {
                try {
                    const isComp = ['completed', 'cancelled'].includes(campStatus);
                    const ttl = isComp ? defaults_1.CACHE_TTL.resultChartCompleted : defaults_1.CACHE_TTL.resultChartActive;
                    await defaults_1.redis.setex(countCacheKey, ttl, String(total));
                }
        ...
        let messages;
        try {
            messages = await (0, database_1.mysqlQuery)(dataSql, dataParams);
        }
        catch (unionErr) {
            console.warn('[messages] UNION ALL 실패 — LOG 테이블 제외 후 LIVE 테이블만 재시도:', unionErr.message);
            // LIVE 테이블만으로 재구성 (LOG 테이블 패턴: SMSQ_SEND_XX_YYYYMM)
            const logPattern = /_\d{6}$/;
            const liveOnlyDataSubs = [];
            const liveOnlyParams = [];
            const smsBaseParams = [id];
            if (searchType && searchValue) {
                ...
                const cleanValue = (searchType === 'phone' || searchType === 'callback')
                    ...
                    : String(searchValue).trim();
                const sv = `%${cleanValue}%`;
                if (['phone', 'callback', 'content'].includes(String(searchType)))
                    smsBaseParams.push(sv);
            }
            const smsFields = SMS_DETAIL_FIELDS;
            let smsWhere = 'WHERE app_etc1 = ?';
            ...
            if (liveOnlyDataSubs.length > 0) {
                ...
                messages = await (0, database_1.mysqlQuery)(fallbackSql, liveOnlyParams);
            }
            else {
                messages = [];
            }
        ...
        const enrichedMessages = messages.map((m) => ({
            ...m,
            status_label: (0, sms_result_map_1.getStatusLabel)(m.status_code),
            status_type: (0, sms_result_map_1.getStatusType)(m.status_code),
            carrier_label: m._channel === 'kakao' ? '카카오' : (0, sms_result_map_1.getCarrierLabel)(m.mob_company),
        }));
        return res.json({
...
router.get('/campaigns/:id/export', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(403).json({ error: '권한이 필요합니다.' });
        const campaignResult = await (0, database_1.query)(`SELECT campaign_name, send_channel, created_at FROM campaigns WHERE id = $1 AND company_id = $2`, [id, companyId]);
        if (campaignResult.rows.length === 0)
            return res.status(404).json({ error: '캠페인을 찾을 수 없습니다.' });
        ...
        const baseParams = [];
        if (sendChannel === 'sms' || sendChannel === 'both') {
            const exportTables = await (0, sms_queue_1.getCompanySmsTablesWithLogs)(companyId, userId);
        ...
        while (true) {
            ...
            const rows = await (0, database_1.mysqlQuery)(`${baseSql} ORDER BY sendreq_time ASC LIMIT ? OFFSET ?`, chunkParams);
            if (rows.length === 0)
                break;
            for (const m of rows) {
                ...
                if (channel === 'kakao') {
                    ...
                    statusDisplay = (0, sms_result_map_1.isSuccess)(m.status_code)
                        ? '카카오성공'
                    ...
                    statusDisplay = (0, sms_result_map_1.getStatusLabel)(m.status_code);
                    carrierDisplay = (0, sms_result_map_1.getCarrierLabel)(m.mob_company);
                }
