...
exports.CAMPAIGN_OPT080_LEFT_JOIN = exports.CAMPAIGN_OPT080_SELECT_EXPR = void 0;
exports.buildUnsubscribeFilter = buildUnsubscribeFilter;
exports.buildUnsubscribeExistsFilter = buildUnsubscribeExistsFilter;
exports.buildUnsubscribeCase = buildUnsubscribeCase;
exports.syncCustomerOptIn = syncCustomerOptIn;
exports.registerBulkCompanyUserUnsubscribes = registerBulkCompanyUserUnsubscribes;
exports.registerUnsubscribe = registerUnsubscribe;
exports.isUnsubscribed = isUnsubscribed;
exports.getUnsubscribedPhones = getUnsubscribedPhones;
exports.findUserBy080Number = findUserBy080Number;
exports.process080Callback = process080Callback;
exports.getUserUnsubscribes = getUserUnsubscribes;
exports.deleteUserUnsubscribes = deleteUserUnsubscribes;
exports.exportUserUnsubscribes = exportUserUnsubscribes;
exports.buildCampaignOpt080LeftJoin = buildCampaignOpt080LeftJoin;
 ...
 */
function buildUnsubscribeFilter(userIdRef, phoneRef) {
 ...
 */
function buildUnsubscribeExistsFilter(userIdRef, phoneRef) {
 ...
 */
function buildUnsubscribeCase(userIdRef, phoneRef, alias = 'is_unsubscribed') {
 ...
 */
async function syncCustomerOptIn(companyId, phones, optIn) {
    if (!phones || phones.length === 0)
        return;
    await (0, database_1.query)(`UPDATE customers SET sms_opt_in = $1, updated_at = NOW()
     WHERE company_id = $2 AND phone = ANY($3::text[])`, [optIn, companyId, phones]);
}
/**
 ...
 */
async function registerBulkCompanyUserUnsubscribes(companyId, source) {
    const result = await (0, database_1.query)(`INSERT INTO unsubscribes (company_id, user_id, phone, source)
     ...
     ON CONFLICT (user_id, phone) DO NOTHING`, [companyId, source]);
    return result.rowCount || 0;
}
async function registerUnsubscribe(companyId, userId, userType, phone, source) {
    let insertCount = 0;
    if (userType === 'company_admin') {
       ...
       ON CONFLICT (user_id, phone) DO NOTHING`, [companyId, userId, phone, source]);
 ...
 */
async function isUnsubscribed(userId, phone) {
    const result = await (0, database_1.query)('SELECT EXISTS(SELECT 1 FROM unsubscribes WHERE user_id = $1 AND phone = $2) as exists', [userId, phone]);
 ...
 */
async function getUnsubscribedPhones(userId, phones) {
    if (!phones || phones.length === 0)
        return [];
    const result = await (0, database_1.query)('SELECT DISTINCT phone FROM unsubscribes WHERE user_id = $1 AND phone = ANY($2)', [userId, phones]);
 ...
 */
async function findUserBy080Number(opt080Number) {
    // 1순위: users 테이블에서 직접 매칭 (사용자 레벨)
    const userResult = await (0, database_1.query)(`SELECT u.id as user_id, u.company_id, c.company_name
     FROM users u
     JOIN companies c ON c.id = u.company_id
     WHERE REPLACE(REPLACE(u.opt_out_080_number, '-', ''), ' ', '') = $1
       AND u.opt_out_auto_sync = true
       AND u.is_active = true
       AND c.status = 'active'`, [opt080Number]);
    if (userResult.rows.length > 0) {
        return userResult.rows.map((r) => ({
            userId: r.user_id,
    ...
    const companyResult = await (0, database_1.query)(`SELECT id, company_name, opt_out_auto_sync FROM companies
     WHERE REPLACE(REPLACE(opt_out_080_number, '-', ''), ' ', '') = $1
       AND status = 'active'
     LIMIT 1`, [opt080Number]);
    ...
    const usersResult = await (0, database_1.query)(`SELECT id FROM users WHERE company_id = $1 AND is_active = true`, [company.id]);
 ...
 * @returns 등록 결과
 */
async function process080Callback(phone, opt080Number) {
    ...
    for (const match of matches) {
       ...
       RETURNING id`, [match.companyId, match.userId, phone]);
    ...
    for (const companyId of companyIds) {
        const adminUsers = await (0, database_1.query)(`SELECT id FROM users
       WHERE company_id = $1 AND user_type = 'admin' AND is_active = true`, [companyId]);
        for (const admin of adminUsers.rows) {
            if (matchedUserIds.has(admin.id))
                continue; // 이미 매칭된 admin이면 스킵
            const result = await (0, database_1.query)(`INSERT INTO unsubscribes (company_id, user_id, phone, source)
         VALUES ($1, $2, $3, '080_ars_sync')
         ON CONFLICT (user_id, phone) DO NOTHING
         RETURNING id`, [companyId, admin.id, phone]);
            if (result.rows.length > 0)
                insertedCount++;
        }
    }
 ...
 */
async function getUserUnsubscribes(userId, options = {}) {
    ...
    const countResult = await (0, database_1.query)(`SELECT COUNT(DISTINCT phone) FROM unsubscribes ${whereClause}`, params);
    const dataResult = await (0, database_1.query)(`SELECT DISTINCT ON (phone) id, phone, source, created_at
     FROM unsubscribes ${whereClause}
     ORDER BY phone, created_at DESC`, params);
    // phone 기준 중복 제거 후 created_at DESC 정렬 + 페이지네이션
    const sorted = dataResult.rows.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
 ...
 * @returns 삭제된 건수
 */
async function deleteUserUnsubscribes(userId, phones) {
    // 먼저 company_id 조회 (sms_opt_in 동기화용)
    const userResult = await (0, database_1.query)('SELECT company_id FROM users WHERE id = $1', [userId]);
    if (userResult.rows.length === 0)
        return 0;
    ...
    if (phones && phones.length > 0) {
        // 선택 삭제
        const result = await (0, database_1.query)(`DELETE FROM unsubscribes WHERE user_id = $1 AND phone = ANY($2) RETURNING phone`, [userId, phones]);
        deletedPhones = result.rows.map((r) => r.phone);
    }
    else {
        // 전체 삭제
        const result = await (0, database_1.query)(`DELETE FROM unsubscribes WHERE user_id = $1 RETURNING phone`, [userId]);
        deletedPhones = result.rows.map((r) => r.phone);
    }
 ...
 */
async function exportUserUnsubscribes(userId) {
    const result = await (0, database_1.query)(`SELECT phone, source, created_at FROM unsubscribes
 ...
 */
exports.CAMPAIGN_OPT080_SELECT_EXPR = `COALESCE(
 ...
 */
function buildCampaignOpt080LeftJoin(campaignAlias = 'c', userIdColumn = 'created_by') {
    return `
 ...
 */
exports.CAMPAIGN_OPT080_LEFT_JOIN = buildCampaignOpt080LeftJoin();
//# sourceMappingURL=unsubscribe-helper.js.map