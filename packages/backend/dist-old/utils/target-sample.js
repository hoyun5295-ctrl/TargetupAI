...
exports.fetchTargetSampleCustomer = fetchTargetSampleCustomer;
 ...
 */
async function fetchTargetSampleCustomer(options) {
    const { companyId, targetFilter, userId, storeCode } = options;
    // 1) 타겟 필터 SQL (CT-01 컨트롤타워)
    const filterResult = (0, customer_filter_1.buildFilterQueryCompat)(targetFilter || {}, companyId);
    ...
    let unsubFilter = '';
    if (userId) {
        unsubFilter = (0, unsubscribe_helper_1.buildUnsubscribeFilter)(`$${paramIdx++}`, 'c.phone');
  ...
  `;
    try {
        const result = await (0, database_1.query)(sql, [companyId, ...filterResult.params, ...extraParams]);
        ...
        const customFlat = Object.fromEntries(Object.entries(row.custom_fields || {}).map(([k, v]) => [k, v == null ? '' : String(v)]));
...
//# sourceMappingURL=target-sample.js.map