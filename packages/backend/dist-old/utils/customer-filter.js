...
exports.buildFilterQueryCompat = buildFilterQueryCompat;
exports.buildDynamicFilterCompat = buildDynamicFilterCompat;
exports.buildFilterWhereClauseCompat = buildFilterWhereClauseCompat;
const normalize_1 = require("./normalize");
const safe_field_name_1 = require("./safe-field-name");
...
function safeDateValue(value) {
    ...
    const normalized = (0, normalize_1.normalizeDate)(s);
    if (normalized)
        return normalized;
...
function normalizeContainsSearch(fieldKey, columnRef, value) {
 ...
 */
function buildCustomerFilter(filters, options) {
    const { tableAlias = '', startParamIndex, storeCodeMode = 'skip', companyIdParamRef = '$1', inputFormat = 'mixed', } = options;
    let sql = '';
    const params = [];
    ...
    if (inputFormat === 'structured') {
        for (const [field, condition] of Object.entries(filters)) {
            ...
            if (['gender', 'grade', 'sms_opt_in', 'region'].includes(field)) {
                ...
                if (effectiveOp === 'eq') {
                    if (field === 'gender') {
                        const gf = (0, normalize_1.buildGenderFilter)(String(value), paramIndex);
                        sql += gf.sql;
                        ...
                        const rf = (0, normalize_1.buildRegionFilter)(String(value), paramIndex);
                        sql += rf.sql;
                ...
                const fieldDef = (0, standard_field_map_1.getColumnFields)().find(f => f.fieldKey === field);
                if (fieldDef) {
                ...
                if (!(0, safe_field_name_1.isValidCustomFieldKey)(field))
                    continue;
    ...
    if (gender) {
        const genderResult = (0, normalize_1.buildGenderFilter)(gender, paramIndex);
    ...
    if (region) {
        ...
        if (regionOp === 'in' && Array.isArray(region)) {
            const allVariants = region.flatMap(r => (0, normalize_1.getRegionVariants)(r));
    ...
    for (const field of (0, standard_field_map_1.getColumnFields)()) {
        if (SPECIAL_FIELDS.has(field.fieldKey))
            continue; // 위에서 이미 처리
    ...
    Object.keys(filters).forEach(key => {
        if (key.startsWith('custom_fields.')) {
            ...
            if (!(0, safe_field_name_1.isValidCustomFieldKey)(fieldName))
                return;
 ...
 */
function buildFilterQueryCompat(filter, _companyId) {
    // ★ D83: 디버그 로그
 ...
 */
function buildDynamicFilterCompat(filters, startIndex) {
 ...
 */
function buildFilterWhereClauseCompat(filters, startParamIndex) {
