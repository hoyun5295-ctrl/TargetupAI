...
exports.buildMonthRangeFilter = buildMonthRangeFilter;
exports.buildPeriodFilter = buildPeriodFilter;
exports.kstGroupBy = kstGroupBy;
exports.kstDate = kstDate;
exports.aggregateSmsCountsByCampaign = aggregateSmsCountsByCampaign;
exports.aggregateSmsSendTimesByCampaign = aggregateSmsSendTimesByCampaign;
exports.querySendStats = querySendStats;
exports.querySendStatsDetail = querySendStatsDetail;
exports.aggregateCampaignPerformance = aggregateCampaignPerformance;
 ...
 */
function buildDateRangeFilter(column, startDate, endDate, startParamIndex = 1) {
    let sql = '';
    const params = [];
 ...
 * // params: ['2026-03-01']
 */
function buildMonthRangeFilter(column, yearMonth, startParamIndex = 1) {
    let sql = '';
    const params = [];
    let paramIndex = startParamIndex;
    // YYYYMM → YYYY-MM-01
 ...
 * @param startParamIndex - 파라미터 시작 인덱스
 */
function buildPeriodFilter(column, options, startParamIndex = 1) {
 ...
 */
function kstGroupBy(column, view) {
 ...
 */
async function aggregateSmsCountsByCampaign(campaigns) {
  ...
  `;
    ...
    const userGroupTables = await Promise.all(Array.from(byUser.values()).map(async (ug) => ({
        ug,
        tables: await (0, sms_queue_1.getCompanySmsTablesWithLogs)(ug.companyId, ug.userId || undefined),
    })));
    // 3) 라인그룹 테이블셋이 동일한 그룹끼리 묶음 (대부분 default BULK_ONLY 공유 → K << N)
    const byTableSet = new Map();
    for (const { ug, tables } of userGroupTables) {
        if (tables.length === 0)
            continue;
        const tableKey = [...tables].sort().join(',');
        if (!byTableSet.has(tableKey))
            byTableSet.set(tableKey, { tables, ids: [] });
        byTableSet.get(tableKey).ids.push(...ug.ids);
    }
    // 4) 라인그룹 테이블셋별 1쿼리(UNION ALL) — K회 호출
    for (const [, group] of byTableSet) {
        const partial = await (0, sms_queue_1.smsBatchAggByGroup)(group.tables, 'app_etc1', smsAggFields, group.ids);
 ...
 */
async function aggregateSmsSendTimesByCampaign(campaigns) {
    const result = new Map();
    if (campaigns.length === 0)
        return result;
    const byUser = new Map();
    ...
    for (const [, group] of byTableSet) {
        if (group.ids.length === 0 || group.tables.length === 0)
            continue;
        ...
        const rows = await (0, database_1.mysqlQuery)(sql, params);
        for (const r of rows) {
            if (!r._grp || !r.first_sendreq_iso)
                continue;
            const d = new Date(String(r.first_sendreq_iso));
            if (!isNaN(d.getTime()))
                result.set(String(r._grp), d);
        }
    }
    return result;
}
 ...
 */
async function querySendStats(options) {
  ...
  `, baseParams);
    ...
    const kakaoCountMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(campaigns.map((c) => c.id));
 ...
 */
async function querySendStatsDetail(options, DEFAULT_COSTS_PARAM) {
    ...
    // 비용 단가
    const costRes = await (0, database_1.query)('SELECT cost_per_sms, cost_per_lms FROM companies WHERE id = $1', [companyId]);
  ...
  `, detailParams);
    ...
    const kakaoCountMap = await (0, sms_queue_1.kakaoBatchAggByGroup)(metaRows.map((c) => c.id));
    ...
    for (const c of metaRows) {
        ...
        if (isSms)
            u.sms_success += success;
        if (isLmsOrMms)
            u.lms_success += success;
 ...
 */
async function aggregateCampaignPerformance(companyId, months = 3) {
    const emptyResult = {
        bySegment: [],
        byTimeSlot: [],
        byMessageType: [],
    ...
    `;
        // 1) 총 캠페인 수
        const totalResult = await (0, database_1.query)(`SELECT COUNT(*) as cnt FROM campaigns c WHERE ${baseWhere}`, [companyId]);
        const totalCampaigns = parseInt(totalResult.rows[0].cnt);
        if (totalCampaigns === 0) {
            return emptyResult;
        }
        // 2) 메시지 타입별 성과
        const byTypeResult = await (0, database_1.query)(`SELECT
        c.message_type,
       ...
       ORDER BY avg_success_rate DESC`, [companyId]);
        // 3) KST 시간대별 성과
        const byTimeResult = await (0, database_1.query)(`SELECT
       ...
       ORDER BY avg_success_rate DESC`, [companyId]);
        ...
        const bySegmentResult = await (0, database_1.query)(`SELECT
        CONCAT_WS(' / ',
       ...
       LIMIT 10`, [companyId]);
        // 5) 성과 좋은 캠페인 TOP 5
        const topResult = await (0, database_1.query)(`SELECT
        c.campaign_name,
        c.message_type,
        c.target_count,
        CASE WHEN c.sent_count > 0
...
//# sourceMappingURL=stats-aggregation.js.map