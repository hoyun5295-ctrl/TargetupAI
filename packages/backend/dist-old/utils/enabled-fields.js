...
exports.detectEnabledFields = detectEnabledFields;
exports.buildDynamicSelectExpr = buildDynamicSelectExpr;
...
function resolveCustomDataType(defType) {
    if (!defType)
        return 'string';
    const upper = defType.toUpperCase();
    if (NUMERIC_TYPE_KEYS.includes(upper))
        return 'number';
    if (DATE_TYPE_KEYS.includes(upper))
        return 'date';
    return 'string';
}
async function detectCustomFieldTypeFromSamples(companyId, scopeWhere, scopeParams, fieldKey) {
    try {
        const sampleResult = await (0, database_1.query)(`SELECT DISTINCT custom_fields->>'${fieldKey}' as val
         FROM customers
        WHERE ${scopeWhere}
          AND custom_fields->>'${fieldKey}' IS NOT NULL
          AND custom_fields->>'${fieldKey}' != ''
        LIMIT 20`, scopeParams);
        const samples = sampleResult.rows
            .map((r) => r.val)
            .filter((v) => v != null && String(v).trim() !== '');
        if (samples.length === 0)
            return 'string';
        const allNumeric = samples.every((v) => NUM_PATTERN.test(String(v).trim()));
        if (allNumeric)
            return 'number';
        const allDate = samples.every((v) => DATE_PATTERN.test(String(v).trim()));
        if (allDate)
            return 'date';
    }
    catch {
 ...
 */
async function detectEnabledFields(params) {
    const { companyId, scopeWhere, scopeParams } = params;
    const fields = [];
    const existingKeys = new Set();
    // ─── 0. customer_field_definitions 라벨/타입 맵 ───
    const fieldDefLabels = {};
    const fieldDefTypes = {};
    const fieldDefOrder = {};
    try {
        const defRes = await (0, database_1.query)(`SELECT field_key, field_label, field_type, display_order
         FROM customer_field_definitions
        WHERE company_id = $1 AND (is_hidden = false OR is_hidden IS NULL)
        ORDER BY display_order`, [companyId]);
        for (const fd of defRes.rows) {
            if (fd.field_label)
                fieldDefLabels[fd.field_key] = fd.field_label;
            if (fd.field_type)
                fieldDefTypes[fd.field_key] = fd.field_type;
            if (fd.display_order != null)
                fieldDefOrder[fd.field_key] = fd.display_order;
        }
    }
    catch (err) {
        console.warn('[CT-18] customer_field_definitions 조회 실패 — FIELD_MAP만 사용:', err?.message);
    }
    // ─── 1. 직접 컬럼 필드 — FIELD_MAP + COUNT FILTER 동적 감지 ───
    // name, phone: 항상 포함 (필수)
    const nameField = (0, standard_field_map_1.getFieldByKey)('name');
    const phoneField = (0, standard_field_map_1.getFieldByKey)('phone');
    fields.push({
        field_key: 'name',
    ...
    if (detectableFields.length > 0) {
        ...
        try {
            const dataCheckResult = await (0, database_1.query)(`SELECT ${countFilters.join(', ')} FROM customers WHERE ${scopeWhere}`, scopeParams);
            ...
            console.warn('[CT-18] 직접 컬럼 COUNT FILTER 실패:', err?.message);
        }
    }
    // ─── 2. 커스텀 필드 — JSONB 실키 ∪ field_definitions union ───
    try {
        const customKeysResult = await (0, database_1.query)(`SELECT DISTINCT jsonb_object_keys(custom_fields) as field_key
         FROM customers
        WHERE ${scopeWhere}
          AND custom_fields IS NOT NULL
          AND custom_fields != '{}'::jsonb`, scopeParams);
        const jsonbKeys = customKeysResult.rows.map((r) => r.field_key);
        ...
        for (const fieldKey of jsonbKeys) {
            ...
            let detectedType = resolveCustomDataType(defType);
            ...
            fields.push({
                ...
                sort_order: fieldDefOrder[fieldKey] != null
        ...
        console.warn('[CT-18] custom_fields JSONB 키 조회 실패:', err?.message);
    }
    // 정렬: sort_order 기준
 ...
 */
function buildDynamicSelectExpr(fields, options = {}) {
    ...
    for (const f of fields) {
        ...
        if (f.field_key === 'sms_opt_in' && unsubParamIndex) {
            parts.push(`CASE WHEN EXISTS (SELECT 1 FROM unsubscribes u WHERE u.user_id = $${unsubParamIndex} AND u.phone = ${tableAlias}.phone) THEN false ELSE ${col} END AS ${f.field_key}`);
        }
...
//# sourceMappingURL=enabled-fields.js.map