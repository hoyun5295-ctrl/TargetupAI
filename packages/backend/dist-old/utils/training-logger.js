...
exports.TRAINING_VERSIONS = void 0;
exports.maskForTraining = maskForTraining;
exports.computeMessageFeatures = computeMessageFeatures;
exports.logTrainingData = logTrainingData;
exports.updateTrainingMetrics = updateTrainingMetrics;
exports.getSourceRef = getSourceRef;
...
// ============================================================
function maskForTraining(text, companyName) {
    if (!text)
        return '';
    let masked = text;
    ...
    masked = masked.replace(/(\d{2,4}[-.\s]?\d{3,4}[-.\s]?\d{4})/g, '{phone}');
    // 금액 마스킹 (₩1,000, 1000원, 10,000원, 50% 등)
    masked = masked.replace(/₩?\s?\d{1,3}(,\d{3})*\s?원/g, '{amount}');
    masked = masked.replace(/\d{1,3}(,\d{3})+/g, '{amount}');
    // URL 마스킹
...
// ============================================================
function computeMessageFeatures(text) {
    if (!text) {
        return {
            emoji_count: 0,
            sentence_count: 0,
...
// ============================================================
function buildCandidates(aiMessages, companyName) {
...
async function logTrainingData(params) {
    ...
    `;
        await database_1.default.query(query, [
            ...
            exports.TRAINING_VERSIONS.prompt,
            exports.TRAINING_VERSIONS.persona,
            exports.TRAINING_VERSIONS.policy,
            params.modelId || defaults_1.AI_MODELS.claude,
            params.modelParams ? JSON.stringify(params.modelParams) : null,
            JSON.stringify(guardrailActions),
            exports.TRAINING_VERSIONS.redaction,
        ]);
...
// ============================================================
async function updateTrainingMetrics(params) {
    try {
        const query = `
...
// ============================================================
function getSourceRef(campaignRunId) {
    return hmacHash(campaignRunId);
}
//# sourceMappingURL=training-logger.js.map