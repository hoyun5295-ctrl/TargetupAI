...
exports.syncCategoriesJob = syncCategoriesJob;
exports.syncPendingTemplatesJob = syncPendingTemplatesJob;
exports.syncSenderStatusJob = syncSenderStatusJob;
exports.startAlimtalkScheduler = startAlimtalkScheduler;
exports.stopAlimtalkScheduler = stopAlimtalkScheduler;
const database_1 = require("../config/database");
const imc = __importStar(require("./alimtalk-api"));
...
async function syncCategoriesJob() {
    if (!envReady()) {
    ...
    try {
        ...
        if (senderRes?.code === '0000' && leafs.length > 0) {
            ...
            for (const leaf of leafs) {
                ...
                const nameRaw = String(leaf?.name || '');
                if (code.length !== 11)
                    continue;
                ...
                if (!seen.has(l1Code)) {
                    seen.add(l1Code);
                    await (0, database_1.query)(`INSERT INTO kakao_sender_categories (category_code, parent_code, level, name, active_yn, synced_at)
             ...
             ON CONFLICT (category_code) DO UPDATE SET
               ...
               name        = EXCLUDED.name,
               active_yn   = 'Y',
               synced_at   = now()`, [l2Code, l1Code, l2Name]);
                    ancestorCount++;
                }
           ...
           ON CONFLICT (category_code) DO UPDATE SET
             ...
             name        = EXCLUDED.name,
             active_yn   = 'Y',
             synced_at   = now()`, [l3Code, l2Code, l3Name]);
                leafCount++;
            }
            log('categorySync', `sender 카테고리 L3 ${leafCount}건 + L1/L2 ${ancestorCount}건 동기화 (총 ${leafCount + ancestorCount}건)`);
        }
        else {
            logErr('categorySync-sender', new Error(`응답 구조 불일치 or 빈 배열 — code=${senderRes?.code}, leafCount=${leafs.length}, keys=${senderRes?.data ? Object.keys(senderRes.data).slice(0, 5).join(',') : 'n/a'}`));
        }
    }
    catch (err) {
    ...
    try {
        ...
        if (tplRes?.code === '0000' && list.length > 0) {
            for (const c of list) {
                if (!c?.code)
                    continue;
                await (0, database_1.query)(`INSERT INTO kakao_template_categories
           ...
           ON CONFLICT (category_code) DO UPDATE SET
            ...
            logErr('categorySync-template', new Error(`응답 구조 불일치 or 빈 배열 — code=${tplRes?.code}, list=${list.length}`));
        }
    }
    catch (err) {
...
// ════════════════════════════════════════════════════════════
async function syncPendingTemplatesJob() {
    if (!envReady()) {
    ...
    let rows = [];
    try {
        const res = await (0, database_1.query)(`SELECT t.id,
              t.company_id,
              p.profile_key,
              p.profile_name,
              t.template_code,
    ...
    for (const row of rows) {
        try {
            const res = await imc.getAlimtalkTemplate(row.profile_key, row.template_code);
            if (res.code !== '0000' || !res.data)
                continue;
            const rawLatestStatus = res.data.status;
            if (!rawLatestStatus)
                continue;
            ...
            const rejectReason = res.data.rejectReason ?? null;
            await (0, database_1.query)(`UPDATE kakao_templates
            SET status          = $1,
                reject_reason   = $2,
                last_synced_at  = now(),
                updated_at      = now()
          WHERE id = $3`, [latestStatus, rejectReason, row.id]);
            updated++;
            ...
            if (terminal && row.alarm_notified_status !== terminal) {
                try {
                    ...
                    await (0, database_1.query)(`UPDATE kakao_templates SET alarm_notified_status = $1 WHERE id = $2`, [terminal, row.id]);
                    if (count > 0)
                        notified += count;
                    log('pendingTemplateSync', `alarmNotify ${terminal} ${row.template_name} → ${count}명`);
                }
                catch (notifyErr) {
 ...
 */
function normalizeImcTemplateStatus(status) {
...
async function notifyTemplateInspectionResult(params) {
    // 1) 활성 수신자 조회
    const usersRes = await (0, database_1.query)(`SELECT name, phone_number
       FROM kakao_alarm_users
      WHERE company_id = $1 AND COALESCE(active_yn, 'Y') = 'Y'`, [params.companyId]);
    const users = usersRes.rows || [];
    if (users.length === 0)
        return 0;
    // 2) 해당 발신프로필 관리자 번호 조회 (callback으로 사용)
    const profRes = await (0, database_1.query)(`SELECT admin_phone_number FROM kakao_sender_profiles WHERE profile_key = $1 LIMIT 1`, [params.profileKey]);
    const callback = String(profRes.rows[0]?.admin_phone_number || '').replace(/\D/g, '');
    if (!callback) {
        // callback 미등록 시 발송 불가. 로그만 남기고 스킵 (기간계 안정성 우선)
        logErr('notifyTemplateInspectionResult', new Error(`profile.admin_phone_number 없음 — profile_key=${params.profileKey}`));
        return 0;
    }
    // 3) 메시지 빌드
    const notifyMessage = (0, auto_notify_message_1.buildTemplateInspectionNotifyMessage)({
    ...
    const authTable = await (0, sms_queue_1.getAuthSmsTable)();
    const rows = [];
    ...
    if (rows.length === 0)
        return 0;
    await (0, sms_queue_1.bulkInsertSmsQueue)([authTable], rows, true);
    return rows.length;
}
// ════════════════════════════════════════════════════════════
// 3) 발신프로필 상태 폴링 (1시간 주기)
// ════════════════════════════════════════════════════════════
async function syncSenderStatusJob() {
    if (!envReady()) {
    ...
    let rows = [];
    try {
        const res = await (0, database_1.query)(`SELECT id, profile_key, status, yellow_id
...
let _categoryTimer = null;
let _templateTimer = null;
let _senderTimer = null;
function nextKst03() {
...
function startAlimtalkScheduler() {
    ...
    _templateTimer = setInterval(() => {
        syncPendingTemplatesJob().catch((e) => logErr('pendingTemplateSync-interval', e));
    }, 5 * 60 * 1000);
    // 3) 발신프로필 — 1시간 주기
...
/** 테스트/재시작용 */
function stopAlimtalkScheduler() {
    if (_categoryTimer)
        clearTimeout(_categoryTimer);
    if (_categoryTimer)
        clearInterval(_categoryTimer);
    if (_templateTimer)
        clearInterval(_templateTimer);
    if (_senderTimer)
        clearInterval(_senderTimer);
    _categoryTimer = _templateTimer = _senderTimer = null;
    _scheduled = false;
}
//# sourceMappingURL=alimtalk-jobs.js.map