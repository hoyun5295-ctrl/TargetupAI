...
exports.AiMappingUnavailable = exports.AiMappingQuotaExceeded = void 0;
exports.callAiMapping = callAiMapping;
...
class AiMappingQuotaExceeded extends Error {
...
exports.AiMappingQuotaExceeded = AiMappingQuotaExceeded;
class AiMappingUnavailable extends Error {
    code = 'AI_MAPPING_UNAVAILABLE';
    constructor(detail) {
        super(`AI 매핑 호출 실패: ${detail}`);
    }
}
exports.AiMappingUnavailable = AiMappingUnavailable;
// ============================================================
 ...
 */
async function checkQuotaAndReset(companyId) {
    ...
    if (ai_mapping_last_month !== now) {
        await (0, database_1.query)(`UPDATE companies SET ai_mapping_calls_month = 0, ai_mapping_last_month = $1, updated_at = NOW() WHERE id = $2`, [now, companyId]);
...
/** 호출 성공 후 카운트 증가 */
async function incrementQuota(companyId) {
    const now = currentYearMonth();
    await (0, database_1.query)(`UPDATE companies
       SET ai_mapping_calls_month = COALESCE(ai_mapping_calls_month, 0) + 1,
           ai_mapping_last_month = $1,
           updated_at = NOW()
     WHERE id = $2`, [now, companyId]);
}
// ============================================================
// 프롬프트 구성
// ============================================================
 ...
 */
function buildFieldMapReference() {
    const lines = [];
...
async function callClaudeModel(modelId, input) {
...
function sanitizeMapping(mapping, columns) {
    const validKeys = new Set([
        ...standard_field_map_1.FIELD_MAP.filter((f) => f.storageType === 'column').map((f) => f.fieldKey),
...
function estimateCost(modelUsed, tokensUsed) {
 ...
 */
async function callAiMapping(companyId, input) {
    ...
    try {
        ...
        console.log(`[AI Mapping] Opus 호출 성공 (company=${companyId}, tokens=${raw.tokensUsed}, cacheHit=${raw.cacheHit})`);
    }
    catch (opusErr) {
        ...
        try {
            ...
            console.log(`[AI Mapping] Sonnet 폴백 성공 (company=${companyId}, tokens=${raw.tokensUsed})`);
        }
        catch (sonnetErr) {
            console.error(`[AI Mapping] Sonnet도 실패 (${sonnetErr.status || sonnetErr.message})`);
            throw new AiMappingUnavailable(`Opus+Sonnet 모두 실패. Agent 로컬 autoSuggestMapping을 사용하세요.`);
        }
    }
    // 응답 파싱
    ...
    console.log(`[AI Mapping] 완료 (company=${companyId}, model=${raw.modelUsed}, used=${usedBefore + 1}/${limit})`);
    return {
        mapping,
...
//# sourceMappingURL=ai-mapping.js.map