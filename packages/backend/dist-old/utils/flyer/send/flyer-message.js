...
exports.replaceFlyerVariables = replaceFlyerVariables;
exports.buildFlyerAdMessage = buildFlyerAdMessage;
exports.stripFlyerAdParts = stripFlyerAdParts;
exports.prepareFlyerSendMessage = prepareFlyerSendMessage;
const VAR_ALIAS_TO_KEY = {
    '이름': 'name',
    '고객명': 'name',
    '성명': 'name',
    '전화': 'phone',
...
function formatVarValue(key, value) {
    if (value == null || value === '')
        return '';
    // 숫자: 천단위 콤마
 ...
 */
function replaceFlyerVariables(template, customer) {
    if (!template)
        return '';
    return template.replace(/%([^%\n]+)%/g, (_match, varName) => {
        const trimmed = varName.trim();
        const key = VAR_ALIAS_TO_KEY[trimmed] || (trimmed in customer ? trimmed : null);
        if (!key)
            return '';
 ...
 */
function buildFlyerAdMessage(body, isAd, opt080) {
    if (!isAd)
        return body;
    ...
    if (opt080)
        lines.push(`무료수신거부 ${opt080}`);
    return lines.join('\n');
}
/**
 * (광고) 프리픽스/무료거부 라인 제거 — 표시용 (DB에 저장된 원문에 이미 있으면 정규화).
 */
function stripFlyerAdParts(body) {
    if (!body)
        return '';
    let clean = body.replace(/^\(광고\)\s*/, '').trim();
    clean = clean.replace(/\n*무료(수신|거부).*$/s, '').trim();
