...
exports.buildFlyerCustomerFilter = buildFlyerCustomerFilter;
exports.countFlyerCustomers = countFlyerCustomers;
exports.selectFlyerCustomers = selectFlyerCustomers;
const database_1 = require("../../../config/database");
/**
 * WHERE 절 빌더. company_id는 항상 강제 포함.
 * 반환 params의 첫 번째는 항상 companyId.
 */
function buildFlyerCustomerFilter(companyId, filter = {}) {
    const conditions = [`company_id = $1`, `deleted_at IS NULL`];
    const params = [companyId];
    const add = (cond, ...values) => {
 ...
 * 필터 결과 COUNT.
 */
async function countFlyerCustomers(companyId, filter = {}) {
    const { whereClause, params } = buildFlyerCustomerFilter(companyId, filter);
    const result = await (0, database_1.query)(`SELECT COUNT(*)::int AS cnt FROM flyer_customers WHERE ${whereClause}`, params);
    return result.rows[0]?.cnt || 0;
}
/**
 * 필터 결과 phone + id 목록 (발송 대상 추출).
 */
async function selectFlyerCustomers(companyId, filter = {}, options = {}) {
    const { whereClause, params } = buildFlyerCustomerFilter(companyId, filter);
    ...
    const result = await (0, database_1.query)(`SELECT ${fields} FROM flyer_customers
     WHERE ${whereClause}
     ORDER BY last_purchase_at DESC NULLS LAST, created_at DESC
     LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
    return result.rows;
}
//# sourceMappingURL=flyer-customer-filter.js.map