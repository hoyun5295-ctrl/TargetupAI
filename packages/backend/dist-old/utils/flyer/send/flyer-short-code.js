...
exports.getShortUrlDomain = getShortUrlDomain;
exports.generateShortCode = generateShortCode;
exports.bulkInsertTrackingUrls = bulkInsertTrackingUrls;
exports.generateTrackingUrls = generateTrackingUrls;
exports.getTrackingStats = getTrackingStats;
...
function generateUniqueCodes(phones) {
...
// ============================================================
async function bulkInsertTrackingUrls(flyerId, companyId, campaignId, recipients) {
...
async function insertBatch(flyerId, companyId, campaignId, batch) {
  ...
  `;
    ...
    if (inserted < batch.length) {
        ...
        const existing = await (0, database_1.query)(`SELECT code FROM short_urls WHERE code = ANY($1)`, [codes]);
...
// ============================================================
async function generateTrackingUrls(flyerId, companyId, campaignId, phones) {
    if (phones.length === 0)
        return new Map();
    // 1. 코드 생성 (메모리)
...
async function getTrackingStats(flyerId, campaignId) {
    // 발송된 추적 URL 전체 조회
    const whereExtra = campaignId ? ' AND su.campaign_id = $2' : '';
    const params = [flyerId];
    if (campaignId)
        params.push(campaignId);
    const sentResult = await (0, database_1.query)(`SELECT su.phone
     FROM short_urls su
     WHERE su.flyer_id = $1 AND su.url_type = 'tracking'${whereExtra}`, params);
    const sentPhones = sentResult.rows.map((r) => r.phone);
    ...
    const clickResult = await (0, database_1.query)(`SELECT uc.phone,
     ...
     GROUP BY uc.phone`, params);
    const clickedSet = new Set();
    const clickedList = clickResult.rows.map((r) => {
...
//# sourceMappingURL=flyer-short-code.js.map