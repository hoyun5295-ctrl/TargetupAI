...
exports.verifyPosAgent = verifyPosAgent;
exports.ingestSales = ingestSales;
exports.ingestInventory = ingestInventory;
exports.ingestMembers = ingestMembers;
exports.updateAgentHeartbeat = updateAgentHeartbeat;
exports.getTopSellingProducts = getTopSellingProducts;
exports.getPosAgentStatusList = getPosAgentStatusList;
exports.ingestPromotions = ingestPromotions;
const database_1 = require("../../../config/database");
const normalize_phone_1 = require("../../normalize-phone");
/**
 * POS Agent 인증 (agent_key 검증).
 */
async function verifyPosAgent(agentKey) {
    const result = await (0, database_1.query)(`SELECT pa.id, pa.company_id
     FROM flyer_pos_agents pa
     WHERE pa.agent_key = $1
       AND EXISTS (SELECT 1 FROM flyer_companies fc WHERE fc.id = pa.company_id AND fc.deleted_at IS NULL)`, [agentKey]);
 ...
 * - 카탈로그 자동 등록 (신상품 감지)
 */
async function ingestSales(companyId, agentId, items) {
    let accepted = 0;
    let rejected = 0;
    const errors = [];
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            // 판매 데이터 UPSERT
            await (0, database_1.query)(`INSERT INTO flyer_pos_sales
         ...
         DO UPDATE SET
           ...
           pos_raw = EXCLUDED.pos_raw`, [
            ...
            if (item.pos_member_id) {
                await (0, database_1.query)(`UPDATE flyer_customers
           SET total_purchase_amount = COALESCE(total_purchase_amount, 0) + $3,
               ...
               updated_at = NOW()
           WHERE company_id = $1 AND pos_member_id = $2`, [companyId, item.pos_member_id, item.total_amount || item.sale_price || 0, item.sold_at]);
            }
            // 카탈로그 자동 등록 (신상품)
            if (item.product_code && item.product_name) {
                await (0, database_1.query)(`INSERT INTO flyer_catalog (company_id, product_name, category, default_price, pos_product_code)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (company_id, pos_product_code) WHERE pos_product_code IS NOT NULL
           DO UPDATE SET
             default_price = EXCLUDED.default_price,
             updated_at = NOW()`, [companyId, item.product_name, item.category || '기타', item.sale_price || item.unit_price || 0, item.product_code]);
            }
            accepted++;
        }
        catch (err) {
 ...
 */
async function ingestInventory(companyId, agentId, items) {
    let accepted = 0;
    let rejected = 0;
    const errors = [];
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            await (0, database_1.query)(`INSERT INTO flyer_pos_inventory
 ...
 * - 마스킹된 번호 자동 스킵 + 경고
 */
async function ingestMembers(companyId, agentId, members) {
    let accepted = 0;
    let rejected = 0;
    const errors = [];
    for (let i = 0; i < members.length; i++) {
        ...
        try {
            ...
            // 전화번호 정규화
            const phone = m.phone ? (0, normalize_phone_1.normalizePhone)(m.phone) : null;
            // 마스킹된 전화번호 감지 → 스킵
            ...
            if (!phone) {
                ...
                const existing = await (0, database_1.query)(`SELECT id FROM flyer_customers WHERE company_id = $1 AND pos_member_id = $2 LIMIT 1`, [companyId, m.pos_member_id]);
                if (existing.rows.length > 0) {
                    await (0, database_1.query)(`UPDATE flyer_customers SET
               name = COALESCE($3, name),
               gender = COALESCE($4, gender),
               pos_grade = COALESCE($5, pos_grade),
               pos_points = COALESCE($6, pos_points),
               updated_at = NOW()
             WHERE id = $2`, [companyId, existing.rows[0].id, m.name, normalizeGender(m.gender), m.grade, m.points]);
                    accepted++;
                }
                else {
                    rejected++;
 ...
 * Agent 하트비트 업데이트.
 */
async function updateAgentHeartbeat(agentId, lastSyncAt, pendingCount, errorCount24h) {
    await (0, database_1.query)(`UPDATE flyer_pos_agents SET
       last_heartbeat = NOW(),
       last_sync_at = $2,
       sync_status = CASE WHEN $4 > 5 THEN 'error' ELSE 'connected' END,
       updated_at = NOW()
     WHERE id = $1`, [agentId, lastSyncAt, pendingCount, errorCount24h]);
}
/**
 * ★ POS 판매 데이터 기반 인기 상품 TOP N 추천.
 * 최근 period일간 판매 수량 기준 정렬.
 */
async function getTopSellingProducts(companyId, limit = 20, period = 30) {
    const result = await (0, database_1.query)(`SELECT
     ...
     LIMIT $2`, [companyId, limit, String(period)]);
 ...
 */
async function getPosAgentStatusList() {
    const result = await (0, database_1.query)(`SELECT pa.id AS agent_id, pa.company_id,
...
async function ingestPromotions(companyId, posAgentId, items) {
    let accepted = 0;
    let rejected = 0;
    const errors = [];
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            await (0, database_1.query)(`INSERT INTO flyer_pos_promotions
...
function normalizeGender(gender) {
    ...
    if (['M', '남', '남성', '1', 'MALE'].includes(g))
        return 'M';
    if (['F', '여', '여성', '2', 'FEMALE'].includes(g))
        return 'F';
    return null;
}
//# sourceMappingURL=flyer-pos-ingest.js.map