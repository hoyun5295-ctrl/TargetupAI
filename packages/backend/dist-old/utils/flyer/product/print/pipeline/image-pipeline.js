...
const flyer_rembg_1 = require("../../flyer-rembg");
const product_images_1 = require("../../../../product-images");
const flyer_naver_search_1 = require("../../flyer-naver-search");
// ============================================================
// 이미지 다운로드/디코딩
// ============================================================
...
// ============================================================
async function processOne(product, opts) {
    let imageUrl = product.imageUrl || '';
    // 1. 이미지 없으면 기존 자산에서 찾기
    if (!imageUrl && opts.autoMatchImage) {
        // 1-1. PRODUCT_MAP (한국 마트 상품 60개 키워드 매핑, 로컬 서빙)
        const display = (0, product_images_1.getProductDisplay)(product.productName || '');
        if (display.imageUrl) {
            ...
            try {
                const shopResult = await (0, flyer_naver_search_1.searchNaverShopping)(product.productName || '', 1);
                const first = shopResult?.items?.[0] || (Array.isArray(shopResult) ? shopResult[0] : null);
                if (first?.image)
    ...
    if (imageUrl && opts.autoRembg) {
        ...
        if (buffer && buffer.length > 0) {
            try {
                const removed = await (0, flyer_rembg_1.removeBackground)(buffer);
...
// 공개 API — 상품 배열 일괄 처리
// ============================================================
async function processProductImages(products, opts = {}) {
    if (!opts.autoRembg && !opts.autoMatchImage)
        return products;
