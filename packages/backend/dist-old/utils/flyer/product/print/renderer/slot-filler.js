...
exports.FILL_RUNTIME = void 0;
exports.resolveSlotData = resolveSlotData;
// ============================================================
// 유틸
// ============================================================
...
function matchCategory(p, prefer) {
 ...
 */
function ribbonTextByRate(rate, isTopItem) {
    if (isTopItem && rate >= 40)
        return '한정특가';
    if (rate >= 50)
        return '파격세일';
    if (rate >= 40)
 ...
 */
function badgeByRate(rate, index) {
    if (rate >= 45)
        return { kind: 'hot', text: 'HOT' };
    if (rate >= 35)
        return { kind: 'best', text: 'BEST' };
    if (rate >= 25)
        return { kind: 'pick', text: 'PICK' };
    if (rate >= 15)
        return { kind: 'new', text: '추천' };
 ...
 */
function toCardViewModel(p, index = 0) {
...
function resolveTextSlot(slot, input) {
    ...
    if (typeof ov === 'string' && ov.length > 0)
        return { value: ov };
    // id 매핑 예약어
    if (slot.id === 'hero_title' && input.heroTitle)
        return { value: input.heroTitle };
    if (slot.id === 'hero_subcopy' && input.heroSubcopy)
        return { value: input.heroSubcopy };
    return { value: slot.fallback || '' };
}
function resolveSectionBanner(slot, input) {
    const ov = input.slotOverrides?.[slot.id];
    return {
...
function resolveStoreHeader(slot, input) {
    const s = input.store || {};
    const q = input.qr || {};
    return {
        store: {
...
function resolveProductGrid(slot, input) {
    const selection = slot.selection || { mode: 'highest_discount' };
    const minItems = slot.minItems || 0;
    const maxItems = slot.maxItems || slot.cols * slot.rows || 99;
...
function resolveCategoryGrid(slot, input, usedCategories) {
    const cfg = slot.category || { mode: 'auto' };
    const cols = slot.cols || 3;
    const rows = slot.rows || 3;
    ...
    if (cfg.mode === 'fixed' && cfg.name) {
        ...
        if (!categoryLabel) {
            ...
            for (const p of input.products) {
                ...
                if (!c || seen.has(c) || usedCategories.has(c))
                    continue;
                seen.add(c);
            }
            for (const c of seen) {
...
function resolveFooterNotice(slot, input) {
    const ov = input.slotOverrides?.[slot.id];
...
// ============================================================
function resolveSlotData(manifest, input) {
    const out = {};
 ...
 */
exports.FILL_RUNTIME = String.raw `
(function(){
