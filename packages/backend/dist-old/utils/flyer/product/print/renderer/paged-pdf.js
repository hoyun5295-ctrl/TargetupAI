...
exports.renderFlyerPdf = renderFlyerPdf;
const flyer_pdf_1 = require("../../flyer-pdf");
...
// HTML 조립
// ============================================================
function htmlEscapeJsonForScript(obj) {
    // <script> 안에 직접 삽입할 때 </script> 이스케이프 처리
    return JSON.stringify(obj).replace(/</g, '\\u003c');
}
function assembleHtml(templateHtml, templateCss, slotData, opts) {
...
<script>${slot_filler_1.FILL_RUNTIME}</script>
${pagedScript}
...
// Public API
// ============================================================
async function renderFlyerPdf(options) {
    const startedAt = Date.now();
    const timeoutMs = options.timeoutMs || 60000;
    const log = (...a) => { if (options.debug)
        console.log('[paged-pdf]', ...a); };
    const format = options.format === 'png' ? 'png' : 'pdf';
    // 1) 템플릿 로드
    const tpl = await (0, template_registry_1.loadTemplate)(options.templateId);
    ...
    // 2) 슬롯 데이터 resolve
    const slotData = (0, slot_filler_1.resolveSlotData)(tpl.manifest, options.input);
    ...
    // 4) Puppeteer 렌더
    const browser = await (0, flyer_pdf_1.getPuppeteerBrowser)();
    const page = await browser.newPage();
    try {
        const dims = (0, PAPER_SIZES_1.getPaperDimensions)(paperSize, orientation);
        ...
        const pageCount = options.skipPagedJs
            ? 1
            : (await page.evaluate('document.querySelectorAll(".pagedjs_page").length')) || 1;
...
//# sourceMappingURL=paged-pdf.js.map