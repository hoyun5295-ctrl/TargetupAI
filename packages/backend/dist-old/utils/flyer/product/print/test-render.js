...
const paged_pdf_1 = require("./renderer/paged-pdf");
const flyer_pdf_1 = require("../flyer-pdf");
// ─── SVG placeholder (외부 네트워크 의존 없음) ─────
function svgPlaceholder(label, bg = '#F0FDF4', fg = '#0F172A') {
...
async function renderOne(templateId) {
    console.log('━'.repeat(60));
    console.log('▶', templateId);
    console.log('━'.repeat(60));
    const t0 = Date.now();
    const result = await (0, paged_pdf_1.renderFlyerPdf)({
        templateId,
        input: dummyInput,
        debug: true,
        timeoutMs: 90000,
    });
    const outDir = path_1.default.resolve(process.cwd(), 'packages/backend/pdfs');
    fs_1.default.mkdirSync(outDir, { recursive: true });
    if (result.pdf) {
        const pdfPath = path_1.default.join(outDir, `${templateId}_test.pdf`);
        fs_1.default.writeFileSync(pdfPath, result.pdf);
        console.log('   PDF      :', pdfPath, '(' + (result.pdf.length / 1024).toFixed(1) + ' KB)');
    }
    if (result.screenshot) {
        const pngPath = path_1.default.join(outDir, `${templateId}_test.png`);
        fs_1.default.writeFileSync(pngPath, result.screenshot);
        console.log('   PNG      :', pngPath, '(' + (result.screenshot.length / 1024).toFixed(1) + ' KB)');
    }
    if (result.html) {
        const htmlPath = path_1.default.join(outDir, `${templateId}_test.html`);
        fs_1.default.writeFileSync(htmlPath, result.html);
    }
...
async function main() {
    ...
    try {
        ...
        await (0, flyer_pdf_1.closePdfBrowser)().catch(() => { });
    }
}
main();
