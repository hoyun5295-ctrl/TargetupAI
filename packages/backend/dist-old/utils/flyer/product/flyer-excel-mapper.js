...
exports.mapFlyerExcelHeaders = mapFlyerExcelHeaders;
exports.applyFlyerMapping = applyFlyerMapping;
exports.getFlyerMappingFields = getFlyerMappingFields;
const defaults_1 = require("../../../config/defaults");
// ============================================================
// 매핑 대상 필드 정의
// ============================================================
const FLYER_PRODUCT_FIELDS = {
...
// AI 자동 매핑
// ============================================================
async function mapFlyerExcelHeaders(headers) {
    ...
    try {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                ...
                max_tokens: defaults_1.AI_MAX_TOKENS.fieldMapping || 1024,
        ...
        const gptRes = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: defaults_1.AI_MODELS.gpt || 'gpt-4o-mini',
                max_completion_tokens: defaults_1.AI_MAX_TOKENS.fieldMapping || 1024,
                messages: [{ role: 'user', content: mappingPrompt }],
            }),
        });
...
// ============================================================
function applyFlyerMapping(rows, mapping) {
    // 역매핑: DB필드 → 엑셀헤더
    const reverseMap = {};
    for (const [excelHeader, field] of Object.entries(mapping)) {
        if (field)
            reverseMap[field] = excelHeader;
    }
    return rows
        .map(row => {
        const getName = (field) => {
        ...
        if (/메인|best|특가|대표/.test(promoTypeRaw))
            promoType = 'main';
        else if (/서브|추천|인기/.test(promoTypeRaw))
            promoType = 'sub';
...
// ============================================================
function getFlyerMappingFields() {
...
//# sourceMappingURL=flyer-excel-mapper.js.map