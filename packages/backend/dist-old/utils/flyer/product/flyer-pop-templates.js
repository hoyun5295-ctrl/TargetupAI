...
exports.POP_TEMPLATES = void 0;
exports.renderPricePop = renderPricePop;
exports.renderMultiPop = renderMultiPop;
exports.renderPromoPop = renderPromoPop;
exports.POP_TEMPLATES = [
...
function metaText(item) {
    const parts = [];
    if (item.unit)
        parts.push(esc(item.unit));
    if (item.origin)
        parts.push(esc(item.origin));
    return parts.join(' / ');
}
...
// ============================================================
function renderHotPop(item, opts) {
...
// ============================================================
function renderClassicPop(item, opts) {
    const paper = getPaperSize(opts);
    const disc = calcDisc(item.originalPrice, item.salePrice);
    const hasOrig = item.originalPrice > 0 && item.originalPrice !== item.salePrice;
    const saved = hasOrig ? item.originalPrice - item.salePrice : 0;
...
// ③ 심플 화이트 — 깔끔한 흰 배경
// ============================================================
function renderSimplePop(item, opts) {
    const paper = getPaperSize(opts);
    const disc = calcDisc(item.originalPrice, item.salePrice);
    const hasOrig = item.originalPrice > 0 && item.originalPrice !== item.salePrice;
    const saved = hasOrig ? item.originalPrice - item.salePrice : 0;
...
// ============================================================
function renderDarkPop(item, opts) {
    const paper = getPaperSize(opts);
    const disc = calcDisc(item.originalPrice, item.salePrice);
    const hasOrig = item.originalPrice > 0 && item.originalPrice !== item.salePrice;
    const saved = hasOrig ? item.originalPrice - item.salePrice : 0;
...
// ============================================================
function renderJumboPop(item, opts) {
    const paper = getPaperSize(opts);
    const disc = calcDisc(item.originalPrice, item.salePrice);
    const hasOrig = item.originalPrice > 0 && item.originalPrice !== item.salePrice;
    const meta = metaText(item);
...
// ============================================================
function renderPricePop(item, options = {}) {
...
function renderMultiPop(items, splits, options = {}) {
...
// ★ 홍보POP (코너 안내판)
// ============================================================
function renderPromoPop(category, items, options = {}) {
    const paper = getPaperSize(options);
...
//# sourceMappingURL=flyer-pop-templates.js.map