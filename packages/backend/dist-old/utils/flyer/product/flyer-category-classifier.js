...
// 1단계: 키워드 규칙 매칭
// ============================================================
function classifyByKeywords(items, businessType) {
    ...
    for (const item of items) {
        ...
        for (const rule of rules) {
            if (rule.pattern.test(name)) {
                if (!classified[rule.category])
                    classified[rule.category] = [];
...
// 2단계: 카탈로그 DB 참조
// ============================================================
async function classifyByCatalog(unclassified, companyId) {
    if (unclassified.length === 0)
        return { classified: {}, remaining: [] };
    const placeholders = unclassified.map((_, i) => `$${i + 2}`).join(',');
    const result = await (0, database_1.query)(`SELECT product_name, category FROM flyer_catalog
     WHERE company_id = $1 AND product_name IN (${placeholders}) AND category IS NOT NULL`, [companyId, ...unclassified]);
...
// 3단계: AI 폴백 (미분류만 배치)
// ============================================================
async function classifyByAI(remaining, businessType, availableCategories) {
...
//# sourceMappingURL=flyer-category-classifier.js.map