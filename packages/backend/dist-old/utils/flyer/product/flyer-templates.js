...
function resolveImg(name, size, imageUrl) {
    const absUrl = toAbsUrl(imageUrl || (0, product_images_1.resolveProductImageUrl)(name));
    return (0, product_images_1.renderProductImage)(name, size, absUrl || undefined);
}
/** 상품 메타 칩(규격/원산지) HTML */
function renderMetaChips(item, chipBg, chipColor) {
    const chips = [];
    if (item.unit)
        chips.push(`<span class="mc">${esc(item.unit)}</span>`);
    if (item.origin)
        chips.push(`<span class="mc og">${esc(item.origin)}</span>`);
    if (chips.length === 0)
        return '';
    return `<div class="mw" style="--chip-bg:${chipBg};--chip-color:${chipColor}">${chips.join('')}</div>`;
}
/** 카드할인 라인 HTML */
function renderCardDiscount(item, iconColor) {
    if (!item.cardDiscount)
        return '';
...
function renderAiCopy(item, color) {
    if (!item.aiCopy)
        return '';
...
function renderDynamicSection(d) {
    const parts = [];
    // GIF 배너
    if (d.bannerGifUrl) {
    ...
    if (d.announcements && d.announcements.length > 0) {
        const items = d.announcements.map(a => `<details class="dyn-ann"><summary>${esc(a.title)}</summary><p>${esc(a.content)}</p></details>`).join('');
...
// ============================================================
function htmlWrap(title, css, body, script) {
    return `<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
// ============================================================
function renderCatTabs(d) {
    return d.categories.map((cat, i) => `<a class="ct${i === 0 ? ' on' : ''}" href="#s${i}">${esc(cat.name || '')}</a>`).join('');
}
// ============================================================
// ★ 엔진 1: GRID — 2열 카드 그리드 (가격 최우선 + 장식 히어로)
// ============================================================
function renderGridEngine(d, t) {
...
// ============================================================
function renderMagazineEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderEditorialEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderShowcaseEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderCompactEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderHeroBannerEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderSwipeEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
// ============================================================
function renderMosaicEngine(d, t) {
    const catTabs = renderCatTabs(d);
    let sections = '';
    for (let ci = 0; ci < d.categories.length; ci++) {
        const cat = d.categories[ci];
...
//# sourceMappingURL=flyer-templates.js.map