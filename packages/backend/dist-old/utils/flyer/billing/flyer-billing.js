...
exports.aggregateFlyerMonthlyUsage = aggregateFlyerMonthlyUsage;
exports.recordFlyerMonthlyBilling = recordFlyerMonthlyBilling;
exports.canFlyerCompanySend = canFlyerCompanySend;
exports.canFlyerStoreSend = canFlyerStoreSend;
exports.deductFlyerPrepaid = deductFlyerPrepaid;
exports.refundFlyerPrepaid = refundFlyerPrepaid;
const database_1 = require("../../../config/database");
/**
 * 회사 월 발송량 집계 (flyer_campaigns 기준).
 * 기본 정액 15만원 + 초과분 (단가 x 발송수) 계산.
 */
async function aggregateFlyerMonthlyUsage(companyId, yearMonth // 'YYYY-MM'
) {
    ...
    const result = await (0, database_1.query)(`SELECT
     ...
     WHERE company_id = $1
       ...
       AND status = 'completed'`, [companyId, monthStart]);
    const { sms, lms, mms } = result.rows[0] || { sms: 0, lms: 0, mms: 0 };
    // 단가 조회
    const priceRes = await (0, database_1.query)(`SELECT sms_unit_price, lms_unit_price, mms_unit_price, monthly_fee
     FROM flyer_companies WHERE id = $1`, [companyId]);
 ...
 */
async function recordFlyerMonthlyBilling(companyId, yearMonth) {
    const summary = await aggregateFlyerMonthlyUsage(companyId, yearMonth);
    const monthStart = `${yearMonth}-01`;
    const feeRes = await (0, database_1.query)(`SELECT monthly_fee FROM flyer_companies WHERE id = $1`, [companyId]);
    const monthlyFee = Number(feeRes.rows[0]?.monthly_fee || 150000);
    const overage = Math.max(0, summary.total_cost - monthlyFee);
    await (0, database_1.query)(`INSERT INTO flyer_billing_history
     ...
     ON CONFLICT (company_id, billing_month) DO UPDATE SET
       ...
       total_amount = EXCLUDED.total_amount`, [companyId, monthStart, monthlyFee, overage, summary.total_cost]);
}
/**
 * [하위호환] 총판(flyer_companies) 레벨 발송 가능 여부.
 * 총판 정지 시 하위 전체 매장 차단. canFlyerStoreSend에서 내부 호출됨.
 */
async function canFlyerCompanySend(companyId) {
    const result = await (0, database_1.query)(`SELECT payment_status, plan_expires_at FROM flyer_companies WHERE id = $1`, [companyId]);
    if (result.rows.length === 0)
        return { ok: false, reason: 'company_not_found' };
    const c = result.rows[0];
    if (c.payment_status === 'suspended')
        return { ok: false, reason: '총판 계정이 정지되었습니다' };
 ...
 */
async function canFlyerStoreSend(userId) {
    const userRes = await (0, database_1.query)(`SELECT u.payment_status, u.plan_expires_at, u.company_id
     FROM flyer_users u WHERE u.id = $1 AND u.deleted_at IS NULL`, [userId]);
    if (userRes.rows.length === 0)
        return { ok: false, reason: '매장 정보를 찾을 수 없습니다' };
    const u = userRes.rows[0];
    // 매장 레벨 체크
    if (u.payment_status === 'suspended')
        return { ok: false, reason: '매장 구독이 정지되었습니다' };
    if (u.payment_status === 'pending')
        return { ok: false, reason: '매장 구독이 아직 활성화되지 않았습니다' };
 ...
 */
async function deductFlyerPrepaid(userId, count, messageType) {
    // 단가 조회
    const priceRes = await (0, database_1.query)(`SELECT sms_unit_price, lms_unit_price, mms_unit_price, prepaid_balance
     FROM flyer_users WHERE id = $1 AND deleted_at IS NULL`, [userId]);
    ...
    const updateRes = await (0, database_1.query)(`UPDATE flyer_users
     SET prepaid_balance = prepaid_balance - $1
     WHERE id = $2 AND prepaid_balance >= $1 AND deleted_at IS NULL
     RETURNING prepaid_balance`, [totalAmount, userId]);
 ...
 */
async function refundFlyerPrepaid(userId, amount) {
    if (amount <= 0)
        return { ok: true };
    const result = await (0, database_1.query)(`UPDATE flyer_users
     SET prepaid_balance = prepaid_balance + $1
     WHERE id = $2 AND deleted_at IS NULL
     RETURNING prepaid_balance`, [amount, userId]);
    if (result.rows.length === 0)
        return { ok: false };
    return { ok: true, balance: Number(result.rows[0].prepaid_balance) };
}
//# sourceMappingURL=flyer-billing.js.map