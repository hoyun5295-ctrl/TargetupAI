...
exports.createCouponCampaign = createCouponCampaign;
exports.listCouponCampaigns = listCouponCampaigns;
exports.getCouponCampaign = getCouponCampaign;
exports.updateCouponCampaign = updateCouponCampaign;
exports.disableCouponCampaign = disableCouponCampaign;
exports.getCampaignByQrCode = getCampaignByQrCode;
exports.claimCoupon = claimCoupon;
exports.redeemCoupon = redeemCoupon;
exports.lookupCouponsByPhone = lookupCouponsByPhone;
exports.getCouponStats = getCouponStats;
exports.listCoupons = listCoupons;
exports.renderCouponPage = renderCouponPage;
exports.getCouponDashboard = getCouponDashboard;
exports.buildCouponSmsMessage = buildCouponSmsMessage;
...
async function generateQrCode() {
    for (let i = 0; i < 5; i++) {
        const code = crypto_1.default.randomBytes(4).toString('base64url').slice(0, 6).toUpperCase();
        const exists = await (0, database_1.query)('SELECT 1 FROM flyer_coupon_campaigns WHERE qr_code = $1', [code]);
...
async function generateCouponCode() {
    for (let i = 0; i < 10; i++) {
        const code = crypto_1.default.randomBytes(3).toString('base64url').slice(0, 4).toUpperCase();
        const exists = await (0, database_1.query)('SELECT 1 FROM flyer_coupons WHERE coupon_code = $1', [code]);
        if (exists.rows.length === 0)
            return code;
    }
    return crypto_1.default.randomBytes(4).toString('base64url').slice(0, 6).toUpperCase();
}
...
// ============================================================
/** 쿠폰 캠페인 생성 */
async function createCouponCampaign(params) {
    const qrCode = await generateQrCode();
    const qrUrl = `${COUPON_DOMAIN}/q/${qrCode}`;
    const qrDataUrl = await generateQrDataUrl(qrCode);
    const result = await (0, database_1.query)(`INSERT INTO flyer_coupon_campaigns
...
/** 쿠폰 캠페인 목록 조회 */
async function listCouponCampaigns(companyId) {
...
/** 쿠폰 캠페인 상세 조회 */
async function getCouponCampaign(id, companyId) {
    const result = await (0, database_1.query)(`SELECT * FROM flyer_coupon_campaigns WHERE id = $1 AND company_id = $2`, [id, companyId]);
    return result.rows[0] || null;
}
/** 쿠폰 캠페인 수정 */
async function updateCouponCampaign(id, companyId, updates) {
    const sets = [];
    const vals = [];
    let idx = 3;
    if (updates.couponName !== undefined) {
        sets.push(`coupon_name = $${idx++}`);
        vals.push(updates.couponName);
    }
    if (updates.discountValue !== undefined) {
        sets.push(`discount_value = $${idx++}`);
        vals.push(updates.discountValue);
    }
    if (updates.discountDescription !== undefined) {
        sets.push(`discount_description = $${idx++}`);
        vals.push(updates.discountDescription);
    }
    if (updates.minPurchase !== undefined) {
        sets.push(`min_purchase = $${idx++}`);
        vals.push(updates.minPurchase);
    }
    if (updates.maxIssues !== undefined) {
        sets.push(`max_issues = $${idx++}`);
        vals.push(updates.maxIssues);
    }
    ...
    if (sets.length === 0)
        return getCouponCampaign(id, companyId);
    sets.push('updated_at = NOW()');
    const result = await (0, database_1.query)(`UPDATE flyer_coupon_campaigns SET ${sets.join(', ')} WHERE id = $1 AND company_id = $2 RETURNING *`, [id, companyId, ...vals]);
    return result.rows[0] || null;
}
/** 쿠폰 캠페인 비활성화 */
async function disableCouponCampaign(id, companyId) {
    const result = await (0, database_1.query)(`UPDATE flyer_coupon_campaigns SET status = 'disabled', updated_at = NOW() WHERE id = $1 AND company_id = $2`, [id, companyId]);
...
// ============================================================
/** QR 코드로 캠페인 조회 (공개) */
async function getCampaignByQrCode(qrCode) {
    const result = await (0, database_1.query)(`SELECT cc.*,
...
async function claimCoupon(qrCode, phone, name) {
    ...
    const dup = await (0, database_1.query)(`SELECT 1 FROM flyer_coupons WHERE campaign_id = $1 AND customer_phone = $2`, [campaign.id, normalized]);
    if (dup.rows.length > 0) {
        return { ok: false, error: '이미 쿠폰을 받으셨습니다.' };
    }
    // 쿠폰 코드 생성 + INSERT
    const couponCode = await generateCouponCode();
    await (0, database_1.query)(`INSERT INTO flyer_coupons (campaign_id, company_id, customer_phone, customer_name, coupon_code)
     VALUES ($1, $2, $3, $4, $5)`, [campaign.id, campaign.company_id, normalized, name || null, couponCode]);
    // 캠페인 발급수 증가
    await (0, database_1.query)(`UPDATE flyer_coupon_campaigns SET issued_count = issued_count + 1, updated_at = NOW() WHERE id = $1`, [campaign.id]);
    return {
        ok: true,
        couponCode,
...
// ============================================================
/** 쿠폰 코드로 사용 처리 */
async function redeemCoupon(couponCode, companyId, redeemedBy, purchaseAmount) {
    const result = await (0, database_1.query)(`SELECT c.*, cc.coupon_name, cc.discount_value, cc.discount_description, cc.coupon_type
     FROM flyer_coupons c
     JOIN flyer_coupon_campaigns cc ON cc.id = c.campaign_id
     WHERE c.coupon_code = $1 AND c.company_id = $2 AND c.status = 'issued'`, [couponCode.toUpperCase(), companyId]);
    if (result.rows.length === 0) {
        return { ok: false, error: '유효한 쿠폰을 찾을 수 없습니다.' };
    }
    const coupon = result.rows[0];
    // 사용 처리
    await (0, database_1.query)(`UPDATE flyer_coupons SET status = 'redeemed', redeemed_at = NOW(), redeemed_by = $2, purchase_amount = $3
     WHERE id = $1`, [coupon.id, redeemedBy, purchaseAmount || null]);
    // 캠페인 사용수 + 할인액 업데이트
    await (0, database_1.query)(`UPDATE flyer_coupon_campaigns
     SET redeemed_count = redeemed_count + 1,
         total_redemption_amount = total_redemption_amount + $2,
         updated_at = NOW()
     WHERE id = $1`, [coupon.campaign_id, coupon.discount_value]);
    return {
        ok: true,
...
/** 전화번호로 미사용 쿠폰 조회 */
async function lookupCouponsByPhone(phone, companyId) {
    const normalized = (0, normalize_phone_1.normalizePhone)(phone);
    if (!normalized)
        return [];
    const result = await (0, database_1.query)(`SELECT c.*, cc.coupon_name, cc.discount_value, cc.discount_description,
            cc.coupon_type, cc.expires_at as campaign_expires_at
     FROM flyer_coupons c
     JOIN flyer_coupon_campaigns cc ON cc.id = c.campaign_id
     WHERE c.customer_phone = $1 AND c.company_id = $2 AND c.status = 'issued'
     ORDER BY c.issued_at DESC`, [normalized, companyId]);
    return result.rows;
}
// ============================================================
// 통계
// ============================================================
/** 쿠폰 캠페인 통계 */
async function getCouponStats(campaignId, companyId) {
    const result = await (0, database_1.query)(`SELECT
       cc.issued_count,
       cc.redeemed_count,
     ...
     FROM flyer_coupon_campaigns cc
     WHERE cc.id = $1 AND cc.company_id = $2`, [campaignId, companyId]);
    if (result.rows.length === 0)
        return null;
...
/** 캠페인의 발급된 쿠폰 목록 */
async function listCoupons(campaignId, companyId) {
    const result = await (0, database_1.query)(`SELECT * FROM flyer_coupons
     WHERE campaign_id = $1 AND company_id = $2
     ORDER BY issued_at DESC`, [campaignId, companyId]);
    return result.rows;
}
// ============================================================
// 공개 페이지 HTML 렌더링
...
/** QR 쿠폰 공개 페이지 HTML */
function renderCouponPage(campaign) {
...
async function getCouponDashboard(companyId) {
    const [aggResult, trendResult, campaignResult] = await Promise.all([
        (0, database_1.query)(`SELECT COUNT(*) AS total_campaigns, SUM(issued_count) AS total_issued, SUM(redeemed_count) AS total_redeemed,
              CASE WHEN SUM(issued_count) > 0 THEN ROUND(SUM(redeemed_count)::numeric / SUM(issued_count) * 100, 1) ELSE 0 END AS conversion_rate
       FROM flyer_coupon_campaigns WHERE company_id = $1`, [companyId]),
        (0, database_1.query)(`SELECT TO_CHAR(c.issued_at, 'YYYY-MM-DD') AS date, COUNT(*) AS issued,
       ...
       GROUP BY TO_CHAR(c.issued_at, 'YYYY-MM-DD') ORDER BY date`, [companyId]),
        (0, database_1.query)(`SELECT id, coupon_name, coupon_type, discount_value, issued_count, redeemed_count,
              CASE WHEN issued_count > 0 THEN ROUND(redeemed_count::numeric / issued_count * 100, 1) ELSE 0 END AS conversion_rate,
              TO_CHAR(created_at, 'YYYY-MM-DD') AS created_at
       FROM flyer_coupon_campaigns WHERE company_id = $1 ORDER BY created_at DESC LIMIT 20`, [companyId]),
    ]);
...
/** 쿠폰 수령 완료 SMS 메시지 생성 */
function buildCouponSmsMessage(storeName, couponCode, discountDesc, expiresAt) {
    const lines = [
...
//# sourceMappingURL=flyer-coupons.js.map