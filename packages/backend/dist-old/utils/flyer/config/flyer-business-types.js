...
exports.invalidateBusinessTypeCache = invalidateBusinessTypeCache;
...
function invalidateBusinessTypeCache() {
    _cache = null;
    _cacheTime = 0;
}
// ============================================================
...
async function getBusinessTypes() {
    ...
    if (_cache && now - _cacheTime < CACHE_TTL_MS)
        return _cache;
    const result = await (0, database_1.query)(`SELECT type_code, type_name, category_presets, default_template, is_active, sort_order
 ...
 * 단건 조회. 캐시에서 검색.
 */
async function getBusinessType(typeCode) {
 ...
 */
async function getCategoryPresets(typeCode) {
 ...
 */
async function getAvailableTemplates(typeCode) {
    // 공통 템플릿 (모든 업종 사용 가능)
    const commonCodes = [
    ...
    const typeCodes = Object.keys(exports.TEMPLATE_REGISTRY).filter(code => commonCodes.includes(code) || code.startsWith(prefix));
    return typeCodes
        .map(code => exports.TEMPLATE_REGISTRY[code])
        .filter((t) => !!t);
}
/**
...
//# sourceMappingURL=flyer-business-types.js.map