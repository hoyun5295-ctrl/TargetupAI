...
exports.detectFieldTypeFromSamples = detectFieldTypeFromSamples;
exports.validateUploadMapping = validateUploadMapping;
const database_1 = require("../config/database");
/**
 * 샘플 값 배열에서 field_type 자동 감지.
 * upload.ts D101 로직과 동일 규칙 (YYMMDD 우선 → 숫자 → VARCHAR fallback).
 */
function detectFieldTypeFromSamples(samples) {
    const valid = samples.filter(v => v != null && v !== '');
    if (valid.length === 0)
        return 'VARCHAR';
    // 날짜 감지 — YYYY-MM-DD / YYYYMMDD / YYMMDD / Date 객체
    const allDate = valid.every((v) => {
    ...
    const looksLikeDate6 = valid.every((v) => {
        const s = String(v).trim();
        if (!/^\d{6}$/.test(s))
            return true;
    ...
    if (allDate && looksLikeDate6)
        return 'DATE';
    // 숫자 감지 — 전체가 숫자로 파싱 가능
    const allNumeric = valid.every((v) => {
        const s = String(v).trim();
        if (!s)
            return false;
        // 전화번호 제외
 ...
 */
async function validateUploadMapping(companyId, proposedMapping, customLabels, sampleData) {
    // 기존 customer_field_definitions 전부 조회
    const existingResult = await (0, database_1.query)(`SELECT field_key, field_label, field_type
     FROM customer_field_definitions
     WHERE company_id = $1 AND field_key LIKE 'custom_%'`, [companyId]);
    // ★ D114 P2: 고객 0명이면 충돌을 warning으로 격하 (기존 데이터 없으므로 의미 충돌 없음)
    const customerCountResult = await (0, database_1.query)('SELECT COUNT(*) as cnt FROM customers WHERE company_id = $1 AND is_active = true', [companyId]);
    ...
    const labelToHeadersInFile = new Map();
    let standardCount = 0;
    let customCount = 0;
    let ignoredCount = 0;
    for (const [header, fieldKey] of Object.entries(proposedMapping)) {
        ...
        if (!labelToHeadersInFile.has(normLabel))
            labelToHeadersInFile.set(normLabel, []);
...
//# sourceMappingURL=upload-mapping-validator.js.map