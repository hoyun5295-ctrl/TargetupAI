...
exports.runTrialDowngradeJob = runTrialDowngradeJob;
exports.startTrialDowngradeWorker = startTrialDowngradeWorker;
exports.stopTrialDowngradeWorker = stopTrialDowngradeWorker;
...
// 강등 작업
// ════════════════════════════════════════════════════════════
async function runTrialDowngradeJob() {
    ...
    const res = await (0, database_1.query)(`UPDATE companies c
        SET plan_id             = $1,
    ...
    RETURNING c.id, c.company_name`, [freePlanId]);
    const rows = res.rows;
    if (rows.length > 0) {
        log('job', `${rows.length}개 회사 FREE(미가입) 강등 완료:`, rows.map(r => `${r.company_name}(${r.id.slice(0, 8)})`).join(', '));
    }
    else {
        log('job', '만료 대상 없음');
    }
...
function startTrialDowngradeWorker() {
    if (_scheduled)
        return;
    _scheduled = true;
...
function stopTrialDowngradeWorker() {
    if (_timer) {
        clearTimeout(_timer);
...
//# sourceMappingURL=trial-downgrade-worker.js.map