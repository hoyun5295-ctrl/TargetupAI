...
exports.enrichWithCustomFields = enrichWithCustomFields;
exports.cleanLeftoverVars = cleanLeftoverVars;
exports.replaceVariables = replaceVariables;
exports.bulkReplaceVariables = bulkReplaceVariables;
exports.replaceWithFirstCustomer = replaceWithFirstCustomer;
exports.getOpt080Number = getOpt080Number;
exports.buildAdMessage = buildAdMessage;
exports.stripAdParts = stripAdParts;
exports.buildAdSubject = buildAdSubject;
exports.prepareSendMessage = prepareSendMessage;
exports.prepareFieldMappings = prepareFieldMappings;
 ...
 */
async function enrichWithCustomFields(fieldMappings, companyId) {
    try {
        const defResult = await (0, database_1.query)(`SELECT field_key, field_label, field_type FROM customer_field_definitions
       WHERE company_id = $1 AND (is_hidden = false OR is_hidden IS NULL)`, [companyId]);
        for (const def of defResult.rows) {
            ...
            const mappedType = ft === 'NUMBER' || ft === 'INTEGER' || ft === 'NUMERIC' ? 'number' :
 ...
 */
function cleanLeftoverVars(text) {
    if (!text)
        return '';
 ...
 * @returns 치환 완료된 메시지
 */
function replaceVariables(template, customer, fieldMappings, addressBookFields, options) {
    if (!template)
        return '';
    let result = template;
    ...
    if (addressBookFields) {
        ...
        const fmtExtra = (val) => {
            ...
            if (options?.skipNumberFormatting)
                return val; // 직접발송: 원본 그대로
            return (0, format_number_1.formatNumericLike)(val) ?? val; // AI발송: 숫자 포맷팅
        };
        result = result
    ...
    for (const [varName, mapping] of Object.entries(fieldMappings)) {
        ...
        const displayValue = (0, standard_field_map_1.renderFieldValue)(rawValue, mapping.column);
 ...
 * AI발송 경로에서 사용
 */
function bulkReplaceVariables(template, customers, fieldMappings) {
 ...
 */
function replaceWithFirstCustomer(template, customers, fieldMappings) {
 ...
 */
async function getOpt080Number(userId, companyId) {
    if (userId) {
        const userResult = await (0, database_1.query)('SELECT opt_out_080_number FROM users WHERE id = $1', [userId]);
        const userOpt = userResult.rows[0]?.opt_out_080_number;
        if (userOpt)
            return userOpt;
    }
    // ★ D123: opt_out_080_number가 비어있으면 reject_number 폴백 (D120 이전 데이터 호환)
    const compResult = await (0, database_1.query)(`SELECT COALESCE(NULLIF(opt_out_080_number, ''), reject_number) AS opt080 FROM companies WHERE id = $1`, [companyId]);
 ...
 */
function buildAdMessage(message, msgType, isAd, opt080Number) {
    if (!isAd)
        return message;
 ...
 */
function buildAdSubject(subject, msgType, isAd) {
    if (!isAd)
        return subject;
 ...
 */
function prepareSendMessage(template, customer, fieldMappings, options) {
    // 1. 변수 치환
 ...
 * 인라인으로 반복되던 코드.
 */
async function prepareFieldMappings(companyId) {
    const schemaResult = await (0, database_1.query)('SELECT customer_schema FROM companies WHERE id = $1', [companyId]);
    const { fieldMappings } = (0, ai_1.extractVarCatalog)(schemaResult.rows[0]?.customer_schema);
