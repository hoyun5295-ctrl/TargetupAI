...
exports.STANDARD_VALUES = void 0;
exports.normalizeGender = normalizeGender;
exports.getGenderVariants = getGenderVariants;
exports.normalizeGrade = normalizeGrade;
exports.getGradeVariants = getGradeVariants;
exports.normalizeRegion = normalizeRegion;
exports.getRegionVariants = getRegionVariants;
exports.normalizeSmsOptIn = normalizeSmsOptIn;
exports.normalizeMarried = normalizeMarried;
exports.normalizePhone = normalizePhone;
exports.isValidKoreanPhone = isValidKoreanPhone;
exports.isValidKoreanLandline = isValidKoreanLandline;
exports.normalizeStorePhone = normalizeStorePhone;
exports.normalizeAge = normalizeAge;
exports.ageFromBirthYear = ageFromBirthYear;
exports.ageFromBirthDate = ageFromBirthDate;
exports.normalizeAmount = normalizeAmount;
exports.normalizeDate = normalizeDate;
exports.normalizeCustomFieldValue = normalizeCustomFieldValue;
exports.normalizeCustomerRecord = normalizeCustomerRecord;
exports.buildGenderFilter = buildGenderFilter;
exports.buildRegionFilter = buildRegionFilter;
exports.buildGradeFilter = buildGradeFilter;
exports.normalizeEmail = normalizeEmail;
exports.normalizeByFieldKey = normalizeByFieldKey;
...
function getGenderVariants(standardValue) {
    if (standardValue === 'M')
        return ['M', 'm', '남', '남자', '남성', 'male', 'Male', 'MALE', '1', 'man', 'Man', 'MAN'];
    if (standardValue === 'F')
        return ['F', 'f', '여', '여자', '여성', 'female', 'Female', 'FEMALE', '2', 'woman', 'Woman', 'WOMAN'];
...
/** 필터용: 표준값 → DB에 존재할 수 있는 모든 변형값 배열 */
function getGradeVariants(standardValue) {
    const variants = [];
    for (const [key, val] of Object.entries(GRADE_MAP)) {
        if (val === standardValue)
            variants.push(key);
    }
...
/** 필터용: 표준값 → DB에 존재할 수 있는 모든 변형값 배열 */
function getRegionVariants(standardValue) {
    const variants = [];
...
function normalizeSmsOptIn(value) {
    if (value == null || value === '')
        return null;
    if (typeof value === 'boolean')
        return value;
    const v = String(value).trim();
    if (SMS_OPT_IN_TRUE.has(v))
        return true;
    if (SMS_OPT_IN_FALSE.has(v))
        return false;
    return null;
}
// ============================================================
...
function normalizeMarried(value) {
    if (value == null || value === '')
        return null;
    if (typeof value === 'boolean')
        return value;
    const v = String(value).trim();
    if (MARRIED_TRUE.has(v))
        return true;
    if (MARRIED_FALSE.has(v))
        return false;
    return null;
}
// ============================================================
...
function normalizePhone(value) {
    ...
    if (!v.startsWith('0') && (/^2\d{7,8}$/.test(v) || // 02 서울 (총 9~10자리, 0 빠짐 = 8~9자리)
...
function isValidKoreanPhone(phone) {
    ...
    if (/^01[16789]/.test(cleaned))
        return cleaned.length >= 10 && cleaned.length <= 11;
    // 050x 안심번호 (0502~0508) → 11~12자리
    if (/^050[2-8]/.test(cleaned))
 ...
 */
function isValidKoreanLandline(phone) {
    if (!phone)
        return false;
    const cleaned = phone.replace(/\D/g, '');
    // ★ D102: 범위 제한 없이 — 0으로 시작 + 휴대폰(01X) 아닌 7자리 이상이면 유선번호로 인정
    if (cleaned.startsWith('0') && !/^01[016789]/.test(cleaned) && cleaned.length >= 7)
        return true;
    // 1588, 1544, 1577 등 대표번호 → 8자리
 ...
 * - 하이픈 포함 형태로 반환
 */
function normalizeStorePhone(value) {
    if (value == null || value === '')
        return null;
    let v = String(value).trim();
    // 특수문자 제거
...
// ============================================================
function buildGenderFilter(value, paramIndex) {
...
function buildRegionFilter(value, paramIndex) {
...
function buildGradeFilter(value, paramIndex) {
...
// ============================================================
function normalizeByFieldKey(fieldKey, value) {
    if (value == null || value === '')
        return null;
    const field = (0, standard_field_map_1.getFieldByKey)(fieldKey);
