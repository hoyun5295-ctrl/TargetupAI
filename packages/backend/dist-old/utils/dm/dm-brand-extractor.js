...
exports.toBrandKitPatch = toBrandKitPatch;
const FETCH_TIMEOUT_MS = 5000;
const USER_AGENT = 'Mozilla/5.0 (compatible; TargetUpDMBot/1.0; +https://hanjul.ai)';
...
function pickBestIcon(links, baseUrl) {
    const iconLinks = links.filter((l) => /(^|\s)(apple-touch-icon|icon|shortcut icon|mask-icon)(\s|$)/.test(l.rel));
...
function extractContactHints(html, baseUrl) {
    // mailto:/tel: 링크 우선
 ...
 */
async function extractBrandFromUrl(targetUrl) {
    ...
    const ogImage = meta.get('og:image') ||
        ...
        undefined;
    const themeColorRaw = meta.get('theme-color') ||
        meta.get('msapplication-tilecolor') ||
        undefined;
    const themeColor = themeColorRaw && isValidHexColor(themeColorRaw) ? themeColorRaw : undefined;
 ...
 */
function toBrandKitPatch(result) {
    const patch = {};
    if (result.logo_url)
        patch.logo_url = result.logo_url;
    if (result.primary_color)
        patch.primary_color = result.primary_color;
...
//# sourceMappingURL=dm-brand-extractor.js.map