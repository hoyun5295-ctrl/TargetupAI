...
exports.validatePersonalization = validatePersonalization;
exports.validateCoupons = validateCoupons;
exports.validateCountdown = validateCountdown;
exports.validateLayout = validateLayout;
exports.validateStyle = validateStyle;
exports.validateContent = validateContent;
exports.validateRequiredInfo = validateRequiredInfo;
exports.validateData = validateData;
exports.validateOperation = validateOperation;
exports.validateDm = validateDm;
...
function validatePersonalization(sections) {
    const out = [];
    for (const s of sections) {
        if (!s.visible)
            continue;
        ...
        const vars = (0, dm_variable_resolver_1.extractVariables)(texts.join('\n'));
        if (vars.length === 0)
            continue;
...
function validateCoupons(sections) {
    const out = [];
...
function validateCountdown(sections) {
    const out = [];
    const now = Date.now();
...
function validateStyle(sections, brandKit) {
    const out = [];
    const primary = brandKit?.primary_color;
    if (primary && /^#[0-9a-fA-F]{6}$/.test(primary)) {
        const ratio = (0, dm_tokens_1.getContrastRatio)(primary, '#ffffff');
        if (ratio < 4.5) {
            out.push({
                area: 'style',
                severity: 'recommend',
...
function validateContent(sections) {
    const out = [];
    for (const s of sections) {
        if (!s.visible)
            continue;
...
function validateRequiredInfo(sections) {
    const out = [];
...
function validateData(sections, sampleCustomers) {
    const out = [];
    ...
    for (const s of sections) {
        if (!s.visible)
            continue;
        const texts = Object.values(s.props || {}).filter((v) => typeof v === 'string');
        const vars = (0, dm_variable_resolver_1.extractVariables)(texts.join('\n'));
        for (const v of vars) {
...
// ────────────── 통합 진입점 ──────────────
async function validateDm(dm, opts) {
    const sections = typeof dm.sections === 'string'
        ? (() => { try {
            return JSON.parse(dm.sections);
        }
        catch {
            return [];
        } })()
        : Array.isArray(dm.sections) ? dm.sections : [];
    const brandKit = typeof dm.brand_kit === 'string'
        ? (() => { try {
            return JSON.parse(dm.brand_kit);
        }
        catch {
            return undefined;
        } })()
        : dm.brand_kit || undefined;
    const items = [
    ...
    const level = stats.fatal > 0 ? 'error' : stats.recommend > 0 ? 'warning' : 'pass';
    return {
        level,
        items,
...
//# sourceMappingURL=dm-validate.js.map