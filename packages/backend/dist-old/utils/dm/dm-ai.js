...
function extractJson(raw) {
    ...
    if (first < 0 || last < first)
        throw new Error('JSON 형식이 아닙니다: ' + s.slice(0, 120));
...
async function parsePrompt(rawPrompt) {
    const now = new Date();
...
위 입력을 JSON으로 변환하세요.`;
    const text = await (0, ai_1.callAIWithFallback)({
    ...
    // Normalize + defaults
    const spec = {
        brand: { name: parsed.brand?.name || '', tone: parsed.brand?.tone },
        objective: parsed.objective || 'sale',
        target: parsed.target || {},
        benefit: parsed.benefit || undefined,
        urgency: parsed.urgency || undefined,
        personalization: parsed.personalization || [],
        tone: parsed.tone || parsed.brand?.tone || 'friendly',
...
function recommendLayout(spec) {
    ...
    const sections = types.map((type, idx) => {
        const base = (0, dm_section_registry_1.createSection)(type, cryptoId(), idx);
        // objective별 기본값 초기 주입
        switch (type) {
            case 'header':
                base.props.variant = 'logo';
                if (spec.brand.name)
                    base.props.brand_name = spec.brand.name;
                break;
            case 'hero':
                if (spec.benefit?.value)
                    base.props.headline = `${spec.benefit.value} 특별 혜택`;
                break;
            case 'coupon':
                if (spec.benefit?.type === 'discount' || spec.benefit?.type === 'coupon') {
                    base.props.discount_label = spec.benefit.value || '';
                    base.props.discount_type = 'percent';
                }
                if (spec.urgency?.end_datetime)
                    base.props.expire_date = spec.urgency.end_datetime;
                break;
            case 'countdown':
                if (spec.urgency?.end_datetime)
                    base.props.end_datetime = spec.urgency.end_datetime;
                if (spec.urgency?.label)
                    base.props.urgency_text = spec.urgency.label;
                break;
            case 'cta':
                base.props.buttons = [{ label: '자세히 보기', url: '', style: 'primary' }];
                break;
            case 'footer':
                base.props.show_unsubscribe_link = true;
                break;
        }
        // 스타일 변형 적용 (해당 섹션이 지원하는 경우만)
        if (dm_section_registry_1.SECTION_META[type].supportsStyleVariants.includes(variant)) {
...
- JSON 외 다른 텍스트 출력 금지`;
async function generateCopy(spec, section) {
...
- JSON 외 다른 텍스트 금지`;
async function transformTone(text, targetTone) {
    const userMessage = `원문: "${text}"
목표 톤: ${targetTone} (${exports.TONE_LABELS[targetTone] || targetTone})

변환 결과를 JSON으로 반환.`;
    const raw = await (0, ai_1.callAIWithFallback)({
        system: TONE_SYSTEM,
        userMessage,
        maxTokens: 300,
        temperature: 0.7,
...
async function improveMessage(sections, brandKit) {
    // AI가 분석 가능한 섹션만 추려 페이로드 축소
    const payload = sections
        .filter((s) => dm_section_registry_1.SECTION_META[s.type].aiAware && s.visible && !s.ai_locked)
        .map((s) => ({
        section_id: s.id,
        type: s.type,
...
function extractEditableFields(section) {
...
void dm_section_registry_1.SECTION_DEFAULTS;
//# sourceMappingURL=dm-ai.js.map