...
exports.youtubeEmbedUrl = exports.inlineImage = void 0;
exports.renderDmViewerHtml = renderDmViewerHtml;
exports.renderDmViewerHtmlWithCustomer = renderDmViewerHtmlWithCustomer;
exports.renderDmErrorHtml = renderDmErrorHtml;
/**
 ...
 */
const dm_viewer_utils_1 = require("./dm-viewer-utils");
Object.defineProperty(exports, "inlineImage", { enumerable: true, get: function () { return dm_viewer_utils_1.inlineImage; } });
Object.defineProperty(exports, "youtubeEmbedUrl", { enumerable: true, get: function () { return dm_viewer_utils_1.youtubeEmbedUrl; } });
const dm_section_renderer_1 = require("./dm-section-renderer");
const dm_tokens_1 = require("./dm-tokens");
const dm_variable_resolver_1 = require("./dm-variable-resolver");
// ────────────────── 헤더 템플릿 4종 ──────────────────
function renderHeader(template, data, storeName) {
    const d = data || {};
    switch (template) {
        case 'banner':
...
// ────────────────── 푸터 템플릿 4종 ──────────────────
function renderFooter(template, data, storeName) {
    const d = data || {};
    switch (template) {
        case 'cta':
...
function renderLegacySlidesHtml(dm, trackApiBase) {
    const pages = Array.isArray(dm.pages) ? dm.pages : (typeof dm.pages === 'string' ? JSON.parse(dm.pages) : []);
    ...
    const slidesHtml = pages.sort((a, b) => (a.order || 0) - (b.order || 0)).map((p, i) => {
        const imgSrc = p.imageUrl ? (0, dm_viewer_utils_1.inlineImage)(p.imageUrl) : '';
        const embedUrl = p.videoUrl ? (0, dm_viewer_utils_1.youtubeEmbedUrl)(p.videoUrl) : null;
    ...
    const dotsHtml = pages.map((_, i) => `<span class="dot${i === 0 ? ' active' : ''}" data-idx="${i}"></span>`).join('');
    return `<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
function parsePages(dm) {
    ...
    if (secs.length > 0)
        return [{ id: 'p-legacy', sections: secs }];
    return [];
}
...
function renderPagesHtml(dm, trackApiBase, resolvedPages, mode = 'scroll') {
    const pages = resolvedPages || parsePages(dm);
    const brandKit = parseBrandKit(dm.brand_kit);
    const storeName = dm.store_name || '';
    const title = dm.title || '모바일 DM';
    ...
    const pagesHtml = pages.map((page, i) => {
        const secHtml = (0, dm_section_renderer_1.renderSections)(page.sections, {
            brandKit,
            storeName,
            trackApiBase,
            shortCode,
        ...
        return `<section class="dm-page" data-page-idx="${i}" data-page-id="${(0, dm_section_renderer_1.escapeHtml)(page.id)}">${secHtml}</section>`;
    }).join('');
    const tokensCss = (0, dm_tokens_1.renderDmTokensCss)(brandKit);
    const baseCss = (0, dm_tokens_1.renderDmBaseCss)();
...
<title>${(0, dm_section_renderer_1.escapeHtml)(storeName ? `${storeName} - ${title}` : title)}</title>
...
<script>
(function(){
  var CODE = 'dm-${(0, dm_section_renderer_1.escapeHtml)(shortCode)}';
  var TRACK_URL = '${(0, dm_section_renderer_1.escapeHtml)(trackApiBase)}';
  var PHONE = new URLSearchParams(location.search).get('p') || '';
...
${hasCountdown ? dm_section_renderer_1.COUNTDOWN_SCRIPT : ''}
</script>
</body>
</html>`;
}
 ...
 */
function resolvePagesMode(dm, pages) {
    if (pages.length === 0)
        return null;
...
function renderDmViewerHtml(dm, trackApiBase) {
 ...
 */
async function renderDmViewerHtmlWithCustomer(dm, trackApiBase, customer, companyId) {
    const pages = parsePages(dm);
    const mode = resolvePagesMode(dm, pages);
    if (mode === null)
        return renderLegacySlidesHtml(dm, trackApiBase);
    ...
    for (const p of pages) {
        const resolvedSecs = await (0, dm_variable_resolver_1.resolveSections)(p.sections, customer, companyId);
        resolvedPages.push({ id: p.id, name: p.name, sections: resolvedSecs });
    }
    return renderPagesHtml(dm, trackApiBase, resolvedPages, mode);
}
// ────────────────── 만료/404 에러 페이지 ──────────────────
function renderDmErrorHtml(message) {
...
//# sourceMappingURL=dm-viewer.js.map