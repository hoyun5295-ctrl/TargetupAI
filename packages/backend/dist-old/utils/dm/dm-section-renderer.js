...
exports.COUNTDOWN_SCRIPT = void 0;
...
function renderHeader(props, ctx) {
    ...
    switch (variant) {
        case 'banner': {
            const img = props.banner_image_url ? (0, dm_viewer_utils_1.inlineImage)(props.banner_image_url) : '';
        ...
        default: {
            const logo = props.logo_url ? (0, dm_viewer_utils_1.inlineImage)(props.logo_url) : '';
...
function renderHero(props) {
    const img = props.image_url ? (0, dm_viewer_utils_1.inlineImage)(props.image_url) : '';
...
function renderVideo(props) {
    const embedUrl = props.video_type === 'youtube' ? (0, dm_viewer_utils_1.youtubeEmbedUrl)(props.video_url) : null;
    const thumb = props.thumbnail_url ? (0, dm_viewer_utils_1.inlineImage)(props.thumbnail_url) : '';
...
function renderStoreInfo(props) {
    const items = [];
    if (props.phone)
        items.push(`<a href="tel:${escapeHtml(props.phone)}" style="color:var(--dm-primary)"><strong>전화</strong> ${escapeHtml(props.phone)}</a>`);
    if (props.website)
        items.push(`<a href="${safeUrl(props.website)}" target="_blank" style="color:var(--dm-primary)"><strong>홈페이지</strong> ${escapeHtml(props.website.replace(/^https?:\/\//, ''))}</a>`);
    if (props.email)
        items.push(`<a href="mailto:${escapeHtml(props.email)}" style="color:var(--dm-primary)"><strong>이메일</strong> ${escapeHtml(props.email)}</a>`);
    if (props.address)
        items.push(`<span><strong>주소</strong> ${escapeHtml(props.address)}</span>`);
    if (props.business_hours)
        items.push(`<span><strong>영업시간</strong> ${escapeHtml(props.business_hours)}</span>`);
    if (items.length === 0)
        return '';
...
function renderPromoCode(props) {
    if (!props.code)
        return '';
...
`;
//# sourceMappingURL=dm-section-renderer.js.map