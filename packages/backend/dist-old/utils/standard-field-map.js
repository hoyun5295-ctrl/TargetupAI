...
exports.FIELD_DISPLAY_FORMAT_MAP = exports.FIELD_DISPLAY_MAP = exports.FIELD_MAP = exports.CATEGORY_LABELS = void 0;
exports.getFieldByKey = getFieldByKey;
exports.getFieldsByCategory = getFieldsByCategory;
exports.applyFieldAliases = applyFieldAliases;
exports.getColumnFields = getColumnFields;
exports.getCustomFields = getCustomFields;
exports.fieldKeyToColumn = fieldKeyToColumn;
exports.fieldKeyToCustomKey = fieldKeyToCustomKey;
exports.getAllCategories = getAllCategories;
exports.getInsertColumns = getInsertColumns;
exports.fieldKeyToSqlRef = fieldKeyToSqlRef;
exports.upsertCustomFieldDefinitions = upsertCustomFieldDefinitions;
exports.reverseDisplayValue = reverseDisplayValue;
exports.renderFieldValue = renderFieldValue;
...
function getFieldByKey(fieldKey) {
    return exports.FIELD_MAP.find(f => f.fieldKey === fieldKey);
}
/** 카테고리별 필드 목록 */
function getFieldsByCategory(category) {
    return exports.FIELD_MAP.filter(f => f.category === category).sort((a, b) => a.sortOrder - b.sortOrder);
}
 ...
 */
function applyFieldAliases(fieldMappings, availableVars, findEntryByColumn) {
    for (const f of exports.FIELD_MAP) {
...
function getColumnFields() {
    return exports.FIELD_MAP.filter(f => f.storageType === 'column');
}
/** custom_fields JSONB 필드만 (커스텀 15개) */
function getCustomFields() {
    return exports.FIELD_MAP.filter(f => f.storageType === 'custom_fields');
}
/** field_key → 실제 customers 컬럼명 (직접 컬럼일 때) */
function fieldKeyToColumn(fieldKey) {
    const field = exports.FIELD_MAP.find(f => f.fieldKey === fieldKey && f.storageType === 'column');
    return field ? field.columnName : null;
}
/** field_key → custom_fields 내 키 (JSONB일 때) */
function fieldKeyToCustomKey(fieldKey) {
    const field = exports.FIELD_MAP.find(f => f.fieldKey === fieldKey && f.storageType === 'custom_fields');
    return field ? field.columnName : null;
}
/** 모든 카테고리 목록 (정렬 순서대로) */
 ...
 */
function fieldKeyToSqlRef(fieldKey) {
    const field = getFieldByKey(fieldKey);
    if (!field)
        return null;
    if (field.storageType === 'column')
...
function toDbFieldType(input) {
    ...
    const mapped = FIELD_TYPE_DB_MAP[trimmed] ||
...
async function upsertCustomFieldDefinitions(companyId, definitions) {
    ...
    for (const def of definitions) {
        if (!def.fieldKey.startsWith('custom_'))
            continue;
        ...
        try {
            await (0, database_1.query)(`INSERT INTO customer_field_definitions (id, company_id, field_key, field_label, field_type, display_order, is_hidden, created_at)
         VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, false, NOW())
         ON CONFLICT (company_id, field_key) DO UPDATE SET
           field_label = EXCLUDED.field_label,
           field_type = EXCLUDED.field_type,
           display_order = EXCLUDED.display_order`, [companyId, def.fieldKey, def.label, dbFieldType, displayOrder]);
            upsertedCount++;
        }
        catch (err) {
            ...
            console.error(`[CT-07] ${def.fieldKey} upsert 실패 (company=${companyId}, inputType="${def.fieldType ?? 'undefined'}", dbType="${dbFieldType}", labelLen=${(def.label ?? '').length}):`, msg);
        }
    }
    if (upsertedCount > 0) {
 ...
 */
function reverseDisplayValue(fieldKey, dbValue) {
    if (dbValue === null || dbValue === undefined)
        return '';
    const map = exports.FIELD_DISPLAY_MAP[fieldKey];
    if (!map)
...
function fmtPhoneDisplay(value) {
...
function fmtIntDisplay(value) {
    if (value === null || value === undefined || value === '')
        return '';
    return String(value).trim();
}
/** 성별 표시 — DB enum (M/F/male/female/남/여) → 한글 (남성/여성) */
function fmtGenderDisplay(value) {
    if (value === null || value === undefined || value === '')
        return '';
    const k = String(value).trim().toLowerCase();
    const map = { m: '남성', f: '여성', male: '남성', female: '여성', 남: '남성', 여: '여성' };
    return map[k] || String(value);
}
/** 수신동의 표시 — boolean → '동의'/'비동의' */
function fmtSmsOptInDisplay(value) {
    if (value === null || value === undefined || value === '')
        return '';
    if (value === true || value === 1 || value === 'Y' || value === 'y')
        return '동의';
    if (value === false || value === 0 || value === 'N' || value === 'n')
        return '비동의';
...
function fmtStringDisplay(value) {
 ...
 */
exports.FIELD_DISPLAY_FORMAT_MAP = {
    // ── basic 8 ──
...
function renderFieldValue(value, fieldKey) {
    ...
    if (!fieldKey)
        return String(value);
    const formatter = exports.FIELD_DISPLAY_FORMAT_MAP[fieldKey];
    if (formatter)
        return formatter(value);
    return String(value);
}
//# sourceMappingURL=standard-field-map.js.map