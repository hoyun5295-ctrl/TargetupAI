 ...
 */
async function getStoreScope(companyId, userId) {
    ...
    if (storeCodes && storeCodes.length > 0) {
        ...
        const validCheck = await (0, database_1.query)(`SELECT EXISTS(
         SELECT 1 FROM customer_stores
          WHERE company_id = $1 AND store_code = ANY($2::text[])
       ) AS has_match`, [companyId, storeCodes]);
    ...
    const hasStores = await (0, database_1.query)('SELECT EXISTS(SELECT 1 FROM customer_stores WHERE company_id = $1 LIMIT 1) as has_stores', [companyId]);
...
//# sourceMappingURL=store-scope.js.map