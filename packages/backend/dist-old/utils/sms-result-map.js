...
exports.SPAM_RESULT_LABEL = exports.SPAM_RESULT = exports.CARRIER_MAP = exports.PENDING_CODES_SQL = exports.SUCCESS_CODES_SQL = exports.PENDING_CODES = exports.SUCCESS_CODES = exports.STATUS_CODE_MAP = void 0;
exports.isSuccess = isSuccess;
exports.isFail = isFail;
exports.isPending = isPending;
exports.getStatusLabel = getStatusLabel;
exports.getStatusType = getStatusType;
exports.getCarrierLabel = getCarrierLabel;
exports.getSpamResultLabel = getSpamResultLabel;
exports.getSpamResultType = getSpamResultType;
/** QTmsg status_code → 한줄로 결과 매핑 (유일한 정의) */
exports.STATUS_CODE_MAP = {
    // 성공
...
exports.SUCCESS_CODES = [6, 1000, 1800, 7830, 7831];
/** 대기 코드 배열 */
exports.PENDING_CODES = [100, 104];
/** 성공 여부 판별 */
function isSuccess(statusCode) {
    return exports.SUCCESS_CODES.includes(statusCode);
}
/** 실패 여부 판별 (성공도 아니고 대기도 아닌 모든 코드) */
function isFail(statusCode) {
    return !isSuccess(statusCode) && !isPending(statusCode);
}
/** 대기 여부 판별 */
function isPending(statusCode) {
    return exports.PENDING_CODES.includes(statusCode);
}
/** status_code → 라벨 문자열 (매핑에 없으면 '코드 NNN') */
function getStatusLabel(statusCode) {
    return exports.STATUS_CODE_MAP[statusCode]?.label || `코드 ${statusCode}`;
}
/** status_code → 타입 (매핑에 없으면 'unknown') */
function getStatusType(statusCode) {
    return exports.STATUS_CODE_MAP[statusCode]?.type || 'unknown';
}
/** SQL용: 성공 코드 IN 절 문자열 — 예: "6, 1000, 1800" */
exports.SUCCESS_CODES_SQL = exports.SUCCESS_CODES.join(', ');
/** SQL용: 대기 코드 IN 절 문자열 — 예: "100, 104" */
exports.PENDING_CODES_SQL = exports.PENDING_CODES.join(', ');
// ========================
// Part 2: 통신사 코드 매핑
// ========================
...
function getCarrierLabel(mobCompany) {
    return exports.CARRIER_MAP[mobCompany] || mobCompany || '알 수 없음';
}
// ========================
// Part 3: 스팸필터 판정 결과
// ========================
...
/** 스팸필터 result → 표시명 */
exports.SPAM_RESULT_LABEL = {
    [exports.SPAM_RESULT.PASS]: '정상',
    [exports.SPAM_RESULT.BLOCKED]: '차단',
    [exports.SPAM_RESULT.FAILED]: '실패',
    [exports.SPAM_RESULT.TIMEOUT]: '시간초과',
};
/** 스팸필터 result → 표시명 (null/undefined → '대기') */
function getSpamResultLabel(result) {
    if (!result)
        return '대기';
    return exports.SPAM_RESULT_LABEL[result] || '대기';
}
/** 스팸필터 result → CSS 타입 (프론트 배지 색상 결정용) */
function getSpamResultType(result) {
    if (!result)
        return 'pending';
    if (result === exports.SPAM_RESULT.PASS)
        return 'pass';
    if (result === exports.SPAM_RESULT.BLOCKED)
        return 'blocked';
    if (result === exports.SPAM_RESULT.FAILED || result === exports.SPAM_RESULT.TIMEOUT)
        return 'fail';
    return 'pending';
}
//# sourceMappingURL=sms-result-map.js.map