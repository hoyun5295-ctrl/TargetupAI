...
exports.getMmsImagePath = getMmsImagePath;
exports.getMmsImageDisplayName = getMmsImageDisplayName;
exports.normalizeMmsImagePaths = normalizeMmsImagePaths;
/**
 * mms_image_paths에서 서버 절대경로만 추출.
 * QTmsg Agent가 파일 시스템에서 읽을 경로.
 */
function getMmsImagePath(item) {
    if (!item)
        return '';
 ...
 */
function getMmsImageDisplayName(item) {
    if (!item)
        return '';
    if (typeof item === 'object' && item.originalName)
 ...
 */
function normalizeMmsImagePaths(items) {
    if (!Array.isArray(items))
        return [];
    return items.map(getMmsImagePath).filter(Boolean);
}
//# sourceMappingURL=mms-image-util.js.map