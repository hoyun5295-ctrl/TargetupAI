...
exports.calcNextRunAt = calcNextRunAt;
exports.kstToUtc = kstToUtc;
exports.runAutoCampaignWorker = runAutoCampaignWorker;
exports.startAutoCampaignScheduler = startAutoCampaignScheduler;
const database_1 = require("../config/database");
const customer_filter_1 = require("./customer-filter");
...
const callback_filter_1 = require("./callback-filter");
const ai_1 = require("../services/ai");
const spam_test_queue_1 = require("./spam-test-queue");
const unsubscribe_helper_1 = require("./unsubscribe-helper");
// ★ D114 P7: personalFields → displayName 변환용
const standard_field_map_1 = require("./standard-field-map");
const defaults_1 = require("../config/defaults");
// ★ B5/B6: 신규 컨트롤타워
const target_sample_1 = require("./target-sample");
const auto_notify_message_1 = require("./auto-notify-message");
// ============================================================
 ...
 */
function calcNextRunAt(scheduleType, scheduleDay, scheduleTime) {
    ...
    let tYear = kstYear, tMonth = kstMonth, tDay;
    if (scheduleType === 'daily') {
        tDay = kstDay;
        if (targetMinutes <= kstNowMinutes)
            tDay++;
    }
    else if (scheduleType === 'weekly') {
        const daysUntil = (scheduleDay - kstDow + 7) % 7;
        tDay = kstDay + daysUntil;
        if (daysUntil === 0 && targetMinutes <= kstNowMinutes)
            tDay += 7;
    }
...
function isWithinSendHours() {
    const now = new Date();
    const kstHour = parseInt(now.toLocaleString('en-US', { timeZone: 'Asia/Seoul', hour: '2-digit', hour12: false }));
    return kstHour >= defaults_1.SEND_HOURS.start && kstHour < defaults_1.SEND_HOURS.end;
}
// ============================================================
...
// ============================================================
async function runMessageGeneration() {
    ...
    try {
        ...
        const result = await (0, database_1.query)(`SELECT * FROM auto_campaigns
       WHERE status = 'active'
       ...
       ORDER BY next_run_at ASC`);
        ...
        for (const ac of result.rows) {
            ...
            const lockResult = await (0, database_1.query)(`UPDATE auto_campaigns SET generating_at = NOW()
         ...
         RETURNING id`, [ac.id]);
            ...
            try {
                ...
                await (0, database_1.query)('UPDATE auto_campaigns SET generating_at = NULL WHERE id = $1', [ac.id]);
...
async function generateMessageForAutoCampaign(ac) {
    const logPrefix = `[auto-worker][gen][${ac.id}]`;
    try {
        console.log(`${logPrefix} AI 문안 생성 시작 (${ac.campaign_name})`);
        // 회사 정보 조회 (AI 생성에 필요)
        const companyResult = await (0, database_1.query)(`SELECT company_name, brand_name, brand_tone, brand_description, brand_slogan,
              COALESCE(reject_number, opt_out_080_number) as reject_number, customer_schema
       FROM companies WHERE id = $1`, [ac.company_id]);
        const companyInfo = companyResult.rows[0] || {};
        // 사용자별 080번호 우선
        if (ac.user_id) {
            const userOptResult = await (0, database_1.query)('SELECT opt_out_080_number FROM users WHERE id = $1', [ac.user_id]);
            const userOpt080 = userOptResult.rows[0]?.opt_out_080_number;
            if (userOpt080)
                companyInfo.reject_number = userOpt080;
        }
        const { fieldMappings: varCatalog, availableVars } = (0, ai_1.extractVarCatalog)(companyInfo.customer_schema);
        // ★ D121: 실제 데이터가 있는 필드만 남김 — filterVarCatalogByData 컨트롤타워
        await (0, ai_1.filterVarCatalogByData)(varCatalog, availableVars, ac.company_id);
        // 타겟 통계 조회 (간략)
        const statsResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM customers WHERE company_id = $1 AND is_active = true AND sms_opt_in = true`, [ac.company_id]);
        const targetInfo = {
        ...
        const rawPersonalFields = Array.isArray(ac.personal_fields) ? ac.personal_fields : [];
        // 커스텀 필드(custom_N) 라벨 조회
        let customFieldLabels = {};
        if (rawPersonalFields.some(k => k.startsWith('custom_'))) {
            const cfdResult = await (0, database_1.query)(`SELECT field_key, field_label FROM customer_field_definitions WHERE company_id = $1 AND field_key LIKE 'custom_%'`, [ac.company_id]);
            for (const row of cfdResult.rows) {
                customFieldLabels[row.field_key] = row.field_label;
            }
        }
        const personalizationVars = rawPersonalFields.map(key => {
            // FIELD_MAP에서 displayName 조회 (직접 컬럼 필드)
            const field = (0, standard_field_map_1.getFieldByKey)(key);
            if (field)
        ...
        const aiResult = await (0, ai_1.generateMessages)(ac.ai_prompt || ac.campaign_name, targetInfo, extraContext);
        let generatedContent = '';
        let generatedSubject = '';
        let aiGenerationStatus = 'ai_generated';
        let spamTestResult = null;
        if (aiResult.variants && aiResult.variants.length > 0) {
            ...
            if (ac.callback_number && channel !== '카카오') {
                ...
                const sampleResult = await (0, target_sample_1.fetchTargetSampleCustomer)({
                ...
                try {
                    const spamResult = await (0, spam_test_queue_1.autoSpamTestWithRegenerate)({
                        companyId: ac.company_id,
                        userId: ac.user_id,
                        callbackNumber: ac.callback_number,
                        messageType: channel,
                        // ★ B12: testedVariants(1개) 전달 — 스팸테스트 3회 → 1회
                        variants: testedVariants.map((v) => ({
                        ...
                        rejectNumber: companyInfo.reject_number,
                        firstRecipient,
                        regenerateCallback: async (blockedVariantId) => {
                            try {
                                console.log(`${logPrefix} 스팸 차단 variant ${blockedVariantId} 재생성`);
                                const regenResult = await (0, ai_1.generateMessages)((ac.ai_prompt || ac.campaign_name) + '\n(이전 문안이 스팸필터에 차단되었습니다. 다른 표현으로 작성해주세요.)', targetInfo, extraContext);
                                if (regenResult.variants?.length > 0) {
                                    const nv = regenResult.variants[0];
                                    return {
                    ...
                    for (const sv of spamResult.variants) {
                        const original = testedVariants.find((v) => (v.variant_id || v.variantId) === sv.variantId);
                        if (original && sv.regenerated) {
                            original.message_text = sv.messageText;
                            original.sms_text = sv.messageText;
                            original.lms_text = sv.messageText;
                            if (sv.subject)
                                original.subject = sv.subject;
                        }
                    }
            ...
            // 최고 점수 variant 선택
            const best = testedVariants.sort((a, b) => (b.score || 0) - (a.score || 0))[0];
        ...
        await (0, database_1.query)(`UPDATE auto_campaigns SET
        generated_message_content = $2,
        generated_message_subject = $3,
        generated_at = NOW(),
        updated_at = NOW()
       WHERE id = $1`, [ac.id, generatedContent, generatedSubject || null]);
        ...
        const phones = ac.notify_phones || [];
        if (phones.length > 0) {
            try {
                const companyTables = [await (0, sms_queue_1.getAuthSmsTable)()];
                const sendTime = (0, sms_queue_1.toKoreaTimeStr)(new Date());
                ...
                const genOpt080 = ac.is_ad ? await (0, messageUtils_1.getOpt080Number)(ac.user_id, ac.company_id) : '';
                const genNotifyMsg = (0, auto_notify_message_1.buildAiGeneratedNotifyMessage)({
                ...
                for (const phone of phones) {
                    const cleanPhone = (0, normalize_phone_1.normalizePhone)(phone);
                    if (!cleanPhone)
                        continue;
                    genNotifyRows.push([
                ...
                if (genNotifyRows.length > 0) {
                    await (0, sms_queue_1.bulkInsertSmsQueue)(companyTables, genNotifyRows, true);
                    ...
                    try {
                        const runNumberResult = await (0, database_1.query)(`SELECT COALESCE(MAX(run_number), 0) + 1 as next_run FROM auto_campaign_runs WHERE auto_campaign_id = $1`, [ac.id]);
                        await (0, database_1.query)(`INSERT INTO auto_campaign_runs (
...
// ============================================================
async function runPreNotification() {
    const logPrefix = '[auto-worker][notify]';
    try {
        const result = await (0, database_1.query)(`SELECT ac.* FROM auto_campaigns ac
...
async function sendPreNotification(ac) {
    ...
    try {
        ...
        let targetCount = 0;
        try {
            const filterResult = (0, customer_filter_1.buildFilterQueryCompat)(ac.target_filter, ac.company_id);
            let storeFilter = '';
            const storeParams = [];
            if (ac.store_code) {
            ...
            const unsubFilter = (0, unsubscribe_helper_1.buildUnsubscribeFilter)(`$${unsubParamIdx}`, 'c.phone');
            const countResult = await (0, database_1.query)(`SELECT COUNT(*) as cnt FROM customers c
         WHERE c.company_id = $1 AND c.is_active = true AND c.sms_opt_in = true
         ${filterResult.where}${storeFilter}${unsubFilter}`, [ac.company_id, ...filterResult.params, ...storeParams, ac.user_id]);
        ...
        const messageContent = ac.ai_generate_enabled && ac.generated_message_content
            ...
            : ac.message_content;
        // ★ D111 P5: (광고)+무료거부 부착 — isAd/opt080Number 전달 (buildAdMessage 내장)
        const preOpt080 = ac.is_ad ? await (0, messageUtils_1.getOpt080Number)(ac.user_id, ac.company_id) : '';
        const notifyMessage = (0, auto_notify_message_1.buildPreNotifyMessage)({
            campaignName: ac.campaign_name,
            scheduledDateStr,
            scheduledTimeStr,
            targetCount,
        ...
        if (notifyRows.length > 0) {
            await (0, sms_queue_1.bulkInsertSmsQueue)(companyTables, notifyRows, true);
        }
...
// ============================================================
async function executeAutoCampaign(ac) {
    ...
    try {
        ...
        const lockResult = await (0, database_1.query)(`UPDATE auto_campaigns SET status = 'executing', updated_at = NOW()
       ...
       RETURNING id`, [ac.id]);
        ...
        // ★ 라인그룹 미설정 체크
        if (!(await (0, sms_queue_1.hasCompanyLineGroup)(ac.company_id))) {
        ...
        const companyTables = await (0, sms_queue_1.getCompanySmsTables)(ac.company_id, ac.user_id);
        // 회사 스키마 조회 (변수 치환용)
        // ★ D102: prepareFieldMappings 컨트롤타워로 통합 (customer_schema 조회 + extractVarCatalog + enrichWithCustomFields)
        const fieldMappings = await (0, messageUtils_1.prepareFieldMappings)(ac.company_id);
        // 동적 SELECT 컬럼 구성
       ...
       ${filterResult.where}${storeFilter}${unsubFilter}`, [ac.company_id, ...filterResult.params, ...storeParams, ac.user_id]);
        ...
        const campaignResult = await (0, database_1.query)(`INSERT INTO campaigns (
      ...
      RETURNING id`, [
        ...
        const runResult = await (0, database_1.query)(`INSERT INTO auto_campaign_runs (
      ...
      RETURNING id`, [
        ...
        const deduct = await (0, prepaid_1.prepaidDeduct)(ac.company_id, customers.length, ac.message_type, campaignId, ac.user_id);
        if (!deduct.ok) {
            console.warn(`${logPrefix} 잔액 부족 — ${deduct.error}`);
            await (0, database_1.query)(`UPDATE auto_campaign_runs SET status = 'failed', completed_at = NOW(), cancel_reason = $2 WHERE id = $1`, [runId, `잔액 부족: ${deduct.error}`]);
            await advanceNextRun(ac);
            return;
        }
        // ★ MySQL 큐 INSERT
        const sendTime = (0, sms_queue_1.toKoreaTimeStr)(new Date());
        const msgTypeCode = (0, sms_queue_1.toQtmsgType)(ac.message_type);
        const mmsImages = []; // MMS 이미지는 추후 지원
        // ★ D93: 개별회신번호 사용 시 CT-08 필터링 적용
        let filteredCustomers = customers;
        if (ac.use_individual_callback) {
            const { filterByIndividualCallback } = await Promise.resolve().then(() => __importStar(require('./callback-filter')));
        ...
        if (ac.channel === 'alimtalk') {
            ...
            if (!ac.alimtalk_template_code || !ac.alimtalk_profile_id) {
                ...
                await (0, database_1.query)(`UPDATE auto_campaign_runs SET status = 'failed', completed_at = NOW(), cancel_reason = $2 WHERE id = $1`, [runId, '알림톡 템플릿 또는 발신프로필 미설정']);
                await advanceNextRun(ac);
                return;
            }
            const gate = await (0, database_1.query)(`SELECT t.status AS tstatus, p.approval_status, p.profile_key
           FROM kakao_templates t
           JOIN kakao_sender_profiles p ON p.id = t.profile_id
          WHERE t.id = $1 AND t.company_id = $2 LIMIT 1`, [ac.alimtalk_template_id, ac.company_id]);
            if (gate.rows.length === 0 || !['APPROVED', 'APR', 'A'].includes(String(gate.rows[0].tstatus).toUpperCase()) || gate.rows[0].approval_status !== 'APPROVED') {
                ...
                await (0, database_1.query)(`UPDATE auto_campaign_runs SET status = 'failed', completed_at = NOW(), cancel_reason = $2 WHERE id = $1`, [runId, '알림톡 템플릿 또는 발신프로필이 승인되지 않음']);
                await advanceNextRun(ac);
                return;
            }
            const senderKey = gate.rows[0].profile_key;
            const etcJson = JSON.stringify({ senderkey: senderKey });
            const { insertAlimtalkQueue } = await Promise.resolve().then(() => __importStar(require('./sms-queue')));
            const alimRows = filteredCustomers.map((customer) => {
                // 템플릿 기반 content 치환 + #{변수} variable map 적용
                let finalMessage = messageContent;
                const { message: personalizedMessage } = (0, messageUtils_1.prepareSendMessage)(finalMessage, customer, fieldMappings, {
                ...
                for (const [rawKey, rawVal] of Object.entries(varMap)) {
                    ...
                    const resolved = v.startsWith('@@') && v.endsWith('@@')
                        ? String(customer[v.slice(2, -2)] ?? '')
                        : v;
                    finalMessage = finalMessage.replace(new RegExp(`#\\{${cleanKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\}`, 'g'), resolved);
                }
                const cleanPhone = (0, normalize_phone_1.normalizePhone)(customer.phone);
                const callback = (0, callback_filter_1.resolveCustomerCallback)(customer, ac.use_individual_callback || false, ac.callback_number);
                return {
                    phone: cleanPhone,
                    callback,
            ...
            const autoOpt080 = (ac.is_ad) ? await (0, messageUtils_1.getOpt080Number)(ac.user_id, ac.company_id) : '';
            const autoSmsRows = [];
            for (const customer of filteredCustomers) {
                // ★ D103: prepareSendMessage 컨트롤타워 — 변수 치환 + (광고)+080 + ★ KISA 2026-05 제목(광고) 통합
                const { message: personalizedMessage, subject: personalizedSubject } = (0, messageUtils_1.prepareSendMessage)(messageContent, customer, fieldMappings, {
            ...
            sentCount = await (0, sms_queue_1.bulkInsertSmsQueue)(companyTables, autoSmsRows, true);
        }
        // ★ 부분 실패 시 환불
        ...
        if (failCount > 0) {
            ...
            try {
                await (0, prepaid_1.prepaidRefund)(ac.company_id, failCount, ac.message_type, campaignId, `자동발송 부분실패 ${failCount}건 환불`);
            }
            catch (refundErr) {
        ...
        await (0, database_1.query)(`UPDATE auto_campaign_runs SET
        sent_count = $2, success_count = 0, fail_count = $3,
        status = $4, completed_at = NOW()
       WHERE id = $1`, [runId, sentCount, failCount, sentCount === 0 ? 'failed' : 'completed']);
        // ★ campaigns 상태 업데이트
        ...
        await (0, database_1.query)(`UPDATE campaigns SET status = $2, sent_count = $3, sent_at = NOW() WHERE id = $1`, [campaignId, autoStatus, sentCount]);
        // ★ campaign_runs 연결 레코드
        await (0, database_1.query)(`INSERT INTO campaign_runs (campaign_id, run_number, target_count, sent_count, status, sent_at)
       VALUES ($1, 1, $2, $3, $4, NOW())`, [campaignId, customers.length, sentCount, autoStatus]);
        ...
        if (ac.ai_generate_enabled) {
            await (0, database_1.query)(`UPDATE auto_campaigns SET generated_message_content = NULL, generated_message_subject = NULL, generated_at = NULL WHERE id = $1`, [ac.id]);
        }
...
// ============================================================
async function markFailed(ac, reason) {
    try {
        const runNumberResult = await (0, database_1.query)(`SELECT COALESCE(MAX(run_number), 0) + 1 as next_run FROM auto_campaign_runs WHERE auto_campaign_id = $1`, [ac.id]);
        await (0, database_1.query)(`INSERT INTO auto_campaign_runs (auto_campaign_id, run_number, status, scheduled_at, completed_at, cancel_reason)
       VALUES ($1, $2, 'failed', $3, NOW(), $4)`, [ac.id, runNumberResult.rows[0].next_run, ac.next_run_at, reason]);
    }
    catch (err) {
...
async function advanceNextRun(ac, sentCount) {
    try {
        ...
        const params = sentCount !== undefined
            ...
            : [ac.id, nextRunAt];
        await (0, database_1.query)(`UPDATE auto_campaigns SET ${updateFields} WHERE id = $1`, params);
    }
    catch (err) {
...
// ============================================================
async function runPreSendSpamTest() {
...
async function executePreSendSpamTest(ac) {
    ...
    try {
        ...
        if (ac.ai_generate_enabled && !ac.generated_message_content) {
            ...
            try {
                ...
                const refreshed = await (0, database_1.query)('SELECT * FROM auto_campaigns WHERE id = $1', [ac.id]);
                if (refreshed.rows[0])
                    Object.assign(ac, refreshed.rows[0]);
            }
            catch (genErr) {
        ...
        const sampleRes = await (0, target_sample_1.fetchTargetSampleCustomer)({
            companyId: ac.company_id,
            targetFilter: ac.target_filter,
            userId: ac.user_id,
            storeCode: ac.store_code,
        ...
        const spamRejectNumber = await (0, messageUtils_1.getOpt080Number)(ac.user_id, ac.company_id);
        const spamResult = await (0, spam_test_queue_1.autoSpamTestWithRegenerate)({
            companyId: ac.company_id,
            userId: ac.user_id,
            callbackNumber: ac.callback_number,
            ...
            regenerateCallback: async (blockedVariantId) => {
             ...
             FROM companies WHERE id = $1`, [ac.company_id]);
                    ...
                    const regenResult = await (0, ai_1.generateMessages)((ac.ai_prompt || ac.campaign_name) + '\n(이전 문안이 스팸필터에 차단되었습니다. 다른 표현으로 작성해주세요.)', { total_count: 0, avg_purchase_count: 0, avg_total_spent: 0 }, {
                        brandName: companyInfo.brand_name || companyInfo.company_name || '브랜드',
                        brandTone: companyInfo.brand_tone,
                        brandDescription: companyInfo.brand_description,
                        brandSlogan: companyInfo.brand_slogan,
        ...
        if (regeneratedFinal?.regenerated && regeneratedFinal.messageText) {
            await (0, database_1.query)(`UPDATE auto_campaigns SET generated_message_content = $1, generated_message_subject = $2 WHERE id = $3`, [regeneratedFinal.messageText, regeneratedFinal.subject || ac.generated_message_subject, ac.id]);
            ac.generated_message_content = regeneratedFinal.messageText;
            if (regeneratedFinal.subject)
                ac.generated_message_subject = regeneratedFinal.subject;
        ...
        if (phones.length > 0) {
            ...
            // ★ D111 P5: (광고)+무료거부 부착 — isAd/opt080Number 전달 (buildAdMessage 내장)
            const spamOpt080 = ac.is_ad ? await (0, messageUtils_1.getOpt080Number)(ac.user_id, ac.company_id) : '';
            const notifyMsg = (0, auto_notify_message_1.buildSpamTestResultNotifyMessage)({
                campaignName: ac.campaign_name,
                scheduledTimeStr,
...
// ============================================================
async function runAutoCampaignWorker() {
...
//# sourceMappingURL=auto-campaign-worker.js.map