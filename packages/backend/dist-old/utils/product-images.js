...
exports.getProductDisplay = getProductDisplay;
exports.getEmoji = getEmoji;
exports.renderProductImage = renderProductImage;
exports.generateProductImage = generateProductImage;
exports.generateFlyerImages = generateFlyerImages;
exports.getGeneratedImageUrl = getGeneratedImageUrl;
 ...
 */
function getProductDisplay(productName) {
 ...
 */
function renderProductImage(productName, size = 48, generatedImageUrl) {
...
// 디렉토리 자동 생성
if (!fs_1.default.existsSync(PRODUCT_IMAGE_DIR)) {
    fs_1.default.mkdirSync(PRODUCT_IMAGE_DIR, { recursive: true });
}
...
function loadExistingImages() {
    try {
        const files = fs_1.default.readdirSync(PRODUCT_IMAGE_DIR);
        for (const file of files) {
            if (file.endsWith('.png')) {
                const keyword = decodeURIComponent(file.replace('.png', ''));
                generatedImageCache.set(keyword, path_1.default.join(PRODUCT_IMAGE_DIR, file));
            }
        }
...
텍스트나 글자 없이 상품만 표시.`;
        ...
        const filePath = path_1.default.join(PRODUCT_IMAGE_DIR, filename);
 ...
 */
async function generateFlyerImages(categories) {
    const results = {};
    const allItems = [];
 ...
 */
function getGeneratedImageUrl(productName) {
    const cached = generatedImageCache.get(productName);
    if (cached && fs_1.default.existsSync(cached)) {
 ...
 */
function resolveProductImageUrl(productName) {
