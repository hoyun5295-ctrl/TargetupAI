...
D=$(awk '/^### 완료분 잔여/,/^## /' status/STATUS.md 2>/dev/null | grep -c '^| ')
# ↑ `|| echo 0` 금지 — grep -c는 0건이어도 "0"을 출력하며 exit 1이라, || 가 "0\n0" 이중 출력을 만든다
#   (2026-08-14 완료분 잔여 절 폐지로 실증 — 절이 없어도 D=0으로 통과해야 정상)
...
R=$(awk '/^## 1\) 라우팅 표/,/^---$/' status/STATUS.md 2>/dev/null | grep -c '^| ')
