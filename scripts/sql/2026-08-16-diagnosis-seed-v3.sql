-- 마케팅 진단 seed v3 (2026-08-16 · v3 설계서 §7이 이 파일을 가리킨다 — 원문 소유 = 이 파일)
-- 실행: docker exec -i targetup-postgres psql -U targetup targetup < ~/targetup-app/scripts/sql/2026-08-16-diagnosis-seed-v3.sql
-- 멱등: 기존 v3 행을 지우고 다시 넣는다(터미널 붙여넣기 사고로 들어갔을 수 있는 불완전 행 정리 포함).
--       v3 제출 행이 이미 있다면 그 행의 관리 라벨 조회가 이 재삽입 정의를 쓰게 된다(문항 동일 = 무해).
BEGIN;
DELETE FROM diagnosis_question_sets WHERE version = 'v3';
