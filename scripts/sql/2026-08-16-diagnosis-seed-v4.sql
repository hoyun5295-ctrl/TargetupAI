-- 마케팅 진단 seed v4 (2026-08-16 · 기존 도구 스택 축 신설 — ERP·CRM·발송 도구. Harold 지적 반영)
-- 실행: docker exec -i targetup-postgres psql -U targetup targetup < ~/targetup-app/scripts/sql/2026-08-16-diagnosis-seed-v4.sql
-- 멱등: 기존 v4 행을 지우고 다시 넣는다. v3 대비 = list.locked 라벨에 ERP 포함 + locked_tool·unified_tool·send_tool 3문항.
BEGIN;
DELETE FROM diagnosis_question_sets WHERE version = 'v4';
UPDATE diagnosis_question_sets SET is_active = false WHERE is_active;
INSERT INTO diagnosis_question_sets (version, definition, is_active) VALUES ('v4', $$
{
 "version": "v4",
 "rule_version": "r4",
 "meta": {
