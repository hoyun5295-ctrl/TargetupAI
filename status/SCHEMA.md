...
| 75-A+ | sales_outreach_jobs 칸 추가 **(★2026-09-23 운영 ALTER 완료 · information_schema 8행 실측)** | 담당자 직접 발송 · 5칸 = `contact_email text` · `contact_name text` · `contact_basis text`(주소를 알게 된 근거 · 공란 = 발송 잠금) · `naver_store_slug text`(`brand:slug`·`smartstore:slug` · fetch 가능한 주소가 아니다) · `reviewed_asset_id uuid`(사람이 확인한 email_html 판 · 최신과 다르면 발송 잠금). 파기 때 contact 3칸 NULL. jsonb 키 추가(stage_results) = `review`{by,at,assetId,via} · `hold`{reason,by,at} · `domain_ack`{email,by,at} · `auto_confirmed_at`(되돌리기가 지우지 않는다) · `auto_send`{by,at,rule}(묶음 사전 승인) · `direct_last`{outcome,detail,at,mode} · brand_profile `contactPages`(제휴·문의 페이지 링크 ≤3) · email_html payload `subjectCandidates` · `adFooter`{fromEmail,unsubscribeUrl}. 설계 = [docs/2026-09-23-outreach-direct-send-design.md §4](../docs/2026-09-23-outreach-direct-send-design.md). **★2026-09-24 jsonb 키(DDL 0)** = stage_results `store_grab` — 판 1 {text,chars,at,by}(0924 글자 긁기 · 후보에 안 쓰인다) / **판 2 {v:2, store, material{storeName, interestCount, campaigns[{id,name,link,evidence,productIds,lead}], products[≤12 {id,name,tags,price,salePrice,discountRatio,reviewCount,rating}]}, at, by}**(네이버 스토어 판독 결과 · 상태 원문·회원 칸 0 · 있으면 자동 확정 제외 · 되돌리기가 지우지 않는다) · event_quote.candidates[] `parts`{title,benefit,period}(홈페이지 인용문 세 조각 · 원문 부분 문자열만 · 옛 후보에는 없다) · `source` 값 `naver_store` 추가(스토어 기획 카드 · 확정 때 candidates 에 함께 저장). 설계 = [품질 A](../docs/2026-09-24-outreach-quality-a-design.md) · [B 판독기](../docs/2026-09-24-outreach-naver-store-reader-design.md) |
| 75-C | sales_outreach_sends **(★2026-09-23 운영 CREATE 완료)** | 담당자 직접 발송 원장(주소 원문 0 · 파기 대상 아님). `id uuid PK DEFAULT gen_random_uuid(), job_id uuid NOT NULL FK sales_outreach_jobs(id)(CASCADE 없음), asset_id uuid NOT NULL(나간 email_html 판), to_hash text NOT NULL(HMAC 주소 해시), host_key text NOT NULL(normalizeHomepageKey), mode text NOT NULL CHECK(manual·bulk·auto), outcome text NOT NULL DEFAULT 'sending' CHECK(sending·sent·rejected·unknown), detail text, meta jsonb NOT NULL DEFAULT '{}'(mode·edited·autoConfirmed·selectionChanged·domainVerdict·batch·approval), review_flag text CHECK(pending·ok·wrong)(단계 3 사후 확인), reopened_at timestamptz(같은 회사 재접촉 열기), created_by uuid, created_at timestamptz NOT NULL DEFAULT NOW(), finished_at timestamptz`. **UNIQUE(host_key) WHERE outcome IN (sending,sent) AND reopened_at IS NULL**(같은 회사 재발송 DB 차단) · INDEX(created_at DESC)·(job_id). 쓰기 = `sales-outreach-direct-jobs.ts sendDirectCore` 트랜잭션 · sweeper 끊긴 sending → unknown |
| 75-D | sales_outreach_suppressions **(★2026-09-23 운영 CREATE 완료)** | 직접 발송 수신거부 원장(영구 · 해시만). `id uuid PK, email_hash text NOT NULL, host_key text, scope text NOT NULL CHECK(address·company), reason text NOT NULL CHECK(unsub·not_contact·bounce·manual), source_job_id uuid, created_at timestamptz NOT NULL DEFAULT NOW()`. INDEX(email_hash)·(host_key) WHERE scope='company'. 판정 = 주소 해시 일치 또는 회사(company) host_key 일치 |
...
| user_prompt | text |
| ai_mode | boolean |
| send_type | varchar(20) · **DEFAULT `'ai'`**(2026-08-23 `information_schema` 실측) · CHECK 없음. 값 집합의 소유자는 `utils/send-type-axis.ts`(direct·ai·auto·journey·operator·agency) — 여기 재서술 금지. **새 발송 경로를 만들면 그 CT에 먼저 등재한다**(안 하면 기본값 `'ai'`로 적재되어 결과 동기화·환불·후불 청구 세 축에서 통째로 빠진다) |
| target_spec | jsonb |
| target_filter | jsonb |
| target_count | integer |
...
| api_secret | varchar(100) | ★ 싱크에이전트 인증 전용 (CDP 아님) · **★2026-09-12 폐기 예정 — 원문 미저장으로 전환**(마이그레이션으로 NULL). 남아 있으면 전환 전 행이다 · **★2026-09-14 운영 원문 전량 NULL 실행완료**(해시 백필 83행 → 원문과 해시가 일치하는 행만 비움 84행 → 잔존 0 실측) |
...
| SysId | 수집 서버 구분 — ★0820 실측: `54`·`57`·`58` = QTmsg 중계 3대(실시간 갱신 확인) / **`65` = 비토 게이트웨이**(0815 확정 · ★0916 실적 적재 시작 = V0001 랩디·V0002 아이티앤·V0003 이터널그룹 · 발급·보고 전환은 게이트웨이 Agent 수정 창이 소유). ⛔ PK 에 SysId 가 없다 — 같은 (DestDt,CustId,StoreId,MsgType) 를 다른 SysId 가 쓰면 덮어쓴다. QTmsg 는 B/C/D, 비토는 V 접두라 충돌하지 않으며 **V 접두를 QTmsg 쪽에서 쓰지 않는 것이 전제**다 |
...
| **RemAmt** | **현재 잔액 (float) — 잔액의 유일한 소스.** ★2026-07-27 확정. 발송이 나가는 대로 실시간으로 깎인다(D0078 4,881,401.2 → 몇 분 뒤 4,881,227.5). 62·143 값 완전 일치. ★2026-09-23 **V 발송ID(비토 게이트웨이 · SysId 65)는 게이트웨이가 선불일 때만 1분마다 지갑 잔액을 스냅샷으로 쓴다**(후불 V 는 NULL 그대로). V 발송ID 의 충전 행(`RSRM_FillAmtHist`)은 게이트웨이가 지갑에 반영하고 `Y` 로 표시한다 — 한줄로 충전 흐름은 그대로(bito-gateway `status/domains/COMMERCIAL_ACCOUNTS.md` 과금 계약) |
...
> **★2026-09-21 `idx_customers_company_lower_email`(company_id, LOWER(email))** — 비유니크 · `CREATE INDEX CONCURRENTLY`(Harold 실행). 소비처 = `cdp-identity.ts identifyCustomer` 2단계 이메일 매칭 1곳(`LOWER(email) = $2`). 없을 때 실측: 그 회사 고객 26만 행을 병렬 4워커로 훑는 Bitmap Heap Scan(cost 45,196) → 회원 가져오기가 고객 수에 비례해 느려졌다(분당 3,700 → 1,700명 · BUGS B-0921-4).
...
| imc_template_status | ⛔ **운영 미존재(2026-09-19 실측 `column t.imc_template_status does not exist`)** — 아래는 설계 정의일 뿐 ALTER 미실행. 조회 SQL에 넣지 말 것 · varchar(10) NULL — ★ 2026-06-10 신설(CT-87) → **2026-06-11 정의 정정(휴머스온 공식)**: S=중지 / A=정상 / R=대기(발송 전 — 첫 발송 시 자동 A 전환, **차단 사유 아님**). 검수상태(status)와 별개. 79738 7300의 진짜 원인은 R이 아니라 대표링크(represent_link) 발송 미동봉이었음 — CT-87의 R 차단은 신규 템플릿 첫 발송을 영구 차단하는 역효과라 해제 정정 의무(미배포). 동기화 = 30분 워커 syncTemplateStatuses + 5분 job + 단건 GET. ALTER 미실행 상태(2026-06-11 운영 실측 — 컬럼 없음, 가드는 null 안전 통과). 실행 SQL: `ALTER TABLE kakao_templates ADD COLUMN IF NOT EXISTS imc_template_status varchar(10);` |
...
| group_type | varchar(20) | bulk/test/auth/bito · ★2026-09-23 test 그룹은 **첫 테이블 = 적재**(`getTestSendTable` · insertTestSmsQueue·MFA 같은 자리) · 나머지 테이블 = 옛 라인 이력·정산 조회용 |
...
| `event_list` · `crawling_cards` · `palette_render` | ★2026-09-06 v3 · 3값 `ok` / `no_content` / `unavailable`(이벤트 목록 카드 추출 = 행사 링크가 있었을 때만 · 카드 상세 1홉 = 면허 카드가 있었을 때만 · 팔레트 렌더 = 항상) | 크롤 단계(renderKeys) | 근거 패널 · 레시피 gates.eventList |
| `auto_seq` | ★v3 · `{ dm, dm_reasons[], at }` 자동 재조립 카운터(사람 `regen_seq` 와 분리 · dm·image 재생성이 지운다) | producing_dm(mergeStageResultsOwned) | 상한 판정(1회) · 근거 패널 관측 1줄 |
| `ai_cost` | ★v3 · `{ calls, bySource{source: n}, ms }` 아웃리치 축 AI 호출 계수(토큰 제외 · companyId 주입 0) · 누적 | 분석 종료 · ready 전이 직전 · 실패 종결 직전 | 근거 패널 관측 1줄 |
| `edits` | ★v3 · `[{ kind hide|reselect|edit|reply, keys?[], counts?{products, gallery}, fieldPath?, reason? no_text|duplicate|blurry|wrong|tone|null, beforeLen?, afterLen?, at, by }]` 상한 100 · **append CT 1곳(`appendStageResultArray` · jsonb_set 한 문장)** | recordOutreachEdit(숨김·재선택·문안·제목·회신) | 레시피 승격 meta.edits 통계 |
| `reply_line` | ★v3 · `{ text(≤60), at, by }` 또는 null(기본 문장) | editOutreachReplyLine(라우트 `/reply-line` · 감사 `sales_outreach.reply_line`) → 메일 재조립 | assembleProposalEmail replyLine |
| `event_slices` · `event_slices_detail` | ★2026-09-09 · 3값 `ok` / `no_content` / `unavailable`(면허 카드가 있어 카드 1번 상세를 렌더했을 때만 키 존재 · unavailable = 워커 부재·차단·시간 초과·호스트 이탈 · ★v4 프로모션 페이지에서 찾아도 `ok`) · 정제 문자열 ≤300 | 크롤 단계(renderKeys · 면허 카드 1번 상세 렌더 1회 · v4 프로모션 루프) · ★0909(5) 제작 단계(producing_image 진입 · 확정 1번 카드 상세가 현재 묶음과 다르면 1회 렌더 → `ok` + detail `확정 행사 상세 N장(제작 단계)` · `brand_profile.eventSlices` source `event_card` 교체) | 근거 패널 |
| `promo_pages` · `promo_pages_detail` | ★2026-09-09 v4 · 3값 `ok` / `no_content` / `unavailable`(홈에 걸린 프로모션·기획 페이지 후보가 있었을 때만 · 최대 3 렌더 · `render_meta.promoTried[]` 동반) · `rendering` 사유에 `material_first`(정적이 두꺼워도 항상 렌더) | 크롤 단계(renderKeys) | 근거 패널 |
...
★2026-09-06 v3 `brand_profile` 추가 키: `homeCaptureUrl`(375×900 공개 사본 · 팔레트 렌더의 뷰포트 캡처 · null 가능) · `brand.colorSource` 4값(`render`·`meta`·`icon`·`neutral`) · `brand.palette` · `materials.eventCards[{title, periodRaw, startDate, endDate, imageUrl(원 URL), linkUrl, order}]` + `materials.counts.eventCards` · `media.products[].awards?`(상세 어워즈 조각 ≤3) · `media.products[]` 에 목록 4키(badges·rating·review_count·discount_rate)가 실제로 실린다(상세·목록 productKey 병합).
★2026-09-09 `brand_profile` 추가 키: `eventSlices{ detailUrl, finalUrl, images[{url(원 URL), width, height, order}], candidates, at, source?('event_card'|'promo_page' ★v4) }` 또는 null(면허 카드 1번 상세 또는 ★v4 홈에 걸린 프로모션 페이지를 렌더해 찾은 세로 슬라이스 묶음 · 3~20장 · `utils/sales-outreach-slices.ts detectEventSlices`) · `media.slices[]`(StoredImage · 재료 순서 · 폭 ≥600 · 갤러리와 분리 · 제작 단계 전용 예산 · 파기가 함께 삭제) + `media.stats.slicesTried / slicesPassed / slicesTimedOut`.
★2026-09-09 v4 `brand_profile` 추가 키: `heroBanners[{url, width, height, href|null, order, alt?(★0910 렌더 alt · 있을 때만 · 글자 카드 ↔ 배너 대조)}]`(홈 상단 배너 · 렌더 기하 `heroBannersOf` · 갤러리 후보 맨 앞에 이미 실림 · 렌더 없으면 []) · `materials.eventCards[].source?('event_list'|'promo_page')`·`homeLinked?`(코드가 세운 프로모션 카드 · 종료일 없는 면허 근거 = 불변 42) · `listProducts[]` 에 가격 null 인 렌더 DOM 카드(`extractRenderedProductCards`) 포함 · `media.stats.priceRenderTried / priceRenderFilled`(상세 워커 렌더 가격 채우기 · 상위 3) · `event_quote.candidates[].source?`(화면 라벨 "홈에 걸린 기획 페이지") · ★v4-3 `media.imageKinds{<사본 URL>: {kind('banner'|'product'|'document'|'photo'|'other'), text(bool)}}`(슬라이스 + 홈 갤러리 앞 8장 · 모델 1회 분류 · 없으면 판정 없음 = 선별 폴백) + `media.stats.imageKindsJudged`.
`event_quote` jsonb: `{ candidates[{quote, sourceUrl, startDate, endDate, benefitLicensed, origin('crawl'|'manual'|'card'), title?, periodRaw?, bannerUrl?(원 URL), detailUrl?}], selectedList[](★v3 ≤3 · 누른 순서 · ★0910 제작 단계(producing_image)가 배너 없는 확정 행사의 페이지를 그려 `bannerUrl`(원 URL)을 채워 넣는다 · `setEventQuoteOwned` 소유권 조건 · selected 도 함께 갱신), selected(대표 1건), confirmedBy, confirmedAt, generatedAt }`.
...
- `studio_image`: `{ url, usedCutout, personJudge, skippedReason, width, height, templateId, category, kind, media(stats)|null, mediaError|null, regenCount }` · ★2026-09-06 S3 `+ posterTexts{label,title,subtitle,dropped[]}, cutoutSource('alpha_png'|'rembg'|null), posterScore{outcome,digits,headlineVisible}|null, posterRegenerated, bannerUrl|null, bannerSize|null` · ★2026-09-09(6) `+ cutoutFrom(누끼를 딴 원천 URL · 선택 이미지 또는 상품 사본 · 누끼 0 = null)` · ★2026-09-10 `+ cutoutMode('server_compose'|null · 제품은 서버가 누끼 PNG 를 픽셀 그대로 얹음 · 배경만 생성)` · ★2026-09-15 `+ posterInk('dark'|'light'), bannerInk('dark'|'light'|null)`(B-0915-4 · 글자 띠 밝기 실측으로 고른 글자색 · light = 흰 글자 + 그림자)
- `dm`: `{ dmId, dmUrl, structureRef|null, benefitStripped, sectionTypes[], exemplarCount, regenCount }` · ★2026-09-06 `+ heroFallback(S2), visionScore{outcome,items{12항목 bool(★v3)},at}|null(S3 · 채점 없음 = null)` · ★v3 `+ blockGate{short[{type,field,len}], kept}(경고만), eventCards(수), bannerFallback(bool), autoRetry{n, reasons[]}|null, recipe{v, materials{products,banners,eventCards,licensed,licensedChars}, sectionTypes[], bindings[{sectionId,type,src,reader,ref?}], gates, look, vision, cost}(원문 0)` · ★2026-09-09 `+ sliceMode(bool), sliceCount`(기획전 슬라이스 모드 회차 · 숨김 재실행은 승계 · bindings src 에 `slice` 추가 · ref `n=장수`) · ★2026-09-15 `+ catalogDmId|null, catalogUrl|null, catalogViewerUrl|null, catalogPages(수), catalogImageUrls[](서버 합성 상품 카드 · 파기 때 삭제), catalogSkipped('too_few_images'|'error'|null)`(아웃리치 카탈로그 DM · `settings.catalog` slides · 매 회차 새로 만든다 · `stopSupersededDms`·`purgeOutreachJobArtifacts` 가 catalogDmId 도 중지)
- `studio_image` ★v3: 포스터 생략(실측 배너 ≥3 · 카드 ≥1)이면 `{ url: null, skipped: 'banners_and_cards', skippedReason, … }` 행이 남는다(하류 null 분기)
- `best_copy_assets` kind `recipe`(★v3 · CHECK 없음 실측 2026-09-06 = PK 만): `content` = 섹션 타입 순서 문자열 · `channel` 'DM' · `meta{ v, source{jobId, dmId, companyName, industryCode}, recipe, edits{total, byKind, byReason}, promotedBy, promotedAt }` · 같은 jobId 1회(`findOutreachRecipeByJob`)
- `email_html`: `{ subject, intro, html, text, placeholderCount, brandSections[], brandSubject, brandStripped, exemplarCount, subjectEditedAt?, subjectEditedBy?, regenCount }` · ★2026-09-09 `+ brandSliceMode(bool), brandSliceCount`(브랜드 시안이 기획전 슬라이스 모드였는가 · 재조립은 직전 값 승계)
`regen_count` 컬럼 = INSERT 시점의 `stage_results.regen_seq[kind]`(최초 0).

## MySQL 테이블 (QTmsg - smsdb)

### SMSQ_SEND_1~11 / 13~16 (SMS 발송 큐 - Agent 라인그룹 분배)
...
> + **SMSQ_SEND_16 = 한줄로 시스템 발송 전용 비토 라인** (★2026-09-23 신설 · Agent `hanjul-04` · .62 `/opt/bito-agent-hanjul-04` · `LIKE SMSQ_SEND_13` · 기록 = bito-gateway `status/STATUS.md` 2026-09-23 hanjul-04).
>   라인그룹 = 테스트발송 `{16,10}`(적재 = 첫 테이블 16 · 로그인 인증 문자·스팸테스트·담당자 테스트 · 10 은 이력·정산 연속용) · 슈퍼관리자인증 `{16}` · `비토게이트웨이_4`(bito) `{16}`(인증 카카오 · `SYSTEM_ALIMTALK_LINE_GROUP`). 운영 확인 0923 오후(Harold 실행 · 라인그룹 9개 · env 일치).
>   비토 라인이라 env `SMS_TABLES` 에 넣지 않는다(넣으면 라인 없는 고객사 발송이 샌다 · `BULK_ONLY_TABLES` 제외 목록에 16 은 아직 없다) · LOG 없음(13~15 와 같다). `k_etc_json` 은 13 에서 8192 를 상속한다(16 폭 실측 전).
...
| item_qty | integer NULL | ★2026-09-16 ALTER 신설(서수란 접수). **추가 항목(`channel='extra'`) 전용** — 청구 수량. 발송 행·요금제 행은 NULL. **NULL = 1**로 읽는다(`normalizeExtraQty` CT — 옛 발행분은 "1행 = 1건"이었다). ⛔ 수량을 `total_count`·`success_count`에 싣지 않는다: PDF 2페이지 '전송'·'성공' 열과 상세 모달 세로합이 발송 건수로 오염된다(요금제 일수의 0726 선례) |
| item_label | varchar(200) NULL | ★2026-09-16 ALTER 신설. **수기 부가서비스 전용** — 사람이 입력한 항목명(`billing_extra_items.label` 스냅샷). 화면·PDF의 '유형' 칸과 1페이지 항목명이 이 값을 쓴다(없으면 유형키 표시명으로 폴백). 080 행은 NULL — 그 이름은 유형키가 소유한다 |
...
| meta | jsonb DEFAULT '{}' | 자사몰별 추가 데이터 (mall_name, plan, currency 등). **★2026-09-18 provider 공통 키 `store_code`** = 이 몰의 분류코드(몰 1행 = 코드 1개 · 없으면 회사 공용). 적재가 고객을 `customer_stores`에 이 코드로 기록하고 사용자 범위(`store-scope.ts`)가 그것을 거른다. 읽는 곳 = `woocommerce-client.ts toIntegration` · `integration-scope.ts resolveStoreCodeByOriginHost`(SDK Origin). 설계서 = docs/2026-09-18-mall-integration-user-scope-design.md. **★2026-09-21 우커머스 전용 키 `woo_backfill`** = 기존 회원·주문 가져오기 진행 상태 객체(`stage` customers·orders·done / `customers_page`·`orders_page` 다음 페이지 / `orders_after` 시작 때 고정한 90일 바닥 / `customers_imported`·`orders_imported` / `customers_no_phone`·`orders_no_phone`(휴대폰 번호 없어 넣지 않은 건) · `failed`(그 건만 건너뛴 예외) / `truncated` / `started_at`·`updated_at`·`done_at`). 쓰는 곳 = `woocommerce-client.ts saveBackfill`(페이지마다) 하나 · 읽는 곳 = `toIntegration`(`readBackfill`) · `woocommerce-sync-worker.ts`(`stage !== 'done'` 이면 줄 세움) · 화면 진행 표시(`getWooStatus.backfill`). DDL 0. 설계서 = docs/2026-09-14-woocommerce-integration-design.md §5-0-1 |
...
**agency_send_requests** (접수 1건 = 1행): `id uuid PK, company_id uuid NOT NULL REFERENCES companies(id), created_by uuid NULL, status varchar(24) NOT NULL DEFAULT 'received' CHECK IN (received·testing·awaiting_approval·test_failed·approved·final_testing·queued·reapproval·expired·**cancelling**·cancelled·**sent**) ★2026-08-28 실측 정정 — `cancelling`이 문서에서 빠져 있었다(제약에는 있었다). `sent`는 0828 DDL로 추가(예약 시각이 지난 발송 종결), callback_number varchar(20) NOT NULL, message_type varchar(4) NOT NULL DEFAULT 'SMS' CHECK IN (SMS·LMS·MMS), subject varchar(60), mms_image_paths jsonb, **mms_image_names jsonb NULL**(★2026-09-10 신설 · **DDL 실행완료 2026-09-10 (information_schema 실측 = jsonb · is_nullable YES)** · 이미지 원본 파일명 문자열 배열 · mms_image_paths와 같은 순서 · 표시 전용 = 발송 배관은 안 읽는다 · 코어가 information_schema 존재 확인 뒤에만 INSERT에 싣는다 · B-0910-9), is_ad boolean NOT NULL DEFAULT false, original_content text NOT NULL, current_content text NOT NULL, content_version integer NOT NULL DEFAULT 1, requested_at timestamptz NOT NULL, manager_phone varchar(20) NOT NULL, file_name varchar(200), phone_column varchar(100) NOT NULL, var_mapping jsonb NOT NULL DEFAULT '{}', recipient_count integer NOT NULL DEFAULT 0, test_round integer NOT NULL DEFAULT 0, last_test_result jsonb, last_test_at timestamptz, approved_at timestamptz, approved_by uuid, approval_version integer, reapproval_count integer NOT NULL DEFAULT 0, final_test_at timestamptz, campaign_id uuid, queued_at timestamptz, cancelled_at timestamptz, cancel_reason varchar(200), expired_at timestamptz, lock_at timestamptz, created_at·updated_at timestamptz NOT NULL DEFAULT NOW()` + **`dispatch_key uuid`**(★2026-08-23 ALTER 실행완료). 인덱스 = `(company_id, created_at DESC)` · `(status, requested_at)`(워커 픽업).
...
**agency_send_email_senders** (허용 발신자 원장): `id uuid PK, company_id uuid NOT NULL REFERENCES companies(id), email_norm varchar(320) NOT NULL, user_id uuid NOT NULL, label varchar(100), is_active boolean NOT NULL DEFAULT true, created_by uuid, created_at·updated_at timestamptz NOT NULL DEFAULT NOW()`. **부분 UNIQUE = ★2026-08-27 `(email_norm, user_id) WHERE is_active`**(`uq_agency_email_sender_active_user` · 설계서 §20 · 옛 `(email_norm)` 단독 `uq_agency_email_sender_active`는 DROP · **교체 실행완료 = pg_indexes 실측으로 새 인덱스만 잔존 확인**) — 같은 주소를 여러 귀속(청구 계정)에 등록할 수 있고, 여럿이면 요청서 "청구 계정" 지정으로 판정한다(`matchBillingTarget` · label 또는 login_id 정확 일치 · 자동 선택 0). 다중 등록의 `label` = 담당자가 요청서에 적는 지정 이름(다중이 되는 등록은 label 필수 · routes/admin.ts가 겹침 예방). ⛔ `user_id` = 이 주소 접수의 `created_by`(발송·정산 귀속 · 없으면 dispatch_no_owner 사망). FK 없음(0728 원칙) · 접수 시점에 활성 재확인.
