...
| `event_slices` · `event_slices_detail` | ★2026-09-09 · 3값 `ok` / `no_content` / `unavailable`(면허 카드가 있어 카드 1번 상세를 렌더했을 때만 키 존재 · unavailable = 워커 부재·차단·시간 초과·호스트 이탈 · ★v4 프로모션 페이지에서 찾아도 `ok`) · 정제 문자열 ≤300 | 크롤 단계(renderKeys · 면허 카드 1번 상세 렌더 1회 · v4 프로모션 루프) | 근거 패널 |
