...
| `event_slices` · `event_slices_detail` | ★2026-09-09 · 3값 `ok` / `no_content` / `unavailable`(면허 카드가 있어 카드 1번 상세를 렌더했을 때만 키 존재 · unavailable = 워커 부재·차단·시간 초과·호스트 이탈) · 정제 문자열 ≤300 | 크롤 단계(renderKeys · 면허 카드 1번 상세 렌더 1회) | 근거 패널 |
...
★2026-09-09 `brand_profile` 추가 키: `eventSlices{ detailUrl, finalUrl, images[{url(원 URL), width, height, order}], candidates, at }` 또는 null(면허 카드 1번 상세를 렌더해 찾은 세로 슬라이스 묶음 · 3~20장 · `utils/sales-outreach-slices.ts detectEventSlices`) · `media.slices[]`(StoredImage · 재료 순서 · 폭 ≥600 · 갤러리와 분리 · 제작 단계 전용 예산 · 파기가 함께 삭제) + `media.stats.slicesTried / slicesPassed / slicesTimedOut`.
