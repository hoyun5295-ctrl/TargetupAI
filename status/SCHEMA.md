...
> `lock_at` = 워커 선점 표시이자 **lease**다. 워커는 선점할 때 DB가 적은 이 값을 이후 모든 쓰기 조건에 걸고, lock 복구도 관찰한 값으로만 되돌린다(조건 없이 쓰면 서로를 덮는다 — 경위 = LESSONS_BACKEND 핵심 원칙).
