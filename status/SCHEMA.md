...
★2026-09-09 v4 `brand_profile` 추가 키: `heroBanners[{url, width, height, href|null, order}]`(홈 상단 배너 · 렌더 기하 `heroBannersOf` · 갤러리 후보 맨 앞에 이미 실림 · 렌더 없으면 []) · `materials.eventCards[].source?('event_list'|'promo_page')`·`homeLinked?`(코드가 세운 프로모션 카드 · 종료일 없는 면허 근거 = 불변 42) · `listProducts[]` 에 가격 null 인 렌더 DOM 카드(`extractRenderedProductCards`) 포함 · `media.stats.priceRenderTried / priceRenderFilled`(상세 워커 렌더 가격 채우기 · 상위 3) · `event_quote.candidates[].source?`(화면 라벨 "홈에 걸린 기획 페이지").
