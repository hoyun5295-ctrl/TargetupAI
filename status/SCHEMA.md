...
### cdp_inapp_messages (In-app Message 정의) — D175-A 신규 + D215+ 확장 (★ 2026-06-11 실측 32컬럼 + 2026-06-17 channel 1컬럼 + 2026-06-27 블록 3컬럼 + 2026-07-07 card_style 1컬럼 = 37)
...
- ★ 2026-07-11 여정 강화 패키지 ALTER 6건 — **미실행(Harold 실행 대기·코드는 42703 가드로 안전)**: journeys `goal_kind`(varchar20 NOT NULL DEFAULT 'purchase' — 목표 종류 purchase/click/visit) · `holdout_pct`(smallint NOT NULL DEFAULT 0 — 홀드아웃 대조군 %) · `personal_send_time`(boolean NOT NULL DEFAULT false — send-time 개인화) / journey_steps `not_met_goto`(int — condition 미충족 분기 대상 step_order) · `wait_event_name`(varchar50 — wait 이벤트 대기) · `wait_timeout_hours`(int). journey_executions.status 신규 값 **'holdout'**(컬럼 아님 — 미발송 대조군, 발송 tick의 active 조회에서 자동 제외). 실행 후 이 줄 "적용 완료"로 갱신.
