...
| 3 | **.62** | `pay-ingest-db` MySQL이 `0.0.0.0:23388` 개방. **ufw active인데 실제로는 뚫림**(`.65`→`.62` 접속 OPEN 실측 — Docker가 ufw를 우회) | **0.0.0.0은 2026-07-07 Harold 승인된 의도적 설계**(PAY 수집). 다만 설계에 있던 **DOCKER-USER 출처 제한 규칙이 미적용**. DB 계정이 IP로 묶여 1차 방어는 살아 있음(`sales@.54/.57/.58`). 조치 = 설계서 232~235행 규칙 적용 |
