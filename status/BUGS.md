...
| **상태** | 🟡 수정완료-검증대기 (D62 전수점검 — AI캠페인/직접발송 모두 30분 타임아웃으로 통일. 실동작 검증 필요) |
| **증상** | 발송 후 결과가 계속 "발송중"으로 표시 |
| **근본원인** | sync-results가 success/fail > 0일 때만 업데이트. 콜백 미수신 시 영구 pending. AI캠페인은 60분, 직접발송은 30분으로 비대칭 |
| **수정** | 30분 타임아웃으로 통일(AI캠페인 60→30분). pending만 남아있으면 강제 실패 처리 + 환불 |
...
|------|------|
| **심각도** | 🟠 Major |
| **상태** | 🟡 수정완료-검증대기 (D62 전수점검 — B12-02와 동일 근본원인. AI캠페인/직접발송 30분 타임아웃 통일로 해결. 실동작 검증 필요) |
| **발견자** | 서수란, 임은지, 남지현 (B8-05 검증 시 발견) |
| **도메인** | hanjul.ai — 발송결과 |
| **증상** | 결과 수신 완료 후에도 "발송중(수동)" 또는 "진행중" 유지. 자동 상태전환 미발생 |
| **근본 원인** | sync-results의 AI캠페인 타임아웃이 60분으로 설정, 직접발송(30분)과 비대칭. B12-02와 동일 근본원인 |
| **수정** | AI캠페인 타임아웃 60→30분 통일 (campaigns.ts L1791). 직접발송(L1879)과 동일 기준 |
| **수정 파일** | `campaigns.ts` |
