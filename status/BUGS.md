...
| **근본 원인** | 분석 필요 — normalize.ts Date 객체 처리 또는 upload.ts cellDates 배포 여부 확인 |
...
| **근본 원인** | 분석 필요 — AiCampaignResultPopup.tsx "수정 완료" 버튼 confirm 로직 확인 |
...
| **출처** | PPT 슬라이드1 |
| **관련** | 신규 (CT-03 관련) |
| **증상** | 슈퍼관리자에서 cpb 계정에 080번호(080-540-5648) 설정 + 자동연동 ON → 수신거부 목록 0건, 동작 안 함 |
| **근본 원인** | 분석 필요 — CT-03 unsubscribe-helper.ts 배포 여부 + users 테이블 opt_out_080_number 컬럼 존재 여부 확인 |
...
| **근본 원인** | 분석 필요 — B16-03에서 AiCustomSendFlow에 추가한 테스트 버튼의 핸들러/props 확인 |
...
| **근본 원인** | 분석 필요 — customer_schema 라벨 복구 + customer_field_definitions 조회 로직 확인 |
...
| **근본 원인** | 필터 UI 구조 개선 필요 — 스크롤 선택 또는 직접 입력 방식으로 변경 |
...
| **근본 원인** | 분석 필요 — Toast 컴포넌트 auto-dismiss 타이머 누락 가능 |
...
| **근본 원인** | 분석 필요 — customers.ts GET / SELECT에 컬럼 추가 배포 여부 확인 |

---

*최종 업데이트: 2026-03-11 D66 17차 버그리포트 기록. 체크리스트 30건(O15/X8/▲4/미검증3) + PPT 9슬라이드 분석 완료. 16건 🔵 Open — 근본 원인 분석 순차 진행.*
