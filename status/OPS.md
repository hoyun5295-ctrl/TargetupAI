...
7. ★0924 스토어 가져오기(★B 판 2 · DDL 0 · ENV 0 · 백엔드·프론트 둘 다 빌드) — 네이버 스토어 주소를 적은 업체 1건 등록 → 확인 화면 [버튼 설치]의 [한줄로 가져오기]를 **다시** 크롬 북마크바로 끌어다 놓기(판 1 버튼은 수신 탭이 다시 설치를 안내) → [스토어 열기] → 스토어 **첫 화면**에서 북마크 → 새 탭 "○○에 네이버 스토어 기획 N개 · 상품 M개를 넣었습니다" 후 닫힘 → 확인 화면 행사 후보에 "네이버 스토어" 표시 카드(톤캉스 등 · 대표 상품 줄) + 스토어 칸 요약 줄. 확인 SQL = `SELECT company_name, jsonb_array_length(stage_results->'store_grab'->'material'->'campaigns') AS campaigns, stage_results->'store_grab'->>'at' FROM sales_outreach_jobs WHERE stage_results->'store_grab'->>'v' = '2' ORDER BY created_at DESC LIMIT 3;`. 롤백 = 코드 되돌리기(저장된 키는 구코드가 읽지 않는다).
