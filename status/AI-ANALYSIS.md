...
- [ ] 비즈니스용 멀티턴 프롬프트 작성 (5~8회 호출 시나리오)
- [ ] PDF 생성 로직 (기본 1~2p / 상세 5~10p 분기)
- [ ] PDF 디자인: 한줄로 로고, 기간 표시, 인사이트별 섹션
- [ ] 분석 결과 캐싱 검토 (동일 기간 재요청 시)
- [ ] TypeScript 타입 체크 통과

**수정 파일:** analysis.ts (프롬프트 + PDF 로직 추가)

---

### 세션 4 (선택): 데모 데이터 + 통합 테스트
**상태:** ⬜ 대기

**체크리스트:**
- [ ] 가상 고객사 데모 데이터 SQL (고객/캠페인/구매 — 중진공 시연용)
- [ ] 전체 플로우 테스트 (베이직→프리뷰, 프로→분석, 비즈니스→고급분석)
- [ ] 엣지 케이스: 캠페인 0건인 고객사, 구매 데이터 없는 고객사
- [ ] 미세 조정 (프롬프트, UI, PDF 레이아웃)
