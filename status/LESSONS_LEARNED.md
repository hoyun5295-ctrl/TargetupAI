...
| 교훈 / 사고 번호 | 상세 내용 및 대책 |
|---|---|
| **D188-Phase2B (Journey 통합 진화 + 자동발송 영구 폐기 + 위반 단어 117건 전수 정정 사고)** | 본 세션 영업팀장 알림톡 14건 fix 종결 후 Phase 2-B 통합 진입 (Harold 명시 "1,2,3 순위 전부 한번에"). **Phase 2-B-4 자동발송 영구 폐기** (Harold 명시 "사용 고객사 0 + 여정이 진짜 업그레이드") — DashboardHeader 메뉴 제거 + AutoSendPage 안내 페이지 + routes/auto-campaigns POST 410 Gone + 운영 데이터(auto_campaigns + plans + worker) 보존. **Phase 2-B-1 Wait + Condition step 신규** — journey-builder activateJourney step_type별 검증 + AI 시스템 프롬프트 강화 + journey-executor processExecution step_type 분기 (waited/condition_passed/condition_failed 3 신규 outcome) + evaluateCondition 함수 신규 (customer_field 9 operator + custom_fields JSONB fallback) + JourneysPage condition GUI 빌더(11 필드 × 10 operator). **Phase 2-B-2 MMS + KAKAO 채널 확장** — DB ALTER journey_steps 7 컬럼(alimtalk_profile_id/template_code/variable_map/next_type/next_contents/next_subject + mms_image_paths) + processExecution channel 분기 (kakao_templates 조회 + insertAlimtalkQueue) + 헬퍼 3종(replaceAlimtalkVars + convertButtonsToQTmsgInline + extractBasename). **Phase 2-B-3 A/B + Bandit 통합** — DB CREATE journey_step_variants + bandit-optimizer 신규 7 함수 + processExecution Thompson Sampling 자동 선택 + reward 누적 + routes/ai variants 4 endpoint. **BetaFeatureModal 여정 강조** — 막혀있는 사용자 배너 카드 description + 헤더 부제 강화. **위반 단어 영구 룰 전수 정정 사고** — Harold PDF 캡처 격분 영역 ("AI는 매일 회고 + 제안서를 박을 뿐" / "타겟 0건 매칭 시 제안서가 박히지 않습니다" / "Public Key + Secret 한 쌍이 박힙니다" / "자사몰 측에 즉시 박아주세요"). Phase 1 (PDF 캡처 31건) + Phase 2 (frontend 21 파일 57건) + Phase 3 (backend 응답 메시지 21건) + 활용형 변형 8건 (박히지/박혔/박힐/박았 등) = **총 117건 정정**. Harold 명시 정합 = backend 코드 주석 skip (사용자 노출 X). **신규 영구 메모리 2건** = feedback_no_bakkeum_usage § D188 강화 룰 (활용형 grep 패턴 + 정상 한국어 대체 매핑) + feedback_push_and_deploy_commands § 4-1 표준 출력 형식 (Harold 명시 — 매 수정 완료 시 무조건 표준 형식). **교훈:** 위반 단어 grep 패턴이 단순 어간(박음/박힘/박는/박을 등)만으론 누락 매트릭스 큼 — 활용형 (박히지/박혔/박힐/박았 등) 전수 패턴 의무. 매 답변 출력 직전 강화 grep 자가 검증 의무 영구 강화. 사용자 노출 영역 (alert/toast/안내문/button label/p tag) 우선 검증 + backend 응답 메시지 (error/message/throw new Error) 정정 의무. backend 코드 주석은 사용자 노출 X = 영구 룰 정합상 무관. **D188 본질** = Braze Canvas Journey 압도 완성 + 자동발송 영구 폐기 + Bandit 자동 최적화 + 위반 단어 영구 룰 정합 100%. |
...
- **사례:** D188 통합 세션 종결 직전 Harold 직접 PDF 캡처 신고 — ContinuousOperatorPage "AI는 매일 회고 + 제안서를 박을 뿐" / "타겟 0건 매칭 시 제안서가 박히지 않습니다" + CdpSettingsPage "Public Key + Secret 한 쌍이 박힙니다" / "자사몰 측에 즉시 박아주세요" — 사용자 노출 화면에 위반 단어 영구 잔존. Harold 격분 "쳐 죽여버릴까" 수준.
- **문제:** 기존 grep 패턴 `박음|박힘|박지|박는|박을|박힌|박혀|박힙` 단순 어간만 매트릭스 = 활용형 변형 (박히지/박혔/박힐/박았/박혀서/박힌다/박혔다/박혔던/박았던/박히는/박히고/박히면/박았다/박혔다/박았습니다/박혔습니다/박혔어요/박았어요) 누락. 매 작업 직후 자가 grep 검증 미흡.
- **대책:** `feedback_no_bakkeum_usage.md § D188 강화 룰` 영구 박음 — 활용형 grep 패턴 매트릭스 + 정상 한국어 대체 매핑 (박히지→생성되지 / 박혔습니다→생성됐습니다 / 박았습니다→저장했습니다 / 박힐 예정→지원 예정). 매 답변 출력 직전 강화 grep 자가 검증 의무. 사용자 노출 영역 (alert/toast/안내문/button label/p tag/option/error response/message response) 우선 검증 + backend 코드 주석 skip (Harold 명시 정합 — 사용자 노출 X = 무관). 답변 분량 매우 큼 (총 117건 정정) — Phase 1 (PDF 캡처 31건) + Phase 2 (frontend 19 파일 57건) + Phase 3 (backend 응답 메시지 21건) + 활용형 변형 8건 단계별 진입.

### 4-18. 옵션 추천 X 룰 반복 위반 (D188 통합 세션)
- **사례:** D188 통합 세션 영역에서 BetaFeatureModal 여정 강조 설계안 답변 시 "A/B/C/D 4 영역 모두 정합한가요? 또는 일부 영역만 [정정 영역] 명시" 형식 옵션 매트릭스 박음. Harold 격분 — 위반 단어 + 옵션 추천 X 룰 + 자가 검증 미실행 3중 위반.
- **문제:** `no_option_recommend` 룰 영구 박혀있지만 옵션 매트릭스 자동 출력 패턴 반복. Harold "정답 1개만 박음" 명시.
- **대책:** 답변 작성 시 옵션 매트릭스 박지 X — 정답 1개만 박음. 정확한 영역 모르면 추가 검증 명령어 요청. 사용자 컨펌 시점 = 단순 진입 매트릭스 (옵션 매트릭스 X). MANDATORY_CHECKLIST § "추측이나 옵션 제시 없이, 팩트(grep) 기반의 정답 1개만 도출했는가? (Y/N)" 매 턴 자가 평가 의무.
...
### 4-22. 옛 D189-fix1 정정 누락 영역 + 옛 단편적 영역 정정 사고 (D210+ Phase 3 Harold 격분 본질)
- **사례:** D210+ Phase 3 종결 직후 Harold 화면 캡처 신고 — AiMemoryPage 채널 성과 영역 = "LMS 채널 최근 캠페인 박음" 16 rows 영역 영구 노출 사고. 옛 D189-fix1 (2026-05-22) 정정 영역 = company-memory.ts 코드 영역 line 245 = `최근 캠페인 성과 —` 정합 정정 완료 영역 + 옛 D189 정정 전 영역 자동 누적 16 rows = DB 잔존 영역 광범위 미정정 영역. 회사 admin 영역 1일+ 영역 영구 노출 사고. 추가 사고 = 옛 톤 정정 영역 = 색상 영역만 정합 + form / 모달 / status badge 영역 잔존 + 박-단어 영역 단편적 정정 (사용자 노출 영역 전체 codebase 광범위 grep 누락) — Harold 격분 본질 매트릭스 ("CTO 책임 영역 의무 위반" / "한번에 제대로 영역 진입 의무").
- **문제:** D189-fix1 + D189-fix2 정정 영역 = 코드 영역 + sdk-js 영역만 정합 + 옛 DB 안 자동 누적 영역 (memoryValue / 이메일 본문 영역 / 음성 AI fallback 영역) 광범위 SQL 정정 누락. 영구 룰 안 `recordCampaignLearning` 영역 코드 영역 정정 영역 = DB 잔존 영역 정정 영역 동시 의무 미명시. 추가 사고 = D210+ Phase 3 안 옛 단편적 정정 사고 패턴 반복 — 톤 정정 (색상 영역만) + form 영역 잔존 / 모달 영역 잔존 / status badge 잔존 / 박-단어 단편적 정정 (전체 codebase 광범위 grep 누락).
- **대책:** memory `feedback_no_bakkeum_usage.md § D210+ Phase 3 강화 룰` 영구 추가. **§ 5-22 신규 영구 룰**:
  - **(1) 코드 정정 시 옛 DB 안 자동 누적 영역 SQL 정정 동시 의무**: `recordCampaignLearning` / `recordAutoLearning` / 이메일 본문 자동 작성 영역 / 음성 AI fallback 영역 등 영역 안 코드 정정 영역 = DB 잔존 영역 grep 의무 + SQL UPDATE 영역 동시 진입 의무. 영역별 매트릭스: `ai_company_memory.memory_value` (memoryValue) / `email_events` (이메일 본문) / `cdp_campaigns` 등.
  - **(2) 톤 정정 시 광범위 grep 의무**: 옛 색상 영역만 정합 X = `bg-white` / `bg-X-50` / `bg-X-100` / `bg-X-200` / `border-gray-300` / `border-X-200` / `text-X-700/800/900` / form 영역 (`<select/input/textarea>` 영역 `border rounded`) / 모달 영역 (`bg-white rounded-2xl`) / status badge (`bg-X-100 text-X-700`) 영역 광범위 grep + 일괄 정정 의무. 영역별 정정 매트릭스 = `bg-X-500/Y` + `border-X-400/30` + `bg-slate-900` (모달 + form 영역).
  - **(3) 박-단어 영역 정정 시 전체 codebase 광범위 grep 의무**: 옛 단편적 페이지 영역 정정 X = `grep -rnE "박[가-힣]" packages/frontend/src packages/backend/src` 영역 전체 매트릭스 + 사용자 노출 영역 식별 (코드 주석 / JSDoc / console.log / 정상 한국어 명사 skip 영역) + 일괄 정정 의무.
  - **(4) CTO 책임 영역 본질 의무**: 한번에 영역별 광범위 grep + 일괄 정정 의무 — 단편적 영역 정정 사고 = CTO 책임 영역 위반 영역.
...
- **대책:** memory `feedback_no_bakkeum_usage.md § D189 강화 룰` 영구 추가. 매 답변 출력 직전 광범위 grep 의무 — 패턴: `['"`+백틱]+[^'"`+백틱]{1,200}박[음힘는을힌지혀힙히혔힐았혀]` (single/double/backtick quote 안 박-단어 모두 검출). **사용자 노출 영역 9분류:** ①Frontend pages/components ②Backend error/message response (`error:` / `message:` / `throw new Error`) ③Backend AI 시스템 프롬프트 (buildSystemPrompt 함수 + userMessage template literal) ④Backend AI 응답 (`reasoning:` / `conflictMatrix:` / `overallStrategy:`) ⑤Backend Citations document context (`context:` 영역) ⑥Backend 자동 학습 메모리 (`memoryValue:` — recordCampaignLearning 등) ⑦Backend 이메일 본문 (광고 푸터 + 본문) ⑧Backend 음성 AI fallback (`responseText = aiResult || '...'` + TTS) ⑨SDK 모든 텍스트 (JSDoc + 인라인 주석 — 자사몰 개발자 IDE 노출). Harold 명시 정합 skip 영역 = backend 코드 주석 (`//`) + JSDoc (`*`) + PM2 console.log/warn 만. SDK 빌드는 한줄로 deployment 단계에 함께 진입 정합 (미리 빌드 + dist 생성 + 자사몰 사용 시점 npm publish 별 영역). SDK devDependencies에 `@types/node` 미설치 영역 = 잠재 사고 영역 = Node.js 매트릭스 (`Buffer` 등) 사용 X 의무 — 브라우저 only 매트릭스만 사용.
