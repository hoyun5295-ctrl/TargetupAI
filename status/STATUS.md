...
### 마케팅 플래너 (AX 월간 대행) — Phase 0~4 **전량 배포완료 + DDL 실행완료**(0813 · 남은 DDL 0)
...
> 잔여 = **실측만** — Phase 3·4 5건(풀사이클·참여·알림톡 검수·취소 환불·보류 재개) + Phase 1·2 화면 7건. **[기능 문서 §8](../docs/FEATURE-MARKETING-PLANNER.md)이 착수 원장** · 워커 등재 실측 완료(0813 20:36 `실행 10분·대조 1시간`) · 발송 창 08~21시(KST)
...
> 잔여 = 아난티 설치 재시도 회신 대기(서버 등재분 배포 필요) · 마법사 자동 주행 smoke 편입 · 원격 릴리즈 버전 가드(§6) · backdating 재대조 워커(§4) · 마법사 탈출구 배너(런북 §6)
...
> ⑩ **원스텝 AI 컨텐츠 생성** — 호출어 **"원스텝 생성"** → **[설계서](../docs/2026-08-13-one-step-content-interview-design.md)가 전부 소유**(판정 원장 §0 · 질문표 §2 · 확정 §6 · 착수 원장 §8). ★0813 설계 확정(브레인스토밍 5역할 · DM 1차 · 델타 50). 다음 = §8 Phase 0 실측 SQL 1건.
