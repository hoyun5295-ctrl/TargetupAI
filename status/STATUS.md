...
| 한 일 (계속) | ③**대행발송 치환 미리보기**(서수란) — 승인 즉시 예약안은 불변 1·2 충돌로 불수용, 접수 상세에 상위 50건 실물 문장(발송과 같은 CT · `agency-send-preview.ts` 승격 + 고객·관리자 preview 라우트 각 1) + **진입 문 2곳**(고객 목록 「상세 보기」 버튼 · 슈퍼관리자 내역 상세 모달 신설) = [B-0828-5](BUGS.md) ④**메일 MMS 개통**(서수란) — "SMS·LMS만" 축 폐기(차단 근거가 성립 안 함 실증). 별도 첨부 3장 · JPG 실체·300KB 규격이면 접수, 벗어나면 파일별 사유 반려(변환 0) · 양식 안내 개정+왕복 실측 = [B-0828-6](BUGS.md) |
