...
### ⏱ 세션 포인트 체크 — 2026-09-04(3) (직원 접수 2건 — 정산 랩디 + 모바일 DM CTA)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | **접수 2건 — 둘 다 접수 내용과 실제 원인이 달랐다.** ①정산 랩디([B-0904-5](BUGS.md)) = `KL`은 브랜드메시지가 아니라 **카카오 실패 LMS 대체발송**. 축에 별칭 흡수 + **조사 중 접수에 없던 이중 계상 발견**(전환 원본이 알림톡 성공으로도 세어짐) ②모바일 DM CTA([B-0904-4](BUGS.md)) = 배치를 읽는 렌더가 `classic` 하나뿐. 판정을 구도 축 CT로 통일 + 죽은 컨트롤 은닉. **후속 3건**(이메일 배치·인앱 말풍선·단가 문구)도 같은 날 닫았다 |
| 지금 상태 | **접수 2건 배포 완료**(게이트웨이 `f9f4f0d` · 한줄로 `0df27d96`). 게이트웨이 해시 대조 일치 · **8월 재적재 실측 `K` 1223/1215 → 1175/1175** · pm2 교체 확인. **후속 3건도 배포완료**(vitest 227파일 3,625건 · FE 빌드 통과) |
| 다음 한 수 | **화면 실측 5건**(전량 배포완료) = ①랩디 8월 정산 차단 소멸 + 공급가액 **20,079원** ②임은지 「아디다스(임은지6)」 CTA 가로 ③단가 화면 빈 칸 문구(실적 조건·테스트 상속) ④이메일 CTA 가로 배치 ⑤인앱 말풍선에서 「정렬」 컨트롤 소멸. 전부 Ctrl+F5 후 · 상세 = [B-0904-4·5](BUGS.md) |
| 미결 | **남은 구조 부채 = `norm_status`가 API 응답용·과금용 두 의미 겸용**(이번엔 집계 쪽에서 막았을 뿐 · 발송 경로라 별건). 상세·경위 = [B-0904-5](BUGS.md) |
| 배운 것 | **접수자 화면 상태는 직원 말고 원장에 묻는다**([[feedback_ask_the_ledger_not_the_staff]]) · **스냅샷 동기화에서 "행 제거"와 "0으로 덮기"는 다르다**(빠진 키를 안 지우면 옛 값이 화석 · Codex high 수용) · **한 컬럼이 두 질문에 답하면 읽는 쪽 하나가 조용히 틀린다** · **`pm2 reload`가 프로세스를 안 바꿀 수 있다**(uptime·dist 시각 대조로 검출) · **공용 편집기를 고치면 그것을 쓰는 채널 전부를 맞춰야 한다**(DM만 고쳐 이메일에 죽은 컨트롤을 남겼다) · **"조건이 있다"만 보는 계약은 `{true &&`를 통과시킨다**(같은 약점을 DM에서 겪고 인앱에서 반복) · **명령에 비밀번호가 찍히게 하지 마라**([[feedback_commands_must_never_print_secrets]]) · 교훈 원문 = [LESSONS_BACKEND](lessons/LESSONS_BACKEND.md)·[LESSONS_DEPLOY](lessons/LESSONS_DEPLOY.md) |

---

> 직전 세션(0904(2) 비토 게이트웨이 접수 4건 + 상용 관리 2메뉴 통합) 카드는 [archive/TASKS_2026-09.md](archive/TASKS_2026-09.md)로 회전.
...
### 대행발송 셀프 접수 — 입구 3 전량 배포완료 · **★0905 메일 1통 = 요청서 N건 + `발송 ID` 칸 코드완료·배포 대기(§21 · 선행 결함 2건 동반 · DDL 0)**
...
> ★0828 = 취소 불능·회신 침묵 정정 + `sent` 신설(DDL 실행완료 · 불변 20~22). 잔여 = **운영 실측만**(§20-5 5 + §15~§18-12 이월 + 0828 화면 육안·이메일 재시도) · 수용 위험 §16-3·§17-3·§18-13·§19-5·**§20-6**(form 파싱 전 반려는 다중 귀속 주소에서 회사 미기록 · 상한 재조임 때 함께 볼 것) · 별도 과제 §14-6·§18-4·[B-0825-6](BUGS.md)·[B-0825-7](BUGS.md). 남은 DDL 0
...
### AI 영업 아웃리치 — ★0905 샘플 학습 층·재료 실측·검수 발송·운영 조작 + **실물 예시 DB 학습(B안)** **0905 12:25 배포완료 · 핫픽스 [B-0905-2](BUGS.md) + 퀄리티 상향 수렴안(룩·검수 축 · 설계서 §22) 코드완료·배포 대기(DDL 0)**
> 호출어 **"영업 아웃리치"** → **[FEATURE-SALES-OUTREACH.md](../docs/FEATURE-SALES-OUTREACH.md)가 전부 소유**(불변 25 §2 · 파일별 §3 · ENV §4 · 이력 §5) · 근거 = [0905 설계서](../docs/2026-09-05-ai-sales-outreach-refinement-design.md)(§19 구현 이력 · §20 B안) · 배포 = **[OPS §2-2-E ⑦](OPS.md)**
> ⛔ 요지: 발송 잠금 5종 = `computeSendLock` 하나 / DB 선점 없이 SMTP 금지 / 실패 종결 `markFailed` · 되돌리기 `resetJobTo` 하나 / 크롤은 가드 경로만·같은 호스트 1홉만 / DM·이메일 시안 카피도 혜택 기계 차단 / 검수 발송 수신처 = 허용 도메인 안에서만 / 실물 예시 = 마스킹본만 저장(kind `outreach_example`)
> 잔여 = 메일 실측 완료(OPS ⑦ ②) → 배포(DDL 0 · [B-0905-3](BUGS.md) + 0905(4)·(5) = 설계서 §22-6~8) → 이니스프리 **새 업체 등록** 육안 → Harold 결재 2(불변 18 · 취소선) + 네이버 새 키 게이트 → OPS ⑦ 실측 7(화면에서 29건 코드 적재 · 이메일 4건 → 제작 1건 근거 패널) → Codex 리뷰(설계서 §15 · Harold가 /codex:review 직접). `scratch/` = git 제외
...
> ⑮ **브랜드메시지 — ★0828 자유형 5종 개통(한줄로 3축 + 게이트웨이 3건 전량 배포 · 남은 것 = 유형별 실측 5회)** — 호출어 **"브랜드메시지"** → [FEATURE-GW-BRAND-MESSAGE.md](../docs/bito-gateway/FEATURE-GW-BRAND-MESSAGE.md)가 전부 소유(원인·증거 §3 · REST 축 §3-1). 다음 = 발송결과 표시·청구 실측 1건. 정산 회계분 = [정산 §7](../docs/FEATURE-BILLING.md)
