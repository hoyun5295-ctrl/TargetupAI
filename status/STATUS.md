...
| 다음 한 수 | **배포**(tp-push + build:safe 양쪽 + pm2 reload) 후 실측 = 이메일 편집기 X·ESC·히어로 높이 육안 · 대행발송 상세 미리보기(치환·페이징) · **메일 MMS 실측 1건**(JPG 2장 첨부 → 접수 회신에 순서 → 담당자 테스트 문자가 이미지 포함 MMS · 규격 밖 PNG 1건 반려 회신) |
