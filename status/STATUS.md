...
### ⏱ 세션 포인트 체크 — 2026-08-27 (서체 미리보기 · 청구 계정 다중 귀속 · 소개 페이지 전면 재작성)
...
| 한 일 (계속) | ③**AI Operator 공개 소개 페이지 전면 재작성**(`public/about-ai-operator.html` · v4 → v5). 낡음의 정체 = 7월 15일 판이라 원스텝 생성·이미지 스튜디오·고객 360·RCS·대행발송이 통째로 빠져 있었고, 줄표 60개·영문 eyebrow·그라데이션 글자로 AI 티 표면 신호가 깔려 있었다. 14장으로 재구성(기능 7 → 9 + 자사몰 연동 + **대행발송** + 클로징) · 화려함은 장식이 아니라 **실물 재현**으로(타이핑 명령 바·여정 캔버스·제안 카드·풀 폰 DM·메일 창·자사몰 팝업·스튜디오 3안·학습 전후·고객 타임라인·연동 파이프) · 데모는 한 이야기로 관통. **갱신 규약을 파일 상단 주석에 심었다**(기능 늘면 같이 갱신 · 캐시버스터 3곳 · 문안 규약 · 배포법) |
| 지금 상태 | **셋 다 배포완료 · DDL(인덱스 교체) 실행완료**(Harold 실측: 새 `uq_agency_email_sender_active_user`만 잔존). 게이트 = backend tsc 0 · vitest 205파일 3,175건 · frontend tsc 0·production 빌드 · 산출물 CSS 규칙 실존 grep · 양식 재생성 파서 왕복 실측 · 회귀 주입 2회. **Codex 적대 2R 종결**(1R high 2·medium 3 → 4건 정정 / 2R medium 2 → 1건 정정·1건 수용 위험 §20-6) |
| 다음 한 수 | 운영 실측 = **§20-5 5건**(금강제화형 2계정 등록 → 지정 없이 = 목록 반려 → "청구 계정: 금강" = 그 계정 명의 접수 · 오타 반려 · 단일 귀속 무변화 · 등록 모달 400 안내·뱃지) + **서체 육안**(이메일 편집기 [서체]·DM 퀵바 [서체]·브랜드킷·테마 모달 2곳에서 12종이 서로 다른 글꼴로 보이는가) + **소개 페이지 육안**(`/about-ai-operator.html?v=5` 14장 · PC 덱과 모바일 스크롤 양쪽) |
| 미결 | 이월 = 대행발송 운영 실측 §15~§18-12 · SMTP 발신 curl · 하이웍스 접속 정책 · [B-0825-6](BUGS.md)·[B-0825-7](BUGS.md) · 화면 접수 무헤더 명단(공용 업로드 파서) · 어드민 탭 톤 전환. **신규 = [B-0827-3] 추가 과제**(DM 편집 캔버스·인앱 미리보기 서체도 같은 전역 규칙에 눌려 있다 = "편집 화면 = 발행물" 서체 축 위반 · 근본 수정은 인라인 51곳 전부 소생이라 별도 세션) · **소개 페이지 축 밖 발견 2**(ⓐ`public/`에 옛 실험본 `about-ai-operator-v5.html`·`v6.html`과 안 쓰는 `about-ai-operator/slide-1~17.png` 4.3MB 잔존 = 언제든 삭제 가능 ⓑ`docs/AI_OPERATOR_기능정의서.md`가 2026-05-25 기준이라 지금 기능과 어긋난다 · 내부 문서라 급하지 않지만 대외 인용 금지) |
