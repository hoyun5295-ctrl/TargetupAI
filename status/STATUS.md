...
### ⏱ 세션 포인트 체크 — 2026-09-10 (⛔사고: 발송 문안 치환 배선 배포 · 원복 완료)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | **⛔ 내가 낸 사고가 이 세션의 축이다([B-0910-4](BUGS.md))** — `bulkInsertSmsQueue`에 문안 치환을 배선했다가 제거했는데 **커밋을 빠뜨려** 12:11 빌드에 실려 배포됐다. `▶`→`>`, `★`→`*` 로 바뀌어 나갔고 시세이도 클레임이 그것이다. 그 밖에 ①통계가 `send_config`를 안 읽어 비토 라인 캠페인이 전부 0([B-0910-1](BUGS.md)) ②예약 건이 커서에 갇힘([B-0910-2](BUGS.md)) ③게이트웨이가 검증오류를 재시도로 분류([B-0910-3](BUGS.md)) ④발급 묶음 config ↔ 생성문 체계 불일치 |
| 지금 상태 | **사고 = 원복·복구 완료**(배포본 `sanitizeForSms` 0 · 시세이도 109건 역치환 109/0 · **이미 나간 3건은 되돌릴 수 없다**). ①④배포 완료 · ②1.0.27 두 라인 적용 확인 · ③P1 배포·실측 완료(게이트웨이 `5b3d48e`) · UTF-8 = 젬텍 213 반려로 불가 확정 · **규격 밖 글자 P2 배포·실측 완료**([설계서](../docs/2026-09-10-unsupported-char-substitution-design.md)) |
| 다음 한 수 | **[0911 인계](../docs/2026-09-11-session-handoff.md) §2 순서** — 아침 상태 확인 → 08:00~20:50 이모지 브랜드 1건 실측([B-0910-11](BUGS.md)) → P3-4(Agent encoding 1 안내 · 젬텍 213) → P3-2 → P3-3 |
| 미결 | ★0910 밤 결정 = **막고 계도**(자동 대체 안 함 · 설계서 §0-1) · 휴머스온 RESEND_* 문자 재발송의 이모지 처리 미검증 · P4 = CT-46 정리([B-0910-5](BUGS.md)) · 비토 라인 예약이 라인 변경 뒤 적재될 때 커서 동작 실측 |
| 배운 것 | **"제거했다"는 커밋·배포본까지 확인해야 성립한다**([[feedback_removed_means_committed_and_deployed]] — 로컬만 보고 "배포된 적 없다"고 여러 번 단정했다. Harold님이 "네가 건드린 게 문제"라고 여러 번 말했는데도) · **고객 문안은 우리가 바꾸지 않는다**([[feedback_never_alter_customer_copy_silently]]) · **검사는 예시 말고 실제 발급본을 대조한다**([[feedback_test_the_issued_artifact_not_the_example]]) · **부품이 다 있어도 부르는 곳이 없으면 죽은 검사다** · 경위 전량 = memory `project_2026_0910_send_result_and_agent_cursor` |
