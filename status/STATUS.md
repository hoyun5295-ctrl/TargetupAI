...
### ⏱ 세션 포인트 체크 — 2026-08-26(2) (대행발송 통일 양식 + 사용자 격리 + 슈퍼관리자 내역)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**(다음 세션이 자기 것으로 덮어쓴다). 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | 대행발송 5축(설계서 §19): ①**통일 양식** = 업계 실물(카톡 수신 xlsx · "내용"+"고객리스트" 한 파일)로 세 입구 통일(화면 슬롯 1개 · 이메일 1파일 표준 · **배포 양식 = 실물 셀 단위 복제** §19-6) ②**사용자 격리** = company_user 본인만·관리자 전체 + 접수 계정 표시 ③**슈퍼관리자 "대행발송 내역" 탭** 신설(레일+접수구분+고객사·신청자) ④**운영 취소**(§19-7) = 취소 CT 승격 + admin API + 확정 시 담당자 안내 문자 ⑤**보낼 시각 연도 필수 폐지 + 표기 확장**(§19-8·§19-9 · 혼합·상대 날짜·시간대 낱말·반 · "밤 12시"는 반려) ⑥**승인 문자 단축 URL(CT-40)+요청 건수** + 허용 이메일 "아이디 - 이름" 표시(§19-9) |
| 지금 상태 | **코드완료 · 배포 대기**(backend+frontend 둘 다 · DDL 0 · 신규 의존성 0). 게이트 = 파서 왕복 실측(신양식·실물 양식) · backend/frontend tsc 0 · vitest 202파일 3,102건 · production 빌드 통과 · `created_by` 백필 불필요 실측(5행 no_owner 0). Codex 생략(Harold 지시)·자체 적대 검토 종결(정정 2건 반영) |
| 다음 한 수 | 배포(OPS §2-2 + **DDL 1**: `ALTER TABLE agency_send_requests ADD COLUMN requested_at_original timestamptz` · 컬럼 폴백이 있어 배포 뒤 실행해도 안전) → 실측 6: ①새 양식으로 화면 접수 1건 ②이메일 통합 1파일 접수 1건 ③company_user 격리 + 관리자 전체·접수 계정 표시 ④슈퍼관리자 내역 탭 육안 ⑤내역 탭 운영 취소(담당자 문자까지) ⑥**촉박한 시각(20분 뒤)으로 접수 → 조정 안내 4곳 확인**(확인 화면·담당자 문자 문안·회신 메일·시각 변경 토스트) |
| 미결 | 직전 세션분 이월 = 이메일 운영 실측 7(§18-12) · SMTP 발신 curl · 하이웍스 접속 정책 · [B-0825-6](BUGS.md) · [B-0825-7](BUGS.md) · 화면 접수 무헤더 명단(공용 업로드 파서) · 어드민 탭 톤 전환(별도 트랙) · 고객사 수정 화면 육안 판정 |
| 배운 것 | **양식 값 칸의 안내문은 값으로 접수된다** — 안내는 안내 열·안내 시트에만(빈칸 규약 + 파서 PLACEHOLDER가 짝) · **명단 시트는 1행=열 이름**(안내 배너 = 헤더 오독 · 적대 검토 실검출) · **라벨 인식 증명은 빈 템플릿으론 안 된다**(미인식과 빈칸이 같은 결과 · 값 채운 픽스처로) · 격리 판정은 JWT 어휘로만(SCHEMA users 함정) |
...
### 대행발송 셀프 접수 · 0826 이메일 접수 배포완료·워커 가동 · **0826(2) 통일 양식+격리+슈퍼관리자 내역 코드완료·배포 대기**
> 호출어 **"대행발송"** → **[FEATURE-AGENCY-SEND.md](../docs/FEATURE-AGENCY-SEND.md)가 전부 소유**(★0826 상설 승격 — §2 불변 18개 필독 → §3 파일별 소유 → §4 운영 설정 · 이력·뒤집힌 판단 §5). 시점 근거 = [0822 설계서](../docs/2026-08-22-agency-send-design.md)(통일·격리 = §19) · 운영 명령 = [OPS.md §2-2-D](OPS.md)
> ⛔ 큐 직접 금지 / **정산 축 새 값 금지**(`send_type='direct'`) / 소유권 = `revision`·`lock_token`(타임스탬프 토큰 금지) / `final_test_at`은 통과 분기만 / 접수 쓰기 = `createRequestCore` 하나 / **이메일 = DELE 0 · allowlist_only · 확정 경로 AI 0**(전문 = §18-2·§18-5) / **★0826(2) 격리 = JWT 어휘로만 · 양식 값 칸 안내문 금지·명단 시트 1행=헤더**(§2 불변 9·11)
> 잔여 = **0826(2) 배포 + 실측 4**(세션 포인트 체크) → 운영 실측 §15~§18-12 · 수용 위험 §16-3·§17-3·§18-13·§19-5 · 별도 과제 §14-6·§18-4·[B-0825-6](BUGS.md)·[B-0825-7](BUGS.md). DDL 0
...
### AI 영업 아웃리치 v2 — 0824 코드 전량 완료·**배포 대기(ENV+DDL만 남음)** · **0826 v3(스마트스토어 재료 축) 착수 보류 = 게이트 G4 실패**
> 호출어 **"영업 아웃리치"** → **[FEATURE-SALES-OUTREACH.md](../docs/FEATURE-SALES-OUTREACH.md)가 전부 소유**(불변 17개 §2 필독 · 파일별 §3 · ENV §4 · 이력 §5). 근거 = [0731 설계서](../docs/2026-07-31-ai-sales-outreach-design.md) §15(v2)·§16-9-1(게이트) · 배포 = **[OPS §2-2-E](OPS.md)**
> ⛔ 요지만: 발송 3중 잠금 / **DB 선점 없이 SMTP 금지** / 산출물 INSERT는 `insertAssetOwned`만 / fail-closed 게이트 / 크롤은 가드 경로만 / 자동 재시도·발송 0 / **v3 = 행사 인용은 공식몰 전용·스토어는 재료만·slug 완전 일치·lprice 미수집** — **전문은 기능 문서 §2 + 설계서 §16-2**
> 잔여 = **배포**(ENV 7키 + DDL 2 · 절차·SQL 전부 [OPS §2-2-E](OPS.md)) → 실측 1건 → 샘플 15건. **v3 보류** = 네이버 `shop.json` 404(같은 키 blog 200)로 재료 소스 부재([B-0826-2](BUGS.md) · 재개 조건 §16-9-1) · 별건 [B-0824-2·B-0824-3](BUGS.md)
