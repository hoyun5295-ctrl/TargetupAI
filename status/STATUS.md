...
### 🟢 2026-07-12 (5) — AI 학습메모리 전수점검 + 일괄 강화 (코드완료 — 배포 대기)
> 전수 점검(CT 7종·라우트 ai-memory 1,084+ai.ts CRUD·주입 소비처·발원 배선·프론트 4종) 후 Harold "P0-1부터 한번에 끝까지" 지시로 일괄 구현: **P0 주입 슬롯 잠식 종식** — listMemories 타입 무필터라 비학습 타입(brand_guideline 10·대표문안 8·brand_link 7)이 중요도 상위 슬롯 점유 → 문안 생성(6슬롯)은 브랜드보이스 등록 회사에서 학습 주입 0이던 결함. `LEARNING_MEMORY_TYPES`(학습 5종) 단일 정의+`memoryTypes` 화이트리스트 옵션(빈 배열=0건)으로 10곳 통일(buildMemoryPromptContext·overview·top-impact·search related·learning-summary·legacy GET·citations·crm-agency-proposal·dm-event-recommender·dm-self-diagnosis — 전수 grep이 설계 밖 직접 소비 4곳 추가 적발). usage_count 거짓 집계(비렌더 brand_link가 "AI 최다 참고"로 노출)·guideline JSON 원문 노출 동시 종식. **P1** legacy POST validTypes 5종 축소(brand_guideline 자유 텍스트 upsert=구조 파손 벡터)+2000자 가드+source 서버 강제(ai_auto 위장 차단) / **여정 학습 실클릭 연결**(clickCount 0 하드코딩=게이트 영구 차단이던 결함 — 분모=실발송 EXISTS·properties.journey_id 귀속·신원 2축·source='short_url_click'+hash 한정 4중 경계) / stale 등급 인사이트 정리(3중 경계 DELETE+선정·삭제 술어 일치+qualifying 0 회사 정리 진입) / 라벨 정직(compliance="자동마케팅 정지 사유 자동"·brand_tone=자동 2경로 실존 반영). Codex 적대 4라운드(gpt-5.5·xhigh — 1R 3건→2R 2건→3R 1건 정정→4R "배포 가능·잔존 0"). 검증 BE tsc0·vitest 54파일 457(신규 계약 테스트)·FE tsc0·금지 패턴 신규 0·DDL 0. **잔여 = 배포(tp-push + build:safe frontend·backend + pm2 reload).** 별도 과제 후보: 여정 클릭 목표(goal_kind=click) 판정 동일 한계·cdp_events 인덱스(DDL)·스팸차단/알림톡 반려→compliance 자동 배선. 상세 [[project_2026_0712_ai_memory_full_reinforcement]]

### 🟢 2026-07-12 (4) — 인앱메시지 일괄 강화 P0-1~P2-2 (★배포완료 2026-07-12 — Harold 선언 / 실측 5건 = 직원 테스트)
> SoT=[specs/2026-07-12-inapp-full-reinforcement-design.md](../docs/superpowers/specs/2026-07-12-inapp-full-reinforcement-design.md) 전 항목 구현: **P0** ①트리거 임계값 검증 — SDK 자동 트리거 발화 시 실측값 동봉(scrollPercent·timeOnPageSeconds)+순수 passesTriggerThresholds(기본 50%/10초·미전달 통과·cart_value 기존 유지)+편집기 임계 입력칸(scroll %·체류 초·장바구니 금액)+직접 trigger() 호출 시 실측 보강(Codex 1R — 콘솔 우회 차단) ②SDK 버튼 4곳 safeNavigate 스킴 화이트리스트(javascript:/data: 차단 — new URL 파싱 후 protocol 판정)+서버 저장 3경로 sanitizeActionUrl 무해화(cta_group 포함·camel/snake 양쪽 수용[Codex 2R]·placeholder 보존) ③track 소유 검증(trackImpression 초입 SELECT+uuid 형식 → 400) ④중복 렌더 가드(같은 data-hanjullo-msg DOM 잔존 시 skip — always 의미 보존) **P1** 부분 PUT presence flag 원자 병합(CASE WHEN $32~$37 — 선조회 병합은 동시 PUT 경합이라 폐기[Codex 1R]·action_url 명시 삭제/위험 스킴 클리어 지원)·새벽 시간대 amber 경고(차단 아님 — 법 판정 §0)·stats 파라미터 bind 버그 1줄 수리·variant 6컬럼 상속(content_blocks=replaceBlockTexts 사본+theme/accent/card_style/badge/channel — quickActionAIRefine 동일 경로 해소)·레거시/토스트 (광고) 자동 표기+AI 프롬프트 080 거짓 서술 삭제 **P2** 블록 드래그앤드롭+복제(@dnd-kit 이식·uid는 블록 객체 참조 reconcile[Codex 1R]·commit 패턴으로 편집 중 포커스 보존)·상품 블록 가격 구조화(price/discount_price — SDK+BlockPreview+에디터 3면, 할인가 accent 800+정가 취소선, meta는 가격 미입력 시만=하위호환). Codex 적대 3라운드(gpt-5.5·xhigh): 1R 3건+추가 1건 정정 → 2R 잔존 1건 정정 → 3R "잔존 결함·새 회귀 없음 — 배포 가능". 검증 BE tsc0·vitest 53파일 455(신규 18)·SDK tsc0·vitest 141(신규 10)·FE tsc0·금지패턴0·DDL 0. **SDK 번들 재빌드 → company-frontend/public/sdk v0.3.8·v0.3.9 제자리 갱신(md5 3경로 동일 — 설치 몰 재설치 불필요)**. **★배포완료(2026-07-12) + 과거 위험 action_url 진단 SELECT 0건 실측 — 잔여 = 실측 5건(직원 — 스크롤 70% 임계 동작 / javascript: URL 저장→무해화 / 블록 부모 A/B variant 동일 디자인 / 드래그 순서 변경·복제 / 상품 블록 가격 표시).** 상세 [[project_2026_0712_inapp_full_reinforcement]]
