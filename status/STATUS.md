...
### 🚨🚨🚨 최우선 — 발송 파이프라인 전면 복구 + GPT P0 결함 수정 (2026-02-26~)

> **현상:** 8차 전수점검 "통과" 보고 후에도 심각한 구조적 결함 7건 추가 발견. 1단계 코드 검증이 형식적(라인 존재만 확인)이었음이 판명.
> **GPT 크로스체크:** 파일 6개를 한 번 보고 P0급 결함 5건 즉시 발견 (Claude 수십 세션 동안 미발견). P0-5는 이번 수정에서 해결, P0-1~P0-4 추가 수정 완료.
> **Harold님 판단:** "Claude 못 믿겠다" — 모든 수정 결과는 GPT 교차검증 필수
> **근본 원인:** 발송 5개 경로에 변수 치환 로직이 제각각 분산 구현 + 보안/정산/시간대 구조적 결함
> **상세 → BUGS.md 섹션 3 (구조적 결함) + 섹션 3-1 (GPT P0 결함)**
...
| 순번 | 작업 | 관련 버그 | 상태 |
|------|------|----------|------|
| 1-0 | **공통 치환 함수 설계 → Harold님 컨펌** | 전체 | ✅ messageUtils.ts 신규 생성 |
| 1-1 | 직접타겟발송 customMessages 백엔드 수용 | S9-01 (🔴치명) | ✅ 코드완료 (배포대기) |
| 1-2 | 스팸필터/테스트 서버 DB 직접 조회 전환 | S9-02 (🔴치명) | ✅ 코드완료 — 프론트 하드코딩 제거 + 서버 DB SELECT로 전환 |
| 1-3 | sent_at 경쟁 조건 해결 | S9-04 (🟠중요) | ⬜ |
| 1-4 | ai.ts subject 키 중복 정리 | S9-05 (🟡경미) | ⬜ |
| 1-5 | 스팸필터 테스트 라인 선택 통일 | S9-06 (🟡경미) | ✅ Closed (정상운영) |
| 1-6 | 5개 경로 공통 함수 교체 + 전수 검증 | 전체 | ✅ 코드완료 (배포대기) |

#### 🔴 GPT P0 결함 — 보안/정산/시간대 (GPT 크로스체크 발견)

| 순번 | 작업 | 관련 버그 | 상태 |
|------|------|----------|------|
| P0-1 | spam-filter.ts 권한 체크 `role` → `userType` 수정 | GP-01 (🔴보안) | ✅ 코드완료 (배포대기) |
| P0-2 | `/uploads` 정적 서빙 인증 없음 (PII 노출) | GP-02 (🔴🔴보안) | ⬜ 파일 위치 확인 필요 |
| P0-3 | 선불 차감 후 발송 실패 시 환불 미보장 | GP-03 (🔴🔴정산) | ✅ 코드완료 — AI발송/직접발송/테스트 3경로 환불 추가 (배포대기) |
| P0-4 | 예약/분할 발송 UTC/KST 불일치 | GP-04 (🟠시간대) | ✅ 코드완료 — MySQL 세션 TZ +09:00 고정 (배포대기) |
| P0-5 | 직접발송 잔여 %변수% strip 누락 | GP-05 (🟠치환) | ✅ 이번 수정에서 해결 (안전망 strip 추가) |

#### 🟠 2덩어리 — 데이터/성능 안정성

| 순번 | 작업 | 관련 버그 | 상태 |
|------|------|----------|------|
| 2-1 | 나이 필터 하드코딩 → 동적 연도 계산 | S9-03 (🟠중요) | ✅ 코드완료 (1덩어리와 함께 처리) |
...
> 다음 세션에서 반드시 처리/확인해야 할 항목. 놓치면 안 됨.

**1. 배포 전 필수 확인 (Harold님)**
- [ ] `prepaidRefund` 함수 존재 여부 확인 — P0-3 환불 로직이 이 함수에 의존. 없으면 신규 생성 필요
- [ ] TypeScript 타입 체크 (`tsc --noEmit`) — campaigns.ts 변경량 많음 (+50줄), 타입 에러 가능성
- [ ] `mysqlQuery("SET time_zone = '+09:00'")` — 커넥션 풀 환경에서 세션 단위 설정이 유효한지 확인. 풀 전체에 적용 안 될 수 있음

**2. 배포 후 실동작 검증 3가지**
- [ ] 스팸필터 테스트 — `%이름%`이 실제 고객 이름으로 치환되는지 (DB 직접 조회 확인)
- [ ] 담당자 사전수신 — `%이름%`이 실제 고객 이름으로 치환되는지
- [ ] 직접입력 발송 — `%이름%`이 실제 고객 이름으로 치환되는지

**3. 미수정 잔여 결함**
- [ ] GP-02: `/uploads` 정적 서빙 보안 — 어떤 파일에서 서빙하는지 확인 필요 (app.ts? nginx?)
- [ ] S9-04: sent_at 경쟁 조건 — sync-results 재점검
- [ ] S9-05: ai.ts subject 키 중복 — 프롬프트 JSON 정리
- [ ] S9-07: AiCustomSendFlow alert/confirm → 커스텀 모달
- [ ] S9-08: results.ts 서버측 페이지네이션

**4. GPT 교차검증 대상**
- [ ] 이번 수정 파일 3개 (campaigns.ts, spam-filter.ts, Dashboard.tsx) GPT에게 전달하여 추가 결함 확인
- [ ] 특히 P0-3 환불 로직 — prepaidRefund 호출 인자/타이밍/에러처리 GPT 검증
- [ ] P0-4 MySQL TZ — 커넥션 풀 환경에서 세션 설정 유효성 GPT 검증

---

### ✅ 이전 완료 요약

> - GPT P0 결함 4건 수정 (P0-1 권한체크, P0-3 환불보장, P0-4 MySQL TZ, P0-5 잔여변수 strip)
> - 발송 5개 경로 통합: 프론트 하드코딩 치환 전면 제거 → 서버 DB 직접 조회 전환
