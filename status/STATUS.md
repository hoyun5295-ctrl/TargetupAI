...
### 0801 여정 재설계 — §11-1~11-3 배포완료+DDL / **§11-4 코드완료, 배포+DDL 대기**
> SoT [여정 재설계](docs/2026-08-01-journey-redesign-design.md) **§11-C(원장 직독 — 적대검증이 뒤집은 판단) → §11-A·§11-B → §11(순서)** · 기억 [[project_2026_0801_journey_redesign]] · 호출어 **여정 재설계 이어가자** · 다음 = 배포 → **§11-C-5 DDL**(인덱스 먼저, 커서 컬럼 2개) → 실측 5건 → §11-5
> ⛔ **정답표를 안 갖는다**(§2-3) / **진실을 복사하지 않는다** — 미러는 멱등키·대조워커·지표오염을 부른다(§11-C-0) / **문은 회사마다 하나**(자사몰 현역이면 원장 문 닫힘) / 유예는 대량 발화를 못 막는다(상한 필수가 막는다) / 익명 소급(N5)은 착수 5번 몫
> 잔여 = §11-4 배포·DDL·실측 · §11-5~7 · Harold 확인 1건(예약 취소·노쇼 구분) · 별건 2(journey-stats step·segment 전환이 쓰이지 않는 'order'만 읽음 · schema template `purchase_date` TIMESTAMPTZ 불일치)
