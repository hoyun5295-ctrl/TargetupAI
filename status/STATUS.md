...
> 잔여 = **Phase 0~4 전량 구현완료(0810·배포 대기)** — bySource + 정규화 CT + 판정 훅 + 현황판 + 3단계 스테퍼(검증 탭 제거·몰별 폼 이관) + 개발자 안내 복사 + 계약 테스트 22건(결함 주입 검출 확인). 롤백 = `CDP_DASHBOARD_V2=false`. 다음 = **배포·실측** → Phase 5 파일 분해. 착수 전 확인 §9 중 4·6·7·8 미해결
