...
### ⏱ 세션 포인트 체크 — 2026-09-04(2) (비토 게이트웨이 접수 4건 + 상용 관리 2메뉴 통합)
...
| 지금 상태 | **코드 완료 · 미커밋 · 배포 대기**(저장소 = `projects/bito-gateway`). 대시보드 계약 25건 · api 400건 · Go 게이트웨이 패키지 전량 · production build 통과. **DDL 1건(056) 선행 필수** |
| 다음 한 수 | ①배포 = **056 → api 8파일 → Gateway 바이너리 → dist** 순서(뒤집으면 이력 적재가 42P01로 스스로 멈춘 채 끝난다) ②실측 = 회선별 표와 상위단 청구 대조 1건 · Agent 재접속 후 "마지막 접속" 갱신 1건 · 통합 화면 육안(고객사 행 작업 4종 · Agent 행 작업 5종) |
| 미결 | Go `internal/agent/bootstrap` 테스트 2건은 Windows 권한 모델 실패(내 변경과 무관 · WSL 통과 기왕 확인) · 접속 이력 보존 기간 정책 미정(현재 무한 보존) |
| 배운 것 | **실측 SQL 두 줄이 설계 두 곳을 확정했다** — `route_id` 0건이 회선 축을 정했고, 등록부 `source` 1행이 T4 제거 범위를 정했다 · **"활성"과 "접속"은 다른 말이다** — 화면이 오래 보여 준 것은 `is_active` 플래그였고 접수자가 물은 것은 실제 접속이었다 · **이력 적재가 발송을 막으면 안 된다** — 두 writer 모두 실패를 삼키고 테이블 부재도 조용히 지나간다 · **메뉴 통합은 화면이 아니라 코드를 옮기는 일** — 원본을 줄 단위로 복사하고 옮긴 화면을 전부 렌더해 봤다(import 성공은 증거가 아니다) · **옛 화면을 지울 때 그 화면이 지키던 계약도 같이 지우면 안 된다** — Token 직접 입력 금지 등 2건을 남은 테스트로 이관 |
