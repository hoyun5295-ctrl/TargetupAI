...
> **잔여 = Harold 배포(tp-push+frontend build:safe) → 로그인 페이지 Ctrl+F5 → 서비스 소개 클릭 실측.** 상세 [[project_2026_0713_operator_intro_effects]]
...
### 🔵 다음 세션 (예정)
> ⓪-A **이메일 디자인 3.0 (독립 세션 — Harold 확정 2026-07-13)** — DM 디자인 3.0([[project_2026_0713_dm_design_3]])의 이메일판. 재사용: 테마 프리셋 데이터 모델(brand_kit JSONB 동승·DDL 0 지향)·서체 카탈로그(웹폰트 미지원 클라이언트 폴백 스택 필수)·골든 템플릿 방법론·구도 시스템·배경면 리듬(테이블 bgcolor 치환)·헤드라인 마커. 이식 불가: 모션 팩·스티키 CTA·글래스(클라이언트 렌더 제약). 이메일 고유 축: 클라이언트 다크모드 대응(Gmail 임의 반전)·프리헤더·불릿프루프 버튼(VML)·모바일 조판. 시작 전 이메일 편집기·발송 렌더러 현황 전수 파악부터(0712 이메일 강화 14건 = [[project_2026_0712_email_marketing_audit]]).
