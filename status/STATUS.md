...
- 🟢 2026-07-21 — 담당자 생성물 격리 + 인앱 관리자 전용 재조정(스튜디오 로그아웃 근본수정) — 인앱·이메일 라우트가 company_admin 전용이라 담당자(user_type='user'→JWT company_user) 진입 시 /login 튕김이 근본. 신규 `utils/owner-scope.ts` CT + created_by 격리(assets·이메일·행사초안: admin/super=회사전체·담당자=본인) · 인앱=전 경로 관리자 전용(공유 화면) · 이메일=담당자 개방 · DM 기존 격리(canAccessDm). **코드완료·Codex 적대검증 7R 배포가능·재배포 대기(프론트 build:safe 필수).** 교훈=lessons/LESSONS_BACKEND·FRONTEND
