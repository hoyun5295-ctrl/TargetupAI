...
### ⏱ 세션 포인트 체크 — 2026-09-20 (브랜드메시지 발송 창 개편 · **자유형 5종 시험 개방** · Agent 1.0.31)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 [BUGS](BUGS.md) B-0920-1~3 · 게이트웨이 쪽은 그 저장소 `status/BUG_HISTORY.md` 2026-09-20·(2) · 구조는 [FEATURE-GW-BRAND-MESSAGE §7-9](../docs/bito-gateway/FEATURE-GW-BRAND-MESSAGE.md).

| 항목 | 상태 |
|---|---|
| 한 일(한줄로) | 발송 창 개편(유형 선택 창 · 선택 목록 · 080 설정값 고정/하이픈 자동 · 실수신 기준 미리보기) · 5종 시험 개방(`BUBBLE_TYPE_TRIAL` + ENV `BRAND_TRIAL_LOGIN_IDS` · 캐러셀 배선 · 이미지 창구 6 · `k_etc_json` 테이블 폭 판정) · 이미지 규격 상시 표시 + 실측 경고 창·자동 맞춤 · 값 검사(동영상 카카오TV만 · 가격 · 링크) · 링크 칸 자동 `https://` · 다중 업로드 응답 추출 정정(`data.success[]`) · 말풍선 버튼 줄 깨짐 정정 |
| 한 일(게이트웨이) | Agent 빈 본문 허용 → **1.0.31 서명 릴리즈 · hanjul01·02·03 원격 배포 완료** · 원격 배포 워커 정지 결함(고아 격리 EACCES 가 주기 전체 롤백) 정정·배포 · 설치 안내서 5건 정정 · 1.0.31 게시 5파일 · 서버 발급 코드 8/8 저장소와 일치 확인 |
| 지금 상태 | backend 339파일 5,153건 · tsc 0(백·프) · **전량 배포완료** · MySQL `SMSQ_SEND_13~15` `k_etc_json` 8192(PG DDL 0) · **실측 성공 2종 = 캐러셀 피드 · 커머스(`1800` · 실수신)** · 5종은 아직 시험 계정(psy5868)에만 보인다 |
| 다음 한 수 | ①남은 3종 직원 실측(캐러셀 커머스 · 와이드 리스트 · 프리미엄 동영상=카카오TV 주소) — 직원 계정은 ENV 명단에 추가 ②성공 유형을 `BUBBLE_TYPE_TRIAL` → `BUBBLE_TYPE_OPENED` 로 옮기고 프론트 규격 사본 `opened` 갱신(파리티 테스트가 대조) ③발송 창 8종 화면 육안 확인 |
| 미결(축 밖 · 기록만) | 말풍선 버튼·쿠폰·이미지 링크의 서버 검사(스킴 없는 링크를 카카오가 거절하는지 실측 뒤) · 5종 미리보기 배치(실수신 캡처 대조 전) · 템플릿 등록 화면 미리보기 고정 문구 · 기본형(FB) 적재 미구현 · 게이트웨이: 패키지 안 README 옛 절 · npm 스크립트에 안 걸린 테스트 34개 |
| 배운 것 | **한 번도 끝까지 돌아 본 적 없는 경로는 부품이 다 있어도 죽어 있다**(다중 업로드 응답 · Agent 빈 본문 · 7일 뒤에야 발동하는 고아 격리 — 셋 다 테스트는 있었고 셋 다 **지어낸 대역**으로 통과했다 → 대역은 운영 로그 원문으로) · **공용 클래스에 덧붙인 Tailwind 값은 `!` 없이는 질 수 있다**(적힌 순서가 아니라 생성 CSS 순서 · `w-full` > `w-28`) · **청소 작업을 본업과 한 트랜잭션에 묶지 마라**(격리 실패가 명령 발행을 되돌렸다) · **승인 전에 게시**(승인 순간 발급이 새 버전 파일을 찾는다) |
...
> ★ **SNS 채널 — 0921 S0~S3 코드 완료 · 미배포 · 게시 실측 대기** — 호출어 **"SNS 채널"** → **[FEATURE-SNS-CHANNEL.md](../docs/FEATURE-SNS-CHANNEL.md)가 전부 소유**(불변 §2 · ⛔원본 무접촉 §5 · 남은 것 §7). 다음 한 수 = 배포(한산한 시간) → 게시 1건 실측 → Codex 적대 검토
