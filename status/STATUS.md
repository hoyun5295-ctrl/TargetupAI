...
### 🔵 다음 세션 (예정)
> ⓪ **자동마케팅 [타겟확인] 구현(발송 대상 리스트)** — 설계 확정·DDL 0·구현 미착수. 기원=박성용 신고+고객사 AI 리스트 불신. ★0710 직원 회의 반영: 여정=보류(실시간성)·추천 카드 [타겟확인] 버튼(승인/거부 왼쪽)·조건 필드 동적 컬럼(FIELD_MAP 라벨)·표시 상한 100(LIMIT 100 단일 쿼리+클라 페이징=부하 상수화). 설계서만 읽고 바로 구현 가능하게 실측 계약 전부 명기: **SoT=[docs/superpowers/specs/2026-07-10-send-target-list-three-phase-design.md](../docs/superpowers/specs/2026-07-10-send-target-list-three-phase-design.md)** (§9 시작 체크리스트부터).
> ⓪-2 **비토 게이트웨이 API 직접연동 테스트** — 방화벽(invito→443) 해결·자격만 잔여(자비스 확인 요청 발신). 매뉴얼 v1.4 검토 완료·curl 테스트 세트 준비됨 = [[project_2026_0710_bito_api_direct_test]].
> ⓪-3 **싱크에이전트 원격 관리 전수 점검·개선** — 이새 매장명 문의로 결함 8종 실측 확정(매핑 빈 화면=전체 교체 함정·소스 컬럼 미상·ACK 부재·At-Most-Once 유실·진실 이원화 등). 이새 자체는 종결(현상 유지·2개월 뒤 타 업체 재연동). **SoT=[docs/superpowers/specs/2026-07-10-sync-agent-remote-admin-audit.md](../docs/superpowers/specs/2026-07-10-sync-agent-remote-admin-audit.md)** (§4 점검 A~G부터 — A=에이전트 소스 위치 확정 필요).
