...
### 자동마케팅 — 재설계 **전량 배포완료**(0804), 화면 실측 대기
...
> ⛔ 새 축 판정 기준은 §0이 소유(사건 = 여정 / 상태·변화 = 자동마케팅) — 여기 재서술 금지. DDL 전량 실행완료
> 잔여 = **남은 개발 7건 = [문서 §12](../docs/FEATURE-AUTOMARKETING.md)가 착수 원장**(1순위 매장 미배정 사유 화면 · 조건부 대기 = 상품 데이터 · 기존 구조 3건은 Harold 판단) + 화면 실측 7건(§12 하단). 운영을 막는 건 0
