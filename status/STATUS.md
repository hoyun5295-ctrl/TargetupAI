...
### 자사몰 연동 화면 재설계 — 설계 확정·착수 대기 (0809 신설)
> 호출어 **"자사몰 연동 재설계"** → **[설계서](../docs/2026-08-09-cdp-integration-redesign-design.md)가 전부 소유**(진단 §1 · 현황 실측 §2 · 원칙 §3 · 화면 흐름 §5 · 단계 §7 · 착수 전 확인 §9). 경위 = 0809 Harold 지시(연동은 핵심 진입점인데 초기 모델 정체)
> ⛔ **없는 것 만들지 않는다** — `install-status`가 실시간 수신 진단을 이미 반환(웹 탭 안쪽에 숨어 있을 뿐) / 독자는 마케팅 담당자 → 개발자용은 "개발자에게 보내기"로 위임 / 검증 탭 폐지·3단계에 녹임 / 0718 스플리팅 사고 회피(동적 import 신중)
> 잔여 = **Phase 0·1·2 완료 + Phase 3 부분완료(0810·배포 대기)** — bySource + 정규화 CT + 판정 훅 + 현황판 + 3단계 스테퍼(검증 탭 제거) + 계약 테스트 16건(결함 주입 검출 확인). 롤백 = `CDP_DASHBOARD_V2=false`. 다음 = **몰별 폼을 스테퍼 ① `connectSlot`으로 이관**(§7 Phase 3 잔여) → Phase 4. 착수 전 확인 §9 중 2·4·6·7·8 미해결
