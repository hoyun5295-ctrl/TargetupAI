...
> 상태 = 전량 배포완료(0726 `b396e2a5` · 0728~0731 · 0804 2차 · **0805 재오픈 2건·총 정산표·확정 축**). **팝빌 운영 전환 완료**(`★운영★` 로그 확인) — 기한 도래분이 실전 발행된다(문서 §5).
> 잔여 = **1순위 = `failed` 2건 수동 재시도**(크로커다일 25,248·유에스소프트 2,319,247 — 자동 재시도 안 되는 것 실측 확정) · 실측(재오픈 2건 회귀 · 총 정산표 · 실회사 드라이런 · 거래내역서 MMS 308,043건) · 특수정산 엑셀 업로드는 신규 테이블 필요(문서 §7).
