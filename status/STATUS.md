...
> **★0717 대시보드 성능 1~4차(★배포완료·실측 종결)** — 이새(13.7만) 지연: ①MySQL (app_etc1,sendreq_time) 커버링 90 ALTER **root 재실행으로 실적용**(어제 "실행 완료" 기록은 smsuser 권한 거부로 미반영이 진실) → sendTimes 3~8초→13~36ms ②stats·campaigns 집계 = 캠페인 실적재 라인 축 축소(27→24테이블·mysqlUnion 823→324ms) ③stats·enabled-fields SWR 캐시(재진입 SLOW 0 실측) — Codex 5라운드(수용 8·기각 5)+신규 테스트 14 ④customers 중복 유니크 DROP 2건 실행 완료(19:24 백업 후) **⑤(0717 밤·확장성 지시) 대시보드 2라우트를 getCampaignResultCounts CT 합류** — 확정 캠페인=PG 캐시·진행 중만 MySQL(발송결과·발송통계·관리자와 동일 소스 통일)+CT 내 SMS·카카오 병렬 복원. **잔여 = ⑤ 배포+이새 재측정 1회. 이후 후보(pg_stat_statements·ioredis timeout·롤업·프론트 fetch 병합)는 기록만.** 상세=[[project_2026_0717_dashboard_performance]].
