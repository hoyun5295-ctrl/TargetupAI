...
| 한 일(2세션 · 정산) | ⑧**[B-0916-5](BUGS.md)** 계정별 정산에서 테스트·스팸이 공통 장으로 가던 것 정정(`USER_SHEET_CHANNELS`에 `test`·`spam` · **0726 결정 개정** = [FEATURE-BILLING §2-21](../docs/FEATURE-BILLING.md) · DDL 0) ⑨**[B-0916-4](BUGS.md)** 수기 부가서비스 수량을 행 수 → 컬럼으로(`quantity`·`item_qty`·`item_label` **ALTER 3 대기**) · 1행 = 단가×수량 · 항목명이 유형 칸·항목줄 이름 · 추가 항목 행 수량 4칸 '-'(PDF와 통일) = [§2-22](../docs/FEATURE-BILLING.md) |
| 지금 상태 | backend 318파일 4,897건 · tsc 0(백·프) · **DM = DDL 0 / 정산 = ALTER 3 실행 대기** · **전량 배포 대기**(Harold push·배포) · 미실측 = 쪽 합성 실물·효과 실단말·스튜디오 삽입 1건·목록 대표 이미지 · **정산 2건(장 재분배·수기 1줄)** |
| 다음 한 수 | **정산 ALTER 3 → 배포** → 시세이도 9월 장 삭제·재발행(계정 장에 테스트·스팸) · 수기 항목 재입력 1줄 확인 → 화면 확인(시작 화면·블록 창·목록) → 쪽 합성 1건 실측 → 효과(책장·페이드) 폰 확인 → [0915 인계 §5-2](../docs/2026-09-15-session-handoff.md) 잔여 실측 |
