...
### ⏱ 세션 포인트 체크 — 2026-09-19~20 (접수 4건 수정 · **인증번호 알림톡 전환**)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 [BUGS](BUGS.md) B-0919-1~4 · 알림톡 = [전송자격인증 §4-J](../docs/2026-08-18-transmission-qualification-cert.md).

| 항목 | 상태 |
|---|---|
| 한 일(접수 4건) | 재오픈 2건(이메일 헤더·히어로 흰색 = 회사 킷 주색·강조색 `#ffffff` · 보정을 렌더 엔진 흰 면 두 자리로 · 어두운 면 3종은 제외) · 대행 메일 수신거부 중복 P0(고객 원문의 `무료 수신거부 :` 를 판정식이 못 알아봄 · 백엔드 CT + 프론트 미러) · 보류 충전 승인 실패(대기 목록 조회에 보류 컬럼 3개 누락) · DM 블록 창 오른쪽 부착(lg 이상) = [B-0919-1~4](BUGS.md) |
| 한 일(인증번호 알림톡) | 로그인 다중인증·발신번호 추가인증을 승인 템플릿으로 · 실패 시 같은 문구 SMS 전환 · CT `system-alimtalk.ts` · ENV 두 개(기본 꺼짐) = [전송자격인증 §4-J](../docs/2026-08-18-transmission-qualification-cert.md) |
| 지금 상태 | backend 336파일 5,098건 · tsc 0(백·프) · 프론트 운영 빌드 확인 · **DDL 0 · 배포완료 `2afc8b81`**(0919 16:24) · 알림톡 **두 종 켜짐**(`mfa_login,sender_auth` · `비토게이트웨이_1`) · **0920 실측 = 로그인·발신 인증 모두 카카오톡 도착** |
| 다음 한 수 | ①카카오 미사용 번호로 SMS 전환 1건 실측(인증 5분 창 안에 오는지) ②접수 4건 직원 실측(BUGS 각 항목 "실측" 줄) ③베이컨 20,000원 대기 요청 **거절로 닫기**(수기 충전 완료분 · 승인하면 이중 충전) |
| 미결(축 밖 · 기록만) | DM도 회사 킷 색을 보정 없이 읽는다 · 이메일 "버튼·강조색" 칸 기본 표시 ≠ 실제 색 · 헤더·히어로 블록 색 지정 칸(직원 제안) · 3자리 hex 흰 강조색 · 보류 충전 담당자 안내 문자(Harold 결정 대기) · 입금안내·충전확인 알림톡(이번 제외) · 0918 11:00 폴라초이스 본 발송 중복 여부 미확인 |
| 배운 것 | **대비 보정은 면 계약이다** — 흰 바탕 기준 보정을 토큰 전체에 걸면 어두운 면에서 반대로 사라진다(어두운 면 3종을 grep으로 전수하고 각각 출력 무변화를 테스트로 고정) · **교훈을 적고도 같은 결정을 하면 안 쓴 것과 같다**(0915 "장치가 래퍼에 있으면 두 번째 입구는 맨몸"을 적고 보정을 래퍼에만 넣어 이틀 뒤 재오픈) · **SCHEMA.md에 있어도 운영에 없을 수 있다**(`imc_template_status` = 조회 ERROR · 코드 주석에 이미 "ALTER 미실행 가능성"이 있었다) · **꺼짐을 로그로 남기지 않으면 "왜 안 되냐"를 못 가른다**(스위치 오프는 무로그 = 되돌리기 명령이 지운 것을 코드 결함으로 의심) |
