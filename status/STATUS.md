...
### 2-6. 수정 파일 제공 방식
- 대시보드처럼 큰 파일은 **"기존코드 → 새코드"** 형식으로 제공.
- 그 외에는 수정된 완성 파일 전체를 제공.
...
- **현재 목표:** AI 맞춤한줄 기능 구현 — 발송 확정 플로우 (타겟 조회 + 회신번호 + 예약 + 발송)
...
- **진행 상황:**
  ...
  - [x] AiCustomSendFlow.tsx Step 1~4 (전체 UI + 핸드폰 3열)
  - [x] 백엔드 parse-briefing API (프로모션 카드 + 타겟 조건 파싱)
  - [x] 백엔드 generate-custom API (개인화 맞춤 문안 생성)
  - [x] Step 3 타겟 조건 카드 UI (프로모션 카드 + 타겟 카드 2열)
  - [x] routes/ai.ts 코드 정리 (들여쓰기 + business_type 수정)
  - [x] 서버 배포 + Step 1→2→3→4 동작 확인
  - [ ] **발송 확정 모달** (AiCustomSendConfirmModal.tsx)
  - [ ] **백엔드 resolve-target API** (타겟 조건 → DB 필터 → 대상자 수)
  - [ ] **회신번호 선택** (단일 / 매장별 자동배정)
  - [ ] **예약시간 선택** (즉시 / 날짜시간)
  - [ ] **기존 campaigns.ts 발송 로직 연결**
  - [ ] 전체 통합 테스트 (실제 발송)

- **완료 기준 (DoD):**
  - [x] AI 추천발송 클릭 → 모달에서 "AI 한줄로" / "AI 맞춤한줄" 분기 동작
  - [x] AI 한줄로 선택 → 기존 AI 추천발송 플로우 그대로 진입
  - [x] AI 맞춤한줄 → 개인화 필드 선택 UI (고객사 DB 필드 기반 체크박스)
  - [x] AI 맞춤한줄 → 프로모션 브리핑 입력 (자연어 + 타겟 포함 유도)
  - [x] AI 맞춤한줄 → URL 입력 + 톤/분위기 선택
  - [x] AI 파싱 → 프로모션 카드 + 타겟 조건 카드 구조화 표시 + 수정 기능
  - [x] 확인 후 AI 최종 문안 생성 (개인화 변수 + 프로모션 정보 반영)
  - [x] SMS/LMS 바이트 제한 고려한 문안 생성 + 핸드폰 3열 UI
  - [ ] 발송 확정 → 타겟 조건 DB 필터 변환 + 수신거부 제거 + 대상자 수 표시
  - [ ] 회신번호 선택 (단일 선택 / 매장별 자동배정)
  - [ ] 예약시간 (즉시 / 예약)
  - [ ] 실제 발송 연결 (campaigns.ts 활용)
...
| D6 | 02-22 | 브리핑에서 타겟 조건도 함께 AI 파싱 (프로모션 + 타겟 동시 추출) | 타겟 별도 선택이면 직접타겟발송과 차별점 없음. 브리핑 하나로 타겟+문안 모두 AI 처리가 핵심 가치 |
| D7 | 02-22 | 회신번호는 발송 확정 모달에서 선택 (단일/매장별 2모드) | Step 1~4 UI를 깔끔하게 유지. 매장별 자동배정 = callback_numbers.store_code 매칭 |
...
| 날짜 | 완료 항목 |
|------|----------|
| 02-22 | AI 맞춤한줄 Step 1~4 완료: parseBriefing 타겟조건 파싱 + Step3 타겟카드 2열 UI + 핸드폰 3열 문안생성 + routes/ai.ts 정리 배포 |
