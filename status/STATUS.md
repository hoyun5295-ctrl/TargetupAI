...
> 잔여 = 배포(DDL 0 · [B-0905-3](BUGS.md) + 0905(4) 전문가 느낌 상향 = 설계서 §22-6 동반) → 이니스프리 재생성 육안 → Harold 결재 2(불변 18 · 취소선) + 네이버 새 키 게이트 → OPS ⑦ 실측 7(화면에서 29건 코드 적재 · 이메일 4건 → 제작 1건 근거 패널) → Codex 리뷰(설계서 §15 · Harold가 /codex:review 직접). `scratch/` = git 제외
