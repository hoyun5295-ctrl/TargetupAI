...
| 미결 | 알림톡 검수 동기화 3중 결함(5분 폴링이 IMC 4011을 **무기록**으로 건너뜀 · `syncTemplateStatuses`가 없는 컬럼 SELECT로 통째 skip · 그 함수가 목록의 `inspectionStatus`를 안 읽음) — 착수 판단 Harold · 한줄로에 수신번호 형식 검사 부재(0911 접수 1번 기원) |
| 배운 것 | **게이트는 발송이 실제로 쓰는 값을 그대로 받는다** — 개별 회신번호 모드·발신번호를 게이트가 따로 읽으면 계정 축 인증으로 특정 번호 발송이 통과한다(Codex 2R high ×2) · **한 컬럼이 두 질문에 답하면 하나가 조용히 틀린다** — `consumed_at`(썼거나 폐기)으로 세션을 판정하면 폐기 행이 인증 성공으로 읽힌다 · **빈 값은 통제가 강해지는 쪽으로 접는다** — 명단이 비면 전면 시행이 아니라 미시행 · **삭제할 수 없는 것은 원장이 참조하기 때문이다** — `campaigns`가 참조하는 발신프로필은 지울 수 없고 사용 중지가 답이다 |
