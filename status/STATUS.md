...
### ⏱ 세션 포인트 체크 — 2026-08-28(2·3) (이메일 캠페인 2건 · 대행발송 개선 2건 · 충전 승인 링크)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**(다음 세션이 자기 것으로 덮어쓴다). 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | ①**이메일 캠페인 닫기 버튼 유실**(임은지) — 헤더 입력에 `min-w-0`이 없어 flex가 못 줄고 `overflow-hidden`에 닫기부터 잘렸다. 줄어드는 쪽/줄지 않는 쪽 분리 + ESC 닫기(IME 가드 · dirty면 확인 모달) = [B-0828-3](BUGS.md) ②**히어로 높이 무시**(남지현) — 값은 정확히 실렸는데 셸 CSS `.em-hero{height:auto !important}`가 모바일 폭에서 전부 덮었다. 규칙 제거 + "덮는 쪽까지" 계약 = [B-0828-4](BUGS.md) |
| 한 일 (계속) | ③**대행발송 치환 미리보기**(서수란) — 승인 즉시 예약안은 불변 1·2 충돌로 불수용, 접수 상세에 상위 50건 실물 문장(발송과 같은 CT · `agency-send-preview.ts` 승격 + 고객·관리자 preview 라우트 각 1) + **진입 문 2곳**(고객 목록 「상세 보기」 · 슈퍼관리자 내역 상세) + **미리보기 별도 모달 공용화**(`AgencyPreviewModal` = 좌 목록 10건 페이징·우 폰 화면·누르면 바뀜 · portal 필수) = [B-0828-5](BUGS.md) ④**메일 MMS 개통**(서수란) — "SMS·LMS만" 축 폐기(차단 근거가 성립 안 함 실증). 별도 첨부 3장 · JPG 실체·300KB 규격이면 접수, 벗어나면 파일별 사유 반려(변환 0) · 양식 안내 개정+왕복 실측 = [B-0828-6](BUGS.md) ⑤**충전 승인 문자 링크**(Harold 지시 · 웹+에이전트) — 승인 대기 생성 시 ENV 번호 3개로 문자 + 무로그인 링크 1클릭 승인. **Codex 3R 종결 · 출하 가능.** 출하 기준 = 화면과 같은 문(같은 코어) + 재클릭 멱등(Harold 0829 확정). 1R·2R 전건 + 3R 내 변경분 3건 정정(IDN 화이트리스트 · 정리 트랜잭션 · 워커 기아), 기존 배관 성질 3건은 [B-0829-1](BUGS.md) 분리. 배포 = tp-push + ENV `CHARGE_APPROVAL_NOTIFY_PHONES` + build:safe 양쪽 + reload. 이력 = [충전 설계서 §16](../docs/2026-07-24-agent-prepaid-charge-design.md) |
| 지금 상태 | ①② = `83e303f9`로 커밋 완료 · ③④ = 코드완료·커밋 대기. **전량 DDL 0 · ENV 0 · 의존성 0.** 게이트 = BE·FE tsc 0 · vitest 217파일 3,351건(신설 34) · production 빌드 2회 · harness-check · 결함 주입 6회 검출 · Codex 면제(UI·조회·메일 게이트 = 돈·DDL 무관) |
| 다음 한 수 | **배포**(tp-push + build:safe 양쪽 + pm2 reload) 후 실측 = 이메일 편집기 X·ESC·히어로 높이 육안 · 미리보기 폰에 **이미지 실물**이 보이는지 · 슈퍼관리자 상세에 **진행 기록(승인 번호)** 이 보이는지 · **메일 MMS 규격 밖 반려 1건**(PNG). ⛔ 남은 미검증 = 본문에 드래그해 넣은 이미지(inline+cid)는 첨부로 안 잡힌다 = 요청서 이미지 파일명 칸도 비면 조용히 LMS로 나간다(방어는 하이웍스 실물 확인 뒤에) |
| 미결 | 이월 = 직전 세션 실측 전부(브랜드메시지 5회 · 대행발송 화면 육안 · 서수란 이메일 재시도) + 대행발송 §15~§18-12 실측 · [B-0823-1](BUGS.md)(**결함 순서 1순위**) · [B-0825-6](BUGS.md)·[B-0825-7](BUGS.md) · nginx sites-enabled 백업 2개 활성(한줄전단 축) · 아웃리치 ENV 대기 |
| 배운 것 | **"값이 실렸다"만 보는 계약은 그 값을 덮는 규칙을 못 잡는다** — 히어로 높이 파리티가 하루 전 그 자리에 있었는데 `toContain`이라 통과했다. 싣는 쪽과 덮는 쪽을 함께 고정(memory `feedback_contract_must_pin_the_overriding_rule`) · **접어 둔 축의 차단 근거는 다시 접수되면 근거부터 재검증** — "메일로는 실물 승인을 못 지킨다"는 입구 셋·배관 하나 구조에서 애초에 성립하지 않았다 · **flex 잘림은 분기마다 폭이 달라 한 상태만 보면 못 잡는다**(완성 전 2버튼에서만 재현) |
