...
> 상태 = 0805(2)까지 전량 배포완료 + DDL 실행완료(`47cae365` · 팝빌 운영 전환 완료). **0806 코드 완료·배포 대기** = 품목 거래일자 · 테스트베드 운영 재발행(`is_test` DDL·백필 완료) · 해지 목록 제외 · 정산 목록 검색·페이징.
> 잔여 = **1순위 = 테스트베드 12장 20,066,393원 국세청 미발행 — 기한 8/10**(문서 §7) · `failed` 2건 수동 재시도(문서 §5) · 실측(0805(2) 6건 · 0806분 · 재오픈 2건 · 총 정산표 · 거래내역서 MMS) · 특수정산 엑셀 = 문서 §7.
