...
### 대행발송 셀프 접수 · 0825 전량 배포완료(§15~§17-7) · 남은 것 = 운영 실측 · **다음 축 = 이메일 접수(챕터 2 · 브레인스토밍 먼저)**
> 호출어 **"대행발송"** → **[2026-08-22-agency-send-design.md](../docs/2026-08-22-agency-send-design.md)가 전부 소유**(30초 요약 §0 → 불변 §3 → 구조 §4 → 소유권 §12-F → 계약 §12-G · 당일 재검사 폐지 §14 · **화면 개편 §15 · 헤더 입구 §15-6 · 링크 승인 §16 · 요청서 원스텝 §17 · AI 매핑 §17-6 · 명단 미리보기 §17-7** · 다음 축·잔여 = §11)
> ⛔ 큐 직접 금지(직접발송 배관 위임) / **정산 축에 새 값 금지**(`send_type='direct'`) / 소유권 = `revision`·`lock_token`, **타임스탬프 토큰 금지** / **`final_test_at`은 통과 분기만**(§14-3) / 담당자 판정 `agencyManagerPhones` 한 벌 · 승인 토큰 헤더·fragment · 접수 쓰기 `createRequestCore` 하나
> 잔여 = **운영 실측만** = §16 링크 승인 4 · §17 요청서 원스텝 5 · §15 화면 개편 5 → §14 당일 접수 건 T-2h 검사 0회. 수용 위험 = §16-3·§17-3. 별도 과제 = §14-6 · 안건2(이메일 접수 · 착수 전 브레인스토밍 1회 추천). DDL 0
