...
> 구조·불변 원칙 15개·이관 절차·남은 것·이력을 **그 문서 하나가** 갖는다. 시점별 설계서 6종은 그 문서 §9가 안내한다. 여기 다시 쓰지 않는다.
> 상태 = 전량 배포완료(0726 `b396e2a5` · 0728~0731 · 0804 2차 · 0805 재오픈 2건·총 정산표·확정 축) · **팝빌 운영 전환 완료**(`★운영★`). **0805(2) 서수란 접수 5건 = 코드 완료·배포 대기**(문서 §8-3) — **DDL 2컬럼은 배포 직후**(`invoice_confirmations.confirmed_by_admin`·`confirm_note`, 그 전까지 [업체 확인 기록]만 `DB_MIGRATION_PENDING`).
> 잔여 = **1순위 = `failed` 2건 수동 재시도**(크로커다일 25,248·유에스소프트 2,319,247 — 자동 재시도 안 되는 것 실측 확정) · 실측(0805(2) 5건 · 재오픈 2건 회귀 · 총 정산표 · 실회사 드라이런 · 거래내역서 MMS 308,043건) · 특수정산 엑셀 업로드는 신규 테이블 필요(문서 §7).
