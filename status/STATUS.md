...
### 🚨🚨🚨 최우선 — 발송 파이프라인 전면 복구 (2026-02-26~)

> **현상:** 8차 전수점검 "통과" 보고 후에도 심각한 구조적 결함 7건 추가 발견. 1단계 코드 검증이 형식적(라인 존재만 확인)이었음이 판명.
> **Harold님 판단:** 현 상태로는 상용화 불가. 솔루션 전면 복구 필요.
> **근본 원인:** 발송 5개 경로에 변수 치환 로직이 제각각 분산 구현 — 한 곳 고치면 다른 곳에서 터지는 구조
> **상세 → BUGS.md 섹션 3 (구조적 결함 7건)**

#### 🔴 1덩어리 — 발송 파이프라인 통합 (최우선, 근본 원인 해결)

> 공통 치환 함수 `replaceVariables(template, customerData, fieldMappings)` 하나로 5개 경로 통합.
> 한 곳만 수정하면 5개 경로에 반영되는 구조로 전환.

| 순번 | 작업 | 관련 버그 | 상태 |
|------|------|----------|------|
| 1-0 | **공통 치환 함수 설계 → Harold님 컨펌** | 전체 | ⬜ |
| 1-1 | 직접타겟발송 customMessages 백엔드 수용 | S9-01 (🔴치명) | ⬜ |
| 1-2 | AI한줄로 스팸필터 실제 고객 데이터 치환 | S9-02 (🔴치명) | ⬜ |
| 1-3 | sent_at 경쟁 조건 해결 | S9-04 (🟠중요) | ⬜ |
| 1-4 | ai.ts subject 키 중복 정리 | S9-05 (🟡경미) | ⬜ |
| 1-5 | 스팸필터 테스트 라인 선택 통일 | S9-06 (🟡경미) | ⬜ |
| 1-6 | 5개 경로 공통 함수 교체 + 전수 검증 | 전체 | ⬜ |

#### 🟠 2덩어리 — 데이터/성능 안정성 (1덩어리 완료 후)

| 순번 | 작업 | 관련 버그 | 상태 |
|------|------|----------|------|
| 2-1 | 나이 필터 하드코딩 → 동적 연도 계산 | S9-03 (🟠중요) | ⬜ |
| 2-2 | results.ts 대량 캠페인 서버측 페이지네이션 | GPT지적 (🟠중요) | ⬜ |

#### 🟡 3덩어리 — UI/UX 품질 (2덩어리 완료 후)
...
### 🚨 최우선 — 발송 파이프라인 전면 복구
- [ ] **발송 5개 경로 공통 치환 함수 통합** — 구조적 결함 근본 해결 (→ CURRENT_TASK 1덩어리)
- [ ] **구조적 결함 7건 수정** — 직접타겟 개인화 깨짐, 스팸필터 치환 등 (→ `BUGS.md` 섹션 3)
- [ ] **results.ts 대량 캠페인 서버측 페이지네이션** — 30~40만건 고객사 대응
...
| R15 | 발송 5개 경로 치환 로직 분산 → 수정할 때마다 다른 경로에서 재발 | 5 | 5 | 25 | 🔴 진행중: 공통 치환 함수 통합으로 구조적 해결 (D32). 현재 가장 높은 리스크 |
