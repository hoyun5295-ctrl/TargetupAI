...
| 지금 상태 | **접수 2건 배포 완료**(게이트웨이 `f9f4f0d` · 한줄로 `0df27d96`). 게이트웨이 해시 대조 일치 · **8월 재적재 실측 `K` 1223/1215 → 1175/1175** · pm2 교체 확인. **후속 3건은 코드 완료·배포 대기**(vitest 227파일 3,625건 · FE 빌드 통과) |
| 다음 한 수 | **화면 실측 2건** = ①랩디 8월 정산 차단 소멸 + 공급가액 **20,079원** ②임은지 「아디다스(임은지6)」 CTA 가로(Ctrl+F5). 그리고 후속 3건 배포 |
...
### AI 영업 아웃리치 v2 — 코드 완료·**배포 대기(ENV만 남음 · DDL 2는 0828 실행완료)** · **0826 스마트스토어 축 폐기(출구 IP 429) → 공식몰 행사 추출 정교화로 전환·코드 완료**
...
> 잔여 = **배포**(ENV 7키 · DDL은 0828 완료 · 절차 [OPS §2-2-E](OPS.md)) → 실측 1건 → 추출기 현행 대비 인용 건수 비교(§17-5). 스마트스토어 축 폐기 = 검색 API 404 + 크롤 429 양쪽 차단 · [B-0826-1](BUGS.md) 종결 · [B-0826-2](BUGS.md) 잔존
