...
### ⏱ 세션 포인트 체크 — 2026-08-28 (브랜드메시지 5종 개통 · 대행발송 취소 축 · 로그 소음)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**(다음 세션이 자기 것으로 덮어쓴다). 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | ①**브랜드메시지 자유형 5종 개통**(접수 = 「브랜드메시지 화면구성.docx」) — 차단 사유였던 "조립 예시 미확보"가 `attachment_method.pdf` §3.4·§5.3에 있어 해소. 규격 CT 확장(캐러셀 3단 신설·아이템 5→4·버튼명/부가정보/상품명 축 신설) · 조립기(`buildCarouselJson` 되살림 · 본문 필수를 유형 기준으로 · `BUBBLE_TYPE_OPENED` 원장화) · 등록 화면 전면 재작성(27개 누락분 · 모달→전체화면 · 규격 숫자를 프론트 사본에서 읽음). **재작성 중 발견 = 옛 화면이 규격 밖 키를 보내 5종이 발송 검사를 통과할 수 없던 것 3건**(`video.url`·커머스 할인율·아이템 `url_mobile`). 게이트웨이 3건은 인계서로 분리했고 **그쪽도 완료**(`e9004a7` · 결과코드 141건 등재) |
| 한 일 (계속) | ②**대행발송 취소 불능**(서수란 접수) — 캠페인이 생성 51ms 뒤 `completed`가 되는데 상태 게이트가 `scheduled`·`draft`만 통과시켜 **예약이 잡힌 접수를 아무도 취소할 수 없었다.** `queueOnly` 신설(큐만 중화·상태 유지 = 청구 축 보존) · `alreadySent`(막을 것 없으면 되돌리고 사실 안내) · **발송 완료 상태 `sent` 신설**(DDL 1) ③**이메일 회신 침묵** — 0826에 일일 상한을 ENV 무제한으로 풀 때 `REPLY_RATE_PER_HOUR=5`만 하드코딩으로 남아 있었다. ENV 이관 + 상한 도달 1회 안내 ④**시스템 알림 문구**(반말·UUID·줄뭉침) — 알림 CT에 구조화 입력 신설 + 내부 식별자 자동 제거 ⑤**로그 소음 3건**(아웃리치 DDL·CORS·PG 재시도) |
| 지금 상태 | **전량 배포완료 · DDL 2건 실행완료**(대행발송 `sent` · 아웃리치 2테이블). 게이트 = BE·FE tsc 0 · vitest 215파일 3,317건 · production 빌드 · harness-check · **결함 주입 5회 검출**. 운영 실측으로 닫힘 확정 = 대행발송 3건이 `cancelling`·`queued` → **`sent`**(다음 tick 지나도 고정) · CORS 경고 소멸 |
| 다음 한 수 | **브랜드메시지 유형별 실측 5회**(유형 하나 = 발송 1건 → REPORT `0000` → 그 유형만 원장에 추가 · ⛔ 한 번에 5종 금지) + **화면 육안**(대행발송 3건이 「발송 완료」로 보이고 취소 버튼 사라짐 · 브랜드 등록 화면 8종) + **서수란 이메일 재시도** |
| 미결 | 이월 = 대행발송 §15~§18-12 실측 · [B-0823-1](BUGS.md)(워커가 의존 장애를 판정으로 읽는다 · **결함 순서 1순위**) · [B-0825-6](BUGS.md)·[B-0825-7](BUGS.md). **신규 = nginx `sites-enabled`에 백업 파일 2개가 활성**(`nginx -t`가 conflicting server name 3건 경고 · 지금 장애는 없으나 설정을 고칠 때 어느 파일이 먹는지 모르는 상태 · 한줄전단 축이라 미착수) · 아웃리치 기능 개통은 ENV 대기 |
| 배운 것 | **적재 완료는 "더 나가지 않는다"가 아니다** — 큐가 예약 시각을 들고 있으면 캠페인이 끝나도 발송은 남는다. 그래서 `inspectAttemptCampaign`을 고치지 않았다(고쳤으면 취소 건의 큐 회수를 놓쳤다) · **새 종결 상태를 만들면 그것을 되돌리는 자리를 함께 찾아야 한다** — `sent` 신설 직후 대조 워커가 매 tick 되돌렸고, "그 분기는 문제가 아니다"라고 본 것이 원인이었다(`queued`일 때 안 탄다는 것만 보고 새 상태를 만들면 탄다는 것을 못 봤다) · **로그 문자열을 변수로 빼면 판정 규칙이 바뀐다**(`console.*` 안쪽만 내부 로그로 제외 · push 훅이 배포 전에 잡았다) |
