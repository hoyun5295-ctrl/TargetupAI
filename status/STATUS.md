...
> **★2026-07-17 인앱 재작업** — ① 웹 중앙정렬 **원인 확정·코드완료(배포·실측 잔여)**: 실렌더 근본=팝폰 웹 v0.3.9 고정 로드 + 0717 SDK 빌드가 v0.3.11에만 복사(→4폴더 동기화) / 미리보기=Overlay(BlockPreview 경로) 미적용(→cardBase SDK 미러) / 데이터는 정상(실서버 응답 text_align 동봉 실측). 상세=BUGS §B-0717-1. ② 인앱 아키텍처 재설계(편집기가 앱 지원범위 안내/게이팅 + SDK 릴리즈=전 서빙 폴더 동기화 자동화) = 다음 작업. 팝폰앱 수정분=Harold 직접 재빌드·커밋 예정(0718)·비토 배제. 상세=[[project_2026_0717_inapp_debug_session_incomplete]].
