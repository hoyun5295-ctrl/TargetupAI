...
### 🟢 2026-07-15 (2) — AI Operator 소개 v4 스펙터클 (코드완료 — 배포 대기)
> S1 칩 대형화(카드형+샤인+역회전 링+7 글로우) / S2 "마케터, 단 한 명" 대형 강조 / S3 여정 = 문자 기본·STEP 1~3·구매 자동 종료 / **자사몰 연동·싱크 에이전트 2장 분리 신설(13장)** — 자사몰(카페24·아임웹·메이크샵·고도몰·네이버 전부 지원·구매·장바구니·회원 자동 수집) / 싱크(설치+주기·읽기전용) / v4.1 핵심 문장 찬란 강조(emPop 플래시·.note 스포트라이트 5·hot 칩 4·approve·형광펜) / 전역 이펙트 상향. **진입점 3곳 ?v=4 동시 갱신.** 검증 = 슬라이드 12·JS 구문·tsc 0. 상세=[[project_2026_0713_operator_intro_effects]]. **잔여 = 배포(tp-push+build:safe frontend) + Harold 시각 확인.**

### 🟢 2026-07-15 — 모바일 DM 한글 주소(hlj.kr/반짝세일_07) + 추적 2원화 (코드완료 — 배포·DDL 대기)
> **전 상세=[[project_2026_0715_dm_korean_alias]].** 발행 DM 한글 별칭(무료·DM당 1개·비파괴) + 추적 2원화(개인화/공용 링크 탭·?src 유입태그→entry_source 격리·public-link-stats·CT-40 hlj.kr 재단축 skip). 검증 tsc 0·vitest 615·금지 grep 0. **잔여 = DDL 3건(서버 psql) + 배포(tp-push+build:safe frontend+pm2) + 실측(nginx 한글 경로 curl / 별칭 왕복 / 단말 문자 링크 인식=성패 축·직원) + Codex 리뷰 합류(본 세션 플러그인 미로드).**
...
> **SoT(전 상세)=[specs/2026-07-13-email-design-3-product-link-design.md](../specs/2026-07-13-email-design-3-product-link-design.md) · [[project_2026_0713_email_design_3]].** 제품 링크 유실 정정+og 채움 / 렌더러 3.0(완전 문서·VML 버튼·구도 5·배경면·다크 셸·서체 폴백 6·프리헤더) / email_campaigns.design jsonb(DDL 1) + 편집기(테마 8·픽커·골든 12) / Codex 적대 5R 전건 정정(SSRF·VML 추적·다크 미가독·쓰기 원자성) / 0714 후속 2(상품 카드 등고·버튼 명시색). 검증 tsc 0·vitest 492·금지 0. **잔여 = ①`ALTER TABLE email_campaigns ADD COLUMN IF NOT EXISTS design jsonb;` → 배포 ②실측(직원): Gmail 라이트/다크·모바일·네이버 / 행사 원문→매핑 / 테마 8 전환·발송 왕복.**
