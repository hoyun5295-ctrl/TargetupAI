...
### ⏱ 세션 포인트 체크 — 2026-09-12 (전송자격인증 4.1·4.2·3.5 + 직원 접수 4건)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | ①**3.5 발신 인증을 발송 경로에 실제 배선**([§4-K](../docs/2026-08-18-transmission-qualification-cert.md)) — 8월 제출본은 "9/1 시행"이라 적었지만 팝업만 있고 발송 앞에 안 붙어 있었다. 시범 명단 3계정 · 신규 테이블 `sender_auth_challenges` · 발송 3경로 게이트 · 인증 팝업 훅. **13:31 운영 실측 통과** ②**4.2 서버 로그 1년 보관**([§4-L](../docs/2026-08-18-transmission-qualification-cert.md)) — 앱 로그 회전 신설(없었다 · 단일 파일 827MB) · 웹서버 14일→400일 · 저널 12G·1년 ③직원 접수 **2·3·4번 코드 완료**(위임장 서명란 · 080 통화료 다중 줄 · 발신프로필 사용 중지) ④**알림톡 검수 동기화 3중 결함 종결** — 단건 조회 무기록 건너뜀 2곳 · 없는 컬럼으로 30분 안전망이 통째로 멎던 것 · 목록의 `inspectionStatus` 미독 |
| 지금 상태 | 인증 = 4.1·4.2 **반려 항목 중 개발로 닫을 수 있는 것 전부 종결**(제출 캡처 11장 = `~/Downloads`). 남은 하나는 회원가입 로그이고 역발급이라 담당 직원 설명으로 정리. 접수 = 1번 회신만 남음 · 2·3·4번 **배포 대기**(4번 인덱스 교체는 이미 반영) |
| 다음 한 수 | `tp-push` 후 배포 → 슈퍼 화면에서 **한빛주택종합관리 [사용 중지] → 프로필 재등록** → 감사 로그 확인. 그 뒤 **[0911 인계](../docs/2026-09-11-session-handoff.md) §2**(이모지 브랜드 1건 실측 → P3-4 → P3-2 → P3-3)로 복귀 |
| 미결 | **`HREJ`(중계사 내부 반려)가 담당자 알림 선택 조건에 없다**([alimtalk-jobs.ts:245](../packages/backend/src/utils/alimtalk-jobs.ts)) — 안전망이 살아나 HREJ가 제대로 찍히면 화면엔 반려인데 문자는 안 가는 건이 는다. 조건에 넣으면 **과거 HREJ 행에 문자가 몰릴 수 있어** 건수 확인이 선행. 착수 판단 Harold · 한줄로에 수신번호 형식 검사 부재(0911 접수 1번 기원) |
| 배운 것 | **안전망은 주인이 아니다** — 목록 동기화가 `status`를 쓰되 진행 중 행만, 그것도 조회 시점 조건을 UPDATE에 걸어 5분 폴링이 저장한 승인을 덮지 않는다(Codex high) · **한 컬럼이 없다고 함수를 통째로 멈추면 로그는 축소 보고가 된다** — "활성상태 동기화 skip"이라 적혔지만 실제로는 검수상태 동기화까지 멎어 있었다 · **게이트는 발송이 실제로 쓰는 값을 그대로 받는다** — 개별 회신번호 모드·발신번호를 게이트가 따로 읽으면 계정 축 인증으로 특정 번호 발송이 통과한다(Codex 2R high ×2) · **한 컬럼이 두 질문에 답하면 하나가 조용히 틀린다** — `consumed_at`(썼거나 폐기)으로 세션을 판정하면 폐기 행이 인증 성공으로 읽힌다 · **빈 값은 통제가 강해지는 쪽으로 접는다** — 명단이 비면 전면 시행이 아니라 미시행 · **삭제할 수 없는 것은 원장이 참조하기 때문이다** — `campaigns`가 참조하는 발신프로필은 지울 수 없고 사용 중지가 답이다 |
...
> ⑮ **브랜드메시지 · 게이트웨이 발송 속도 — ★0906 게이트웨이 전량 배포완료(057·058·059 + TPS 평활화)** — 호출어 **"친구 단가 축" / "발송 속도 축"** → [FEATURE-GW-BRAND-MESSAGE.md](../docs/bito-gateway/FEATURE-GW-BRAND-MESSAGE.md) §9. 다음 = `212` 실측 검증 · 한줄로 쪽 4칸 · Agent send_concurrency
