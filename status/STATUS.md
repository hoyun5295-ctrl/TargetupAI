...
| 다음 한 수 | 배포(OPS §2-2: pull → backend `build:safe` → frontend `build:safe` → `pm2 reload targetup-backend` · npm install 불필요) → 실측 5: ①새 양식 내려받아 화면 접수 1건 ②이메일 통합 1파일 접수 1건 ③company_user 계정으로 본인 것만 보이는지 + 관리자 전체·접수 계정 표시 ④슈퍼관리자 대행발송 내역 탭 육안 ⑤내역 탭에서 시험 접수 1건 운영 취소(담당자 취소 문자 수신까지) |
...
### AI 영업 아웃리치 v2 — 0824 코드 전량 완료·Codex 종결(high 0)·배포 대기
> 호출어 **"영업 아웃리치"** → **[FEATURE-SALES-OUTREACH.md](../docs/FEATURE-SALES-OUTREACH.md)가 전부 소유**(정의 §1 → **불변 원칙 17개 §2(필독)** → 파일별 소유 §3 → ENV·테이블 §4 → 이력·뒤집힌 판단 §5). 시점 근거 = [0731 설계서 §15](../docs/2026-07-31-ai-sales-outreach-design.md)(현행 설계 원문·브레인스토밍 수렴·Codex 이력)
> ⛔ 요지만: 발송 3중 잠금 / **DB 선점 없이 SMTP 금지** · 기록 0행이면 성공으로 답하지 않는다 / 산출물 INSERT는 `insertAssetOwned`만 / fail-closed 게이트(super_admin 우회 0) / 크롤은 가드 경로만 / 자동 재시도·발송 0 — **전문은 기능 문서 §2**
> 잔여 = **배포**(ENV 6종 + DDL 2테이블 = §15-8) → 실측 1건(등록→확인→제작→자사 메일 도착) → **샘플 세트 15건 수령**(업종당 5 · 직원 2명 · 오늘 오후 제출 예정 · 오면 `sales-outreach-style.ts` 교체) · 별건 = [B-0824-2·B-0824-3](BUGS.md) · medium 부채 2건 = §15-9-1
