...
> ⑮ **브랜드메시지 — 0814(3) 원인 확정(우리 코드 아니었다)** — 호출어 **"브랜드메시지"** → [[project_2026_0729_brand_message_billing]] 상단이 소유. 게이트웨이 개발은 자비스 몫. 우리 잔여 = **능력 기반 라우팅**(brand-message.ts:618·743)
> ⑯ **자비스 적재 규격서 출고** — [규격서](../docs/2026-08-14-bito-gateway-result-contract.md)가 전부 소유. 다음 = 회신 6항목 → 우리 준비 3건(계정·매핑·단가)
