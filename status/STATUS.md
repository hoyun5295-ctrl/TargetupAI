...
### 0731 정산 누락 정정(레거시 예약 직접발송) — 코드완료·배포대기(Codex 4R)
> SoT = 기억 [[project_2026_0731_billing_legacy_direct_leak]] · 다음 = 배포 → 07-01~07-31 미리보기 금액 **불변** 확인(7월 누락 성공 0건이므로 증가분 0이 정답)
...
> 잔여 = 배포·실측 · 별건 3(아이디룩 미발송 7,701건 · 레거시 예약 1,653건 전량 실패 · 여정 후불 미청구 통로) · 대기 게이트 완성(채널별 종결 판정 정규화)
