...
### 도움말 봇 · 기능 카탈로그 — 0822 1단계 코드완료 · 배포 대기(DDL 1건)
> 호출어 **"도움말 봇"** → [FEATURE-HELP-CATALOG.md](../docs/FEATURE-HELP-CATALOG.md)가 전부 소유(불변 §3 · 구조 §4 · 게이트 §5 · 1단계 §6 · 인수 조건 §6-1 · 상태 §9). 회의 정의 개정 = COLLAB §1
> ⛔ 정의에 요금제 이름·크레딧 숫자·버튼 라벨을 문자열로 적지 않는다 / 매칭 실패면 모델 호출 0 / 봇 노출 = 요금제 사용 회사만(서버 판정) / 발송 이력 판정에 MySQL 금지
> 잔여 = DDL `help_questions` → §6-1 인수 3건(백지 계정 리허설 1일 · 375px 실측 · 라벨 1:1 완료) · 학습데이터 (나) Harold 답 대기
