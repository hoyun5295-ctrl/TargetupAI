...
### 0801 여정 재설계 — §11-1~4 코드완료·push(`83bd1b02`) + **§11-4 DDL 실행완료**(0802 실측)
> SoT [여정 재설계](docs/2026-08-01-journey-redesign-design.md) **§11-D(§11-5 1차 — 레지스트리·신규 2종) → §11-C → §11-A·§11-B** · 기억 [[project_2026_0801_journey_redesign]] · 호출어 **여정 재설계 이어가자** · 다음 = Codex 판정 반영 → 배포+DDL 2건(§11-C-5는 완료 / §11-D-3 CHECK) → 실측 → §11-D-4 잔여
> ⛔ **정답표를 안 갖는다**(§2-3) / **진실을 복사하지 않는다**(§11-C-0) / **문은 회사마다 하나**(자사몰 현역이면 원장 잠금) / **커서를 DB 밖(JS Date)으로 내보내지 않는다** — µs 절사=중복 발송(F1) / 익명 소급(N5)·커서 상한(C6)은 착수 5번 몫
> 잔여 = 실측 5건(구매 여정 활성화 필요 — 현재 전부 비활성) · §11-5~7 · Harold 확인 1건(예약 취소·노쇼 구분) · 별건 2(journey-stats 'order' 축 · schema template TIMESTAMPTZ 불일치)
