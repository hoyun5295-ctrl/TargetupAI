...
### 🟡 2026-07-12 (2) — 모바일 DM 종합 연구 + 강화 D-1~D-4 (코드완료 — 배포·실측 대기)
> 전수 연구(발행·발송·뷰어·추적·이벤트 라우트 70개+토큰·추적 CT 정독 — 코어 견고 확인, 실결함 2건뿐) 후 일괄 구현: **D-1 발행비 정합** — 발행비(멱등키 dm-publish:dmId) 미납+실발송 이력(dm_recipient_tokens) 없음+크레딧제 적용 회사면 send-to-target 402 PUBLISH_FEE_REQUIRED → 프론트 CreditConfirmModal 확인 → confirmPublishFee 재요청 시 서버 인라인 확정(/publish와 동일 멱등키 — Codex 402 루프 High 지적으로 /publish 왕복 재시도 폐기 재설계). 과거 실발송 DM 소급 면제·무과금 회사 면제·판정 실패=발송 우선. 테스트 발행(무과금)·API 직행 우회 차단. **D-2 야간 광고 가드 공통화** — createDirectSendCampaign(직접발송·DM·자율발송 단일 길목) adEnabled+발송 시각(즉시/예약) KST 창(SEND_HOURS 8~21) 밖=NIGHT_AD_RESTRICTED 400 + DM 발송 모달 사전 안내. ★직접발송 야간 광고도 이제 차단(정책 변화 — 운영 공지 검토 필요). **D-3 재타겟 1클릭** — 미열람 재발송을 세그먼트 4종(미열람/열람·무반응/클릭/응모)으로 일반화(백엔드 무변경 — resendCustomerIds 재사용·자격/수신거부 서버 재적용). **D-4** 발송 추적 구매 전환 소스 라벨(매장·ERP purchases ≠ 자사몰 cdp 정직 표기). Codex 적대 2라운드(402 루프 정정→PASS: 멱등·플래그 결합·순서 정합 확인). 검증 BE/FE tsc0·vitest 433·금지패턴0. DDL 없음. **잔여 = 배포+실측 3건(발행비 402→확인→발송 / 야간 광고 400 / 세그먼트 재발송).** 상세 [[project_2026_0712_dm_reinforcement]]

### 🟢 2026-07-12 — 자동마케팅 종합 연구 + 일괄 강화 C-1~C-5 (★배포완료 2026-07-12 — Harold 선언 / 실측 5건 = 직원 테스트)
> 전수 연구(빈틈 10·견고 확인·업그레이드 6) 보고 후 Harold "전체 다, C-1부터 한번에 끝까지" 지시로 일괄 구현: **C-1 안전** ①중지·보관 시 예약 자율 발송 취소(pause/archive 캐스케이드 + 발송 직전 오퍼레이터 상태 게이트 2중) ②야간(21~08시) 광고 발송 차단(저장 가드+fixed 클램프+발송 패스 게이트+야간 승인=다음 9시 예약 전환·[정지] 취소 안내) **C-2 거짓 설정 정리** UI "발송 주기" select 제거·예산 임계 알림 실동작(decideBudgetAlert 교차 판정=마커 없이 멱등)·죽은 컬럼 5종 코드 참조 제거·죽은 admin-confirm 라우트 제거·부분 patch 예산 NULL 함정 봉합(현재값 선조회) **C-3 성과 루프** 전환(구매) 자동 귀속(발송+7일 창·다음 발송 상한=이중 귀속 차단·신규 conversion_attributed_at 멱등)+회고 문자 구매·매출 라인(LEAST 상한)+런처 ROI 상설 카드 **C-4** 타겟 축 일반 개방(설정 모달 select+PUT, 별도 UPDATE 42703 skip) **C-5** 회고·D-2·만료임박·귀속을 예측 크레딧 루프에서 독립(AI 브리핑만 잔류). C-6(알림톡 전환)=카카오 템플릿 검수 선행이라 제외. Codex 적대 2라운드(4건 정정→PASS). 검증 BE tsc0·vitest 433·FE tsc0·금지패턴0. **DDL 완료(2026-07-12 — conversion_attributed_at ALTER 실측·target_hint 1행 존재 확인) → 잔여 = 배포+실측 5건.** 상세 [[project_2026_0712_automarketing_full_reinforcement]]

### 🟢 2026-07-11 (2) — 여정 전수점검 일괄 강화 9건 + Codex 적대 정정 (★배포완료 2026-07-11 커밋 0db38480 — Harold 선언 / 운영 실측 = Harold)
> 전수점검(빈틈4·강화7·구조3) 보고 후 Harold "한번에 끝까지" 지시로 일괄 구현: ①거짓 설정 2개 제거(Settings·Admin "일일 한도·중복 방지" — 발송 소비처 0 실측, 피로도 보호 안내 대체) ②여정 [타겟확인](발송 추출과 동일 함수 LIMIT 100·시점 정직 라벨·공용 TargetRecipientsModal) ③목표 종류 goal_kind 구매/클릭/방문(실행기 분기·운영 중 변경 허용) ④wait-until-event(이벤트 대기·waiting_event 앵커 로그·15분 재폴링·타임아웃 진행) ⑤진짜 분기 not_met_goto(condition 미충족=전방 점프 — 기존 조건 3종과 결합해 클릭 분기 완성) ⑥활성 중 문안 수정(문안 키만+명시 opt-in → 새 snapshot+pretest dedup 삭제=자동 재검사. 2h 이내 발송 한계 모달 고지) ⑦홀드아웃 대조군 holdout_pct 0~30%(진입 4지점 확률 배정 + **멤버십 영속**[Codex HIGH 정정 — 재진입·앵커 스텝에서도 홀드아웃 유지]·stats 증분 비교 카드·카드 뱃지) ⑧send-time 개인화(반응 최빈 시간대·최근 90일·최소 3건·09~20 클램프) ⑨알림톡 폴백=기배선 확인. **신규 컬럼 6개 전부 발송 본류 SELECT 밖 지연 조회+42703 가드 — DDL 전에도 발송 tick 무영향.** Codex 5건 판정: 홀드아웃 오염=정정 / 2h 창=한계 고지 / 다중 인스턴스 2건=PM2 단일 전제(확인 잔여) / 1건 무조치. 검증 BE/FE tsc0·vitest 52파일 428·options verify 21. **잔여 = 실측 시나리오 5건(직원 테스트 예정 — 클릭목표/분기/이벤트대기/홀드아웃/활성수정).** DDL 6건 = **적용 완료(2026-07-11 information_schema 6행 실측)** · PM2 인스턴스 = **fork 단일 확인(2026-07-11)** — SKIP LOCKED 클레임 별도 과제 불요, Codex 다중 인스턴스 전제 지적 2건 종결. SoT=[specs/2026-07-11-journey-full-reinforcement.md](../docs/superpowers/specs/2026-07-11-journey-full-reinforcement.md) · 상세 [[project_2026_0711_journey_full_reinforcement]]

### 🟢 2026-07-11 — 직원 디버깅 2건 + AI Operator 소개 페이지 전면 개편 + 폐기플랜 이관 기록 (★배포완료 커밋 ece2f4a8)
