...
### RCS 연동 — 설계 확정 + **T0(즉시 정정 3건) 코드완료·미배포** (0817 신설)
> 호출어 **"RCS 연동"** → **[2026-08-17-rcs-integration-design.md](../docs/2026-08-17-rcs-integration-design.md)가 전부 소유**(확정 사실 §1 · 티켓 순서 §7 · 실측 게이트 §8 · T0 결과 §9). 한줄로 = RBC 대행사 / 브랜드 = 고객사 소유(Harold 확정). 발송 = 게이트웨이 `gemtek_rcs` → 젬텍 NGS RCS
> ⛔ **성공 리포트 코드 실물 없이 결과 판정 코드를 쓰지 마라**(미등재면 도달 성공 전량이 실패환불 + 대체문자 중복 발송) / **화이트리스트에 채널을 먼저 넣지 마라**(적재 경로 없는 채널 = 차감만 남는다) / 대체발송은 게이트웨이 기존 축 재사용(무리포트 타이머 신설 금지 — 중복 수신)
> 잔여 = **T0 배포 + [§9-1 SQL](../docs/2026-08-17-rcs-integration-design.md) 1건** → 젬텍 계정·IP·에러코드표 수령(Harold) → T1~T5(P0). 화면·에이전트 경로는 P2. DDL 0
