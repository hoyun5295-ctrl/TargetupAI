...
> **★0727 §5-4 충전 요청(고객사) 코드완료 — DDL 대기.** 고객사 `/manage` **충전 요청** 탭(에이전트·겸용 회사만) → 슈퍼 접수함 **[충전 폼에 담기] 1클릭** → §5-3 그대로 실행 → **반영(`Y`) 확인 후에만 완료 전이**. `deposit_requests` 재사용은 폐기(그 승인은 웹 `balance`를 올린다 — 지갑이 다르다). 신규 CT `utils/agent-charge-orders.ts`(계약 테스트 24)·신규 테이블 `agent_charge_orders`. DDL·불변규칙 = SoT §5-4.
> **충전·잔액 축 잔여**: `agent_charge_orders` DDL 실행 → 서수란 선불 발송ID·단가 자료 수령 → `billing_type='prepaid'` 지정(283행 전부 postpaid = **지금은 충전 등록도 요청 탭도 열리지 않는다**) → 실업체 왕복 1건 실측 / §5-5 고아 대조 워커 / 컷오버(143 SeqNo>7042 백필 — **RsApplyFlag 'Y' 그대로**, 'N'이면 이중 증액) + 강문희 143 연동 종료 통지 / §8-2·8-8 실측.
> **★0726 실측 확정(SoT §11)**: 62 충전 원장이 **0707 dump 이후 유입 0**(0707 초과 행 = 우리 테스트 2건뿐). 런소프트 7월 통장 3,100만 중 700만만 보임. 위 143 백필이 바로 이 건이다. **백필 시 SeqNo 그대로 이관 금지**(62의 7044·7045는 우리 행 — 143과 번호 의미가 다름). 발송ID 283행 매핑·노출은 정상(B129·C62·D92)이나 `prepaid` 지정 **0건**이라 충전 등록 자체가 거부되는 상태.
