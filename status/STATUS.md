...
> **⑥ 카페24 공식 앱 연동 개통 (★실측 완주 — 금액 fix만 미배포)**: 개발자센터 앱 "한줄로AI"(client iQ5aZ…) 설정 완료(권한 9종+웹훅 5종: 90023/25/26/29/84) + ENV 4건 + 수신부 실측 정정(X-API-Key+event_no 매핑, 미연동 몰 200 무시 — 실패 누적 시 수신 자동차단 정책 대응) + 연동 화면 공식앱 기본(자체앱=접이식). 테스트몰 gyunoo83 실측: OAuth 토큰 저장 + cart.added/order.created processed + cdp_events cart_add·purchase(고객 매칭). 발견 결함 fix: 금액 "0.00" 문자열 truthy로 `||` fallback 미동작 → `firstPositiveAmount` CT(normalize) 신설, cafe24+naver 적용 / items는 extra_info로 옴. 금액 fix 배포+입금확인(90025) 실측 완료(purchase 12,500 갱신 — 웹훅 3종 실전 검증). 잔여: 회원 웹훅=개인정보(Privacy) 권한 별도 절차(운영자 화면에 없음 — 문의 필요) / 앱 실행 랜딩(심사 대비) / scripttags 자동 주입 / 판매정보+심사 신청 — **완주 설계서 작성 완료(다음 세션 = 심사요청 클릭까지): docs/superpowers/specs/2026-07-03-cafe24-review-readiness-design.md** (결정 대기 1건: FREE 계정 카페24 연동 허용 — 심사위원 동선). 상세 [[project_2026_0703_cafe24_app_store_integration]].
> **⑦ isae 싱크에이전트 완결 (★배포·백필 완료)**: 에이전트 1.5.7 원격 교체(서팀장 원격 1회) + custom 매핑 15/15(update_config, custom_14=최종접속일시 가설 적중) + 클린 재적재(고객 137,267/구매 1,854,706, 중복 370만 정리) + 구매이력 화면 **두 앱**(app.hanjul.ai CustomersTab·hanjul.ai/manage ManageCustomersTab 사본 2곳) + 고객목록 등록일 fix + custom 날짜 ISO 표시 정정(`formatIfIsoDate` CT) + 고객DB 현황 성능(CT-18 타입감지 조기종료+결과캐시 5분·degraded 가드+filter-options 단일스캔) + **구매요약 집계 CT**(customer-purchase-aggregates: 배치 훅 재계산, 웹훅 `revenue_applied` 이중진실 가드, NULLS LAST — 백필 126,099명 완료). **미해결 1건 = updater 자기교체 결함**(update.bat이 자기 작업 job 종료 → 1.5.8 버전업 전 필수 수정, SYNC-AGENT-TROUBLESHOOTING §2-6). 상세 [[project_2026_0703_isae_157_mapping_purchase_summary]].
> **잔여**: ②③⑤ tp-push+build+pm2 재배포(KAKAO 중복 가드 포함) / 실측(KAKAO 1행·customer_send_stats·⑤ 고객등급 모달 1건 — v1.1-cold는 07-03 실측 확인 완료) / 레거시 폐기 프로젝트 = 서팀장 5문항 회신 대기(docs/레거시서버_폐기_플랜.md SoT) / ⑦ updater 자기교체 근본수정(다음 버전업 전).

---

### 🔵 다음 세션 (예정) — 싱크에이전트 updater 자기교체 근본수정 + 전 티어 검증
> **설계서(SoT)**: `docs/superpowers/specs/2026-07-03-sync-agent-updater-selfreplace-design.md` — 이 문서대로 spike→개발→티어 실측→릴리즈.
> **핵심**: updater 교체 스크립트가 자기 서비스 job을 자살시켜 무선 교체 실패(isae 2026-07-03 실측). Win=일회성 예약작업 런처+원자적 스왑, Linux=서비스 밖 실행(systemd-run)+샌드박스(ProtectSystem) 회피. 5티어 전부 재빌드 + **티어별 RED→GREEN+실패주입+오배포거부 하드게이트**(될 것 같다 릴리즈 금지). isae(win-legacy 1.5.7)는 동결. **착수 즉시 spike 먼저**(2008R2 VM·Linux Docker로 job/cgroup 탈출 실측) — 통과해야 코드 확정. 미read=agent-build-tiers·build-tiers·api/client·types/api·heartbeat/scheduler·버전상수. open: 테스트베드(모던Win/2012R2 VM)·티어 우선순위·버전번호(1.5.8 vs 1.6.0).
