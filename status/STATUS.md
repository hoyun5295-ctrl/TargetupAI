...
| 지금 상태 | B-0914-1 배포완료·복구완료·화면 육안 대기 · B-0914-2·3 **코드완료·push·배포 대기(프론트 빌드 필요)** · backend 285파일 4,424건 · frontend tsc 0 · DDL 0 · 백업표 `campaigns_b0914_backup` 잔존(하루 뒤 DROP) |
...
- [ ] **우커머스 연동(일본이모 4몰 · 박성용 경유)**: [설계서](../docs/2026-09-14-woocommerce-integration-design.md) **W1~W7 배포완료(0914) · ① 1클릭 연결(wc-auth 관리자 승인 → 키·웹훅 자동) + ② 플러그인 zip 코드완료(0914(2) · tsc 0 · 4,660 · DDL 0) · 배포 대기** → 운영 첫 승인 1건이 실측 게이트 ②(콜백 본문·웹훅 생성). 회신 잔여 = 수신동의 메타키·규모·담당자
