...
### 도움말 봇 · 기능 카탈로그 — 0822 1단계 배포완료 · 육안 확인 완료 · 다음 세션 접수 3건
...
> ⛔ 정의에 요금제 이름·크레딧 숫자·버튼 라벨을 문자열로 적지 않는다 / 매칭 실패면 모델 호출 0 / 봇 노출 = 요금제 사용 회사만(서버 판정) / 발송 이력 판정에 MySQL 금지
> 잔여 = **§10 접수 3건**(빈칸 27개 본문+이름 지어내기 차단+답변 틀 · 봇 발견성[헤더 버튼·말풍선] · 백지 계정 리허설 1일) · DDL `help_questions` 미실행 시 문의 남기기만 잠김 · 학습데이터 (나) Harold 답 대기

### 다음 세션 (예정)
> 경위·범위·확정 사실은 링크가 소유한다. 여기엔 제목·다음 한 수·호출어만.
> ★ **고객 360 — Phase 0 + 격리 정정 + v2(2열·검색) 0822 배포완료 · 실측 대기** — 호출어 **"고객 360"** → **[FEATURE-CUSTOMER-360.md](../docs/FEATURE-CUSTOMER-360.md)가 전부 소유**(정의 §1 → 불변 13개 §2 → 구조 §3 → 이력·교훈 §5 → 다음 축 §7). 시점 근거 = 1차·v2 설계서(그 문서 표가 링크)
