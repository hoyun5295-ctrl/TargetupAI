...
### AI 영업 아웃리치 v2 — 0824 코드 전량 완료·배포 대기 · **0826 v3(스마트스토어 재료 축) 설계 승인·다음 세션 구현**
> 호출어 **"영업 아웃리치"** → **[FEATURE-SALES-OUTREACH.md](../docs/FEATURE-SALES-OUTREACH.md)가 전부 소유**(정의 §1 → **불변 원칙 17개 §2(필독)** → 파일별 소유 §3 → ENV·테이블 §4 → 이력·뒤집힌 판단 §5). 시점 근거 = [0731 설계서 §15](../docs/2026-07-31-ai-sales-outreach-design.md)(v2 현행) · **§16(v3 스마트스토어 · 4역할 수렴 · 착수 전 게이트 G1~G5)**
...
> 잔여 = **배포**(ENV 6종 + DDL 2테이블 = §15-8) → 실측 1건 → 샘플 세트 15건 → **v3 착수(§16-10 순서 = G1~G5 실측 먼저 · 선결 [B-0826-1](BUGS.md))** · 별건 = [B-0824-2·B-0824-3](BUGS.md) · medium 부채 2건 = §15-9-1
