...
### 0801 여정 재설계 — §11-1·11-2·11-3 코드 완료(Codex 6R), 배포 대기
> SoT [여정 재설계](docs/2026-08-01-journey-redesign-design.md) **§11-A·§11-B(구현 결과·정정) → §11(순서)** · 기억 [[project_2026_0801_journey_redesign]] · 호출어 **여정 재설계 이어가자** · 다음 = 배포 → **§11-B-2 DDL 4단계**(순서 엄수) → §11-4 연동 배선
> ⛔ **정답표를 안 갖는다**(§2-3) / **유예는 대량 발화를 못 막는다 — 상한 필수가 막는다** / 유예 대상 ≠ 상한 필수 대상 / **커서 축 전환으로도 익명 소급(N5)은 안 닫혔다 — 5번 몫**
> 잔여 = 배포·DDL 4단계 · Harold 확인 2건(싱크 적재 주기 · 예약 취소·노쇼 구분) · 상한 없는 활성 여정은 재개 시 상한 입력 필요
