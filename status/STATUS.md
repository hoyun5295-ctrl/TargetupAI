...
> **★0717 대시보드 성능 1~4차(★배포완료·실측 종결)** — 이새(13.7만) 지연: ①MySQL (app_etc1,sendreq_time) 커버링 90 ALTER **root 재실행으로 실적용**(어제 "실행 완료" 기록은 smsuser 권한 거부로 미반영이 진실) → sendTimes 3~8초→13~36ms ②stats·campaigns 집계 = 캠페인 실적재 라인 축 축소(27→24테이블·mysqlUnion 823→324ms) ③stats·enabled-fields SWR 캐시(재진입 SLOW 0 실측) — Codex 5라운드(수용 8·기각 5)+신규 테스트 14. **잔여 = ④customers 중복 유니크 DROP 2건(pg_dump 후·DDL은 핸드오프에) + 3차 후보(pg_stat_statements·ioredis timeout·롤업)는 기록만.** 상세=[[project_2026_0717_dashboard_performance]].
