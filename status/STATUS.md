...
- **네이밍 (확정):**
  - **AI 한줄로** — 기존 AI 추천발송 (자연어 한 줄 → 타겟+문안 자동설계)
  - **AI 맞춤한줄** — 개인화 필드 선택 + 프로모션 브리핑 → AI가 1:1 맞춤 문안 생성

- **핵심 설계 (프로모션 브리핑 방식):**
  - 폼도 아니고 단순 자유텍스트도 아닌, 마케터가 회의에서 브리핑하듯 자연스럽게 작성
  - AI가 브리핑을 파싱 → 프로모션 카드로 구조화 → 마케터 확인/수정 → 최종 문안 생성
  - 데이터가 풍성할수록 맞춤한줄의 정확도가 올라감 (고객사 데이터 확보 동기 부여)

- **AI 맞춤한줄 플로우:**
  1. **개인화 필드 선택** — 고객사 DB 필드 목록에서 이번 발송에 활용할 필드 체크 (이름, 등급, 매장명, 구매금액 등)
  2. **프로모션 브리핑 입력** — 자연어로 프로모션 상세 작성 (예시 placeholder 제공)
  3. **문안 옵션** — URL 입력 (바로가기 ▶ 위치에 자동 배치), 톤/분위기 선택
  4. **AI 파싱 → 프로모션 카드 확인** — AI가 브리핑을 구조화해서 카드로 표시, 마케터가 확인/수정
  5. **최종 문안 생성** — 개인화 필드 + 프로모션 카드 + 옵션 조합하여 AI가 완성 문안 생성

- **진입 UI (모달):**
  - 대시보드 3개 메뉴 유지 (고객DB업로드 / 직접타겟설정 / AI추천발송)
  - AI 추천발송 클릭 → 모달 2분기:
    - 🚀 **AI 한줄로** — "자연어 한 줄이면 충분합니다. 타겟 선정부터 문안 작성까지 AI가 자동 설계"
    - ✨ **AI 맞춤한줄** — "프로모션을 브리핑하면 AI가 1:1 맞춤 문안을 생성합니다. 쿠폰, 혜택, 조건 빠짐없이 반영"

- **구현 위치:**
  - hanjul.ai (서비스 사용자): 대시보드 모달 + AI 맞춤한줄 전체 플로우
  - 백엔드: AI 프롬프트 확장 (개인화 필드 + 프로모션 카드 → 문안생성)

- **범위:**
  - Phase 1: 모달 분기 UI + AI 맞춤한줄 플로우 (필드선택 → 브리핑 → 파싱 → 문안생성)
  - Phase 2 (이후): 브리핑 히스토리 저장, 템플릿화, 등급별 자동 분기 발송

- **완료 기준 (DoD):**
  - [ ] AI 추천발송 클릭 → 모달에서 "AI 한줄로" / "AI 맞춤한줄" 분기 동작
  - [ ] AI 한줄로 선택 → 기존 AI 추천발송 플로우 그대로 진입 (진행상황 표시 포함)
...
- **착수 조건 (DoR):**
  - [x] AI 문안생성 기능 기존 구현 완료 (Claude API 연동)
  - [x] 네이밍 확정: AI 한줄로 / AI 맞춤한줄
  - [x] 플로우 설계 확정: 필드선택 → 브리핑 → 파싱카드 → 문안생성
  - [x] 모달 진입 구조 확정: 대시보드 3메뉴 유지, AI추천발송에서 분기
