...
### ⏱ 세션 포인트 체크 — 2026-09-10 (발송 결과 0 · 예약 커서 갇힘 · EUC-KR 반려)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | **하루에 서로 다른 다섯 결함.** ①통계가 `send_config`를 아예 안 읽어 비토 라인 캠페인이 전부 0([B-0910-1](BUGS.md)) ②예약 건이 커서에 갇혀 정각에 안 나감([B-0910-2](BUGS.md)) ③게이트웨이가 검증 오류를 재시도로 분류해 864건이 저널에 갇힘([B-0910-3](BUGS.md)) ④발급 묶음만으로 신규 고객사 시작 불가(발급 config 체계 ↔ 동봉 생성문 체계 불일치) ⑤IDC 방화벽 9443 미개방(전날 건 · 해소) |
| 지금 상태 | **①배포 완료**(한줄로 `0910(2)`) · **②1.0.27 승인 완료 · rollout 대기** · **④코드 완료 · 게이트웨이 배포 대기** · ③미착수. 실측 = 리스킨_대행 성공 17,133·실패 435·대기 437 / 시세이도 858건 en dash 치환 후 발송 재개(잔여 27건까지 확인) |
| 다음 한 수 | **1.0.27 rollout(hanjul02 → hanjul01)** 후 다음 예약 시각에 `lastFetchCount`가 0에 머물지 않는지 확인. 이어서 한줄로 배포(**administrator 계정으로만**) · 게이트웨이 `web/api` 배포 · encoding UTF-8 실측 1건 · 게이트웨이 검증오류 영구분류 |
| 미결 | **`encoding` 축**(기본 0=EUC-KR인데 UTF-8 통로가 이미 있다 · 실측 1건 전에는 열지 않는다) · **게이트웨이 검증오류 `ErrorCode` 미탑재**(허용 목록 방식이라 새 검증마다 반복) · 발급 묶음 배포 |
| 배운 것 | **고객 문안은 우리가 바꾸지 않는다**([[feedback_never_alter_customer_copy_silently]] — 발송 직전 치환 배선을 만들었다가 제거) · **검사는 예시 말고 실제 발급본을 대조한다**([[feedback_test_the_issued_artifact_not_the_example]]) · **부품이 다 있어도 부르는 곳이 없으면 죽은 검사다**(CT-46이 저니 경로에만 배선) · **재기동은 메모리만 비운다**(저널에서 claim이 복원돼 안 풀렸다) · **서버 명령은 소유 계정을 먼저 확인한다**(root로 빌드해 dist 소유권이 깨졌다) · 경위 전량 = memory `project_2026_0910_send_result_and_agent_cursor` |
