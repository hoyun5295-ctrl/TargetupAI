...
### AI 영업 아웃리치 — ★0906 캠페인 엔진 S1~S6 코드완료·배포 대기(DDL 0 · 렌더 워커 PM2 앱 1개 추가) · 0905 12:25 배포분 위에 얹는다
...
> 잔여 = 배포(OPS ⑧ · 워커 기동) → 게이트 실측(`scratch/proto/render-gate.js`) → 아이소이 재생성 육안(면허 기간 · 포스터 3칸 · 증거 카드 · 열람) → 원클릭 캠페인 재료 1회 실측(5크레딧) → Codex 리뷰(Harold가 /codex:review 직접 · 발송 잠금·삭제·크레딧 경로) → 0905 잔여(Harold 결재 2 · 네이버 키)
