...
### ⏱ 세션 포인트 체크 — 2026-09-02 (브랜드 IMAGE 이미지 출처 · 여정 포커스 · 스튜디오 +53종)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**(다음 세션이 자기 것으로 덮어쓴다). 경위·상세는 SoT 문서·memory 소유.

| 항목 | 상태 |
|---|---|
| 한 일 | **접수 3건 + 확장 1건.** ①[B-0902-6](BUGS.md) 브랜드 자유형 IMAGE가 우리 서버 URL을 실어 발송이 통째로 죽던 것(9/1 2건 `9999` 실측) → **카카오 콘텐츠 서버 업로드 확정 CT 신설 + 조립기 거부 게이트 + 조립기 호출 4경로 전량 배선 + 화면 URL 직접입력 제거** ②[B-0902-5](BUGS.md) 마케팅 여정 만들기 입력칸 포커스 강탈(혜택 칸 입력 불능·한글 자모 분리) → 모달 껍데기 effect 의존성을 `[open]` 하나로 ③싱크에이전트 주기 점검(코드 무수정·실측만) ④[이미지 스튜디오](../docs/FEATURE-IMAGE-STUDIO.md) 템플릿 **+53종**(249→302) · 예시 실샘플 배치 완료 |
| 지금 상태 | **전량 배포완료**(Harold 확인 · 커밋 `b5a8feef` B-0902-5 · `922cd669` B-0902-6 · `57b68ff0` 템플릿 · `7ac45b36` 문서 · 프론트 `build:safe` 포함). 백엔드 반영은 운영이 302종을 응답하고 예시 53장이 실패 0으로 생성된 것으로도 확인된다. 게이트 = BE·FE tsc 0 · **backend 222파일 3,537건** · 이 축 계약 38건 + 카탈로그 6건 · 변이 2건으로 계약 실효 확인 · FE production 빌드 · harness-check |
| 다음 한 수 | **실측 3건** — ①자유형 IMAGE 1건 발송 후 `SMSQ_SEND_14`의 `k_etc_json.img_url`이 카카오 도메인인지·`status_code` 1800인지 ②여정 혜택 칸 한글 입력 완주 ③스튜디오 갤러리 육안(특히 클래스·체험 5종이 "업종"이 아니라 "조건"으로 읽히는지) |
| 미결 | **IMC 회신 대기 2건** = 9/1 IMAGE 2건 종료 사유(카카오 라운지 확인 중) · **카카오 이미지 URL 유효기간**(이게 있어야 [B-0902-6](BUGS.md) 미해결① AI 캠페인 예약 축을 닫는다). 그 밖 = B-0902-6 미해결②(길이 초과 교정 문구가 500으로 덮임) · 아난티 에이전트 무응답(14:25 이후 침묵 — 고객사 장비 확인 필요, 그때까지 B-0902-4 실측② 불가) |
| 배운 것 | **외부 규격이 "어느 서버에 올라간 것"을 요구하면 키만 맞추고 값을 놓친다** — 그 실패는 접수 성공(`1000`) 뒤 큐에는 들어가고 발송만 죽어 화면에 사유가 안 남는다 · **업로드 창구는 이미 있었다**(템플릿 화면이 쓰고 있었다) — 기능이 없어서가 아니라 발송 경로가 그 창구를 안 지나서 생긴 결함 · **치환은 발송 직전, 거부는 조립기 안**(호출부 4곳 중 1곳만 고치면 나머지가 같은 방식으로 조용히 죽는다 — 0818 결함 2와 같은 부류 2회째) · **판정과 치환의 순서가 계약이면 주석이 아니라 테스트로 고정** · **포커스를 옮기는 effect의 의존성에 콜백 prop을 넣지 마라**(렌더마다 포커스를 뺏고 한글 조합을 끊는다) · **소재·업종 축을 다 판 카탈로그는 연출 축으로 넓힌다**(더 파면 "황토"가 나온다) — 전문 = [LESSONS_BACKEND](lessons/LESSONS_BACKEND.md)·[LESSONS_FRONTEND](lessons/LESSONS_FRONTEND.md) 0902 |
