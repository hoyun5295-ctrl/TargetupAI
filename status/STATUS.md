...
> 구조·불변 원칙 18개·이관 절차·남은 것·이력을 **그 문서 하나가** 갖는다. 설계서 6종은 §9가 안내한다. 여기 다시 쓰지 않는다. 상태 = **0806(1)까지 배포완료 + DDL 실행완료 / 0806(2)는 코드 완료·배포 대기**(구분값 말줄임 폐기 — 아직 운영에 안 나갔다) · **테스트베드 12장 운영 재발행 완료**(이중발행 없음 확정 — 문서 §8-4).
> ⛔ 크로커다일 `failed` 취소 장은 **[재시도] 금지**(국세청 미발행 · 취소 이유 없음) · 기발행분 품목 거래일자 공란은 §2-8 축이라 Harold 판단 · **문서 표의 행 높이는 내용에서 만든다·자르지 않는다**(§2-16 — `lineBreak: false`는 막아주지 못한다)
> 잔여 = **0806(2) 배포**(구분값 말줄임 폐기 · Codex 3R 통과) · 화면 실측(0805(2) 6건 · 0806분) · `failed` 2건 수동 재시도(문서 §5) · 시세이도 계산서 비고(PO) 착수 대기 · 특수정산 엑셀 = 문서 §7 · **0806(2)에서 갈라 둔 4건(거래내역서 엑셀 · 발송 지연 구간 **미확정** = 업체 메일 원문 확보가 완료 조건 · 구분 칸 발송ID 유지 여부 · 불변 발송 시도 기록) = 문서 §7**.
