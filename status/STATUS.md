...
### ⏱ 세션 포인트 체크 — 2026-09-14 3세션 (박성용 접수 2건: 주소록 번호 앞 0 · 알림톡 예약·분할)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 BUGS·memory 소유. **이 세션 순서·명령 = [0914 인계 §6](../docs/2026-09-14-session-handoff.md)** · 2세션(AI 자동제작·우커머스·B-0914-3) 잔여 = 같은 문서 §5.

| 항목 | 상태 |
|---|---|
| 한 일 | ①**[B-0914-4](BUGS.md) 주소록 번호 앞 0 복원** 배포(`address-books.ts` 저장·추가·조회·다운로드 4곳 · 옛 저장분은 읽을 때 복원 · DB 무변경) ②**[B-0914-5](BUGS.md) 알림톡 예약·분할** 배포(알림톡 창 옵션 · 확인 창 전달 · CT-04 예약 시각 · processor 시각 전달 · 예약 문안 수정 알림톡 거부) · **운영 실발생 1건 확정**(17:35 예약이 17:29:09 즉시 발송 · 비토 라인 14) |
| 지금 상태 | backend 301파일 4,677건 · tsc 0 · frontend tsc 0 · DDL 0 · 배포완료(restart 692 · 서버 src 4곳·프론트 `Dashboard-CB1fZZTv` 확인) · **미실측** = 알림톡 예약·취소·분할(표준 QTmsg·비토 라인 각 1회) · 주소록 0 빠진 CSV 등록→발송 1건 |
| 다음 한 수 | [인계 §6](../docs/2026-09-14-session-handoff.md) 실측 1(알림톡 20분 뒤 예약 → PG 조회 → 큐 K행 `sendreq_time`) → 2 취소 → 3 분할 → 4 주소록 · 2세션 잔여(§5: 우커머스 첫 승인 · AI 자동제작 실측 5건 · B-0914-3 재확인) |
| 미결 | 표준 QTmsg 라인 에이전트의 예약 보류(서버 select_sql · 코드로 확인 불가) · 옛 `/direct-send` 알림톡 분기 시각 미적재(화면 도달 0 · 기록만) · 한줄전단 주소록 동일 결함 여부 미확인 · 2세션 미결 = §5 |
| 배운 것 | **적재 CT가 받는 필드를 선언만 하고 SQL은 `NOW()`면 계약이 죽어 있다**(형제 함수와 줄 대칭) · **다른 창과 공유하는 전역 state를 발송 값으로 쓰면 확인 창과 서버가 갈라진다** · **공용 파서가 떨어뜨린 값은 입구 한 곳만 되살리면 다른 저장소로 샌다** · 산출물 확인은 고유 식별자로(난독화) |
