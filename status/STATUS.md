...
> ⛔ 새 축 판정 기준은 §0이 소유(사건 = 여정 / 상태·변화 = 자동마케팅) — 여기 재서술 금지. 0804 DDL 전량 실행완료 · **0805 신규 DDL 1건은 배포 뒤 실행**(§7-7)
> 잔여 = **남은 개발 4건 = [문서 §12](../docs/FEATURE-AUTOMARKETING.md)가 착수 원장**(조건부 대기 = 상품 데이터 · 나머지는 다른 트랙과 함께) + 화면 실측 7건(§12 하단) + 0805 DDL 1건. 운영을 막는 건 0
