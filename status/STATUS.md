...
> 잔여 = 배포(pull+build+reload · DDL 0) → 이니스프리 재생성 육안(룩·재료 탭·숨김) → Harold 결재 2(불변 18 목록 1홉 · 취소선 정책) + 네이버 새 키 게이트 → OPS ⑦ 실측 7(화면에서 29건 코드 적재 · 이메일 4건 → 제작 1건 근거 패널) → Codex 리뷰(설계서 §15 · Harold가 /codex:review 직접). `scratch/` = git 제외
