...
> ⑮ **브랜드메시지 — ★0906(2) 친구·비친구 단가 축 게이트웨이 코드완료·배포 대기(DDL 057·058)** — 호출어 **"브랜드메시지" / "친구 단가 축"** → [FEATURE-GW-BRAND-MESSAGE.md](../docs/bito-gateway/FEATURE-GW-BRAND-MESSAGE.md) §9가 전부 소유(결과코드 미분류 별건 = §9-4). 다음 = 배포 → 화면 육안 → 한줄로 쪽 같은 축
