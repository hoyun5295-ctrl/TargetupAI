...
> **★0718 성능·사고 후속(활성)**: B-0718-1 종결 조건 ①서버 git 일치(d6958238) ②재발 방지 게이트(safe-build 3-1 lazy 청크 실존/3-2 비literal 동적 import 검출 + verify-live-chunks.sh 라이브 전수) 완료 — Codex 5라운드 통과·하네스 실측. 로컬 "온전" 빌드도 19/38지점 암호화 불량 실측 정정. **0718 오후: ③exclude 3회 반복 빌드 실측 완료(청크 8/8·비literal 0·원복) · ⑦ 백엔드 실측 종결(이새 진입 2회 후 SLOW 0) · 양쪽 프론트 게이트 배포·라이브 검사 통과. 잔여 = ④M1 재시도(Harold 승인) · 화~수 관측 상위 20 사이클 · M3/롤업. M2=무효 — 고객사 관리자=hanjul.ai "관리" 메뉴 정책(별도 로그인 사이트 금지·Harold 0718). **app.hanjul.ai 관리자 화면 nginx 차단 완료(0718 — location / 404·/sdk·/api 유지·외부 실측 7종 통과) + SDK 실물 backend/sdk-serving 이전 + company-frontend 패키지 제거(코드 완료 — 배포·서버 SDK 합류 시퀀스 잔여).** 단축 URL 실태 확정 = 발급은 모바일 DM(hlj.kr)뿐·message_short_urls 0건 실측(hlj.kr 통일 불필요 판정).** SoT=docs/2026-07-18-frontend-splitting-incident-handoff.md · 상세=[[project_2026_0717_dashboard_performance]] · 0717 성능 1~7차 원문=archive/TASKS_2026-07.md.
