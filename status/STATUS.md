...
| 항목 | 상태 |
|---|---|
| 한 일 | 대행발송 3축(설계서 §19): ①**통일 양식** = 업계 실물(카톡 수신 xlsx · 시트 "내용"+"고객리스트" 한 파일)로 세 입구 통일(화면 슬롯 1개 · 이메일 1파일 표준·2파일 관용 · 파서 별칭·플레이스홀더·한국어 시각·문자타입 반려 · **배포 양식 = 실물 셀 단위 복제** §19-6 Harold 반려 정정) ②**사용자 격리**(서수란 접수 · Harold 확정) = company_user 본인 접수만·관리자 회사 전체 + 접수 계정 표시 ③**슈퍼관리자 "대행발송 내역" 탭** 신설(진행 레일 공용화 + 접수구분 + 고객사·신청자) |
...
| 다음 한 수 | 배포(OPS §2-2: pull → backend `build:safe` → frontend `build:safe` → `pm2 reload targetup-backend` · npm install 불필요) → 실측 4: ①새 양식 내려받아 화면 접수 1건 ②이메일 통합 1파일 접수 1건 ③company_user 계정으로 본인 것만 보이는지 + 관리자 전체·접수 계정 표시 ④슈퍼관리자 대행발송 내역 탭 육안 |
