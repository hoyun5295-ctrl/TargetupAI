...
> **① 직접 시점 선택(공용 DateTimeField 재작성)**: 인라인 날짜·오전오후·시:분 입력(칸 넘침) 폐기 → 트리거 버튼 1개(선택값 한 줄) + createPortal 다크 모달 피커(달력 그리드·오전/오후·시/분 클릭·과거 비활성, z-[2000]). value/onChange(ISO) 계약 유지로 소비 6곳(AiOperator·이메일·인앱·DM발송추적·여정 정보알림·DM 폼컨트롤) 자동 반영. AiOperator는 "직접 시점 선택" 클릭 시 모달 즉시 오픈(controlled open + overlay stopPropagation로 portal 이벤트 버블 차단).
> **② 추출 타겟 리스트 공용 모달(TargetRecipientsModal 신설)**: "추출된 타겟 N명" 클릭 → 조건(목표·해석) 표시 + 15명/페이징 + 연락처 중간 마스킹 + Source caption(createPortal z-[2000]). 데이터원천 = 페이지로더 1개(arrayPager|fetchPage)로 통일. 배선: AI Operator(preview-recipients)·이메일/인앱/DM(TargetExtractModal→신규 `POST /api/targets/recipients`)·AI캠페인 ResultPopup(메모리)·카카오RCS DB추출(메모리)·맞춤한줄/자동발송(신규 `POST /api/ai/target-recipients` compat필터). 대시보드 직접타겟은 기존 수신자목록 내장이라 무변경. 신규 엔드포인트 2개 SELECT 전용·기존 검증 컬럼 재사용(DDL 없음).
> **③ DM 섹션 정렬 전수 일관화**: 공통 "정렬(좌/중/우)"이 다수 섹션서 무력화 — 하드코딩 `text-align:center`/`justify-content:center`가 래퍼(SectionRenderer/renderSection의 text-align 상속 + `--dm-section-justify`)를 덮음 + 헤더 로고형 좌/우는 row+justify-content라 편집기 contentEditable(브랜드)가 폭 꽉 채워 flex-end 안 먹던 근본. 프론트 canvas + 백엔드 SSR(dm-section-renderer) **미러**로: 헤더 로고형=column+align-items 통일 / 헤더 카운트다운·쿠폰형·coupon(3 treatment)·countdown·promo_code(SSR)·footer 하드코딩 center 제거→래퍼 상속 / sns·상품캐러셀 justify→`var(--dm-section-justify)`. 기본 center는 래퍼가 유지=회귀0. 미디어박스·이벤트카드휠·폼입력·점·placeholder는 구조적 중앙 유지.
> **④ 개인화 변수 발송 빈칸 근본수정(단일소스)**: FIELD_MAP과 별개 하드코딩 라벨 테이블 3중(company-data-profile ANALYZED_FIELDS·ai.ts mapSampleCustomerRow+샘플SQL·inapp LEGACY_VARIABLE_MAP)이 발송 사전(extractVarCatalog=displayName)과 어긋나 미리보기만 매칭·발송 빈칸(등급/포인트는 0708 alias 2개만 땜질, 등록매장·매장명·구매액 등 방치). 넷을 FIELD_MAP displayName 단일소스 파생으로 통일 + `applyFieldDisplayNames` 신설(standard-field-map — customer_schema 라벨 회사여도 displayName 항상 유효 토큰, 스키마라벨/alias 유지=하위호환). FIELD_MAP 밖 필드(avg_order_value·ltv_score·결혼기념일)는 발송 불가라 자연 제외=잠재 빈칸 동시 제거.
> **검증**: frontend tsc 0 · backend tsc 0 · vitest 382/382 · 금지패턴 0. 프론트 변경: DateTimeField·**TargetRecipientsModal(신설)**·TargetExtractModal·AiCampaignResultPopup·AiCustomSendFlow·AutoSendFormModal·AiOperatorPage·KakaoRcsPage · dm canvas 6(HeaderSection·CouponSection·CountdownSection·FooterSection·SnsSection·NewSections). 백엔드: targets.ts·ai.ts·dm-section-renderer.ts·standard-field-map.ts·services/ai.ts·company-data-profile.ts·inapp-personalization.ts. **DDL 없음**. **잔여**: Harold 운영 실측(발송 시 %고객등급%/%등록매장정보% 채움·직접시점 모달·타겟 리스트 페이징) + Codex /codex:review. 상세 [[project_2026_0709_datetime_target_dm_var_unify]].
