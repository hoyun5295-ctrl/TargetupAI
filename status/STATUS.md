...
> **⑥ 카페24 공식 앱 연동 개통 (★실측 완주 — 금액 fix만 미배포)**: 개발자센터 앱 "한줄로AI"(client iQ5aZ…) 설정 완료(권한 9종+웹훅 5종: 90023/25/26/29/84) + ENV 4건 + 수신부 실측 정정(X-API-Key+event_no 매핑, 미연동 몰 200 무시 — 실패 누적 시 수신 자동차단 정책 대응) + 연동 화면 공식앱 기본(자체앱=접이식). 테스트몰 gyunoo83 실측: OAuth 토큰 저장 + cart.added/order.created processed + cdp_events cart_add·purchase(고객 매칭). 발견 결함 fix: 금액 "0.00" 문자열 truthy로 `||` fallback 미동작 → `firstPositiveAmount` CT(normalize) 신설, cafe24+naver 적용 / items는 extra_info로 옴. 잔여: 금액 fix 배포 후 입금확인 처리(90025) 실측 / 회원 웹훅=개인정보(Privacy) 권한 별도 절차(운영자 화면에 없음 — 문의 필요) / 앱 실행 랜딩(심사 대비) / 판매정보+심사 신청. 상세 [[project_2026_0703_cafe24_app_store_integration]].
