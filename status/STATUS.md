...
### 대행발송 셀프 접수 · 0823 배포완료(예약 12:00 실측 확인) · 0823(2) 당일 재검사 폐지 코드완료·배포 대기
> 호출어 **"대행발송"** → **[2026-08-22-agency-send-design.md](../docs/2026-08-22-agency-send-design.md)가 전부 소유**(30초 요약 §0 → 불변 §3 → 구조 §4 → 소유권 모델 §12-F → 계약 §12-G. **당일 재검사 폐지 = §14**)
> ⛔ 큐에 직접 안 넣는다(직접발송 배관 위임) / **정산 축에 새 값 만들지 마라**(`send_type='direct'`로 탄다) / 소유권 = `revision`·`lock_token` 둘뿐, **타임스탬프를 토큰으로 쓰지 마라** / **`final_test_at`은 통과 분기에서만 찍는다**(§14-3) / 담당자 번호 복수 · 문안 변수 4칸
> 잔여 = **배포 후 실측 = 당일 접수 건에서 T-2h 스팸검사 0회 + 예약 생성**(§14) → 발송결과 표시 → 담당자 복수 실측 → 당일 차단 경로. 별도 과제 = §14-6(검사 실패를 판정으로 읽는 자리). DDL 0

### 도움말 봇 · 기능 카탈로그 · 0822 1단계 배포완료 · 0824 접수 2건 + 모델 경로 확장(Sonnet 5 확정) 코드완료·배포 대기
...
> 잔여 = **배포 후 §10-5·§10-6 실측**(모달 딥링크 · 구어 질문 · 후속 대화 · ceo 질문 이력 탭) → §10-1 육안 40개 → 백지 리허설. DDL `help_questions` 미실행 시 문의 남기기·질문 이력만 잠김 · 학습데이터 (나) Harold 답 대기
