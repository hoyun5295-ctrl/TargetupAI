...
> **★0717 대시보드 성능 1~3차(배포됨·★판정 1건 보류 — 다음 세션 1순위)** — 이새(13.7만) 지연 추적: 인덱스 4종(PG purchases·balance_tx 적용 / MySQL (app_etc1,sendreq_time) 커버링 90 ALTER 적용 — **지배 구간=발송시각 집계 3~8초의 처방**) + 쿼리 구조(안티조인·병렬화·캐시) + SLOW/SLOW-STAGE 계측 상시화. **다음 세션 = ①인덱스 빌드 완료 확인 후 이새 재측정(16:54 측정은 빌드 중 오염 가능성) ②stats 27테이블 합집합 구조 ③enabled-fields 콜드(SWR 후보) ④customers 중복 유니크 DROP 2건 실행.** 상세=[[project_2026_0717_dashboard_performance]] (전 실측·소거 과정 기록).
