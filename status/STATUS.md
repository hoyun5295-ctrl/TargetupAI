...
| 지금 상태 | backend 336파일 5,098건 · tsc 0(백·프) · 프론트 운영 빌드 확인 · **DDL 0 · 배포완료 `2afc8b81`**(0919 16:24) · 알림톡 `mfa_login` **켜짐**(`비토게이트웨이_1`) · **0920 로그인 실측 = 카카오톡 도착** |
| 다음 한 수 | ①카카오 미사용 번호로 SMS 전환 1건 실측 → `SYSTEM_ALIMTALK_KINDS`에 `sender_auth` 추가 ②접수 4건 직원 실측(BUGS 각 항목 "실측" 줄) ③베이컨 20,000원 대기 요청 **거절로 닫기**(수기 충전 완료분 · 승인하면 이중 충전) |
