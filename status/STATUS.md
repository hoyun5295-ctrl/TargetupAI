...
| 지금 상태 | **사고 = 원복·복구 완료**(배포본 `sanitizeForSms` 0 · 시세이도 109건 역치환 109/0 · **이미 나간 3건은 되돌릴 수 없다**). ①④배포 완료 · ②1.0.27 두 라인 적용 확인 · ③P1 배포·실측 완료(게이트웨이 `5b3d48e`) · UTF-8 = 젬텍 213 반려로 불가 확정 · **규격 밖 글자 P2 코드완료·배포 대기**([설계서](../docs/2026-09-10-unsupported-char-substitution-design.md)) |
| 다음 한 수 | **P2 배포 → 실측 1건**(직접발송에 `–` `🎉` → 안내·바꾸기·발송 막힘 확인 후 비토 라인 발송) → P3(게이트웨이 동의형 대체 · DDL) · 이미 나간 3건 고객사 알림 판단 |
| 미결 | 카카오 실패 대체 문자가 어느 라인으로 나가는지 확인(P2 제외) · P4 = CT-46 정리([B-0910-5](BUGS.md)) · 비토 라인 예약이 라인 변경 뒤 적재될 때 커서 동작 실측 |
