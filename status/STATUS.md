...
### ⏱ 세션 포인트 체크 — 2026-09-17~18 (SNS 게시 설계 · **몰별 사용자 연동 설계·구현**)

> 다음 세션이 3줄로 이어받는 자리. **직전 세션 것만 둔다**. 경위·상세는 설계서·BUGS 소유. **별건과 그 사유 = 설계서 §8 · SNS 설계서 §9.**

| 항목 | 상태 |
|---|---|
| 한 일(0917 · SNS 게시) | Harold 아이디어(외부 답변 지참) → 플랫폼 사실 공식 문서로 직접 확인 → **브레인스토밍 전원합의체**(1차 · 교차 1R · 회의론자 최종 9정정 · 갈린 2 = 명칭 4:1 · 골격 2:2) → [설계서](../docs/2026-09-17-sns-publish-design.md)(불변 20 · 테이블 5 · 상태 7 · 1차-A 인스타·Threads → 1차-B 릴스·페북·X → 2차). 코드 0 · **Harold 검토 대기 · 결재 4**(캡션 표시 의무 · NEW 배지 · X 계정 · 크레딧 값=2차) |
| 한 일(0918 · 몰별 사용자 연동) | 이에스페이먼트 몰별 계정 연결 잠김 접수 → 상시 원칙(관리자 전체 · 사용자 분류코드) 구멍 3 확인 → [설계서](../docs/2026-09-18-mall-integration-user-scope-design.md) → 측정 SQL 3(영향 사용자 0 · 소급 0 · 시세이도 고객 0) → 승인 → **P1~P4 구현**(CT 2 신설 · 공용 적재 선택 인자 · 우커머스 권한 CT · 격리 CT 폴백 정정 · 화면) = [B-0918-1](BUGS.md) · 결정 [D92](DECISIONS.md) |
| 지금 상태 | backend 330파일 5,032건 · tsc 0(백·프) · **DDL 0 · 배포완료**(0918 08:57 reload · 서버 실측) · Codex 미실행(`/codex:review` 1R 권장 · 돈·DDL 무접촉) · 이에스페이먼트 연결 **보류 중**(실측 §6 뒤) |
| 다음 한 수 | ①배포 → 실측(설계서 §6 · 관리자 등록부 4 → 배정 4 → 사용자 연결 · 타인 몰 409 · 목록 격리 · 직접 타겟 대상 수) → 이에스페이먼트 안내 ②SNS 설계서 결재 4 → S0(Meta 앱 2유스케이스 · raw curl 1건) ③설계서 §7-4 `users.store_codes` 타입 SQL → SCHEMA 등재 |
| 미결(별건 · 사유는 설계서 §8) | 여정 엔진 분류코드 무시(몰별 사용자 회사 여정 = 관리자 계정만) · P5 나머지 5 provider · 서버 API 키 분류 축 · `customer_stores` 인라인 3곳 통합 · `manage-users` 등록부 대조 0 · `company_integrations` provider 조건 없는 조회 2곳 fail-open · SNS 설계서에 같은 축 반영 · `mssms_agreement_label` 고객사 확인 |
| 배운 것 | **상시 원칙은 되묻지 않는다 · 회사 단위 자원은 "주인이 사용자인가"부터**([[feedback_user_scope_is_a_standing_principle]]) · **새 적재 경로는 형제 경로의 `customer_stores` 쓰기와 줄 단위 대칭** · **"내 것 0건 = 제한 없음" 폴백은 다른 회사에서 구멍**(유령 배정) · 영향 판단을 적기 전에 SCHEMA 그 표의 진실 선언을 읽는다(합치기 기준이 바뀐다고 틀리게 적었다) · vitest `beforeEach(() => mock.mockReset())`는 반환값을 정리 함수로 알고 다시 부른다(블록 본문으로) |
