...
### ⏱ 세션 포인트 체크 — 2026-09-16 (모바일 DM 목록 리스트형 · 블록 조립 신설 · 시작 화면 입구 통합)
...
| 항목 | 상태 |
|---|---|
| 한 일 | ①**[B-0916-1](BUGS.md)** 시작 화면 타일 2×2 정정 ②**내 DM 목록 리스트형(A안)** = 카드 3열 → 한 줄 1 DM(대표 이미지·상태·열람·섹션·수정 + 추적/주소/편집 + ⋯ 메뉴 · 페이지 10) · 서버 `section_summary.cover` 신설(보이는 섹션 첫 이미지 · 공개 경로 정규화 · `pages->0` 폴백) ③목록 화면 **AI 진단 카드·1-click 3카드 숨김**(편집 화면 quick-action 기능 유지) ④**블록 조립 신설**(블록 14종 = 기존 섹션 타입 · 블록 창 = 기존 섹션 편집기 재사용 · 시작 세트 4 · 이미지 스튜디오 겹쳐 띄워 제작·삽입) ⑤**쪽 템플릿 4종 + 합성**(`dm-catalog-templates`·`dm-catalog-render` · `POST /dm/catalog/render-pages` · 크레딧 0) + **상품 칩**(가격·링크는 이미지 밖 `data-dm-chip`) + **넘김 효과 3종**(`settings.effect` · 미지정 = 현행 바이트 동일) ⑥**시작 화면 입구 통합**([B-0916-2](BUGS.md) 시나리오 12종 제거 · 완성 슬라이드+카탈로그+라이브러리 → 완성 이미지 하나 · 카드띠 [직접 제작] → [블록으로 만들기] · 패널 2열 재조합) ⑦**[B-0916-3](BUGS.md)** 블록 창 흰 표면 색 정정 + 표면 계약 테스트 |
| 한 일(2세션 · 정산·단축URL) | ⑧**[B-0916-5](BUGS.md)** 계정별 정산에서 테스트·스팸이 공통 장으로 가던 것 정정(`USER_SHEET_CHANNELS`에 `test`·`spam` · **0726 결정 개정** = [FEATURE-BILLING §2-21](../docs/FEATURE-BILLING.md) · DDL 0) ⑨**[B-0916-4](BUGS.md)** 수기 부가서비스 수량을 행 수 → 컬럼으로(`quantity`·`item_qty`·`item_label` **ALTER 3 실행완료**) · 1행 = 단가×수량 · 항목명이 유형 칸·항목줄 이름 = [§2-22](../docs/FEATURE-BILLING.md) · **⑧⑨ 배포완료(`572e133a`) · 실측 진행 중** ⑩**[B-0916-6](BUGS.md)** 단축 URL 한글 주소 지정 신설(검증·선점은 DM 별칭과 같은 함수 · 409 + 과금 0) + 발급 **100 → 20크레딧**(Harold 확정 · AI 0 · 확인 모달 유지) · DDL 0 · 배포 대기 |
| 지금 상태 | backend 319파일 4,909건 · tsc 0(백·프) · DDL 0(정산 ALTER 3은 실행완료) · **정산 = 배포완료·실측 중 / DM·단축URL = 배포 대기** |
| 다음 한 수 | 단축 URL 배포 → 한글 주소 1건 발급 실측(20크레딧 차감·재발급 409) → 화면 확인(시작 화면·블록 창·목록) → 쪽 합성 1건 실측 → 효과(책장·페이드) 폰 확인 → [0915 인계 §5-2](../docs/2026-09-15-session-handoff.md) 잔여 실측 |
...
| 배운 것 | **입구가 겹치면 먼저 짚고 한 번에 정리한다**(라이브러리·직접 제작·시나리오를 세 번에 나눠 고쳐 Harold 가 세 번 지적) · **창과 화면은 표면 색이 다르다**(흰 모달에 다크 전제 색 = 글씨 소멸) · **품질 문제가 프롬프트가 아니라 재료 부재일 수 있다**(빠른 시작 = 사진 0장) |
