# BUGS 아카이브 — 종결 버그·과거 현황 (BUGS_RESOLVED)
...
> **타임라인 해명(로그 실측)**: "어제는 되다가 아침에 뻗은" 정체 = 사고 빌드를 밤새 열어뒀던 브라우저 탭들이 아침(09:00~10:44)에 탭 복원되며 원시 경로 요청 → HTML 폴백 → 오류. 재배포(16:05) 이후 원시 경로 요청 0건. 사고 빌드는 38지점 중 19지점만 깨진 "반쯤 산 빌드"라 어젯밤 일부 화면은 정상으로 보였던 것.

### (원문) B-0718-1 프론트 스플리팅 배포 → 전 고객 화면 진입 불능 (Critical) — 2026-07-18
...
> **복구**: 프론트만 f8ac12f6 원복+build:safe — 외부 실측 정상 확인.
> **종결 조건**: ①서버 git 정합(revert 커밋) = 완료(0718 d6958238) ②safe-build 산출물 기계 검증 게이트 = 완료(0718 — Codex 5라운드 통과·하네스 실측) ③난독화-동적 import 상호작용 제거 = 실측 완료 후 M1 재도입 패키지로 배포 완료. SoT=docs/2026-07-18-frontend-splitting-incident-handoff.md
> **추가 실측(판정 정정)**: "온전"으로 봤던 로컬 76청크 빌드도 동적 import 38지점 중 19지점이 `import(W(491))` 형태 암호화 불량 — 로컬도 라우트 상당수 런타임 깨짐이었음. 상세 = SoT §2.
...
## 종결 배치 전체 (원본 60행~)
## 2-0) 📋 D132 — CT-17 요금제 게이팅 + subscription_status 네이밍 정리 (2026-04-22) — ✅ 배포+검증 완료

> **배경:** 30일 PRO 무료체험 시스템(CT-17) 구축 과정에서 여러 경로가 `subscription_status`를 `'active'`로 덮어써서 체험 상태가 파괴되는 3중 버그 발견 → `'paid'`로 통일하여 근본 정리.
> **상세:** STATUS.md `💎 D132` 섹션 참조. memory/project_plan_gating_policy.md에 세션 기록.

### 해결 건 (4건, 모두 ✅ Closed)

- **B32-01** 🔴 `admin.ts` 요금제 수정/승인이 무조건 `subscription_status='active'`로 덮어쓰기 → grant-trial 후 저장만 눌러도 `'trial'`→`'active'` 체험 파괴. **근본:** TRIAL plan 분기 없음 + `'active'` 네이밍이 `companies.status`와 충돌. **해결:** `admin.ts:384,1031`에 plan_code 분기 추가 + 전체 `'active'`→`'paid'` 교체
- **B32-02** 🔴 `trial-downgrade-worker`가 `subscription_status='trial'` 조건만으로 강등 → `'active'`로 오염된 체험은 **영원히 강등 불가 (무제한 체험 버그)**. **해결:** SQL에서 `subscription_status` 조건 제거, `plan_code='TRIAL'` + `trial_expires_at<NOW()`만으로 판정
- **B32-03** 🔴 프론트 `isOnTrial` 판정이 `subscription_status` 기반 → `'active'`로 바뀌면 D-N/violet 안내 카드 전부 미표시. **해결:** PricingPage/Dashboard/plan-guard 전부 `plan_code='TRIAL'` 기반으로 변경 (진실의 원천)
- **B32-04** 🟡 AdminDashboard grant-trial 응답 후 `planCode: 'PRO'` 하드코딩 → 실제는 TRIAL인데 PRO로 표시. **해결:** API 응답 `plan_code` 사용 + companies.ts grant-trial RETURNING에 `plan_code` 추가
...
1. **보조 상태값 의존 판정은 취약** — 여러 경로가 덮어쓸 수 있는 컬럼(`subscription_status`)에 로직 의존하면 깨짐. **`plan_code` 같은 진실의 원천 기반 판정이 견고**
2. **컬럼 간 동일 값 혼용 금지** — `companies.status='active'`(운영 활성)와 `subscription_status='active'`(정식 구독) 네이밍 충돌 → 인지 혼선 + 디버깅 난이도 ↑. **용도 다르면 값도 달라야 함** (구독은 `'paid'` 표준)
3. **설계서와 실제 구현 불일치 정기 점검** — CT-17 설계서에 `'paid'`로 정의했으나 코드는 `'active'` 사용 → 오픈 전 동기화 필수
4. **오픈 D-Day 전 기술 부채 정리** — 네이밍 충돌 같은 근본 이슈는 운영 들어간 후 고치면 데이터 마이그레이션 리스크 ↑. 오픈 전이 최적 시점 (Harold님 판단)
5. **UI 버그 → 실데이터 흐름 끝까지 추적** — D-N 안 보임을 보고 "프론트 문제"로 판단하지 않고 DB→API→프론트 전 경로 확인 → `admin.ts`가 체험 상태 덮어쓰는 진짜 범인 발견
...
- **N3** 특수문자 커서 삽입 안 됨 🟠 — 신규 `utils/textInsert.ts` 컨트롤타워(3함수) + 4곳 인라인 제거
- **N4** MMS 이미지 호버 시 UUID 파일명 🟡 — JSONB 객체 배열 확장 + 신규 컨트롤타워 2종(`mms-image-util.ts`/`mmsImage.ts`) + 업로드 응답 originalName(한글 latin1→utf8) + 소비처 전수 호환
- **N5** AI 문안 하단 붙음 🟠 — D123 P12 `dnCount ≤ 3` 제거, 프롬프트 "마무리 앞 빈 줄 필수" 추가
- **+α** 무료수신거부 앞 빈 줄 추가 — `buildAdMessage`/`buildAdMessageFront` `\n\n무료수신거부` + 본문 끝 개행 정규화. 5경로+스팸테스트+표시경로 자동 반영
- **+β** DM-PRO-DESIGN.md 설계서 완성 — 다음 세션 구현 착수

### 핵심 교훈

1. **SQL 파일 자동생성 금지** — OPS.md 기반 Harold님 직접 실행 안내로 대체 (feedback_no_sql_generation.md 메모리 기록)
2. **MVP 마인드 탈피** — 프로 요금제 차별화 기능은 기능·디자인 동시 완성도 확보
3. **컬럼 통일은 순서·이름 양쪽** — 데이터 같아도 헤더명·순서 다르면 혼란 (웹 기준으로 엑셀 맞춤)
4. **2곳 이상 중복 = 즉시 CT** 원칙 반복 적용 (커서 삽입 4곳 / MMS 경로 추출 여러 곳)
5. **JSONB 확장 > 스키마 변경** — 하위호환 안전. DB 변경 최소화
6. **컨트롤타워 시그니처 범용 우선** — `(newValue: string) => void` 형태가 React Dispatch와 일반 콜백 모두 호환
7. **재작업은 사용자 의도 재확인부터** — N2는 PDF 오독으로 3번 뒤집음. 빠른 수정보다 제대로 읽기

---

## 2-0) 📋 D120 — UI 통일 + 캘린더 이동 + 080 저장 버그 (2026-04-14) — 🟡 수정완료-검증대기

> **배경:** 한줄로AI UI 통일감 작업 + 헤더 메뉴 정리 + 080 수신거부 저장 버그 수정

### 해결 버그 (1건)
- **B120-01** 080 수신거부 저장 안 됨 🔴 — PUT /settings가 `companies.reject_number`에만 저장, GET은 `getOpt080Number()`로 `opt_out_080_number`를 읽어 덮어씀 → 저장해도 다시 로드 시 이전 값. **수정:** PUT에서 `opt_out_080_number`도 함께 UPDATE

### UI 개선 (4건)
- **U120-01** AiPreviewModal 핸드폰 프레임 리뉴얼 — 960px 텍스트 모달 → 400px 핸드폰 프레임 (맞춤한줄과 통일), 하단 버튼 제거
- **U120-02** DashboardHeader 메뉴 정리 — AI 분석 + 캘린더 메뉴 제거
- **U120-03** ResultsModal 캘린더 + 메시지 모달 — 콘텐츠 우측 상단에 캘린더 버튼 추가, 스팸필터 이력 문안 클릭 가능(MessageCell), 메시지 상세 모달 핸드폰 프레임
- **U120-04** CalendarModal embedded prop 추가

---

## 2-0) 📋 D114 — 0410 PDF 버그 10건 + 엑셀 다운로드 신규 (2026-04-12) — ✅ 배포완료

> **배경:** 직원 검수 PDF(한줄로_20260410.pdf) 11건. P11(스팸테스트 불안정) 서버 조회 → 테스트폰 앱 상태 문제 확인(코드 이슈 아님, 재부팅 후 정상). 나머지 10건 수정.

### 해결 버그 (10건)
- **P1** 고객 전체삭제 후 업로드 충돌 — delete-all에 customer_field_definitions/customer_stores/unsubscribes 삭제 + customer_schema 초기화 추가
- **P2** 매핑 충돌 덮어쓰기 실패 — 고객 0명이면 충돌 severity를 warning으로 격하 (slot_label/slot_type/label_moved 3종 전부)
- **P3** 수신거부 DB업로드 시 사용자 간 공유 — company_user 경로에 store_code 필터 추가
- **P4** 직접발송 숫자 구분자 미적용 — replaceVariables 주소록 기타1/2/3에 formatNumericLike 적용
- **P5b** 맞춤한줄 필드명 영문 출력 — FIELD_KEY_DISPLAY_MAP 컨트롤타워 + RecommendTemplateModal 한글 변환
- **P6** 자동발송 AI문안 알림 2건 중복 — generating_at 컬럼 + 원자적 잠금 (UPDATE RETURNING)
- **P7** 자동발송 AI문안 변수 미치환 — personal_fields key→displayName 변환 (getFieldByKey + customer_field_definitions)
- **P8** 자동발송 실행이력 3건 — 알림/스팸 run 건수 기록 + started_at + success_count=0 초기 + sync 연동
- **P9** 슈퍼관리자 고객사 고객 수 0 — companies.ts 목록 SELECT에 total_customers 서브쿼리 추가
- **P10** 발송통계 엑셀 다운로드 (신규) — admin.ts CSV 엔드포인트 + fetch+blob 다운로드

### P11 스팸테스트 불안정 — 코드 이슈 아님
서버 spam_filter_test_results 조회 결과: `received=true AND result IN ('blocked','timeout')` = **0건** (false positive 없음). blocked/timeout 건은 전부 `received=false` (앱 수신 보고 미도달). KT폰 집중 발생. 직원 재부팅 후 4/10 이후 전부 정상. **원인: 테스트폰 앱 백그라운드 종료.**

---

## 2-0) 📋 D111 — 0408 검수 9건 + 오픈 전 결정사항 2건 (2026-04-09) — ✅ 배포완료

> **배경:** 4/13 레거시 이관 D-4. 0408 직원 검수리스트/PDF 9건 + 오픈 전 결정사항 2건(업로드 매핑 충돌 / 숫자 포맷) 통합 처리.

### 해결 버그
- **P0** 계정 중복접속 차단 — D100의 5개 허용 폐기. `app_source`(hanjul/flyer/super) 분리로 전단AI 공존 유지 + 같은 앱 1세션 강제. CT: `session-manager.ts`
- **P2** isoi 직접발송 `%이름%` NULL — FIELD_MAP aliases + applyFieldAliases + replaceVariables 폴백 + NameEmptyWarningModal 4단계 방어
- **P3** 맞춤한줄 (광고)+080 중복 — Dashboard.tsx 미리보기 prop + 실발송 body 둘 다 `customSendData.isAd`로 수정
- **P4** 예약대기 race + UNION ALL alias 충돌 + 과거 예약 차단 + orphan 32건 정리 — `smsSelectAll` 컨트롤타워 `extractAliasMap`/`rewriteSuffixWithAliases` 근본 수정. `campaign-validation.ts` 신설
- **P5** 자동발송 사전알림 (광고) 누락 — CT-B 3개 빌더에 isAd/opt080Number 파라미터 추가 + buildAdMessage 호출
- **P6** 자동발송 스팸결과 "통과 통과" 중복 — 호출부 라벨 단순화 + 빌더 replace 제거
- **E1** gender 'F' 노출 — TargetSendModal/DirectTargetFilterModal 인라인 GENDER_DISPLAY_MAP 삭제 → FRONT_FIELD_DISPLAY_MAP 통합
- **E2** 자동발송 시간 — calcNextRunAt routes/utils 2곳 중복 → utils 하나로 통합 + routes import
- **E3** 캘린더 취소 이력 — campaigns.ts `cancelled` 제외 조건 삭제 (draft만 제외)
- **①** 업로드 매핑 충돌 검증 — `upload-mapping-validator.ts` + `/validate-mapping` API + `UploadMappingConflictModal.tsx` 신설. 충돌 4종 × 해결 4종
- **②** 숫자/소수점 포맷 — `formatNumericLike()` 백/프론트 동일 컨트롤타워. 정수 그대로 / trailing zero 제거 / 유효 소수 보존 / 전화/YYMMDD 제외

### 핵심 교훈
1. **컨트롤타워 승격 시 호출부 suffix 호환 필수** — D110 `smsSelectAll` UNION ALL 승격 때 `ORDER BY seqno`가 inner alias `idx`와 충돌. 호출부 땜질 revert + 컨트롤타워에 alias 자동 재작성 로직 추가
2. **"2곳 이상 중복 = 즉시 컨트롤타워" 원칙 반복 적용** — calcNextRunAt 2곳 중복, GENDER_DISPLAY_MAP 2곳 중복, 소수점 규칙 2곳 불일치
3. **이관 D-4 리스크 관리** — 땜질/대강 훑기 절대 금지. "설계 → 컨펌 → 전수 파악 → 구현 → 검증" 프로세스 철저

---

## 2-0) 📋 D110 — 캠페인 상세조회 0건 + CT-04 UNION ALL 전면 승격 (2026-04-08) — ✅ 배포완료 (D111과 동시)
...
### B103-01 🟡 (광고) 중복 발송 — 수정완료-검증대기
- **심각도:** 🔴 Critical
- **현상:** AI 캠페인 발송 시 `(광고) (광고) [시세이도]` + `무료수신거부 080-540-5648` 2줄 표시
- **근본 원인:** 프론트에서 `buildAdMessageFront()`로 (광고)+080을 붙여 DB에 저장 → 발송 시 백엔드 `buildAdMessage()`가 또 추가
- **수정:**
  - `prepareSendMessage()` 컨트롤타워 신설 — 변수 치환 + (광고)+080 한 함수로 통합
  - 프론트 API body 5곳에서 `buildAdMessageFront` 제거 → 순수 본문만 전달
  - `buildAdMessage()`에 중복방지 안전장치 추가 (이미 포함 여부 체크)
- **영향 범위:** AI즉시/AI예약/AI맞춤/직접/타겟/테스트/자동발송/브랜드수정 — 전 8경로
- **상태:** 🟡 수정완료-검증대기

### B103-02 🟡 개별회신번호 드롭다운에 비전화번호 필드 노출 — 수정완료-검증대기
- **심각도:** 🟠 Major
- **현상:** 회신번호 선택 드롭다운에 생일, 주소, 이름 등 전화번호가 아닌 필드 표시
- **근본 원인:** displayName 하드코딩 필터('전화','회신','연락처') 또는 필터 자체 없음
- **수정:**
  - 백엔드: `isPhoneLikeValue()` + `detectPhoneFields()` 컨트롤타워 → enabled-fields API에 `phoneFields` 응답 추가
  - 프론트: `detectPhoneHeaders()` → 파일 업로드 시 전화번호 컬럼만 동적 감지
  - 5곳 드롭다운 전부 `__col__` + `phoneFields` 기반 동적 필터로 통일
  - 하드코딩 'store_phone' 기본값 제거
- **상태:** 🟡 수정완료-검증대기
...
### B104-01 🟡 발송통계/발송결과 당일 데이터 조회 불가 (슈퍼관리자+중간관리자) — 수정완료-검증대기
- **심각도:** 🔴 Critical
- **현상:** 2026-04-02~2026-04-02 당일 범위 → 0건. 시작일이 항상 제외됨
- **근본 원인:** PostgreSQL TZ=UTC 환경에서 `$N::date AT TIME ZONE 'Asia/Seoul'` 패턴이 timestamptz 비교 시 0건 반환 (서버 검증: AT TIME ZONE 0건 vs 단순비교 7건)
- **수정:** stats-aggregation.ts 컨트롤타워 `($N || ' 00:00:00+09')::timestamptz` 패턴으로 변경. admin.ts/manage-stats.ts/results.ts/campaigns.ts 4개 파일 컨트롤타워 import 교체. billing.ts 2곳 인라인 동일 패턴 적용(SQL 구조상 import 불가). **서버 검증 7건 확인**
- **상태:** 🟡 수정완료-검증대기

### B104-02 🟡 직접타겟발송 보유포인트 필터 안 됨 (전체 인원 조회) — 수정완료-검증대기
- **심각도:** 🔴 Critical
- **현상:** 포인트 어떤 조건 눌러도 전체 9,116명 조회
- **근본 원인:** (1) POINTS_PRESETS가 `filterValues['points']` 설정 → buildDynamicFiltersForAPI는 `filterValues['points_min']`만 읽음 → 무시. (2) **buildDynamicFiltersForAPI가 filterValues 순회** → 숫자 필드 UI는 `_min/_max`만 설정하고 본 키 미설정 → 숫자 핸들러 미도달 → **누적구매금액/평균주문금액 등 기존 숫자 필드 전체 동일 문제**
- **수정:** buildDynamicFiltersForAPI를 `selectedFields` 순회로 변경. 포인트 UI를 min/max 범위 + 프리셋 병행. B17-06 보조 루프 중복 제거
- **상태:** 🟡 수정완료-검증대기

### B104-03 🟡 직접발송 YYMMDD 날짜 미리보기/스팸테스트 쉼표 처리 — 수정완료-검증대기
- **심각도:** 🟠 Major
- **현상:** 첫구매날짜 '250103'이 미리보기/스팸테스트에서 '250,103'으로 표시. 실발송은 정상
- **근본 원인:** formatPreviewValue + formatNumberPreview에 YYMMDD 6/8자리 패턴 감지 없음 → 순수 숫자로 인식 → toLocaleString 쉼표
- **수정:** formatPreviewValue + formatNumberPreview 양쪽에 YYMMDD 보호 추가 (mm 01-12, dd 01-31 범위 체크). 컨트롤타워이므로 직접발송 미리보기/수신자별 미리보기/스팸필터 3개 경로 자동 반영
- **상태:** 🟡 수정완료-검증대기

### B104-04 🟡 직접타겟발송 날짜 이상 표시 (ISO 원본 노출) — 수정완료-검증대기
- **심각도:** 🟠 Major
- **현상:** 생일/최근구매일이 `1985-04-11T15:00:00.000Z` 형태로 표시
- **근본 원인:** TargetSendModal formatCellValue에서 date 핸들러 없음 → String(value) → ISO 원본
- **수정:** formatCellValue를 formatByType 컨트롤타워 사용으로 교체. 인라인 toLocaleString 제거
- **상태:** 🟡 수정완료-검증대기

### B104-05 🟡 발송결과 30초 대기시간 → 5초 축소 — 수정완료-검증대기
- **심각도:** 🟡 Minor
- **현상:** 조회 필터 변경마다 30초 대기 → 원하는 데이터 조회에 1~2분 소요
- **수정:** cooldown 30초→5초 + fetchData(force) 파라미터 추가. 날짜 변경 시 force=true로 cooldown 무시. onClick 래핑(MouseEvent 방지). 예약취소 후 force=true
- **상태:** 🟡 수정완료-검증대기

### B104-06 🔵 (광고)+080 중복 — D103에서 수정 완료
- **D103 B103-01과 동일.** 예약대기에서 확인 시 중복 표시. D103 배포 시 해결

### B104-07 ✅ 자동발송 AI문안 생성 후 담당자 알림 미발송 — D105 수정완료
- **심각도:** 🟠 Major
- **수정:** AI 문안 생성 완료 시 notify_phones에 생성된 문안 + 발송 예정시각 SMS 발송 추가

### B104-08 ✅ 자동발송 스팸필터 미리보기 개인화 미적용 — D105 수정완료
- **심각도:** 🟠 Major
- **수정:** AutoSendFormModal에서 recommend-target의 sample_customer_raw를 SpamFilterTestModal에 firstRecipient로 전달

### B104-09 ✅ 자동발송 D-1 담당자 사전알림 메시지 부실 — D105 수정완료
- **심각도:** 🔴 Critical
- **수정:** CT-01+CT-03으로 타겟 수 실시간 조회 포함, "X월 X일 XX:XX / N명" 형식으로 개선

### B104-10 ✅ 자동발송 설정시간 최대 10분 지연 — D105 수정완료
- **심각도:** 🟠 Major
- **수정:** 워커 간격 10분→5분 축소 (최대 5분 지연)

---

## 3) 📋 D97 — 디버깅 이슈 4건 (2026-03-27) — 🟡 수정진행중

> **배경:** D97 브랜드메시지 구현 중 발견된 기존 버그 4건.

### B97-01 🟡 스팸필터 미리보기 %회신번호% 숫자 포맷팅 (D95 재발) — 수정완료-검증대기
- **심각도:** 🟠 Major
- **이전 수정:** D95에서 formatPreviewValue에 0시작 숫자 보호 추가 → 미해결 (0으로 시작하지 않는 회신번호 `1800-8125` 케이스 누락)
- **현상:** 스팸필터 점검 모달에서 `%회신번호%`가 `18,008,125`로 표시됨. 실제 회신번호는 `1800-8125`
- **근본 원인:** (1) SpamFilterTestModal에 인라인 replacePreviewVars가 %회신번호% 매핑 자체 누락 (2) formatPreviewValue가 하이픈 포함 숫자열 미보호
- **수정 (D97 세션):**
  - SpamFilterTestModal 인라인 `replacePreviewVars` 삭제 → `replaceDirectVars` 컨트롤타워 호출로 교체
  - DirectPreviewModal 인라인 `replaceVarsDirect` 삭제 → `replaceDirectVars` 컨트롤타워 호출로 교체
  - `replaceDirectVars` 컨트롤타워에 `useFallbackLabels` 파라미터 추가 (미리보기용 기본값 표시)
  - `formatPreviewValue`에 하이픈 포함 숫자열 보호 추가 (`/^\d[\d-]+\d$/` + `-` 포함 시 숫자 포맷팅 제외)
  - callback(회신번호)은 `replaceDirectVars`에서 `formatPreviewValue` 적용 제외 (기존 로직 유지)
- **상태:** 🟡 수정완료-검증대기

### B97-02 ✅ 담당자 설정 회사 전체 공유 (D91/D96 재발) — Closed
- **심각도:** 🔴 Critical
- **이전 수정:** D91에서 users.manager_contacts 컬럼 추가, D96에서 companies.manager_contacts 폴백 제거
- **현상:** 회사계정 아래 사용자 계정에서 담당자 번호 변경 시, 회사 아래 전체 사용자의 담당자가 동일하게 변경됨
- **최종 수정:** CT-11 test-contact-helper.ts + test_contacts 테이블로 담당자 완전 이관. user_id 기반 사용자별 격리. 프론트 Settings.tsx는 /api/test-contacts API 사용. companies.manager_contacts는 settings 응답에서 삭제 처리
- **상태:** ✅ Closed

### B97-03 ✅ 직접발송 파일업로드 전화번호 포맷팅 미비 — Closed
- **심각도:** 🟡 Minor
- **현상:** 엑셀에 `010-5295-8517`로 있는데 하이픈 없이 `01052958517`로 표시됨. 하이픈 없는 번호도 포맷팅(`010-5295-8517`)해서 표시 필요
- **근본 원인:** DirectSendPanel 수신자 테이블에서 formatPhoneNumber 미적용. 저장은 정규화 필요하지만 표시는 포맷팅 필요
- **수정:** DirectSendPanel.tsx에서 수신자 phone, callback 컬럼, 회신번호 드롭다운 3곳 모두 formatPhoneNumber 적용 완료
- **상태:** ✅ Closed

### B97-04 ✅ 생일 날짜 밀림 (D93 재발) — Closed
- **심각도:** 🟠 Major
- **이전 수정:** D93 Math.round → D98 로컬TZ getDate() → 둘 다 실패
- **현상:** 엑셀 업로드 시 생일/가입일시 등 날짜가 하루 밀려서 저장됨
- **근본 원인:** xlsx cellDates:true → 엑셀 시리얼→Date 변환 시 부동소수점 오차로 자정에서 ~9시간 부족한 값 생성 (예: 3/1/95 → `1995-02-28T14:59:08Z`). 어떤 TZ든 getDate()=28 → 하루 전
- **수정 (D99):**
  - normalize.ts normalizeDate: `Math.ceil(ms/dayMs)*dayMs` UTC 올림으로 다음 자정 복원 → getUTCFullYear/Month/Date
  - normalize.ts normalizeByFieldKey: normalizeFunction 없는 커스텀 필드에 Date 객체가 오면 `String()` 변환 방지 → Date 그대로 반환하여 normalizeCustomFieldValue에서 Math.ceil 분기 진입
  - 10/10 테스트 케이스 전수 통과 + Harold님 실데이터 업로드 검증 완료
- **상태:** ✅ Closed

---

## 2-1) 📋 D98 — PPT 버그리포트 11건 전면 수정 + 재검증 (2026-03-27) — ✅ 배포완료
...
## 2-1) 📋 D89 — 마이너 수정 5건 + D88 직접발송 잠금 회귀 수정 (2026-03-20) — ✅ 배포 완료
