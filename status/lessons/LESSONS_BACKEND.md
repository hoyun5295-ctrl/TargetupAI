...
## 핵심 원칙
- **★ 플랫폼 페이지(한 회사가 만든 틀 위의 수많은 가게)는 화면 글자를 긁기 전에 페이지에 실린 상태 데이터부터 찾는다 · "막힌 축" 판정은 두 번째 출구에서 재현한 뒤에 (2026-09-24 AI 영업 네이버 스토어 · [B 설계서](../../docs/2026-09-24-outreach-naver-store-reader-design.md))** — 북마크 1차는 화면 복사본의 글자를 홈페이지용 추출기에 넣었다. 저장된 2,000자 중 스토어 내용은 두 줄뿐이고 나머지는 네이버 공통 메뉴·알림 창·**보는 직원의 계정 패널**이었다(복사본에는 화면 스타일이 없어 닫혀 있던 층까지 글자가 된다 · `<header>` 만 걷어서는 안 걸린다). 같은 화면 안에 `window.__PRELOADED_STATE__`(220KB)가 있었고 상품 이름·정가·할인가·할인율·리뷰·기획 메뉴·관심고객수가 전부 구조로 들어 있었다. 네이버 스토어는 모든 가게가 한 틀이라 판독기 하나가 전부를 읽는다. 처방 = 상태에서 **허용 칸만**(회원·주소 칸은 버튼 안에서 버림) → 순수 판독기 → 실물 원본으로 만든 고정 자료로 테스트. 곁들여 0923에 "브랜드스토어는 .62 출구 IP 만 막힌다"고 결론 냈는데 0924 사무실 PC curl 도 429였다(같은 PC 크롬은 정상) — **출구 한 곳의 실패로 "IP 차단"을 단정하지 않는다.** **판별 둘: ①수많은 가게가 같은 틀을 쓰는 사이트(스토어·쇼핑몰 빌더)는 먼저 `__NEXT_DATA__`·`__PRELOADED_STATE__`·`application/ld+json` 을 grep한다 ②보는 사람의 브라우저에서 읽는 도구는 그 사람의 개인 정보가 같은 객체에 있다는 전제로 허용 목록을 짠다.**
- **★ 외부 API 응답의 테스트 대역은 운영 로그 원문으로 만든다 — 지어낸 대역은 결함을 덮는다 (2026-09-20)** — 다중 이미지 창구 응답을 `data.list[{imageUrl,imageName}]` 로 지어내 테스트를 통과시켰는데 실제는 `data.success[{index,formField,imageUrl}]` 였다. 카카오 업로드는 성공(`0000`)했는데 추출이 빈 목록을 내 발송이 「등록 결과를 확인하지 못했습니다」로 죽었고, 나는 그것을 이미지 규격 문제로 추정해 Harold 에게 보고했다. 같은 날 게이트웨이에서도 두 번 반복됐다(고정 오류 문구로 시험한 로그 억제 · 아무도 안 부르는 테스트 파일). **응답 구조를 직접 본 적이 없으면 대역을 만들기 전에 1건 실측해서 로그 원문을 뜬다. 실패 원인은 로그를 본 뒤에 말한다.**
- **★ "남은 것은 실측뿐"이라고 적기 전에 그 값이 지나는 입구를 끝까지 걸어 본다 · 두 시스템 사이의 계약은 한쪽 부품만 보고 맺으면 나머지 입구가 그 계약을 막는다 (2026-09-20 브랜드메시지 자유형 5종 · [B-0920-1](../BUGS.md))** — 0828에 규격 CT·조립기·검사기·등록 화면·게이트웨이 커넥터를 다 만들고 "남은 것 = 유형별 실측 5회"라 적었다. 시험 발송을 준비하며 발송 1건의 경로를 입구부터 따라가 보니 죽은 자리가 다섯이었다: ①`/brand-send`가 캐러셀 조립 함수를 부르지 않고 자리표시 `'[]'`를 넘김 ②큐 `k_etc_json`이 varchar(1024)라 캐러셀이 물리적으로 안 실림 ③조립기는 본문 없는 유형의 `msg_contents`를 **일부러 비우는데** 큐 적재 방어는 빈 본문을 거절 ④이미지 확정 창구가 2종·1자리뿐 ⑤게이트웨이 Agent가 빈 본문 행을 「필드 매핑 실패」로 거절. ③⑤는 같은 뿌리다: "본문 없는 유형은 `msg_contents`를 비운다"는 계약을 **게이트웨이 커넥터 하나만 보고** 맺었고, 그 값이 먼저 지나는 큐 적재 방어와 Agent 매퍼·검증기는 보지 않았다. 유형이 입구 게이트에 막혀 있어 아무 테스트도 이 경로를 끝까지 타지 못했고(검사기 테스트는 `assertCarouselSpec`을 직접 불렀다), 그래서 부품별 테스트는 전부 초록이었다. 처방 = 시험 계정에게만 여는 원장(`BUBBLE_TYPE_TRIAL`)으로 **조립기 경로 테스트가 실제로 돌게** 만들고, 컬럼 폭은 상수가 아니라 DB에서 읽는다(`getEtcJsonCapacity`). **판별 셋: ①입구 게이트로 막아 둔 기능은 "게이트만 열면 된다"가 아니다 — 게이트 뒤 경로는 한 번도 실행된 적이 없다. 열기 전에 발송 1건을 입구(화면 payload)부터 출구(소켓 프레임)까지 코드로 따라간다 ②값을 비우거나 형태를 바꾸는 계약을 맺을 때는 그 값을 읽는 방어·검증 코드를 양쪽 저장소에서 grep한다(`== ""`·`required`·「비어」) ③varchar 컬럼에 JSON을 싣는 구조는 새 유형을 열 때 최대 구성의 실제 길이를 계산해 컬럼 폭과 대조한다.**
- **★ 회사 단위 자원의 주인이 사용자일 수 있으면 분류코드 축을 처음부터 넣는다 · 새 적재 경로는 형제 적재 경로의 `customer_stores` 쓰기와 줄 단위로 대칭을 본다 · "내 것 0건 = 제한 없음" 폴백은 다른 회사에서 구멍이 된다 (2026-09-18 이에스페이먼트 몰별 사용자 연동 · [B-0918-1](../BUGS.md) · [D92](../DECISIONS.md))** — 상시 원칙(관리자 전체 · 사용자는 분류코드로 자기 것만)의 장치(`store-scope.ts` × `customer_stores`)는 있었는데 자사몰 적재 `identifyCustomer`(호출처 12)만 `customer_stores`를 안 썼고(업로드·싱크·단건은 썼다), 연결은 7 라우트 24곳이 관리자 한정이라 "사용자마다 자사몰이 따로 있는 회사"를 설계에서 한 번도 다루지 않았다. 게다가 격리 CT의 유령 배정 폴백(D136 · 인비토 사례용)이 "내 코드 매칭 0건"을 회사 전체로 열어 조회뿐 아니라 직접 타겟 발송 3곳까지 넓히고 있었다(운영 배정 7명 전원이 그 갈래였고 그중 시세이도 브랜드 계정 5개는 고객 0이라 무해했을 뿐). 처방 = 선택 인자 `storeCode`(생략 = 1바이트 불변 · 캡처 테스트 먼저) + 권한 CT(`integration-scope.ts` · 분류코드는 본문이 아니라 세션 DB 값) + 폴백은 회사 전체 0행일 때만. **판별 셋: ①회사 단위 기능(연동·SNS 계정·발신 자원)을 만들 때 첫 질문 = "이 자원의 주인이 회사인가 사용자인가" ②적재 함수를 하나 더 만들면 형제 함수가 쓰는 표(`customer_stores` 등)를 grep해 같은 줄이 있는지 본다 ③"매칭 0건이면 필터 해제" 분기를 보면 그 0건이 "체계 없음"인지 "아직 내 것 없음"인지 두 갈래로 나눈다.** 곁들여: 영향 판단을 문서에 적기 전에 SCHEMA 그 표의 진실 선언(customers "폰당 1행 · 다매장은 customer_stores")을 읽는다 — 합치기 기준이 바뀐다고 틀리게 적었다가 정정했다. 테스트 도구 함정 하나 = vitest `beforeEach(() => mock.mockReset())`처럼 화살표가 mock을 돌려주면 vitest가 그것을 정리 함수로 알고 테스트 뒤에 인자 없이 부른다(블록 본문 `{ }`로).
- **★ 적재 CT가 받는 필드를 선언만 하고 SQL은 상수면, 호출부가 값을 넘겨도 계약은 죽어 있다 · 같은 큐의 형제 적재 함수와 줄 단위로 대칭을 본다 (2026-09-14 알림톡 예약 즉시 발송 · [B-0914-5](../BUGS.md))** — `insertAlimtalkQueue`(CT-04)는 행 타입에 `reservedDate`를 선언해 두고 VALUES는 `sendreq_time = NOW()` 고정이었다. 워커는 채널 무관하게 수신자별 예약·분할 시각을 계산했고 브랜드 적재(`insertBrandQueue`)는 `reservedExpr`로 그 값을 썼지만, 알림톡은 processor가 시각을 버리고 CT도 무시했다. PG는 예약인데 큐는 즉시라 운영에서 17:35 예약이 17:29:09에 나갔다(비토 라인 14 · seqno 47238). 타입은 맞고 값만 버려지므로 tsc·기존 테스트 어디에도 안 걸린다. 처방 = 시각이 있으면 `?`, 없으면 `NOW()`(형제 함수 미러) + processor 행에 `reservedDate` + **SQL에 파라미터를 결합해 `sendreq_time` 자리 값을 검사하는 테스트**. 곁들여 예약 행이 새로 생기면 큐를 바꾸는 쓰기 경로(취소·수신자 삭제·시각 변경·문안 수정)를 전수로 본다: 문안 수정은 문자 규약이라 K행을 덮으면 템플릿 불일치 → F행과 같은 방식으로 거부. **판별: 적재 CT의 행 타입 필드마다 그 필드를 읽는 SQL 자리가 있는지 grep한다. 한 큐에 채널별 적재 함수가 여럿이면 한 채널의 기능(예약·분할·대체)을 다른 함수에서 같은 줄로 찾는다.**
- **★ 공용 파서가 떨어뜨리는 값은 입구 한 곳에서만 되살리면 다른 저장소로 샌다 · 되살리기는 그 저장소를 소유한 경로에서 (2026-09-14 주소록 번호 앞 0 · [B-0914-4](../BUGS.md))** — `/api/upload/parse`(`raw:false`)는 CSV 원문 `01000000000`도 숫자로 읽어 0을 떨어뜨린다(로컬 실측 · XLSX 숫자 셀 동일). 직접발송·알림톡 파일등록은 화면이 `normalizePhoneKr`로 되살렸는데, 같은 파일을 주소록으로 넣는 경로는 저장·조회 어디에도 복원이 없어 저장된 `1055775868`이 발송에서 "수신번호 형식 오류"가 됐다. 처방 = 파서는 공용이라 두고, `address_books`를 읽고 쓰는 유일한 파일(`routes/address-books.ts`)의 저장·추가(중복 판정)·조회·다운로드 4곳이 같은 CT로 복원 · 옛 저장분은 DB UPDATE 없이 읽을 때 복원(두 화면이 같은 API라 화면 수정 0). **판별: 파서 결과가 들어가는 저장소마다 "그 저장소의 쓰기·읽기 경로가 복원하는가"를 본다. 화면에서 되살린 입구는 그 화면만 고쳐진다.**
- **★ "전 라인 합집합"이라는 안전망은 라인 종류가 늘 때마다 같이 늘어야 한다 · 집계가 0건을 돌려주면 "발송 0"이 아니라 "못 찾음"일 수 있다 (2026-09-14 금강제화 33,346건 · [B-0914-1](../BUGS.md))** — 0611에 "라인 해제/재배정 뒤에도 과거 발송이 보이게" 전 bulk 라인을 합쳤는데, 0704 비토 라인이 생길 때 그 합집합은 안 늘었다. 0717 정산만 bulk+bito로 고쳤고 집계·재대조·sweeper 14곳은 그대로였다. 회사 라인이 비토 1번(13)에서 대량발송(2)로 바뀌자 재대조 워커가 0건을 읽어 PG 카운트를 0/0/0으로 덮고 굳혔다. 화면 다섯이 0이 됐고 `sentTables`로 읽는 상세 상단만 실값이라 "같은 페이지 안에서 숫자가 다른" 모양으로 드러났다. 처방 = ①합집합에 활성 bito 전체 합류 ②원장을 덮어쓰는 워커는 실측 0건에 적재 증거(PG 카운트·`sentTables`)가 맞서면 덮지 않고 로그 ③카운트를 세는 SELECT는 `sentTables`를 싣는다(0910이 5곳만 고치고 4곳을 남겼다 — 같은 패턴은 grep으로 한 번에). **일반화: 새 라인 종류를 만들 때 "전 라인"이라 적힌 함수를 grep해 그 뜻이 여전히 전부인지 본다. 그리고 "0을 읽었다"와 "없다"는 다른 사실이다.**
- **★ 죽어 있던 안전망을 살리면 그 안전망이 새 실패 경로가 된다. 살린 날 적대검토를 붙인다 (2026-09-13 싱크에이전트 로그 마스킹)** — 0912에 "한 번도 동작하지 않던" 마스킹을 info 전체에 걸리게 고쳤다. 그러자 ①axios 오류의 순환 요청 객체에서 무한 재귀 → winston이 포맷 예외를 **호출부로 던져** 등록 실패 시 로컬 모드 계속·전송 실패 시 오프라인 큐 저장이 죽는 경로가 생겼고 ②오류를 통째로 복사해 가리다 보니 **JSON 문자열인 요청 본문**의 `apiSecret`이 키 기준 마스킹을 통과했다. 둘 다 안전망이 죽어 있을 때는 없던 경로다. 처방 = 로그 포맷은 어떤 값에도 던지지 않게 감싸고, 오류는 **가릴 것을 찾지 말고 남길 것을 고른다**(허용 목록). **일반화: 동작하지 않던 방어 코드를 되살리는 변경은 기능 추가와 같은 무게로 검토한다. 그 코드는 한 번도 실데이터를 지나 본 적이 없다.**
- **★ "이번 tick은 넘긴다"는 마감이 있는 일에서는 "하지 않는다"와 같다 (2026-09-13 대행발송 동시 예약 상한 · Codex 3R high)** — 풀 고갈을 막으려고 동시 상한에 걸린 예약을 `approved`로 되돌려 다음 5분 tick에 맡겼다. 재승인 직후 트리거는 적재 마감이 임박한 건을 위해 있는 경로라, 넘기면 다음 tick에서 이미 마감 밖이다. 처방 = 자원을 쥐지 않은 채 **기다리고**(프로세스 안 대기열), 기다린 뒤 소유권을 다시 확인한다. **일반화: 경합을 피하려고 일을 미룰 때는 그 일에 마감이 있는지 먼저 본다.**
- **★ 결정의 근거를 코드로 다시 본다. 근거가 틀렸으면 결정도 다시 연다 (2026-09-12 대행발송 회신번호 분할 · [B-0912-5](../BUGS.md))** — "회신번호 열 방식이면 접수를 쪼갠다"의 근거는 "적재 배관이 수신자별 회신번호를 나르지 못한다"였다. 실제로는 발송 워커가 staging의 `callback`을 함께 읽고 프로세서가 `resolveCustomerCallback`으로 행마다 싣는다. 대행발송 적재만 그 컬럼을 비워 넣고 있었다. 결과 = 16종이면 스팸 검사·담당자 문자·승인이 16번. **일반화: 불편 접수가 설계 결정에서 나왔으면, 결정문의 근거 문장을 그대로 코드에서 확인한다.**
- **★ 전환기 폴백의 갈래는 실제 실패로 판정한다. 사전 탐지 캐시는 폴백을 구멍으로 바꾼다 (2026-09-12 싱크에이전트 시크릿 해시 전환)** — 컬럼 존재를 `information_schema`로 미리 조회해 옛 경로/새 경로를 고르면, 그 조회가 한 번 실패했을 때 "없음"이 캐시되어 옛 경로(원문 매칭)로 내려간다. 원문을 지운 뒤라면 그 창 동안 전 에이전트가 401이다. 처방 = 새 경로 SELECT를 먼저 시도하고 **`42703`일 때만** 옛 경로. **일반화: "없으면 옛 방식" 분기는 옛 방식이 살아 있는 동안만 안전하다. 옛 방식을 걷어낼 계획이 있으면 판정을 실제 실패로 옮긴다.**
- **★ 성능 처방은 측정이 뒤집을 수 있다 (2026-09-12 대행발송 100만 행)** — "파서가 워크북을 통째로 메모리에 올리니 스트리밍으로 바꾼다"는 가설이었다. 같은 조건 실측 = 현재 방식 RSS 310MB · exceljs 스트리밍 1,574MB(공유 문자열 표를 통째로 들고 있어야 한다). 실제로 건수를 막던 것은 **적재가 앱 메모리를 거치는 구조**였고, `INSERT … SELECT` 한 문장으로 풀렸다. 경위 = FEATURE-AGENCY-SEND 불변 25.
- **★ 안전망은 주인이 아니다 — 두 경로가 같은 값을 쓰면 늦게 도는 쪽이 앞선 판정을 덮는다 (2026-09-12 알림톡 검수 동기화)** — 단건 조회가 죽었을 때를 대비한 30분 목록 동기화를 되살리면서 그 경로도 `status`를 쓰게 했다. 그런데 진행 중 여부는 **목록 페이지를 돌기 전** SELECT 값으로 정하고 UPDATE는 id만 봤다. 페이지 순회에 몇 분이 걸리는 동안 5분 폴링이 승인을 저장하면, 안전망이 그것을 다시 진행 중으로 덮어 **승인된 템플릿의 발송이 막힌다**(Codex 적대검토 high). 처방 = 안전망 쪽 UPDATE의 WHERE에 **조회 시점 상태**를 조건으로 걸고, 0행이면 그대로 둔다. 주인(단건 폴링)에는 조건을 걸지 않는다 — 걸면 교정을 못 한다. **일반화: 같은 컬럼을 두 경로가 쓰면 누가 주인이고 누가 안전망인지 먼저 정하고, 안전망에만 조건을 건다.** 비대칭이 정답이다.
- **★ 한 컬럼이 없다고 함수를 통째로 멈추면 로그가 축소 보고가 된다 (2026-09-12 · 직원 접수 4번 6일 침묵의 절반)** — `syncTemplateStatuses`가 첫 SELECT에 활성상태 컬럼(미생성)을 넣어 **함수 전체가 return 했다.** 로그는 "활성상태 동기화 skip"이라 적어, 읽는 사람은 그 한 축만 쉬는 줄 알았다. 실제로는 검수상태·반려사유 동기화까지 함께 멎어 안전망이 통째로 죽어 있었다. 나머지 절반은 단건 조회가 IMC 비-0000을 **아무 기록 없이** 건너뛴 것이다(발신프로필 키가 외부에서 `_unused`로 바뀌어 4011이 6일간 반복). 처방 = 컬럼은 **따로 시도하고 없으면 그 축만** 건너뛴다 · 건너뛴 건수를 코드별로 세어 사이클마다 한 줄 남긴다(첫 건만 상세 — 5분 주기라 폭주 금지). **일반화: skip의 범위와 로그 문장이 다르면 그 로그는 거짓이다.**
- **★ 지울 수 없는 것은 원장이 참조하기 때문이다 — 그때 답은 사용 중지다 (2026-09-12 카카오 발신프로필)** — 고객사가 프로필 삭제를 요청했는데 `campaigns.kakao_profile_id`가 ON DELETE 없이 참조해 하드 삭제가 23503으로 막힌다. 뚫으려면 발송·정산 원장인 캠페인을 지워야 하고, `brand_message_templates`는 CASCADE라 **경고 없이 함께 사라진다.** 처방 = 상태만 내리고(사용 중지) 원장은 전부 남긴다 + 누가 내렸는지 감사 기록. 접수자도 "이력 아예 삭제가 아닌 관리를 위해서 사용불가 표기"를 요구했다. **곁들여: 중지가 화면에 반영되려면 목록 조회가 그것을 빼야 하고, 재등록이 되려면 중복 가드와 UNIQUE 인덱스를 활성 한정으로 좁혀야 한다** — 상태 한 칸을 내리는 일이 조회·제약 세 곳을 함께 건드린다. 삭제 기능을 만들기 전에 **참조하는 표를 pg_constraint로 먼저 센다.**
- **★ 스냅샷 동기화에서 "행을 빼는 것"과 "값을 0으로 만드는 것"은 다르다 — 빠진 키를 지우는 쪽이 없으면 전자는 옛 값을 화석으로 남긴다 (2026-09-04 정산 이중 계상)** — 게이트웨이가 한줄로 수집 DB에 일통계를 **스냅샷 UPSERT**로 적재한다(키마다 현재 누적값을 덮어쓴다 · append 금지). 여기서 이중 계상 원인 행(카카오 실패 대체발송의 **원본**)을 집계에서 빼려고 `WHERE`에 조건을 걸었는데, 그러면 **그 그룹의 레코드가 전부 원본일 때 키 자체가 산출되지 않는다.** UPSERT는 안 돌려준 키를 지우지 않으므로 먼저 적재된 옛 값이 그대로 남고, 되돌아보기를 늘려 재적재해도 안 고쳐진다. 자식 행은 따로 청구되므로 **이중 청구가 영구히 굳는다.** 처방 = **카운터 3개(ok·fail·ready) 모두에 제외 조건을 걸어 그룹은 계속 산출되게 한다** — `(0,0,0)`이 옛 값을 덮어 수렴하고, `TotCnt = Ok+Fail+Ready` 계약도 `0+0+0`으로 성립한다(WHERE를 택한 원래 근거였던 "FILTER로 빼면 TotCnt가 깨진다"는 애초에 성립하지 않았다). **일반화: 원격 상태를 스냅샷으로 맞추는 모든 경로에서 "안 보낸 키는 어떻게 되는가"를 먼저 답한다.** 지우는 쪽이 없으면 삭제를 표현하는 유일한 수단은 **0으로 보내는 것**이다. Codex 적대검토 high 수용 · 경위 = [FEATURE-BILLING §8-8](../../docs/FEATURE-BILLING.md) · [BUGS B-0904-5](../BUGS.md).
- **★ 한 컬럼이 두 질문에 답하면 그것을 읽는 쪽 하나가 조용히 틀린다 (2026-09-04 알림톡 전환분 40건 과다 청구)** — 게이트웨이 `message_request.norm_status`는 API 소비자에게 "이 요청이 최종적으로 전달됐는가"를 답한다. 그래서 카카오 실패 후 문자 대체발송이 성공하면 **원본 알림톡까지 `DONE`으로 덮는다** — 그 관점에서는 맞는 판단이다. 그런데 청구 집계가 **같은 컬럼**을 "이 유형으로 과금 가능한 성공"으로 읽었다. 전달을 만든 것은 자식(문자)인데 원본(알림톡)이 그 공을 가져가, 카카오에 도달한 적 없는 40건이 알림톡 단가로 청구되고 있었다. **에러도 경고도 없고 금액만 틀린다.** 발견 단서는 **일자별 대조에서 K와 KL의 실패 건수가 8일 내내 정확히 일치한 것**이었다(우연으로 나오는 모양이 아니다 → 코드 추적 → 확정). **처방 = 값의 의미가 읽는 쪽마다 다르면 컬럼을 나누는 것이 원칙이고, 발송 경로를 건드리는 위험이 더 크면 집계 쪽에서 막되 구조가 남았다는 사실을 등재한다.** 일반화: "이 값이 무엇을 뜻하는가"에 두 개 이상의 답이 나오면 그것은 이미 두 컬럼이다.
- **★ 외부 규격이 "어느 서버에 올라간 것"을 요구하면 키만 맞추고 값을 놓친다 — 그리고 그 실패는 큐에는 들어가고 발송만 죽는다 (2026-09-02 브랜드 자유형 IMAGE · [B-0902-6](../BUGS.md))** — 카카오 `ATTACHMENT.image.img_url`은 **콘텐츠 서버 업로드본만** 받는다. 우리는 키(`img_url`)를 맞게 골라 놓고 값으로 자체 자산 URL을 실었다. 접수는 성공(`RETURN_CODE 1000`)이고 화면에는 아무 사유도 안 남는다 — `status_code 9999`로 1초 만에 끝났고, 같은 구조의 TEXT 건은 성공이라 **"이미지만 안 된다"는 신호조차 흐렸다.** ⛔ 업로드 창구는 **이미 만들어져 있었다**(템플릿 등록 화면이 쓰고 있었다) — 기능이 없어서가 아니라 **발송 경로가 그 창구를 안 지나서** 생긴 결함이다. 새 기능을 붙이기 전에 "같은 일을 하는 창구가 이미 있는가"를 먼저 본다. **처방의 형태가 중요하다: 치환은 발송 직전에, 거부는 조립기 안에.** 조립기를 부르는 경로가 4곳이라 한 곳만 고치면 나머지 셋이 같은 방식으로 조용히 죽는다 — 조립기가 우리 서빙 URL을 거절하게 만들면 빠뜨린 경로가 **그 자리에서 사유와 함께 멈춘다.** 이것은 0818 결함 2(검사가 조립기 밖에 있어 호출부 6곳 중 1곳만 통과)와 **같은 부류의 두 번째**다 — 같은 부류가 두 번 나오면 개별 수정을 멈추고 구조로 간다. 곁들여: 판정과 치환의 **순서가 계약**이 될 수 있다(AI 생성 이미지 판정은 우리 서빙 URL을 근거로 삼으므로 치환보다 앞이어야 한다) — 순서가 계약이면 주석이 아니라 **테스트로 고정**한다. 상세 = [브랜드메시지 §4-4](../../docs/bito-gateway/FEATURE-GW-BRAND-MESSAGE.md).
- **★ DB 오류를 "행 문제 / 계통 문제"로 가를 때는 코드 대역이 아니라 **폴백이 그 오류를 해결하는가**를 기준으로 한다 (2026-09-02 아난티 25,244건)** — 배치 실패 시 단건 폴백 여부를 `code.startsWith('22') || startsWith('23')`으로 갈랐는데, `21000`(cardinality_violation = "배치 안에 같은 충돌 키가 두 번")이 그 대역 밖이라 **계통 오류로 분류돼 폴백이 생략**됐다. 21000은 단건으로 쪼개면 그냥 풀리는 오류다 — 폴백이 부하를 늘리는 연결·구문·자원 오류와 **정반대인데 같은 취급**을 받았다. 그래서 중복 1건 때문에 청크 500건이 통째로 버려졌고 53청크가 죽었다. **분류 기준을 "이 오류에 폴백을 돌리면 상황이 나아지는가"로 되물으면 대역이 아니라 성질로 가른다.** 곁들여: 같은 사고의 1차 원인은 **upload 경로에만 dedupe를 넣고 sync 경로를 빠뜨린 것**(2026-08-14) — 공용 충돌 축(`company_id, phone`)을 쓰는 경로는 **전부** 배치 내 dedupe가 필요하다. 경위·코드 = [SYNC-AGENT-TROUBLESHOOTING §2-10](../SYNC-AGENT-TROUBLESHOOTING.md).
- **★ AI에게 주는 후보 목록이 곧 게이트다 — 목록이 틀리면 AI는 "그 목록 안에서 최선"을 골라 조용히 틀린다 (2026-09-02 아난티 구매 98,600건 전량 드롭)** — 설치 시 컬럼 매핑을 AI가 추천하는데, 프롬프트를 만드는 함수가 **인자를 안 받아** 대상(customers/purchases)과 무관하게 늘 **고객 필드표만** 줬다. `customer_phone`·`purchase_date`가 목록에 없으니 AI는 고를 수 없고, 전화번호를 보면 `phone`, 구매일을 보면 `recent_purchase_date`를 고르는 것이 **그 목록에서는 정답**이다. 결과로 구매 매핑이 통째로 고객 필드가 됐고 에이전트가 전량 드롭했다. **AI 잘못도 담당자 잘못도 아니다.** 게다가 응답 후처리(`sanitizeMapping`)의 허용 키도 고객 고정이라 **프롬프트만 고쳐도 안 됐다**(이중 구멍 — 입력 목록과 출력 화이트리스트는 같은 축이므로 함께 본다). **처방 = ①대상별 목록을 CT 하나가 소유하고 프롬프트·후처리·저장 검증 세 곳이 그것을 쓴다 ②"AI가 고를 수 없으면 나타날 수 없다"를 게이트로 설계한다.** 그리고 **저장 시점 검증이 없으면 잘못된 설정이 조용히 눌러앉는다** — `update_config`는 "mapping 객체가 있는가"만 보고 값은 한 번도 안 봐서 이 상태로 몇 달을 갔다.
- **★ 주기·임계는 상수로 적지 말고 원장에서 읽는다 — "60분 주기니까 130분" 같은 상수는 고객사마다 다른 설정 앞에서 거짓말이 된다 (2026-09-02 재발)** — 2026-04-21에 판정 기준을 10/30분 → 70/130분으로 고친 것은 상수를 옮겨 적은 것일 뿐이었다. **동기화 주기는 `config` jsonb ▸ 레거시 컬럼 ▸ 기본값 3겹이고 기본이 360분(6시간)** 이라, 6시간 주기 고객사는 정상인데도 3시간 만에 오프라인으로 떴다. 같은 파일이 화면에 표시하는 주기도 컬럼만 보고 기본 60을 써서 실제와 달랐다. **처방 = 해석 규칙을 CT 하나로 세우고(`sync-intervals.ts`) 판정·표시가 같은 값을 쓴다.** 관련 = [[feedback_registry_owns_the_value_not_a_constant]] 부류.
- **★ 진실의 자리를 옮기면 판정 코드만이 아니라 읽기·쓰기·제약 3축을 전수로 옮겨야 한다 — 하나라도 남기면 다음 날 재오픈된다 (2026-09-02 게이트웨이 식별코드 재오픈, Harold "왜 한 번에 안 하냐")** — 0901에 공용 식별코드 원천을 Go 상수에서 등록부로 옮기고 발송 판정까지 고쳤지만 ①발급 시 등록부 값을 계정 컬럼에 복사하는 한 줄(스냅샷) ②그 컬럼을 읽는 표시·응답 15곳(사업자·계정 배지·Agent 수정·dry-run 문구·온보딩 계약·config payload·운영 스크립트) ③`migrations/048` CHECK가 옛 코드를 리터럴로 고정한 것을 남겼다. 등록부를 바꾼 다음 날 화면은 옛 코드를 보였고 invito_default 발급계정 저장은 제약 위반으로 조용히 깨져 있었다(pg_constraint 실측). 처방 = ①읽기 전용 스냅샷은 저장을 끊고(NULL) 유효값은 함수 하나(`resolveEffectiveIdentityCode`)가 계산해 응답 필드로 싣는다 ②리터럴 제약은 마이그레이션으로 재생성하되 배포 순서에 무관하게 쓰기가 안 막히는 형태로 ③착수 전 `AS 별칭`·`INSERT/UPDATE`·`CHECK`·화면 prop·deploy 스크립트·README 기대값까지 grep. 경위 = 게이트웨이 `status/BUG_HISTORY.md` 2026-09-01 ★재오픈 절 · memory `feedback_moving_a_truth_source_means_all_readers_and_constraints`.
- **★ 하드 삭제 판정에 파생 로그를 이력으로 세지 마라 — 표적 사례(발급 실수 계정)를 반드시 막는다 (2026-09-02 게이트웨이 alert_event, 서수란 접수)** — 삭제 관문이 15개 테이블을 셌는데 `alert_event`(heartbeat 없음 알림)는 설치된 적 없는 활성 Agent에 cooldown마다 쌓여, 발급만 하고 하루 지난 계정은 무조건 "발송·과금 이력 있음"으로 막혔다. 접수자 가설(상용계층 발송계정 활성 잔존)은 판정 조건에 없었고 **테이블별 카운트 SQL 한 줄이 원인을 정했다.** 처방 = 운영 알림·모니터링 산출물은 세지 않고 정리 삭제 목록에 넣는다(건수는 audit_log에 남긴다). 같은 뿌리가 형제 경로(고객사 카운트·발송계정 FK)에도 있어 승인 후 함께 닫는다. 경위 = 게이트웨이 `status/BUG_HISTORY.md` 2026-09-02.
- **★ 선례를 복제할 때는 그 선례가 성립한 조건부터 본다 · 소재의 완성 주체가 고객사 재료면 AI가 발행하면 안 된다 (2026-09-02 마케팅 플래너 문자+DM 접수 · 읽기 전용 검토 18에이전트 + Codex 2R)** — "초안 대기 접점은 `planned` + 단계 표식"이라는 알림톡 검수 선례(§3-14)를 DM에 그대로 옮겼는데, 알림톡은 **소재 채널이 아니라 제작 패스에 들어오지 않아서** 성립한 것이었다. DM은 제작 패스·당일 실행·예외 복구·재개 네 곳이 `planned` 행을 다시 집어 승격시켰다 — 설계 검증이 잡지 않았으면 "발행 전에는 안 나간다"가 첫 한 시간 만에 무력화됐다. 같은 세션의 다른 뿌리 셋: ①**같은 날 같은 사람에게 가는 두 채널은 1통이고 캐리어는 하나다** — 형제는 같은 잠금에서 함께 선점하고 쌍의 상태 전이는 **한 문장**으로(두 문장이면 재기동 때 한 행만 남아 문자가 링크 없이 재개된다) ②**검사한 문안 = 나가는 문안** — 스팸 게이트 뒤에 링크를 붙이면 게이트가 검사한 적 없는 문안이 나간다 ③**완성 판정은 고객이 보는 실물로, 그것도 두 축으로**(렌더 문구 + 섹션 데이터 — 렌더러가 문구를 안 찍는 빈 이미지 자리가 있다) ④**GET 응답 조립은 읽기만** — 조회가 승격·통지를 하면 지난 달 화면을 여는 것만으로 옛 행이 살아나고 동시 조회가 통지를 두 번 보낸다 ⑤**CAS 0행은 실패가 아니다** — 재조회로 "이미 됨"과 갈라야 패자가 발행된 DM을 미발행으로 읽지 않는다. 구조 = [플래너 §3-20](../../docs/FEATURE-MARKETING-PLANNER.md) · 접수 = [B-0902-1](../BUGS.md).
...
## 자사몰 연동 = 우커머스(워드프레스) 전용 provider · 앱 인증 1클릭 (2026-09-14 추가)

- **"플러그인으로 연동 안 되나요"는 그 플랫폼의 표준 설치 경로 질문이다.** 워드프레스는 플러그인 설치, 카페24는 앱스토어 승인. 답은 우리 방식 설명이 아니라 그 경로를 주는 것. 우커머스에는 **앱 인증 엔드포인트(`/wc-auth/v1/authorize`)가 내장**돼 관리자 승인 1회로 REST 키가 우리 콜백에 온다(0914 두 몰 302 실측) → 플러그인 없이도 1클릭. 키를 사람이 발급·전달하는 설계는 그 뒤에야 "고급 옵션"이다.
- **콜백은 즉시 200, 무거운 일은 응답 뒤.** 우커머스가 200 을 못 받으면 승인 화면이 실패로 끝난다. 검증 1콜·웹훅 생성·백필은 `void (async …)()` 로 뒤에 돌리고 실패는 화면이 읽는 meta 한 곳(`woo_sync_error`)에 남긴다.
- **콜백 `user_id` 가 곧 우리 state** — 서명(HMAC)·1회용(DB 행 삭제)·TTL 셋이 다 있어야 남의 회사 행에 키가 꽂히지 않는다(카페24 install state 선례 복제).
- **다몰 = 식별자 접두.** 워드프레스 user id·주문 id 는 몰마다 1부터라 한 회사가 몰 여러 개를 붙이면 `{mall}:{id}` 없이는 타 몰 고객이 한 사람으로 합쳐진다(카페24·고도몰은 몰 1개라 접두가 없었다).
- **REST 요청 기준 주소 = 저장된 몰 주소(www 포함) · 리다이렉트 0.** 식별자(www 뗀 값)로 보내면 301 에 Authorization 이 묻힌다. 식별자 밖 호스트면 `https://{mall}` 로 강제(SSRF).
- **의존성 추가는 배포 함정**(0826 mailparser) — 텍스트 몇 개 zip 은 저장 방식 작성기를 직접 쓴다(`utils/zip-store.ts` · 의존성 0).
- **★ 연결 확인 1콜이 통과했다고 가져오기가 되는 것이 아니다 — 규모 상수는 첫 실몰로 잰 뒤에야 값이다 (2026-09-21 iroirotokyo.net).** 키 없이 잰 응답(1KB·0.7초)만 보고 만든 상수 넷이 첫 실몰에서 전부 틀렸다: 헤더 상한 16KB(인증 응답에만 디버깅 헤더 21KB) · 페이지 100(13.8초 · 제한 20초) · 회원 상한 5,000명 · 회원 역할 기본값(그 몰은 `bronze_member` → 0명). 그리고 **수백 페이지를 도는 일을 "한 번 실패 = 전체 실패 · 재개 없음"으로 짜면 끝나지 않는다.** 처방 = 인증된 호출로 크기·시간·건수·역할을 먼저 잰다(`scripts/diagnose-woo-headers.ts`) · 페이지마다 진행을 저장하고 워커가 이어 간다 · 상한은 업무 한도가 아니라 폭주 방지선으로 둔다 · **새 시도가 시작되면 저장된 옛 실패 문구를 지운다**(안 지우면 고친 뒤에도 고객 화면은 "동일"하다). 고객사에 원인을 넘기기 전에 우리 키로 우리가 재현한다.
- 경위·미검증 = [우커머스 설계서](../../docs/2026-09-14-woocommerce-integration-design.md) §1·§5-0·§5-0-1·§6.

## 로그인 세션 만료·다중인증 인계 (2026-09-15 추가 · [B-0915-1](../BUGS.md))

- **세션을 만들 때 만료를 길게 잡고 "활동 갱신"에서만 줄이면, 로그인 직후 닫힌 세션이 옛 토큰 요청 한 번에 되살아난다.** 고객사 로그인은 만료 24시간으로 생성되고 30분으로 줄이는 것은 로그인 5분 뒤 첫 요청뿐이었다. 5분 안에 창을 닫으면 서버 세션이 하루 유효 → 다음 날 같은 브라우저가 옛 토큰으로 화면 기록 1건을 보내자 만료 판정을 통과하고 활동 시각까지 갱신돼, 4초 뒤 본인 로그인이 "다른 곳에서 사용 중"으로 막혔다. 슈퍼관리자 경로는 처음부터 세션 시간이었다. **처방 = 생성 시점부터 유휴 기준값(회사 세션 시간)** · 같은 규칙을 쓰는 형제 경로와 한 줄 대칭으로 대조한다.
- **인증 단계 뒤 결말이 여러 갈래(성공·접속 중·국외 차단)면, 앞에서 소비한 1회용 자원과 만든 부산물을 모든 갈래 응답이 책임진다.** 인증번호는 소비되고 신뢰 기기 토큰은 등록됐는데 토큰이 성공 응답에만 실려, 409 갈래는 인증번호 재입력도 인계 동의도 못 하는 막다른 길이 됐다. 처방 = 409에도 토큰 동봉 → 인계 동의 뒤 재시도가 신뢰 기기로 통과.
- **"다른 곳에서 사용 중"의 정체는 추측하지 말고 기록 두 개를 맞춘다** — 세션 `last_activity_at`과 `audit_logs` `page_view`의 시각·IP가 같으면 그 요청이 세션을 되살린 것이다(이번엔 본인 브라우저).
- **배포 직후 "실패" 보고의 첫 판정 = 서버에 기록이 있는가.** 로그인 요청 로그·감사 기록 둘 다 없으면 요청이 서버에 닿지 않은 것이다(재기동 창). 같은 시간대 다른 회사 성공 기록으로 전면 장애 여부를 먼저 가른다.

## AI 자동제작 흰 CTA (2026-09-15 추가 · [B-0915-2](../BUGS.md))

- **엔진을 공용한다고 품질 장치가 따라오지 않는다: 장치가 엔진 밖 래퍼에 있으면 두 번째 입구는 맨몸으로 나간다.** 흰 글자 대비 보정(`accessiblePrimaryOf`)은 아웃리치 전용 조립 함수 두 곳에만 있었고, 고객 입구는 같은 엔진을 부르면서 회사 킷 주색 `#ffffff`를 그대로 실었다. 공용 엔진에 입구를 새로 붙이면 "엔진 안에 있는 것"과 "옛 입구의 래퍼에만 있는 것"을 나눠 전수로 대조한다.
- **다시 렌더되는 산출물의 보정은 저장 HTML이 아니라 재렌더 입력에 싣는다.** 이메일은 미리보기·발송이 매번 회사 킷 + 캠페인 `design`으로 다시 그린다. `html_body`만 고치면 테스트 발송은 멀쩡하고 실제 발송만 흰색이 되는 검증 착시가 생긴다.
- **편집 흔적의 출처를 DB로 못 가르면 그 값을 쓰는 코드를 전수로 센다.** 버전 기록이 0이라 버튼 색 `#ffffff`가 생성 때 들어갔는지 알 수 없었는데, 쓰는 코드가 편집기 입력 1곳뿐(프론트·백엔드 grep)이라 사람 조작으로 판정했다. 같은 조건의 다른 초안(0건)이 방증이다.
- **구조를 정하는 단계가 뒤에서 지워질 글을 모르면, 지워진 자리에 빈 껍데기가 남는다.** 고객 채우기가 본문 카드·부제·상품 카드를 먼저 만들고 혜택 차단기가 뒤에서 수치 문장을 지우자, 태그만 남은 카드 · 빈 부제 · 가격 없는 상품 카드가 나왔다(적대 검토 5건 중 3건이 이 뿌리). 처방 = 구조를 정하는 쪽이 **차단기 자체를 후보 섹션에 미리 돌려** 살아남을 글을 본다(실제 제거·계측은 차단기가 그대로). 같은 판정 함수를 줄마다 따로 부르는 짐작은 2라운드에서 또 어긋났다(차단기는 필드 전체를 보고 짧은 필드는 통째로 비운다) · 사실 수치(가격)는 문안 칸이 아니라 차단기가 보지 않는 데이터 칸(상품 슬라이드 항목)에 싣는다.

## 이메일 흰 킷 재오픈 (2026-09-19 추가 · [B-0919-1](../BUGS.md))

- **교훈을 적어 놓고 같은 결정을 하면 교훈을 안 쓴 것과 같다.** 0915에 위 절("장치가 엔진 밖 래퍼에 있으면 두 번째 입구는 맨몸")을 쓰고도 보정은 AI 자동제작 래퍼에 두고 "회사 킷 무검증 = 범위 밖"으로 넘겼다. 이틀 뒤 편집기 입구로 두 건이 재오픈됐다. 교훈이 가리키는 처방 자리(엔진 안)를 범위 밖으로 미룰 때는 그 입구가 몇 개 남는지(이번엔 5곳)를 세어 함께 적는다.
- **렌더 수정의 검증 색 = 접수 회사 원장 값.** 0916 수정은 "렌더러가 값을 읽는가"를 테스트 보라색으로만 확인했고, 접수 회사(킷 주색·강조색 `#ffffff`)에서는 새로 그린 요소가 전부 흰 바탕 흰색이었다. 접수 캡처에 회사가 보이면 그 회사 킷을 SQL로 먼저 뽑아 fixture로 쓴다. 캡처 픽셀(헤더 면 흰색 → 204)이 흰 킷 그라데이션 값과 일치하는 것으로 원인을 SQL 전에 좁힐 수 있었다.
- **대비 보정은 면 계약이다.** 흰 글씨·흰 바탕 기준 보정을 브랜드 토큰 전체에 걸면 어두운 면에서는 반대로 사라진다(첫 구현이 어두운 배경면·히어로 문구 밴드 위 장식을 `#1f2937`로 만들었다). 보정을 넣기 전에 렌더러의 어두운 면을 grep(`b.dark`·`#171717`·`onDark`)으로 전수하고, 면마다 "보정 전과 출력이 같다"를 테스트로 고정한다. 같은 부류가 두 번 나오면 세 번째 면을 찾기 전에 멈추고 목록부터 만든다.

## 0919 접수 (수신거부 중복 · 보류 충전 · [B-0919-2·3](../BUGS.md))

- **"이미 있는가" 판정은 우리 출력 형식이 아니라 사람이 쓰는 형식을 기준으로 한다.** `buildAdMessage`는 자기가 만드는 `무료수신거부 080…`만 알아봐서, 고객이 직접 쓴 `무료 수신거부 : 080…`에 한 줄을 더 붙였다(P0 · 13,518명 예약 건). 같은 파일의 제거 함수(`stripAdPartsDeep`)는 이미 띄어쓰기를 허용하고 있었다 — **붙이는 쪽과 걷어내는 쪽의 인식 너비가 다르면 그 차이만큼 중복이 난다.**
- **기능을 붙일 때는 "그 화면이 실제로 부르는 조회"를 고친다.** 0819 명의 확인 보류는 `/deposit-requests` 목록에 컬럼을 넣었지만 "한줄로 충전" 탭은 `/charge-management`의 대기 목록을 쓰고 있어, 승인 게이트만 살고 해제 수단은 죽어 있었다. 화면 필드를 추가할 때 `grep "setXxx(" → fetch 주소`로 데이터 출처부터 짚는다.
