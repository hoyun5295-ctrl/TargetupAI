...
- **★ 새 발송 경로가 큐를 직접 만들면 공용 계약이 통째로 빠진다 (2026-08-23 대행발송 · 배포 전 발견)** — 새 축이 `campaigns` INSERT + `bulkInsertSmsQueue`를 손으로 쓰자 그 자리에서 **선불 차감 0 · 후불 청구 축 0 · 결과 동기화 0 · 실패 환불 0 · 수신거부 제외 0 · 부분 적재 롤백 0 · 재시도 멱등 0**이 한꺼번에 빠졌다. 본 발송이 전액 무과금이었고 tsc·vitest·구현자 자가 리뷰 셋 다 통과했다(빠진 것을 검사하는 테스트가 없으니 당연하다). **처방 = 새 발송 경로는 큐를 스스로 만들지 않는다.** `campaign_send_staging` 적재 → `createDirectSendCampaign(sendType: ...)`으로 위임하고, **그 값을 `utils/send-type-axis.ts`에 먼저 등재한다** — 안 하면 `campaigns.send_type` DEFAULT `'ai'`로 적재되어 `campaign_runs`도 없고 `DIRECT_PIPELINE_SEND_TYPES`에도 없는 **유령 발송**이 된다(결과 동기화·환불·청구 세 축에서 동시에 사라진다. 2026-08-18 오퍼레이터 축과 같은 부류의 재발). 부수 = **치환 CT는 값을 DB 컬럼 이름으로 찾는다**(`customer[mapping.column]`). 업로드 명단처럼 컬럼이 없는 데이터는 주소록 슬롯(`%이름%`·`%기타1~3%`)으로 넘겨야 하고, 변수명을 키로 넘기면 하나도 못 찾은 뒤 안전망이 `%...%`를 지워 **전 건이 빈칸으로 발송된다**(에러도 로그도 없다). 경위 = [대행발송 설계서 §12](../../docs/2026-08-22-agency-send-design.md).
