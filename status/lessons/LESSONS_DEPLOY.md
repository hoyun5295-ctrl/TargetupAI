...
- **★ 레포가 실어 나를 산출물이 `.gitignore` 전역 패턴에 걸리면 push가 조용히 성공하고 서버에만 없다 (2026-08-16 진단 seed)** — 문항 seed를 `scripts/sql/*.sql` 파일로 만들어 tp-push했는데 `.gitignore`의 `*.sql`(데이터 덤프용 전역 무시)에 걸려 **커밋에서 빠졌다.** git은 경고하지 않고 push도 성공하며, 서버에서 `git pull`이 `Already up to date`를 찍은 뒤 `No such file or directory`로만 드러난다. **처방 = ①레포가 실행·배포에 쓰는 산출물(seed·마이그레이션·픽스처)을 새 폴더에 만들 때는 그 폴더가 무시 패턴에 걸리는지 먼저 확인한다(`git check-ignore -v <경로>` 한 줄) ②전역 패턴은 유지하고 폴더만 예외로 뚫는다(`!scripts/sql/*.sql`) ③예외 줄에 사고 경위를 주석으로 남긴다** — 다음 사람이 "왜 예외가 있지"로 지우면 같은 사고가 재발한다. 부수: 긴 SQL을 **터미널에 붙여넣지 마라**(입력 버퍼·자동 개행으로 중간이 잘려도 JSON이 우연히 파싱되면 조용히 불완전한 행이 들어간다). 표준 = 파일 + `docker exec -i … psql -U … < 파일`, 확인은 `jsonb_array_length` 같은 **내용 카운트 SELECT**(행 존재 확인만으로는 잘림을 못 잡는다).
- **★ pre-push를 막은 게 "테스트 실패"가 아니라 "테스트가 느려서"일 수 있다 (2026-08-17 마케팅 진단 v8 push 차단)** — 실패 출력이 `Test timed out in 5000ms`면 단정 실패가 아니라 **타임아웃**이다. 불변식 위반이 아니라 그 테스트가 못 끝난 것이고, 원인은 대개 소스 전수 스캔이 **테스트마다 트리를 다시 읽는** 구조다(실측 573파일·12MB × 불변식 3 = 동기 읽기 1,719회. 168개 테스트 파일이 병렬로 도는 러너 + 실시간 검사 환경에서 기본 5초를 넘긴다). **처방 = 타임아웃을 늘리는 게 아니라 루트별 1회만 읽어 캐시한다**(`src/utils/__tests__/source-scan.ts` — 테스트 전용 공용 스캐너). 2026-08-03에 `brand-axis-invariants`만 60초로 늘려 둔 적이 있는데, 그 증상 처치가 뿌리를 남겨 다른 파일에서 그대로 재발했다. **같은 형태의 스캔 테스트를 새로 만들 때는 자기 walker를 두지 말고 이 스캐너를 쓴다.** 판별 = `readdirSync` grep(테스트 파일 한정).
- **★ 의존성이 늘어난 배포에서 `npm install` 단계를 빠뜨리면 백엔드가 크래시 루프에 빠진다 = 전 API 다운 (2026-08-26 대행발송 이메일 접수 · 3분)** — 신규 의존성(`mailparser`)을 추가하고 배포 인계문에 서버 `npm install`을 안 적었다. **safe-build는 `node_modules/typescript`가 있으면 npm install을 건너뛰고**(0815에 이미 기록된 함정), 백엔드는 **ts-node가 `src/app.ts`를 직접 실행**하므로 모듈이 없으면 컴파일 자체가 실패한다 — 빌드는 통과했는데 pm2가 07:42부터 재시작을 반복했고 API 전체가 죽었다(로그 지문 = `error TS2307: Cannot find module '<패키지>'` + 그 뒤 줄줄이 `TS7006 implicitly has an 'any' type`, ↺ 카운트 증가). **처방 = ①`package.json` 의존성이 바뀐 배포는 인계문에 `cd packages/<pkg> && npm install`을 반드시 넣는다(OPS §2-2 2번) ②`.env`를 함께 바꿨으면 재시작은 `pm2 restart <앱> --update-env`다(reload는 env를 다시 읽지 않는다) ③배포 후 확인은 pm2 status의 ↺ 증가 여부 + 그 기능의 부팅 로그 한 줄로 한다("online"만으로는 크래시 루프를 못 가른다).** 부수 = 서버 npm install이 lock 파일을 고쳐 다음 `git pull`이 거부될 수 있다(OPS §2-2 4-1 · lock만 버린다). ⛔ **진단 순서**: 워커·기능 로그가 0줄이면 "로그가 밀렸나"를 의심하기 전에 **에러 로그부터** 본다 — 이번엔 out 로그가 비어 있고 error 로그에 전부 있었다. 경위 = 대행발송 설계서 §11 0826(3)행 · memory `feedback_deploy_handoff_follows_ops_doc`.
- **★ `pm2 reload`가 "✓"를 찍어도 프로세스가 안 바뀔 수 있다 — 판정은 uptime과 dist 시각 대조로 한다 (2026-09-04 정산·DM 배포)** — build:safe 두 개를 돌리고 `pm2 reload targetup-backend`를 실행해 `[PM2] [targetup-backend](1) ✓`까지 받았는데, **프로세스는 26분 전 것 그대로였다.** Node는 시작 시점에 모듈을 읽으므로 그 뒤 `dist`가 바뀌어도 **이미 로드된 코드는 안 바뀐다** — 화면·API는 옛 동작을 그대로 하고, `pm2 status`는 `online`이라 아무 신호가 없다. 그냥 넘어갔으면 "배포했는데 그대로"가 되고, 그 상태로 정산서를 발행했으면 옛 로직으로 청구된다. **판정 = ①`pm2 describe <앱> | grep -iE "uptime|created at"` 의 uptime이 배포 시각 이후로 리셋됐는가 ②`stat -c '%y %n' <dist의 변경 파일>` 과 프로세스 시작 시각 비교 — 파일이 더 나중이면 그 코드는 안 실려 있다.** `pm2 status`의 `online`·`↺` 증가만으로는 못 가른다(↺는 이전 값을 모르면 무의미하고, online은 옛 코드로도 online이다). ⛔ **dist에 코드가 들어간 것(`grep -rl` 로 확인)과 그 코드가 실행 중인 것은 다른 사실이다** — 이번에 dist 4파일 반영은 확인됐는데도 프로세스가 옛 것이었다. 배포 확인은 **산출물 축과 프로세스 축을 따로** 밟는다.
- **★ 배포 명령을 기억으로 재구성하지 마라 — `status/OPS.md` §2-2에서 복사한다 (2026-08-16 재발)** — 레포 루트에서 `npm run build:safe`를 안내해 `Missing script`로 배포가 멈췄다. **build:safe는 루트 스크립트가 아니라 `packages/backend`·`packages/frontend` 각각의 스크립트**이고, 프론트가 바뀌었으면 두 번 돌린 뒤 `pm2 reload targetup-backend`까지가 한 벌이다. 안내문은 매번 OPS.md 해당 절을 열어 그대로 옮긴다 — "지난번에 이랬지"는 근거가 아니다.
