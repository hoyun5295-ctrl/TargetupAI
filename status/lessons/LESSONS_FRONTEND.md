...
- **★ 마스터-디테일에서 목록을 접지 않으면 둘 다 좁아진다 (2026-08-22 Harold "너무 보기 힘들다")** — 컬럼 10개짜리 표 옆에 440px 상세 패널을 붙였더니 표는 컬럼이 잘리고 패널은 필터 칩이 가로 스크롤에 갇혀, 패널이 표 위에 얹힌 것처럼 보였다. **상세를 열면 목록은 식별 최소값(이름·번호·등급)만 남기고 300px로 접는다** — 그래야 패널이 화면을 넉넉히 쓴다. 닫으면 표가 전체 폭 복귀. 덧붙여 패널이 1,000px을 넘으면 **읽기 폭(`max-w-[880px]`)** 을 따로 잡는다 — 줄이 끝까지 퍼지면 오히려 읽기 나쁘다.
