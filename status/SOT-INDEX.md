...
| 트랙 | SoT 문서 | 읽는 범위 | 호출어 |
|---|---|---|---|
| **자사몰 연동 × 사용자 분류코드 설계서** — 2026-09-18 이에스페이먼트(몰 4 · 몰별 사용자 4) 발단 · 상시 원칙(관리자 전체 · 사용자는 분류코드로 자기 것만)을 자사몰 연동에 적용 · 연동 행 분류코드 + 자사몰 적재의 `customer_stores` 기록 + 사용자 자기 몰 연결 + 격리 CT 폴백 정정(설계안 · 승인 대기 · 코드 0 · DDL 0) *(기능 상설 = §0 **자사몰 연동**)* | [2026-09-18-mall-integration-user-scope-design.md](../docs/2026-09-18-mall-integration-user-scope-design.md) | 착수 전 **§1-2 구멍 3개 → §2 불변 원칙 10개 → §4 영향표 → §5 단계(P0 측정 SQL 먼저)** · 실측 §6 · 미검증 §7 · 별건 §8(여정은 분류코드를 안 본다) | **몰별 사용자 연동** |
