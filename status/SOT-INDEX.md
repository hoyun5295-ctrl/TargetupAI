...
| **AI 자동제작** — 재료(행사 카드·상품·이미지·기능 칩) → 버튼 1 → DM·이메일 완성본(원클릭 캠페인 승격 · 신규 엔진·키·DDL 0) | **AI 자동제작** | [2026-09-14-ai-auto-build-design.md](../docs/2026-09-14-ai-auto-build-design.md) | **§2 불변 원칙 14개(필독 — 돈 단위 = attemptToken · 가격·링크 AI 미경유 · 몰 재조회는 상품번호 · 칩은 엔진이 만드는 것만 · 신규 ENV 비면 미노출) → §3 수렴 판정 → §5 요금 → §6 서버 계약 → §12 착수 원장 T0~T8.** 회의론자 최종 8건 = §10 · 미검증 = §11. 상설 문서는 구현 종결 뒤 신설 |
| **우커머스 연동** — 워드프레스 자사몰 전용 provider(고도몰 뼈대 · REST 백필 + 기본 웹훅 + Store API 상품 · 다몰 접두 · DDL 0) | **우커머스 연동** | [2026-09-14-woocommerce-integration-design.md](../docs/2026-09-14-woocommerce-integration-design.md) | **§2 불변 원칙 9개(필독 — 몰 식별자 함수 하나 · `{mall}:{id}` 접두 · 매출은 paid·completed만 · 저장≠연결 · 웹훅 형식은 게이트 ② 전까지 미검증) → §4 매핑 계약 → §5 착수 원장 W1~W8.** 미검증 = §6 · 고객사 후속 = §7. 종결 뒤 FEATURE-CDP-INTEGRATION §4에 흡수 |
