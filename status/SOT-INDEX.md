...
| **고객 360** — 고객 1명 기준 발송·반응·구매·동의·자동화·문의 시간순 뷰 (AI CRM 1축) | **고객 360** | [FEATURE-CUSTOMER-360.md](../docs/FEATURE-CUSTOMER-360.md) | **§2 불변 원칙 13개(필독 — 발송은 MySQL만 · 번호 조회에 회사 조건 · 식별 축 · 시각 축) → §3 구조 → §5 이력·교훈(§5-2 타사 노출 사고 · §5-1 화면 4회 왕복).** 시점 근거 = 1차·v2 설계서(문서 머리 표) |
| **기능 카탈로그 · 도움말 봇** — "고객이 하려는 일" 단위 정의 원장(코드 상수 1파일) + 온보딩·도움말 겸용 봇 + `/guide` 안내 화면(원고 0장). 실사용 고객 0 전제 | **도움말 봇** / **기능 카탈로그** | [FEATURE-HELP-CATALOG.md](../docs/FEATURE-HELP-CATALOG.md) | **§0 30초 요약 → §3 불변 원칙(필독 — 요금제 이름·크레딧 숫자 문자열 금지 · 매칭 실패면 모델 호출 0) → §4 구조 → §5 게이트 8종.** 정의 추가 = `content/feature-catalog.ts` 집필 규약. **다음 세션 착수 원장 = §10** |
...
| **SNS 채널** — 회사 SNS 계정 OAuth 연결 → 한 번 쓰면 채널마다 규격 맞춰 게시·예약 (인스타·Threads 실동작 · 페북·X 1차-B) | **SNS 채널** / **SNS 게시** | [FEATURE-SNS-CHANNEL.md](../docs/FEATURE-SNS-CHANNEL.md) | **§2 불변 원칙 8개(필독 — 저장≠연결 · 회사 조건 직접 · 성공은 재조회로만 · 이중 게시 3겹) → §5 ⛔ 원본 무접촉 규칙 → §3 파일별 소유.** 채널 추가 = §4 · 운영·ENV = §6 · 남은 것 = §7 · 뒤집힌 판단 = §8. **실측 원문·회의록·예정 DDL = 시점 설계서**([2026-09-17](../docs/2026-09-17-sns-publish-design.md) §1-4·§1-5·§3-1·§8) |
...
| **AI 영업 아웃리치** — 업체 홈페이지 → 맞춤 산출물 자동 제작 → 자사 메일 인계(슈퍼관리자 ceo 전용 · 내부 영업 도구) | **영업 아웃리치** | [FEATURE-SALES-OUTREACH.md](../docs/FEATURE-SALES-OUTREACH.md) | **§2 불변 원칙 17개(필독 — 발송 경로는 사람 클릭 하나 · DB 선점 뒤에만 발송 · 3중 잠금 · 행사는 인용하고 서버가 재대조 · 자산 INSERT는 소유권 결속)** → §3 파일별 소유 → §4 ENV·테이블. 이력·뒤집힌 판단 = §5. 시점 근거 = 문서 머리 표(0731 설계서 §15가 현행) |
| **대행발송 셀프 접수** — 직원 수작업을 기계로(승인만 담당자) · **입구 3 = 화면·요청서 원스텝·이메일** | **대행발송** | [FEATURE-AGENCY-SEND.md](../docs/FEATURE-AGENCY-SEND.md) | **§2 불변 원칙 17개(필독 — 승인·당일검사 없는 발송 0 · 적재는 직접발송 배관 위임 · 정산 축 새 값 금지 · 접수 쓰기는 코어 하나 · 타임스탬프 토큰 금지 · 이메일 7개)** → §3 파일별 소유 → §4 운영 설정. 이력·뒤집힌 판단 = §5. 시점 근거 = 문서 머리 표(0822 설계서 §12·§14·§15~§17·**§18 이메일**) · 운영 명령 = [OPS.md §2-2-D](OPS.md) |
| **AI 자동제작** — 재료(행사 카드·상품·이미지·기능 칩) → 버튼 1 → DM·이메일 완성본(원클릭 캠페인 승격 · 신규 엔진·키·DDL 0 · 0914 배포완료 · 0915 전 회사 개방) | **AI 자동제작** | [FEATURE-AI-AUTO-BUILD.md](../docs/FEATURE-AI-AUTO-BUILD.md) | **§2 불변 원칙 15개(필독 — 돈 단위 = attemptToken + 과금 지문 · 판독비는 초안 뒤 정산 · 가격·링크 AI 미경유 · 몰 재조회는 상품번호 · 칩은 엔진이 만드는 것만 · 신규 ENV 비면 미노출 · `*` = 전 회사) → §3 구조 → §4 요금·ENV → §5 실측 5건(미실행 · ★0915 §7 직원 테스트로 대체 · ENV 비우기 = 전 회사 영향).** 이력·뒤집힌 판단 = §6 · 남은 것 = §7. 시점 근거 = [0914 설계서](../docs/2026-09-14-ai-auto-build-design.md)(§13 구현 기록 · §14 배포 원장) |
| **카탈로그 DM** — 장마다 이미지 1장인 슬라이드 DM을 PC에서 책처럼 두 쪽 펼침(고른 DM만 `settings.catalog` · 휴대폰은 슬라이드 그대로) · 입구 3 = DM 카드·토글 / AI 자동제작 채널 10크레딧 / AI 영업 자동 조립(AI 0) · DDL 0 · 0915 배포 | **카탈로그 DM** / **메이크뷰** | [FEATURE-CATALOG-DM.md](../docs/FEATURE-CATALOG-DM.md) | **§0 30초 요약 → §2 불변 원칙 11개(필독 — 고른 DM만 · 자격은 플래그와 AND · 조건 밖 바이트 동일 · 쪽 조립 CT 한 곳 · 추적 무변경 · 이미지 글자 숫자 0) → §3 파일별 소유 → §4 입구 3곳.** 요금 = §5 · 추적 = §6 · 실측 원장 = §7 · 이력 = §8 · 남은 것(Harold 「수정 필요」 접수 대기) = §9 |
| **우커머스 연동** — 워드프레스 자사몰 전용 provider(고도몰 뼈대 · 앱 인증 1클릭 + REST 백필 + 기본 웹훅 + 플러그인 zip + Store API 상품 · 다몰 접두 · DDL 0 · 0914 전량 배포완료) | **우커머스 연동** | [2026-09-14-woocommerce-integration-design.md](../docs/2026-09-14-woocommerce-integration-design.md) | **§2 불변 원칙 12개(필독 — 몰 식별자 함수 하나 · `{mall}:{id}` 접두 · 매출은 paid·completed만 · 저장≠연결 · 앱 인증 state 서명·1회용·TTL · 콜백 즉시 200 · 플러그인 비밀 0 · 웹훅 형식은 게이트 ② 전까지 미검증) → §1 실측 → §4 매핑 계약 → §5 원장 W1~W10(운영 첫 승인 = W8).** 결정 = §5-0 · 미검증 = §6 · 고객사 후속 = §7. 종결 뒤 FEATURE-CDP-INTEGRATION §4에 흡수 |
| **비토 링크가드** — 문자 URL 을 KISA 신고·수동 목록과 대조해 발송 전 차단(판매 제품 · 관제센터 Go 단일 바이너리 + DBMS 무관 에이전트 + 검사 API + 게이트웨이 훅) | **링크가드** | `projects/linkguard/docs/FEATURE-LINKGUARD.md`(별도 저장소) · 게이트웨이 접점(훅·ENV·정책 토글·배포·운영·실측) = [FEATURE-GW-LINKGUARD.md](../docs/bito-gateway/FEATURE-GW-LINKGUARD.md)(피더 · 호출어 **게이트웨이 링크가드**) | **§3 불변식 9(필독 — 큐당 세션 1 · held 고아 0 · 잠금 안 재평가 · 해시 세대 · 원문 보존 · 전환 순서 · 토큰 1회 · advisory seq · KISA 평문 0) → §1 구조 → 작업 축(§5 계약 · §7 에이전트 · §8 게이트웨이 · §13 배포).** 이력·Codex 4R = §14 · 착수 원장 = §15 |

| **규격 밖 글자 처리** — 문자망(CP949)에 없는 글자를 고객 클릭·계정 동의로 가장 가까운 글자로 대체 · 반려는 즉시 영구 실패(갇힘 0) | **규격 밖 글자** | [2026-09-10-unsupported-char-substitution-design.md](../docs/2026-09-10-unsupported-char-substitution-design.md) | **§2 불변 조건 7개(필독 · 몰래 바꾸기 0 · CP949 에 있는 글자 불변)** → §3 대체표 v1 → §4 구조 → §6 단계(P1~P4 각각 승인). 실측 근거 = §1(젬텍 UTF-8 10건 · .58 치환) |
| **서버 용량 · DB 분리 서버** — 수용 한계 산식(1명당 1.9KB 실측) · 현재 서버 계획선 1,000만 · 첫 증설 = PG 전용 DB 분리 서버 사양·절차 항목(D94) | **서버 용량** / **DB 분리 서버** / **수용 고객 수** | [FEATURE-INFRA-CAPACITY.md](../docs/FEATURE-INFRA-CAPACITY.md) | **§0 30초 요약 → §2 산식·기준·트리거(필독) → §4 사양·수용 표.** 실측 원문 = §1 · 선택지·결정·슈퍼관리자 중앙화 점검 = §3 · 분리 절차 항목 = §6(착수 시 설계서) · 측정 명령 = §8. 서버 원장(허브) = `status/SERVERS.md` · 02-25 멀티노드 = `archive/DESIGNS/SCALING.md` |
...
| 트랙 | SoT 문서 | 읽는 범위 | 호출어 |
|---|---|---|---|
| **한줄로 소스 전수점검 장부** — 2026-09-25 속도·숨은 결함·헛도는 작업·데이터 낭비 + 개선 여지·올드 디자인 메모 · 점검 → 운영 측정 → 브레인스토밍 → 건별 트랙 · 직접 점검(에이전트 대량 투입 중단) | [2026-09-25-hanjul-source-audit.md](../docs/2026-09-25-hanjul-source-audit.md) | 재개 시 **§1 진행표 → §3 측정 대기 → 다음 부분**. 확정 결함 §2 · 1차 후보(미검증) = [candidates](../docs/2026-09-25-hanjul-source-audit-candidates.md) | **한줄로 전수점검** |
| **몰별 수신동의·수신거부 격리 설계서** — 2026-09-22 이에스페이먼트 4몰 실데이터 발단 · H1 동의 단위 = 몰 · H2 몰 간 영향 0 · 고객 1행 + 소속 행이 동의 소유 · 회사 공통 동의 퇴역 · 수신거부 입구 격리 · 단계 S0~S7 · 호출어 **몰별 수신동의 / 몰 동의 격리** | `docs/2026-09-22-mall-consent-isolation-design.md` | D93 · BUGS B-0922-1·2 | D93 · BUGS B-0922-1·2 |
| **자사몰 연동 × 사용자 분류코드 설계서** — 2026-09-18 이에스페이먼트(몰 4 · 몰별 사용자 4) 발단 · 상시 원칙(관리자 전체 · 사용자는 분류코드로 자기 것만)을 자사몰 연동에 적용 · 연동 행 분류코드 + 자사몰 적재의 `customer_stores` 기록 + 사용자 자기 몰 연결 + 격리 CT 폴백 정정(**P1~P4 구현 완료 · 배포완료 0918 · 실측 대기** · CT-77·78 신설 · DDL 0 · P5 = 나머지 5 provider 별건) *(기능 상설 = §0 **자사몰 연동**)* | [2026-09-18-mall-integration-user-scope-design.md](../docs/2026-09-18-mall-integration-user-scope-design.md) | 재개 시 **§9 구현 기록 → §6 실측 시나리오(이에스페이먼트 연결) → §7-4 `users.store_codes` 타입 SQL** · 별건 §8 = 항목마다 "왜 이번 축이 아닌가·위험·재개 조건"(8-1 여정이 분류코드를 안 본다 = 가장 큰 것) | **몰별 사용자 연동** |
| **SNS 게시 설계서** — 2026-09-17 전원합의체 수렴 · 고객사가 자기 SNS 계정을 OAuth로 연결 → 사진·영상 업로드 → AI 캡션 다듬기 + 고정 세트에서 태그 선택 → 게시·예약(성공 = 플랫폼 재조회) · 1차-A 인스타·Threads → 1차-B 릴스·페북·X → 2차 고객 개방·자동제작·플래너 합류(신규 트랙 · 코드 0 · DDL 0) | [2026-09-17-sns-publish-design.md](../docs/2026-09-17-sns-publish-design.md) | 착수 전 **§2 불변 원칙 20개 → §3-5 상태 머신 → §5 단계·게이트(S0 raw curl 먼저)** · 회의록 §8 · 미검증 §7 · 별건 §9 | **SNS 게시** |
| **캠페인 엔진 설계서** — 2026-09-06 회의 4단계 수렴 + Harold 순서 확정(AI 영업 먼저) · S1 렌더 크롤 구현 원장 · S2~S6 수렴안 *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-09-06-campaign-engine-design.md](../docs/2026-09-06-campaign-engine-design.md) | 착수 전 **§2 세션 순서 → 해당 세션 절**(S1 §3 · S2 §4 · S3 §5 · S4 §6 · S5 §7 · S6 §8) · 뿌리 5개 §0 · 범위 밖 §9 | **캠페인 엔진** |
| **AI 영업 직접 발송 설계서** — 2026-09-23 브레인스토밍 5역할(1차 → 교차 1R → 회의론자 최종 14항목) 수렴 · 담당자 직접 발송 단계 1·2·3 · 발송 원장·수신거부 원장 · 엑셀 7열 자동 영업 · 제안 메일 첫 화면 재구성 · 작업대 · 네이버 스토어 저장만 *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-09-23-outreach-direct-send-design.md](../docs/2026-09-23-outreach-direct-send-design.md) | 착수 전 **§3 불변 44~51 → §5 발송 흐름 → §6 단계 조건** · DDL §4(실행 완료) · 계약 §14 | **AI 영업 직접 발송 / 작업대** |
| **AI 영업 B 네이버 스토어 전용 판독기 설계서** — 2026-09-24 북마크 판 2(스토어 상태 허용 칸) · 판독기(기획·상품·관심고객수) · 확인 화면 후보 합치기 함수 하나 · 저장본 업로드 공용 *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-09-24-outreach-naver-store-reader-design.md](../docs/2026-09-24-outreach-naver-store-reader-design.md) | 착수 전 **§1 원본 사실 → §2 설계 → §5 실측(다른 스토어 2건)** | **네이버 스토어 판독기** |
| **AI 영업 산출물 품질 A 설계서** — 2026-09-24 톤28 실측 결함 6종(포스터 제목 · 행사 카드 세 조각 · [영업] 제목 노출 · 메일 중복 · 포스터 배경 소품 · 카탈로그 캡션·여백) *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-09-24-outreach-quality-a-design.md](../docs/2026-09-24-outreach-quality-a-design.md) | 착수 전 **§1 축 대조표 → §3 영향표** · 추가 과제 §4 | **AI 영업 품질 A** |
| **AI 영업 v3 설계서(브랜드 페이지 재구성 엔진)** — 2026-09-06 전원합의체 2라운드 + 회의론자 최종 검증 · 이벤트 목록 카드 1순위 · 설명 없는 이미지 0 · 상품 밀도 배관 · 갤러리 0 · 학습 원장 · 고객 재료 카드 3 · **다음 세션 구현(T0~T8 · 검증 17건 반영)** *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-09-06-outreach-v3-brand-page-recomposition-design.md](../docs/2026-09-06-outreach-v3-brand-page-recomposition-design.md) | 착수 전 **§1 명세 6문장 → §7 DM 표준 → §13 구현 순서** · 갈린 지점 §3-2 · 검증 §16 · information_schema §11 | **AI 영업 v3 / 브랜드 페이지 재구성** |
| **AI 영업 아웃리치 시점 설계서** — 0731 1차 구상(§1~§14) + **0824 v2 현행(§15)** *(기능 상설 = §0 **AI 영업 아웃리치**)* | [2026-07-31-ai-sales-outreach-design.md](../docs/2026-07-31-ai-sales-outreach-design.md) | 상설 문서에서 링크로 들어간다. 근거가 필요할 때만: 브레인스토밍 수렴 §15-1~§15-8 · Codex 검증 이력·잔여 부채 §15-9-1 · 대량 업로드 §15-9-2 · Harold 확정 현황 §15-10 | **영업 아웃리치** |
| **대행발송 시점 설계서** — 0822 1차 구상 + 0823 정정(§12·§14) + 0825 화면·링크 승인·원스텝(§15~§17) + **0826 이메일 접수(§18)** *(기능 상설 = §0 **대행발송**)* | [2026-08-22-agency-send-design.md](../docs/2026-08-22-agency-send-design.md) | 상설 문서에서 링크로 들어간다. 근거가 필요할 때만: 정정 경위 §12 · 재검사 폐지 §14 · 링크 승인 §16 · 원스텝 §17 · **이메일 = §18-2 구조 · §18-3 확정 조건 · §18-6 멱등 · §18-8 검증 15건 · §18-10 DDL · §18-12 실측 7** | **대행발송** |
| **고객 360 시점 설계서 2종** — 1차(타임라인·확정 사실·격리 결함 §6-R-2) · v2(2열·검색·접기) *(기능 상설 = §0 **고객 360**)* | [1차](../docs/2026-08-22-customer-360-timeline-design.md) · [v2](../docs/2026-08-22-customer-360-v2-design.md) | 상설 문서에서 링크로 들어간다. 시점 근거·실측 원장·구현 결과가 필요할 때만 |
...
| **콘솔 화면 톤 통일 — 라이트 + 인디고** (카카오&RCS · 발송결과 · 수신거부 · 회사설정 · **슈퍼관리자 고객사 수정**) | [2026-08-18-console-ui-unification.md](../docs/2026-08-18-console-ui-unification.md) | 화면 손대기 전 **§2 불변 원칙 6개(필독 — ⛔3 모달 껍데기에 backdrop-filter·transform 금지)** → §3 무엇이 어디를 소유하나. 이력 §4(**§4-4 = 0826 고객사 수정 탭 재구성·실험실 강등**) · 남은 것 §5 · 뒤집힌 판단 §6. 값 자체는 `utils/console-ui.ts`가 소유 | **콘솔 톤** |
