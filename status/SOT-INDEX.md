...
| **기능 카탈로그 · 도움말 봇** — "고객이 하려는 일" 단위 정의 원장(코드 상수 1파일) + 온보딩·도움말 겸용 봇 + `/guide` 안내 화면(원고 0장). 실사용 고객 0 전제 | **도움말 봇** / **기능 카탈로그** | [FEATURE-HELP-CATALOG.md](../docs/FEATURE-HELP-CATALOG.md) | **§3 불변 원칙(필독 — 요금제 이름·크레딧 숫자 문자열 금지 · 매칭 실패면 모델 호출 0 · 봇 컨텍스트는 카탈로그 공개 필드뿐) → §4 구조 → §5 게이트 8종.** 정의 추가 = `content/feature-catalog.ts` 집필 규약. 상태 §9 |
