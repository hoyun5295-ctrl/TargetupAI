...
## ★ 진행 상태 (2026-08-16(2) 갱신)

**§1 전량 완료 · §2-6의 ①②까지 완료.** 결과·경위·교훈은 **허브 §7-4가 소유한다**(여기 재서술 금지).
이 문서에 남은 것은 **§2-6 ③④⑤ 뿐** — 감사 로그·통계 → 과금·정산 → 나머지 페이지에 패턴을 기계적으로 이식하는 일이다.

이식할 때 쓸 것(이번 회차에 신설·검증 완료):
- 클래스 = `console.css` 의 `.gw-page-head` · `.gw-filter-bar`/`.gw-filter` · `.gw-id` · `.gw-tbl--rows`
- 컴포넌트 = `ui.jsx` 의 `PageHead` · `FilterBar` · `Filter` · `IdCell` · `TypeBadge`
- 본보기 = `MessageHistoryPage.jsx`(그대로 따라 쓰면 된다)
- 게이트 = `npm run test:token-roles` 가 색 역할 위반을 잡는다. 신규 인라인 style에 리터럴 색을 쓰면 여기서 걸린다

⚠ **§1의 좌표는 실측과 달랐다.** §1-1은 1곳이 아니라 14곳이었고 "「-line」이 color에 쓰인 곳은 전부 결함" 판정에는 오탐이 3건 있었다. §1-2는 4곳이 아니라 8곳이었고 그중 셋(알림 워커·고객사 웹훅)은 조회 표시가 아니었다. **아래 §1 본문은 그때의 판단 기록으로 남긴다 — 지금 좌표로 쓰지 말 것.**
