...
**2026-08-17 조치** — 조기 return 3곳 + 예외 경로 2곳(안쪽 catch·최상위 catch)을 전부 닫았다(§9-1-C). 카카오 활성 검사는 run INSERT 앞으로 옮겼고, 나머지는 `failCampaignRun`으로 종결한다.

### §9-3-A. 남은 같은 가족 — **취소 경로도 실행 행을 남긴다** (미착수)

배포 후 실측에서 **네 번째 사례**가 나왔다. 캠페인 `748ead6a`(테스트계정, 2026-03-13 생성·3-30 예약)는
`campaigns.status='cancelled'`인데 `campaign_runs`만 `scheduled`로 넉 달 반 남아 있었다.
취소 경로는 실행 행도 `cancelled`로 바꾸게 돼 있는데([campaign-lifecycle.ts:332](../packages/backend/src/utils/campaign-lifecycle.ts:332)) 이 건은 안 바뀌었다 —
그 시점에 그 코드가 없었거나, 취소가 다른 경로(`cleanupScheduledCampaigns` 등)로 일어났다는 뜻이다.

**발송 영향은 0으로 확정** — MySQL `SMSQ_SEND_14`에서 `app_etc1` 조회 결과 **0건**(Harold 실행). `target_count=25,656`은 대상 규모일 뿐 적재는 없었다. 회사도 `테스트계정`(postpaid)이라 고객 영향 없음.
⚠ 이 확정 과정에서 배운 것 — **후불 업체는 `balance_transactions`에 행이 안 생긴다.** 선불 원장 0행을 "발송 없음"의 근거로 쓰면 틀린다(그렇게 판정했다가 Harold님이 정정). 발송의 진실은 MySQL 큐뿐이다.

착수 판단은 Harold님 몫. 착수 시 §9-3의 처방 방향 ②(상태 전이 규약·sweeper·재발송 UX 확인)를 그대로 적용한다.

## §9-4. 예약 발송이 "안 나간 것"으로 읽히던 축 — 2026-08-17 정정·**배포완료**(예약내역 탭 실측 확인)

배포 후 Harold님 실사용에서 드러났다. **발송은 정상인데 화면이 예약을 숨기고 있었다.**

| 결함 | 실측 | 정정 |
|---|---|---|
| AI 오퍼레이터가 예약·즉시를 뭉갠다 | 기본 모드가 `aiRecommended`라 **승인만 하면 예약**인데([AiOperatorPage.tsx:327](../packages/frontend/src/pages/AiOperatorPage.tsx:327)), 결과 카드는 예약이어도 "발송 처리 완료 · 발송 성공 0건" | 예약이면 **"예약 완료 + 발송 예정 시각"**, 성공·실패 지표는 미렌더(0이 실패로 읽힌다). 승인 **전** 확인 단계도 신설 |
| 예약분이 목록에서 사라진다 | 발송결과 목록이 기간 필터를 타는데 기본이 오늘까지 → **내일 예약분이 안 보임** | `results.ts`에 `scope=scheduled` 추가 — 그 모드는 **기간 필터를 걸지 않는다** |
| 예약 조회 입구가 없다 | `setShowScheduled(true)` 호출부 **0곳**(타겟 추출 탭 카드 4개 삭제 때 입구가 같이 사라짐) | 발송결과 모달을 **3탭(전송결과 / 예약내역 / 테스트발송내역)**으로 나눠 제자리에 복원 |
| 탭 활성색이 죽어 있다 | `border-${tab.color}-500` 런타임 조립 — safelist 부재로 CSS 미생성 | 정적 클래스로 교체(KakaoRcsPage와 같은 부류) |

**예약내역 탭 규약** — 기간 UI를 두지 않는다(Harold 확정: "예약된 문자를 다 보여줘야지"). 예약은 아직 안 나간 것이라 유한하고, 자르면 이번 사고가 그대로 재발한다. 요약 카드도 결과 축(성공·실패·비용) 대신 **예약 건수·발송 대상·가장 이른 발송 예정**을 쓴다. 목록 렌더러는 전송결과와 **공유**한다(복제하면 한쪽만 고쳐진다).
