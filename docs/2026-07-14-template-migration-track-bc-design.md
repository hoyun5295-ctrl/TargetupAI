...
| 항목 | 확정 내용 |
|---|---|
| 엔드포인트 | **`http://58.227.193.58:25230/tmpl-mgr`(58) · `http://58.227.193.54:25230/tmpl-mgr`(54)** — ★정의서 3·4장 원문이 두 IP를 직접 명시("외부에서 요청시에는 127.0.0.1 대신 58.227.193.58(현재 테스트/서비스 가능) 또는 58.227.193.54로 변경하여 호출"). **포트·경로는 두 서버 공통 25230 / `/tmpl-mgr`** — 추정 아님. **54=P코드 계정 / 58=R코드 계정**(서팀장 0720). 0720 시점 가동 = 58만, 54는 당일 야간 포팅 완료 통지 |
| 문자셋 | **utf8만 사용**(정의서 2장 명시 — json 변수명은 전부 소문자). 강문희 구두 고지의 "54는 EUC-KR·DB charset latin1"은 **DB 저장 쪽 이야기이며 API 규약과 별개** → 그래도 한글 `billnm` 왕복 실측은 개통 조건으로 유지 |
| 화이트리스트 등재 | 정의서 에러코드 장: "White list는 **2026.07.20일 현재 58.227.193.62 등록**되어 있음" = 그 시점 가동 중이던 **58 기준**. 54는 당일 포팅이라 별도 등재 필요 |
...
| **54 인코딩** | ~~54는 표준 한글이 EUC-KR·DB charset latin1이라 추가 확인 필요~~ → **★0720 야간 해소**: 한글 변환은 54 `tmpl-mgr` 엔진 내부에서 처리하며 **외부 API는 58과 동일하게 UTF-8 통일**(강문희 회신 + 정의서 2장 "문자셋은 utf8만 사용"). **클라이언트 인코딩 분기 불필요.** P0001 한글 `billnm` 왕복 실측 통과(§4-9-J) |
...
- **알림 억제 백필 완료** — `imported-alarm-suppress billIds=["R0007"]`로 0715분 847건(APPROVED 827·KREJ 20) 처리, 잔존 0. **5분 폴링 대상 747 → 0**. 알림 SMS 발화 0건(로그 전량 `alarmNotify skip count=0`). 6초 간격으로 두 번 호출됐으나 두 번째는 `updated=0`으로 멱등 동작 확인.
- **이관 후 최종 집계(운영 DB)** — 연결회사 30 · 이관회사 템플릿 **4,426**(신규 3,579 + 아난티 기존 847) · 프로필 **199**. 아난티 913건(889 APPROVED + 24 REJECTED) 전량 알림 상태 기록.

### J. ★0720(9) 54 개통 시도 — 62→54 TCP 전체 차단 확인(강문희 회신 대기)

강문희 메일 "54서버 포팅·기동 완료, 58과 동일 사양, 문제분 수정 반영" 수령 → `GATEWAY_TMPL_54_ENABLED=true`로 전환(마스터 게이트는 false 유지) → P0001 bill 등록 + 테스트 매핑 1건(한글 `billnm`) pending 적재 → push 시도 결과 **`connect EHOSTUNREACH 58.227.193.54:25230`**.

**62서버에서 실측한 사실만**:

| 확인 | 결과 |
|---|---|
| `ping 58.227.193.54` | **정상**(0.3ms, ttl 64, 손실 0%) — 호스트 살아 있음 |
| `ip route get 58.227.193.54` | `dev eno1 src 58.227.193.62` 온링크 — .58과 동일 경로 |
| TCP `58.227.193.58:25230` | **OPEN** (HTTP 200) |
| TCP `58.227.193.54:25230` | 연결 실패 — **애플리케이션 오류 코드 `EHOSTUNREACH`**(Node가 OS 오류 그대로 전달) |
| TCP `58.227.193.54:22` · `:9999` | 연결 실패 |
| TCP `58.227.193.58:22`(대조군) | **연결 실패 — .54와 같은 양상(5ms)** |

**⛔ 근거로 쓰면 안 되는 것(0720 실측으로 자체 기각)**: ①"5ms 즉시 실패라서 방화벽 REJECT" — .58:22도 동일해 이 망에서 막힌 포트의 공통 양상일 뿐 54 고유 신호가 아니다 ②"22번까지 막혔으니 호스트 단위 차단" — .58:22도 막혀 있어 무의미 ③`curl: (7) Couldn't connect`는 refused/unreachable을 **구분하지 못한다**(curl 오류 문구로 판정 금지).

**유효한 근거는 하나** — push 시도 시 Node가 받은 `connect EHOSTUNREACH`. 포트에 데몬이 없으면 `ECONNREFUSED`가 나오므로, 이 코드는 "포트 미기동"이 아니라 "호스트에 닿지 못함"을 가리킨다. 다만 그 차단이 ufw인지 상위 장비인지는 **우리 쪽에서 확정 불가** — 외부에 단정해 말하지 않는다.

포트·IP는 정의서 §4-0 명시값과 일치(정의서 전문 안 포트 언급 = `:25230` 2회뿐, 54 전용 포트 표기 없음). 요청 사항 = **54에 62 인바운드가 허용됐는지 확인**(강문희 0720 메일: "화이트 list는 장치로서 ufw방화벽에도 있으므로 필요한 경우 함께 변경해야 함" — 그 문장을 인용해 질문 형태로).

**★해소(0720 야간) — 원인 = 54는 ufw가 아니라 `firewalld`, 25230 미개방**(강문희 회신). 58만 ufw이고 54는 firewalld였다. 그쪽 내부 점검은 `127.0.0.1`로만 수행돼 외부 유입 경로가 검증되지 않은 상태였다. firewalld에서 25230 개방 후 57에서 호출 성공 → 62에서도 통과.

> ⚠ 교훈: "58과 동일 사양"이라는 통지는 **데몬·설정 파일 기준**이지 OS 방화벽까지 포함하지 않는다. 방화벽 계열이 서버마다 다를 수 있으므로(ufw vs firewalld) 개통 확인은 항상 **우리 쪽에서 실제 호출**로 한다. 또 우리 질문을 "ufw에 62 추가해달라"는 **단정** 대신 "반영되었는지 확인 부탁"이라는 **확인 요청**으로 보낸 덕에, 전제가 틀렸는데도(ufw 아님) 상대가 실제 구성을 알려주며 바로 풀렸다. 외부 문의는 원인 단정 없이 관측값+확인 요청 형태로.

**★54 개통 검증 통과(0720 야간·실측)** — P0001 push 결과 `picked 1 / synced 1 / failed 0 / held54 0`. 매핑 행 `sync_status='synced'`·`last_error=null`. **synced는 upsert 응답이 아니라 GET 재조회 4필드 대조를 통과해야 부여**되므로, 한글 `billnm`("한줄로연동테스트")이 54를 왕복해 원문 그대로 돌아온 것이 확인됐다. ⇒ **EUC-KR·latin1 우려 해소**(강문희: 한글은 엔진 내부 처리, 외부 API는 58과 동일 UTF-8). `GATEWAY_TMPL_54_ENABLED=true` 유지.

**잔여** — ①테스트 행 정리(웹관리자 수동: R0001 3건·P0001 1건 — 강문희 요청분) ②P0001을 R0001처럼 `is_active=false`로 내리기(대조 주기 제외용·현재 `is_active` 토글 endpoint 없음) ③마스터 게이트(`GATEWAY_TMPL_SYNC_ENABLED`)는 여전히 false — 개방은 아난티 파일럿과 함께 별도 결정.

**부수 확인** — R0001이 `is_active=false`라 `reconcile {billId:"R0001"}`이 빈 결과를 반환한다(라우트 존재 확인에는 is_active 조건이 없어 404가 아님). 이 응답을 "58 정상"의 근거로 쓰면 안 된다.

### K. ★0720(9) 게이트웨이 자동 등록 개통 — 720건 push 완료 + 마스터 게이트 ON

**밀린 물량 전량 등록(Harold 실행·실패 0)**

| 차수 | 대상 | 건수 | 결과 |
|---|---|---|---|
| 1 | 아난티 R0007 (58) | 3 | synced 3 — 58 경로 전 사이클 검증 |
| 2 | 티앤비소프트 P0059 (54) | 2 | synced 2 — **54 실데이터 첫 투입** |
| 3 | 나머지 18곳 | 490 | 10회차 전량 synced (9×50 + 40) |
| 4 | 더화이트 P0032 | 225 | 5회차 전량 synced (4×50 + 25) |
| **계** | **21 bill** | **720** | **retried 0 · failed 0** |

- 사전 산출(적재 스캔 조건 그대로 SQL 재현)이 720건이었고 실행 결과와 **정확히 일치**.
- 소요 = 회차당 50건·20초(호출 간격 200ms × PUT+GET 2콜). 게이트웨이 한계 500/s의 1/100로 의도적 저속.
- **usemod 사전 검증**: 신규 행은 bill의 `default_usemod`로 등록되므로 혼재 bill을 전수 확인. 신규 대상은 더화이트(HM1~3 1,866행 vs HM1~4 48행)·신성통상(HM1~6 150행 vs HM1~4 14행) 둘뿐이고 **둘 다 기본값이 압도적 다수** → 그대로 진행. 나머지 혼재 3곳(아이올리·메트로시티·아이비케이)은 B_ 신규 0건이라 무관.

**auto_push 전량 ON (33 linked bill)** — 밀린 물량이 있던 21곳 + 잔량 0이던 12곳. **후자를 빠뜨리면 그 회사들의 "신규 승인 → 자동 등록"이 조용히 안 돈다**(이미 등록돼 있어 밀 게 없었을 뿐). 아이비케이저축은 billid에 세미콜론이 있어 URL에서 `P0042%3BR0003`로 인코딩.

**마스터 게이트 ON** — `GATEWAY_TMPL_SYNC_ENABLED=true`. 기동 로그 `SYNC_ENABLED=true 54_ENABLED=true` 실측 확인(pm2 restart만으로 반영 — 백엔드가 dotenv로 .env 직접 로드). pending 0 상태에서 켜서 즉시 나가는 물량 없음. **이 시점부터 승인 확정 템플릿이 5분 주기로 자동 등록된다.**

**동반 — 카카오&RCS 요금제 게이트 제거(Harold 확정)**: ENTERPRISE 한정은 내부 검증용 임시 조치였고 알림톡은 직접발송 모달에도 진입점이 있어 막을 이유가 없음 → 전 요금제 개방. 제거 위치 = `DashboardHeader`(메뉴 게이팅 + 잠금 모달 — 이 게이트를 쓰던 메뉴가 카카오&RCS 하나뿐이라 기계 전체 삭제)·`KakaoRcsPage`(진입 가드 54줄). 백엔드 플랜 게이팅은 0건, 라우트 가드는 사용자 유형만 검사. **브랜드메시지 탭 잠금(`isBrandLocked`)은 별개 기능이라 유지.** ★이 게이트가 남아 있었으면 이관 30곳 중 `usage_type != 'agent'`인 15곳(아난티 포함)이 화면에 못 들어갔다.

**정돈 단계(0720 야간 착수)** — 서수란 팀장 계정 점검표 발송. `Downloads/한줄로_카카오이관_계정점검표_20260720.xlsx`(33행·확인요망 23행, 생성 스크립트는 세션 스크래치패드). 우리가 현재 상태를 채우고 서팀장은 O/X + 정정값만 기입하는 형태. 회신 후 유형별 일괄 처리 예정 — **회사명·회사코드·아이디 변경과 bill 연결 변경은 기존 경로로 즉시 가능하고, 회사 병합만 전용 도구가 필요**(템플릿·프로필·bill·계정·PAY CustId가 여러 테이블에 걸쳐 있어 dryRun 필수). ⛔**업체 안내(아이디·초기 비번) 발송 전에 회신을 받아야 한다** — 안내 후 아이디 변경은 그 자체가 사고.

점검표가 짚는 지점(규칙으로 도출, 임의 판단 0): ①PAY 발송ID 없는 에이전트 계정 3곳(아이비케이저축·쎌렉박스·topcleaningup) — 발송결과 화면이 빈다 ②회사코드가 납입자ID 그대로 15곳(예 `P0027` = 교촌에프앤비) — `company_code`를 조회 조건으로 쓰는 곳은 일회성 시드 경로 2곳뿐이고 관계는 전부 uuid라 **정정 가능** ③아이디가 납입자ID 그대로 4곳(p0019·p0070·r0019·r0029) ④사용구분 불일치(게스코리아 = web인데 템플릿 28건) ⑤납입자ID 2개가 한 회사로 묶인 3곳(마리오아울렛·엔그램·한국고용노동교육원).

**잔여(이 트랙 밖)** — ①인비토·팝폰 프로필의 `테스트계정2`(IVITO123) 귀속 정리 ②게이트웨이 미등록 초과분의 push 등록(마스터 게이트 개방 시 자동) ③`targetup-backend-error.log` PG `DateTimeParseError`(이번 작업과 무관·기존 건) ④Codex adversarial 리뷰 미수행.
