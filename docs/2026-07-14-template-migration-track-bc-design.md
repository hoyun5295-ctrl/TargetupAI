...
**우리 쪽 상태(대기)** — `54_ENABLED=true` / 마스터 게이트 false / P0001 테스트 매핑 1건 pending(`last_error=EHOSTUNREACH`, 자동 재시도는 마스터 게이트 off라 미동작). 54 개통 즉시 push 1회로 한글 왕복까지 확인 가능. 점검 종료 후 P0001은 R0001처럼 `is_active=false`로 내린다.
