...
> 상태 = **전량 배포완료(2026-08-08).** 착수 1~5 + Codex 2R 반영 — 구현 결과·실측 지점 = **§9-A**. 남은 것은 운영 실측뿐.
...
- ⚠ **동작이 바뀐 지점 1건(배포됨 — 실측에서 볼 것)** — 생성 경로 혜택 차단이 이 트랙 밖 흐름(자연어·빠른 시작)에도 적용된다. 빠른 시작은 목표 문장이 없어 AI가 쓴 할인율·쿠폰이 전부 `[혜택 안내 — 직접 수정해주세요]`로 바뀐다. `ai_no_arbitrary_benefit` 방향이지만 **기존 동작 변경**이다. → **같은 날 후속 접수(혜택 입력 — [여정 문서 §6-1 08-08(2)](FEATURE-JOURNEY.md))가 보완**: 진입 모달의 혜택 입력이 허용 근거가 되고, placeholder가 나와도 스튜디오 카드가 입력 한 번으로 채운다.
