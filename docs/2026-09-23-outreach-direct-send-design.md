...
- 네이버 브랜드스토어는 **우리 서버 IP가 막혀 있다**(0826 · 0923 두 번 실측 429). 다른 출구로 우회하지 않는다. 주소는 저장하고, 필요하면 사람이 저장한 기획전 페이지 파일을 올려 서버가 그 파일만 읽는다.
...
1. **brand.naver.com 은 우리 운영 서버(.62)에서 429다.** 0826 3회(curl 봇 UA · curl 크롬 UA · Puppeteer 실렌더) + **0923 Harold 실행 curl 크롬 UA 1회 = 429**. 차단 축 = 출구 IP.
