...
**버전:** v1.3.0 (D217+ AI Operator 10 메뉴 강화 종결 — AI 메모리 + AI 사용량 Journey Builder 동급 8 화면 완성)
**최종 갱신일:** 2026-05-25
...
| v1.0.2 | 2026-08-18 | Backend(출처·타겟 축) | **발송 출처 축 `send_type='operator'` 신설** + **고객명 필터 개방**(값 목록 미노출) + **빈 조건 = 전원 해석 차단**(`TARGET_FILTER_EMPTY`). 경위·근거·함정·잔여 전부 = **[오퍼레이터 발송 축](2026-08-18-operator-send-axis.md)**(호출어 "오퍼레이터 발송 축"). 여기 재서술하지 않는다 |
...
| **v1.3.0** | 2026-05-25 (D217+ AI Operator 10 메뉴 강화 종결) | Backend/Frontend/DB | **AI 메모리 (`/ai-memory`) + AI 사용량 (`/ai-usage`) Journey Builder 동급 8 화면 완전 강화.** **(Backend 신규 2 파일)** routes/ai-memory.ts 신설 3 endpoint (GET /overview 5 metric + 자율 진단 한 줄 + 5 타입 분포 + 영향도 top 1 / POST /search-natural 자연어 질문 → buildMemoryPromptContext + callAIWithFallback opus + 시스템 프롬프트 안 "구체 혜택 임의 생성 X" + 모델명 X 명시 / GET /top-impact 영향도 top N usage_count + importance DESC). routes/ai-usage.ts 신설 4 endpoint (GET /overview 5 metric + 전월 대비 격차 + 일평균 + 한도 도달 예측일 + 모델 추상 분포 + Batch 30일 + threshold_config / GET /forecast 향후 30일 선형 회귀 y=ax+b + 한도 도달 시점 / POST /search-natural ai_call_log 14일 + cache 통계 + 모델 추상 분포 시스템 프롬프트 + 모델명 X 명시 / POST /threshold-alert companies.ai_usage_threshold_config UPDATE). **(옛 사고 영구 정정)** utils/ai-rate-limit.ts getMonthlyUsage 안 옛 SQL `LEFT JOIN plans p ON c.plan_code = p.plan_code` 사고 정정 — companies 안 plan_code 컬럼 X (옛 D215+ PM2 로그 "column c.plan_code does not exist" silent fallback 0 사고 영구 종결). 정정 = `c.plan_id = p.id`. 전수 grep 결과 1곳만 사고 (다른 30+ 호출처 모두 정합). **(DB ALTER 1건)** companies.ai_usage_threshold_config jsonb DEFAULT '{}'::jsonb (한도 알림 임계값 50/80/95 + 채널 email/sms/inapp + enabled). **(Frontend 6 파일)** pages/AiMemoryPage.tsx 전면 재작성 옛 457줄 → 758줄 — sticky 헤더 + AI 자율 진단 카드 + 자연어 입력 + 빠른 시작 7 카드 (VIP 패턴 / 휴면 회수 / 채널 성과 / 브랜드 톤 / 컴플라이언스 / 직접 추가 / 자동 정리) + 5 타입 도넛 SVG + 영향도 top 10 카드 + 1-click 액션 3 + 자세히 분석 토글 (출처 도넛 + 월별 bar + 중요도 histogram + 최근 활용 timeline) + Source caption. components/AiMemory/ 3 신설 — TopImpactCard.tsx (rank + 5 타입 badge + AI 활용 횟수 + 중요도 막대) / MemoryTypeGuideModal.tsx (5 타입 가이드 + 예시 3건 + 자동/직접 분류) / AddMemoryModal.tsx (5 타입 + key 200자 + value 2000자 + 중요도 1~10 range). pages/AiUsagePage.tsx 전면 재작성 옛 295줄 → 627줄 — sticky 헤더 emerald→cyan + AI 자율 진단 (한도 % + 일평균 + 도달 예측일 + cache 히트율) + 자연어 입력 + 빠른 시작 5 카드 (월별 트렌드 / AI 추론 모드 분포 / 비용 절감 / 한도 알림 / Batch 가이드) + 5 metric 격차 (이번 달 + 한도 + Cache hit + 일평균 + Batch 처리) + 비용 예측 라인 차트 (CostForecastChart) + 1-click 액션 3 + 자세히 분석 토글 (AI 추론 모드 도넛 + source top 10 bar + 일별 비용 area) + Source caption. components/AiUsage/ 3 신설 — ThresholdAlertModal.tsx (50/80/95 임계값 + email/sms/inapp 채널 + enabled toggle) / BatchModeGuideModal.tsx (실시간 vs Batch 비교 + 4 use case 시나리오 + 활성화 흐름) / CostForecastChart.tsx (선형 회귀 SVG + 옛 30일 실제 line + 향후 30일 예측 dashed line + 한도 수평 dashed line + 한도 도달 시점 경고). **(영구 룰 정합 8건 100%)** no_model_name_ui_exposure (UI 안 Opus/Sonnet/GPT/Claude/Anthropic 0건 — 추상 명칭 "고급/표준/보조 추론 모드" 사용) + db_alter_safety_net (모든 endpoint catch `column does not exist` → 503 DB_MIGRATION_PENDING 분기) + marketing_user_ux_priority (빠른 시작 카드 = 1 클릭 = 즉시 AI 호출 + 자연어 자동 prefill + AI 답변 모달) + design_quality_minimum_journey_level (Journey 동급 8 화면 양 페이지) + feedback_no_native_browser_dialog (ConfirmModal + useToast 사용 / native alert/confirm/prompt 0건) + feedback_no_bakkeum_usage (박-단어 14 변형 grep 0건 = 5 신규 frontend + 옛 정정 backend 1) + feedback_ai_no_arbitrary_benefit (search-natural 시스템 프롬프트 안 "구체 혜택 임의 생성 X" 명시) + cto_mandate_for_vito (옛 `c.plan_code` 사고 전수 grep + 1곳만 정정 — 단편 fix X). **(영구 룰 신규 강화 1건)** feedback_default_superpowers_workflow § D217+ 강화 — 매 답변 작성 직전 자가 grep 의무 매트릭스 5건 (verification-before-completion / brainstorming / writing-plans / test-driven-development / systematic-debugging / dispatching-parallel-agents) + D217+ 본 세션 옛 위반 사고 사례 영구 등록 (인지 박혀있지만 실제 호출 X = 옛 D191 박-단어 자기 강화 루프 동일 패턴 인정). **(검증 evidence)** backend tsc 0 errors + frontend tsc 0 errors + 박-단어 grep 0건 (5 신규 + 옛 1 정정) + 모델명 grep 0건 (5 신규 frontend) + native dialog grep 0건 (5 신규 frontend) + app.ts 라우트 등록 2건 (aiMemoryRoutes + aiUsageRoutes). Harold 직접 배포 완료 + Codex /codex:adversarial-review 권장 영역. **AI Operator 10 메뉴 모두 Journey Builder 동급 강화 완료** — 1번 여정 자동화 + 2번 AI 자율 예측 + 3번 AI 자동 마케팅 + 4번 성과리포트 + 5번 자사몰+데이터 융합 + 6번 인앱메시지 + 7번 Email 캠페인 + 8번 모바일 DM + 9번 AI 메모리 + 10번 AI 사용량 |
