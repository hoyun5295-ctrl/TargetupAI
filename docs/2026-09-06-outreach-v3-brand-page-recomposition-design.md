...
### 17-6. 두 번째 실측(이니스프리 `dm-lc2su0e` · 2026-09-06 재배포 뒤) 결과와 정정

DM 을 열어 본 사실 = 히어로 헤드라인 "9.1(화) 9.6(일)" · 버튼 "9.1(화) 9.6(일) 보기" · "9.2(수) 9.16(수 보기"(괄호 깨짐) · 카드2 헤드라인도 날짜 · 스포트라이트 버튼 "[대용량] 블랙티 유스 보기" · 헤드라인 "…미백 탄력케"(낱말 중간 절단) · 캐러셀 "28% 19,200원 19,200원" · "45% 38,350원 38,350원". 이니스프리 목록 DOM(`/kr/ko/ca/event`) = `<li><div class="event_item" ap-click-data="제목|26.9.1(화) ~ 26.9.6(일)|id"><a><img alt="제목"><p class="date">9.1(화) ~ 9.6(일)</p><strong>제목</strong></a></div></li>`: 첫 줄이 **연도 없는 날짜**, 연도는 감싸는 div 의 속성에만.

| 원인 | 실측 사실 | 정정 |
|---|---|---|
| 제목 = 날짜 | 카드 본문 첫 조각이 `<p class="date">` · 연도가 없어 `CARD_DATE_RE` 에 안 걸려 기간으로도 안 빠졌다 | `periodRawOf` 가 연도 없는 범위(`M.D(요일) ~ M.D(요일)` · `M/D ~ M/D` · `M월 D일 ~`)도 기간 원문으로 뺀다 · 제목은 `titleFromCardBlock`(class tit·title → strong·h → img alt → 본문 조각 · 날짜만인 후보는 건너뛴다 `isDateLikeText`) |
| 종료일(면허) | 연도는 감싸는 요소의 `ap-click-data` 에만(`26.9.6(일)`) | 앵커 앞 감싸는 요소(`<li` 부터)를 `lead` 로 넘겨 두 자리 연도 + 요일 괄호 날짜(`YY_DATE_RE`)를 종료일로 읽는다 · 원문 문자열 그대로 · 본문에 연도가 있으면 그것이 우선 |
| 헤드라인·버튼 방어 | 제목이 날짜여도 `headlineFromCard` 가 그대로 실었다 · 라벨 절단이 글자 단위라 괄호가 깨졌다 | `headlineFromCard` 가 날짜 조각을 걷고 글자 2자 미만이면 demoted · `cutAtWord` 로 낱말 경계 절단(버튼·스포트라이트 헤드라인 공통) · `productCtaLabel` 이 앞머리 `[대용량]` 꼬리표를 뗀다 |
| 기간 원문 | 면허 없는 카드는 기간을 안 실었다(연도 없는 기간은 면허 없음) | 기간 원문(`periodRaw`)은 면허와 무관하게 sub_copy·body 에 싣는다(날짜는 혜택 수치가 아니다) · 면허는 countdown 만 가른다 |
| 같은 가격 두 번 | 상품 카드 `discount_price === price` 인데 목록 할인율(28%)이 덧입혀졌다 | `toProductItems`: `discount_price` 는 정가보다 낮을 때만 · `discount_rate` 는 그 할인가 쌍이 있을 때만 |
| `~50%` 걷기 | `CARD_BENEFIT_TOKEN_RE` 가 `50%` 만 걷어 `~` 가 남았다 | 수치 앞뒤에 붙은 `~` 를 함께 걷는다 |
| 상단 배너가 카드 자리를 먼저 차지 | 실물 앵커 순서 = 상단 배너(`추석선물` · 기간 0 · 같은 행사 링크) → 목록 카드. 같은 링크 중복 제거가 앞 배너를 남기고 기간 있는 카드를 버렸다(브라우저 JS 로 앵커 순서 실측 · 12장) | 같은 링크가 다시 오면 기간 있는 뒤 카드가 앞 배너를 덮는다(자리 유지) |

실물 HTML 3장 + 상단 배너 앵커를 픽스처로 고정(`__tests__/fixtures/innisfree-event-list.html`) · 테스트 = 제목·기간 원문·종료일·배너·헤드라인·버튼·상품 CTA 라벨.

### 17-7. 세 번째 실측(토니모리 `dm-7EHTaPI` · 2026-09-06 · Harold "진짜 내용 없다") 결과와 정정

DM 실물 = 헤더 · 히어로(체험단 제목 + sub_copy "기간 기간 : 2026.09.01 ~ 2026.09.14 응모 : 88 명 후기 작성 : 2026.09.16") · 버튼 · 스포트라이트 태그 **"줄번호가 없는 케이스(예: 본인인증"** · 상품 2 · 증거 · 카운트다운(후기 작성일 기준 10일) · 지난 8월 체험단 버튼 · 꼬리말. 브라우저로 실물을 열어 확인한 사실:

| 원인 | 실측 사실 | 정정 |
|---|---|---|
| 기획전 카드 0 | 목록 `/plan/shop_plan_exhb_main.do` 의 기획전 3장(`/plan/shop_plan_exhb_view.do?planexhb=…` · `<h3>` 제목 · 기간 0 · img alt "기획전")이 "기간 없는 카드는 링크가 행사성일 때만" 규칙에 걸려 전부 버려졌다 → 남은 것이 체험단뿐 | 목록 페이지와 **같은 첫 경로 조각**(`/plan/…`)이면 기간 없어도 카드(면허 0) · 목록 자기 링크 제외 |
| 종료일 = 후기 작성일 | 체험단 카드 본문 "신청 기간 : A ~ B 응모 : 88명 후기 작성 : C ~ D" 에서 `periodRawOf` 가 첫 날짜부터 **마지막 날짜(D)** 까지 늘려 잡았다 → 카운트다운·면허가 후기 작성일 기준 | 첫 날짜 바로 뒤의 `~`·`부터` + 둘째 날짜까지만(`CARD_RANGE_SECOND_RE`) |
| 지난 행사 버튼 | 8월 체험단(2026.08.01 ~ 08.17)이 후기 작성 범위(~09.02)로 살아남아 후보·버튼이 됐다 | 종료일을 아는데 지난 카드는 후보에서 제외(`eventCandidatesFromCards`) · 종료일 모르는 카드는 면허 없이 남긴다 |
| "기간 기간 :" | 원문이 "기간 : …" 인데 접두 "기간 " 을 또 붙였다 | `periodLineOf`: 원문 앞머리 "기간 :·일정 :" 을 떼고 한 번만 |
| 수상 태그 = JS 주석 | 상품 페이지 `<script>` 안 주석 "…줄번호가 없는 케이스(예: 본인인증 opener 오류)…" 를 `stripTags` 가 글로 남기고 `인증` 이 수상 낱말에 걸렸다(fetch 로 확인 · inScript true) | `stripTags` 가 script·style·noscript·template·svg 블록을 본문째 버린다 · `인증`·`대상` 은 앞 문맥(피부과·더마·비건·저자극… / 대상 수상·선정)이 있을 때만 |

실물 목록 `<li>` 4장(기획전 2 · 체험단 2 · 아이콘 앵커 2)을 픽스처로 고정(`__tests__/fixtures/tonymoly-event-list.html`). 픽스처 결과 = 카드 4 · 후보 3(9월 체험단 면허 → 기획전 2 면허 없음 · 8월 체험단 제외) · 헤드라인 "시작과 마무리를 한 번에🎈" · 버튼 "미리 준비하는 추석 보기" · 기간 줄 "기간 2026.09.01 ~ 2026.09.14".

**남긴 축(추가 과제 · 미착수)**: 토니모리 홈 상단 슬라이드 배너 5장(1380×1184 · alt 제목 · 기획전 링크)은 카드 재료로 안 쓴다. 목록 카드가 얇을 때 alt 제목 + 링크가 있는 홈 배너를 카드 2순위로 붙이는 축은 별도 승인 뒤.
...
**3차 실측(배포 뒤 재등록 `578bab981c` · DM `dm-UKcJ9E9`)** — 위 4건은 닫혔다(포스터 생성 · 판정 14장 · 행사 3건 전부 · `stdHero=poster`). 산출물을 내려받아 새로 본 결함 4가지:
| # | 실측 | 원인 | 수정 |
|---|---|---|---|
| 5 | 포스터에 톤28 제품이 아니라 모델이 지어낸 병·튜브·상자(뭉개진 라벨) | 누끼 원천이 "사람이 고른 이미지" 하나뿐 → 배너·정물이거나 없으면 누끼 0 → '이벤트' 템플릿 → 모델이 상품을 발명 | 누끼 후보 = [선택 이미지(판정상 상품일 때만 앞) → 상품 사본 상위 3] 최대 3번(`cutoutCandidates` · `cutoutFrom`) + `posterStyleHint(brand, hasProduct)` "첨부 상품만 상품 · 첨부 없으면 상품 0" |
| 6 | 행사 블록에 같은 사진이 카드 이미지·갤러리로 두 번 | 카드 배너 사본 ≠ 슬라이스 사본(URL 만 다른 같은 원본) | 슬라이스가 있으면 카드 이미지 = 첫 슬라이스(갤러리는 URL 같음으로 뺀다) |
| 7 | "오늘핫딜 < > 지성두피 다시마" | 면허 없는 "3+1" 을 걷어낸 자리의 빈 괄호 | 빈 괄호 통째로 제거(`headlineFromCard`) |
| 8 | 인용문 글자 카드 사진 0 · 버튼 = 홈 | 인용문 카드는 이미지·링크 원천이 없었다 | 제목 안 상품명 대조(`matchProductInTitle` · 4자 이상 · 긴 이름 우선) → 상품 사본 + 상세 링크(상세가 따로 있으면 링크 유지) |

**4차 실측(0910 · `b5d37144ea` · DM `dm-J3jgN9b`)** — 5~8 닫힘. 남은 약점 3(Harold "약점이 있으면 고쳐야") → 각 1곳:
| # | 실측 | 원인 | 수정 |
|---|---|---|---|
| 9 | 포스터 속 제품 라벨 글자가 뭉개진다 | 모델이 누끼를 장면에 녹여 다시 그린다(원리상 보존 불가) | 배경만 생성(누끼 미첨부 · 상품 0 · "빈 진열면" 요구) → 서버가 누끼 PNG 를 픽셀 그대로 합성(`composeImage` cutoutPath · 포스터 아래 가운데 62% · 배너 오른쪽 30%) · 유출 검사(숫자)는 배경에만 · `cutoutMode='server_compose'` |
| 10 | 확정 2·3번 행사 사진 0 | 재료 상품 상한(6)에 확정 제목의 상품이 밀림 · 확정 행사 페이지는 1번만 렌더 · 상품도 상세도 없는 소식은 원천 0 | (a) 확정 제목의 상품을 상한 앞으로(`orderProductsByPreference`) (b) 배너 없는 확정 행사 페이지 15초씩(총 40초) 렌더 → 첫 넓은 이미지 → `selectedList[i].bannerUrl`(`pickEventBannerImage` · `setEventQuoteOwned`) (c) 홈 상단 배너 alt 대조(`matchHeroBannerInTitle` · `heroBanners[].alt`) |
| 11 | 홈 첫 화면 출시 티저가 후보에 없다 | 분석 프롬프트가 "행사·이벤트·프로모션"만 요구 | "날짜가 명시된 출시·오픈·선공개·티저는 혜택이 없어도 후보 · 시작 전이면 start_date · 홈 첫 화면 큰 소식 우선" 1줄(면허·차단기 그대로) |
합성 배치는 제안값이다(실측 1건 뒤 조정): 포스터 3:4 = 문구 위(0.065~0.2) · 제품 바닥 0.86 · 폭 62% / 배너 16:9 = 문구 아래 가운데 · 제품 오른쪽(x 0.78 · 폭 30%).
