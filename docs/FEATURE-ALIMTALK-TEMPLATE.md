...
- **강조표기형(TEXT) 입력 필드 정정** — **0805 배포완료.** 남은 것은 화면 실측 3건뿐 = [2026-08-05 설계서](2026-08-05-alimtalk-emphasize-text-design.md) §5
...
| **0805** | **강조표기형(TEXT) 보조문구 필수화 + 표시 자수 안내** — 화면이 `[선택]`이라 적어 규격과 달랐다. 라벨·`validate()`·payload 3지점 통일 | `frontend tsc 0` · 기존 TEXT 42건 중 보조문구 빈 건 **0**(분포 NONE 4,518·TEXT 42·IMAGE 19)이라 저장이 막히는 기존 템플릿 없음 |
| **0817~0818** | **카카오 & RCS 화면 전면 재작성**(목록 3탭 + 폼 모달 3 + 부속 모달 3 + 편집기 3) — 이모지 0 · 탭 3색→인디고 · 관리 열 대표1+`⋯` · 모바일 카드 · 삭제 확인 공용 CT 통일 | 경위·원칙·잔여 = **[콘솔 톤 §4-1](2026-08-18-console-ui-unification.md)**. 여기 재서술하지 않는다 |
