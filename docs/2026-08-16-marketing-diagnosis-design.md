...
- **배포 게이트(순서 의무)**: ①git 커밋·배포(tp-push→build:safe→pm2) ②DDL §3 한 트랜잭션(서버 psql) ③**자리표시 PNG 40종 → 이미지 스튜디오 실렌더 교체(§5-5 — 교체 전 seed 활성화 금지: 자리표시 노출 금지)** ④seed v1(§3 최종형) ⑤§9 실측 13종.
