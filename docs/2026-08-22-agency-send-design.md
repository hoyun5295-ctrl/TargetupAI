...
**★2026-08-23 추가 DDL (Codex 적대 검토 반영) · 실행완료**
...
- 옛 코드는 이 컬럼을 모르므로 **배포 전에 먼저 실행해도 무해하다**(0822 DDL과 같은 순서). 컬럼 부재 시 라우트는 503, 워커는 조용히 넘어간다.
...
1. **§12-E 구조 수정 5건**(착수 승인 대기). Codex 2라운드 critical 3·high 3이 남아 있고 뿌리는 하나다. **이 축은 그 전까지 실발송에 열지 않는다**(슈퍼관리자 플래그를 켜지 않으면 아무도 못 들어온다 = 배포 자체는 안전하다)
...
### 12-E) Codex 2라운드 (2026-08-23 · critical 3 · high 3 · 라운드 상한 도달 · **1건만 즉시 정정, 5건은 착수 대기**)
...
**남은 5건은 뿌리가 하나다.** 이 축은 같은 행을 **워커·담당자·lock 복구·승인 직후 트리거 네 주체**가 만지는데, 소유권 모델 없이 조건을 자리마다 덧대고 있다. 1라운드도 2라운드도 지적의 실체는 전부 "그 조건이 안 걸린 자리"였다. 조건을 하나 더 붙이면 다음 자리가 샌다.

| # | 남은 지적 | 새는 결과 |
|---|---|---|
| 1 | 캠페인 생성이 lease 경계 밖이고, 생성 뒤 `queued` CAS가 0행이어도 성공으로 넘어간다 | 살아 있는 캠페인이 원장과 끊긴 채 발송·과금된다 |
| 2 | 결과 미확정 오류인데 `expired`로 닫고, 재예약이 `dispatch_key`를 지운다 | 옛 캠페인과 새 캠페인이 둘 다 발송된다 |
| 3 | `saveTestResult`가 lease 없이 쓴다. 1차 경로는 lease 확인 전에 테스트 문자까지 보낸다 | stale 워커가 담당자가 고친 문안을 옛 문안으로 덮는다 |
| 4 | lock 복구가 캠페인 `send_phase`를 안 보고 `queued`로 만든다 | 발송되지 않는 캠페인을 "예약 완료"로 표시한다 |
| 5 | 시각 변경 뒤에도 `status`·`content_version`이 그대로라 옛 승인이 통과한다 | 담당자가 못 본 시각으로 발송이 승인된다 |

**구조 수정안(착수 승인 대기).** 조건을 더 붙이지 않고 소유권을 한 곳으로 모은다. 고치면 지금 자리마다 붙어 있는 `AND status = ?`·`AND content_version = ?`·`AND lock_at = ?`가 **사라진다**(코드가 는다면 또 덧댄 것이다).

1. **행 수정 번호** `revision integer NOT NULL DEFAULT 0` 신설. 모든 변경이 +1. 담당자 경로(승인·문안 수정·시각 변경·취소)는 전부 `WHERE revision = $관찰값` 하나로 통일한다 → 5번이 자동으로 닫힌다.
2. **소유권 토큰** `lock_token uuid` 신설. 워커가 선점할 때 발급하고, 워커의 모든 쓰기(`saveTestResult` 포함)와 lock 복구가 이 토큰을 조건으로 쓴다 → 3번이 닫히고, 타임스탬프를 토큰으로 쓰는 위태로움도 사라진다.
3. **캠페인 활성화를 토큰 소비와 묶는다.** 생성 뒤 CAS가 0행이면 그 캠페인을 취소 CT로 중화한다(`skipTimeCheck`) → 1번이 닫힌다.
4. **미확정은 닫지 않는다.** 결과를 확인하지 못한 오류는 `expired`가 아니라 소유권을 놓고 `approved`로 되돌려, 다음 tick이 `dispatch_key`로 캠페인을 찾아 phase로 확정한다 → 2번이 닫히고 4번도 같은 분류를 쓰게 된다.

DDL 2컬럼(`revision`·`lock_token`) 추가가 필요하다. 착수는 Harold 판단.
...
7. **읽은 값을 조건에 넣고 쓴다**(★0823 Codex critical). 라우트 3곳은 관찰한 `status`(문안 수정은 `content_version`까지)를 WHERE에 넣고, 0행이면 아무것도 바꾸지 않고 409로 돌려보낸다. 워커는 선점할 때 DB가 적은 `lock_at`을 **lease**로 삼아 이후 모든 쓰기에 건다. 조건 없이 덮으면 **취소한 건이 다시 예약되고, 워커가 잡은 건의 lock이 깨진다.**
