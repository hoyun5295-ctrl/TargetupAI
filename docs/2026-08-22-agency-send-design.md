...
### 14-6) 이번 축 밖으로 뺀 것 (Codex 적대 검토가 올린 것 · 손대지 않았다)

**전부 당일 재검사 폐지와 무관하게 이전부터 있던 것**이고, 고치려면 "의존 장애를 판정으로 읽지 않는" 재시도·백오프 설계가 함께 필요하다. 이번 접수 축이 아니라 기록만 한다.

| 어디 | 무엇 | 왜 이번에 안 고쳤나 |
|---|---|---|
| `agency-send-worker.ts` `runSpamRound` | 통과 판정이 `!== 'blocked'`다. 상류 계약은 `pass·blocked·failed·timeout`이고 응답이 비면 `undefined`라 **검사가 못 돈 것도 통과로 읽는다** | 워커 B에도 똑같이 있다. **오늘 운영에서도** T-2h 재검사가 타임아웃이면 통과로 읽고 발송한다. 별개 결함이다 |
| 같은 함수 | `refineForSpam`이 API 장애를 `{ok:false, reason:'model-error:...'}`로 돌려주는데 `anchor-lost` 같은 **내용 반려**와 같이 취급한다. 다듬기 AI가 잠깐 죽으면 `test_failed`가 되고 승인된 건에 예약 취소 통지가 간다 | 위와 같은 뿌리 |
| `runExpire` | 후보를 "2시간 안" 전부로 담고 `LIMIT 20`을 먼저 건다. 아직 만료가 아닌 건이 스무 자리를 채우면 만료된 건이 밀린다 | 2시간 창에 20건이 동시에 있어야 도달한다. 지금 볼륨에서 발생하지 않는다 |
| 승인 라우트 | 마감을 애플리케이션 시각으로 보고 CAS는 상태·`revision`만 건다. 그 사이 마감을 넘기면 "승인되었습니다"를 돌려주고 아무도 그 건을 맡지 않는다 | `reapproval`은 **이전부터** 10분 마감이라 같은 경합이 이미 있었다. 밀리초 창이고 결과는 미발송 안내다 |

⛔ **한 번 시도했다가 되돌린 길**: 위 첫 줄을 이번 세션에서 고치려고 `error` 결말을 새로 만들어 잡기 전 상태로 되돌리게 했더니, 픽커가 `ORDER BY created_at LIMIT 5`라 **꾸준히 실패하는 다섯 건이 전 고객사의 매 tick을 독점**했다(`received`는 만료 대상도 아니다). 재시도·백오프·`received` 만료를 함께 설계하지 않고 실패 처리만 바꾸면 더 나빠진다. 별도 과제로 통째로 다룬다.
