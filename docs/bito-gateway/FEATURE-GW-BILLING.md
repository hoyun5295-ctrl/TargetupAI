...
1. [x] 죽은 코드 2건 제거(§4-1·2) — ★2026-09-23 §4-1 은 이미 없었고 §4-2 `finalizePendingBillingFailures` 삭제
2. [x] 운영 DB 트리거 세대 확인(§4-4) — ★2026-09-23 운영 실측 `notify_refund_target` 이 `charge_amount` 판(031)
3. [ ] credit_limit 정책 결정(§4-3) — 강제할 것인지, 컬럼을 걷어낼 것인지. Harold 판단 필요
4. [x] 수동 충전 요청 단위 멱등(§4-5) — ★2026-09-23 화면이 창을 열 때 만든 요청 키 필수 · 같은 키 = 앞 결과(충전·조정 둘 다)
5. [ ] 후불 정산·청구 흐름 부재 확인 — ACCRUED 통계를 청구서로 바꾸는 경로가 게이트웨이에 없다(관리 화면 조회뿐). 고객사 후불 계약을 받기 전 필요 여부 판단
6. [x] §4-7 미지 유형 SMS 단가 폴백 — 057 에서 `ELSE NULL`(= no_price)로 이미 닫혀 있다(2026-09-23 확인)

★2026-09-23 **선불 전수 점검(Harold 지시)** — 위 원장 밖에서 결함 5건이 더 나와 같은 날 조치했다: 한줄로 충전(PAY 충전 원장)이 게이트웨이 지갑에 반영되지 않음 · 환불 SQL 첫 실행이 상태를 못 바꿈(원장 전후 잔액 영구 NULL) · 발송 차감이 안 읽는 고객사 범위 지갑 · 잔액 사본 어긋남 · 잔액 부족 결과코드 미등재. 기록·계약·배포 순서 = bito-gateway `status/BUG_HISTORY.md` 2026-09-23 선불 · `status/domains/COMMERCIAL_ACCOUNTS.md` 과금 계약.
