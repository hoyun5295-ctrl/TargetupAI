...
**바꿔도 결과가 안 바뀌면 원인이 아니다.** 결정적 대조 = 같은 세션·같은 계정·같은 발신프로필에서 **알림톡이 날짜 없는 MSG_UID(`00001707055`)로 `0000` 성공**(10:49 실측 로그). 코드명 `SerialNumberPrefixDateException`만 보고 MSG_UID를 지목한 것이 오판이었다. → 브랜드 전용 규칙 **철회**(`11f6287` — 알림톡과 동일 형식 환원). RESERVED_DATE·CONTENTS 제거(`9a86918`)는 규약서 정합이라 유지하되 **4101 원인은 아니었다**.

### 남은 단 하나의 미충족 조건 — `businessProfile: false`
...
매뉴얼 §2.2.1 = "I 타겟팅 — **비즈니스 인증 채널일 경우 발송 가능**"(추가 요건 없음). 확인한 전 항목 중 유일한 미충족이다.
⚠ **인과 미확정** — `alimtalk: false`인데 알림톡은 성공하므로 이 플래그들이 발송 게이트와 1:1이 아닐 수 있다. 확정은 비즈니스 인증 후 같은 프레임 재발송으로만.
...
2. [ ] **4101 — 다음 한 수 = 카카오 비즈니스 채널 인증**(§3 하단 표. Harold 실행 영역 · 코드 아님). 인증 후 같은 프레임 재발송으로 인과 확정. 그래도 4101이면 그때 휴머스온에 **10:49 실측 로그 2건(알림톡 0000 / 브랜드 4101 프레임 원문)**을 붙여 문의한다 — 추측 형식 시도는 금지(3회 소진)
