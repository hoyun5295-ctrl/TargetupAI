...
| # | 내용 | 위치 |
|---|---|---|
| 1 | **poison 결과 dead-letter 부재** — 고객 DB에서 영구 반영 불가한 결과(행 삭제·seq 불일치)는 ACK가 영원히 안 나가 재연결마다 무한 재전달. 다른 결과 처리는 안 막는다(워커 병렬·개별 ACK). 운영 증상 = reportDBFailures 누적. ★2026-09-14 실사례 = 아이티앤(`itensms03`) 반영 불가 24건 → 게이트웨이 재전달 포기(20회·5분 · 0909 추가)까지 약 1초 간격 재접속 · 포기 결과를 끝내는 곳이 없어 통계 「진행 중」 영구 잔존 → web/api 집계만 정정(상태·과금 불변 · 게이트웨이 `status/BUG_HISTORY.md` 2026-09-14). **★0914(2) 게이트웨이 쪽 종결(배포·화면 확인 완료 0915 00:0x · `2a79bbf`)** = 같은 결과 60초 간격 제한 + 붙어 있는 연결 60초 주기 재전달 + 포기 10회·10분 + 운영 대시보드 「반영 불가」 목록·다시 보내기(게이트웨이 `status/BUG_HISTORY.md` 2026-09-14(2)). Agent 쪽(절단 대신 오류 코드 ACK)은 1.0.29 | `poller.go:571-575`(0914 기준 · 0815 표기 555-559) | 
...
1. [ ] **poison 결과 처리 설계**(§3-1) — 후보 축: 반영 불가 N회 초과 시 별도 원장(dead-letter 테이블 또는 저널 격리)+운영 알림, ACK는 사람 판정 후. **자동 성공 처리 금지**(결과 유실이 되므로). 허브 §8-10과 동일 항목, 설계는 Harold 승인 후. ★0914 Harold 동의 방향 = Agent 1.0.29 합류 · 설계 입력(확인된 사실 7 · 미확인 3) = [2026-09-14 게이트웨이 인계](../2026-09-14-gateway-session-handoff.md) §8(Agent 는 결과 코드 0 인 ReportAck 만 보냄 · 게이트웨이는 코드 ≠ 0 ReportAck 를 버림 `grpc_server.go:405-412` · 포기 기준은 web/api 계약 테스트 11 과 묶임). ★0914(2) 게이트웨이 쪽 원장·경보·화면은 끝났다(같은 인계 §12) — 남은 것은 Agent 가 반영 불가 결과에 절단 대신 오류 코드 ACK 를 보내고 게이트웨이가 그 코드를 dead-letter 로 받는 1.0.29 뿐
