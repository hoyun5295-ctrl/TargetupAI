...
- **★0914(3) 안 돌던 `web/api/test/customer-downloads-test.js` 현행화·등록(Harold 지시)**: 다운로드 경로(감사 기록·`X-Artifact-SHA256`·내부 파일과 0906 내린 슬롯 비노출·404·버전 잠금)를 검사하는 유일한 테스트라 지우지 않았다. 슬롯 7 · 안내서 pdf · `bootstrap_version` 부재 · 통합 묶음 v1.2(종전 기대값 v1.1은 파일 목록과도 맞지 않았다) · 승인 릴리즈 조회 응답 · 승인 버전 판정(리눅스 패키지 제거 → 불가 · 윈도우 준비)으로 맞추고 `npm test`에 등록. 검증 = 단독 `CUSTOMER_DOWNLOADS_TEST_OK` · API `npm test` exit 0 · 대시보드 `npm test` 25건 실패 0 · 빌드 번들에 「13개」 0. **배포(0914 11:20)** = `check.sh` `GW_CHECK_OK` → 대시보드 manifest `e44e03932ff1` 46 files `GW_DEPLOY_OK`(백업 `deploy-backups/20260914-112018/dist`) · live 번들 `CustomerDownloadsPage-BkwSgLS2.js` 「13개」 0·새 문구 1 · 화면(Harold) = 「설치·API 연동용 7개 공개 파일만 표시합니다」 · 묶음 발급 가능 유지. API 파일 변경 없음(테스트·`package.json`만). **커밋** = 게이트웨이 `108443e`(9-5·9-6·보안 §8 A3 web 13파일 · 0914 push · `c18d587..108443e`). `Bito Agent/` 안내서는 `.gitignore`(`/Bito Agent/` · `*.pdf`)라 저장소에 없다 — 게시본 해시 `aed92147…`가 기록이다.
