...
## §4-2 컨펌 창구·이의 통지·수정 발행 (★2026-08-04 — 서수란 0803·0804 접수)

접수 셋이 한 흐름이다. **업체가 이의를 낼 창구 → 우리가 아는 길 → 고쳐서 다시 내보내는 길**이 전부 비어 있었다.

| 접수 | 실측한 공백 |
|---|---|
| "거래내역서나 정산서 안에 이의 신청도 없어" | 정산서 메일 템플릿이 **두 벌**이었다. 컨펌 버튼·이의 안내가 일괄발급 경로(`invoice-confirm.ts`)에만 있고, 정산 목록 발송 모달(`routes/billing.ts /:id/send-email`)로 나간 메일에는 `token`·`confirm`·`이의` grep이 **0건**이었다 |
| 이의가 들어와도 아무도 모른다 | `POST /:token/objection`은 DB만 바꾸고 알림 호출이 **0건**. 현황판에 들어가 봐야 알았다 |
| "수정 발행이 불가" | 복귀 경로 미구현. `superseded_at`을 세팅하는 코드 **0곳**, `objection_resolved_at`은 코드에 **0건** |

**① 컨펌 창구** — 토큰 확보(`ensureConfirmationToken`)와 안내 문구(`renderConfirmBlockHtml`)를 CT로 빼고 두 경로가 같이 쓴다. 본문 전체를 합치지 않은 것은 두 메일의 성격이 다르기 때문이고(개별 발송은 항목표를 본문에 인쇄한다), 문구만 공용이면 갈라지지 않는다.
- 토큰은 **발송 표시와 같은 트랜잭션**에서 확보한다 — 메일은 나갔는데 추적행이 없으면 링크가 죽는다.
- 살아 있는 추적행이 있으면 그 토큰을 재사용한다(재발송에도 링크가 안 바뀐다). 수신자를 화면에서 바꿔 보내도 **기존 행의 `recipient_email`은 건드리지 않는다** — 추적행은 장당 하나여야 상태가 갈라지지 않는다.
- 토큰을 못 만들면 **보내지 않는다.** 컨펌 블록만 빼고 보내면 업체는 이의를 낼 곳이 없는데 3일 뒤 자동 계산서는 그대로 나간다.

**② 이의 통지** — 이의 트랜잭션은 손대지 않는다. 거기서 SMTP를 기다리면 발급 큐를 보류한 채 그 회사 발행이 멈춘다(0731 `taxbill_email_resends`와 같은 계약). `objection_at`이 이미 내구 기록이라 **별도 큐가 필요 없다** — `taxbill-worker` 5분 tick이 미통지 이의를 집어 보낸다.
- 시도 횟수를 **보내기 전에** 올린다. 크래시해도 같은 행을 무한히 다시 잡지 않는다(상한 5).
- 수신처 = ENV `BILLING_OBJECTION_ALERT_TO`(표시) · `BILLING_OBJECTION_ALERT_BCC`(숨은 참조). 미설정이면 발송을 생략하되 **기동 후 한 번은 경고를 남긴다** — 조용히 넘기면 이의가 영영 묻힌다.
- 팝빌 게이트 밖·자기 try에 둔다. 이 패스가 tick 전체(상태 전이·발행)를 멈추면 안 된다.

**③ 수정 발행** — 수량은 사람이 조정한다(Harold 확정). 컬럼·불변식은 `status/SCHEMA.md` `billing_qty_adjustments` 절이 소유한다.
- 화면은 **실제 수량을 그대로 적고** 델타는 서버가 준 `base`(조정 전 원래 수량)로 계산한다 — 사람에게 뺄셈을 시키지 않고, 이미 조정된 청구서에서 또 조정할 때 이중 적용도 막는다.
- 항목줄은 청구서·PDF와 **같은 함수**(`buildInvoiceLines`)로 만들어 내려준다. 화면이 따로 묶으면 표시 수량이 인쇄물과 갈라진다.
- 조정 귀속 계정은 **그 장의 것**을 그대로 쓴다. 계정별 발행 회사는 장마다 수량이 다르다.
- [수정 재발행] = 삭제 → 같은 기간 재발행. 조정은 회사×기간 축이라 그대로 다시 실린다. 재발행 단위는 `billings.scope`가 `combined`면 `combined`, 아니면 `by_user`다(공통 장은 계정별 발행에서만 생긴다).
- **`superseded_at`은 쓰지 않는다** — 정산을 지우면 추적행이 CASCADE로 함께 사라져 무효화 마커가 의미를 잃는다. 근거는 조정 행의 `reason`이 갖는다.

**④ Codex 재검증에서 굳은 것 (2026-08-04, 8건 중 7건 수용)**

| 결정 | 이유 |
|---|---|
| 컨펌 행은 **`manual_wait`으로 만들고 메일이 나간 뒤에만 승격**(`markConfirmationDelivered`) | 전달 여부와 발행 정책이 한 컬럼에 섞여 있었다. `pending`으로 먼저 만들면 SMTP 실패·타임아웃으로 고객이 못 받았는데도 3일 뒤 세금계산서가 자동 발행된다. 승격이 한 곳뿐이라 재발송으로 살아난 건도 그때 마감이 다시 잡힌다 — "수동에 영영 갇히는" 경로가 함께 사라졌다 |
| 재발행 가드는 `ready`까지 막고 `taxbill_issues`를 **`FOR UPDATE`로 잠근다** | `ready`는 발급 큐 진입 상태다. 워커가 그 행을 집어 외부로 발행하는 동안 장을 지우면 발행은 옛 내용으로 나가고 새 장이 다시 원본 대상이 되어 **국세청에 같은 건이 두 장** 나간다 |
| 삭제·재발행 완전 원자화는 **불수용** — 대신 **삭제 전 preflight** | 발행 코어를 외부 트랜잭션에 참여시키는 공사다. 재발행 실패의 실질 원인 둘(조정 대상 줄 없음·조정 후 음수)을 삭제 전에 걸러 "정산만 사라지는" 창을 없앤다. 완전 원자화는 별도 과제 |
| 음수 판정은 **장 분할 이후 장별**로, 키에 계정을 넣지 않는다 | 회사 전체 합으로 보면 계정 A가 −1인데 B가 100건이라 통과해 과청구가 된다. 반대로 키에 계정을 넣으면 합산 발행의 조정(`user_id` NULL)이 원본 행과 다른 버킷이 되어 **정상 하향 조정이 거부된다**(직전 수정의 회귀) |
| 적용 여부는 **시각 추론이 아니라 `applied_delta` 기록** | 조정을 수정하면 `updated_at`이 갱신돼 이미 실린 델타까지 미적용으로 뒤집혔다 |
| 세션 advisory lock은 **전용 client**로 획득·해제 | `pool.query`로 잡고 풀면 해제가 다른 연결에서 돌아 실패하고, 소유 연결이 대여되면 재진입으로 중복 실행이 뚫린다 |
| 조정 화면은 **요청 세대 + 대상 대조** | 늦게 도착한 옛 조회 응답이 화면을 덮으면 그 뒤 저장이 다른 회사 정산을 바꾼다 |
