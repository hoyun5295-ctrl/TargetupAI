...
### 4-4. M1 재시도 판단 (Harold 승인 게이트) — ★2026-07-18 저녁 재도입 배포·종결(B-0718-1 Closed)
> Harold 승인(토요일 저트래픽) → 965a6618 원본 복원 + exclude 패키지(커밋 eab1f413) → 로컬 3회 반복 빌드 통과 → Codex 3라운드(성능 결함 3건 정정: 선로딩 미작동·복원 전 이중 선로딩·Dashboard↔위저드 청크 결합) → 서버 게이트 실전 통과(35건 실존·비literal 0) → 라이브 전수 177건 실패 0 → Harold 화면 실측 통과. 첫 로드 5MB→0.37MB(약 13배). 아침 잔재 트래픽(열린 탭 복원)도 로그로 해명 — 재배포 후 원시 경로 요청 0건.
