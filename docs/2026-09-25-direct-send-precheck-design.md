...
7. **⛔ 본문 칸 높이 = 글 길이**(약 1,000byte까지 · `--ds-editor-cap: 500px`) · 넘으면 칸 안 스크롤 + "미리보기로 한 번에 보기". 창 높이 = 최소 600 · 최대 화면 높이.
...
- 실제 문자 문안에서 AI 교정 품질(오탐률) · 1,000byte 높이(500px)가 실제 글꼴에서 몇 byte까지 보이는지.
