...
| 0925 | **배포완료**(Harold 세션 마무리 · DDL 2 는 배포 전 실행 완료) · 다음 = 실측 |
| 0926 | **사용 현황 화면(ceo 전용 · Harold 지시)**: 슈퍼관리자 발송 관리 > "점검 사용 현황"(`components/admin/PrecheckUsageTab.tsx`) · 집계 CT `utils/precheck-usage.ts`(읽기만 · 원천 = `spam_filter_tests.source` · `spell_check_uses` · `agency_send_events` 'spell_checked' · 새 칸 0) · 무료 한도 표시 = 고객 화면과 같은 함수(`readSpamTrialStatus` · `loadPlanContext`+`isActivePaidPlan` · `readSpellUsage`) · 게이트 = `isPrecheckUsageViewer`(ENV `PRECHECK_USAGE_VIEWER_IDS` 기본 'ceo' + 권한표 `precheckUsage` 최고 등급 R) · `GET /api/admin/precheck-usage`(+`/access`) · 다른 계정은 메뉴 없음 · 직접 호출 403. 기간 = 오늘(기본)·최근 7일·이번 달(한국 자정 기준) · 요약 4칸 · 업체별 · 최근 기록 10건씩 · 테스트 precheck-usage-0926 10 |
