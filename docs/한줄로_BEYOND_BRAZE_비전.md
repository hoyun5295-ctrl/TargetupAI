...
**버전:** v0.8 (D217+ AI Operator 10 메뉴 Journey Builder 동급 강화 종결 — AI 메모리 + AI 사용량)
**최종 갱신:** 2026-05-25
**체어맨:** Harold
...
| **v0.8** | 2026-05-25 (D217+ AI Operator 10 메뉴 강화 종결) | AI 메모리 + AI 사용량 Journey 동급 완성 + 옛 사고 영구 정정 | **AI Operator 10 메뉴 모두 Journey Builder 동급 강화 완료** — 마지막 2 메뉴 (9번 AI 메모리 + 10번 AI 사용량) Journey 동급 8 화면 강화로 전체 메뉴 통일 정합. **(AI 메모리 강화)** routes/ai-memory.ts 3 endpoint + AiMemoryPage 전면 재작성 옛 457줄 → 758줄 (자율 진단 + 자연어 + 빠른 시작 7 카드 + 5 타입 도넛 + 영향도 top 10 + 1-click 액션 3 + 자세히 분석 4 차트 + Source caption) + components/AiMemory/ 3 신설. **(AI 사용량 강화)** routes/ai-usage.ts 4 endpoint + AiUsagePage 전면 재작성 옛 295줄 → 627줄 (자율 진단 + 자연어 + 빠른 시작 5 카드 + 5 metric 격차 + 비용 예측 선형 회귀 라인 차트 + 1-click 액션 3 + 자세히 분석 4 차트 + Source caption) + components/AiUsage/ 3 신설 (ThresholdAlertModal + BatchModeGuideModal + CostForecastChart). **(옛 사고 영구 정정)** ai-rate-limit getMonthlyUsage `c.plan_code` → `c.plan_id = p.id` JOIN 정정 (옛 D215+ PM2 로그 silent fallback 0 사고 영구 종결). 전수 grep 1곳만 (다른 30+ 정합). **(DB ALTER 1건)** companies.ai_usage_threshold_config jsonb (한도 알림 설정). **(영구 원칙 #3 모델 분리 + #4 사용자 신뢰 정합)** UI 안 모델명 Opus/Sonnet/GPT/Claude 0건 — 추상 명칭 "고급/표준/보조 추론 모드" 사용. AI search-natural 시스템 프롬프트 안 "구체 혜택 임의 생성 X" + "모델명 노출 X" 명시. **(영구 원칙 #6 회사별 메모리)** AI 메모리 5 타입 (성공 패턴 / 채널 성과 / 고객 인사이트 / 브랜드 톤 / 컴플라이언스 학습) 자율 누적 + 자연어 검색 + 영향도 시각화 = "시간 지날수록 정확도↑" 본질 활성. **(영구 룰 신규 강화)** feedback_default_superpowers_workflow § D217+ 강화 — 매 답변 직전 자가 grep verification-before-completion 호출 의무 + 인지 ≠ 실행 분리 사고 차단. **AI Operator 10 메뉴 통합 매트릭스 종결** — 1번 여정 자동화 + 2번 AI 자율 예측 + 3번 AI 자동 마케팅 + 4번 성과리포트 + 5번 자사몰+데이터 융합 + 6번 인앱메시지 + 7번 Email 캠페인 + 8번 모바일 DM + 9번 AI 메모리 + 10번 AI 사용량. 다음 단계 = D218+ 운영 검증 + 신규 영역 진입 (Harold 본질 명시 영역) |
| **v0.7** | 2026-05-20 (D184 이니시스 결제 한줄로 이전 + D186 모바일 반응형 + D187 Journey 설계) | 결제 모듈 전환 + 모바일 본격 진입 + Journey Builder 설계 | **(D184 이니시스 표준결제 한줄로 이전 완료)** 레거시 invitobiz.com Tomcat6 INIpay50 Java SDK → 한줄로 Node.js HTTP API 이전. CT-41 inicis-client + CT-42 payment-processor 트랜잭션 + idempotent + 금액 위변조 검증. payments ALTER 9 컬럼 + UNIQUE INDEX 2건. 운영 MID usomsms001 + signKey 정합. **(D186 모바일 반응형 3 단계)** Phase 1 = 7 파일 19 모달. Phase 1.5 = DashboardHeader + 메인 카드 stacked + grid 모바일 분기. Phase 2-A = direct-send.css @media 0건 + 스크롤 사고 정정. B2B SaaS 모바일 환경 본격 진입. **(D187 Journey Builder Lite 설계 완료)** status/D187_journey_builder_lite_plan.md SoT. 7 표준 여정 + 회사 자유 임계값 (Harold 사용 사례 영역 정합 — 일 300만건+) + 광고/비광고 둘 다 자동 (4 광고 검증 통과 시) + 5분 cron + 여정별 재진입 default. Braze Canvas 압도 차별화. **다음 세션 = Step 1-A 즉시 진입**. **(영구 원칙 추가 사고 패턴 3건)** LESSONS_LEARNED §4 신규 — MANDATORY_CHECKLIST 출력 누락 + 박음/박힘 thinking leak + 영역/본질 단어 과도 사용. 자가 grep 검증 영구 강화 |
| **v0.6** | 2026-05-19 (D182 직원 신고 fix + Phase 1 UI 활성) | CTO 진정 영구 안전망 + Memory 본질 활성 | **(직원 신고 3건 영구 fix)** 선불 타임아웃 환불 사고(돈 관련) 영구 안전망 — campaign-lifecycle 30→120분 + reverseTimeoutRefundIfRecovered idempotent reverse 함수 + 24h 윈도우. 보관함 메시지 전문 표시 토글. 패스워드 초기화 SMS 담당자 테스트 라인(SMSQ_SEND_10) 분리. **(Phase 1 UI 활성)** AiMemoryPage + AiBatchesPage + AiExplainPage 3 페이지 신설 → endpoint만 박혀있던 Memory/Batch/Citations이 진정 회사 admin 진입 영역으로 활성. **(자동 학습 cron)** mysql-refund-sweeper에 accumulateCampaignLearning 30초 주기 추가 → 캠페인 종료 시 자동 ai_company_memory 누적. **영구 원칙 #6 본질 활성** — "시간 지날수록 정확도↑" 더 이상 수동 호출 X. **(헤더 간소화)** 모바일DM 메뉴 영구 제거 + SUB_MODULE_CARDS 11건으로 확장. 영향받은 디에스패션/태영 86.9원 보전 SQL 안내. Harold 신뢰 회복 — 돈 관련 영역은 단순 임계값 변경 X, 영구 안전망(reverse + cron + idempotent) 구축 |
