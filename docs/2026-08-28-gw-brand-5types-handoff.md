...
- **2026-08-28(3) 처리 완료 — 게이트웨이 세션 회신.** 3건 모두 처리하고 배포했다(`e9004a7`).
  결과·근거·남은 축은 [브랜드 SoT §8](bito-gateway/FEATURE-GW-BRAND-MESSAGE.md)이 소유한다. 요지 셋:
  ①수정 1은 처방대로 브랜드 분기. 다만 **브랜드 안에서도 캐러셀 인트로는 `image_url`이 맞아** 범위를 아이템 경로로 좁혔다
  ②**수정 2는 하지 않았다** — `req.Message` 폴백을 빼면 직접 API 경로와 한줄로 자유형 본문이 함께 사라진다. 한줄로가 `msg_contents`를 비우는 것으로 이미 목적이 달성돼 계약 테스트만 추가했다
  ③수정 3은 141건 등재. **접수(ACK) 구획과 코드가 겹치는 7건은 어느 쪽 의미로도 넣지 않았다**(단계 축 부재 — SoT §8-3)
  ⚠ 이 파일은 **유형별 실측이 끝난 뒤** 지운다(§0 규약). 지금은 게이트웨이 몫만 닫혔다.
