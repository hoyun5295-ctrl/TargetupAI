# 싱크에이전트(Sync Agent) — 기능 상설 SoT

> **호출어: "싱크에이전트"** — Harold님이 싱크에이전트를 언급하면 이 문서를 먼저 연다.
> 이 문서가 **구조·불변 원칙·커서 규약·배포 게이트·원격 릴리즈·이력**을 소유한다.
> 세부 절차만 위임한다 — 검증·빌드 절차 = [빌드 런북](2026-07-28-sync-agent-build-verification-runbook.md), 증상별 진단 = [SYNC-AGENT-TROUBLESHOOTING](../status/SYNC-AGENT-TROUBLESHOOTING.md).
> **상태·잔여는 STATUS §2 카드가 소유한다**(여기 쓰지 않는다 — doc_ownership).

---

## 1) 정의

고객사 내부 DB(Oracle·MSSQL·MySQL·PostgreSQL·엑셀/CSV)를 읽어 한줄로 원장(`customers`·`purchases`)으로 밀어 넣는 설치형 에이전트.

**싱크에이전트와 자사몰 연동은 한줄로 타겟팅의 데이터 기반이다 — 여기가 새면 여정·자동마케팅 전부가 조용히 0건이 된다.**
문은 회사마다 하나(여정 불변 원칙과 동일) — 같은 사실이 두 경로로 들어오면 중복 발송·중복 과금이 된다.

## 2) 불변 원칙 (어길 수 없는 것)

| 원칙 | 뜻 | 어겼을 때 |
|---|---|---|
| **커서에 우리 시계를 넣지 않는다** | 커서는 **가져온 행에서 읽은 값**이다. 동기화 완료 시각을 넣는 순간 소스 컬럼과 축이 어긋난다 | 2026-08-03 이새 — 판매일에 시각이 없어(전량 자정) 그날 첫 배치 뒤 하루치 영구 탈락, 한 달 유실 98% |
| **커서 시각은 DB 원문 문자열로 왕복** | 조회 때 DB가 만든 문자열(`TO_CHAR`·`::text`)을 저장하고 같은 형식으로 파싱시킨다. JS Date 왕복 금지 | 드라이버 변환이 프로세스 TZ 의존 — 재부팅·TZ 변경 사이에 벽시계가 밀린다 |
| **커서는 닫힌 경계까지만** | 아직 행이 추가될 수 있는 구간(열린 ts 버킷·미완 스캔) 안으로 커서를 밀지 않는다 | PK 단조는 아무도 보장 안 한다 — 같은 ts에 낮은 PK가 나중에 삽입되면 영구 유실 |
| **바인드는 컬럼 타입에 맞춘다** | 타입 변환은 항상 상수 쪽에서. Oracle `DATE` 컬럼에 `TIMESTAMP` 바인드 금지 | 컬럼 쪽 캐스트 = 인덱스 불가 = 수백만 행 30분마다 풀스캔 |
| **적재는 멱등** | `source_row_key`(원본 PK) + `ON CONFLICT DO UPDATE`. 같은 행이 몇 번 와도 한 줄 | 재싱크·재전송·커서 겹침이 전부 중복 행이 되고, 고객 구매요약 재계산까지 오염 |
| **키는 자르지 않는다 · 키 없이 보내지 않는다** | 상한 초과·비스칼라는 그 행만 거부+사유 보고 | 자르면 다른 원본 행이 같은 키 / 키 없이 보내면 재조회마다 중복이 쌓인다 |
| **`created_at`(도착 축)은 UPSERT가 갱신하지 않는다** | 여정 구매 원장 커서가 도착 축으로 읽는다 | 값이 올라가면 이미 발화한 구매가 커서 창에 재진입 — 재발송 |
| **판정 근거가 없으면 정직하게 잠근다** | PK 없음·비스칼라 PK·시각 전부 NULL·소스 식별 불가 = 증분 잠금+사유 보고. 자연키를 지어내지 않는다 | 지어낸 키 = 정답표. 같은 날 같은 상품 두 건이 한 건이 된다 |
| **커서 전진은 서버 성공 후** | API 실패(네트워크·5xx) = 미전진(재시도 몫). 행 검증 실패는 전진+`failures` 보고 | 실패 행에 걸려 멈추면 그 청크 영구 재조회 루프 |
| **메타 조회 실패는 fail-closed** | 컬럼 메타가 실패·0행이면 던지고 캐시하지 않는다 | 빈 결과를 성공 캐시하면 보정이 프로세스 수명 동안 꺼진다 |
| **검증된 조합만 출고** | `VERIFIED_COMBOS` fail-closed + **검증은 버전 세대에 귀속**(변경 = 버전 올림 = 재검증) | 런북 §8 — 2026-07-27 빌드 먼저 사고 / 옛 세대 검증 승계 = 다른 물건을 검증됐다고 내보냄 |
| **자동 업데이트는 올라가는 방향만** | 원격 릴리즈로 옛 버전이 내려가면 안 된다 | 고친 결함이 무선으로 되돌아온다(§6) |
| **시크릿은 해시만 남긴다** | `companies.api_secret_hash`(sha256)만 저장하고 원문은 발급·재발급 응답에 **1회만** 싣는다. 인증은 `api_key`(유일)로 찾아 고정 시간 비교(`utils/secret-hash.ts`). ★2026-09-12 Harold 확정 | 원문 컬럼을 되살리면 DB가 새는 순간 전 고객사 키가 그대로 드러난다. 같은 표의 자사몰 키는 처음부터 해시였다 — 이 축만 평문이었다 |
| **마스킹은 최종 출력으로 검사한다** | 로그 마스킹 검증은 포맷을 통과한 **출력 문자열**을 본다(`logger/masking.contract.test.ts`) | 함수 단위 테스트는 통과하는데 연결이 끊겨 있을 수 있다 — 실제로 `info.meta`만 보던 포맷이 호출 형태와 어긋나 **한 번도 동작하지 않았고** 비밀번호가 평문으로 로그에 남았다(2026-09-12 발견·정정) |
| **로그 호출은 어떤 값을 받아도 예외를 던지지 않는다 · 오류는 허용 목록 필드만 남긴다** | 마스킹은 순환 참조를 끊고(`[Circular]`) 깊이를 제한한다. Error는 **허용 목록**(이름·메시지·스택·code·errno·syscall·address·port·status·응답 status·서버 안내 문장·요청 method·url + **DB 드라이버의 구조화된 진단 값** number·state·lineNumber·errorNum·constraint·sqlState 등)만 옮긴다. ⛔ 쿼리 원문 `sql`과 자유 문장 `detail`·`hint`·`sqlMessage`는 넣지 않는다(제약 위반 detail에 행 값이 들어간다 · Codex 4R high). 오류 문장(`message`·`stack`)은 진단에 필요해 남기되 **안의 이메일·휴대폰 번호는 가린다**(`scrubText` · Codex 5R). 순환 판정은 지나온 경로 기준이라 같은 객체를 두 필드에 넘겨도 둘 다 찍힌다. 포맷 전체가 실패하면 level·message·module만 남기고, 콘솔 출력은 직렬화할 수 없는 필드를 생략한다. 인증 헤더(`X-Sync-ApiKey`·`X-Sync-Secret`·`Authorization`)도 키로 가린다 | ★2026-09-13 0912 정정이 info 전체를 따라 내려가자 axios 오류의 순환 요청 객체에서 무한 재귀가 났고, winston은 포맷 예외를 **호출부로 던진다**. 그 호출부 뒤에 첫 등록 실패 시 로컬 모드 계속·전송 실패 시 오프라인 큐 저장이 있어 둘 다 죽는 경로였다(적대검토 high · 에이전트 새 빌드 전이라 고객사 반영 0). 계약 테스트 = 실제 포맷을 import해 순환·axios 오류 사례로 검사. ⛔ **axios `toJSON`을 통째로 쓰지 마라**(Codex 2R high): `config.data`는 JSON **문자열**이라 키 기준 마스킹을 통과하고, 등록 요청 본문에 `apiKey`·`apiSecret`이 들어 있다. 가릴 것을 찾아 지우지 말고 남길 것만 고른다 |

## 3) 구조 — 파일별 소유

| 층 | 파일 | 소유하는 것 |
|---|---|---|
| 엔진 | `sync-agent/src/sync/engine.ts` | 증분·전량 흐름 · 커서 전진 규칙 · fingerprint 조립 · 잠금 판정 |
| 커서 공통 | `sync-agent/src/db/keyset.ts` | 키셋 술어 전개형 · `source_row_key` 직렬화 · 행 커서 성분 추출 (순수 모듈) |
| 커서 저장 | `sync-agent/src/sync/state.ts` (`data/sync_state.json`) | 대상별 커서 · 증분 보류 · 명령 멱등 · ACK 보류함 · 원자 저장 |
| 어댑터 | `sync-agent/src/db/{oracle,mssql,mysql,postgresql,excel-csv,mock}.ts` | 조회 SQL · PK 해석·타입 검증 · **커서 원문 직렬화** · `getSourceId` |
| 전송 | `sync-agent/src/api/client.ts` | 서버 payload(`source_row_key` 포함) · 재시도 |
| 스케줄 | `sync-agent/src/scheduler/index.ts` | 주기 실행(고객·구매 개별 주기) · heartbeat 매 정각 |
| 자동 업데이트 | `sync-agent/src/updater/index.ts` | 버전 확인 · checksum 필수 검증 · 교체 스크립트 |
| 서버 수신 | `packages/backend/src/routes/sync.ts` | 인증 · 배치 상한 · `sync_logs` 기록 · `/version`·`/download` |
| 서버 적재 CT | `packages/backend/src/utils/sync-ingest.ts` | 키 정규화 · 배치 내 중복 제거(키 정렬 = 교착 방지) · 멱등 UPSERT SQL · 42703 강등 |
| 고객 적재 CT | `packages/backend/src/utils/customer-upsert.ts` | `(company_id, store_code, phone)` 멱등 — 처음부터 멱등이었다 |
| 티어·검증·게이트 | `packages/backend/src/utils/agent-build-tiers.ts` | OS 티어 판정 · `VERIFIED_COMBOS` · `CURRENT_AGENT_VERSION` · `isPackageKeyVerified` |
| 배포 화면 | `packages/frontend/src/components/admin/AgentDeployWizard.tsx` | OS→티어→드라이버 판정 표시 · 미검증 안내(게이트는 서버) |

**산출물 축** — 소스 하나, 바이너리는 OS 티어 5종(win-modern·win-2012·win-2008r2·linux-modern·linux-legacy). 드라이버 4종은 한 exe에 들어 있고 지연 로드. **회사별 빌드가 아니라 조합별 검증이다.**

## 4) 커서 규약 (2026-08-03 확정 — Codex 적대검증 5R approve)

대상(customers/purchases)별 `{ tsRaw, keys[], pkColumns[], fingerprint }`.

- `tsRaw` = 타임스탬프 **DB 원문 문자열**(어댑터가 SELECT에 원문 컬럼을 함께 실어 온다). 되돌릴 때는 컬럼 타입에 맞는 상수식으로만 — Oracle `TO_CHAR`/`TO_DATE`·`TO_TIMESTAMP` / MySQL `DATE_FORMAT`/`CAST` / MSSQL `CONVERT` style 121 / PG `::text`+상수 캐스트.
- **저장하는 것은 마지막 행이 아니라 "닫힌 버킷 경계"** — 더 큰 ts가 관측된 버킷의 마지막 행까지. 최대 ts 버킷(열린 버킷)은 저장하지 않고 매 주기 재조회하며, 겹침은 서버 멱등이 흡수한다. `keys=[]` = 열린 버킷 시작 커서(`ts >=` 재조회).
- **`fingerprint` = JSON 배열 직렬화**(dbType·sourceId·테이블·ts컬럼·PK구성). 구분자 결합은 값 안의 구분자로 서로 다른 소스가 같은 문자열이 될 수 있어 폐기했다. `sourceId`도 어댑터가 JSON으로 만든다(host·port·db·**계정**, PG는 스키마 포함 — 계정이 스키마 해석을 정한다). 하나라도 다르면 커서 폐기 → 전량 재기준(fail-closed). **키셋 경로는 `getSourceId` 필수** — 없으면 증분을 잠근다(같은 dbType 다른 소스가 같은 fingerprint가 된다).
- 조회 조건은 전 DB 공통 **전개형** `ts > :t OR (ts = :t AND (k1 > :k1 OR …))`, 정렬 `ts ASC, k1 ASC, …`. Oracle이 행값 튜플 비교를 지원하지 않아 전개형이 유일한 공통 길이다. 페이지 넘김은 런 안에서만 튜플 strict >.
- **키는 fail-closed** — `source_row_key`(PK 값 이스케이프 결합) 직렬화 불가 행(초과 길이·NULL·비스칼라)은 **보내지 않고** 행 실패로 보고한다. PK 타입은 어댑터가 검증한다: Oracle NUMBER는 문자열 fetch+`TO_NUMBER` 바인드(2^53 반올림 차단, 증분·전량 양쪽), MySQL 정수는 `DECIMAL(65,0)` 캐스트(문자열 근사 차단), MSSQL·PG는 정수·문자 계열 화이트리스트 — 밖이면 명시 오류로 잠근다.
- **전량 씨앗은 완전 스캔 + API 무실패일 때만.** 씨앗 = **시작 시점**의 닫힌 버킷 경계(2단 조회: 최대 튜플 → 그보다 이전 최대). 종료 후 재탐침은 씨앗으로 쓰지 않는다(마지막 페이지 이후 삽입분이 씨앗 아래로 들어가 유실). 카운트가 0인데 행을 받았으면 미완으로 취급한다.
- 커서 전진: API 실패 = 미전진 / 행 검증 실패 = 전진+보고 / 경계는 그 행이 포함된 페이지 전송 성공 후에만 저장(청크 단위 — 중간에 죽어도 이어받는다).
- 시각 NULL 행은 증분에 안 잡힌다(전량에서만 — 동작 보존). 전부 NULL이면 증분 보류 + 매 주기 1행 탐침으로 자동 복구. 옛 형식 커서(완료 시각)는 fingerprint가 없어 자동 폐기.
- **한계(기록)** — 과거 날짜로 소급 삽입(backdating)된 행은 어떤 HWM 커서로도 못 잡는다. 주기 재대조 워커가 별도 과제.

## 5) 배포 게이트 — 검증된 조합만 (2026-08-03)

**케이스바이케이스가 운영 절차가 아니라 구조다.**

- 등재 키 = `<osTierId>__<dbId>__<version>`. **검증은 버전 세대에 귀속** — `CURRENT_AGENT_VERSION`과 일치하는 등재만 verified다. 에이전트 버전을 올리면 이 상수도 함께 올리고, 그 순간 전 조합이 자연 폐쇄된다(조합별 재검증 후 개방).
- **실제 게이트는 서버 다운로드 길목**(`isPackageKeyVerified` → 403 `AGENT_BUILD_NOT_VERIFIED`). 화면 잠금만으로는 우회된다.
- 슈퍼관리자 "싱크에이전트 배포" 화면은 **없애지 않는다.** 그 화면의 값은 다운로드 버튼이 아니라 OS→티어→드라이버 판정이다(2008 R2에 node20 빌드를 보내는 실수를 막는 화면). 미검증 조합은 "확인되지 않음 — 담당자 확인 후 개별 전달"로 표시된다.
- packageKey 하나에 여러 (osTier×dbId)가 매핑되므로(oracle zip 하나 = 10g~21c), **그중 하나라도 현 세대 verified면** 그 zip이 열린다.
- 등재 근거는 `scripts/smoke-combos.sh` PASS. 절차·조합 현황 = [런북](2026-07-28-sync-agent-build-verification-runbook.md) §1~§3.

## 6) 원격 릴리즈 (자동 업데이트)

**경로** — 등재(`POST /api/admin/sync/releases`, 같은 티어 기존 활성 자동 비활성) → 에이전트가 매시간 `GET /version`(서버가 `os_info`로 티어 판별, 정확 티어 우선·전역 폴백, 판별 불가 = 대상 제외) → `download_url` 상대경로를 에이전트 baseURL에 결합해 다운로드 → **checksum 없으면 교체 거부** → sha256 검증 → OS별 교체 스크립트 → 재시작.

✅ **다운그레이드는 서버에서 막혀 있다**(★2026-09-12 코드 실측으로 확인 — 종전 "미착수" 서술이 낡았다).
`routes/sync.ts` `GET /version`이 `compareSemver(릴리즈, 에이전트) > 0`일 때만 `updateAvailable`을 주고,
그때만 `downloadUrl`·`checksum`을 응답에 싣는다. 에이전트는 **checksum이 없으면 교체를 거부**하므로(`updater/index.ts`)
옛 버전 활성 행이 남아 있어도 내려가지 않는다. 이중 차단이다.
⛔ 다만 에이전트 쪽 자체 판정은 여전히 `latestVersion === currentVersion`만 스킵한다 — **서버 응답이 유일한 방벽**이다.
`/version`에서 버전 비교를 걷어내거나 `updateAvailable`과 무관하게 checksum을 실어 주면 그 자리에서 구멍이 열린다.

⚠ **이새 1.5.7은 자기교체 결함이 있어 원격 업데이트를 못 받는다** — 유지가 정상이고, 교체는 신규 설치로 한다(2026-07-10 v1.6.1 축 종결 판단). 타 ERP 전환·월말 재싱크 때 1.7.0 신규 설치.

## 7) 이력 색인

| 시점 | 무엇 | 근거 |
|---|---|---|
| **2026-09-12** | **보안 전환: 시크릿 해시 저장 · 로그 마스킹 정정 · 다운그레이드 차단 확인.** ①`companies.api_secret`이 평문이라 DB가 새면 전 고객사 키가 드러났다(같은 표의 자사몰 키만 해시였다) → `api_secret_hash`(sha256) 저장 · 원문은 발급·재발급 응답에 1회만 · 인증은 `api_key`(유일 · 실측 중복 0)로 찾아 고정 시간 비교 · 전환기 폴백 세 갈래 ②로그 마스킹 포맷이 `info.meta`만 봐서 **한 번도 동작하지 않았다**(호출부 256건이 다른 형태 · `password` 평문 기록을 실행으로 확인) → 제자리 마스킹 + 최종 출력 계약 테스트 ③§6 "다운그레이드 미착수"는 낡은 서술이었다(서버 `compareSemver` + checksum 미제공으로 이미 차단). 운영 실측 = 배포 23:55 KST · 이새 원문 폴백 200 → 해시 채움 → 해시 비교 200. 잔여 = §10 | `routes/sync.ts` · `utils/secret-hash.ts` · `logger/masking.contract.test.ts` · SCHEMA `companies` |
| **2026-08-13** | **아난티 설치 현장 실측 — 결함 3부류 발견·전량 정정 = 1.7.1.** ①설치 배치가 마법사를 `start` 새 콘솔에 띄워 코드페이지 미상속 → 한글 전멸(정정 = exe가 기동 시 `chcp 65001` 자가 실행 `setup/console-encoding.ts` + 배치 `start` 줄에도 동봉) ②목록 조회가 `BASE TABLE`만 봐서 **뷰가 0개 감지** → `번호 선택 (1-0)` 무한 재질문(정정 = MySQL·MSSQL·PG 3어댑터 뷰 포함[오라클은 2026-06-30 이새 때 이미 — **같은 결함 2회째**] + `askSelect` 0건 가드 + Step 3 0건 사유 안내·종료) ③**뷰는 PK 메타가 없어 증분 영구 잠금**(설치돼도 최초 전량 후 갱신 0건 — 이새식 조용한 정지) → **지정 키 신설**: 마법사가 키 컬럼을 고르게 하고 `countKeyUniqueness`로 **유일성·NULL을 그 자리에서 실쿼리 검사**, 통과분만 config(`customerKeyColumns`/`purchaseKeyColumns`) 저장, 엔진 `resolvePkColumns`가 메타 PK 부재 시에만 소비(메타 우선 · 실존 검증 · fingerprint 편입 = 키 변경 시 전량 재기준). 검증 = tsc 0 · 테스트 107건(+5 지정 키 계약) · **마법사 전 구간 자동 주행 실측**(뷰 전용 계정 · order_no 단독 중복 검출 → 복합 키 재선택 → 통과 · config 저장 확인) · smoke PASS 등재. 근본 원인 = **검증 사슬에 "사람이 마법사를 끝까지 밟는 경로"가 없었다**(비대화형 `--test-db`만) — 마법사 자동 주행을 검증 절차에 편입해야 한다(잔여) | §7-3 · `setup/cli.ts` · `engine.designated-key.test.ts` |
| 2026-08-03 | **구매 증분 유실 사고 — 발견부터 정정까지.** 이새 6월 하루 820건 → 7월 78 → 8월 3.7(실제의 1.5%). 원장 185만 중 99.98%가 7/3 전량 동기화 35분에 들어왔고 이후 한 달 증분 총합 382건. 오류 없이 매 회차 0건 정상 보고 — 커서=완료시각(시·분·초) × 판매일 컬럼 시각 없음(전량 자정)이 만나 그날 첫 배치 뒤 하루치 영구 탈락. 전 버전(1.5.7·1.6.4·1.6.5) 공통 | §2·§4 · SCHEMA `purchases` 절 |
| 2026-08-03 | **서버 멱등 적재 선행 배포** — `source_row_key` + 부분 유일 인덱스 + `ON CONFLICT DO UPDATE`. 커서를 먼저 고치면 유실이 중복으로 바뀌기만 한다(원장 유일성이 PK뿐이었다). `created_at`은 갱신 제외 | `utils/sync-ingest.ts` |
| 2026-08-03 | **에이전트 1.7.0 커서 재설계** — 키셋 커서·원문 문자열 왕복·복합 PK·fail-closed 키·fingerprint·전량 씨앗. Codex 적대검증 **5라운드 13건 전량 수용 후 approve**. 어댑터 4종+엔진+상태+payload, 테스트 102건 | §4 · `db/keyset.ts` |
| 2026-08-03 | **배포 게이트 신설** — 버전 세대 귀속 등재 + 서버 403 길목 + 화면 안내. 옛 세대 검증 2건 미승계 | §5 |
| 2026-08-03 | **win-modern×mysql 1.7.0 smoke PASS·등재** — exe 실행·MySQL 접속·테이블/컬럼 판독·PK 해석 76ms. 아난티 출고 세트(zip+매뉴얼 v1.7.0) | 런북 §3 |
| 2026-08-03 | **원격 릴리즈 경로 점검** — 사슬 정상·현재 DB 안전(활성 릴리즈는 1.5.7/win-legacy뿐이라 win-modern 무매칭). 다운그레이드 구멍 발견·미착수 | §6 |
| 2026-07-28 | 빌드 검증 런북·`--test-db`·smoke-combos | [런북](2026-07-28-sync-agent-build-verification-runbook.md) |
| 2026-07-27 | Server 2016 VM 검증(1.6.4 win-modern×mysql 전체 통과). **VM은 검증 후 삭제됨(2026-08-03)** | 런북 §0·§3 |
| 2026-06-30 | 이새 연동 시작(1.5.7 · Oracle · 2008 R2) · 전량 키셋 페이지네이션 | `memory/project_2026_0727_sync_agent_vm_verification.md` |
| 2026-06-11 | 인비토 첫 연동 3건(버전 단일 진입점 등) | TROUBLESHOOTING §2-4 |

### 7-1) Codex 적대검증 판정 이력 (2026-08-03 · 5라운드 13건 전량 수용 후 approve)

나중에 "왜 이렇게 짰나"를 되짚는 근거다. 각 항목은 실제로 유실·중복을 만들 수 있던 경로다.

| R | # | 지적 | 반영 |
|---|---|---|---|
| 1R | F1 | 불완전 전량 스캔에도 씨앗 저장 → 누락 행이 커서 아래로 들어가 영구 제외 | 씨앗 게이트 = 완전 스캔 + API 무실패일 때만 |
| 1R | F2 | 같은 ts에 낮은 PK 후행 삽입 시 "마지막 행" 커서가 영구 유실(PK 단조 미보장) | 커서 = 닫힌 버킷 경계, 열린 버킷은 매 주기 재조회 |
| 1R | F3 | 커서가 소스·테이블·ts컬럼 정체성을 안 가짐 → `--edit-config` 변경 후 옛 커서 재사용 | fingerprint 신설, 불일치 시 폐기 후 전량 재기준 |
| 1R | F4 | 직렬화 불가 PK 행을 키 없이 전송 → 재조회마다 중복 INSERT | 그 행은 전송 제외 + `SOURCE_KEY_UNSERIALIZABLE` 행 실패 보고 |
| 1R | F5 | MySQL BIGINT 커서를 문자열 그대로 비교 → 2^53 위 근사 비교로 건너뜀/재조회 | 정수 PK 바인드를 `CAST(? AS DECIMAL(65,0))` |
| 1R | F6 | Oracle NUMBER PK가 JS number로 반올림 → 인접 PK가 같은 멱등키 | NUMBER PK 문자열 fetch + `TO_NUMBER` 바인드 |
| 2R | F7 | 종료 후 재탐침으로 씨앗 생성 → 마지막 페이지 이후 삽입분이 씨앗 아래로 유실 | 씨앗은 **시작 탐침만**, 재탐침은 보류/재기준 판정 전용. 카운트 0인데 행 수신 = 미완 |
| 2R | F8 | Oracle **전량** 경로에 NUMBER 문자열 fetch 미적용 → 전량에서 멱등키 충돌 | `fetchAll`·`fetchAllKeyset`에도 `fetchInfo` 적용 |
| 2R | F9 | fingerprint에 username 없음 → Oracle 계정(=스키마) 변경에도 커서 유지 | 4종 전부 sourceId에 계정 포함(PG는 스키마도) |
| 3R | F10 | Oracle 메타 실패를 빈 결과로 캐시 → 반올림 경로가 프로세스 수명 동안 부활 | 실패는 throw, 성공 메타만 캐시 |
| 3R | F11 | fingerprint 구분자 결합이 비단사 → 다른 소스가 같은 지문 | JSON 배열 직렬화로 교체 |
| 4R | F12 | `getColumns` 0행을 성공으로 캐시(예외만 실패 취급) | 0행도 throw, 캐시 없음 |
| 4R | F13 | `getSourceId` 미구현 어댑터는 같은 dbType끼리 지문 충돌 | 키셋 경로에서 `getSourceId` 필수 계약, 없으면 증분 잠금 |

**불수용 1건** — `source_row_key`에 테이블명 접두(권고). 같은 테이블명으로 ERP를 갈아타면 접두가 있어도 의미 충돌은 그대로다. 그 축은 fingerprint가 강제하는 전량 재기준 + 재싱크 선삭제(§8)가 담당한다.

**검증 실행 기록** — 에이전트 tsc 0 · 테스트 102건(신규 43: 키셋 순수 24 + 엔진 계약 16 + 게이트 5 · 회귀 0). 서버 tsc 0 · 1,905건. Codex는 실행 환경에 Node가 없어 검사를 못 돌렸고 그 몫은 이쪽에서 실행했다.

### 7-2) 고객사 환경 실측

| 회사 | 구성 | 상태 |
|---|---|---|
| **이새에프앤씨** | Oracle · Windows Server 2008 R2(`win32 6.1.7601`) · 에이전트 1.5.7 · 주기 고객 60분/구매 30분 · `company_id 682956b7-37a3-46b5-9868-b63011bda47b` | 1.5.7 유지(자기교체 결함) · **월말 전량 재싱크 예정 = 선삭제 후 1.7.0 신규 설치**(§8) |
| **아난티** | Aurora MySQL `8.0.mysql_aurora.3.09.0` · Windows Server 2016 Datacenter x64 · TLS 비강제 · **고객·주문 원본이 서로 다른 DB → `antsnd`에 뷰로 모아 뷰에만 SELECT 권한**(2026-08-13 김인환 회신로 확정) | **2026-08-13 Aurora 실접속 성공 실측**(설치 화면 로그 — 미검증 항목 종결). 같은 날 설치가 뷰 0개 감지·한글 깨짐으로 중단 → 1.7.1 정정(§7 최상단). **1.7.1 세트(zip+매뉴얼) 재출고 대기** |

**이새 유실 실측(2026-08-03)** — 월별 하루 평균: 2025-07~2026-06 486~886건 안정 → 2026-06 820.0 → **2026-07 77.8 → 2026-08 3.7**. 원장 1,855,088건 중 1,854,706건(99.98%)이 7/3 15:07~15:42 전량 동기화 566배치로 유입, 이후 한 달 증분 총합 **382건**. `sync_logs`는 매 회차 2.1초·0건·오류 없음(같은 시각 고객 동기화는 매시간 3건 정상 = 데이터가 없는 게 아니었다). 7/17만 18배치 161건 — 커서가 뒤로 밀린 날이라는 증거.

**서버 DB 시간축** — 세션 TimeZone `Etc/UTC`. `purchases.purchase_date`는 KST 날짜(전량 `00:00:00`), `created_at`은 `NOW()` 적재라 UTC 벽시계. **한 행에 축이 둘**이니 비교 시 변환을 명시한다(SCHEMA `purchases` 절).

### 7-3) 버전 이력

| 버전 | 내용 | 출고 |
|---|---|---|
| 1.5.7 | 이새 운영분. 자기교체 결함으로 원격 업데이트 불가 | 이새(유지) |
| 1.6.4 | TLS 일체 + BIGINT + MySQL 키셋 + 마법사 IE11 우회(Server→CLI 라우팅) | ~~아난티~~ → 1.7.0으로 대체(커서 결함 공통) |
| 1.6.5 | `--test-db` 신설(비대화형 DB 진단 — smoke 판정의 전제) | 미출고 |
| 1.7.0 | **커서 재설계**(§4 전량) · `source_row_key` payload · 증분 잠금 판정 · 전량 씨앗. 설치·마법사 경로 무변경 | ~~아난티~~ → 1.7.1로 대체(설치 현장 결함) |
| **1.7.1** | **아난티 설치 실측 정정 세대**(§7 최상단) — 마법사 콘솔 인코딩 자가 설정 · 뷰 목록 포함(MySQL·MSSQL·PG) · 0건 가드·사유 안내 · **뷰 지정 키 + 유일성 실쿼리 검사**(config `customerKeyColumns`/`purchaseKeyColumns` · 엔진 메타 PK 부재 시 소비) · 매뉴얼 원격 계정 예시·뷰 절 신설 | **아난티(세트 준비 완료 — zip 34.5MB + 매뉴얼 v1_7_1 docx·PDF)** |

**DDL 이력** — 2026-08-03 실행완료: `purchases.source_row_key varchar(200)` + `CREATE UNIQUE INDEX ux_purchases_company_source_row_key ON purchases (company_id, source_row_key) WHERE source_row_key IS NOT NULL`. 부분 인덱스라 기존 185만 행(키 NULL)은 걸리지 않는다.

**검증 등재 이력** — **`win-modern__mysql__1.7.1`**(2026-08-13 smoke PASS 68ms + **마법사 전 구간 자동 주행 실측** — 뷰 목록·지정 키 중복 검출·복합 키 통과·config 저장). 옛 세대 미승계: `win-modern__mysql__1.7.0`(2026-08-03 PASS — 1.7.1로 폐쇄) · `win-2008r2__oracle-10g`(2026-06-24, 1.5.x) · `win-modern__mysql`(2026-07-27, 1.6.4).

## 8) 운영 절차 — 재싱크·출고

**고객사 재싱크(전량 다시 받기)**
1. 그 회사 `purchases` **선삭제**. 기존 행은 `source_row_key`가 비어 부분 인덱스 밖이라 새 키 행과 충돌하지 않는다 — 안 지우면 그대로 두 배가 된다.
2. 에이전트 `data/sync_state.json` 커서 폐기(또는 fingerprint 변경으로 자동 폐기) → 전량이 기준을 다시 잡는다.
3. 전량 완료 후 씨앗이 심어졌는지 로그로 확인(미완이면 커서를 안 남기고 다음 주기에 다시 돈다).

**출고 (★2026-08-13 사전 질의서 게이트 신설 — Harold 확정 "질의서 토대로 완벽하게 조합해서 나간다")**
0. **사전 질의서 회신부터.** [`sync-agent/PREINSTALL-QUESTIONNAIRE.md`](../sync-agent/PREINSTALL-QUESTIONNAIRE.md)가 SoT(§1 질문 · §2 회신→판정 매핑 · 출고 판정), 고객 전달본 = `SyncAgent_사전질의서_v1_0.docx/pdf`(`build-questionnaire.js` 생성). **회신 없이 출고 금지** — 리허설은 픽스처에 넣은 구성만 지키므로(영문 VM·자작 테이블이 아난티 뷰·한글을 못 잡은 기원), 회신 구성 그대로 픽스처를 재현해 마법사 전 구간 자동 주행까지 밟은 뒤에만 나간다.
1. 조합 확정 → 그 티어 빌드 → 스테이징 → 해당 드라이버 zip만 재포장.
2. `smoke-combos.sh --only <조합>` PASS → `VERIFIED_COMBOS`에 `<조합>__<버전>` 등재 → 배포하면 그 조합 다운로드가 열린다.
3. **zip과 매뉴얼은 같은 버전끼리 세트로** 나간다(질의서 개정 시 docx 재생성도 같은 규칙).

## 9) 착수 전 필독

1. 이 문서 §2(불변 원칙) — 전부 사고 기원이다.
2. 커서를 만지면 §4 전문. 배포·출고면 §5 + [런북](2026-07-28-sync-agent-build-verification-runbook.md) §1·§8.
3. 증상 진단 = [TROUBLESHOOTING](../status/SYNC-AGENT-TROUBLESHOOTING.md) 해당 절.
4. 서버 적재를 만지면 여정 구매 원장 커서(`journey-purchase-ledger`·`journey-target-extractor`)와 도착 축 계약을 먼저 확인한다.

## 10) 남은 것 (착수 원장 · ★2026-08-14 STATUS에서 이관)

> **실측·후속 대기는 이 절이 소유한다.** STATUS는 경로만 갖는다(0814 Harold 방침).

- **★2026-09-13 인증 전환 마무리** — 순서·명령 = [0913 인계](2026-09-13-session-handoff.md) §2-1~§2-4
  - **★0914 실행완료**: 배포 뒤 이새 실제 키 HTTP 200 · 이새 에이전트 동기화 복귀(해시 인증) · 서버 보관 로그의 인증값 흔적 두 회사 모두 없음 · R0(PG 15 · 불일치 0 · `api_key` 중복 0 · 유일 인덱스 없음) → R1 해시 백필 83행 → R2 원문 비움 84행 → **원문 0**. 남은 확인 = R2 뒤 이새 통신 갱신(인계 §2-4 R3) · `api_key` 유일 인덱스 추가 여부(인계 §2-6)
  - ~~이새 0912 23:30 KST 이후 미접속~~ **★0914 복귀 확인**(해시 인증으로 동기화 · 경과 7분 실측). 서버 인증은 실제 키로 HTTP 200 실측
  - 전환기 판정 보강(사전 컬럼 탐지 제거 · `42703`일 때만 옛 경로) **★0914 배포완료**(원문 제거 전 배포 조건 충족)
  - ★0913 Codex 적대 1R 완료 = high 1(아래 원문 제거 전제) · medium 1(관리자 키 API 3곳이 컬럼 부재에 500 → **503 `DB_MIGRATION_PENDING` 반영 · 미커밋**). 기각 확인 = `query`가 pg 오류 `code`를 보존한다(42703 판정 유효) · 마스킹은 중첩 객체를 복사한다(호출자 설정 객체 불변) · 재발급과 해시 채우기 경합은 `IS NULL` 조건이 막는다
  - **원문 `api_secret` 제거 설계(★0913 · 실행은 Harold님 승인 뒤)**: 접속 때 채우는 방식만으로는 **한 번도 접속하지 않은 키(82개)가 해시 없이 남아** 원문을 비우는 순간 영구 401이 된다(Codex high). 그래서 ①DB 안에서 원문으로 해시를 **일괄 백필**(`encode(sha256(convert_to(api_secret,'UTF8')),'hex')` = `hashSecret`와 같은 값 · PG 11+) ②**원문과 해시가 일치하는 행만** 원문을 비운다 ③남은 원문 0 · 이새 인증 200으로 확인. 원문 백업 테이블은 만들지 않는다(백업이 곧 평문 유출이다 · 해시만으로 인증되므로 되돌릴 대상이 없다). 선행 확인 = PG 버전 · 해시 불일치 0 · `api_key` 중복 0. 명령 = [0913 인계](2026-09-13-session-handoff.md) §2-4
  - ★0913 적대검토 등재만(medium 이하): ~~①슈퍼관리자 회사 상세·목록 API가 `SELECT c.*`로 `api_secret`·`api_secret_hash`를 함께 내려준다~~ **★0913(2) 닫음**: 고객사 응답 6곳(목록·상세 2·생성·수정 2)이 `omitCompanySecrets`(`utils/secret-hash.ts`)로 원문·해시·자사몰 해시·SMTP 비밀번호를 뺀다. 수정(PUT) 2곳은 적대검토 1R에서 같은 결함으로 추가 발견 ②0912 배포 뒤 재발급한 키는 원문이 없어, 백엔드를 0912 이전으로 되돌리면 그 에이전트는 401이다(롤백 안전 창은 원문 제거 전까지가 아니라 **첫 재발급 전까지**) **★0913(3) 운영 제약으로 확정**(코드로 없앨 수 없는 성질이다: 옛 코드는 원문으로만 인증한다 · 인계 §3 "하지 말 것"에 첫 재발급 기준으로 적었다) ~~③재발급 직후 사용 토글을 누르면 1회 노출 시크릿이 화면에서 사라진다~~ **★0913(3) 닫음**(토글 응답에는 원문이 없어도 같은 키면 화면 값을 이어 둔다) ~~④재발급 감사 기록 없음~~ **★0913(3) 닫음**(`sync_key_regenerate` 감사 기록 · 키 끝 4자리만 · 시크릿 원문 미기록) ~~⑤`apiSecret` 마스킹은 앞뒤 4자를 남긴다~~ **★0913(2) 닫음**(전부 가림 · 키는 대조용이라 앞뒤 4자 유지) ~~⑥동기화 실패 로그가 고객 전화번호 원문을 `key` 필드로 남긴다~~ **★0913(2) 닫음**: `logResult`가 `maskRecordKey`를 거친다. 숫자 7자리 이상 조각은 번호로 보고 7자리는 통째로, 8자리 이상은 뒤 4자리만 남긴다(공용 `maskPhone`은 8~9자리를 한두 자리만 가려 쓰지 않는다 · 적대검토 2R). **수용 비용**: 날짜·주문번호처럼 숫자가 긴 원본 PK 조각도 가려져 로그만으로 행을 찾기 어렵다(개인정보 쪽을 택함). 전각 숫자는 NFKC로 반각으로 바꿔 센다(적대검토 3R). ~~참고: ICU 없이 빌드된 실행 파일에서는 NFKC가 아무 일도 하지 않아 전각 숫자가 가려지지 않는다 · 아랍-인도 숫자는 세지 않는다~~ **★0913(3) 닫음**(전각·아랍-인도·확장 아랍-인도 숫자를 코드포인트로 직접 옮긴 뒤 센다 · NFKC는 종전 동작 유지용으로 앞에 둔다 · 원문자(①)·위첨자(²)는 종전대로 숫자로 세어져 가리는 방향) ~~⑦로그 요청 명령은 새 빌드 설치 당일 그 이전에 쓰인 줄도 그대로 올린다~~ **★0913(3) 닫음**(로거 초기화가 `logs/.masked-since`를 처음 한 번만 적고 로그 요청은 그 시각 이전 줄과 시각 없는 줄을 올리지 않는다 · 파일을 읽을 수 없으면 이 프로세스 시작 시각 기준) ~~⑧오류 허용 목록이 PostgreSQL `file`·`line`·`dataType`·`internalPosition`, mysql2 `fatal`(불리언)을 남기지 않는다~~ **★0913(3) 닫음**(허용 목록에 추가 · 불리언 허용 · 자유 문장 `where`·`internalQuery`는 여전히 넣지 않는다) ~~⑨로그 요청 명령은 하루 20MB를 넘어 회전된 파일을 못 보고 기본 파일의 옛 줄을 올린다~~ **★0913(3) 닫음**(`sync-날짜.log.N`까지 받고 가장 최근에 쓰인 파일을 읽는다 · 이름 규칙 = file-stream-rotator 소스 확인) ~~⑩DRY RUN이 정규화된 고객 행을 메시지 본문으로 찍어 마스킹을 거치지 않는다~~ **★0913(2) 닫음**(`maskSensitiveData`를 거친 뒤 같은 모양으로 찍는다 · 이름·주소처럼 마스킹 키 목록 밖 값은 남는다)
  - ★0913(3) 아래 범위 밖 발견 **전량 닫음**(Harold 지시): ⓐ서버 전송 오류 문장을 가린 뒤 보낸다(`sync/error-text.ts` · 검증 실패는 받은 값 없이 필드·코드만 · 500자 상한) ⓑ설정 편집 화면은 시크릿을 한 글자도 보이지 않는다(입력 숨김 · 설정 여부만) ⓒ동기화 경로 custom_1~15도 정리 규칙을 탄다(`normalizeSyncCustomFieldValue` · 원본 보존 원칙 그대로, 드라이버가 준 날짜만 원본 DB가 보이는 벽시계 글자로 되돌림 · Buffer 제외) ⓓ공용 `maskPhone` 8~10자리는 뒤 4자리만 ⓔ`+82` 도달하지 않는 분기 제거(백엔드·프론트 정규화). 아래는 당시 기록이다.
  - ★0913(2) 적대검토 중 발견 · **범위 밖 · 등재만**(코드 직접 확인): ⓐ동기화 로그 서버 전송(`sendSyncLog` errorMessage)이 오류 문장을 가리지 않고 올린다. `gender`가 `z.enum`이라 검증 실패 문장에 받은 값이 들어간다(`types/customer.ts:23`) ⓑ설정 편집 화면이 콘솔에 API Secret·DB 비밀번호 앞 4자를 보여 준다(`setup/edit-config.ts` 자체 `mask` · 로그 아님) ⓒ동기화 경로에서는 에이전트의 custom_1~15 값 정규화가 돌지 않는다(`mapRow`가 `custom_fields` 아래에 넣고 `normalizeCustomer`는 최상위 키를 본다 · 서버 적재 쪽 정규화 여부는 미확인) ⓓ공용 `maskPhone`은 8~9자리 번호를 한두 자리만 가린다(`phone` 키 로그 · 휴대폰 11자리는 4자리 가림) ⓔ`normalizePhone`의 `+82` 분기는 `+`를 먼저 지워 도달하지 않는다(동작 영향 없음)
  - ~~★0913 수용 위험 · Harold님 판단 대기~~ **★0913(3) 닫음**: 드라이버가 행 값을 싣는 자리(MySQL 중복 키·잘못된 값 · PostgreSQL 입력 구문·enum·`Key (열)=(값)` · MSSQL 중복 키 값·변환 실패)는 값을 통째로 `***`로 가린다(`DB_VALUE_SLOTS` · 오류 종류·키·열·제약 이름은 남긴다 · 문장 삭제는 여전히 하지 않는다 · 길이 상한으로 제곱 시간 방지). 값의 끝은 **문장 끝(끝 공백 제외)에 딱 붙은 온전한 끝 구문**만 인정하고, 끝 구문이 없는 형식(`Truncated incorrect … value`)이거나 확인되지 않으면 문장 끝까지 가린다 · 앞 후보로 물러나지 않는다 · 가릴 구간은 **원문에서 모든 형식을 먼저 모아 겹치는 구간을 합친 뒤 한 번에** 바꾼다(형식마다 차례로 바꾸면 합쳐진 오류 문장에서 뒤 값의 시작 표식이 지워졌다 · 6R medium) · 스택 모양 줄이나 메시지 위치로 경계를 나누지 않는다. 오류 스택도 통째로 한 문장으로 가린다(DB 값 자리가 있으면 그 뒤 프레임까지 가려지고, 보통 오류는 프레임이 남는다)(Codex 적대 1R·2R high · 3R·4R·5R medium: 값 안의 따옴표·개행·끝 모양 조각을 끝으로 잡거나, 앞줄 가짜 구문으로 물러나거나, 값 안에서 잘린 가짜 스택 줄·생성 뒤 바뀐 메시지를 믿어 뒷부분이 남았다). 당시 기록(Codex 5R high의 불수용분): DB 오류 문장에는 충돌한 행 값이 들어간다(MySQL 중복 키 등). 이메일·휴대폰 번호는 가렸지만 **이름 같은 다른 값은 문장에 남을 수 있다**. 문장을 통째로 지우면 설치 초기부터 기대 온 현장 진단이 사라진다(기존 동작 회귀). 이 로그는 고객사 서버에 쓰이고, 슈퍼관리자가 로그 요청 명령을 보낼 때만 서버로 올라온다. 추천 = 현 상태 유지, 로그 요청 명령 결과 보관 기간을 짧게. 문장 속 휴대폰 번호 가림(`scrubText`)은 흔한 표기(하이픈·공백·괄호·+82·`(0)`)를 막는 **최선 노력**이고 모든 표기를 보장하지 않는다(Codex 6R·7R에서 표기 범위 지적이 반복돼 거기서 닫았다)
  - ★0913 **이미 설치된 에이전트(1.5.7·1.7.1)는 마스킹이 동작하지 않던 빌드**라 전송 실패 로그에 axios 오류가 찍힐 때 `X-Sync-Secret` 헤더가 원문으로 남았을 수 있다(**미검증** · 로그는 고객사 서버에 있고, 슈퍼관리자가 로그 요청 명령을 보냈다면 서버에도 올라온다). 확인 SQL = 인계 §2-4
  - `api_key` 유일 제약은 **미확인**(SCHEMA는 "유일"로 적지만 인덱스 기록 없음 · 0912 실측 중복 0). 중복이 생기면 `rows[0]`만 검증해 정상 회사가 401. 확인 SQL = 인계 §2-4 R0
- **아난티 설치 재시도 회신 대기** — 서버 등재분 배포가 선행. 출고 게이트(사전 질의서)는 §8이 소유
- **마법사 자동 주행 smoke 편입** — 1.7.1에서 주행은 실측했고 smoke 등재만 남았다
- ~~**원격 릴리즈 버전 가드**~~ — ★2026-09-12 **이미 닫혀 있었다**(§6 · 서버 `/version`이 상위 버전만 내려준다). 착수 대상 아님
- **backdating 재대조 워커** — §4 커서 규약
- **마법사 탈출구 배너** — 런북 §6
- ⛔ "문제없다" 표현 금지 · 상태는 검사 범위·결과로만(§7 0813행)
