...
| 0925 | **배포 뒤 Harold 확인 2건 반영** · ① 사진 초안이 포스터 문구를 한 글자도 안 쓰고 소설식 묘사만 냄 → A안: 사진 속 글을 먼저 옮겨 적고(무작위성 0) 그 글 + 회사 이름을 면허로 해요체 게시글을 쓴다 · 면허 안 사실(숫자·혜택·링크·알림 낱말 OPEN→오픈)은 남기고 밖은 문장째 뺀다 · 화면에 '사진에서 읽은 글' ② 올린 기록이 난잡함 → 카드 5 × 2 · 서버 페이지 10개씩(전에는 최근 50에서 끊김) · 누르면 상세 창(채널 탭마다 올라간 글 그대로 · 할 일 버튼은 창 안에만) · 띠 [보기] = 그 글 창 · `GET /posts/:id` · DDL 0 |
