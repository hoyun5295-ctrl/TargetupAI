...
- **몰 동의 분류코드** = 해제 아닌 자사몰 연동 행(`company_integrations`)의 `meta.store_code`. 그 회사의 몰 동의 분류코드가 1개 이상이면 **몰 동의 회사**.
...
| 자리 | 지금 | 바뀜 |
|---|---|---|
| `identifyCustomer` (`input.storeCode` 있음 + 동의 값 있음) | 고객 행 `sms_opt_in` 을 덮는다 | **동결** — 고객 행 `sms_opt_in` 을 건드리지 않는다(신규 INSERT 는 기본 false). 몰 동의는 소속 행에 `upsertStoreConsent`(`sms_opt_in`·`consent_source`·`consent_at`). 소속 행 생성(`linkCustomerStore`) 뒤에 쓴다 |
...
| 삭제 `DELETE /:id` | 격리 ON 이면 **본인 행만** 지운다. 관리자 행은 그 번호에 다른 사용자 행이 하나도 안 남을 때만 함께 지운다. 고객 행 동의를 되살리지 않는다 |
| 슈퍼관리자 `deleteUserUnsubscribes` | 격리 ON 이면 그 사용자 행만(+위와 같은 관리자 행 규칙) · 되살림 없음 |
...
- 발송 자격 조각은 CT `buildSendConsent(...)` 가 만든다. 분류코드 사용자의 코드가 몰 동의 분류코드면:
  ...
  - 범위 서브쿼리에 `AND sms_opt_in = true` 를 넣는다 → **NULL·행 없음 = 모름 = 제외**.
...
| 지점 | 갈림 | 선택 · 이유 |
|---|---|---|
| 과도기 회사 값 | 기획·백엔드 = "하나라도 미동의면 미동의(AND)" / 회의론자·프론트 = 반대 | **동결.** AND 는 몰 A 거부가 몰 B 를 끄므로 H2 위반이고 남겨진 인덱스처럼 굳는다. 동결은 조건 1개 · 삭제로 되돌림 |
