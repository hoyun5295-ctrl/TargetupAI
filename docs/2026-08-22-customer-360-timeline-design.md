...
- **자리(★2026-08-22 Harold 확정: 페이지 신설 대신 모달 확장)**: 대시보드 "DB 현황" 카드의 "상세보기"가 여는 `CustomerDBModal`을 **직접 타겟 발송과 같은 풀스크린 작업면 규격(`SendWorkspaceShell` 92vh, 인디고)** 으로 키운다. 좌 = 목록·필터(기존 로직 그대로) · 우 = `Customer360Panel`. md 이하 = 패널이 전폭 오버레이(뒤로가기로 목록 복귀). DB 현황 카드 자체는 손대지 않는다.
  - **헤더 "고객" 메뉴 신설**: 카카오&RCS와 직접발송 사이. 직접발송·발송결과 메뉴가 모달을 여는 것과 같은 방식으로 **같은 모달**을 연다(페이지 아님). 게이트 = 현재 모달과 같은 구독 잠금만.
