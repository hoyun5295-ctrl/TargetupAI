...
- **자리(★2026-08-22 Harold 확정 · 2차 정정)**: 대시보드 "DB 현황" 카드의 "상세보기"가 여는 `CustomerDBModal`을 **풀스크린 작업면(92vh, 인디고)** 으로 키우고, 목록은 **전체 폭**을 그대로 쓴다. 고객을 고르면 `Customer360Modal`이 **그 위에 자기 창으로** 뜬다(z-[60]).
  ...
  - **360 모달 톤 정정(4차)**: 롤백 뒤에도 360 모달이 콘솔 톤(CUI_*) + 읽기 폭 880px이라 **1,100px 창에서 내용이 왼쪽에 몰려 오른쪽이 비고**, 헤더에 배경이 없어 요약과 구분이 사라졌다(Harold "각이 잡힌 것도 아니고, X는 왜 저 위치냐, 크기도 부자연스럽다"). 처방 = **껍데기·톤을 `CustomerDBModal`과 같은 언어로**: 스크림 `bg-black bg-opacity-50` · 박스 `w-[1100px] max-h-[88vh]`(고정 `h-`는 기록 적은 고객에서 아래가 빈다) · 헤더 `bg-gray-50 border-b px-6 py-4` + 닫기 `w-8 h-8 rounded-full` 우측 끝 · 요약은 `divide-x` 4칸 · 종류 탭은 옛 필터 버튼 규격 · 날짜 머리는 표 헤더처럼 `bg-gray-50` sticky. 강조만 인디고. **읽기 폭 제한(WRAP)은 폐기** — 이 창은 목록에서 열리는 자식 창이라 폭을 다 써야 각이 잡힌다.
  - ⛔ **일반화**: 자식 창의 톤은 **그 창을 여는 부모 화면**을 따른다. 프로젝트 표준 토큰이 있어도 부모가 다른 언어면 그 자식만 이질적으로 뜬다.
...
- 2026-08-22 저녁 **회사 격리 결함 정정**(§6-R-2, 같은 날 배포완료) + **v2 개편**(2열 작업면 · 검색·기간 · 반복 접기 = [v2 설계서](2026-08-22-customer-360-v2-design.md)가 소유. 아래 "다음 축" 2번은 v2에서 닫혔다).
