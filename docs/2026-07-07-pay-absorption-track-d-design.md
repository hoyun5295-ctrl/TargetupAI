...
- **★ 2026-07-25 신규 소비(서수란 접수)**: `RSRM_SalesMst.CustNm` = 발송통계 화면·엑셀의 **발급명 표시 소스**(`pay-stats.ts fetchCustNames`). 내부 코드 B0039만으론 업체가 못 알아봐 `B0039 / 마리오아울렛_EBIZ` 병기. 실측 = CustId 499개 전부 CustNm 보유·우리 283 발송ID 누락 0(143 갱신 불요). **62 사본이 소스라 143 폐기와 무관하게 유지**되나, 사본이 `InsTm 2026-02-26`에 멈춰 있어 그 이후 신설 발송ID는 이름 공란(폴백=CustId만) → 컷오버 시 143 최신 SalesMst 1회 갱신하면 완전.
- **★ 2026-07-25 대상ID(StoreId) 축**: 발송통계 **엑셀 CSV 전용**으로 (기간×발송ID×대상ID×유형) 분해(`queryPayAgentStoreBreakdown`). 마리오=`400`·아난티=24지점·피케이포유=325업체명으로 청구를 나누는 실사용 축. 화면은 폭증 방지로 집계 유지. **인코딩 손상 발견 — 피케이포유 StoreId가 정상 UTF-8(`한솔축산둔산점` 502,944건)과 EUC-KR 이중인코딩(`ÇÑ¼ÖÃà»êµÐ»êÁ¡` 192,097건) 두 벌로 적재**(hex 실측 확정·컬럼 collation=utf8mb3_general_ci). 같은 매장이 두 대상ID로 갈려 집계가 쪼개짐 = 게이트웨이 ingest 손상(우리 코드 무관). 출력은 원본 그대로, **복원은 별도 과제**(강문희 ingest 교정 또는 통제된 1회 정리).
