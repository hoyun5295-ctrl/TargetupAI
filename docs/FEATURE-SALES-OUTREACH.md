...
| `packages/backend/src/utils/sales-outreach-media.ts` | ★0905 **재료 수집 CT** — 순수 층(상품 카드·이미지 후보·상품 링크·딥링크·법정 표기·theme-color·이미지 헤더 실측·상세 페이지 파싱) + 가드 fetch 층(상품 상세 1홉 · 이미지 실측·사본 저장은 호출부가 저장 함수를 주입) |
