...
- ⛔ **(v9) 우리 데이터로 사용자의 답을 대신하지 마라 — 세는 단위가 문항의 뜻과 다르면 거짓이 된다** — "지난 한 달 몇 번 보내셨나요"에 `campaigns` 행 수를 채웠더니 수신자 1명짜리 테스트 6건이 「6번 이상」이 됐다. 게다가 「실측」 배지가 붙어 사용자가 정정할 수도 없었다. **선치환은 "그 값이 문항의 뜻과 정확히 같은 축일 때만" 쓴다.** 애매하면 묻는다 — 문항 하나 값이 축 등급·표지·병목·처방·요금제 추천까지 연쇄로 오염시킨다.
- ⛔ **(v9) 회사 상태를 바꾸는 1회성 행위는 역할로 막는다** — 진단 완주는 회사당 1행으로 굳고 체험 1회를 소진한다. 담당자가 눌러도 되는 일이 아니다. 게이트는 화면이 아니라 서버가 소유하고(`/state.eligible` + 쓰기 endpoint 재검사), 판정 문자열은 CT 한 곳(JWT `company_admin` — DB 원값 `admin`이 아니다)에 두고 테스트로 못 박는다.
- ⛔ **가시성 판정이 두 벌이면 사용자는 같은 화면을 무한히 본다 (2026-08-17 퍼널 A 문진 진행 불가)** — 위저드의 렌더는 `{...prefilled, ...answers}`로, 답 정리(`pruneAnswers`)는 `answers`만으로 판정하고 있었다. 그래서 **실측 선치환된 게이트(`sending`)가 연 분기**(`manual_ratio`·`send_tool`)에 답하는 순간 그 답이 고아로 삭제되고 커서가 같은 문항으로 되돌아왔다 — 선택지와 무관하게 막힌다. **최근 30일 발송 이력이 있는 FREE 고객사는 진단을 완주할 수 없었다**(실적 0이면 선치환이 null이라 정상 동작 — 그래서 신규 계정 실측에서 안 걸렸다). 처방 = 분기 판정을 `frontend/src/utils/diagnosis-branch.ts` CT로 분리하고 **가시성은 언제나 같은 답 집합으로** 판정한다. 재발 차단 = `marketing-diagnosis-branch-parity.test.ts`(백엔드 러너가 프론트 CT를 실행해 서버 `visibleQuestions`와 대조 + 선치환 회귀).
...
- **`sending` 실측 선치환 폐지.** 세던 것은 30일 내 `campaigns` **행 수**(`created_at` 기준·수신자 수 무관)라 수신자 1명짜리 테스트 발송 6건도 「6번 이상」이 됐고, 그게 발송 실행 축을 상위 등급으로 만든 뒤 리포트 관찰에 **「실측」 배지**까지 달았다. 사용자는 그 문항을 보지 못해 정정할 수도 없었다. 문항이 묻는 것(고객에게 실제로 몇 번 닿았나)을 우리 데이터로 정확히 못 잡으므로 **그냥 묻는다**.
  함께 제거 = `computeSendingPrefill` · `/questions`의 `prefilled` 응답 · `/submit`의 서버 병합/`optionalKeys`/저장 분리(`clientAnswers`↔`merged` → `submitted` 한 벌) · `ReportInputs.prefill` · 관찰의 실측 서술 분기.
  **남긴 것 = 위저드·판정 CT의 `prefilled` 배관**(범용 장치 — 되살릴 때 같은 사고를 막는 계약 테스트가 여기 붙어 있다). `DiagnosisObservationItem.measured`도 구 스냅샷용으로 유지.

**v10 — 진단다움 보강 (★2026-08-20 · 코드 완료·배포 대기 · seed·DDL 0)**
Harold 지시("부족한 부분을 짚고, 실제 진단받는 느낌과 한줄로 해결 어필을 보완") → 실제 진료의 틀
(문진 → 검사 → 진단명 → 처방 → 약 → 복약 지도)에서 비어 있던 자리를 채웠다.

- **진단 유형명** = `cover.type_name`(원장 `TYPE_NAMES` — copy CT). 1순위 병목(축×level · COVER_GAP과
  같은 조합만)에 상태명을 단다: 기록 공백형·명단 분산형·전체 발송형·기본 구분 발송형·발송 휴면형·
  간헐 발송형·텍스트 전용형·일회성 관계형·손 발송 의존형·무측정 발송형·건수만 확인형.
  플랫폼 종속 판정(짚임과 같은 판정)이 명단 병목을 이기면 「플랫폼 종속형」, 병목 0 + 단계 발행이면
  「기본기 완성형」. **이미 있는 판정에 이름만 붙인다 — 새 판정 축이 아니다**(쌍둥이 테스트 무접촉).
- **검사 구간** = `result.checkup`(퍼널 A만). submit이 계정 실측 2값(customers count · is_opt_out count —
  기존 컬럼·SCHEMA.md 등재)을 트랜잭션 밖에서 계산해 usage·브랜드 보이스와 함께 4행 표로 스냅샷.
  화면 = 1장 「계정에서 확인된 것」. ⚠ v9 함정 준수 — **답·판정에 개입 0, 사실 표기 전용**(계약 테스트가
  checkup 유무 = 병목·단계·유형명 불변을 고정).
- **문진 반응** = `/questions.echoes`(`buildSectionEchoes` — 인증·공개 동일). 섹션 경계 화면에 즉석 소견
  1줄: gap = 그 조합의 GAPS cause / good = PRAISES 첫 문장. **definition 정적 파생이라 판정 이중화가
  없다**(§9 가시성 사고와 다른 구조 — answers를 받지 않는다). tone만 내려가고 level은 노출하지 않는다.
- **킥 실물화** — 병목 카드에 그 업종 목업 1장(채널 순번 회전 · 기존 24종 재사용 · 캡션 의무 유지 ·
  클릭 확대). **첫 과제 카드** — granted + 병목 1개 이상이면 4장 머리에 「무료체험 7일 · 첫 과제」=
  병목 1번 direction 첫 문장(서버 문장 재사용 — 신규 서버 문구 0). 진단→처방→약→복약 지도 완결.
- 홍보 위치 계약 무변경 — 신규 서버 문장 전부 자사명 0(JSON 전수 테스트가 자동 커버). 신설 계약 6종 =
  유형명 원장 커버·유일·자사명 0 / 파생 3경로(병목·플랫폼·완성형) / 검사 4행·B 부재·판정 불개입 / echoes 전 게이트 정합.
...
1. 실측 = v3 설계서 §8 시나리오 12종 + v6 §9 K-1·K-2·K-4~K-7(K-3은 위 사유로 폐기) + v7 장 넘김 실측
   ...
   + **v10 실측 3건** — A 완주 1건(유형명 배지·검사 4행·첫 과제 카드) / B 공개(경계 즉석 소견·병목 목업 스트립) / 구 스냅샷 재열람(배지·검사 없이 정상).
