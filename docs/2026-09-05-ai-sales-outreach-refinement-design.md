...
- **운영 첫 육안(Harold · 공개 샘플 `4dbf85c91c`)**: 상품 사진 잘림 → [B-0905-3](../status/BUGS.md) = 호출부에서 `image_fit: 'contain'`(캐러셀 항상 · 세로형 히어로). 룩·재료 축의 지표는 잘림을 세지 않았다 — 다음 회차 지표에 "고정 높이 박스 대 원본 비율" 항목을 넣는다.

### 22-6. "전문가가 만든 느낌" 상향 (2026-09-05(4) · Harold "아직도 부족해 · 전문가가 만든 것과 같은 느낌" · 코드 완료 · 배포 대기)
- **근거 = 육안(캡처 375px)**: 운영 DM `dm-ZxYjBWe` ↔ 직원 실물 올리브영 `Q59eDMq` · 조선미녀 `SLk8T3v`. 격차 = ① 브랜드 색이 없어 기본 보라 토큰(`#4f46e5`)이 밴드·버튼에 나간다(theme-color 없음 → 이니스프리 초록은 로고 PNG에만 있다) ② 홈페이지 배너(글씨가 박힌 디자인물 750×680)를 2×2 정사각 썸네일로 잘라 넣어 "썸네일 덤프"가 된다 ③ 모델이 이미지를 못 보고 지은 갤러리 제목("제품 사용 컷"·"텍스처 & 디테일")이 배너와 어긋난다 ④ 상품 6개를 3+3(두 번째는 76px 썸네일 행)으로 나눠 작다 ⑤ CTA 1개 · 알약형 ⑥ 헤더 워드마크가 작다 ⑦ 꼬리말 법정 표기 3중(모델 notes + legal_text + cs_phone) ⑧ 뷰티 모티프 dot이 제목 끝 빨간 점 = 문장부호처럼 보인다.
- **정정(전부 아웃리치 파일 · 공용 렌더러 0)**: ① 브랜드 색 2차 원천 = `msapplication-TileColor` → **아이콘 PNG 지배색**(`resolveBrandColorGuarded` · og:image가 PNG면 로고 · apple-touch-icon · icon png · pngjs 순수 디코드 · 색 1개만 읽고 로고 픽셀은 산출물에 쓰지 않는다 = 불변 11) + **접근성 보정** `accessiblePrimaryOf`(흰 글자 대비 4.5 미만이면 명도만 12% 단계로 낮춰 통과시키는 첫 색 · 밝은 무채색은 null) — 이니스프리 로고 `#12b660` → `#0c7c41` ② 갤러리 = `list_1xN`(배너 통째 · 원본 비율) 2장씩 ③ 갤러리 제목 = 코드가 비운다(계약에서 제거) · 캐러셀 제목 꼬리 문장부호 제거 ④ 캐러셀 = 한 묶음 최대 6개(4개 이상 classic 2열 스와이프 + 점 · 2~3개 focus · list 폐기) · 8개 이상일 때만 2묶음 ⑤ CTA 전부 `bar`(브랜드 색 풀폭) + **2개 보장**(모델이 1개면 첫 상품 묶음 뒤에 코드가 삽입 · 코너 딥링크 우선) ⑥ 헤더 `brand_size: 'lg'` ⑦ notes에 법정 표기가 있으면 비운다 ⑧ 뷰티 모티프 `none`.
- **포스터 히어로**: classic(큰 박스 + 헤드라인 오버레이)을 시험했다가 로컬 렌더 육안으로 **split(브랜드 색 밴드 위 헤드라인 + 포스터 전체 · 잘림 0)** 이 더 낫다고 확정 · 특례 코드 제거.
...
### 22-8. 세션 마무리 실측 (2026-09-05)
- **메일 발송**: 코드 결함이 아니었다. hiworks SMTP `535 authentication failed` = `OUTREACH_SMTP_USER`가 `hanjul`(전체 주소 아님) + 메일 전용 비밀번호 미발급. 둘을 고치고 `pm2 restart --update-env` 뒤 **검수 메일 도착 확인**(Harold). 절차 = OPS ⑦ ②.
- **배포 상태**: 0905(3) 룩·검수 축은 운영에 올라가 있다(DM `dm-ZxYjBWe`에 구도 attr 실측). **B-0905-3 잘림 정정 · 0905(4)·(5)(전문가 느낌 · 배너 히어로 · 로고)는 코드 완료 · 배포 대기** — 배포 뒤 이니스프리를 **새 업체로 등록**해야 로고·배너 순서·브랜드 색을 새로 읽는다.
- **남은 것(Harold)**: 불변 18 개정(목록 1홉) · 취소선 정책 · 네이버 새 키 게이트 · 메일 전용 비밀번호 재발급(대화에 노출) · 로고 무채색 브랜드의 기본 토큰 대안은 실측 뒤.
- **갈 길**: 문장 축(과장·상투어·어미)은 여전히 모델 몫이고 코드 방벽이 없다. 다음 회차 후보 = 산출물 캡처 기반 판정(사람 1문항) 원장 → 재료·룩 규칙 재조정.
