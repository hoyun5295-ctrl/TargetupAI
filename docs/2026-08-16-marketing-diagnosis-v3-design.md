...
| **v6** | 0816 | 병목 채움 킥(`gaps[].fill`) · 30일 실행 각주 폐지 · 미리보기에 업종 예시 복귀 · 변형 도달 가능성 계약 테스트 | 코드 완료 · **seed 무변경** |
| **v7** | 0816 | 리포트 장 넘김(4장 · 미리보기는 마지막 장에 리드 폼) — 프론트 전용 | 코드 완료 · **seed 무변경** |
| **v8** | 0816 | 읽힘 재설계(줄 길이·타이포 3층·키값 표·요약 수치 tally·병목 두 갈래) + 킥 `{gone,how}` 2줄 | 코드 완료 · **seed 무변경** |
