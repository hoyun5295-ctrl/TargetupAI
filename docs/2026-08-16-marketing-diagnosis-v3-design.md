...
## §8-2 후속 버전 이력 (이 설계서의 원칙 위에서 seed·문구만 확장)

| 버전 | 시점 | 내용 | 배포 |
|---|---|---|---|
| v3 | 0816 | 분기형 문진 27문항 · 스토리형 리포트 V2 · 홍보 위치 계약 | 완료 |
| v4 | 0816 밤 | 도구 스택 축(ERP·CRM·발송 도구) 3문항 + 관찰·짚임·처방 배선 | 완료 |
| **v5** | 0816 심야 | 설문 전수점검 — 플랫폼 종속·전문 툴 서사 분화·상담 툴(보기 3 확장·문항 추가 0) | **완료(`v5 | t | 30` 실측)** |
| **v6** | 0816 | 병목 채움 킥(`gaps[].fill`) · 30일 실행 각주 폐지 · 미리보기에 업종 예시 복귀 · 변형 도달 가능성 계약 테스트 | **배포완료 0817** · seed 무변경 |
| **v7** | 0816 | 리포트 장 넘김(4장 · 미리보기는 마지막 장에 리드 폼) — 프론트 전용 | **배포완료 0817** · seed 무변경 |
| **v8** | 0816 | 읽힘 재설계(줄 길이·타이포 3층·키값 표·요약 수치 tally·병목 두 갈래) + 킥 `{gone,how}` 2줄 | **배포완료 0817** · seed 무변경 |
| **v9** | 0817 | 진단 실행 = 고객사 관리자 전용(서버 게이트) · `sending` 실측 선치환 폐지 | **배포완료 0817** · seed 무변경 |
| v6 | 다음 세션 | 병목 채움 킥 + 각주 폐지 + B 미리보기 예시 복귀 — [v6 설계서](2026-08-16-marketing-diagnosis-v6-kick-design.md) 소유 | 미착수(코드만·SQL 0) |
