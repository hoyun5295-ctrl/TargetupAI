...
- **0926 19:21 배포 완료(Harold · tp-push → pull → backend·frontend build:safe → `pm2 restart targetup-backend --update-env`).** 확인: 재시작 루프 없음(↺ 757 유지) · 기동 오류 = 기존 A-08뿐 · MySQL 읽기 정상(예약 상세 17,314건·캠페인 결과 14,003건 화면) · S1-H09 토큰 = 틀린 토큰 401 · 옛 토큰(PREV) 400(인증 통과 · 필수 항목에서 멈춤) · 새 토큰 = .env(값 비출력). **남은 것:** ①S1-H06 실측 1건(후불 테스트 회사 브랜드 테스트 → 테스트 목록 "브랜드메시지" · 정산 미리보기 "테스트 브랜드메시지" = 1 × 브랜드 단가 · MySQL 적재 확인 겸) ②연휴 뒤 폰 3대 "API 토큰" 교체·[등록] → 스팸 검사 1회 → `SPAM_APP_TOKEN_PREV` 삭제 + `pm2 restart targetup-backend --update-env`.
- **추가 과제 A-08(0926 배포 뒤 로그 · 기존 현상):** 기동 때마다 `❌ MySQL 연결 실패: connect ETIMEDOUT`가 찍힌다(09-25 06:41부터 모든 기동 · 2초 뒤 PostgreSQL 연결됨). `config/database.ts` MySQL 기동 검사가 재시도 없는 1회 시도라 기동 순간에만 실패하는 것으로, PostgreSQL 검사에 재시도를 넣은 것(0828)과 같은 부류. 오류 로그에 매번 ❌가 남아 진짜 장애와 섞인다.
