...
> 상태(0724 오후): **§5-1 DDL 적용 완료 / §5-2 잔액 표시 코드 완료 — Codex(gpt-5.6-sol) 4R ready to ship·배포 대기 / §8 중 8-1·8-3·8-4·8-5·8-9·8-10 실측 확정 / 강문희 메일 초안 제공 — 발송 Harold 확인 필요 / 8-2(143 SHOW TABLES) 미실행 / 다음 구현 = 선불 지정·단가 입력 UI(§5-3 전 필수) → §5-3 충전 실행**
...
- **★선불 지정 입력 수단 (Codex R1-6, §5-3 전 단계 필수)**: 발송ID 등록/수정 API·슈퍼관리자 UI가 billing_type·단가를 입력받지 않아 전부 postpaid로 생성됨 → 슈퍼관리자 발송ID 관리에 선불/후불·단가 편집 추가 전까지 잔액 블록은 어떤 회사에도 안 뜬다. 운영 PG 직접 UPDATE는 금지(feedback_no_manual_sql_update)라 **UI 경유가 유일 경로**.
...
## 6. 선행 관문 — 외주(강문희) 요청 ★미발송

**요청 내용 (한 줄):** 충전 수집 엔진의 **소스 DB를 143 → 62(우리 서버)로 변경**. 엔진 로직 무변경, 접속 대상만.

**근거(메일에 그대로 쓸 것):**
- 통계(정방향)는 이미 62로 전환 완료(0717 강문희 통지 + 0720 실측).
- 충전(역방향)만 미전환: 143 최신 `2026-07-23 14:38`, 62는 `2026-07-06`에서 정지(§2-2, §2-3).
- 143 폐기 목표 **7/31** — 이 전환이 없으면 폐기일에 충전이 끊긴다.

**이게 안 되면 §5-2~5-4는 만들어도 게이트웨이에 반영되지 않는다.**

---

## 7. 실행 순서

1. **강문희 메일 발송**(§6) — 최우선. 회신 대기 중에 2번 병행.
2. §8 미확인 항목 실측 확정(특히 8-1 RemAmt 조회 방법, 8-2 고아 ID 소속).
3. §5-1 원장 확장 설계 + DDL(사전 `information_schema` 검증) → Harold 컨펌 → 적용.
4. §5-2 잔액 표시(대시보드) → §5-3 충전 실행 → §5-4 충전 요청 → §5-5 대조 워커.
5. **왕복 실측 1건**(6원칙 ⑤): 62에 테스트 계정(P0001/R0001 또는 소액) 충전 1건 `N` 입력 → 1분 뒤 `Y`+`RsApplyDtTm` 확인 → 게이트웨이 잔액 반영 확인. **이게 통과해야 기능 오픈.**
6. §5-6 기존 선불 잔액 이관.
...
| 8-5 | ~~PayMethod·PayFkey 코드 체계~~ **★0724 분포·상관 확정**: `PayMethod='1'`(69건, 2020-12-23 종료) 중 68건 PayFkey 동반(유니크 8자리 = PG 거래키형) / `PayMethod='2'`(현행 6,957건 전부) PayFkey 전무 = 수동 충전형. **우리 INSERT = `PayMethod='2'` + PayFkey 비움**(현행과 일치). 최종 의미 확정 = 강문희 답신(메일 질문 1) | 분포 완료·의미 강문희 대기 |
| 8-6 | 충전 INSERT 시 `SeqNo` 채번 방식(auto_increment이나 143↔62 충돌 가능성) | 강문희 답신(메일 질문 2 — 엔진이 N 플래그 기준인지 SeqNo 워터마크인지) + 전환 후 실측 |
