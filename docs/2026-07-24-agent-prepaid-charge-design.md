...
**접수 원문 요지**: "업체에서 충전 '신청'한 경우만 이력에 등록되며, 계좌이체 후 슈퍼관리자가 충전할 경우에는 충전 내역이 남지 않는다. 관리자가 충전·차감한 내역이 남아야 한다."

**원인(코드 확정)**: 고객사가 보는 표는 `/manage` → 충전 요청 탭의 **요청 이력**이고, 소스가 `agent_charge_orders`(신청 원장) 하나였다(`routes/agent-charge-orders.ts` `GET /`). §5-3 직원 직접 충전은 게이트웨이 `RSRM_FillAmtHist`에만 INSERT하고 신청 행을 만들지 않는다(§5-4 불변 규칙 — 요청은 대기열일 뿐이라 실행이 요청을 역으로 만들지 않는다). 그래서 신청을 거치지 않은 충전·차감이 고객사 화면에서 통째로 빠졌다.

**전수 대조 — 결함은 한 곳뿐이었다.** "충전 이력"을 그리는 화면 4곳 중 나머지 3곳은 이미 관리자분을 포함한다: 슈퍼 에이전트 이력 = `RSRM_FillAmtHist` 전량(`admin.ts GET /agent-charges`) · 슈퍼 한줄로 잔액 변동 이력 = `admin_charge`/`admin_deduct` 포함 · 고객사 웹 잔액 이력 = 유형 필터 없으면 전량(`balance.ts GET /transactions`).

**설계**: 요청 이력을 고치지 않고 **충전 내역(게이트웨이 원장) 표를 따로 신설**했다. 요청 이력을 원장으로 바꿔 끼우면 접수 대기·반려가 사라진다 — 그 두 상태는 원장에 없다. 두 표는 서로 다른 질문에 답한다(내 신청이 어떻게 됐나 / 내 지갑이 어떻게 움직였나).

- 서버 `GET /api/agent-charge-orders/ledger` — 열람 범위는 **서버가 다시 조회한 소유 발송ID**로만 건다.
- 범위 조건은 CT `buildChargeFilterSql` 한 곳에서만 걸린다(목록·건수가 같은 WHERE를 써야 페이징이 안 어긋난다).
- 열람 대상 = 회사 소유 발송ID **전부**(`loadOwnedSendIds`). 요청 등록 자격(`loadEligibleSendIds` = `prepaid`만)과 **일부러 다른 함수**다 — 이력은 이미 일어난 사실이라 선불 지정이 바뀌었다고 과거 충전이 사라지면 안 되고, 한 함수로 묶으면 한쪽 조건을 고칠 때 다른 쪽이 같이 바뀐다.
- 표시 = 구분(충전/차감) 뱃지 + 부호 유지 금액 + 반영 상태(`RsApplyFlag='Y'`만 "반영 완료" — 등록 자체는 완료가 아니다).

**⛔ 이번 수정의 유일한 위험**: 범위 조건이 빠지면 **전 고객사 충전 내역이 한 화면에 열린다.** 그래서 빈 배열(소유 ID 0건)은 조건 생략이 아니라 `1 = 0`으로 고정했고, 그 한 줄을 계약 테스트 6건으로 묶었다(미지정=제한 없음 / 빈 배열=`1 = 0` / 공백만=`1 = 0` / IN 바인딩 / 단일ID와 AND / 기간과 병용 시 목록·건수 동일).

### 15-2. 에이전트 충전 시 현재 잔액 확인 (서수란 접수 · P2)

**접수 원문 요지**: "충전 시 발송ID를 선택하면 해당 계정의 현재 잔액을 확인하고 충전·차감할 수 있으면 좋겠다. 엔진에 반영된 잔액을 확인할 수 없어 차감(080 사전 차감·환불) 때마다 엔진 확인을 거쳐야 한다. 충전완료 후 잔액도 확인할 수 있으면 좋겠다. 발송ID 선택박스를 조금 줄이고 현재잔액 출력."

**현황**: 잔액 판정 규칙(§14 `pickLedgerBalances`)은 이미 있었으나, 그것을 감싼 `queryPayAgentBalances(companyId)`가 **회사 축 + 선불 한정**이라 슈퍼 충전 폼("지금 고른 이 발송ID의 잔액")에는 답할 수 없었다.

**설계**: 조회 코어를 `queryLedgerBalancesByIds(ids)`로 분리하고 회사 축 판은 그것을 호출하게 했다(원본 코드 이동 — 동작 무변경, 시그니처 무변경이라 소비처 2곳 영향 0). 슈퍼 전용 `GET /api/admin/agent-charges/balances?ids=…`(중복 제거·상한 50 = 폼 최대 행 수) 신설.

- 화면 = 폼 행마다 발송ID를 고르면 **금액 입력칸 왼쪽에** 현재 잔액. 선택박스는 그만큼 좁아진다(접수 요구 그대로).
- **폼 진입 시점 값을 재사용하지 않는다** — 잔액은 발송이 나가는 대로 깎여서, 고른 순간 다시 읽는다. 의존값을 선택 ID 문자열로 좁혀 금액 타이핑마다 조회하지 않는다.
- 반영 완료(`applied`) 확인 시 그 발송ID들의 잔액을 다시 읽어 "이번 등록 반영 상태" 줄에 붙인다 → "충전완료 후 잔액"도 같은 자리에서 닫힌다. 등록 후 폼이 비워지므로 폴러가 대상 ID를 들고 간다.
- 미확정은 숫자 대신 `잔액 확인 불가`. **0원으로 합성하지 않는다** — 있는 돈을 없다고 보여주면 불필요한 충전을 부른다(§14 C0130 전례).

**부수**: 충전 금액 표시 규칙을 프론트 CT `utils/agentLabel.ts` `formatAgentChargeAmount`로 단일화했다(부호 유지·버림 없음). 잔액(`formatAgentBalance`)은 과대 표시를 막으려 버리지만, 원장 금액은 통장·게이트웨이 원본과 대조해야 해서 넣은 값 그대로 보여야 한다. 슈퍼 이력·등록 결과 줄도 같은 함수로 맞췄다.

### 15-3. 범위 · 검증

- **DDL 0.** 신규 컬럼·테이블 0 — 기존 `RSRM_FillAmtHist`·`RSRM_SalesMst`·`company_agent_ids` 컬럼만 쓴다.
...
10. **★2026-08-30 보안 보강(승인 링크 공통 세션 · Codex 2R 종결)** — ①링크 승인 실행 즉시 수신 목록(ENV) **전원 통보 문자**(`notifyChargeApproved` · 남이 승인하면 그 자리에서 발각 = 유일한 탐지 레버 · 실패해도 승인 유효) ②감사 원장 기록 = `recordAuditLog('charge_link_approved')`(ip·ua 포함 · 시스템 관리 감사 화면 노출 · 라벨 = frontend audit-action-labels CT) ③폴백 서명 키 제거 = 미설정 시 서명 throw·검증 null(fail-closed · [BUGS B-0825-2](../status/BUGS.md) 종결) ④승인류 2경로 **각자** IP당 분 30 리미터(버킷 분리) + `/c` 분 600(app.ts) ⑤토큰 캡처 = `useApproveToken` 공용 훅 하나(fragment만 수용 · **query ?t= 거절** · 캡처 즉시 주소 제거 · hashchange 재캡처 · 무토큰 재진입은 빈 값) + 화면 재료를 토큰에 결합(`forToken` · 토큰 변경 즉시 loading 리셋 · done은 늦은 조회가 못 되돌림) = 같은 탭 A→B 재진입 때 A 화면으로 B를 승인하는 불일치 차단 ⑥페이지 하단 "입력을 요구하는 비슷한 화면은 가짜" 고지(모방 스미싱 구분선) ⑦대행 공용 `notifyManager`에 적재 건수 검사(부족 = notify_failed 이력 · 전량 미적재의 조용한 성공 차단). 리뷰 종결 = 1R 5건(수용 3·부분 1·불수용 1 = [B-0830-1](../status/BUGS.md)) · 2R 4건(수용 2·부분 1·기록 1 = [B-0830-2·B-0830-3](../status/BUGS.md)) · 라운드 상한 도달로 종료. **★0830 배포 완료**(같은 날 JWT_SECRET 운영 실측 36자 · B-0825-2 종결 아카이브 회전). 남은 실측 = 무통장 테스트 1건 링크 승인 → 전원 통보 문자 수신 + 감사 로그 "충전 링크 승인"(IP) 육안 · 대행발송은 다음 실접수 승인 때 담당자 전원 통보 확인.
