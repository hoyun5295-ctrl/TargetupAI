...
> **0. ⛔⛔⛔ 추측 절대 금지 — 가장 중요한 끌로드원칙 (D143, 2026-05-04, 정식 오픈 D-Day 1일 전 추가)**
>
> **사람들의 문자메시지를 발송하는 기간계 시스템이다. 추측으로 진단·수정하면 사용자 메시지를 망가뜨린다.**
>
> "왜 A는 정상이고 B만 문제냐?" 같은 차이 질문 / 의심점 / 가설은 **반드시 서버 SQL 쿼리 + 코드 grep으로 직접 검증한 결과로만 답한다.**
>
> **검증 절차 (이 순서 절대 위반 금지):**
> 1. 차이/원인을 만들 변수 후보 → `grep -rn` 전수 리스트업
> 2. 서버 SQL 쿼리로 실제 값 직접 비교 (Harold님이 실행할 명령어 안내)
> 3. 검증 결과(실제 데이터) 받은 후
> 4. 수정방안 제시
>
> **금지 표현:** "~인 것 같습니다", "~일 가능성이 높습니다", "아마 ~일 겁니다" 같은 추측 표현으로 수정방안 제시 절대 금지.
> **금지 행동:** 가설을 결론처럼 말하기 / 검증 1~3단계 없이 4단계 진입 / 같은 코드 다른 결과를 일반론으로 답하기.
> **같은 코드인데 사용자 A는 정상, B는 문제 → 100% 데이터/입력 차이.** 두 데이터 SQL 직접 비교 후에만 답한다.
>
> Harold님 원문: "씨발 사람들의 문자메세지를 발송하는데 추측따위 하지마 추측으로 보는건 절대 없어"
>
...
| CT-07 | standard-field-map.ts | 필드 매핑 + customer_field_definitions UPSERT + **(D142, 2026-04-28) `FIELD_DISPLAY_FORMAT_MAP` 22개 1:1 + `renderFieldValue(value, fieldKey)` 단일 진입점** — Harold님 원칙 "고정 22개 = 룰대로 / 커스텀 = 원본". custom_1~15 의도적 미등록 → 자동 String(원본). 자동 type 추론 1년 반복 사고 구조적 종결 |
...
| — | formatDate.ts (프론트) | calculateSmsBytes + truncateToSmsBytes + replaceMessageVars + formatPreviewValue + DIRECT_VAR_MAP/replaceDirectVars (D96) + mmsServerPathToUrl (D98) + resolveRecipientCallback (D99) + buildAdMessageFront (D102) + **isPhoneLikeValue/detectPhoneHeaders (D103)** + **(D109 추가)** `formatCampaignMessageForDisplay()` + `stripAdParts()` (D103 위반 데이터 정규화 + is_ad 기반 재부착) + `FRONT_FIELD_DISPLAY_MAP`/`reverseDisplayValueFront` (백엔드와 동기화) + `replaceVarsByFieldMeta()` + `replaceVarsBySampleCustomer()` (인라인 7곳 통합) + `mergeAndHighlightVars()` (변수 강조 ↔ 머지 결과 토글) + **(D142, 2026-04-28) `FRONT_DISPLAY_FORMAT_MAP` 22개 1:1 + `displayValue(value, fieldKey)` 단일 진입점** (백엔드 `renderFieldValue` 미러). formatByType/formatPreviewValue/replaceMessageVars/replaceVarsByFieldMeta/replaceVarsBySampleCustomer 5개 함수 모두 displayValue 호출로 통합 + 자동 type 추론 분기 fieldKey 있으면 발동 X (호출부 누락해도 안전한 String 원본) + `normalizePhoneKr` regex `\d{6,10}` → prefix별 `\d{7,8}` 정밀화 (1800 대표번호 0 추가 차단) |
...
| **컬럼 간 동일 값 혼용 → 네이밍 충돌** | `companies.status='active'`(회사 운영 활성)와 `companies.subscription_status='active'`(정식 구독) 두 다른 컬럼에 같은 값 사용 → 코드 읽을 때 어떤 용도인지 분간 불가, admin.ts가 plan_id 변경 시 무조건 'active'로 덮어써서 grant-trial 직후 체험 상태 파괴. 3중 버그 발생 | **용도 다르면 값도 달라야 함.** 구독 상태는 업계 표준 `'paid'`로 통일. CT-17 설계서 vs 실제 코드 불일치 정기 동기화. 오픈 D-Day 전 근본 정리가 최적 시점(데이터 마이그레이션 리스크 ↓) (D132) |
| **보조 상태값 의존 판정은 취약** | 프론트 `isOnTrial = subscription_status === 'trial'` 판정이 `admin.ts` 요금제 수정 등 여러 경로가 덮어쓸 수 있는 컬럼에 의존 → 'active'로 바뀌면 D-N/violet 카드 전부 미표시. `trial-downgrade-worker`도 `subscription_status='trial'` 조건 때문에 'active'로 오염된 체험은 영원히 강등 불가 (무제한 체험 버그) | **진실의 원천 기반 판정 원칙.** 여러 경로가 덮어쓸 수 있는 보조 상태(subscription_status)가 아니라, 명확한 소유자가 있는 단일 필드(plan_code — grant-trial/revoke-trial/Cron 3곳에서만 변경)로 판정해야 견고 (D132) |
| **UI 버그 → 프론트 문제로 속단** | `/pricing`에 D-N 안 보임 → 처음엔 프론트 캐시 의심했지만 실제는 admin.ts가 subscription_status를 'active'로 덮어쓰는 백엔드 경로 문제였음. 데이터 흐름(DB→API→프론트) 전수 추적 안 하면 진짜 범인 놓침 | **UI 버그 발생 시 반드시 DB→API→프론트 전 경로 확인.** psql로 DB 실제 값 + 백엔드 API 응답 + 프론트 조건 평가 3단계 체크. D-N 안 보임 사례: DB는 OK인데 admin.ts가 중간에 덮어쓰고 있었음 (D132) |
...
| **호출부 의존 fieldKey 가드 = 1년 반복 사고 패턴** | D104 → D136 P1 → D141 → D142 (0428 PDF #5). 매번 "fieldKey 가드 추가 + 호출부에서 전달" 누더기 패치 → 새 호출부 추가 시 또 누락 → 또 콤마 사고. **사람 기억 의존 = 100% 재발 보장.** 한 달 넘게 반복되어 Harold님 정당한 분노 | **호출부 의존 자체를 폐기 (D142 구조적 종결).** Harold님 정답: "고정 22개 = 룰대로 / 커스텀 = 원본". 컨트롤타워 자체 안전한 기본값 — `FIELD_DISPLAY_FORMAT_MAP`에 등록된 22개만 displayFormat / 매칭 실패(custom_*, 미지정) = `String(value)` 자동 반환. 호출부에서 fieldKey 누락해도 자동 추론 발동 X. 데이터 출처(backend custom_fields 평면화 4곳)에서 `String()` 박제로 안전망 1차 추가 (D142, 2026-04-28) |
| **자동발송 D-2 등록 시 48h 미만 풀백 무한리턴** | 자동발송 워커 1단계 윈도우 24~48h(D-2)인데, 사용자가 발송시각 24h 미만으로 등록하면 1단계 자체가 스킵 → AI 문안 생성 안 됨 → fallback도 없으면 풀백 메시지 무한 리턴. PDF 0428 #7 신고 | **D-1 시점으로 당김 + 4단계 → 3단계 단순화 (D142, 2026-04-28).** 1단계 윈도우 12~36h, 사전알림(`runPreNotification`) 단계 메인 루프 호출 제거. Harold님 정책: "하루전날 문안생성 + 스팸테스트 → D-day 2h전 스팸 재테스트 + 담당자 테스트발송 → D-day 발송" |
| **SyncAgent total_customers_synced 누적 합산** | sync.ts /sync 후 `total_customers_synced = total_customers_synced + $1` → 매 sync마다 1500건씩 누적. Agent state.ts도 `+= count` 누적. heartbeat가 누적값으로 덮어씀 → 슈퍼관리자 화면에 N×1500건으로 표시 (실제 DB 1,501건). PDF 0428 #9 신고 | **3곳 동시 수정 (D142, 2026-04-28):** ① sync.ts /sync 후 SET 스냅샷 `(SELECT COUNT(*) FROM customers WHERE company_id = $2 AND is_active = true)` ② heartbeat에서 total_customers_synced 갱신 안 함 ③ Agent state `+= count` → `= count` 직접 대입. **누적 합산 ≠ 실제 카운트** — 통계 컬럼은 누적/스냅샷 의미 명확히 분리 |
| **Windows 서비스 `start= auto` 부팅 직후 시작 실패** | sc create ... start= auto 설정 시 부팅 즉시 시작 → 네트워크(Tcpip/Dnscache)나 DB 미준비 상태에서 시작 시도 → 실패 → 자동실행 안 된 것처럼 보임. PDF 0428 #10 | **`start= delayed-auto` (Delayed Automatic Start) — 부팅 후 ~2분 지연 (D142, 2026-04-28).** 시스템 안정화 후 시작. 실패 시 자동 재시작 안전망(60s/60s/300s) 유지 |
| **IMC 이중 래핑 unwrap이 image API에 미적용** | D131에서 `createSender`/`createAlimtalkTemplate` 응답이 `{code,message,data:{data:{templateCode}}}` 이중 래핑 케이스 발견 → unwrap 추가했으나 **image upload API(`uploadSingleImage`/`uploadMultipleImages`)는 빠뜨림** → IMC 이미지 응답이 `data.data.imageUrl`로 한 단계 더 들어가 있을 때 frontend `data.imc.data.imageUrl` undefined → "카카오 응답에 이미지가 없습니다" 사고. 알림톡 PDF 0428 #1-1/#2 신고 | **모든 IMC API에 unwrap 일관 적용 (D142+ 알림톡, 2026-04-29).** `alimtalk-api.ts`의 image upload 함수 2개에도 동일 unwrap 로직 추가 + raw 응답 로그 강화. **신규 IMC API 추가 시 unwrap 패턴 자동 미러 의무화.** 일부 IMC API만 처리하면 다른 API에서 같은 사고가 시간차로 재발 |
| **체크박스 + URL 입력 UX는 한쪽만 충족 케이스 위험** | 알림톡 대표링크 — `representLinkEnabled && Mobile.trim()` 조건이라 직원이 체크박스 안 누르고 URL만 입력하면 페이로드에 미포함 → "대표링크가 IMC로 전달이 안 됩니다" 직원 신고 (D135 → D139에서 가드 강화했으나 PDF 0428에서 또 신고) | **URL 입력 자체가 enabled 시그널 (D142+ 알림톡, 2026-04-29).** Mobile URL 있으면 무조건 페이로드 포함 + URL onChange 시 `representLinkEnabled` 자동 ON. 사용자가 체크 안 누르고 URL만 입력해도 IMC 전달 보장. **체크박스+텍스트 입력 패턴은 둘 중 하나만 만족하는 케이스가 흔하므로 UX 단순화 필수** |
| **신규 기능 보고 시 backend 우선 grep 안 함** | 알림톡 검수요청 코멘트+증빙자료 신규 기능 보고 — 처음에 frontend UI + DB 컬럼 마이그레이션을 고려했으나 **backend `/inspect-with-file` 라우트가 D135부터 이미 존재**. backend grep 안 했으면 불필요한 DB 마이그레이션 + frontend 별도 저장 로직 추가 작업했을 것 (D142+ 알림톡, 2026-04-29) | **신규 기능/UI 보고 시 항상 backend 라우트 grep부터.** `/api/도메인/...` 패턴으로 backend `routes/` 검색 → 이미 있는 라우트는 frontend UI만 신설하면 즉시 동작. backend 우선 점검 = 작업 분량 70% 감소 |
| **🚨🚨🚨 tp-deploy-full 한 줄 의존 = vite emptyOutDir 빌드 실패 시 dist 비어있는 상태 종료 → 9시간 거래처 차단 (D145, 2026-05-07 새벽 critical 사고)** | **오픈 둘째날 새벽 critical 사고.** 5/6 18:26 tp-deploy-full 실행 → backend dist 빌드 완료 → 그 다음 frontend `npm run build` (vite) 단계 → vite는 `emptyOutDir: true`(기본)로 dist 비우고 빌드 시작 → **빌드 비정상 종료** → dist 비어있는 상태 그대로 → tp-deploy-full은 빌드 실패 감지 안 하고 정상 종료 → 5/6 18:54부터 nginx 403 `"directory index of dist/ is forbidden"` → **9시간 거래처 + Harold님 본인 사이트 차단**. 5/7 04:00 Harold님이 SSH 후 `npm run build` 한 줄로 dist 재생성하여 복구. 거래처 신용 + 매출 직격타 + 미팅 안절부절 + 새벽 4시까지 응대 + Harold님 3시간도 못 잠. 분노 표현 다수 정당 | **절대 원칙:** ① `tp-deploy-full` 한 줄 안내 X — 단계별 배포 (`git pull` → `frontend npm run build` → `ls -la dist/index.html` 시각 검증 → `backend npm run build` → `pm2 restart all`). ② **배포 후 즉시 `https://hanjul.ai` 접속 검증 안내** (Ctrl+F5). ③ **atomic deploy 패턴** — `vite build --outDir dist-new` 후 `index.html` 생성 시에만 `dist`로 swap (실패 시 옛 dist 그대로 유지 → 차단 0초). ④ dist/index.html 1분 간격 모니터링 cron + 부재 자동 알림. ⑤ 추측 진단 금지 — nginx error.log + npm logs + zgrep `.gz` 데이터 기반만. ⑥ `logrotate` 고려 안 한 단순 grep 금지 — `zgrep` + `error.log*` 패턴 사용 |
| **Harold님 의도 vs PDF 문구 충돌 시 Harold님 재확인 후 작업 (D145 P10 정정)** | PDF에 "슈퍼관리자에 고객 DB를 확인하고 삭제할 수 있는 기능이 없어도 될 것 같다" 적혀있어서 두 기능(DB 출력 + 삭제) 모두 제거. Harold님 진짜 의도는 **DB 출력만 제거 + 전체 삭제는 유지** → 정정 작업 추가 + Harold님 분노 추가 유발. 또한 P5 작업 시 CustomerDBModal에만 추가하고 ManageCustomersTab(고객사관리자 풀 페이지) 누락 → Harold님 화면 보고 다시 지적 받음 | **PDF 문구 모호하면 Harold님 의도 재확인 후 작업.** "둘 다 제거" 단정 X. 또한 동일 기능이 여러 화면에 분산되어 있을 때 한 곳만 적용 = 누락 사고 → grep으로 동일 기능 컴포넌트 전수 확인 필수 (CustomerDBModal + ManageCustomersTab + CustomersTab 모두 grep으로 확인) |
