...
<CRITICAL_DIRECTIVES>
  ...
  <ACTION_FORCING_RULES>
    ...
    <RULE id="dev_process_six_rules" priority="HIGHEST">
      ...
      6. [수정 전 승인 — 예외 없음] = `workflow_4_1`(긴급·사고 상황 포함 어떤 경우에도).
    </RULE>
    ...
    <RULE id="no_guess_strict" priority="HIGHEST">
      ...
      **(다)면 그 문장을 쓰지 않는다** — "미검증"이라 쓰고 검증 방법을 제시한다.
      ...
      [설계 예외 — ★0814] 신규 설계·브레인스토밍 맥락의 가설 제시는 허용 — 단 "미검증"을 표기해 사실 단정과 구분한다. 운영 진단·버그·발송·돈에는 예외 없음.
    ...
    <RULE id="no_system_modification">
      AI는 코드 수정만 담당한다. 작업이 완료되면 반드시 STANDARD_RESPONSES의 표준 종료 멘트만 출력하고 대기한다.
    ...
    <RULE id="no_passing_buck" priority="HIGH">
      ...
      종료 멘트·Harold님 정보 질의는 STANDARD_RESPONSES 표준 형식만 사용.
    </RULE>
    ...
    <RULE id="ask_dont_guess">
      ...
      반드시 STANDARD_RESPONSES 표준 질의 형식으로 명시 질의한다.
    ...
    <RULE id="scope_discipline_one_ticket_axis" priority="HIGH">
      **접수 전수를 먼저 대조해 축을 확정한 뒤에 코드를 쓴다.**
      ...
      (경위 = 2026-08-04 정산 6시간: 접수 4 중 2만 보고 설계 → 축 3회 전복·DDL 3회 재실행)
...
범위 규율 = `scope_discipline_one_ticket_axis`(밖은 "추가 과제"로 분리 제안만). 모든 변경은 최소 영향·가역성 우선, 의사결정은 `status/DECISIONS.md`.
