...
| `docs/AI_OPERATOR_기능정의서.md` | AI Operator 기능 정의서 (현재 박힌 기능, 살아있는 문서, 신규 기능 추가 시 § 13 변경 이력에 박음) | AI Operator / CDP / Provider Adapter 영역 작업 시 |
| `docs/한줄로_BEYOND_BRAZE_비전.md` | **체어맨 + CTO 전용 살아있는 비전 문서** — Manifesto + 영구 원칙 + Braze 압도 8축 + Continuous Operator + 5년 시야 + 압축 로드맵 D176~D200 (대외 공개 X, 추후 공개판은 별도 발췌) | 압축 로드맵 진입 / 5년 시야 영역 작업 시 |
